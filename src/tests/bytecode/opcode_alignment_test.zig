...
//!
//! 1. Every name in `quickjs/quickjs-opcode.h` exists in the generated `op`
//!    struct with the matching index.
//! 2. The QuickJS opcode header SHA-1 has not drifted from the locked
//!    `QUICKJS_BASELINE_SHA` recorded in `TRACKING.md`.
//!
//! When `quickjs-opcode.h` legitimately changes, regenerate
//! `src/engine/bytecode/opcodes_generated.zig` via
//! `python3 tools/generate_opcodes.py > src/engine/bytecode/opcodes_generated.zig`
//! and bump the SHA constant below to match the new file.
...
/// SHA-1 of `quickjs/quickjs-opcode.h` at QuickJS baseline
/// `64e64ebb1dd61505c256285a699c65c42941c5ed`. Bump in lockstep with
/// `QUICKJS_BASELINE_SHA` in `TRACKING.md`.
...
test "F2.5 representative opcode indices match QuickJS table" {
    ...
    // `quickjs/quickjs-opcode.h` line offsets and are the canonical baseline
    // values referenced in `PARSER_REWRITE_PLAN.md` §2.5.
