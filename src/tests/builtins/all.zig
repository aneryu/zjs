...
test "object keys values entries literal sameValue + function" {
    ...
    const literal_values = [_]core.JSValue{ core.JSValue.int32(10), core.JSValue.int32(20) };
    ...
    try std.testing.expect(builtins.object.sameValue(core.JSValue.float64(std.math.nan(f64)), core.JSValue.float64(std.math.nan(f64))));
    try std.testing.expect(!builtins.object.sameValue(core.JSValue.float64(0.0), core.JSValue.float64(-0.0)));
    ...
    try std.testing.expectError(error.TypeError, builtins.object.ownEntriesArray(rt, core.JSValue.int32(1), .keys));

    const this_value = core.JSValue.int32(7);
...
test "array construct join filter reduce some every indexOf includes at slice splice" {
    ...
    const array_values = [_]core.JSValue{ core.JSValue.int32(1), core.JSValue.int32(2), core.JSValue.int32(3), core.JSValue.int32(4) };
    ...
    const three_arg = [_]core.JSValue{core.JSValue.int32(3)};
    ...
    const at_last_arg = [_]core.JSValue{core.JSValue.int32(-1)};
    ...
    const slice_arg = [_]core.JSValue{core.JSValue.int32(-2)};
    ...
    const splice_values = [_]core.JSValue{ core.JSValue.int32(1), core.JSValue.int32(2), core.JSValue.int32(3) };
    const splice_target = try builtins.array.construct(rt, splice_values[0..]);
    defer splice_target.free(rt);
    const splice_args = [_]core.JSValue{ core.JSValue.int32(1), core.JSValue.int32(1), core.JSValue.int32(9), core.JSValue.int32(8) };
    ...
    try std.testing.expectError(error.TypeError, builtins.array.methodCall(rt, core.JSValue.int32(1), 1, &.{}));
...
test "string construct charAt substring toUpperCase indexOf includes trim fromCharCode" {
    ...
    try expectStringValue(rt, "W", try builtins.string.charAtValue(rt, string_object, core.JSValue.int32(6)));
    try expectStringValue(rt, "Hello", try builtins.string.methodCall(rt, string_object, 1, &.{ core.JSValue.int32(0), core.JSValue.int32(5) }));
    ...
    const string_search_args = [_]core.JSValue{string_search};
    ...
    const string_starts_args = [_]core.JSValue{string_starts};
    ...
    const number_string_object = try builtins.string.construct(rt, &.{core.JSValue.int32(123)});
    defer number_string_object.free(rt);
    try expectStringValue(rt, "123", try builtins.string.methodCall(rt, number_string_object, 9, &.{}));
    try expectStringValue(rt, "Hello", try builtins.string.fromCharCode(rt, &.{ core.JSValue.int32(72), core.JSValue.int32(101), core.JSValue.int32(108), core.JSValue.int32(108), core.JSValue.int32(111) }));
    var many_chars: [65]core.JSValue = undefined;
    for (&many_chars) |*slot| slot.* = core.JSValue.int32(65);
    ...
    const wide_from_char_code = try builtins.string.fromCharCode(rt, &.{core.JSValue.int32(0x0100)});
...
const IteratorNextOOMFixture = struct {
    ...
    fn deinit(self: IteratorNextOOMFixture, rt: *core.JSRuntime) void {
        self.iterator.free(rt);
        self.source.free(rt);
    }
};

fn createIteratorNextOOMFixture(rt: *core.JSRuntime, kind: IteratorNextOOMKind) !IteratorNextOOMFixture {
    return switch (kind) {
        .string => blk: {
        ...
        .collection => blk: {
            ...
            const item = try core.Object.create(rt, core.class.ids.object, null);
            defer item.value().free(rt);
            const set_result = try builtins.collection.methodCall(rt, source, 1, &.{ core.JSValue.int32(1), item.value() });
            set_result.free(rt);
...
fn createIteratorDoneOOMFixture(rt: *core.JSRuntime, kind: IteratorNextOOMKind) !IteratorNextOOMFixture {
    return switch (kind) {
        .string => blk: {
...
fn iteratorNextOOMCall(rt: *core.JSRuntime, kind: IteratorNextOOMKind, iterator: core.JSValue) !core.JSValue {
    return switch (kind) {
...
test "number parseFloat parseInt toFixed + boolean toString + symbol + bigint" {
    ...
    try std.testing.expectEqual(@as(f64, 0.0), try builtins.number.parseIntValue(rt, parse_input, core.JSValue.int32(10)));
    try std.testing.expect(std.math.isNan(try builtins.number.parseIntValue(rt, parse_input, core.JSValue.int32(1))));
    ...
    try std.testing.expect(std.math.isNan(try builtins.number.parseIntValue(rt, binary_int, core.JSValue.boolean(true))));
    ...
    try expectStringValue(rt, "1.10000", try builtins.number.toFixed(rt, core.JSValue.float64(1.1), &.{core.JSValue.int32(5)}));
    try expectStringValue(rt, "1.00", try builtins.number.toFixed(rt, core.JSValue.int32(1), &.{core.JSValue.int32(2)}));
...
test "math abs call + date construct methodCall staticCall + json" {
    ...
    const math_args = [_]core.JSValue{ core.JSValue.int32(9), core.JSValue.int32(3), core.JSValue.int32(6) };
    ...
    const undefined_arg = [_]core.JSValue{core.JSValue.undefinedValue()};
    try std.testing.expect(std.math.isNan(try builtins.math.call(1, &undefined_arg)));
    const null_arg = [_]core.JSValue{core.JSValue.nullValue()};
    try std.testing.expectEqual(@as(f64, 0.0), try builtins.math.call(1, &null_arg));
    const bool_arg = [_]core.JSValue{core.JSValue.boolean(true)};
    ...
    const date_utc_args = [_]core.JSValue{ core.JSValue.int32(2024), core.JSValue.int32(0), core.JSValue.int32(1) };
    try expectNumberValue(rt, 1704067200000, try builtins.date.staticCall(rt, 1, &date_utc_args));
    const date_utc_short_year_args = [_]core.JSValue{ core.JSValue.int32(99), core.JSValue.int32(0), core.JSValue.int32(1) };
    ...
    const date_parse_args = [_]core.JSValue{date_parse_input};
    ...
    const local_date_args = [_]core.JSValue{
        core.JSValue.int32(2024),
    ...
    const epoch_args = [_]core.JSValue{core.JSValue.int32(0)};
    ...
    const invalid_date_args = [_]core.JSValue{core.JSValue.float64(std.math.nan(f64))};
    ...
    try std.testing.expectError(error.TypeError, builtins.date.methodCall(rt, core.JSValue.int32(1), 1));

    const annex_date = try builtins.date.construct(rt, &.{ core.JSValue.int32(1970), core.JSValue.int32(0) });
    defer annex_date.free(rt);
    try expectNumberValue(rt, 70, try builtins.date.methodCall(rt, annex_date, 22));
    const annex_set_year = try builtins.date.methodCallArgs(rt, annex_date, 23, &.{core.JSValue.int32(50)});
...
test "uri encode decode escape unescape + regexp construct methodCall + promise + collections" {
    ...
    const non_ascii_uri_input = try builtins.string.fromCharCode(rt, &.{core.JSValue.int32(0x100)});
    ...
    const lone_surrogate = try builtins.string.fromCharCode(rt, &.{core.JSValue.int32(0xd800)});
    ...
    const annex_escape_input = try builtins.string.fromCharCode(rt, &.{
        core.JSValue.int32(0x21),
        core.JSValue.int32(0x2a),
        core.JSValue.int32(0x7f),
        core.JSValue.int32(0x80),
        core.JSValue.int32(0x100),
        core.JSValue.int32(0xd834),
        core.JSValue.int32(0xdf06),
    });
    ...
    try std.testing.expectError(error.TypeError, builtins.regexp.methodCall(rt, core.JSValue.int32(1), 1, null));
    ...
    const promise_ctx = try core.JSContext.create(rt);
    ...
    const promise_reject = try builtins.promise.staticCall(promise_ctx, 4, core.JSValue.int32(7));
    ...
    const map_set_args = [_]core.JSValue{ map_key, map_stored };
    ...
    const map_second_key = core.JSValue.int32(2);
    const map_second_value = core.JSValue.int32(22);
    const map_second_args = [_]core.JSValue{ map_second_key, map_second_value };
    ...
    const map_get_args = [_]core.JSValue{map_key};
    ...
    const set_add_args = [_]core.JSValue{core.JSValue.int32(1)};
    ...
    const set_second_add = try builtins.collection.methodCall(rt, collection_set, 6, &.{core.JSValue.int32(2)});
    ...
    const weakmap_set = try builtins.collection.methodCall(rt, collection_weakmap, 1, &.{ weakmap_key, core.JSValue.int32(44) });
    ...
    const weakmap_primitive_has = try builtins.collection.methodCall(rt, collection_weakmap, 3, &.{core.JSValue.int32(1)});
    defer weakmap_primitive_has.free(rt);
    try std.testing.expectEqual(@as(?bool, false), weakmap_primitive_has.asBool());
    try std.testing.expectError(error.TypeError, builtins.collection.methodCall(rt, collection_weakmap, 1, &.{ core.JSValue.int32(1), core.JSValue.int32(2) }));
    ...
    const weakmap_set_after_delete = try builtins.collection.methodCall(rt, collection_weakmap, 1, &.{ weakmap_key, core.JSValue.int32(45) });
    ...
    const weakset_args = [_]core.JSValue{weak_key};
    ...
    const weakset_primitive_has = try builtins.collection.methodCall(rt, collection_weakset, 3, &.{core.JSValue.int32(1)});
    ...
    const weakmap_set_after_sweep = try builtins.collection.methodCall(rt, collection_weakmap, 1, &.{ weakmap_key, core.JSValue.int32(46) });
    ...
    try std.testing.expectError(error.TypeError, builtins.collection.methodCall(rt, core.JSValue.int32(1), 3, set_add_args[0..]));
    ...
    try std.testing.expect(builtins.collection.sameValueZero(core.JSValue.int32(1), core.JSValue.int32(1)));
}
...
test "JSON stringify property list release preserves memory account" {
    ...
    try expectStringValue(rt, "{\"a\":1}", try builtins.json.stringify(rt, object.value(), replacer.value(), core.JSValue.undefinedValue()));
...
test "JSON stringify property list OOM releases pending atom" {
    ...
    while (fail_offset < 120) : (fail_offset += 1) {
        ...
        failing.fail_index = failing.alloc_index + fail_offset;
        const result = builtins.json.stringify(rt, object_value, replacer_value, core.JSValue.undefinedValue());
        failing.fail_index = std.math.maxInt(usize);
...
test "JSON parse stringify and rawJSON keep nested values under forced GC" {
    ...
    try expectStringValue(rt, "{\"a\":[1,\"x\",true],\"b\":{\"c\":2}}", try builtins.json.stringify(rt, parsed, core.JSValue.undefinedValue(), core.JSValue.undefinedValue()));
    ...
    try expectStringValue(rt, "{\"raw\":123}", try builtins.json.stringify(rt, wrapper_value, core.JSValue.undefinedValue(), core.JSValue.undefinedValue()));
}
...
fn expectPromiseCombinatorOOMCleanup(mode: u32) !void {
    ...
    while (fail_offset < 160) : (fail_offset += 1) {
        ...
        const item = if (mode == 6)
            try builtins.promise.rejectedWithPrototype(rt, core.JSValue.int32(13), null)
        else
            core.JSValue.int32(13);
...
fn cleanupPromiseCombinatorOOMIteration(rt: *core.JSRuntime, ctx: *core.JSContext, payload: core.JSValue, item: core.JSValue) void {
    if (ctx.hasException()) {
...
test "Map set rolls back inserted entry when size update fails" {
    ...
    const first_set = try builtins.collection.methodCall(rt, map_value, 1, &.{ core.JSValue.int32(1), core.JSValue.int32(11) });
    ...
    rt.setMemoryLimit(rt.memory.allocated_bytes);
    try std.testing.expectError(error.OutOfMemory, builtins.collection.methodCall(rt, map_value, 1, &.{ core.JSValue.int32(2), retained.value() }));
    rt.setMemoryLimit(null);
...
test "Map delete rolls back removed entry when size update fails" {
    ...
    const retained = try core.Object.create(rt, core.class.ids.object, null);
    defer retained.value().free(rt);
    const set_result = try builtins.collection.methodCall(rt, map_value, 1, &.{ core.JSValue.int32(1), retained.value() });
    set_result.free(rt);
    ...
    rt.setMemoryLimit(rt.memory.allocated_bytes);
    try std.testing.expectError(error.OutOfMemory, builtins.collection.methodCall(rt, map_value, 4, &.{core.JSValue.int32(1)}));
    rt.setMemoryLimit(null);

    const has_result = try builtins.collection.methodCall(rt, map_value, 3, &.{core.JSValue.int32(1)});
...
test "Map clear rolls back removed entries when size update fails" {
    ...
    while (fail_offset < 220) : (fail_offset += 1) {
        ...
        const retained = try core.Object.create(rt, core.class.ids.object, null);
        defer retained.value().free(rt);
        const first_set = try builtins.collection.methodCall(rt, map_value, 1, &.{ core.JSValue.int32(1), retained.value() });
        first_set.free(rt);
        const second_set = try builtins.collection.methodCall(rt, map_value, 1, &.{ core.JSValue.int32(2), core.JSValue.int32(22) });
        second_set.free(rt);
...
test "WeakMap set preserves entries when weak entry growth fails" {
    ...
    var keys: [5]core.JSValue = undefined;
    ...
    for (keys[0..4], 0..) |key, index| {
        const set_result = try builtins.collection.methodCall(rt, weakmap_value, 1, &.{ key, core.JSValue.int32(@intCast(index)) });
        set_result.free(rt);
    }
...
fn fillOwnPropertyStorage(rt: *core.JSRuntime, object: *core.Object) !void {
    var index: usize = 0;
...
fn expectStringValueContains(rt: *core.JSRuntime, expected: []const u8, value: core.JSValue) !void {
    defer value.free(rt);
    try std.testing.expect(value.isString());
    const header = value.refHeader().?;
    const string_value: *core.string.String = @fieldParentPtr("header", header);
...
fn expectSameStringValue(expected: core.JSValue, actual: core.JSValue) !void {
...
fn expectNumberValue(rt: *core.JSRuntime, expected: f64, value: core.JSValue) !void {
    defer value.free(rt);
    try std.testing.expectEqual(expected, numberValue(value).?);
}

fn expectObjectClass(value: core.JSValue, expected: core.ClassId) !void {
...
fn expectObjectPropertyClass(rt: *core.JSRuntime, object_value: core.JSValue, name: []const u8, expected: core.ClassId) !void {
...
fn expectIntProperty(rt: *core.JSRuntime, object_value: core.JSValue, name: []const u8, expected: i32) !void {
    try std.testing.expect(object_value.isObject());
...
fn expectNoOwnProperty(rt: *core.JSRuntime, object: *core.Object, name: []const u8) !void {
    const atom_id = try rt.internAtom(name);
    defer rt.atoms.free(atom_id);
    try std.testing.expect(!object.hasOwnProperty(atom_id));
}

fn expectIntIndex(rt: *core.JSRuntime, object_value: core.JSValue, index: u32, expected: i32) !void {
    try std.testing.expect(object_value.isObject());
    const header = object_value.refHeader().?;
    const object: *core.Object = @fieldParentPtr("header", header);
...
fn expectArrayLength(object_value: core.JSValue, expected: u32) !void {
    try std.testing.expect(object_value.isObject());
    const header = object_value.refHeader().?;
    const object: *core.Object = @fieldParentPtr("header", header);
...
test "promise buffers reflect proxy iterator and atomics helpers" {
    ...
    const array_buffer = try builtins.buffer.arrayBufferConstruct(js.runtime, core.JSValue.int32(8));
    ...
    const view_args = [_]core.JSValue{ array_buffer, core.JSValue.int32(1), core.JSValue.int32(4) };
    ...
    const set_u16_args = [_]core.JSValue{ core.JSValue.int32(0), core.JSValue.int32(0x1234), core.JSValue.boolean(true) };
    ...
    const get_u16_args = [_]core.JSValue{ core.JSValue.int32(0), core.JSValue.boolean(true) };
    ...
    const full_view_args = [_]core.JSValue{array_buffer};
    const full_view = try builtins.buffer.dataViewConstruct(js.runtime, full_view_args[0..], null);
    defer full_view.free(js.runtime);
    const set_big_args = [_]core.JSValue{ core.JSValue.int32(0), core.JSValue.shortBigInt(-1), core.JSValue.boolean(false) };
    const set_big = try builtins.buffer.dataViewSet(js.runtime, full_view, 9, set_big_args[0..]);
    defer set_big.free(js.runtime);
    const get_big_args = [_]core.JSValue{ core.JSValue.int32(0), core.JSValue.boolean(false) };
    ...
    const slice = try builtins.buffer.arrayBufferSlice(js.runtime, array_buffer, core.JSValue.int32(1), core.JSValue.int32(3));
    ...
    const out_of_bounds = [_]core.JSValue{core.JSValue.int32(8)};
