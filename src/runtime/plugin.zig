...
fn installSource(ctx: *core.JSContext, target_value: core.JSValue, path: []const u8, source: LoadedSource, options: InstallOptions) !void {
    ...
    // TGC S3 §4 class B: `atoms` is a native array, invisible to both the
    // value-root frames and the conservative scan. Root only the filled
    // prefix -- the tail is still `undefined` -- and extend it as ids land.
    var rooted_atoms: []core.Atom = atoms[0..0];
    var atom_roots = core.runtime.rootAtomList(&rooted_atoms);
    ...
    for (bindings, 0..) |binding_descriptor, index| {
        ...
        atoms[index] = atom_id;
        atom_count += 1;
        rooted_atoms = atoms[0..atom_count];
...
test "runtime Plugin installs synchronous bindings on an ordinary target" {
    ...
    const add_object = core.Object.fromHeader(add_value.refHeader().?);
    ...
    const default_length_object = core.Object.fromHeader(default_length_value.refHeader().?);
...
test "runtime Plugin defers last binding release until its trampoline returns" {
    ...
    _ = try InstalledBinding.call(@ptrCast(binding), .{
        .realm = ctx,
        .output = null,
        .func_obj = target,
...
test "runtime Plugin finalizer reentry mutates, allocates, and does not nest GC" {
    const State = struct {
        ...
        fn finalize(context: ?*anyopaque, object: ffi.OpaqueHostObject) callconv(.c) void {
            ...
            var counter = Counter{ .expected = payload_child.gcHeader() };
    ...
    const target = try core.Object.create(rt, core.class.ids.object, null);
    const target_value = target.value();
    var mutation_atom = try rt.internAtom("finalizerReentered");
    var payload_read_atom = try rt.internAtom("payloadRead");
    var payload_mutation_atom = try rt.internAtom("payloadMutated");
    // TGC S3-c: these ids live on the native `State` below and are used after
    // the forced major, so they need a declared root.
    var atom_roots = core.runtime.rootAtoms(.{ &mutation_atom, &payload_read_atom, &payload_mutation_atom });
    atom_roots.activate(rt);
    defer atom_roots.deactivate(rt);
    ...
    const payload_child_header = payload_child.gcHeader();
    ...
    var state = State{
        ...
        .payload_child = payload_child.value(),
    };
    State.active = &state;
    defer {
        State.active = null;
    ...
    _ = try exec.call.callValue(ctx.core, null, make_value, &.{});
...
test "runtime Plugin closes only after deferred wrapper callbacks release the class generation" {
    ...
    _ = try createOpaqueObjectValue(ctx, plugin, ffi.OpaqueHostObject.from(@ptrCast(&state), State.type_id));
    plugin.release();
