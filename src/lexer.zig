...
pub const Lexer = struct {
    ...
    inline fn releaseTokenPayload(self: *Lexer, tok: *t.Token) void {
        switch (tok.payload) {
            .str => |s| {
            ...
            // here. TGC S3-c: the token's id is
    ...
    pub fn nextIntoReplacing(self: *Lexer, out: *t.Token) Error!void {
    ...
    pub fn dupToken(self: *Lexer, tok: t.Token) Error!t.Token {
        var copy = tok;
        switch (tok.payload) {
            .ident => |ident| {
                var retained = ident;
    ...
    pub fn nextInto(self: *Lexer, out: *t.Token) Error!void {
        try self.skipTrivia();
        self.mark();
    ...
    pub fn nextTemplatePartInto(self: *Lexer, out: *t.Token) Error!void {
        self.mark();
    ...
    inline fn peekAt(self: *const Lexer, n: usize) u8 {
        return if (self.pos + n < self.source.len) self.source[self.pos + n] else 0;
    }

    inline fn remaining(self: *const Lexer) usize {
    ...
    pub fn simpleNextIsArrowNoLineTerminator(self: *const Lexer) ?bool {
        var pos = self.pos;
    ...
    pub fn simpleCurrentParenIsArrowHead(self: *const Lexer) ?bool {
    ...
    fn lexAsciiIdentifierNoEscape(self: *Lexer, out: *t.Token) Error!bool {
    ...
    fn lexPrivateName(self: *Lexer, out: *t.Token) Error!void {
    ...
    fn lexDotOrNumber(self: *Lexer, out: *t.Token) Error!void {
    ...
    fn finishNumber(self: *Lexer, out: *t.Token, start: usize, is_bigint: bool, base: u8) Error!void {
    ...
    fn lexString(self: *Lexer, out: *t.Token, quote: u8) Error!void {
    ...
    fn lexTemplate(self: *Lexer, out: *t.Token, phase: TemplatePhase) Error!void {
        return self.lexTemplateBody(out, phase, true);
    }

    fn lexTemplateBody(self: *Lexer, out: *t.Token, phase: TemplatePhase, expect_open_byte: bool) Error!void {
        if (expect_open_byte) {
    ...
    fn lexRegexp(self: *Lexer, out: *t.Token) Error!void {
        std.debug.assert(self.peek() == '/');
        self.bump(); // leading /
    ...
    fn lexPunctuator(self: *Lexer, out: *t.Token) Error!void {
        const c = self.peek();
        switch (c) {
    ...
    fn lexPlus(self: *Lexer, out: *t.Token) Error!void {
        self.bump();
        if (self.pos < self.source.len) {
            if (self.peek() == '+') {
                self.bump();
...
fn hexNibble(c: u8) u16 {
    return unicode.asciiHexDigitValueByte(c).?;
}
