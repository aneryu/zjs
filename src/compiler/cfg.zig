...
fn formRowIndex(op_id: u8, is_temp: bool) u16 {
    // The temp classification comes from the decode layer (form index
    // 300+), which since C0 closed also covers the lowered-direct
    // residents outside the temp range -- so the rebuild must share the
    // decode layer's mapping instead of re-deriving the temp arithmetic.
    return if (is_temp)
        opcode.decode.lowered_index_table[op_id]
    else
        op_id;
}
...
/// opcodes. Classification is declaration-derived via `opcode.decode`.
