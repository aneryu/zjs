...
/// (4 description) and method (5 [Symbol.toPrimitive]) live in symbol.zig.
const Tag = enum(u32) {
    number = 1,
    boolean = 2,
    bigint = 3,
...
/// function. Symbol's slice lives in symbol.zig. The `.primitive` domain has
/// no single owning class (it is shared across the five wrapper primitives),
/// so this file also carries the generic Number/BigInt/String prototype
/// toString/valueOf entries — their dispatch is the same shared exec op.
...
/// domain (number.zig), so only its valueOf appears here.
...
/// Shared record handler for the `.primitive` domain (used by both
/// boolean.zig and symbol.zig entries). Mirrors the retired `call.zig`
/// `.primitive` arm: it resolves the active realm global (call global, else
/// the function object's realm, else the context global) and delegates to the
/// exec op `qjsPrimitivePrototypeMethod`, which stays in exec because the VM's
/// prototype-method fast path (`call_runtime.callNativeBuiltinRecordForVm`)
/// also calls it (BOTH).
