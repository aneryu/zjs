...
fn isJumpOp(op_id: u8) bool {
    return op_id == opcode.op.if_false or op_id == opcode.op.if_true or op_id == opcode.op.goto;
}
...
fn computeLayout(ctx: *const Context, positions: []usize, sizes: []usize, use_short_opcodes: bool, initial_pc: usize) !usize {
    ...
    var pass: usize = 0;
    while (changed and pass < 8) : (pass += 1) {
        changed = false;
        ...
        while (pc < code.len) {
            ...
            const new_size: usize = if (op == opcode.op.label)
                ...
                loweredInstrSize(code, pc, use_short_opcodes);

            sizes[pc] = new_size;
            if (old_size != new_size) changed = true;
            out_pc += new_size;
            pc += getLoc0Loc1PairSize(code, pc, use_short_opcodes) orelse in_size;
        }
