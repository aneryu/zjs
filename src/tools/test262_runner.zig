...
pub const Config = struct {
    ...
    /// Disable the zjs-test262 batch protocol and force one subprocess per
    /// test. This is useful when debugging runner/engine isolation issues.
    no_batch: bool = false,
...
const BatchWorker = struct {
    ...
    fn init(io: std.Io, engine_path: []const u8) BatchWorker {
        return .{
            .io = io,
    ...
    fn deinit(self: *BatchWorker) void {
        self.close();
    }

    fn close(self: *BatchWorker) void {
        self.stdin_writer = null;
        if (self.child) |*child| {
    ...
    fn ensureStarted(self: *BatchWorker) !void {
        if (self.child != null and self.handled_count < batch_worker_restart_interval) return;
        self.close();
        const argv = [_][]const u8{ self.engine_path, "--test262-batch" };
        ...
        self.stdin_writer = child.stdin.?.writerStreaming(self.io, &self.write_buffer);
        self.child = child;
        self.handled_count = 0;
    }
    ...
    fn run(
        ...
        if (source.len > std.math.maxInt(u32) or test_path.len > std.math.maxInt(u16)) {
            return error.BatchUnsupported;
        }
        try self.ensureStarted();
        const mode: u8 = if (run_as_module) test262_protocol.mode_module else test262_protocol.mode_script;
        const flags: u8 = if (can_block) test262_protocol.request_flag_can_block else 0;
        const timeout: u32 = timeout_ms orelse 0;
        const header = test262_protocol.RequestHeader{
            .source_len = @intCast(source.len),
            .path_len = @intCast(test_path.len),
            .mode = mode,
            .flags = flags,
            .timeout_ms = timeout,
        };
        var stdin = &self.stdin_writer.?.interface;
        stdin.writeStruct(header, .little) catch return self.writeFailed();
        stdin.writeAll(test_path) catch return self.writeFailed();
        stdin.writeAll(source) catch return self.writeFailed();
        stdin.flush() catch return self.writeFailed();

        var response_bytes: [@sizeOf(test262_protocol.ResponseHeader)]u8 = undefined;
        self.readExact(response_bytes[0..], timeout_ms) catch |err| return self.readFailed(err, stderr_storage, stderr_out, timeout_ms);
        var response_reader = std.Io.Reader.fixed(response_bytes[0..]);
        const response = response_reader.takeStruct(test262_protocol.ResponseHeader, .little) catch return self.readFailed(error.BatchProtocolFailed, stderr_storage, stderr_out, timeout_ms);
        if (response.stderr_len > stderr_storage.len) return self.readFailed(error.BatchProtocolFailed, stderr_storage, stderr_out, timeout_ms);
        const stderr = stderr_storage[0..response.stderr_len];
        self.readExact(stderr, timeout_ms) catch |err| return self.readFailed(err, stderr_storage, stderr_out, timeout_ms);
        stderr_out.* = stderr;
        self.handled_count += 1;
        return switch (response.status) {
            test262_protocol.status_passed => true,
            test262_protocol.status_failed, test262_protocol.status_timeout => false,
            else => self.readFailed(error.BatchProtocolFailed, stderr_storage, stderr_out, timeout_ms),
        };
    }

    fn writeFailed(self: *BatchWorker) anyerror {
        self.close();
        return error.BatchProtocolFailed;
    }

    fn readExact(self: *BatchWorker, buffer: []u8, timeout_ms: ?u32) !void {
        const stdout = if (self.child) |*child|
            child.stdout orelse return error.BatchProtocolFailed
        else
            return error.BatchProtocolFailed;
        var offset: usize = 0;
        const timeout = batchReadTimeout(timeout_ms);
        while (offset < buffer.len) {
            var vec = [_][]u8{buffer[offset..]};
            const result = self.io.operateTimeout(.{ .file_read_streaming = .{
                .file = stdout,
                .data = &vec,
            } }, timeout) catch |err| switch (err) {
                error.Timeout => return error.BatchTimeout,
                else => return error.BatchProtocolFailed,
            };
            const n = result.file_read_streaming catch return error.BatchProtocolFailed;
            if (n == 0) return error.BatchProtocolFailed;
            offset += n;
        }
    }
    ...
    fn readFailed(
        ...
        self.close();
        if (err == error.BatchTimeout and timeout_ms != null) {
            // Parent-side batch read timeouts are worker/protocol failures,
            // not confirmed test failures. Let runOneTest rerun this case
            // through the one-process engine path, which has its own timeout.
            _ = stderr_storage;
            _ = stderr_out;
...
fn batchReadTimeout(timeout_ms: ?u32) std.Io.Timeout {
...
fn runSelectedTestsWithReporterMode(
    ...
    const use_batch_engine = use_external_engine and !config.no_batch and isDefaultZjsTest262Engine(engine_path);
    ...
    if (worker_count == 1) {
        ...
        while (spawned < worker_count) : (spawned += 1) {
            contexts[spawned] = .{
                ...
                .use_batch_engine = use_batch_engine,
...
fn runWorkerStride(
    ...
    var batch_worker = BatchWorker.init(io, engine_path);
    defer batch_worker.deinit();
    ...
    while (true) {
        ...
        const result = runOneTest(
            ...
            engine_path,
            use_external_engine,
            if (use_batch_engine) &batch_worker else null,
            &harness_cache,
            harness_prelude,
            test_path,
            index,
...
fn isDefaultZjsTest262Engine(engine_path: []const u8) bool {
    return std.mem.endsWith(u8, engine_path, "zjs-test262");
}
...
fn runOneTest(
    ...
    const exited_zero = if (batch_worker) |worker|
        worker.run(source, test_path, run_as_module, can_block, timeout_ms, stderr_storage, &stderr) catch
...
fn runExternalEngine(
    ...
    const use_zjs_test262_script_mode = !run_as_module and isDefaultZjsTest262Engine(engine_path);
    const argv_script = [_][]const u8{ engine_path, temp_path };
    const argv_script_can_block = [_][]const u8{ engine_path, "--can-block", temp_path };
    const argv_zjs_test262_script = [_][]const u8{ engine_path, "--test262-script", temp_path };
    const argv_zjs_test262_script_can_block = [_][]const u8{ engine_path, "--can-block", "--test262-script", temp_path };
    ...
    const result = std.process.run(allocator, io, .{
        ...
        else if (use_zjs_test262_script_mode)
            if (can_block) &argv_zjs_test262_script_can_block else &argv_zjs_test262_script
        else if (can_block)
...
test "test262 args parse batch disable flag" {
    const config = try parseArgs(&.{ "--no-batch", "-c", "test262.conf", "0", "20" });
    try std.testing.expect(config.no_batch);
    try std.testing.expectEqual(@as(?usize, 0), config.start_index);
    try std.testing.expectEqual(@as(?usize, 20), config.stop_index);
}
...
test "test262 batch read timeout is retriable" {
    var worker = BatchWorker.init(std.testing.io, "zig-out/bin/zjs-test262");
    var stderr_storage: [stderr_storage_len]u8 = undefined;
    var stderr: []const u8 = "";
    try std.testing.expectError(error.BatchTimeout, worker.readFailed(error.BatchTimeout, &stderr_storage, &stderr, 1));
}
...
test "test262 override path maps only manifest entries" {
    ...
    try std.testing.expect(test262Override("tests/zig-smoke/arith.js") == null);
}
