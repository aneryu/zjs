//! R3 roots diagnosis for the conservative scanner (`-Dzjs_gc_roots_diag=true`).
//!
...
//! reaches through it is TRANSITIVE. `gc_conservative.scanWords` publishes the
//! word it is resolving in `current_word`; the census keys each direct hit by
//! (interpreter function, header kind, object class, word source, pointer
//! shape). Everything here is compiled out unless `gc.roots_diag_enabled`.
...
const Metrics = conservative.Metrics;
const spillRegistersAndScan = conservative.spillRegistersAndScan;
const SpillImage = conservative.SpillImage;
const target_supported = conservative.target_supported;
...
/// The word the production scan loop is resolving right now.
pub threadlocal var current_word: DiagWord = .{};
...
// walk per scan is exact; `current_word.sp` doubles as the cache key.
...
/// `current_word.sp` of the scan `diag_frames` was built for; 0 = no table.
    ...
    fn currentFunction(rt: *const JSRuntime) struct { name: []const u8, native: bool } {
        if (rt.hot.current_backtrace_frame) |frame| {
            if (frame.resolver(frame.data, 0)) |snapshot| {
                if (snapshot.function_name != .empty) {
