...
test "array_list_erased append matches std ArrayList growth" {
    ...
    inline for (types) |T| {
        ...
        while (i < 64) : (i += 1) {
            ...
            try append(&erased, std.testing.allocator, item);
    ...
    try std.testing.expectError(
        error.OutOfMemory,
        append(&failing, std.testing.failing_allocator, 1),
    );
}
...
test "array_list_erased toOwnedSlice matches std ArrayList shrink-to-fit" {
    ...
    inline for (types) |T| {
        ...
        const empty_owned = try toOwnedSlice(&empty, std.testing.allocator);
        ...
        const erased_owned = try toOwnedSlice(&erased, std.testing.allocator);
