...
//! users retain one object-model namespace. `Object` is a 16-byte fixed head
...
pub fn destroyDetachedClassPayload(rt: *JSRuntime, class_id: class.ClassId, payload_kind: class.PayloadKind, payload: *class.Payload) void {
    ...
    switch (payload_kind) {
        .iterator => {
            ...
            typed.destroy(rt);
            rt.destroyNative(IteratorPayload, typed);
        },
        .collection => {
            const typed: *CollectionPayload = @ptrCast(@alignCast(ptr));
            typed.destroy(rt);
            rt.destroyNative(CollectionPayload, typed);
        },
        .finalization_registry => {
            const typed: *FinalizationRegistryPayload = @ptrCast(@alignCast(ptr));
            typed.destroy(rt);
            rt.destroyNative(FinalizationRegistryPayload, typed);
        },
        .realm_record => {
...
pub const ObjectFlags = packed struct(u32) {
    ...
    /// `gc_obj_type:7|mark:1` shape; this bit is object-only,
...
/// `__JS_AtomIsTaggedInt` bit test — while the ordinary
/// object add_property runs with no index probe at all.
...
pub const Object = extern struct {
    ...
    comptime {
        ...
        // owns no resident TraceHeader successor word.
        std.debug.assert(gc.bodyOffsetFromHeader(.object) == 0);
        // The fixed head retains qjs's resident property pointer. Only the
        // class arm is layout-dependent and trails this head.
        ...
        if (builtin.mode == .ReleaseFast or builtin.mode == .ReleaseSmall) {
            ...
            std.debug.assert(objectBodyBytes(class.ids.object, true) == 56);
            std.debug.assert(gc.metadata_prefix_size + objectBodyBytes(class.ids.object, true) == 64);
            std.debug.assert(gc.metadata_prefix_size + objectBodyBytes(class.ids.object, false) <= 48);
            std.debug.assert(gc.metadata_prefix_size + objectBodyBytes(class.ids.array, false) <= 64);
        } else {
    ...
    pub inline fn objectBodyBytes(class_id: class.ClassId, slots2_layout: bool) usize {
        return @sizeOf(Object) + objectTailBytes(class_id, slots2_layout);
    }
    ...
    pub inline fn bodyBytes(self: *const Object) usize {
        return self.accountedBodyBytesForPhysical(objectBodyBytes(self.class_id, self.hasSlots2Layout()));
    }

    /// Convert physical alloc/free size to lifetime-account size. The metadata
    /// route byte distinguishes standalone Objects; block Objects use the pure
    /// class rounding shared by credit and debit, never the cold block header.
    inline fn accountedBodyBytesForPhysical(self: *const Object, non_block_bytes: usize) usize {
        if (!gc.Registry.isBlockCellHeader(self.gcHeaderConst())) return non_block_bytes;
        return prospectiveAccountedBodyBytes(non_block_bytes);
    }
    ...
    pub inline fn fromHeader(header: *gc.Header) *Object {
        ...
        return @ptrFromInt(gc.bodyAddressFromHeader(.object, header));
    }

    pub inline fn fromHeaderConst(header: *const gc.Header) *const Object {
        if (comptime std.debug.runtime_safety) {
            std.debug.assert(header.metaConst().flags.kind == .object);
            std.debug.assert(gc.bodyOffsetFromHeader(.object) == 0);
        }
    ...
    pub inline fn objectTailBytes(class_id: class.ClassId, slots2_layout: bool) usize {
        if (slots2_layout) {
    ...
    fn prepareConstructionCharges(
        ...
        if (property_capacity != 0) {
            ...
            const with_prefix = std.math.add(usize, gc.metadata_prefix_size, bytes) catch
                return rt.prepareHeapCharge(std.math.maxInt(usize));
            total +|= gc_alloc.heapChargeForCarrier(with_prefix);
        }
        if (payload_bytes != 0) {
    ...
    inline fn allocCell(rt: *JSRuntime, class_id: class.ClassId, comptime has_trailing: bool) !*Object {
        return rt.gc.createObjectWithFamNoTrigger(
            Object,
    ...
    inline fn allocCellConst(
        ...
        return rt.gc.createObjectConstFamNoTrigger(
            Object,
    ...
    inline fn freeRawCellConst(
        ...
        rt.gc.destroyConstFam(Object, comptime objectTailBytes(class_id, has_trailing), self);
    }
    ...
    inline fn freeRawCell(rt: *JSRuntime, self: *Object, class_id: class.ClassId, comptime has_trailing: bool) void {
        rt.gc.destroyWithFam(Object, self, objectTailBytes(class_id, has_trailing));
    }

    /// The class-data region's base. Slots2 has no arm: its Entry[2] starts at
    /// this same offset and all payload access must use the sparse side table.
    ...
    /// (qjs `JSObject.u.opaque`). Valid only for the non-slots2 layout.
    ...
    // other GC carrier as well); every summary
    ...
    pub inline fn traceShapeSummary(self: *const Object) u8 {
        return self.gcHeaderConst().metaConst().lifetime.object_shape_summary & trace_shape_summary_storage_mask;
    }
    ...
    inline fn storeTraceShapeSummary(self: *Object, summary: u8) void {
        ...
        const state = &self.gcHeader().meta().lifetime;
    ...
    pub fn traceShapeSummaryMatches(self: *const Object) bool {
        const stored_summary = self.gcHeaderConst().metaConst().lifetime.object_shape_summary &
    ...
    pub fn createGeneratorShell(rt: *JSRuntime, class_id: class.ClassId) !*Object {
        ...
        try rt.gc.pins.prepareConstructionRoot(rt);
        ...
        // runs BEFORE the JSObject allocation. Complete the
    ...
    pub fn finishGeneratorShell(self: *Object, rt: *JSRuntime, prototype: ?*Object) !void {
        ...
        const final_shape = try rt.shapes.createObjectRoot(rt, prototype);
        ...
        rt.registerObjectWithBytes(self, self.bodyBytes()) catch |err| {
            ...
            rt.shapes.dropUnshared(rt, final_shape);
            return err;
        };
    ...
    /// retains that final shape and copies its traced value slots.
    ///
    ...
    pub fn createFromPropertyTemplate(rt: *JSRuntime, template: *const Object) !*Object {
        ...
        return createPreparedPropertyTemplate(rt, template, template.propertyEntries());
    }
    ...
    /// ctx, js_dup_shape(ctx->array_shape), JS_CLASS_ARRAY)` (quickjs.c,
    ...
    /// p->fast_array = 1; u.array = empty`, quickjs.c). The general
    ...
    pub fn createArrayFromInitialShape(rt: *JSRuntime, initial_shape: *shape.Shape) !*Object {
        ...
        initial_shape.markShared();

        // qjs allocates `prop[shape->prop_size]` for every object built from a
        // shape; the initial array shape's buffer is reused by
    ...
    /// `JS_NewObjectProtoClass(ctx, proto, JS_CLASS_OBJECT)` (quickjs.c,
    ...
    /// switch arm (quickjs.c — scalar flag init only, no payload, no
    ...
    pub fn createPlainObjectReserved2(rt: *JSRuntime, prototype: ?*Object) !*Object {
        ...
        const shape_ref = blk: {
            ...
            const ordinary_root = try rt.shapes.createObjectRootReserved(rt, prototype);
            if (ordinary_root.prop_size == trailing_property_capacity) {
                @branchHint(.likely);
                break :blk ordinary_root;
            }
            rt.shapes.dropUnshared(rt, ordinary_root);
    ...
    /// them wholesale into `JS_NewObjectFromShape` (quickjs.c,
    /// 16215-16232), whose props arm block-copies the cells (quickjs.c:
    /// The general `createInternal` path re-derives class layout
    ...
    pub fn createArgumentsFromShape(
        ...
        // js_dup_shape on entry; JS_NewObjectFromShape
        // consumes the Shape on every failure path.
        initial_shape.markShared();

        // qjs allocates `prop[shape->prop_size]`; later named
    ...
    fn createInternal(
        ...
        const alloc_size = if (inline_layout) |layout| layout.object_size else objectBodyBytes(class_id, false);
        ...
        const shape_ref = if (property_template) |template| blk: {
            ...
            template.shape_ref.markShared();
            break :blk template.shape_ref;
        } else if (property_capacity == 0)
            try rt.shapes.createObjectRootReserved(rt, prototype)
        else
        ...
        const accounted_alloc_size = if (inline_layout == null)
            prospectiveAccountedBodyBytes(alloc_size)
        else
            alloc_size;
        ...
        const self = if (inline_layout) |layout|
            @as(*Object, @ptrCast(@alignCast(try rt.gc.allocStandaloneObject(
            ...
            try allocCell(rt, class_id, false);
        var initialized = false;
        errdefer {
            if (initialized) {
                destroyFromHeader(rt, self.gcHeader());
            } else if (inline_layout) |layout| {
                rt.gc.freeStandaloneObject(self, layout.allocation_size, layout.allocation_alignment, layout.object_offset);
            } else {
                freeRawCell(rt, self, class_id, false);
            }
        }
    ...
    /// owns teardown). Mirrors the per-arm `errdefer rt.destroyNative( T, ...)`
    ...
    noinline fn freeClassPayloadAllocation(rt: *JSRuntime, payload: class.Payload, payload_kind: class.PayloadKind) void {
        const ptr = payload orelse return;
        switch (payload_kind) {
            .iterator => rt.destroyNative(IteratorPayload, @ptrCast(@alignCast(ptr))),
            .collection => rt.destroyNative(CollectionPayload, @ptrCast(@alignCast(ptr))),
            .buffer => rt.destroyNative(BufferPayload, @ptrCast(@alignCast(ptr))),
            .typed_array => rt.destroyNative(TypedArrayPayload, @ptrCast(@alignCast(ptr))),
            .regexp => {},
            .bound_function => {},
            .proxy => {},
            .arguments => {},
            .object_data => {},
            .weak_ref => rt.destroyNative(WeakRefPayload, @ptrCast(@alignCast(ptr))),
            .var_ref => {},
            .promise => {},
            .generator => {
                const typed: *GeneratorPayload = @ptrCast(@alignCast(ptr));
    ...
    /// (`binding.zig`), which never select slots2 or a wide union arm. Their
    /// payload trails the resident property pointer in the fixed head and the
    /// narrow class arm; neither may alias the inline payload. Asserted at every
    /// consumer that also holds the class id.
    ...
    /// quickjs.c); only inline-payload classes rederive the aligned
    ...
    inline fn freeObjectAllocation(rt: *JSRuntime, self: *Object, definition: class.Table.DefinitionPlan) void {
        ...
        if (self.hasSlots2Layout()) {
            ...
            return rt.gc.destroyConstFam(Object, comptime objectTailBytes(class.ids.object, true), self);
        }
        if (self.class_id == class.ids.object) {
            return rt.gc.destroyConstFam(Object, comptime objectTailBytes(class.ids.object, false), self);
        }
        rt.gc.destroyWithFam(Object, self, objectTailBytes(self.class_id, false));
    }
    ...
    pub fn allocationSize(self: *const Object, rt: *const JSRuntime) usize {
        const physical_bytes = if (inlineClassPayloadLayout(rt.classes.recordPtr(self.class_id))) |layout| blk: {
            std.debug.assert(unionArmBytes(self.class_id) == union_arm_min_bytes);
            break :blk layout.object_size;
        } else @sizeOf(Object) + objectTailBytes(self.class_id, self.hasSlots2Layout());
        return self.accountedBodyBytesForPhysical(physical_bytes);
    }
    ...
    pub fn setCachedFunctionProto(self: *Object, rt: *JSRuntime, prototype: ?*Object) !void {
        ...
        // trace and must not be remembered.
        if (prototype) |stored| {
            if (ctx.header.metaConst().alloc_info.heap_accounted) rt.gc.generationalBarrier(&ctx.header, stored.gcHeader());
        }
    }
    ...
    /// Store an owned value into the realm's `cached_values[slot]`. The
    /// REALM is the owner of that slot, so the generational barrier must
    /// remember the realm header: remembering `self` (the global object)
    ...
    noinline fn destroyFromHeaderSlow(rt: *JSRuntime, header: *gc.Header) void {
        ...
        // (qjs `js_rc(p)->mark = 1` before free, quickjs.c).
        ...
        const definition = rt.classes.destructionPlan(destroying_class_id).?;
        // qjs free_object never derives a layout at teardown: the malloc block
        // header already carries what the free needs.
        ...
        const alloc_size = if (has_inline_payload)
            ...
            @sizeOf(Object) + objectTailBytes(destroying_class_id, self.hasSlots2Layout());
    ...
    fn finalizeClassPayload(self: *Object, rt: *JSRuntime, generation: u64, inline_payload: bool) void {
        ...
        var remaining_payload = payload_slot.*;
        payload_slot.* = null;
        self.flags.class_payload_kind = .none;
    ...
    fn clearWeakIdentities(
        ...
        if (self.collectionPayload()) |payload| {
            const old_len = payload.weak_entries.items.len;
    ...
    // ===== collection* / weak* =====
    pub fn collectionEntriesSlot(self: *Object) *std.ArrayListUnmanaged(CollectionEntry) {
        return &payloadPresent(self.collectionPayload()).entries;
    }
    ...
    pub fn collectionActiveCountSlot(self: *Object) *usize {
        return &payloadPresent(self.collectionPayload()).active_count;
    }
    ...
    /// qjs-shaped growth (start at 8, then double) kept exactly: the
    /// allocation sizes are part of the Map/Set memory profile.
    pub fn ensureCollectionEntryCapacity(self: *Object, rt: *JSRuntime, min_capacity: usize) !void {
        const entries = self.collectionEntriesSlot();
        if (entries.capacity >= min_capacity) return;
    ...
    pub fn appendCollectionEntryUnindexed(self: *Object, rt: *JSRuntime, entry: CollectionEntry) !usize {
        ...
        rt.gc.generationalBarrier(self.gcHeader(), entry.key.cycleMarkHeader());
        rt.gc.generationalBarrier(self.gcHeader(), entry.value.cycleMarkHeader());
        return index;
    }
    ...
    pub fn clearCollectionIndex(self: *Object, rt: *JSRuntime) void {
        ...
        heads.* = &.{};
        if (old_heads.len != 0) rt.freeNative(usize, old_heads);
    }

    pub fn weakCollectionEntriesSlot(self: *Object) *std.ArrayListUnmanaged(WeakCollectionEntry) {
        return &payloadPresent(self.collectionPayload()).weak_entries;
    }
    ...
    pub fn ensureWeakCollectionEntryCapacity(self: *Object, rt: *JSRuntime, min_capacity: usize) !void {
        const entries = self.weakCollectionEntriesSlot();
        if (entries.capacity >= min_capacity) return;
        var next_capacity = if (entries.capacity != 0) entries.capacity else entries.items.len;
    ...
    pub fn appendDisposableResource(
        ...
        // both values.
        rt.gc.generationalBarrier(self.gcHeader(), resource_value.cycleMarkHeader());
        rt.gc.generationalBarrier(self.gcHeader(), method.cycleMarkHeader());
    }
    ...
    pub fn setOptionalValueSlot(self: *Object, rt: *JSRuntime, slot: *?JSValue, next_value: ?JSValue) !void {
        ...
        // builds.
        if (next_value) |stored| rt.gc.generationalBarrier(self.gcHeader(), stored.cycleMarkHeader());
    }

    pub fn clearOptionalValueSlot(self: *Object, _: *JSRuntime, slot: *?JSValue) void {
        _ = self;
        slot.* = null;
    }
    ...
    /// shape property (`ctx->regexp_shape`; quickjs.c).
    ...
    pub fn initializeRegExpLastIndex(self: *Object, rt: *JSRuntime) !void {
        ...
        try self.appendPreparedPropertyEntry(
            rt,
            atom.ids.lastIndex,
            property.Flags.data(.{ .writable = true }),
    ...
    pub fn clearRegexpCompiledBytecode(self: *Object, _: *JSRuntime) void {
    ...
    pub fn setRegexpCompiledBytecodeString(self: *Object, rt: *JSRuntime, bytecode: *string.String) !void {
        ...
        if (self.regExpPayload()) |payload| {
            ...
            rt.gc.generationalBarrierValue(self.gcHeader(), bytecode.value());
        } else {
            std.debug.assert(self.flags.class_payload_kind == .regexp);
            unreachable;
        }
    ...
    /// `build_arg_list`'s JS_CLASS_ARGUMENTS arm, which
    ...
    /// arm. qjs guards that arm with `p->fast_array`,
    ...
    /// JS_CLASS_MAPPED_ARGUMENTS arm, which sits beside
    ...
    /// redefined (convert_fast_array_to_array, quickjs.c); zjs records the
    ...
    /// JS_GetPropertyValue JS_CLASS_MAPPED_ARGUMENTS, quickjs.c).
    ...
    /// qjs JS_GetPropertyValue mapped arm:
    ...
    fn ensureArrayBufferCapacity(self: *Object, rt: *JSRuntime, needed_len: usize) !void {
        ...
        // Mirror QuickJS expand_fast_array:
    ...
    pub fn promiseResultSlot(self: *Object) *?JSValue {
        return &payloadPresent(self.promisePayload()).result;
    }
    ...
    pub fn promiseReactionsSlot(self: *Object) *[]JSValue {
        return &payloadPresent(self.promisePayload()).reactions;
    }
    ...
    pub fn promiseReactionsCapacitySlot(self: *Object) *usize {
        return &payloadPresent(self.promisePayload()).reactions_capacity;
    }

    pub fn promiseIsRejectedSlot(self: *Object) *bool {
        return &payloadPresent(self.promisePayload()).is_rejected;
    }
    ...
    pub fn setGeneratorThis(self: *Object, next_value: JSValue) void {
        self.generatorThisSlot().* = next_value;
    }
    ...
    pub fn setGeneratorCurrentFunction(self: *Object, next_value: JSValue) void {
        self.generatorLiveExecution().current_function = next_value;
    }
    ...
    /// The engine-internal host-callable id this native function dispatches
    /// through (`core.host_function.ids`), or null for every other function.
    pub fn hostFunctionKind(self: *const Object) ?i32 {
        if (class.isBytecodeFunctionClass(self.class_id)) return null;
        const payload = self.functionPayloadConst() orelse return null;
        return if (payload.native.host_function_kind != 0) payload.native.host_function_kind else null;
    }
    ...
    /// `JSObject.u.cfunc` (`c_function` and `realm`; quickjs.c).
    ...
    pub fn setFunctionBytecodeValue(self: *Object, rt: *JSRuntime, next_value: JSValue) !void {
        ...
        rt.gc.generationalBarrier(self.gcHeader(), &fb.header);
    }
    ...
    /// qjs `js_closure2`: `js_mallocz` the capture
    ...
    /// changes nothing.
    pub fn clearModuleImportCaptureSlot(self: *Object, index: usize) !void {
        if (!class.isBytecodeFunctionClass(self.class_id)) return error.InvalidBytecode;
        const slots = self.bytecodeArm().*.captureSlots();
        if (index >= slots.len) return error.InvalidBytecode;
    ...
    pub fn setCallSiteMetadata(
        ...
        const next_file = file;
        const next_function = function_name;
        ...
        // `function_name` may be the callee Function object, not just a name
        // string, and the call-site object can already be old by the time its
    ...
    pub fn callSiteFile(self: *const Object) ?JSValue {
        return (self.ordinaryPayloadConst() orelse return null).callsite_file;
    }

    pub fn callSiteFunctionName(self: *const Object) ?JSValue {
        return (self.ordinaryPayloadConst() orelse return null).callsite_function;
    }
    ...
    pub fn errorStack(self: *const Object) ?JSValue {
        return (self.ordinaryPayloadConst() orelse return null).error_stack;
    }

    pub fn setErrorStackSites(self: *Object, rt: *JSRuntime, sites_value: JSValue) !void {
        const payload = try self.ensureOrdinaryPayload(rt);
        const next_value = sites_value;
    ...
    pub fn errorStackSites(self: *const Object) ?JSValue {
        return (self.ordinaryPayloadConst() orelse return null).error_stack_sites;
    }
    ...
    pub fn promiseCapabilityResolve(self: *const Object) ?JSValue {
        return (self.ordinaryPayloadConst() orelse return null).promise_capability_resolve;
    }
    ...
    pub fn promiseCapabilityReject(self: *const Object) ?JSValue {
        return (self.ordinaryPayloadConst() orelse return null).promise_capability_reject;
    }
    ...
    pub fn setPromiseCapability(self: *Object, rt: *JSRuntime, next_resolve: ?JSValue, next_reject: ?JSValue) !void {
        ...
        // The single-slot setters above go through `setOptionalValueSlot`,
        // which barriers; this paired store bypassed it. A capability record
        // that survived a minor before its resolve/reject functions were
        // created is exactly the old-to-young edge the S0 gc-stress gate
        // reported (MINOR-AUDIT: ordinary owner -> condemned young function).
        if (next_resolve) |stored| rt.gc.generationalBarrier(self.gcHeader(), stored.cycleMarkHeader());
        if (next_reject) |stored| rt.gc.generationalBarrier(self.gcHeader(), stored.cycleMarkHeader());
    }
    ...
    pub fn promiseCombinatorResolve(self: *const Object) ?JSValue {
        return (self.ordinaryPayloadConst() orelse return null).promise_combinator_resolve;
    }
    ...
    pub fn promiseCombinatorReject(self: *const Object) ?JSValue {
        return (self.ordinaryPayloadConst() orelse return null).promise_combinator_reject;
    }
    ...
    pub fn promiseCombinatorValues(self: *const Object) ?JSValue {
        return (self.ordinaryPayloadConst() orelse return null).promise_combinator_values;
    }
    ...
    pub fn promiseCombinatorKeys(self: *const Object) ?JSValue {
        return (self.ordinaryPayloadConst() orelse return null).promise_combinator_keys;
    }
    ...
    /// p->u.cfunc.realm`. C_FUNCTION_DATA deliberately
    ...
    /// Pointee type of the class payload arm for `kind` (the kinds whose
    /// payload is a plain `payloadArm()` allocation).
    fn PayloadStorage(comptime kind: class.PayloadKind) type {
        return switch (kind) {
            ...
            .typed_array => TypedArrayPayload,
            .bound_function => BoundFunctionPayload,
            .proxy => ProxyPayload,
            .arguments => ArgumentsPayload,
            .object_data => ObjectDataPayload,
            .var_ref => VarRefPayload,
            .promise => PromisePayload,
            .generator => GeneratorPayload,
            else => @compileError("payload kind " ++ @tagName(kind) ++ " has a hand-written fetcher"),
        };
    }

    fn PayloadPtr(comptime Self: type, comptime kind: class.PayloadKind) type {
        return if (@typeInfo(Self).pointer.is_const) *const PayloadStorage(kind) else *PayloadStorage(kind);
    }

    /// This object's payload as `kind`, or null when it carries another kind
    /// (or no payload yet). Const-ness follows `self`.
    fn payloadOf(self: anytype, comptime kind: class.PayloadKind) ?PayloadPtr(@TypeOf(self), kind) {
        if (self.flags.class_payload_kind != kind) return null;
        const ptr = self.payloadArm().* orelse return null;
        return @ptrCast(@alignCast(ptr));
    }

    /// Unwrap a payload fetch whose kind the caller has already established;
    /// the slot accessors below are only called on objects of their kind.
    inline fn payloadPresent(payload: anytype) @TypeOf(payload.?) {
        return payload.?;
    }
    ...
    fn markClassPayload(self: *Object, rt: *JSRuntime, visitor: *class.PayloadVisitor) bool {
        ...
        // Resident arm (base contract): admission is the non-null arm, not the
        // built-in discriminator -- embedding/dynamic classes keep
    ...
    pub fn isDetachedGeneratorShellForGc(self: *const Object) bool {
        return (self.class_id == class.ids.generator or self.class_id == class.ids.async_generator) and
            ...
            !self.gcHeaderConst().metaConst().alloc_info.heap_accounted;
    }
    ...
    pub fn traceDetachedGeneratorShellEdges(self: *Object, visitor: anytype) !void {
        std.debug.assert(self.class_id == class.ids.generator or self.class_id == class.ids.async_generator);
        std.debug.assert(self.flags.class_payload_kind == .generator);
        std.debug.assert(!self.gcHeaderConst().metaConst().alloc_info.heap_accounted);
    ...
    inline fn tracePropertyEntriesFallible(
        ...
        for (self.propertyStorageEntries(count), 0..) |*entry, index| {
            ...
            if (slot_flags.kind == .data) {
                try gc_visit.value(visitor, &entry.slot.data);
                continue;
            }
    ...
    pub inline fn traceChildEdgesFallible(self: *Object, rt: *JSRuntime, visitor: anytype) !void {
        try gc_visit.shape(visitor, self.shape_ref);
        ...
        // predicate the property-storage audit uses.
        ...
        // A dense Array's edges are exactly shape + properties + elements + the
        // iterator-next cache visited above; it cannot own any of the
        ...
        if (class.isBytecodeFunctionClass(self.class_id)) {
            // qjs js_bytecode_function_mark, the class
    ...
    pub fn setPrototype(self: *Object, rt: *JSRuntime, prototype: ?*Object) Error!void {
        ...
        _ = rt.shapes.replacePrototypeAssumePrepared(self.shape_ref, prototype);
    ...
    pub fn setFreshObjectPrototype(self: *Object, rt: *JSRuntime, prototype: ?*Object) Error!void {
        ...
        const replacement = try rt.shapes.createObjectRoot(rt, prototype);
        const previous = self.shape_ref;
        self.shape_ref = replacement;
        self.refreshTraceShapeSummary();
        rt.gc.generationalBarrier(self.gcHeader(), &replacement.header);
        rt.shapes.dropUnshared(rt, previous);
        self.flags.is_std_array_prototype = false;
    }
    ...
    pub fn getOwnProperty(self: *const Object, rt: *JSRuntime, atom_id: atom.Atom) PropertyReadError!?descriptor.Descriptor {
        ...
        if (self.isArray() and atom_id == atom.ids.length) {
            return descriptor.Descriptor.data(arrayLengthValue(self.arrayLength()), .{ .writable = self.flags.length_writable });
        }
        if (self.mappedArgumentsBindingIndexFromAtom(rt, atom_id)) |mapped_index| {
            ...
            if (self.findProperty(atom_id)) |property_index| {
                ...
                if (!flags.deleted and flags.kind == .data) {
                    return descriptor.Descriptor.data(mapped_value, .{ .writable = flags.writable, .enumerable = flags.enumerable, .configurable = flags.configurable });
                }
            }
            return descriptor.Descriptor.data(mapped_value, .all);
        }
    ...
    /// qjs `JS_GetOwnPropertyInternal` (quickjs.c else-branch). It walks
    ...
    /// ("nothing to do", quickjs.c). It throws `ReferenceError` for an
    /// uninitialized VARREF / module-namespace binding, matching qjs
    /// quickjs.c. Exotic numeric indices (fast/dense arrays) and
    ...
    pub fn existsOwnProperty(self: *const Object, rt: *JSRuntime, atom_id: atom.Atom) !bool {
        ...
        // builds a full descriptor, but for the non-test class set this hook is
        // never installed (see `exoticMethodsForClassId`) so the cost is paid
        // only by the exotic classes that genuinely need it.
        if (self.exoticMethods(rt)) |methods| {
            if (methods.get_own_property) |hook| {
                if (hook(@constCast(self), atom_id) != null) return true;
            }
        }
    ...
    pub fn getProperty(self: *const Object, atom_id: atom.Atom) PropertyReadError!JSValue {
        ...
        if (self.denseArrayElement(atom_id)) |stored| return stored;
    ...
    fn commitAutoInitValue(self: *Object, rt: *JSRuntime, index: usize, materialized: JSValue) !JSValue {
        ...
        if (!self.isGlobal()) {
            ...
            rt.gc.generationalBarrier(self.gcHeader(), materialized.cycleMarkHeader());
    ...
    fn commitAutoInitVarRef(self: *Object, rt: *JSRuntime, index: usize, cell: *var_ref_mod.VarRef) JSValue {
        const old_flags = self.propFlagsAt(index);
        self.propertyEntry(index).*.slot = .{ .var_ref = cell };
        rt.gc.generationalBarrier(self.gcHeader(), &cell.header);
        self.updateShapePropertyFlags(rt, index, old_flags.withKind(.var_ref));
    ...
    fn materializeAutoInitEntryForMutation(self: *Object, index: usize) !void {
        ...
        _ = self.propertyEntry(index).*.slot.auto_init.realm_and_id.realmHeader() orelse return error.InvalidBuiltinRegistry;
        _ = try materializeAutoInit(self, index);
    }
    ...
    fn materializePropAutoInit(realm: *context_mod.RealmContext, info: *const property.AutoInit) PropertyReadError!JSValue {
        if (info.materialize_host) |materialize| return materialize(&realm.header);
    ...
    /// `js_instantiate_prototype`, quickjs.c). `self` is the owner
    ...
    /// it).
    pub fn getOwnConstructorPrototypeObject(self: *Object, _: *JSRuntime) !?*Object {
        if (self.hasExoticMethods()) return null;
        ...
        switch (flags.kind) {
            .data => {},
            .auto_init => {
                _ = try self.materializeAutoInit(index);
            },
            else => return null,
        }
        const stored = self.asDataAt(index) orelse return null;
    ...
    pub fn getOwnDataPropertyLookup(self: *const Object, atom_id: atom.Atom) ?DataPropertyLookup {
        ...
        if (self.findProperty(atom_id)) |index| {
            const stored = self.asDataAt(index) orelse return null;
            return .{ .index = index, .value = stored };
        }
        return null;
    }
    ...
    pub fn defineOwnProperty(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, desc: descriptor.Descriptor) !void {
        ...
        if (try self.defineModuleNamespaceProperty(atom_id, desc)) return;
        var actual_desc = desc;
        try self.prepareMappedArgumentsDescriptorForDefine(rt, atom_id, &actual_desc);
    ...
    pub fn defineOwnDataPropertyAssumingNewFromRootedAtom(
        ...
        try self.appendPreparedPropertyEntryImpl(
            ...
            rt,
            atom_id,
            comptime property.Flags.data(.all),
            .{ .data = data_value },
        );
    }
    ...
    pub fn defineJsonParseDataProperty(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, new_value: JSValue) !void {
        ...
        try self.addProperty(rt, atom_id, descriptor.Descriptor.data(new_value, .all));
    }
    ...
    pub fn reserveOwnPropertyCapacityAssumingPlain(self: *Object, rt: *JSRuntime, needed: usize) !void {
        ...
        try self.ensureUniqueShapeForMutation(rt);
        try self.ensurePropertyCapacity(rt, needed);
        try rt.shapes.reservePropertyHash(rt, &self.shape_ref, needed);
        // Every write to `shape_ref` is an owner adopting a Shape: the clone or
        // relocation this call may perform produces a fresh, young one, and a
        // long-lived owner reaching it is an old-to-young edge the minor's
        // sticky marks would otherwise stop short of.
    ...
    pub fn defineModuleVarRefProperty(
        ...
        const flags = property.Flags.varRef(.{ .writable = true, .enumerable = true });
    ...
    pub fn defineModuleAutoInitProperty(
        ...
        try self.appendModuleAutoInitProperty(
            rt,
            atom_id,
            property.Flags.data(.{ .writable = true, .enumerable = true }),
            realm,
            owner,
        );
    }
    ...
    pub fn appendDenseArrayIndex(self: *Object, rt: *JSRuntime, index: u32, atom_id: atom.Atom, new_value: JSValue) !bool {
        // qjs add_fast_array_element: the dense append
    ...
    /// post-`JS_NewArray` body: `expand_fast_array` +
    ...
    pub fn setProperty(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, new_value: JSValue) !void {
        ...
        while (prototype) |proto| {
            if (proto.findProperty(atom_id)) |index| {
                ...
                // qjs JS_SetPropertyInternal retry2:
    ...
    fn defineOwnDataPropertyForSetKnownNoOwn(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, new_value: JSValue) !void {
        const desc = descriptor.Descriptor.data(new_value, .all);
        if (self.exoticMethods(rt)) |methods| {
            if (methods.define_own_property) |hook| {
                if (!hook(self, atom_id, desc)) return error.IncompatibleDescriptor;
                return;
    ...
    fn defineNewOwnDataPropertyForSimpleSetKnownNoOwn(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, new_value: JSValue) !bool {
        ...
        // (qjs add_property, quickjs.c; the index question lives in
        // the fast_array arm's __JS_AtomIsTaggedInt, quickjs.c).
    ...
    /// qjs JS_SetPropertyInternal's single-walk core for
    ...
    /// plain writable data holder, then add_property
    /// with C_W_E flags straight into the slot — no
    ...
    pub fn setOrDefineOwnDataPropertyForPutFieldOwned(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, new_value: JSValue) align(16) PutFieldFast {
        ...
        if (self.findPropertyProbeTrusted(atom_id)) |lookup| {
            ...
            switch (entry_flags.kind) {
                // qjs's single-mask fast case: swap the
                ...
                .auto_init => {
                    ...
                    self.setEntryKindAndSlot(rt, lookup.index, entry_flags.withKind(.data), .{ .data = new_value });
                    self.pruneBorrowedReferenceHolderIfEmpty(rt);
                    return .done;
                },
        ...
        // Extensibility AFTER the walk (quickjs.c, see doc note).
        if (!self.flags.extensible) return .slow;

        // add_property(C_W_E) + direct slot store.
        // On append failure the staged value is removed before returning
        // `.slow`; on success the slot becomes traced at the shape commit.
    ...
    fn defineModuleNamespaceProperty(self: *Object, atom_id: atom.Atom, desc: descriptor.Descriptor) !bool {
        if (self.class_id != class.ids.module_ns) return false;
    ...
    fn deleteOrdinaryPropertyAt(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, index: usize) bool {
        ...
        if (self.class_id == class.ids.mapped_arguments) {
            if (array.arrayIndexFromAtom(rt.atoms, atom_id)) |mapped_index| {
                if (mapped_index < self.argumentsVarRefs().len) self.deleteMappedArgumentsBinding(mapped_index);
            }
        }
        // qjs remove_global_object_property: deleting a
            ...
            object_shape.deletedPropCount() >= object_shape.prop_count / 2)
        {
            ...
            rt.shapes.compactProperties(rt, self) catch {};
        }
        return true;
    }
    ...
    pub fn deleteProperty(self: *Object, rt: *JSRuntime, atom_id: atom.Atom) bool {
        ...
        if (self.mappedArgumentsBindingIndexFromAtom(rt, atom_id)) |mapped_index| {
            self.deleteMappedArgumentsBinding(mapped_index);
            return true;
        }
    ...
    pub fn freeKeys(rt: *JSRuntime, keys: []atom.Atom) void {
        if (keys.len != 0) rt.freeNative(atom.Atom, keys);
    }
    ...
    pub fn freeze(self: *Object, rt: *JSRuntime) !void {
        ...
        self.detachAllMappedArgumentsBindings();
        for (0..self.shape_ref.prop_count) |index| {
            var entry_flags = self.propFlagsAt(index);
    ...
    fn defineOrdinaryOwnProperty(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, desc: descriptor.Descriptor) !void {
        ...
        if (classOwnsIndexedElementStorage(self.class_id)) {
            if (array.arrayIndexFromAtom(rt.atoms, atom_id)) |array_index| {
                ...
                if (element_index < self.arrayElements().len) {
                    try self.convertDenseArrayElementsToSparseProperties(rt);
                    const index = self.findProperty(atom_id).?;
                    try self.materializeAutoInitEntryForMutation(index);
                    if (!isCompatible(self.propFlagsAt(index), self.propertyEntry(index).*.slot, desc)) return error.IncompatibleDescriptor;
                    try self.replaceProperty(rt, index, desc);
                    return;
    ...
    fn defineArrayLength(self: *Object, rt: *JSRuntime, desc: descriptor.Descriptor) !void {
        ...
        // `[count, target_len)` — faithful to set_array_length,
    ...
    pub fn convertDenseArrayElementsToSparseProperties(self: *Object, rt: *JSRuntime) !void {
        ...
        // convert_fast_array_to_array materializes only the
        ...
        for (elements, 0..) |stored, index| {
            ...
            try self.addProperty(rt, atom_id, descriptor.Descriptor.data(stored, .all));
        }
    ...
    inline fn addProperty(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, desc: descriptor.Descriptor) !void {
        const slot = slotFromDescriptor(desc);
    ...
    /// for a NON-exotic object (quickjs.c `if (p->is_exotic)` gates the whole
    ...
    /// leg goes through `replaceProperty` before the new value is published.
    ///
    ...
    /// straight into add_property — no descriptor record is
    ...
    /// quickjs.c) and keeps only pc/sp live across it. Inlining this into
    ...
    inline fn definePlainDataPropertyKnownFastMut(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, data_value: JSValue) !void {
        if (self.findPropertyIndexTrusted(atom_id)) |index| {
            const desc = descriptor.Descriptor.data(data_value, .all);
            try self.materializeAutoInitEntryForMutation(index);
            if (!isCompatible(self.propFlagsAt(index), self.propertyEntry(index).*.slot, desc)) return error.IncompatibleDescriptor;
            try self.replaceProperty(rt, index, desc);
            return;
        ...
        // constant. The value becomes visible to tracing only when the shape
        // transition commits; until then the mutation-window root in
        // `definePlainDataPropertyKnownFast` keeps it live.
        try self.appendPreparedPropertyEntryImpl(
    ...
    /// matches qjs add_property's probe-free named add
    ...
    inline fn appendPreparedPropertyEntryWork(comptime named_put_no_index: bool, self: *Object, rt: *JSRuntime, atom_id: atom.Atom, entry_flags: property.Flags, slot: property.Slot) !void {
        ...
        errdefer if (inserted) {
            ...
            if (grew_properties) {
                ...
                if (self.hasSlots2Layout() and old_storage == self.trailingPropertyStorageBase()) {
                    // Installing the spill pointer consumed Entry 0 in the
                    // inline tail. An OOM after that publication must copy the
                    // previously live entries back before clearing the spill
                    // bit; selecting the tail alone would expose pointer bytes
                    // as the first JSValue.
                    @memcpy(
                        self.trailingPropertyStorageEntries()[0..old_len],
        ...
        // qjs add_property probes the transition cache
        ...
        if (is_array_index or !rt.shapes.tryCachedTransition(rt, &self.shape_ref, atom_id, entry_flags.bits(), current_capacity)) {
    ...
    fn ensureUniqueShapeForMutation(self: *Object, rt: *JSRuntime) !void {
        ...
        rt.gc.generationalBarrier(self.gcHeader(), &next_shape.header);
    }
    ...
    fn adoptShapeForNewProperty(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, flags: u6, property_capacity: usize, is_array_index: bool) !void {
        ...
        // quickjs.c, runs in the append frame); the remaining triage
        ...
        if (is_array_index) {
            try self.ensureUniqueShapeForMutation(rt);
            try rt.shapes.addProperty(rt, &self.shape_ref, atom_id, flags);
            rt.shapes.adoptionBarrier(self, self.shape_ref);
            return;
        }
        try rt.shapes.transitionPropertyUncached(rt, &self.shape_ref, atom_id, flags, property_capacity);
    ...
    fn ensurePropertyCapacity(self: *Object, rt: *JSRuntime, needed: usize) !void {
        ...
        // append path takes.
        rt.gc.rememberOwnerForBulkWrite(self.gcHeader());
        try rt.shapes.reserveProperties(rt, &self.shape_ref, next_capacity);
        // Every write to `shape_ref` is an owner adopting a Shape: the clone or
        // relocation this call may perform produces a fresh, young one, and a
        // long-lived owner reaching it is an old-to-young edge the minor's
        // sticky marks would otherwise stop short of.
    ...
    /// In slots2 layout the two inline entries begin immediately after the
    /// resident property pointer. There is no class-payload arm in this form.
    ...
    pub inline fn propertyStoragePointerIsExternal(self: *const Object, ptr: [*]property.Entry) bool {
        return ptr != emptyPropertyStorageBase() and
            (!self.hasSlots2Layout() or ptr != trailingPropertyStorageBase(self));
    }

    /// Install an external property buffer by replacing qjs's resident
    /// `prop_values` pointer. Inline spill has no separate representation bit.
    pub inline fn setPropertyStorageExternal(self: *Object, ptr: [*]property.Entry) void {
        std.debug.assert(@intFromPtr(ptr) & (@alignOf(property.Entry) - 1) == 0);
        std.debug.assert(ptr != emptyPropertyStorageBase());
        self.prop_values = ptr;
    }

    pub inline fn setPropertyStorageInline(self: *Object) void {
        std.debug.assert(self.hasSlots2Layout());
        self.prop_values = self.trailingPropertyStorageBase();
    }

    inline fn setPropertyStorageEmptyForDestroy(self: *Object) void {
        self.prop_values = if (self.hasSlots2Layout())
            self.trailingPropertyStorageBase()
        else
            emptyPropertyStorageBase();
    }

    inline fn restorePropertyStorage(self: *Object, ptr: [*]property.Entry) void {
        self.prop_values = ptr;
    }

    /// Base-address choke point for every named-property access. Both external
    /// and trailing storage use the resident Entry pointer, matching base.
    ...
    pub fn replaceOwnPropertyWithVarRefCell(
        ...
        if (old_flags.kind == .data) {
            cell.setVarRefValue(rt, old_slot.data);
        }
        cell.is_lexical = false;
    ...
    /// new slot arm and the shape `Flags.kind` in lockstep. The caller must
    /// have ensured a unique shape for mutation. `next_flags` carries the new
    /// kind; `next_slot` must match `next_flags.kind`.
    ...
    /// `find_own_property`, which is force-inlined and trusts
    ...
    fn updateMappedArgumentsBinding(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, desc: descriptor.Descriptor) !void {
        ...
        if (desc.kind == .accessor) {
            self.deleteMappedArgumentsBinding(index);
            return;
        }
    ...
    fn prepareMappedArgumentsDescriptorForDefine(self: *Object, rt: *JSRuntime, atom_id: atom.Atom, desc: *descriptor.Descriptor) !void {
        if (self.class_id != class.ids.mapped_arguments) return;
        if (desc.kind != .data or desc.value_present) return;
        if (desc.writable == null or desc.writable.? != false) return;
        const index = array.arrayIndexFromAtom(rt.atoms, atom_id) orelse return;
    ...
    fn deleteMappedArgumentsBinding(self: *Object, index: u32) void {
        const slot_index: usize = @intCast(index);
        const refs = self.argumentsVarRefsMut();
        if (refs[slot_index] == null) return;
    ...
    fn materializeMappedArgumentsProperty(self: *Object, rt: *JSRuntime, atom_id: atom.Atom) !void {
        ...
        const mapped_value = self.mappedArgumentsBindingValue(index) orelse return;
        try self.addProperty(rt, atom_id, descriptor.Descriptor.data(mapped_value, .all));
    }
    ...
    fn detachAllMappedArgumentsBindings(self: *Object) void {
        if (self.class_id != class.ids.mapped_arguments) return;
        for (self.argumentsVarRefs(), 0..) |mapped, index| {
            if (mapped == null) continue;
            self.deleteMappedArgumentsBinding(@intCast(index));
        }
    }
};
...
fn flagsFromDescriptor(desc: descriptor.Descriptor) property.Flags {
    return switch (desc.kind) {
        ...
        .data => property.Flags.data(.{ .writable = desc.writable orelse false, .enumerable = desc.enumerable orelse false, .configurable = desc.configurable orelse false }),
        .accessor => property.Flags.accessorFlags(.{ .enumerable = desc.enumerable orelse false, .configurable = desc.configurable orelse false }),
    };
}

fn slotFromDescriptor(desc: descriptor.Descriptor) property.Slot {
    return switch (desc.kind) {
        .generic => .{ .data = JSValue.undefinedValue() },
        .data => .{ .data = desc.value },
...
fn mergeDescriptor(current_flags: property.Flags, current_slot: property.Slot, desc: descriptor.Descriptor) descriptor.Descriptor {
    return switch (desc.kind) {
        .generic => switch (current_flags.kind) {
            .data => descriptor.Descriptor.data(current_slot.data, .{ .writable = current_flags.writable, .enumerable = desc.enumerable orelse current_flags.enumerable, .configurable = desc.configurable orelse current_flags.configurable }),
            .accessor => descriptor.Descriptor.accessor(current_slot.accessor.getterValue(), current_slot.accessor.setterValue(), .{ .enumerable = desc.enumerable orelse current_flags.enumerable, .configurable = desc.configurable orelse current_flags.configurable }),
            .var_ref => descriptor.Descriptor.data(current_slot.var_ref.varRefValue(), .{ .writable = current_flags.writable, .enumerable = desc.enumerable orelse current_flags.enumerable, .configurable = desc.configurable orelse current_flags.configurable }),
        ...
        .data => descriptor.Descriptor.data(if (desc.value_present) desc.value else switch (current_flags.kind) {
            .data => current_slot.data,
            .var_ref => current_slot.var_ref.varRefValue(),
            .accessor, .auto_init => desc.value,
        }, .{ .writable = desc.writable orelse if (current_flags.isAccessor()) false else current_flags.writable, .enumerable = desc.enumerable orelse current_flags.enumerable, .configurable = desc.configurable orelse current_flags.configurable }),
        .accessor => descriptor.Descriptor.accessor(if (desc.getter_present) desc.getter else switch (current_flags.kind) {
        ...
        }, .{ .enumerable = desc.enumerable orelse current_flags.enumerable, .configurable = desc.configurable orelse current_flags.configurable }),
    };
}
...
fn arrayLengthNumber(rt: *JSRuntime, value: JSValue) !?f64 {
    ...
    if (value.is(.object)) {
        ...
        if (object.class_id == class.ids.number or object.class_id == class.ids.boolean) {
            const primitive = (object.objectData() orelse return null);
...
test "unmapped arguments share a prepared shape and use dense element storage" {
    ...
    try template.defineOwnPropertyAssumingNew(
        rt,
        atom.ids.length,
        descriptor.Descriptor.data(JSValue.int32(0), .method),
    );
    const iterator_key = comptime atom.predefinedId("Symbol.iterator", .symbol).?;
    try template.defineOwnPropertyAssumingNew(
        rt,
        iterator_key,
        descriptor.Descriptor.data(JSValue.int32(17), .method),
    );
    const callee_key = comptime atom.predefinedId("callee", .string).?;
    try template.defineOwnPropertyAssumingNew(
        rt,
        callee_key,
        descriptor.Descriptor.accessor(JSValue.undefinedValue(), JSValue.undefinedValue(), .none),
    );
    ...
    try arguments.defineOwnProperty(
        ...
        descriptor.Descriptor.data(JSValue.int32(41), .none),
    );
    try std.testing.expect(!arguments.flags.fast_array);
    try std.testing.expectEqual(@as(?i32, 31), (try arguments.getProperty(atom.Atom.taggedInt(0))).as(.int));
