...
pub const VerifyError = error{
    ...
    AddressIndexRangeMismatch,
    AddressScanFilterMissing,
...
/// Per-span register copy of the conservative prefilters. JSC copies its
/// TinyBloomFilter before walking a span for the same aliasing reason:
...
pub const Table = struct {
    ...
    pub fn auditArenas(self: *Table) usize {
        const Audit = struct {
            ...
            fn visit(ctx: *anyopaque, user: [*]u8, is_free: bool) void {
                ...
                if (audit.reported < 8) {
                    audit.reported += 1;
                    std.debug.print("gc: ARENA AUDIT free block at 0x{x} reads heap_accounted (kind .{s}, lifetime_word 0x{x})\n", .{
                        @intFromPtr(user),
    ...
    /// Prove the two-way index used by conservative candidate resolution.
    ///
    ...
    pub fn verifyIndex(self: *Table, verify_scan_cache: bool) VerifyError!void {
        ...
        while (by_it.next()) |entry| {
            ...
            while (page <= last_page) : (page += 1) {
                const bucket = self.pages.getPtr(page) orelse return error.AddressIndexMissingPage;
                var matches: usize = 0;
                for (bucket.occupants.items) |candidate| {
                    if (occupantsEqual(candidate, occupant)) matches += 1;
                }
                if (matches == 0) return error.AddressIndexMissingPage;
        ...
        while (page_it.next()) |entry| {
            ...
            for (bucket.occupants.items, 0..) |occupant, index| {
                const first_page = occupant.lo >> page_shift;
                const last_page = (occupant.hi - 1) >> page_shift;
                if (page < first_page or page > last_page) return error.AddressIndexOrphanPage;
                const canonical = self.by_header.get(occupant.ptr) orelse return error.AddressIndexOrphanPage;
                if (!occupantsEqual(canonical, occupant)) return error.AddressIndexRangeMismatch;
                for (bucket.occupants.items[0..index]) |previous| {
    ...
    pub fn rebuildScanFilter(self: *Table) ScanFilter {
        var bits: usize = 0;
        var arena_it = self.arenas.keyIterator();
    ...
    fn forEachGcObjectInBlocks(
        ...
        var probe = addr;
        while (true) {
            if (heap.blockOfWithFilter(@ptrFromInt(probe), block_filter)) |block| resolve: {
                owns_address = true;
        ...
        return .{ .owns_address = owns_address, .hits = hits };
    }
    ...
    ///
    /// This is the conservative scanner's word-at-a-time primitive and its
    /// only caller. Scan-level lookup/hit statistics are deliberately batched
    ...
    pub fn forEachTraceCandidateAt(
        ...
        if (addr < scan_filter.bounds_lo or addr >= scan_filter.bounds_hi) return 0;
        ...
        // can be dismissed.
        const block_result = self.forEachGcObjectInBlocks(addr, scan_filter.block_bits, context, visit);
        if (block_result.owns_address) {
        ...
        if (filterRulesOutBits(scan_filter.address_bits, base) and
            filterRulesOutBits(scan_filter.address_bits, prev_base)) return 0;
