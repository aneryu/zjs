...
fn promiseStaticEntry(
    ...
    return .{
        ...
        .magic = @intCast(id),
        .cproto = .generic_magic,
        .native_function = builtin_dispatch.genericMagicFunction(&promiseStaticCallNative),
    };
}
...
fn promiseResolveCall(
    ...
    return promiseResolveStaticCall(
        host_call.ctx,
        host_call.output,
        realm.global,
        host_call.this_value,
...
fn generatorState(gen: *core.Object) State {
...
fn complete(ctx: *core.JSContext, gen: *core.Object) void {
    if (generatorState(gen) == .completed) return;
...
pub fn asyncGeneratorEnqueue(
    ...
    const promise = try core.promise.constructWithPrototype(ctx, promisePrototypeFromGlobal(rt, gen_global));
    const resolving = try createPromiseResolvingPair(rt, gen_global, promise);
...
pub fn asyncGeneratorResolveFunctionCall(
    ...
    switch (action) {
        ...
        .await_resume => {
            ...
            if (generatorState(gen) != .executing) return core.JSValue.undefinedValue();
            if (is_reject) {
