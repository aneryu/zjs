//! PERF-T-SPIKE (branch spike/perf-t-main, never main): the site registry
//! behind the guarded direct-slot opcode `tspike_get_slot`.
//!
...
//! guard + direct slot load. Sites are rewritten from get_field by
//! resolve_labels.zig when ZJS_TSPIKE=1; each site owns one registry
...
//!
//! 2026-09-06 re-run on main (typed plan §十一 R11): the 2026-08-26 spike's
//! hit path carried `value.dup()` + a receiver release; main has no
//! reference counts, so both the baseline `op_get_field` hit and this hit
//! lost the same constant. The percentage read here is the one that counts.
//!
//! TWO PROTOTYPE-FAIRNESS RULES (R12), learned from the first draft's
//! disassembly (which would have UNDER-priced the mechanism):
//!   1. `capture()` must NEVER be inlined into a resident handler. Inlining
//!      it forced callee-saved spills and gave the handler a prologue +
//!      epilogue that the baseline `op_get_field` (a leaf) does not pay.
//!      Capture lives behind the cold `h_field` shell.
...
//!          already in hand). On main an UNSHARED shape mutates in place
//!          (append / delete / flag update / proto swap at the same
//!          address), so this arm is NOT sound as a production guard; it is
//!          the cost floor of the mechanism, nothing else. That load is the
//!          entire measurable difference between the arms.
//!
...
//! machinery; prototype support is one level deep; reads only (the typed
//! workloads are read-dominated, and 255 stays unclaimed for the
//! invalid-opcode test).
...
/// Returns true when the entry now guards this object, so the next execution
/// of the site takes the resident handler's hit path.
pub noinline fn capture(obj: *core.Object, atom_id: core.Atom, e: *Entry) bool {
    var slow = false;
