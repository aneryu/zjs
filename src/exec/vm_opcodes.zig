...
pub noinline fn execVm(
    ...
    return switch (sub) {
        ...
        bytecode.opcode.ext0_sub.to_object => {
            _ = try toObjectVm(ctx, output, stack, frame, catch_target, global);
            return .done;
        },
        ...
        bytecode.opcode.ext0_sub.check_ctor_return => {
            _ = try checkCtorReturnVm(ctx, output, stack, frame, catch_target, global);
            return .done;
        },
