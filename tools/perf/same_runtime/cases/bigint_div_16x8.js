...
// README). Shapes are ordered: the numerator has 16 limbs of 64 bits and the
// divisor 8. The numerator's top limb is saturated so the numerator is
...
//
// The iteration count is 100 rather than the multiplication cases' 20000
// because the current implementation walks the numerator one bit at a time and
// a single division costs microseconds.
const a = ((1n << 16n * 64n) - 1n);
const b = (1n << 8n * 64n - 1n) | 3n;
function run() {
    let r = 0n;
    for (let i = 0; i < 100; i++) r = a / b;
