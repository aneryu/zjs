...
rule-2 gate (bench-v8 A/B against a frozen merge-base build, 8 samples,
median no regression on the composite Score) — or, where noted, the cheaper
identity gates of rule 4 — before merging.
Estimated verification cost: about an hour of measurement per A/B item, down
from the half day the zoo protocol cost. Try the identity gate first: most
items in this queue are moves and renames, and a machine-code-identical
result is both cheaper and stronger than a statistical one.

**Outstanding from the suspension window**: H13's Contract F-5/F-6 dead-code
deletion (−79 lines) landed unpriced while rule 2 was suspended, after a first
attempt had been caught by the identity gate as a QCP-1B-class layout
perturbation (`.text` −432 B). It is the one hot-path change in `main` with a
known, measured layout effect and no A/B behind it. Price it on the next
bench-v8 run.
...
The COLD-zone line-count work is tracked separately in
[code-volume.md](code-volume.md), including the categories of apparent
duplication that were ruled load-bearing.

| # | Item | Evidence | Notes / gate |
|---|---|---|---|
| H1 | Rehome overflow content out of `call_runtime.zig`. **Atomics, Reflect, the iterator protocol, `Error.stack` and dynamic `Function` construction all landed 2026-08-20** (Atomics: 1,000 lines → `atomics_ops.zig`, which had held only the 88-line record table while its own doc comment pointed at `call_runtime` for the bodies; Reflect: 11 interleaved decls → `reflect_ops.zig`; iterator protocol: 15 decls → `iterator_ops.zig`, which turned up two of the moved names already living there — one a pure forwarding shell, one the same body under a different error set). The file is now **5,542 lines / 136 top-level pub fns**, from 7,510 / 208. | Every domain the item named has been rehomed. What remains in the file is the call chain itself plus generator/global-lexical/construct machinery — no longer 'overflow'. Note the Reflect batch had to promote three call-chain helpers to `pub`: splitting a domain out makes the seam explicit, which is the point, but it is not free. A 2026-08-20 audit proposes the next seams (generator ops, global-lexical env) — those are a new item, not this one | Move in batches; one bench-v8 A/B per batch (the parallel protocol makes that ~50 s, so batch small) |
...
| H7 | Bring the orphan section `.text.zjs.nmfd_term` under explicit linker-script management | `vm_call.zig:780` emits it; `tail_hot_layout_aarch64.ld` never mentions it, so its placement relies on orphan-section defaults | Linker change: always bench-v8 A/B |
...
| ~~H12~~ | **Done 2026-08-19**: every same-name/different-meaning pair is resolved — `isConstructErrorObjectName`, `getValuePropertyViaGlobalSlots`, `defineDataPropertyByName`, `checkedInt32*` (renames); `functionPrototypeFromGlobal`, `numberValue`, the `expectObject` family (merges onto single owners). One follow-up parked: `vm_arith.fastInt32*` (widening) vs `vm_property.checkedInt32*` (overflow-intrinsic) are now honestly named but strategically contradictory — A/B one strategy under the reinstated rule-2 gate. | — | — |
...
- **Renaming `compiler_v2` to `compiler`**: initially ruled out (2026-08-18)
  ...
  `-Dzjs_v2_layout` → `-Dzjs_compiler_layout`. Both deprecated aliases were
  retained at the time and removed on 2026-08-19. The attested signature string
