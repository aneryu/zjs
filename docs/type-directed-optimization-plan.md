...
| F4 | **opcode 空间接入**（opcode namespace 方案已升为独立工作项 **PERF-OPCODE-SPACE**，roadmap v1.7） | short 区 178-253 已被 temp 区+融合 op 占用（`bytecode.zig:348-424`）；T1-T4 预计需 20-40 个新 id，现平面放不下 | 未开工；**范围已按 2026-08-27 调研+裁决修订**（docs/perf/opcode-space-survey.md）：wide/prefix 平面**退出范围**——四引擎源码证实 V8/JSC 的前缀扩的是操作数宽度而非编号，买不到新编号且要付第二次间接跳转+handler 三倍化。现行范围 = **编号回收（参照 V8：退役冷 op + 冷 op 降级到子操作码平面）**、编码版本策略、四消费方共享生成源（compiler/disassembler/serializer/JIT）、降级集的 decode 成本 A/B。实测：254 个终流 opcode 中 102 个频次 ≤0.0001%，typed 家族只需 20-40；本计划 F4 与 FN-M1A、SER-ARTIFACT 均依赖它 |
