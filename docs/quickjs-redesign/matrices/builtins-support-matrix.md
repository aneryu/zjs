...
| Map | Map builtin functions | `builtins/collection.zig` | SameValueZero keys, iteration order, constructor iterable | Map now uses object-owned multi-entry storage with SameValueZero lookup, `set` returning `this`, VM-created instances inheriting registered `Map.prototype` methods, `Symbol.species`, array-entry `new Map(iterable)`, `Map.prototype.keys`, and the first Map.groupBy tranche. Map smoke, builtins helper coverage, and `built-ins/Map 0 21` pass 0/22 errors. Wider Map validation is still not green: `built-ins/Map 0 60` reports 27/61 errors across iterator closing, custom adder invocation, prototype descriptor, and `clear` receiver semantics. | fixture_validated |
