...
| Map | Map builtin functions | `builtins/collection.zig` | SameValueZero keys, iteration order, constructor iterable | Map now uses object-owned tombstone-backed storage with SameValueZero lookup and canonical `-0` callback keys, `set` returning `this`, VM-created instances inheriting registered `Map.prototype` methods, `Symbol.species`, non-owning `Map.prototype.constructor`, `Map.prototype.{keys,values,entries,clear,forEach,getOrInsert,getOrInsertComputed}`, `Map.prototype[Symbol.iterator]`, `Map.prototype[Symbol.toStringTag]`, array-entry and covered iterator-close `new Map(iterable)` behavior, custom adder side effects, `Array.from` over Map iterators, the current Map.groupBy tranche, covered cross-realm `Reflect.construct(Map, [], newTarget)` prototype fallback, and `valid-keys` parser/value coverage. Map smoke and builtins helper coverage pass; `built-ins/Map 0 60` and full `built-ins/Map 0 400` now pass with 0 errors. This is target-slice validated, not a claim that all future Map descriptor/internal-slot combinations are complete. | fixture_validated |
