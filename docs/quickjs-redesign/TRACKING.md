...
| Current validation state | Semantic follow-up local gates pass after shared builtin registration, receiver-preserving `call_prop` dispatch, host test262 callables, prototype inheritance, weak-entry sweep hooks, GC scan scaffolding, VM argument cleanup hardening, and the current Map constructor/groupBy repairs: `zig build qjs --summary all`, `zig build test --summary all` 169/169, `zig build smoke --summary all` 45/45, `zig build run-test262 --summary all`, and `git diff --check`. Focused WQ-012 test262 evidence improved: built-ins/global passes 0/29 errors, Object/is passes 0/21 errors, and `built-ins/Map 0 21` passes 0/22 errors. Broader Map, Promise, WeakMap, and WeakSet execution is still not green; current validation records iterator/prototype/internal-slot/promise-combinator blockers rather than marking those domains semantic-complete. |
...
| WQ-012 | in_progress | follow-up | Builtin prototype and descriptor completion | Shared `builtins/registry.zig` now installs standard globals, constructor objects, prototype objects, namespace methods, and descriptor flags through one helper used by both intrinsic bootstrap and host global setup; collection prototype calls lower through generic receiver-preserving `call_prop`, the old `collection_method` opcode has been removed, and VM-created collections inherit registered prototypes. Array/String/Date/RegExp/Promise prototype and static method calls now lower through ordinary `call_prop` to callable native function objects. Host `assert`, `assert.throws`, `verifyProperty`, `verifyCallableProperty`, descriptor flag helpers, and `isConstructor` are ordinary callables. Map now covers `Symbol.species`, BigInt/Number SameValueZero separation, `new Map(iterable)` for array entries, tombstone-backed MapData, non-owning `Map.prototype.constructor`, `Map.prototype.{keys,values,entries,clear}`, `Map.prototype[Symbol.iterator]`, `Map.prototype[Symbol.toStringTag]`, `Array.from` over Map iterators, and the first Map.groupBy tranche; `built-ins/Map 0 60` is currently 49/61 passing and full `built-ins/Map 0 400` improved to 177/204 passing. | Full Map is not yet green: remaining blockers are custom adder invocation/side effects, iterator-close object-literal fixtures, broader parser support for computed `Symbol.iterator` object literals and realm fixtures, `forEach` callback semantics, `getOrInsertComputed` callback semantics, `valid-keys` parser/value coverage, and Set/WeakMap/WeakSet broader parity |
...
| 2026-04-27 | semantic-followups | `./zig-out/bin/run-test262 -v -t 1 -c quickjs/test262.conf -d quickjs/test262/test/built-ins/Map 0 60` | 1 | Wider Map tranche improved after Map iterator/prototype descriptor repairs: prepared 61/204 tests, `Result: 12/61 errors, passed 49`; remaining failures are custom adder/getter invocation, iterator-close parser/runtime fixtures, proto-from-ctor-realm syntax, and iterable custom-set side effects | targeted-test262 |
| 2026-04-27 | semantic-followups | `./zig-out/bin/run-test262 -v -t 1 -c quickjs/test262.conf -d quickjs/test262/test/built-ins/Map 0 400` | 1 | Full Map directory improved but remains open: prepared 204/204 tests, `Result: 27/204 errors, passed 177`; remaining failures concentrate in custom iterable construction, forEach callbacks, getOrInsertComputed callbacks, valid-keys syntax/value coverage, and undefined new.target | targeted-test262 |
