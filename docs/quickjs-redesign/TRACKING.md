...
| Current validation state | Semantic follow-up local gates pass after shared builtin registration, receiver-preserving `call_prop` dispatch, host test262 callables, prototype inheritance, weak-entry sweep hooks, GC scan scaffolding, VM argument cleanup hardening, and the current Map constructor/groupBy/iterator/upsert repairs: `zig build qjs --summary all`, `zig build test --summary all` 169/169, `zig build smoke --summary all` 45/45, `zig build run-test262 --summary all`, and `git diff --check`. Focused WQ-012 test262 evidence improved: built-ins/global passes 0/29 errors, Object/is passes 0/21 errors, `built-ins/Map 0 21` passes 0/22 errors, and full `built-ins/Map 0 400` passes 0/204 errors. Promise, WeakMap, and WeakSet broader execution still have separate recorded blockers; current validation does not mark those domains semantic-complete. |
...
| WQ-012 | in_progress | follow-up | Builtin prototype and descriptor completion | Shared `builtins/registry.zig` now installs standard globals, constructor objects, prototype objects, namespace methods, and descriptor flags through one helper used by both intrinsic bootstrap and host global setup; collection prototype calls lower through generic receiver-preserving `call_prop`, the old `collection_method` opcode has been removed, and VM-created collections inherit registered prototypes. Array/String/Date/RegExp/Promise prototype and static method calls now lower through ordinary `call_prop` to callable native function objects. Host `assert`, `assert.throws`, `verifyProperty`, `verifyCallableProperty`, descriptor flag helpers, and `isConstructor` are ordinary callables. Map now covers `Symbol.species`, BigInt/Number SameValueZero separation, array-entry and iterator-close `new Map(iterable)` cases, tombstone-backed MapData, non-owning `Map.prototype.constructor`, `Map.prototype.{keys,values,entries,clear,forEach,getOrInsertComputed}`, `Map.prototype[Symbol.iterator]`, `Map.prototype[Symbol.toStringTag]`, cross-realm Map prototype selection for the covered `Reflect.construct` fixture, `Array.from` over Map iterators, Map.groupBy tranche coverage, and `valid-keys`; `built-ins/Map 0 60` and full `built-ins/Map 0 400` now pass with 0 errors. | Set/WeakMap/WeakSet broader parity and descriptor/internal-slot completeness remain separate WQ-012 follow-up scope |
