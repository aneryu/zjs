...
随后对“测试加 harness”做逐行删减，得到不依赖 test262 语料的 168 行夹具
`tests/fixtures/gc/pending-call-window.js`，单测以 stress 节奏 4／8／16 运行，修复前崩溃、修复后通过。
...
4. Shape 根表按原型地址哈希，搬迁模式下产生重复根谱系（内存／IC 代价）。对象只在晋升时
   搬迁一次，老代不再移动，因此每个原型至多多出一条谱系。Shape 自身的属性桶也以该哈希
   为种子，搬迁后重算须重建整条谱系；以对象内稳定种子替代地址受限于 13 个保留位，
   大量原型共享种子会拉长热路径的碰撞链，两者都不划算。
5. RootProvider 必须在预扫描与主遍历报告同一组只读根（roots.zig 的既有契约）。主遍历现在
   对违约直接 panic：目标已搬迁，或目标仍在 nursery 而所在页未被预扫描保留。不在主遍历
