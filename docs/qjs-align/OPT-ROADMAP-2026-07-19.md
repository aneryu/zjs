...
| `script_or_module`（独立于diagnostic filename） | 当前QJS沿stack frame跳过eval FB，取首个non-eval FB的debug filename；源码也承认这只是ScriptOrModule近似，strip-debug时明确返回null | real root后先用当前full-debug production的direct/indirect/escaped-eval、dynamic import与error filename判断能否改用同一frame机制并删除；本轮不新增strip feature。若host contract必须区分display filename与referrer，只放optional extension并明确记product divergence，不能再声称total allocation exact |
...
| `class_meta/class_fields_init`及其他过渡side facts | QJS core header无对应pointer | W1c4先移出QJS header，放header后的可定位extension；W1c5 pack时原样搬到exact code后，始终保持core offsets。W1d删除事实本身，full-close要求QJS-compatible ordinary FB无extension |
...
| interrupt | poll counter属于Context、初值10000且无handler时仍持续减/重置；call entry先poll caller context，final arm切realm后body backedge再poll callee context | VM-local `InterruptPoller`按Machine/call重建，budget 1024且只在handler存在时活跃；regexp另有自己的10000 counter | 新立M-INTERRUPT-BUDGET：counter随RealmContext持续，同realm nested call/Machine重建不刷新；跨realm严格区分caller entry与callee body。poll point与budget state分别验收，regexp counter保持独立；它不混进FB/root或tail stack候选 |
...
| interrupt counter lifetime | call/jump poll point已存在，但zjs budget随VM Machine/call重建为1024且无handler时不持续；QJS counter属于Context、初值10000并跨call/backedge持续 | W2-0 M-INTERRUPT-BUDGET；其后重冻M-RETURN-CONT/M-FRAME-CONT，不与tail stack合刀 |
...
| W1b2.5a2 | M-LVALUE-PROVENANCE-CORE：最后发射指令驱动普通reference与call rewrite | emitter `last_opcode_pos`；`get_lvalue/put_lvalue`；simple `var`的`get_lvalue(FALSE)`；call点last-op switch；optional getter+相邻raw label；comma/normal label显式invalidate；with只由`_with_` scope+resolver决定 | 先让code/atom/source/provenance原子commit，再建立phase-1 descriptor并迁assignment/update/delete/typeof/generic-for与simple var；call/optional-call/tagged-template共用一个`rewriteCallReference`，direct eval优先。普通路径正式label/fixup直达put，不能读`findGlobalRefPutTail`扫描；旧pattern unpatched-target仅具名留到b。optional整链只有一个共享LabelSlot/LabelRef。删除`peekParenthesizedBareIdent`、call/source-tail状态、selected-reference scope flag、`result_needed`协议分叉及普通expression对`active_with_atom`的读取。pattern consumer在b迁移；private producer回到baseline，完整binding/lowering留W1d |
...
19. **M-CLOSURE-SIDECHANNEL：把仍活跃的函数语境全部迁回bytecode/closure。**
   ...
   - private field本身不触发shared brand：每个field的lexical slot保存独立`private_symbol`并由它完成identity/access。只有private
     ...
     `initializeClassPrivateMethods`的home-object descriptor scan/copy、private-name remap与constructor/private-name side table；重复class
     evaluation必须由运行时`private_symbol`/closure产生新identity，而非parser atom冒充。non-extensible实例同样以pinned QuickJS结果验收，
     不保留test262语义旁路。该checkpoint绿前不得从CORE提前合入private temp opcode、空声明槽或atom fallback。
...
- closure side channels：base/derived/default/user constructor分别覆盖public/private/computed field、private brand/method、
  ...
  wrong-brand TypeError与non-extensible pinned-QuickJS行为锁定“field独立private symbol + method/accessor每侧按需至多一个brand + lexical method”而非per-instance descriptor copy；删除side slots后
  补每个allocation/capture点OOM recovery；instance initializer由lexical binding调用，static initializer由child以class receiver
  立即调用，`class_static_this`/synthetic atom/runtime vardef scan/eval高位均负向为零；
...
QuickJS的`JS_INTERRUPT_COUNTER_INIT=10000`只是阈值常量，关键机制是counter属于`JSContext`：每次call entry先减
caller context的counter，最终function arm切换后jump/backedge再减callee context的counter；只有到零才重置并读取Runtime handler，
...
| W1b2.5（correctness，当前阶段） | 已完成body pre-scan删除、block/body语法边界、generic-for普通LHS单遍、M-DECL-SCOPE-TOPOLOGY、M-BODY-SCOPE-IDENTITY与M-DEFINE-VAR-CORE/simple producer。当前未提交private半迁移导致3个exec `ClosureVarNotFound`，不是候选；先恢复private baseline。下一原子步M-LVALUE-PROVENANCE-CORE：phase-1 emitter transaction→ordinary descriptor/call→simple `var`正式reference label，普通路径退出bounded scan → M-PARSER-CONTROL-CLEANUP → 不可拆`M-DSTR-QJS-TRAVERSAL`（SOURCE-ORDER→DEFINE-VAR-CLOSE→STACK，迁完全部pattern target并删除unpatched-target/tail scan）→ M-FINALLY-SINGLE-BODY → M-SCOPE-EVENT-PRODUCERS → M-DERIVED-THIS-CANONICAL → (M-FINALIZER-PRECHILD + M-BODY-HOIST-ANCHOR + M-SCOPE-CLOSE-LOWERING)。括号内三项也是同一QJS finalize checkpoint，不从任何中间双表示取性能结论 | row schema/identity/record bytes保持；parser scope/`defineVar`/last-op分别成为scope、declaration、reference/call唯一owner，source semantic scan、parser tail-call pushdown、per-exit optional状态、selected-reference与bounded reference-tail scan为零；ordinary/Annex-B/destructuring raw vardef与child/cpool、finally code、scope-exit、parameter/body final bytecode及derived capture index逐项同构；parent/child/sibling、loop cell identity与Nth-OOM正确。只可声明ordinary/core compiler construction close；private完整slot/brand/lowering、class-fields aggregator/static-block wrapper与提前`<class_fields_init>` row归W1d，module local-export index归W1e，using/TS namespace不在QJS-core结论内。W1b2.5完成前不得进入Realm/FB/pack，总体PMU重冻仍等待W1b2.6 |
| W1b2.6（product correctness，独立） | M-USING-TYPED-CONTROL消费M-PARSER-CONTROL-CLEANUP留下的per-scope typed record/anchor → 八个using `special_object + call` helper改typed opcode/continuation → 删除剩余internal-helper cache/trace/free | sync/async using的structured unwind、suppressed-error precedence与Nth-OOM通过；无QJS exact声明、收益记零。不得拖住W1b2.5 ordinary/core finalizer，但必须在总体性能重冻与W1b3b callable inventory前完成 |
...
| W1c1 | ordinary GLOBAL唯一selector（含AUTOINIT retry）→ root+nested共享closure2 pass1/one-pass fill → owner-only cell flags → properties/capture失败完整rollback | script/direct/indirect各有自己的function/cur_func/final cell array；无placeholder/copy/replace；length/name/lazy prototype在capture后，QJS忽略property失败的quirk不复制；module owner/null slots不在此阶段验收 |
| W1c2（correctness/layout） | pc2line buffer prepend起始line-1/column-1 ULEB128 → full-debug至少两个header LEB → producer/decoder/fallback同构 → 删除final/view平行坐标字段 | diagnostic-qjs byte golden与stack location矩阵一致；不改owner/allocation，收益记零 |
| W1c3（单独ownership候选） | 只按当前full-debug production contract建立source NUL owner+pc2line exact-size producer → 完成真实fallible准备并收紧no-fail publication → atoms/values/内嵌atom ownership move → debug buffers转移 → 清空FunctionDef slots | refcount/accounting/OOM/recovery、现有name/source保留、source NUL及pc2line单owner锁定；不新增strip feature/API，不造假shrink/registry OOM点，不夹带core pack收益 |
| W1c4（表示/layout候选） | 唯一production raw builder+fixture builder → `createWithFam`并zero完整payload → 96B/align8 QJS base+32B production debug tail（fixture可显式no-debug）→ flag masks/ROM zero hole/zero-count NULL → 临时extension；tables/code暂不合并 | raw producer/FAM free/mallocz、flag/null/core offset/load chain、8B GC prefix/slab、production full-debug/fixture no-debug、GC/OOM通过；单独A/B header/inline-debug box，不夹带artifact merge |
| W1c5（单独allocation候选） | 统一checked `FunctionLayout.famBytes`把cpool/vardefs/closures/exact code并入同一`createWithFam` allocation并删除临时block base/旧block allocation，扩大mallocz范围；source/pc2line独立move-owned；extension按显式alignForward/padding搬到code后，析构先capture layout | 默认16B JSValue锁pinned exact core offsets；8B alternate锁参数化order/owner/free并跑altrepr；main payload与side-buffer accounting边界、extension、checked create/destroy/GC/OOM通过。从W1c4稳定点A/B，只称M-FB-PACK-CORE，不虚报跨representation total exact |
...
| W1d | 先做不可拆PRIVATE-BINDING/CLASS-INIT：`add_private_class_field` lexical+const rows、field/method/accessor init、setter companion、capture与VarKind lowering、field独立private symbol、method/accessor每个instance/static侧按需至多一个brand，随后删除descriptor copy/name remap/side table；private再接入CORE descriptor/call rewrite。然后把import-meta/arrow从side slot迁回capture；instance initializer改lexical binding，static initializer改child+class receiver并删除`class_static_this`/runtime scan/eval `0x8000`；裁决ScriptOrModule owner；清剩余EntryContract位；M-FB-PACK-EXACT-CLOSE | private undefined/shadow/repeated-class/field/method/getter/setter/`#x in`/wrong-brand/non-extensible与Nth-OOM逐VarKind对照pinned QJS；QJS-compatible FB extension为空且allocation止于code，或剩余product extension被明确限定；所有eval operand恢复完整u16；任何机制偏离都有具体非偏好证据、双消费者和OOM证明 |
| W1e | module registry/list迁入RealmContext（record不额外own RealmRef）→ persistent artifact/function/capture → record变体slot FB→function owned transition → nullable import staging → indexed wiring/seal → 精确link phase/error/SCC rollback → namespace VARREF/AUTOINIT | 同specifier跨realm完全隔离；record list base-ref/finalizer unlink正确；MODULE_IMPORT保持null直到link；realm内同一module只compile/construct一次且任一时刻只有一个直接artifact owner；host file cache不冒充loaded registry |
...
| W2-0 | 向RealmContext加入interrupt counter 10000并删除VM owner → 同realm跨Machine/nested call/generator持续 → cross-realm caller-entry/callee-body poll矩阵 | handler为null时counter也持续；触发时序与QJS一致，不与tail stack或dispatch候选合刀，收益记零后重冻 |
