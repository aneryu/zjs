...
| P6 | M-SHAPE-PUBLISH：root/transition/property publish | `find_hashed_shape_prop`、`add_property`、`add_shape_property` | capacity-independent cache-hit候选已验证后因PMU硬回退撤销；只有独立profile证明hash/FAM等其他残差，或allocator header表示先因自身机制获批，才重开 |
...
| W4（进行中；standard-object与cross-capacity shape候选已关闭，object free-order correctness已收口） | ✅ force-GC liveness 前置（`f221dfee` + `2ecbf301`/`951726e1`/`1f67bdbc`/`ad3218dd`）→ ✅ `48f6d61d`对齐properties/shape→同步class finalizer→accounting/raw-free顺序；❌ `10348b7d` cross-capacity cache hit虽正确触发，PMU硬回退后由`5cb2dda5`完整撤销 → 继续其余M-ALLOC-LIFECYCLE | standard-object两案均已关闭且收益记零；free-order是correctness且不申报性能。cross-capacity shape direct的两方向及control都越过+1%门槛，当前allocator表示下不再重开。整个M-ALLOC-LIFECYCLE仍未关闭，accounting/allocator geometry仅凭新profile另立；阶段末仍跑统一force-GC/OOM gate |
| W5（**进行中；correctness/initial-yield、signed BigInt、numeric及ordinary/empty string discard、ordinary/untagged-template empty-string late short form、三组store规则、`get_length`、put→get、null/undefined strict fold、typeof test lowering、constant-test lowering、dup-put lowering、inc/dec_loc lowering、add_loc producer域、tail-call producer policy及不可达`to_propkey`已收口**） | ✅ `fde49b15`以fresh function grammar boundary修复默认参数内generator function expression，正例返回`1`且外层direct-yield负例仍与qjs同为`SyntaxError`；✅ `b8f7e0d2`以canonical `OP_initial_yield`替换四-op marker并删除`generatorBodyPc`扫描，保持sync/async-generator suspended-start及ordinary async/fixture pc 0入口；✅ `b39eb39c`以`resolve_labels`相邻peephole折叠signed `push_bigint_i32 + neg`并保持MIN/cpool/source边界；✅ `08e4bbdb`按QJS producer/finalizer边界完成numeric discard和通用tail `drop + return_undef`；✅ `2b5a1129`恢复ordinary/empty string的owned `push_atom_value` phase-1 producer，由`resolve_labels`删除non-tagged discard pair并把live empty缩为`push_empty_string`，`119915c6`记录discard emission封账；本次phase复核进一步关闭ordinary/untagged-template empty short form的owner/source/OOM/consumer证据，RegExp不纳入该结论；✅ 复核`8a09960f`已按QJS phase覆盖discarded indexed/ref、field及post-update store，2026-07-24 `discarded`窄门禁5/5全绿；✅ `ac6aa9a3`证明ordinary/optional value的late `get_length` fold与method consumer边界、atom ledger/source remap及四分配点同Bytecode OOM retry，事务声明只覆盖`resolve_labels.run`；✅ `02c486e5`锁定put/get四族、source marker与idx/jump边界，OOM复用同一`resolve_labels.run`事务证据且不申报PMU；✅ `f727ea54`完成null/undefined strict fold的source回映与producer/negative边界，bytecode 4/4、parser 1/1，OOM只复用`ac6aa9a3`的`resolve_labels.run`证据；✅ `a30bb551`确认typeof接受域此前已对齐，并补齐equality compare及inequality branch source回映，锁定undefined/function equality、`neq`/`strict_neq + if_false`、`neq + if_true`反例、atom ledger与parser最终快照，bytecode 5/5、parser 3/3，OOM同样只复用`ac6aa9a3`的`resolve_labels.run`事务证据；✅ `f891c158`将`push_false`/`push_true`/`null`/`undefined`与`push_i32`五类constant-test producer统一交给`resolve_labels`，五producer×两branch的taken/untaken、branch source回映、interior-target保留及parser六个真实producer最终snapshot均已锁定，bytecode 2/2、parser 1/1，OOM只复用`ac6aa9a3`的`resolve_labels.run`事务证据；✅ `28229419`在既有三形状/四族opcode折叠上补齐put/drop source回映与trailing get `line2`延后语义，并锁定different idx、put/drop/get jump边界和function-local parser producer，bytecode 3/3、parser 1/1，OOM只复用`ac6aa9a3`的`resolve_labels.run`事务证据；✅ `88fe654a`删除无production producer的`fuseIncLoc` prepass及其2–3个allocation/copy/OOM面，让inc/dec_loc只由canonical `resolve_labels` matcher负责，并补齐post/prefix source回映，inc focused为bytecode 2/2、parser 1/1；✅ `03546bd3`加`88fe654a`把add_loc归一为唯一canonical matcher与五类wide RHS域，add focused为bytecode 2/2、parser 1/1；⚠️ `ae26826b`只移除logical-chain的32-hop上限并以33-prefix、双branch/source/interior/parser consumer定向4/4锁定这一刀；✅ tail-call producer确认是已记录的deliberate baseline policy，`cd6dec96`以精确过滤1/1补齐method-return current-baseline snapshot且保持`tail_call_method`为零，redundant `to_propkey`确认在两侧producer census中不可达 → 继续M-EMIT | hoist construction不算peephole；每项只按各自producer/finalizer封账。logical规则仍在zjs `resolve_labels`而pinned QJS位于`resolve_variables`，phase owner及source/OOM等价尚未审完，不封账logical chain或M-EMIT。constant-test、dup-put与inc/dec_loc封账严格限上述`resolve_labels`规则；dead code、logical chain、add_loc、RegExp pattern cpool producer与M-EMIT均未关闭。删除prepass只是缩小`runPhases`的OOM surface，不宣称整个`runPhases` transactional。add_loc-specific source回映仍待，因此即使双入口已归一也不封账add_loc整体；tagged numeric-string/cpool producer及表中其他final-bytecode差异继续逐项裁决。合法zero-length RegExp body不可达只说明它不属于empty short-form，不能据此宣称三producer全等；method-return coverage补齐也不等于QJS final tail exact |
