...
| W5（**进行中；correctness/initial-yield、signed BigInt、numeric及ordinary/empty string discard、三组store规则、`get_length`、put→get、add_loc producer域、tail-call producer policy及不可达`to_propkey`已收口**） | ✅ `fde49b15`以fresh function grammar boundary修复默认参数内generator function expression，正例返回`1`且外层direct-yield负例仍与qjs同为`SyntaxError`；✅ `b8f7e0d2`以canonical `OP_initial_yield`替换四-op marker并删除`generatorBodyPc`扫描，保持sync/async-generator suspended-start及ordinary async/fixture pc 0入口；✅ `b39eb39c`以`resolve_labels`相邻peephole折叠signed `push_bigint_i32 + neg`并保持MIN/cpool/source边界；✅ `08e4bbdb`按QJS producer/finalizer边界完成numeric discard和通用tail `drop + return_undef`；✅ `2b5a1129`恢复ordinary/empty string的owned `push_atom_value` phase-1 producer，由`resolve_labels`删除non-tagged discard pair并把live empty缩为`push_empty_string`；✅ 复核`8a09960f`已按QJS phase覆盖discarded indexed/ref、field及post-update store，2026-07-24 `discarded`窄门禁5/5全绿；✅ `ac6aa9a3`证明ordinary/optional value的late `get_length` fold与method consumer边界、atom ledger/source remap及四分配点同Bytecode OOM retry，事务声明只覆盖`resolve_labels.run`；✅ `02c486e5`锁定put/get四族、source marker与idx/jump边界，OOM复用同一`resolve_labels.run`事务证据且不申报PMU；✅ `03546bd3`让add_loc两入口共享精确QJS RHS predicate，排除cpool Number与tagged numeric-string并锁idx边界，证据为bytecode 2/2、parser 1/1；✅ tail-call producer确认是已记录的deliberate baseline policy，`cd6dec96`以精确过滤1/1补齐method-return current-baseline snapshot且保持`tail_call_method`为零，redundant `to_propkey`确认在两侧producer census中不可达 → 继续M-EMIT | hoist construction不算peephole；每项只按各自producer/finalizer封账。tagged numeric-string/cpool producer、empty-string short-form phase等价及表中其他final-bytecode差异仍待逐项裁决；add_loc的双入口/phase owner合并及add_loc-specific source/jump/OOM证据仍未完成，method-return coverage补齐也不等于QJS final tail exact，禁止宣称add_loc整体或M-EMIT完成 |
