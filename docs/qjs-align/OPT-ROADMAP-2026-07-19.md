...
M-EMIT已确认的源码/phase/consumer残项至此归零；阶段末test262/OOM/checkpoint与总体门禁
完成前仍只称“机制完成、门禁待验”，不得提前关闭W5或整份路线图。
...
dead CFG/final-bytecode 的两个独立子机制现已封账：
...
- `4b2edf67` 对齐 pinned QuickJS `find_jump_target`：threaded `goto` 指向
  ...
  `drop` 后的 `return_undef` 时同样折为 `return_undef`；普通 relocation、jump threading 与
  `goto8` 由显式 non-terminal fixtures 独立保留。证据为 `resolve_labels` 63/63、
  `test-bytecode` 162/162 与 `git diff --check`。

这里只关闭上述 dead graph reachability 与 goto-terminal final-bytecode 两项；不据此关闭整个
dead-code family 或 M-EMIT。logical chain 的 phase owner 仍独立裁决；tagged
...
| W5-emit-audit-remaining（**机制完成，阶段门禁待验**） | 已确认源码/phase/consumer残项归零 | 本行与上面完成行取代W5主行的旧状态；阶段末test262/OOM/checkpoint及总体门禁完成前，M-EMIT/W5仍保持开放 |
