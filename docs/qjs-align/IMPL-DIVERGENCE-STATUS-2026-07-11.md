...
- 基准：历史标准 L0–L5 分层集 + P_tdz/P_ifobj 探针 + 当时新增的 objlit_var/objlit_let churn 对照对（脚本均为包函数形态，防顶层变量污染）。原始 20 项脚本仍未从 job 临时目录恢复；本轮另补的 85 个固定脚本不能反向补足历史样本，因此表中旧数据只能用于提出假设，不能用于验收改动。
...
- Debug unified：最新工作树 1366/1366；`checkpoint-check` 最近阶段 25/25 steps。
- `quick-check` 16/16、`checkpoint-check` 25/25、architecture/API snapshot、OOM-cap 2/2：通过。
- alternate representation：最新工作树 1366/1366。
...
- ReleaseSafe unified：1366/1366，10/10 steps。
...
| 优先级 | 未对齐边界 | 当前证据/影响 |
|---|---|---|
| P0 | 调用帧、arguments 读取与闭包访问 | qjs 的 `JS_CallInternal` 单体路径与寄存器驻留仍未被 zjs 的 Machine/Frame、tail-dispatch 分段路径同构替代；四个普通调用固定脚本仍为 1.62–1.73x。Function.call 已从 6.43x 收到 1.66x；strict plain 为 1.62–1.67x；缺参为 1.59–1.62x；对象 method exact/missing 为 1.44–1.53x，均回到共同前沿。arguments 的 shape/dense/var-ref/序言绑定结构已经对齐，热用例由约 4.27–5.22x 收到 2.21–2.57x；cold 创建仍为 1.68–1.76x，且热读相对 cold 仍比 qjs 多约 50ms/百万次，说明对象创建、局部读取/字段读取与共同帧固定税仍未同构。 |
