# zjs ↔ qjs 实现差异现状全景与整改结果（2026-07-11，更新至 2026-07-15）
...
- [整改前结构·已闭合专有 setup] **method 调用/strict/缺参走重路径 setupInlineEntry**（simple-frame 原先要求 sloppy+无 receiver+argc>=arg_count）。当前 strict plain exact-arity 已用独立预计算资格和 strict setup 实例闭合；缺参由 outlined selector 进入 padded simple-frame；对象与 primitive receiver 都以 exact/padded × snapshot/no-snapshot 四种实例把 raw receiver 直接移入同一 frame，sloppy ToObject 延迟到真正观察 `this` 的 opcode/closure/eval 路径。
...
> 整改后：rc==1 named-shape 原地追加已实现。属性读专项显示 64 个 delete 墓碑对静态/计算读 instructions 几乎无可测惩罚，因此没有冒险做物理 compact；实际热点是计算键反复 `internAtom/free`。`String.internAtom` 先查 back-pointer，`get_array_el` 可借用 live cached atom 后，5M computed-read 约 4.311B→3.150B instructions、756M→578M cycles；静态读约 1.525B instructions，未见回退。另一个独立热点来自 borrowed-reference holder 的生命周期标志误伤普通属性读取；该登记现在不再设置 semantic slow-property 标志，函数、内建 prototype 与 wrapper 均由其真实 class/exotic 状态决定能否走普通 shape/prototype data-property 探测。20M `one.call` 读取由 qjs 的 4.87x 收到 1.14x，5M `Number.prototype.marker` 对象控制由 6.09x 收到 1.00x。
...
arity-pad 跟进用 `call-zero-one-arg-10m.js`、`call-missing-one-arg-10m.js`、`call-missing-two-arg-10m.js` 和 `call-strict-missing-one-arg-10m.js` 把 exact 与缺参拆开。此前未入库的缺参探针由固定脚本替代；最终冻结 baseline/current/qjs SHA 分别为 `4a01441e…`/`3d62093e…`/`b76d1542…`。CPU19、ReleaseFast、9 轮三方轮换中，缺一参为 486.667→340.583ms、qjs 210.314ms（2.31x→1.62x，−30.0%），缺两参为 440.469→331.524ms、qjs 208.324ms（2.11x→1.59x，−24.7%），strict 缺一参为 445.880→335.018ms、qjs 209.929ms（2.12x→1.60x，−24.9%）。有效的 exact control `one(0)` 为 336.855→328.826ms（−2.38%），普通/strict call0 分别为 +0.44%/−0.31%；独立前一重建的三个 exact controls 为 +0.99%/−0.05%/+0.78%，未见跨重建一致回退。一度尝试的 `one(undefined)` 控制会发可观察的全局 `undefined` 读取，已从反馈环删除。根因是 `canBorrowSourceArgs` 在 `argc < arg_count` 时把整个调用送进 `Frame.init → FrameSlab → initFrameLocals → initArgumentsMoved`，而 qjs 只把 `arg_count` 槽放在同一 `local_buf` 前部。当前 outlined fallback 选择 sloppy、strict 或 strict+snapshot 三个 padded specialization：最后一个可失败分配完成后把实际参数所有权移入 slab 前缀，只初始化缺失尾部，并保持 `actual_arg_count` 为真实 argc。公共 `pushFrame` 仍为 0x210，既有三个 setup 仍为 0x374/0x36c/0x49c；fallback 由 0x58 增到 0xf4，最终新增 padded bodies 为 0x3fc/0x3ec/0x550。由此专有 arity-pad 税已闭合，1.59–1.62x 的剩余值回到普通调用共同前沿。

method receiver 跟进用 8 个固定脚本把对象 receiver、属性读取、exact/missing、strict、arguments 和 primitive 拆开；最终冻结 baseline/current/qjs SHA 为 `3d62093e…`/`0e4a4b56…`/`b76d1542…`。CPU19、ReleaseFast、对象组 9 轮三方轮换中，exact method 为 518.753→395.463ms、qjs 274.558ms（1.89x→1.44x，−23.8%），missing 为 538.428→415.029ms、qjs 275.079ms（1.96x→1.51x，−22.9%），strict missing 为 533.277→416.115ms、qjs 272.757ms（1.96x→1.53x，−22.0%）；同属性读取但强制 plain call 的控制为 −0.65%，直接 call0 为 +0.09%。qjs `OP_call_method` 仍把 raw `this_obj` 交给同一 `JS_CallInternal`；zjs 原先却因 `has_receiver` 无条件退回通用 frame、提前做 `coerceCallThis` 和 receiver ownership 判定。当前 outlined selector 对对象 receiver 与 strict primitive 选择 exact/padded × snapshot/no-snapshot 四种 method setup，在最后一个可失败点后把 receiver 从调用区移入 Frame；`SimpleTeardownKind` 让返回热路只在 method frame 释放 owned `this`。公共 `pushFrame` 保持 0x210，既有 plain setup 尺寸保持；fallback 由 0xf4 增到 0x1bc。初版把 borrowed/owned teardown 各内联一份，曾令 `op_return` 0xc94→0x1414；最终合并为单一 simple teardown 后只到 0xdb4（+0x120），上述 plain controls 随之保持中性。

bytecode execution-view memo 跟进冻结当前阶段起点 `ba90dc11…`、最终 ReleaseFast 工作树 `e28372cc…` 与 qjs `b76d1542…`。qjs 的 `JSObject.u.func.function_bytecode` 直接指向可执行结构；zjs 虽已把约 300B `Bytecode` view 缓存在 FB 的 DebugInfo，过去每次 `resolveInlineTarget` 仍要执行 `fb.debug → dbg.cached_view` 两跳。FunctionPayload 同时服务 native/bytecode class，而这两类 callable 互斥使用调用缓存；当前用单指针 `extern union` 让 native function 保持 `InternalRecord` memo，bytecode function 首次调用后在同一 8B 槽保存 non-owning view 指针，payload 尺寸不变。公开 writable FB slot 会在 bytecode class 上先失效 memo，下一次调用重建，避免 embedding 替换 FB 后悬空。完整 test262 首跑在 `Promise.allKeyed/allSettledKeyed` 的 promise-job 尾方法调用暴露了既有 native method 快路只按 `.function` payload kind 分流：bytecode function 也使用该 payload，非空 view 因而曾被误读为 `InternalRecord`；当前按 qjs 的对象 union 规则先以具体 `c_function` class 判别，`nativeRecord` accessor 也拒绝非 native class。两个原崩溃文件、索引 12001–12500 的 500 项窗口及完整 49,775 项 gate 均恢复。CPU19、ReleaseFast、21 轮三方轮换的最终重建中，plain call2 为 366.585→358.960ms、qjs 213.676ms（−2.1%，1.68x），strict call2 为 359.353→356.397ms、qjs 209.048ms（−0.8%，1.70x），closure call2 为 422.680→417.242ms、qjs 241.032ms（−1.3%，1.73x），缺一参为 348.006→336.875ms、qjs 210.605ms（−3.2%，1.60x），method exact/missing/strict-missing 为 408.770→392.096ms、420.963→406.398ms、432.441→408.417ms，qjs 261.549/273.343/274.880ms（−4.1%/−3.5%/−5.6%，1.50x/1.49x/1.49x）。无调用控制 +0.35%，native Array.push 控制 +1.20%；崩溃修复前候选与最终修复版对该 native 控制的直接配对为 282.647→283.552ms（+0.32%），没有显示 class guard 的独立退化。另两份独立 cache 构建全部同方向但幅度约 1–6%，因此这里只记录最终重建、不择优报告。冷 miss handler 拆分和 LIFO Entry 地址 memo 两个候选均未跨重建复现，method guard 前置又令 plain +4.7%，三者均已撤回。

method-frame 阶段的矩阵也把两个更大的边界从 frame setup 中剥离：strict method + arguments 对象只由 1518.789→1440.173ms（−5.2%），qjs 302.497ms，仍为 4.76x；说明 setup 收益存在，但对象物化/映射本体主导。当时 primitive prototype 的强制 plain-property control 是 683.643ms vs qjs 132.975ms（5.14x）；sloppy primitive method 因 eager ToObject 为 1435.832ms vs 130.361ms（11.01x），strict primitive 即使复用 method frame仍为 912.083ms vs 130.247ms（7.00x）。这些结果把 primitive lookup 与 receiver boxing 确立为随后独立处理的前沿，不能被 1.44–1.53x 的对象结果遮蔽。

arguments 对象跟进冻结 `03b5667f` 起点、最终候选与 qjs 二进制 SHA 为 `80cd0395…`、`2e009956…`、`b76d1542…`，用 8 个固定脚本拆分 strict/sloppy、method/plain、exact/missing、对象热读与仅在未命中分支引用的 cold 形态。qjs 的 `js_build_arguments/js_build_mapped_arguments` 从 realm-owned 三属性 shape 直接建对象：unmapped 索引存 `JSValue[]` dense values，mapped 索引存 `JSVarRef*[]`；zjs 原先每次重建三个普通属性和每个 numeric property，并为 mapped 对象另分配 payload。当前 realm 隐藏 template 固定最终 shape 和 `length`/intrinsic `Symbol.iterator`/`callee` 槽，实例只复制属性槽；unmapped 索引进入对象 dense storage，mapped var-ref/value 索引进入对象 union，define/delete/seal/freeze 时再按需要物化并断开映射。裸 `arguments` 也不再每次发 `special_object`：与 qjs 一样由函数序言创建一次并存入隐式局部，修复了 `arguments === arguments` 为 false 和 `arguments.callee` 写入落在临时对象上的回归。CPU19、ReleaseFast、9 轮三方轮换中，strict method missing 为 296.658→151.462ms、qjs 61.521ms（4.82x→2.46x，−48.9%），strict exact 为 317.477→166.499ms、qjs 66.604ms（4.77x→2.50x，−47.6%），sloppy method missing 为 294.672→152.231ms、qjs 65.974ms（4.47x→2.31x，−48.3%），sloppy exact 为 340.515→175.948ms、qjs 79.662ms（4.27x→2.21x，−48.3%）；plain strict missing 为 281.641→138.621ms、qjs 53.981ms（5.22x→2.57x，−50.8%）。exact strict 的 `--perf-json` property lookup 由 1,000,028 降到 29。cold missing/exact 则由 49.112/58.207ms 变为 97.399/111.214ms，qjs 57.931/63.211ms：旧 zjs 只在分支真正读取时惰性创建，虽快却违反绑定身份；当前序言创建是有意接受的 qjs 语义成本，cold 的 1.68–1.76x 与 no-arguments 控制的 1.52x 把剩余值定位为对象创建、读取及共同调用固定税，而不是继续保留惰性捷径。

`OP_get_length` 跟进冻结上述 arguments 阶段二进制 `2e009956…`、最终工作树 `7bc1fb52…` 与 qjs `b76d1542…`，并新增 own/inherited/accessor/Proxy 四个固定 `length` 脚本。qjs 的 `OP_get_length` 直接复用 `GET_FIELD_INLINE(..., JS_ATOM_length)`：字符串与数组专用腿之后仍先做普通 shape walk；zjs 原先把所有其他对象发布到 `getLength → getValueProperty` 冷路。CPU19、ReleaseFast、9 轮三方轮换中，strict exact 为 165.843→115.254ms、qjs 66.639ms（2.49x→1.73x，−30.5%），sloppy exact 为 176.605→126.569ms、qjs 79.801ms（2.21x→1.59x，−28.3%）；strict/sloppy missing 与 5M strict missing 下降约 29.8–32.6%。cold exact/missing 与 no-arguments 控制只在约 −0.1% 到 +2.5% 的重建漂移内。普通 own/inherited data `length` 为 126.748→68.419ms、284.122→75.897ms，qjs 65.125/71.622ms，即 1.05x/1.06x；accessor 与 Proxy 保持原慢路，当前分别约为 qjs 的 4.08x/2.99x，归入 observable call/Proxy 共同前沿。首个把 qjs 的 own-before-exotic 顺序推广到通用字段 helper 的候选触发 mapped Arguments computed-key 4 条 test262 回归：zjs 的 mapped numeric binding 存在 shape 外，而 qjs 用 shape `VARREF` 表示；最终实现因此只对不可能别名 numeric binding 的常量 `length` 使用该顺序，四个原红项 4/4 与 arguments/Proxy/get/Reflect/get/TypedArray length 联合切片 317/317 均恢复。

`OP_get_length` action 跟进冻结上述 `7bc1fb52…` 二进制、最终工作树 `ba90dc11…` 与 qjs `b76d1542…`，新增 no-trap Proxy、typed accessor 与 typed length/byteLength/byteOffset 五个固定脚本。qjs 在 data miss 后仍让同一 `GET_FIELD_INLINE` 进入 `JS_GetPropertyInternal`；zjs 原先却发布整个 VM 并递归调用 getter/Proxy。当前 length miss 尾跳到独立 resident action handler：getter 复用 same-Machine 零参数 method frame，Proxy 复用同一 post-call invariant continuation；1-byte/5-byte 静态 Proxy 共享一份运行时宽度 handler，避免复制约 0x1758 的大体。最终 21 轮三方轮换中，accessor 为 109.887→45.307ms、qjs 26.970ms（4.07x→1.68x，−58.8%），constant trap Proxy 为 110.338→68.347ms、qjs 36.388ms（3.03x→1.88x，−38.1%），no-trap Proxy 为 40.842→31.991ms、qjs 24.592ms（1.66x→1.30x，−21.7%）。own/inherited data 为 −0.3%/+1.0%，普通 static accessor/constant Proxy/no-trap Proxy 三个控制分别为 +0.2%/+0.9%/+0.4%，均在多轮重建漂移内；热 `op_get_length` 反汇编由 0x3d8 缩至 0x3d0，普通 static action tail 仍与冻结基线同为 0x6b0，没有用扩大共享 action handler 换取 typed-array 收益。

同一反馈环的语义差分发现 typed array 自定义 own `length` accessor 被 zjs 的 synthetic length 提前遮蔽，而 qjs 始终先探 own shape；进一步审计又确认替换或清空实例原型时，旧 zjs 仍错误返回 class payload，qjs 则遵循当前原型链。当前 generic resolver 不再合成这三个 named own value；typed 专用 action 依次检查 own shape、当前 prototype chain，并只在首个命中仍是标准 `.buffer` 域 typed accessor record 时直接读取 payload。该 action 由 payload atom 与连续 TypedArray class-id 的廉价前置判断路由到独立尾处理器，不进入普通 static action 的栈帧。static/computed/Reflect.get 的 own shadow、自定义原型 15/16/17、null 原型 `undefined` 与桥接 intrinsic 原型 2/2/0 均和 qjs 一致。正常 typed `length` 为 31.770→29.613ms、qjs 28.315ms，`byteLength` 为 62.882→34.950ms、qjs 32.108ms，`byteOffset` 为 62.051→34.720ms、qjs 31.132ms；typed own accessor 当前 45.137ms、qjs 26.855ms，旧基线结果不正确故不作性能对照。getter receiver/throw/alias、undefined getter、Proxy receiver/no-trap/invariant/revoked、mapped accessor、typed shadow/prototype 的三方差分与 leak-check 均通过，相关 changed-area test262 为 379/379。

primitive lookup 跟进新增 `property-read-primitive-prototype-5m.js` 与 `property-read-primitive-builtin-5m.js`，并冻结 `0a3f4f4e` 起点二进制（SHA `0e4a4b56…`）与 qjs（`b76d1542…`）。修复前 9 轮中位数中，自定义 `Number.prototype` 数据属性读取为 480ms vs qjs 67ms，标准 `toString` 读取为 336ms vs 115ms；源码对照确认 zjs 每次 general primitive read 都经 `primitiveObjectForAccess` 分配 wrapper，而 qjs `JS_GetPropertyInternal` 直接从 `ctx->class_proto[tag]` 开始 walk。单变量语义探针又发现 Proxy 位于 `Number.prototype` 父链时，qjs 调用 `get` trap 并传入原始数字 receiver，zjs 原递归 descriptor 路径却返回 `undefined`。当前实现统一选择 realm intrinsic primitive prototype（补齐 BigInt/Symbol 缓存），从 prototype 本身开始做 data/accessor/exotic-aware walk，并始终把 primitive 作为 getter/Proxy receiver；wrapper 仅保留给真实 ToObject。最终 9 轮三方轮换中，自定义读取 478→158ms、qjs 67ms（7.13x→2.36x，−66.9%），builtin 读取 337→272ms、qjs 115ms（2.93x→2.37x，−19.3%），plain-property call 688→305ms、qjs 133ms（5.17x→2.29x，−55.7%）。

sloppy receiver 跟进把普通 normal bytecode frame 的 raw `this` 保留到首次观察：`OP_push_this`、arrow lexical capture 和 direct eval 共用一次性 frame materialization，因此同一次调用中的直接 `this`、arrow 与 eval 仍看到同一 wrapper；strict/arrow lexical 值保持原样。simple method selector 不再排除 sloppy primitive，general inline 与 recursive normal 路径也不再在入帧前 boxing。最终同一 9 轮轮换中，sloppy primitive method 1436→317ms、qjs 132ms（10.88x→2.40x，−77.9%），strict primitive 914→320ms、qjs 136ms（6.72x→2.35x）；对象 method 控制 397→395ms（−0.5%），plain-property 控制 393→393ms。primitive 的 lookup/normal-call 专有重路径因此已经闭合；该阶段的 2.29–2.40x 残差被定位到 primitive field 冷路径与普通调用共同前沿，并由下段 field-dispatch 跟进继续拆分。async/generator 的 `this` 目前存放在可 suspension 的外部状态，为维持跨 resume wrapper identity 仍保留 eager coercion，未把 normal-frame 结论外推到该路径。

primitive field 分派跟进补入 5 个固定脚本，把普通对象继承读、`Number.prototype` 对象控制、Number wrapper、computed primitive key 和 refcounted function 值分别隔离。第一根因是 `registerBorrowedReferenceHolder` 把纯生命周期登记同时写成 `needs_slow_property`，使 `defineConstructor` 关联 realm 后的内建 prototype 无条件退出普通 shape 快路；当前登记只维护 holder 表，真正的 slow 资格仍由 class/exotic/proxy 状态建立。第二根因是热 `op_get_field` 只接受 object，primitive 即使是普通 prototype data hit 也必须 publish VM 状态并进入通用 resolver；当前 `get_field/get_field2` 对 primitive 尾跳到独立 resident handler table，从 realm prototype 做与 qjs 同构的普通 data-property walk，accessor、auto-init、Proxy/exotic、string index/length 和 private name 仍退回完整 resolver。静态 getter 与 prototype-chain Proxy 回归确认慢路继续收到原始 primitive receiver。
...
computed-property 分派跟进新增 8 个固定脚本，把 own/inherited/missing、int 与 string 数组索引、普通与 primitive accessor/Proxy 分开。qjs 的 `JS_GetPropertyValue` 遇到 string key 后直接把同一个 JSString 当 atom 交给 `JS_GetProperty`；zjs 虽已有 String→Atom 弱回指，热 `op_get_array_el` 此前却只处理 int dense-array hit，其余 string key 仍发布 VM、进入 `arrayElement` 和通用 resolver。当前 int dense-array 腿保持第一分支；atom-backed string 由独立 resident tail handler 完成普通 object/array/global 的单次 self+prototype walk、字符串化数组索引以及 primitive realm-prototype walk。data/missing/index 结果在 frameless 路径完成 ownership 转移；accessor 与首个 Proxy holder 只在确定 action 后发布栈，并分别复用现有 getter call 和 Proxy trap 语义。回归覆盖原始 receiver、getter/Proxy 抛出、undefined getter，以及结果与 receiver/key 别名时的引用所有权。
...
getter/Proxy action 跟进新增 10 个固定脚本。空 getter 5M 控制先证明“不调用”时 zjs/qjs 仅约 1.15x；真正的 getter 残差来自 `get_array_el` 已找到 getter 后仍递归进入 `callValueOrBytecodeClassModePreRooted`，而普通 bytecode call 已能留在当前 Machine。当前 computed/static getter 都把 `[receiver,getter]` 作为零参数 method region 交给 same-Machine frame；static `get_field` 的 data handler 保持独立，只有 miss 尾跳到 accessor/Proxy walker。Proxy 侧按 qjs `JS_ATOM_get` 使用预定义 atom，plain handler data/missing 与 no-trap plain target data 直接 shape probe；trap 返回后的 plain target invariant 也直接读取最新 own-property flags/value，只有 varref、auto-init、Proxy/exotic 才物化完整 Descriptor。回归明确覆盖 handler 的 `get` 自身为 accessor、no-trap target getter 的原 receiver、trap 抛出，以及 trap 内把 target 冻结后才触发 invariant 的顺序。
...
Array.pop 使用新增固定脚本 `array-pop-empty-5m.js` 做 CPU19、ReleaseFast、9 轮轮换顺序墙钟配对；当前容器禁止 PMU 访问，因此这组数据只作同机 A/B，不冒充 instructions/cycles 证据。最终源码的冻结起点 zjs 中位数 618.509ms，当前 392.214ms，qjs 238.340ms，故 zjs/qjs 由 2.60x 降到 1.65x（当前相对起点 −36.6%）。单做 per-method record 只有约 −2.8%，主收益来自真实空 Array 直接读写不可观察的 length 槽；非空真实 Array 也直接读 length，并在 getter 未把当前 length 扩到原值之外时直接完成最终缩短。若 getter 扩容、receiver 是 Proxy 或普通 array-like，则仍走完整 Get/Delete/Set，确保最终缩长会清掉 getter 新增的高位元素且保留 non-writable 抛出顺序。该阶段的提取式 `pop.call([])` 与普通 array-like 探针曾约为 qjs 的 2.98x/3.89x，由此触发了下一段独立的 Function.call 反馈环；把通用慢路强制拆成 noinline 对 array-like 反而约 +2.5%，已撤回。新增 Zig 回归覆盖 frozen empty Array、Proxy traps、普通 length accessor、Array subclass、getter 中途扩容，以及 qjs 的只读 length 错误文本/删除顺序；目标 test262 目录 23/23。

Function.call 使用新增固定脚本 `function-call-zero-arg-5m.js` 做 CPU19、ReleaseFast、9 轮三方轮换墙钟配对：冻结 `a3234c68` 起点 zjs 中位数 1108.234ms，当前 285.819ms，qjs 172.453ms，故 zjs/qjs 由 6.43x 降到 1.66x（当前相对起点 −74.2%）。独立的 `function-prototype-call-read-20m.js` 又把“继承属性读取”从调用体中拆出：same-Machine forwarding 已落地但属性快路尚未修复的冻结二进制为 1855.829ms，当前 434.555ms，qjs 381.290ms，即 4.87x→1.14x（−76.6%）。根因不是 per-domain switch：只把 `call` 拆成独立 record 仅约 −0.4%；真正的第一段收益来自按 qjs `js_function_call` 直接转发 `argv + 1`、复用 cached bytecode view，并让符合资格的同 realm 普通 bytecode 目标留在当前 Machine。第二段根因是函数持有 borrowed realm global 时，纯生命周期簿记设置的 `needs_slow_property` 被属性读取误解为 exotic 语义，导致函数和 `Function.prototype` 在 shape 探测前就退出；该阶段先以非 Proxy、无 exotic methods 的标准函数例外验证了收益，本轮 primitive-field 跟进则删除了根因标志，让 semantic slow 资格只由真实 class/exotic 状态建立，accessor/auto-init/var-ref 仍按现有三态查找退回通用路径。为透明 forwarding 跳过的 `call` record 保留 owned synthetic native frame，所以目标内部捕获的栈仍是 `target → call (native) → caller`。删除原 native backtrace scope 的对照约慢 0.7%，已撤回；未改的直接调用控制为 314.690→315.966ms（+0.4%，中性）。当前 1.66x 已回到普通调用 1.59–1.73x 的共同前沿，不外推为调用帧架构已同构。
...
Promise identity 跟进冻结 `05f0ddd3` 起点、最终候选与 qjs 二进制 SHA 为 `e2867a39…`、`0e5885bf…`、`b76d1542…`，用新增 constructor-read、提取 `.call`、native Object identity 与 JS method identity 四个脚本做 CPU19、ReleaseFast、9 轮三方轮换。`promise-resolve-same-1m.js` 为 113.480→73.428ms、qjs 40.403ms，即 2.81x→1.82x（−35.3%）；提取 `.call` 为 123.901→84.194ms、qjs 52.274ms。根因是 qjs `JS_GetProperty(argv[0], JS_ATOM_constructor)` 直接走 Promise 自身的普通 shape/prototype 链，而 zjs 的 identity body 每次进入兼容 prototype-less internal Promise 的通用 `getValueProperty`；当前先做不可重入的 ordinary data walk，accessor、Proxy/exotic holder 和链尾 missing 仍落回权威 Get。constructor-read 51.211→51.125ms、JS method identity 65.806→65.816ms、native Object identity 125.628→126.768ms、call0 319.716→319.876ms，均未随收益变化。把 identity 再复制到 record 入口的单变量实验为 73.660→73.587ms，已撤回；这证明剩余值已回到共同 method/native-call 前沿，不支持继续扩大 Promise record 特化。Zig 回归覆盖 own accessor/data、data mismatch、Proxy prototype 和 getter throw；Promise/resolve test262 为 30/30。

Object constructor 跟进冻结 `8fab730e` 起点、最终候选与 qjs 二进制 SHA 为 `38a47aad…`、`80cd0395…`、`b76d1542…`，新增全局/提取 plain-call、普通对象类型控制、null/number 分配和全局/提取 direct-construct 六个固定脚本，最终做 CPU19、ReleaseFast、9 轮三方轮换。全局 identity 为 126.165→64.465ms、qjs 30.479ms（4.14x→2.12x，−48.9%），提取 identity 为 112.381→51.670ms、qjs 29.874ms（3.76x→1.73x，−54.0%）；null 为 165.080→103.099ms、qjs 51.765ms（3.19x→1.99x，−37.5%），number 为 173.778→108.752ms、qjs 54.522ms（3.19x→1.99x，−37.4%）；全局 direct construct identity 为 154.766→69.497ms、qjs 28.169ms（5.49x→2.47x，−55.1%），提取构造为 142.077→56.737ms、qjs 27.495ms（5.17x→2.06x，−60.1%）。qjs `js_object_constructor` 是 `JS_CFUNC_constructor_or_func`：只有 `new_target !== active_function` 才 `js_create_from_ctor`，否则 nullish 新建普通对象、其余直接 `JS_ToObject`。zjs 原先没有 Object constructor record，plain call 和 direct construct 都借可见 dispatch name 进入完整 `constructValue` rooting/prototype/name cascade。当前 Object 函数带 construct-capable record；plain call 与 active-function direct construct 直达共享 body，custom new.target、subclass 和 Proxy construct 仍在进入 record 前走原建对象分支。只做“对象参数直返、其他类型回通用构造”的中间候选虽已把 identity 收至 64.635ms，却让 null/number 仍为 155.263/159.386ms，并使 direct construct 154.890→159.705ms；该不完整方案已被完整双态入口取代。完整 test262 首跑又暴露声明表原先用 `id >= 100` 派生 Object prototype 方法，导致 id=100 的 constructor entry 被错误安装为 `Object.prototype.Object`，继而破坏 Proxy `with` HasBinding、Annex B 名字解析和 builtin-no-construct；当前改为按 `PrototypeMethod` 精确判定，四个原红项 4/4 与完整 gate 0/49775 均恢复。最终提取 plain-call 的 1.73x 已回到共同 native-call 前沿；null/number 的约 1.99x 还叠加对象/primitive wrapper 分配与 RC，提取 direct construct 的 2.06x 则仍含统一 construct 分派固定税，不外推为整个构造机制已同构。Zig 回归覆盖原型表隔离、对象身份、所有 primitive wrapper、可见 name 改写、direct construct、Reflect custom new.target 和 Object subclass；Object test262 为 3411/3411。
...
- computed-property changed-area：在上述 2121 项基础上追加 Proxy/get 19、Object/defineProperty 1131、Object/create 320，联合 3591/3591；Zig 回归另覆盖 inherited data、getter/Proxy receiver 与抛出、undefined getter、null-prototype missing，以及 receiver/key/result 的引用别名。
...
- full test262：Object.u.func 完整下沉工作树的默认 8B repr 准备 49,775/53,293，排除 3,518，按 feature 跳过 5,174；通过 44,599，known 2，unexpected 0，`test262-gate` 5/5 steps；alternate repr smoke 12/12、unified 1403/1403。
...
另有明确的反向差异：zjs 通过 staging 的 `Function/arguments-parameter-shadowing.js`，当前 qjs 参照失败。这里保留 test262 所要求的参数环境/函数体同名 `arguments` 分离，不为了逐 bug 复刻 qjs 而制造回退。

direct-eval lvalue 快照也是必须单列的反向差异：仓库既有普通函数回归要求在 RHS eval 执行前固定 identifier assignment 的 LHS reference；本轮让 block-bodied arrow 使用相同预扫描契约。合成用例 `x = (eval("var x;"), 1)` 中，当前 zjs 返回新 eval binding 的 `undefined` 并写入外层 `x`，当前 qjs 则在 eval 后命中新 binding、返回 `1` 且不写外层。这里记录的是既有语义契约的一致化，不能作为 zjs 与当前 qjs 已对齐的证据。
