...
>
> **证据边界**：原始 20 项 best-of-5 仍只有聚合结果，不能回推方差或严谨 IPC。本轮把字符串、属性、peephole、调用、函数生命周期、Array.push/Array.pop 与 Promise.resolve 的 30 个固定脚本补入 `tests/perf/qjs-align/`，但尚未重建原历史 20 项全集和所有 perf.data；新旧数字必须分开解释。
...
- [整改前结构·部分闭合] **method 调用/strict/缺参走重路径 setupInlineEntry**（simple-frame 原先要求 sloppy+无 receiver+argc>=arg_count）。当前 strict plain exact-arity 已用独立预计算资格和 strict setup 实例闭合；method 与缺参仍每 call 付全套，而 qjs 只有缺参时的 arity-pad 循环。
...
| 6 | 完成（边界扩大） | 删除 `Machine.switched` 3 写 0 读；strict plain exact-arity 不再绕行通用 FrameSlab/init 路径，而与 qjs 一样复用同构帧布局，并以独立 comptime setup 保持 sloppy 热路无 strict 分支；物化 strict arguments 时由同族 snapshot setup 保存 original args；method、缺参、arguments 对象本体和共同帧固定税仍独立留置 |
...
- Debug unified：最新工作树 1353/1353；`checkpoint-check` 25/25 steps。
- `quick-check` 16/16、`checkpoint-check` 25/25、architecture/API snapshot、OOM-cap 2/2：通过。
- alternate representation：最新工作树 1353/1353。
- OOM injection：最新工作树 7/7。
- `test262-smoke`：12/12；Array/prototype/push 24/24、Array/prototype/pop 23/23；Promise/resolve 30/30；String 四个 case-conversion 目录 110/110；Function 全目录 509/509（其中 call/apply 97/97）；strict/simple-frame 相关 statements/function、expressions/function、expressions/call、Function/prototype/call、arguments-object、staging Function 联合准备 1167/1172、排除 5、通过 1167/1167；此前联合 changed-area：TLA 251、dynamic-import assignment-expression 28、for-await-of 1234、AsyncGeneratorPrototype return 19、Function call/apply 97，合计 1629/1629。staging `Function/arguments-parameter-shadowing.js` 当前 zjs 1/1，当前 qjs 0/1。
...
- ReleaseSafe unified：1353/1353，10/10 steps。

full test262 的阶段首跑曾暴露 IsHTMLDDA callable 判定与 generator finally rethrow marker；C8–C10 的收口首跑又暴露普通 catch 误判 finally、参数环境 `arguments` 同名声明回填两条回归。本次 strict simple-frame 首跑同样暴露两个 original-args snapshot 红项，最终全量已在 snapshot setup 后恢复 unexpected 0。上面的最终结果均不以逐文件复测替代全量门禁。
