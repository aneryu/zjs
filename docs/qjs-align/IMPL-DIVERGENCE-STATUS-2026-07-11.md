...
- 基准：历史标准 L0–L5 分层集 + P_tdz/P_ifobj 探针 + 当时新增的 objlit_var/objlit_let churn 对照对（脚本均为包函数形态，防顶层变量污染）。原始 20 项脚本仍未从 job 临时目录恢复；本轮另补的 44 个固定脚本不能反向补足历史样本，因此表中旧数据只能用于提出假设，不能用于验收改动。
...
- Debug unified：最新工作树 1356/1356；`checkpoint-check` 25/25 steps。
- `quick-check` 16/16、`checkpoint-check` 25/25、architecture/API snapshot、OOM-cap 2/2：通过。
- alternate representation：最新工作树 1356/1356。
...
- primitive lookup/lazy-this changed-area：expressions/this、call、arrow-function、object method-definition、Function.prototype.call，以及 Number/Boolean/BigInt/Symbol/String prototype 联合 2121/2121。
...
- ReleaseSafe unified：1356/1356，10/10 steps。

full test262 的阶段首跑曾暴露 IsHTMLDDA callable 判定与 generator finally rethrow marker；C8–C10 的收口首跑又暴露普通 catch 误判 finally、参数环境 `arguments` 同名声明回填两条回归。strict simple-frame 首跑同样暴露两个 original-args snapshot 红项，最终全量已在 snapshot setup 后恢复 unexpected 0；arity-pad、method receiver 与本次 primitive lookup/lazy-this 阶段都以完整 test262 49,775 个 prepared tests、known 2、unexpected 0 重新确认，本次另以 16B alternate runner 完整复跑。上面的最终结果均不以逐文件复测替代全量门禁。
...
| P0 | primitive field 分派与 suspended receiver | per-read wrapper 和普通 normal-call eager ToObject 已闭合；custom/builtin Number prototype read 仍为 qjs 的 2.36x/2.37x，plain/strict/sloppy primitive call 为 2.29x/2.35x/2.40x，残差主要回到 primitive `get_field` 冷路径、通用 chain walk 与共同调用税。async/generator 的 `this` 仍由 frame 外的 suspension state 持久化并 eager coercion，尚未按 qjs 把 raw receiver 与持久 wrapper 状态统一。 |
