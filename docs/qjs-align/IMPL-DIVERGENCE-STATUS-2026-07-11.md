...
- 基准：历史标准 L0–L5 分层集 + P_tdz/P_ifobj 探针 + 当时新增的 objlit_var/objlit_let churn 对照对（脚本均为包函数形态，防顶层变量污染）。原始 20 项脚本仍未从 job 临时目录恢复；本轮另补的 67 个固定脚本不能反向补足历史样本，因此表中旧数据只能用于提出假设，不能用于验收改动。
...
- Debug unified：最新工作树 1359/1359；`checkpoint-check` 25/25 steps。
- `quick-check` 16/16、`checkpoint-check` 25/25、architecture/API snapshot、OOM-cap 2/2：通过。
- alternate representation：最新工作树 1359/1359。
...
- ReleaseSafe unified：1359/1359，10/10 steps。
...
| P0 | refcounted property、Proxy trap continuation 与 suspended receiver | per-read wrapper、普通 normal-call eager ToObject、静态 data field 冷分派、computed data/missing/index、getter 的递归调用，以及 no-trap Proxy 的通用 resolver 均已闭合；getter 当前为 qjs 的 1.37–1.79x，no-trap Proxy 为 1.40–1.49x。仍开放的是 refcounted custom/builtin function read 约 1.67x、有 bytecode trap 的 static/computed/primitive Proxy 2.37–2.88x，以及 plain/strict/sloppy primitive call 1.68x/1.67x/1.66x。trap 必须返回后再校验 target invariant，因此需要 same-Machine post-call continuation；不得提前或跳过校验。async/generator 的 `this` 仍由 frame 外的 suspension state 持久化并 eager coercion，尚未按 qjs 把 raw receiver 与持久 wrapper 状态统一。 |
