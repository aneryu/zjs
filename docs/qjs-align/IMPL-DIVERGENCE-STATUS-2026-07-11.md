...
- 基准：历史标准 L0–L5 分层集 + P_tdz/P_ifobj 探针 + 当时新增的 objlit_var/objlit_let churn 对照对（脚本均为包函数形态，防顶层变量污染）。原始 20 项脚本仍未从 job 临时目录恢复；本轮另补的 49 个固定脚本不能反向补足历史样本，因此表中旧数据只能用于提出假设，不能用于验收改动。
...
普通 object own/inherited 控制为 65.949/73.187ms，qjs 为 64.715/71.069ms；`Number.prototype` 对象控制由 399.919ms 降到 65.586ms，qjs 65.644ms；Number wrapper 保持 65.897ms，qjs 65.342ms。primitive 分支经 `Vm` 中的常驻函数表间接尾跳，最终 object `op_get_field` 为 0x208，未超过冻结起点的约 0x20c；因此没有用 object handler 膨胀换取专项数字。静态 primitive data 的专有 dispatch 税至此闭合；1.58–1.68x 的剩余值已被 refcounted `dup/free`、computed-key 通路和普通调用共同固定税解释，另有 async/generator suspended receiver 的 eager coercion 仍未对齐。
...
- Debug unified：最新工作树 1357/1357；`checkpoint-check` 25/25 steps。
- `quick-check` 16/16、`checkpoint-check` 25/25、architecture/API snapshot、OOM-cap 2/2：通过。
- alternate representation：最新工作树 1357/1357。
...
- ReleaseSafe unified：1357/1357，10/10 steps。

full test262 的阶段首跑曾暴露 IsHTMLDDA callable 判定与 generator finally rethrow marker；C8–C10 的收口首跑又暴露普通 catch 误判 finally、参数环境 `arguments` 同名声明回填两条回归。strict simple-frame 首跑同样暴露两个 original-args snapshot 红项，最终全量已在 snapshot setup 后恢复 unexpected 0；arity-pad、method receiver、primitive lookup/lazy-this 与本次 field-dispatch 阶段都以完整 test262 49,775 个 prepared tests、known 2、unexpected 0 重新确认，本次另以 16B alternate runner 完整复跑。上面的最终结果均不以逐文件复测替代全量门禁。
...
| P0 | primitive refcount/computed field 与 suspended receiver | per-read wrapper、普通 normal-call eager ToObject 和静态 data `get_field/get_field2` 冷分派均已闭合；custom data read 为 qjs 的 1.03x，但 refcounted custom/builtin function read 仍为 1.67x，computed data read 为 1.58x，plain/strict/sloppy primitive call 为 1.68x/1.67x/1.66x。残差主要回到 `dup/free`、computed-key 通路与共同调用税。async/generator 的 `this` 仍由 frame 外的 suspension state 持久化并 eager coercion，尚未按 qjs 把 raw receiver 与持久 wrapper 状态统一。 |
