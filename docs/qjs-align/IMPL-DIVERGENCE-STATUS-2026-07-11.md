...
- 基准：历史标准 L0–L5 分层集 + P_tdz/P_ifobj 探针 + 当时新增的 objlit_var/objlit_let churn 对照对（脚本均为包函数形态，防顶层变量污染）。原始 20 项脚本仍未从 job 临时目录恢复；本轮另补的 42 个固定脚本不能反向补足历史样本，因此表中旧数据只能用于提出假设，不能用于验收改动。
...
- [整改前结构·大部闭合] **method 调用/strict/缺参走重路径 setupInlineEntry**（simple-frame 原先要求 sloppy+无 receiver+argc>=arg_count）。当前 strict plain exact-arity 已用独立预计算资格和 strict setup 实例闭合；缺参由 outlined selector 进入 padded simple-frame；对象 receiver 与 strict primitive method 也以 exact/padded × snapshot/no-snapshot 四种实例把 receiver 直接移入同一 frame。只有需要 ToObject 的 sloppy primitive receiver 仍走通用 setup。
...
这次矩阵也把两个更大的边界从 method frame 中剥离：strict method + arguments 对象只由 1518.789→1440.173ms（−5.2%），qjs 302.497ms，仍为 4.76x；说明 setup 收益存在，但对象物化/映射本体主导。primitive prototype 的强制 plain-property control 本身就是 683.643ms vs qjs 132.975ms（5.14x）；sloppy primitive method 因仍需当前 eager ToObject 为 1435.832ms vs 130.361ms（11.01x），strict primitive 虽复用 method frame并下降 10.0%，仍为 912.083ms vs 130.247ms（7.00x）。因此对象 method 的专有 frame 税已经闭合，但 primitive prototype lookup、sloppy primitive 延迟 boxing 和 arguments 对象本体必须作为独立前沿，不能被 1.44–1.53x 的对象结果遮蔽。
...
- Debug unified：最新工作树 1355/1355；`checkpoint-check` 25/25 steps。
- `quick-check` 16/16、`checkpoint-check` 25/25、architecture/API snapshot、OOM-cap 2/2：通过。
- alternate representation：最新工作树 1355/1355。
...
- full test262：最新默认 8B repr 与上一阶段 alternate 16B repr 均准备 49,775/53,293，排除 3,518，按 feature 跳过 5,174；通过 44,599，known 2，unexpected 0；`test262-gate` 5/5 steps。
- ReleaseSafe unified：1355/1355，10/10 steps。

full test262 的阶段首跑曾暴露 IsHTMLDDA callable 判定与 generator finally rethrow marker；C8–C10 的收口首跑又暴露普通 catch 误判 finally、参数环境 `arguments` 同名声明回填两条回归。strict simple-frame 首跑同样暴露两个 original-args snapshot 红项，最终全量已在 snapshot setup 后恢复 unexpected 0；arity-pad 与本次 method receiver 阶段又各以完整 test262 49,775 个 prepared tests、known 2、unexpected 0 重新确认。上面的最终结果均不以逐文件复测替代全量门禁。
...
| P0 | primitive receiver 属性与 boxing | 相同 Number prototype 属性读取并强制 plain call 已为 qjs 的 5.14x；sloppy primitive method 因 eager ToObject 为 11.01x，strict primitive 即使 method frame 对齐后仍为 7.00x。qjs 保留 raw `this_obj`、只在 `OP_push_this` 时 boxing；zjs primitive property lookup 与 sloppy receiver coercion 仍是两条独立重路径。 |
