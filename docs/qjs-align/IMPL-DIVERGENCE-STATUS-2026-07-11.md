...
- 基准：历史标准 L0–L5 分层集 + P_tdz/P_ifobj 探针 + 当时新增的 objlit_var/objlit_let churn 对照对（脚本均为包函数形态，防顶层变量污染）。原始 20 项脚本仍未从 job 临时目录恢复；本轮另补的 57 个固定脚本不能反向补足历史样本，因此表中旧数据只能用于提出假设，不能用于验收改动。
...
| 9 | 原刀否决，替代刀完成（边界扩大） | 64 tombstones 无可测 instructions 惩罚，未做 risky compact；cached-string-atom 先消除反复 intern/free，随后 `get_array_el` 的 atom-backed string 分派直接完成普通 data/missing、prototype walk 与字符串化数组索引。own/inherited/primitive computed data 相对本阶段起点约 −23%/−61%/−40%，getter/Proxy 保留完整 observable 行为并单列为调用/trap 前沿 |
...
- Debug unified：最新工作树 1358/1358；`checkpoint-check` 25/25 steps。
- `quick-check` 16/16、`checkpoint-check` 25/25、architecture/API snapshot、OOM-cap 2/2：通过。
- alternate representation：最新工作树 1358/1358。
...
- ReleaseSafe unified：1358/1358，10/10 steps。

full test262 的阶段首跑曾暴露 IsHTMLDDA callable 判定与 generator finally rethrow marker；C8–C10 的收口首跑又暴露普通 catch 误判 finally、参数环境 `arguments` 同名声明回填两条回归。strict simple-frame 首跑同样暴露两个 original-args snapshot 红项，最终全量已在 snapshot setup 后恢复 unexpected 0；arity-pad、method receiver、primitive lookup/lazy-this、field-dispatch 与本次 computed-property 阶段都以完整 test262 49,775 个 prepared tests、known 2、unexpected 0 重新确认，本次另以 16B alternate runner 完整复跑。上面的最终结果均不以逐文件复测替代全量门禁。
...
| P0 | refcounted property、computed accessor/Proxy 与 suspended receiver | per-read wrapper、普通 normal-call eager ToObject、静态 data field 冷分派和 computed data/missing/index 的重复通用解析均已闭合；primitive computed data 为 qjs 的 0.95x，object own/inherited/missing 为 1.04–1.15x，Array string index 为 0.85x。仍开放的是 refcounted custom/builtin function read 约 1.67x、object computed accessor/Proxy 2.71–2.76x、primitive computed accessor/Proxy 2.57–2.67x，以及 plain/strict/sloppy primitive call 1.68x/1.67x/1.66x。残差集中于 `dup/free`、observable getter/Proxy action 和共同调用帧。async/generator 的 `this` 仍由 frame 外的 suspension state 持久化并 eager coercion，尚未按 qjs 把 raw receiver 与持久 wrapper 状态统一。 |
