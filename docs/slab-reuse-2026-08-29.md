...
## 12. 裁决尾注(2026-08-30,受 owner 委托裁定)

§11 移交的两张牌均已闭合:

1. **growth 175%→200%:已批且已落地——本节只做对账确认,不再是待裁项。**
   逐源核查:`runtime.zig resetGCThreshold` 的编译常数已是 2×(注释记录
   owner 于 2026-08-29 将 §1.3 上限重议为 2.0,并引用本文档的 ABBA n=16
   定价);`tracing-gc-design.md` §1.3 行已同步(P/T 门 36/35 与增长因子
   无关,原样保留);且该 commit(`251558fa` 所在 rc-退役系列)是
   2026-08-29 安静窗口测量臂 `6374ba73` 的祖先——**官方读数
   geomean 1.0281 / splay 1.1724 / RSS ~290MB 已经把 2.0 的收益与代价
   全部计入**,与本文档预测(splay ≈1.205 越停线、RSS +12.7%)方向一致
   且更优。无补充动作。

2. **class-3 空值缓冲刀(§9.9,`prop_count==0` 数组仍按 prop_size=2 配
   值缓冲,splay 10.5 MiB):记分口径裁定如下。** 它是真实的通用浪费
   (qjs 同样支付、rc/trace 同样受益),按 2026-08-14 宪章属**通用机制**,
   走 PERF-MECHANISM-LEDGER 以绝对收益立项;但**禁止计入 GC-GAP 记分**
   ——其比值改善只来自参照臂被冻结,是记分伪影不是收集器进步。鉴于
   splay 已越线,该刀优先级降为 backlog,不占当前窗口。
