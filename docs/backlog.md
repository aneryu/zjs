...
| H10 | Separate name from role in `src/parser.zig` (still hosts `compile_entry`) and `src/bytecode.zig` (hosts `pipeline_*` namespaces). The `v2_builder` / `compileFunctionV2` identity names are gone. | Architecture doc admits the remaining filename mismatch | Large project |
