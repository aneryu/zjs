...
## 12. 全族核对完成（2026-08-27）——§7 的估计表作废，以本节为准

owner 令「全部完整核对一遍」。六个族逐条读实现后，**§7 的估计表整体作废**：
它把两类完全不同的东西混在了一起。逐族结论如下。

### 12.1 `with` 族（5 个）——审计估计**成立**，而且比估计的更好

`with_get_var` / `with_put_var` / `with_delete_var` / `with_make_ref` /
`with_get_ref`。核对发现三件事：

1. **四个已经共用同一个 handler**（`tailcall_dispatch_colds.zig:427` 的
   `h_with_get_or_delete`），共同前缀（binding 探测、`@@unscopables`
   拦截、strict 下的 ReferenceError）全部共享，只有尾部
   `switch (opc)` 分道（`vm_property_ref.zig:83`）——**opcode 身份在这里
   纯粹是模式选择器**。
2. **编译器侧本来就有 kind 值**：`bytecode.zig:6097` 的
   `EvalVarObjectProbeKind.probeOpcode()` 是一张
   `{read, delete, put, get_ref, make_ref} → opcode` 的映射表。也就是说
   我们先把一个枚举**转换成** opcode 身份，运行时再**还原回**枚举。
3. **指令已经带一个 u8 操作数**（`vm_property_ref.zig:17` 读 `pc+8` 的
   `is_with` 布尔）。kind 只需 3 bit，与 `is_with` 同字节即可。

结论：**5 → 1，净省 4，且零字节代价**（`atom_label_u8` 尺寸不变）。改动是
把 `switch (opc)` 换成 `switch (kind)`、把 `probeOpcode()` 换成打包
flag。这是全表性价比最高的一项，也是唯一一个「审计估计经核对完全成立」的族。

### 12.2 Reference 族（6 个）——见 §11，**6 → 3**

变体合并（4 个 `make_*_ref` → 1 条带 kind 操作数），不是整族删除。

### 12.3 TDZ 族（9 个）——见 §4.1 修订，**~6**，但成本最高

三个成本因子全中：3 个走尺寸预言机路径、`put_loc_check_init` 有 15 处
测试引用、`set_loc_uninitialized` 有 14 处，合计约 70 个改动点，且动的是
`let`/`const` 的正确性基础。

### 12.4 super 族（4 个）——**机制判错了**：是降级，不是重设计

- `put_super_value`：已于今日降级，编号 72 已释放。
- `get_super`（1/1，0 次）：V8 有对应物（`GetSuperConstructor`）。
- `get_super_value`（3/1，0 次）：JSC/Hermes 用通用 `*_with_this` /
  `WithReceiver` 形式——**但我们没有那种形式**，要引入就得新增一个
  opcode，净省为 0。我们的 `get_super_value` 就是我们的 with-receiver 读。
- `set_home_object`（2/2，0 次）：三家确实都没有，但它们的替代（context
  slot / 私有属性 `@homeObject` / 折进 `CreateBaseClass`）都需要动表示层。

结论：这一族能拿到 4 个编号，但**靠降级**（全部 0 次执行），不靠语义
重设计。审计把数字猜对了、把机制猜错了。

### 12.5 类定义/命名族（6 个）——同样是降级

`set_name` 独占 166 万次（**保留**），其余五个 0~39 次，**全部可降级**。
审计说的「改 flag 操作数」在 V8 成立，因为它有一条通用 `DefineKeyedOwn`
可以挂 flag；我们的 `define_class`/`define_method` 语义并不重合于某条通用
指令，硬合并要先造出那条通用指令。**5 个靠降级，不靠重设计。**

### 12.6 栈洗牌族（9 个）——不可重设计，但 5 个可降级

`dup`(8964 万)、`insert3`(3514 万)、`insert2`(255 万)、`perm3`(166 万)
**保留**；`nip`、`perm4`、`swap`、`rot3l`（均 0 次）、`nip_catch`(43 次)
**可降级**。三家为零是执行模型差异，不是冗余——换掉它得先换执行模型。
...
| 动作 | 变化 | 净省 | 成本 |
|---|---|---|---|
| `with_*` 合并为一条带 kind 的探测指令 | 5 → 1 | **4** | 低：handler 已共用，kind 已存在，零字节代价 |
| `make_*_ref` 合并为一条带 kind | 4 → 1 | **3** | 中：走下降路径，需同步尺寸计算 |
| TDZ 改「原地检查 + 通用访问」 | 9 → ~3 | **~6** | 高：~70 处，触碰 `let` 正确性 |
| 小计 | | **13** | |

**B. 降级到冷平面（不是重设计，机制已验证）**

| 族 | 可降级 | 说明 |
|---|---|---|
| super | 3（+已释放的 1） | 全部 0 次执行 |
| 类定义/命名 | 5 | `set_name` 除外 |
| 栈洗牌 | 5 | 热的四个除外 |
| 小计 | **13** | |

**A+B ≈ 26 个编号**——与 §7 的总数巧合地接近，但**构成完全不同**：
§7 认为其中大部分来自语义重设计，核对后**一半是降级**，而真正的重设计
只有三项，其中只有 `with` 族一项是低成本的。
