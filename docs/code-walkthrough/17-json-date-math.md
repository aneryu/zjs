...
- **实现**：先把入参 root 住，`appendJsonInputString` 把它摊成 UTF-8 字节；先试 ASCII 整段快路径 `parseSimpleJsonValue`，返回非 null 就直接用。快路径 miss 后若原值本身是字符串，`ensureFlat` 再按 `resolveData()` 的 latin1 / utf16 两臂分别实例化 `jsonParseFull(u8)` / `jsonParseFull(u16)`；原值不是字符串（已被强转过）则走 `jsonParseFullFromBytes`。
...
### `JsonParseWithRecord.deinit` (`src/exec/json_ops.zig:208`)
...
### `parseWithRecord` (`src/exec/json_ops.zig:220`)
...
- **实现**：值是字符串时 `ensureFlat` 后按 `resolveData()` 的 latin1 / utf16 臂调 `jsonParseFullWithRecord(u8/u16)`；不是字符串时先 `appendJsonInputString` 摊成字节、`String.createUtf8` 造出真字符串再 `ensureFlat`，走同样两臂。对照 qjs js_json_parse 的 reviver 分支（带活 `pr` 调 JS_ParseJSON3，quickjs.c:49834）。
...
### `jsonParseFullWithRecord` (`src/exec/json_ops.zig:245`)
...
### `jsonParseFullFromBytes` (`src/exec/json_ops.zig:263`)
...
- **实现**：`String.createUtf8` 建串并 `ensureFlat`，按 `resolveData()` 分发到 `jsonParseFull(u8)` / `jsonParseFull(u16)`。
- **所有权 / 错误 / 调用**：中间字符串是 GC 值，不手工释放。调用方：`parse` 的非字符串臂，以及 `rawJSON` 里那次「解析一遍确认合法且不是对象」的校验。

### `jsonParseFull` (`src/exec/json_ops.zig:279`)
...
### `JsonParseRecord.recordValue` (`src/exec/json_ops.zig:319`)
...
### `JsonParseRecord.findObjectEntry` (`src/exec/json_ops.zig:329`)
...
### `JsonParseRecord.arrayElement` (`src/exec/json_ops.zig:341`)
...
### `JsonParseRecord.deinit` (`src/exec/json_ops.zig:354`)
...
- **所有权 / 错误 / 调用**：只释放 native 内存。⚠️函数上的注释说缓存的 `value` 也在这里释放，实际代码并没有——tracing GC 下那些值不需要手工释放（walk 期间由 `JsonRecordRoots` 报根），注释是 rc 时代的遗留；entry 的 atom 同理。调用方：`jsonParseCall` 的 `defer`（1637）、`jsonParseFullWithRecord` 的 errdefer（254）、`parseObject` / `parseArray` 的 errdefer 与 append 失败路径（613 一带）、`JsonParseWithRecord.deinit`（209）、同文件单测（2638）。

### `JsonRecordRoots.traceRecord` (`src/exec/json_ops.zig:396`)
...
### `JsonRecordRoots.traceRoots` (`src/exec/json_ops.zig:413`)
...
### `JsonRecordRoots.provider` (`src/exec/json_ops.zig:419`)
...
### `JsonRecordRoots.activate` (`src/exec/json_ops.zig:423`)
...
### `JsonRecordRoots.deactivate` (`src/exec/json_ops.zig:429`)
...
### `JsonPendingRecordRoots.traceRoots` (`src/exec/json_ops.zig:469`)
...
### `JsonPendingRecordRoots.provider` (`src/exec/json_ops.zig:486`)
...
### `JsonPendingRecordRoots.activate` (`src/exec/json_ops.zig:490`)
...
### `JsonPendingRecordRoots.deactivate` (`src/exec/json_ops.zig:496`)
...
### `JsonUnitParser` (`src/exec/json_ops.zig:504`)
...
### `JsonUnitParser.peek` (`src/exec/json_ops.zig:515`)
...
### `JsonUnitParser.skipWhitespace` (`src/exec/json_ops.zig:520`)
...
### `JsonUnitParser.expectLiteral` (`src/exec/json_ops.zig:529`)
...
### `JsonUnitParser.parseValue` (`src/exec/json_ops.zig:537`)
...
### `JsonUnitParser.parseValueRecord` (`src/exec/json_ops.zig:545`)
...
### `JsonUnitParser.recordSourceSpan` (`src/exec/json_ops.zig:580`)
...
### `JsonUnitParser.parseObject` (`src/exec/json_ops.zig:596`)
...
### `JsonUnitParser.parseArray` (`src/exec/json_ops.zig:671`)
...
### `JsonUnitParser.parseKeyAtom` (`src/exec/json_ops.zig:735`)
...
### `JsonUnitParser.parseString` (`src/exec/json_ops.zig:745`)
...
### `JsonUnitParser.parseStringUnits` (`src/exec/json_ops.zig:754`)

- **签名**：`fn parseStringUnits(self: *Self, out: *std.ArrayList(u16)) !void`。
- **作用**：把一段 JSON 字符串字面量的内容解成 UTF-16 码元追加进 `out`。
- **实现**：吃掉开引号后逐单元扫：`"` 结束；`\` 进转义 `switch`——`"` / `\` / `/` 原样，`b`/`f`/`n`/`r`/`t` 映到 0x08/0x0c/0x0a/0x0d/0x09，`u` 再取四个 `jsonHexDigit` 拼成裸 u16 直接 append（**不做**代理配对校验，所以孤立代理能活下来），其余转义字符 `error.SyntaxError`；裸控制字符（< 0x20）非法；普通单元原样写出。文本提前结束同样 `error.SyntaxError`。对照 qjs js_parse_string 的 JSON 模式。
...
### `JsonUnitParser.parseNumber` (`src/exec/json_ops.zig:797`)
...
### `jsonHexDigit` (`src/exec/json_ops.zig:853`)
...
### `appendWtf8FromUnits` (`src/exec/json_ops.zig:864`)
...
### `rawJSON` (`src/exec/json_ops.zig:894`)
...
### `isRawJsonEdgeWhitespace` (`src/exec/json_ops.zig:933`)
...
### `isRawJSON` (`src/exec/json_ops.zig:937`)
...
### `parseInt` (`src/exec/json_ops.zig:944`)

- **签名**：`pub fn parseInt(bytes: []const u8) !i32`。
- **作用**：把 ASCII 十进制字节解析成 i32（`core.value_format.parseAsciiInt` 的薄再导出）。
- **实现**：整体转调 `core.value_format.parseAsciiInt(i32, bytes, 10)`，错误集原样透传。
- **所有权 / 错误 / 调用**：不分配。本仓库内没有其他调用点，它是留给嵌入方的 `pub` 再导出；文件内部的整数解析走 `SimpleJsonParser.parseInt32Number` 与 `parseNumber` 各自的 `parseAsciiInt` 调用。

### `createSimpleJsonAsciiStringValue` (`src/exec/json_ops.zig:948`)
...
### `appendJsonValue` (`src/exec/json_ops.zig:952`)
...
### `appendJsonArray` (`src/exec/json_ops.zig:1008`)
...
### `appendJsonObject` (`src/exec/json_ops.zig:1035`)
...
### `SimpleJsonParser.parse` (`src/exec/json_ops.zig:1081`)
...
### `SimpleJsonParser.parseValue` (`src/exec/json_ops.zig:1094`)
...
### `SimpleJsonParser.parseObject` (`src/exec/json_ops.zig:1113`)
...
### `SimpleJsonParser.parseArray` (`src/exec/json_ops.zig:1164`)
...
### `SimpleJsonParser.parseSimpleStringBytes` (`src/exec/json_ops.zig:1194`)
...
### `SimpleJsonParser.parseInt32Number` (`src/exec/json_ops.zig:1209`)
...
### `SimpleJsonParser.skipWhitespace` (`src/exec/json_ops.zig:1230`)
...
### `SimpleJsonParser.consumeLiteral` (`src/exec/json_ops.zig:1239`)
...
### `SimpleJsonParser.consumeByte` (`src/exec/json_ops.zig:1246`)
...
### `SimpleJsonParser.expectByte` (`src/exec/json_ops.zig:1252`)
...
### `SimpleJsonParser.peek` (`src/exec/json_ops.zig:1256`)
...
### `parseSimpleJsonValue` (`src/exec/json_ops.zig:1262`)
...
### `objectPrototypeFromGlobal` (`src/exec/json_ops.zig:1281`)
...
### `arrayPrototypeFromGlobal` (`src/exec/json_ops.zig:1290`)
...
### `cachedRealmObject` (`src/exec/json_ops.zig:1299`)
...
### `constructorPrototypeFromGlobal` (`src/exec/json_ops.zig:1309`)
...
### `objectInStack` (`src/exec/json_ops.zig:1319`)
...
### `isCallableJsonOmittedObject` (`src/exec/json_ops.zig:1326`)
...
### `isArrayObject` (`src/exec/json_ops.zig:1354`)
...
### `stringifyPropertyList` (`src/exec/json_ops.zig:1361`)
...
### `stringifyPropertyListAtom` (`src/exec/json_ops.zig:1397`)
...
- **实现**：字符串走 `String.internAtom`；int32 用 `formatInt64` 打成十进制再 `rt.internAtom`；f64 先特判 NaN / ±Infinity / `0` 的规范拼写，其余 `formatFiniteNumberAssumeCapacity`，再 intern。对象只认 String 与 Number 两个 class（Boolean 不算，符合 spec 对 replacer 数组的规定），取 `primitiveValue` 后**递归**自己一次；其它值一律 null。
...
### `stringifyGap` (`src/exec/json_ops.zig:1435`)
...
- **实现**：数值臂把 `space` 夹到 [0,10] 后填等量空格；字符串臂追加其 UTF-8 字节，超过 10 字节直接截断（`out.items[0..10]`）。Number/String 包装对象先经 `primitiveValue` 取内部值再走对应臂。 关键调用：`primitiveValue`、`out.appendNTimes`、`core.string.appendValueUtf8`。 用 `ValueRootFrame` / `rootValues` 钉住跨分配窗口的值。
- **所有权 / 错误 / 调用**：返回的 `std.ArrayList(u8)` 归调用方（`stringify` 里 `defer gap.deinit`）。

### `primitiveValue` (`src/exec/json_ops.zig:1477`)

- **签名**：`fn primitiveValue(rt: *core.JSRuntime, object: *core.Object) !?core.JSValue`。
- **作用**：读原始值包装对象（String / Number / Boolean / BigInt / Symbol）的内部数据槽，不是包装器就给 null。
- **实现**：`rt` 未使用。String 先单独试一次 `objectData()`，随后 `switch` 覆盖 number / boolean / big_int / symbol 四个 class，取到就返回、取不到 null；其它 class 一律 null。只读内部槽，不走属性查找、不回调 JS——非 VM 路径必须如此。
- **所有权 / 错误 / 调用**：返回的是借用的内部值（不是副本，也不 retain）。签名带 `!` 但函数体里没有任何 error 源。调用方全在非 VM 路径：`appendJsonValue` 的包装对象臂（996）、`stringifyPropertyListAtom`（1431）、`stringifyGap`（1452、1469）、`appendJsonInputString`（1545）；VM 路径的对应物是同文件的 `jsonPrimitiveWrapperValue`。

### `atomListContains` (`src/exec/json_ops.zig:1492`)
...
### `freePropertyList` (`src/exec/json_ops.zig:1499`)
...
### `appendIndent` (`src/exec/json_ops.zig:1503`)
...
### `defineData` (`src/exec/json_ops.zig:1508`)
...
### `appendJsonInputString` (`src/exec/json_ops.zig:1518`)
...
### `JsonStringifyPropertyList.deinit` (`src/exec/json_ops.zig:1581`)
...
### `deinitLengthIndexAtom` (`src/exec/json_ops.zig:1592`)
...
### `jsonParseCall` (`src/exec/json_ops.zig:1594`)
...
### `jsonInternalizeProperty` (`src/exec/json_ops.zig:1698`)
...
### `jsonInternalizeChild` (`src/exec/json_ops.zig:1787`)
...
### `jsonReviverContext` (`src/exec/json_ops.zig:1818`)
...
### `jsonCreateDataProperty` (`src/exec/json_ops.zig:1839`)
...
### `jsonStringifyCall` (`src/exec/json_ops.zig:1869`)
...
### `jsonStringifySimpleNoOptions` (`src/exec/json_ops.zig:1968`)
...
### `jsonAppendSimpleValue` (`src/exec/json_ops.zig:1980`)
...
### `jsonSimplePrototypeChainHasNoToJSON` (`src/exec/json_ops.zig:2042`)
...
### `jsonAppendSimpleArray` (`src/exec/json_ops.zig:2053`)
...
### `jsonAppendSimpleObject` (`src/exec/json_ops.zig:2092`)
...
### `jsonStringifyPropertyList` (`src/exec/json_ops.zig:2146`)
...
### `jsonStringifyPropertyListAtom` (`src/exec/json_ops.zig:2188`)
...
### `jsonIsStringOrNumberObject` (`src/exec/json_ops.zig:2212`)
...
### `jsonAtomListContains` (`src/exec/json_ops.zig:2217`)
...
### `jsonStringifyGap` (`src/exec/json_ops.zig:2224`)
...
- **实现**：先拆包装对象并**递归**：Number 包装走 `toPrimitiveForNumber` + `toNumberValue`，String 包装走 `toStringForAnnexB`，Boolean 包装取内部值，各自再调自己一次。原始字符串：`ensureFlat` 后按 `resolveData()` 取前 10 个码元——latin1 直接按码点补成 1-2 字节 UTF-8；utf16 先把「切在代理对中间」的尾巴退一位（孤立高代理无法输出），再逐单元编码，成对代理合成码点，孤立代理跳过。数值：`@floor` 后夹到 [0,10] 个空格。其他形态给空 gap。
- **所有权 / 错误 / 调用**：返回的 `std.ArrayList(u8)` 归调用方释放（`jsonStringifyCall` 用 `defer gap.deinit`）。包装对象分支会回调 JS，因而要带 caller 帧。

### `jsonSerializeProperty` (`src/exec/json_ops.zig:2309`)
...
### `jsonAppendValue` (`src/exec/json_ops.zig:2364`)
...
### `jsonAppendArray` (`src/exec/json_ops.zig:2444`)
...
### `jsonAppendObject` (`src/exec/json_ops.zig:2486`)
...
### `jsonPrimitiveWrapperValue` (`src/exec/json_ops.zig:2548`)

- **签名**：`pub fn jsonPrimitiveWrapperValue(rt: *core.JSRuntime, object: *core.Object) !?core.JSValue`。
- **作用**：取原始值包装对象（String/Number/Boolean/BigInt/Symbol）的内部数据槽。
- **实现**：`rt` 未用。String 先单独试一次 `objectData()`，随后 `switch` 覆盖 number / boolean / big_int / symbol 四个 class，取到就返回、取不到返回 null；其他 class 一律 null。只读内部槽，不走属性查找、不回调 JS。
- **所有权 / 错误 / 调用**：返回的是借用的内部值。调用方：`jsonStringifyGap` 与 `jsonAppendValue` 的 Boolean 包装分支（非 VM 路径的对应物是同文件的 `primitiveValue`）。

### `jsonObjectInStack` (`src/exec/json_ops.zig:2563`)
...
### `jsonAppendIndent` (`src/exec/json_ops.zig:2570`)
...
### `S3DupKeyMajorProbe.trigger` (`src/exec/json_ops.zig:2585`)
...
- **实现**：建一个 `class.ids.date` 对象（失败有 errdefer 销毁），再按参数个数定 `[[DateValue]]`：>= 2 个走 `constructDateFromParts`（本地时间）；恰 1 个时字符串走 `parseDateString`、Date 实例复制其时间值、其余 `toNumber` 后 `timeClip`（symbol/bigint 取不到数就 `error.TypeError`）；0 个用 `currentTimeMs()`。 可返回 `error.TypeError`。 关键调用：`core.Object.create`、`constructDateFromParts`、`defineDateValue`、`parseDateString`、`dateValue`、`timeClip`。
...
### `defineDateValue` (`src/exec/date_ops.zig:737`)

- **签名**：`fn defineDateValue(rt: *core.JSRuntime, object: *core.Object, ms: f64) !void`。
- **作用**：把 `[[DateValue]]` 写进 Date 对象的内部数据槽。
- **实现**：薄包装 `setDateValue`（不是属性定义，写的是 objectData 槽）。 关键调用：`setDateValue`。
- **所有权 / 错误 / 调用**：不分配；只把 f64 立即数写进 objectData 槽，没有旧值释放也没有写屏障。签名的 `!void` 完全来自 `setDateValue`，实际不会失败。三个调用点都在 `constructWithPrototype`：多参分支（721）、单参分支（729）、无参分支（731）。

### `staticCall` (`src/exec/date_ops.zig:742`)
...
### `methodCall` (`src/exec/date_ops.zig:753`)
...
### `methodCallArgs` (`src/exec/date_ops.zig:757`)
...
### `methodCallArgsWithCapturedMs` (`src/exec/date_ops.zig:796`)

- **签名**：`pub fn methodCallArgsWithCapturedMs(rt: *core.JSRuntime, object_value: core.JSValue, method: u32, captured_ms: f64, args: []const core.JSValue) !core.JSValue`。
...
### `setterSpan` (`src/exec/date_ops.zig:807`)
...
### `setDateFieldBody` (`src/exec/date_ops.zig:830`)

- **签名**：`fn setDateFieldBody(rt: *core.JSRuntime, object: *core.Object, captured_ms: f64, args: []const core.JSValue, argc: usize, span: SetterSpan) !core.JSValue`。
...
### `setYear` (`src/exec/date_ops.zig:853`)

- **签名**：`fn setYear(rt: *core.JSRuntime, object: *core.Object, ms: f64, args: []const core.JSValue) !core.JSValue`。
...
### `setYearNumber` (`src/exec/date_ops.zig:858`)

- **签名**：`pub fn setYearNumber(rt: *core.JSRuntime, object_value: core.JSValue, captured_ms: f64, year_number: f64) !core.JSValue`。
...
### `setYearNumberOnObject` (`src/exec/date_ops.zig:866`)

- **签名**：`fn setYearNumberOnObject(rt: *core.JSRuntime, object: *core.Object, ms: f64, year_number: f64) !core.JSValue`。
...
### `setTime` (`src/exec/date_ops.zig:876`)

- **签名**：`fn setTime(rt: *core.JSRuntime, object: *core.Object, args: []const core.JSValue) !core.JSValue`。
...
### `getDateFieldValue` (`src/exec/date_ops.zig:885`)
...
### `constructDateFromParts` (`src/exec/date_ops.zig:905`)
...
### `parse` (`src/exec/date_ops.zig:917`)
...
- **所有权 / 错误 / 调用**：返回数值；中途 `value_ops.toStringValue` 可能新建字符串，那是 GC 值、不手工释放。本函数不回调 JS——对象直接判 NaN 正是为了这条不变量。剩下的 error 只有 `toStringValue` 与 `parseDateString`（`ensureFlat`）的 OOM。唯一调用方是 `staticCall` 的 selector 2（745）。

### `getTimezoneOffsetForTime` (`src/exec/date_ops.zig:967`)
...
### `floorDiv` (`src/exec/date_ops.zig:1001`)
...
### `daysFromYear` (`src/exec/date_ops.zig:1006`)
...
### `daysInYear` (`src/exec/date_ops.zig:1012`)
...
### `yearFromDays` (`src/exec/date_ops.zig:1019`)
...
### `getDateFields` (`src/exec/date_ops.zig:1045`)
...
### `timeClip` (`src/exec/date_ops.zig:1092`)
...
### `setDateFields` (`src/exec/date_ops.zig:1100`)
...
### `setDateFieldsChecked` (`src/exec/date_ops.zig:1150`)
...
### `dayName` (`src/exec/date_ops.zig:1163`)
...
### `monthName` (`src/exec/date_ops.zig:1167`)
...
### `writeYearPadded4` (`src/exec/date_ops.zig:1174`)
...
### `getDateStringValue` (`src/exec/date_ops.zig:1186`)
...
### `writeDateString` (`src/exec/date_ops.zig:1220`)
...
### `parseDateString` (`src/exec/date_ops.zig:1292`)

- **签名**：`fn parseDateString(rt: *core.JSRuntime, value: core.JSValue) !f64`。
- **作用**：把一个 JS 字符串值折成 127 字节的 ASCII 缓冲，再交给日期字符串解析器。
- **实现**：非字符串直接 NaN。`ensureFlat` 后把码元拷进 128 字节缓冲并截到 127 字节：latin1 直接 memcpy，utf16 里 > 255 的单元按 U+2212 → `'-'`、其余 → `'x'` 折叠。补 NUL 后交 `dateParseBytes`。 关键调用：`string_value.ensureFlat`、`dateParseBytes`。 注释对照 quickjs.c:55926。
- **所有权 / 错误 / 调用**：128 字节缓冲在栈上，不分配；`ensureFlat` 可能让字符串重排但值由调用方持有。不抛日期相关错误——解析失败就是 NaN。调用方：`dateCall` 的 `Date.parse` 臂、纯 body `parse`、以及 `constructWithPrototype` 的字符串参数分支。

### `dateParseBytes` (`src/exec/date_ops.zig:1316`)
...
### `stringSkipChar` (`src/exec/date_ops.zig:1341`)
...
### `stringSkipSpaces` (`src/exec/date_ops.zig:1350`)
...
### `stringSkipSeparators` (`src/exec/date_ops.zig:1361`)
...
### `stringSkipUntil` (`src/exec/date_ops.zig:1373`)
...
### `stringGetDigits` (`src/exec/date_ops.zig:1384`)
...
### `stringGetMilliseconds` (`src/exec/date_ops.zig:1405`)
...
### `upperAscii` (`src/exec/date_ops.zig:1429`)
...
### `stringGetTzOffset` (`src/exec/date_ops.zig:1434`)
...
### `matchLiteral` (`src/exec/date_ops.zig:1474`)
...
### `findAbbrev` (`src/exec/date_ops.zig:1485`)
...
### `stringGetMonth` (`src/exec/date_ops.zig:1498`)
...
### `jsDateParseIsostring` (`src/exec/date_ops.zig:1508`)
...
### `stringGetTzAbbr` (`src/exec/date_ops.zig:1583`)
...
### `adjustTwoDigitYear` (`src/exec/date_ops.zig:1593`)
...
### `jsDateParseOtherstring` (`src/exec/date_ops.zig:1600`)
...
### `expectDateObject` (`src/exec/date_ops.zig:1729`)
...
### `setDateValue` (`src/exec/date_ops.zig:1737`)

- **签名**：`fn setDateValue(_: *core.JSRuntime, object: *core.Object, ms: f64) !void`。
- **作用**：把时间值写进 Date 对象的内部数据槽（`[[DateValue]]`）。
- **实现**：取 `object.objectDataSlot()` 直接存 `float64(ms)`；runtime 参数未使用，签名只为与其它写入路径对齐。
- **所有权 / 错误 / 调用**：不分配；直接拿 `object.objectDataSlot()` 写一个 f64 立即数——槽里原来也是数值，所以没有旧值释放、也不需要写屏障。签名的 `!void` 只为跟其它写入路径对齐，实际不会失败（`rt` 参数也被丢弃）。调用方：`defineDateValue`（738）、`setDateFieldBody`（849）、`setTime`（879）。

### `isoStringForInspector` (`src/exec/date_ops.zig:1746`)
...
### `dateValue` (`src/exec/date_ops.zig:1752`)
...
### `dateObjectFromValue` (`src/exec/date_ops.zig:1757`)
...
### `numberValue` (`src/exec/date_ops.zig:1765`)
...
### `numberResult` (`src/exec/date_ops.zig:1771`)
...
### `toNumber` (`src/exec/date_ops.zig:1778`)
...
### `appendStringValueAscii` (`src/exec/date_ops.zig:1799`)
...
### `currentTimeMs` (`src/exec/date_ops.zig:1812`)
...
### `preparedOpCall` (`src/exec/math_ops.zig:302`)
...
- **所有权 / 错误 / 调用**：不分配（`value_ops.numberToValue` 产出的是立即数），`args` 借用。未知 id 返回裸 `error.TypeError`；其余错误都来自 `mathArg` → `toMathNumber`，抛出前 JS 异常已挂好。树内调用方只有本文件的 `mathOpCall`（291）与 `mathMinMaxDirect`（106）——函数头注释说的「opcode 辅助函数绕开记录表直接调它」正是这个 `pub` 的用意，但当前树里还没有这样的调用点。

### `mathArg` (`src/exec/math_ops.zig:348`)
...
### `toMathNumber` (`src/exec/math_ops.zig:353`)
...
### `mathMinMax` (`src/exec/math_ops.zig:367`)
...
### `mathMinMaxPrimitiveFast` (`src/exec/math_ops.zig:402`)
...
### `primitiveMathNumber` (`src/exec/math_ops.zig:418`)
...
### `fmin` (`src/exec/math_ops.zig:427`)
...
### `fmax` (`src/exec/math_ops.zig:432`)
...
### `mathPow` (`src/exec/math_ops.zig:437`)
...
### `mathRound` (`src/exec/math_ops.zig:442`)
...
### `mathHypot` (`src/exec/math_ops.zig:461`)
...
### `mathImul` (`src/exec/math_ops.zig:472`)
...
### `mathSign` (`src/exec/math_ops.zig:477`)
...
### `mathSumPreciseCall` (`src/exec/math_ops.zig:484`)
...
### `mathSumPrecise` (`src/exec/math_ops.zig:503`)
...
### `exactF64Sum` (`src/exec/math_ops.zig:558`)
...
### `exactF64ScaledInteger` (`src/exec/math_ops.zig:573`)
...
### `scaledIntegerToF64` (`src/exec/math_ops.zig:588`)
...
### `shouldRoundScaledIntegerUp` (`src/exec/math_ops.zig:628`)
...
### `call` (`src/exec/math_ops.zig:643`)
...
### `log2` (`src/exec/math_ops.zig:696`)
...
### `mathHypotPrimitive` (`src/exec/math_ops.zig:701`)
...
### `exactPowerOfTwoExponent` (`src/exec/math_ops.zig:709`)
...
### `numberValue` (`src/exec/math_ops.zig:723`)
...
### `parseFloat` (`src/exec/number_ops.zig:202`)

- **签名**：`pub fn parseFloat(bytes: []const u8) !f64`。
- **作用**：薄再导出层：把字节切片交 `libs/number_format.zig` 的 `parseNumber`。
- **实现**：整体转调 `dtoa.parseNumber(bytes)`（`libs/number_format.zig`），错误集透传。
- **所有权 / 错误 / 调用**：不分配。本仓库内没有其他调用点——它是留给嵌入方的 `pub` 便捷入口；引擎内部的 `parseFloat` 走 `core.number.parseFloatValue` 或 `builtin_glue.globalParseFloat`。

### `toString` (`src/exec/number_ops.zig:206`)

- **签名**：`pub fn toString(buf: []u8, value: f64) ![]const u8`。
...
- **所有权 / 错误 / 调用**：返回的切片指向**调用方**给的 `buf`，本函数不分配、不建 JS 值，生命周期归调用方。两个调用点都给 64 字节栈缓冲（足够容纳最长的十进制形态，因此都 `catch unreachable`）：`toStringMethod`（255）与 `numberStringValue`（279）。本仓库内没有其他调用点，`pub` 是留给嵌入方的格式化入口。

### `toFixed` (`src/exec/number_ops.zig:210`)
...
### `toExponential` (`src/exec/number_ops.zig:219`)
...
### `toPrecision` (`src/exec/number_ops.zig:235`)
...
### `toStringMethod` (`src/exec/number_ops.zig:244`)
...
### `numberStringValue` (`src/exec/number_ops.zig:277`)
...
### `dtoaStringValue` (`src/exec/number_ops.zig:284`)
...
### `integerDigitsArgument` (`src/exec/number_ops.zig:291`)
...
- 清单函数数: 238（`src/exec/date_ops.zig` 81 + `src/exec/json_ops.zig` 102 + `src/exec/math_ops.zig` 40 + `src/exec/number_ops.zig` 15）
- 本文标题覆盖: 238
- 未覆盖: 无
