...
`sigIdByName` 在 `abi.signatures` 表里按名取 u16。导出常量：`VOID_TO_VOID`、`I32_TO_I32`、`F64_TO_F64`、`F64_F64_TO_F64`、`BOOL_TO_BOOL`、`STATE_*`、`STRING_I32_TO_*`、`SELF_*`（K2 方法叶，`self` 是 NativeObject payload）。

对应 C 原型类型：`LeafF64ToF64` 等。`STRING_I32_TO_I32` 负返回表示形状不匹配，走 fallback。

---

### `sigIdByName` (`src/exec/native_legacy.zig:23`)

- **签名**：`pub fn sigIdByName(comptime name: []const u8) u16`。
- **作用**：把 schema 名编成签名 id。
- **实现**：comptime 扫 `abi.signatures`；找不到 `@compileError`。
- **所有权 / 错误 / 调用**：纯编译期。

### `args` (`src/exec/native_legacy.zig:79`)
...
### `managedGeneric` (`src/exec/native_legacy.zig:84`)
...
### `managedGeneric.thunk` (`src/exec/native_legacy.zig:86`)
...
### `managedGenericMagic` (`src/exec/native_legacy.zig:94`)
...
- **所有权 / 错误 / 调用**：comptime 工厂，返回指向匿名 struct 内 `thunk` 的静态函数指针，不分配。调用方 `entryFromInternal` 的 `.generic_magic` / `.constructor_magic` / `.constructor_or_func_magic` 三臂（`native_legacy.zig:185`/`193`/`201`），以及 `f_f`/`f_f_f` 的 `fallback` 字段（`225`/`232`）。

### `managedGenericMagic.thunk` (`src/exec/native_legacy.zig:96`)
...
### `getterThunk` (`src/exec/native_legacy.zig:103`)

- **签名**：`fn getterThunk(comptime body: core.host_function.NativeGetterFn) core.native_entry.GetterFn`。
- **作用**：无 magic getter。
- **实现**：包 `body(ctx, this)`。
- **所有权 / 错误 / 调用**：comptime 工厂，产静态 thunk 指针，不分配。唯一调用方 `entryFromInternal` 的 `.getter` 臂（`native_legacy.zig:205`）。

### `getterThunk.thunk` (`src/exec/native_legacy.zig:105`)
...
### `getterMagicThunk` (`src/exec/native_legacy.zig:112`)

- **签名**：`fn getterMagicThunk(comptime body: core.host_function.NativeGetterMagicFn) core.native_entry.GetterFn`。
- **作用**：magic getter。
- **实现**：传 `entry.magic`。
- **所有权 / 错误 / 调用**：comptime 工厂，产静态 thunk 指针。唯一调用方 `entryFromInternal` 的 `.getter_magic` 臂（`native_legacy.zig:209`）。

### `getterMagicThunk.thunk` (`src/exec/native_legacy.zig:114`)
...
### `setterThunk` (`src/exec/native_legacy.zig:120`)

- **签名**：`fn setterThunk(comptime body: core.host_function.NativeSetterFn) core.native_entry.SetterFn`。
- **作用**：无 magic setter。
- **实现**：包 `body(ctx, this, new_value)`。
- **所有权 / 错误 / 调用**：comptime 工厂，产静态 thunk 指针。唯一调用方 `entryFromInternal` 的 `.setter` 臂（`native_legacy.zig:213`）。

### `setterThunk.thunk` (`src/exec/native_legacy.zig:122`)
...
### `setterMagicThunk` (`src/exec/native_legacy.zig:129`)

- **签名**：`fn setterMagicThunk(comptime body: core.host_function.NativeSetterMagicFn) core.native_entry.SetterFn`。
- **作用**：magic setter。
- **实现**：传 magic。
- **所有权 / 错误 / 调用**：comptime 工厂，产静态 thunk 指针。唯一调用方 `entryFromInternal` 的 `.setter_magic` 臂（`native_legacy.zig:217`）。

### `setterMagicThunk.thunk` (`src/exec/native_legacy.zig:131`)
...
### `leafF64` (`src/exec/native_legacy.zig:137`)
...
### `leafF64.thunk` (`src/exec/native_legacy.zig:139`)
...
### `leafF64F64` (`src/exec/native_legacy.zig:145`)

- **签名**：`fn leafF64F64(comptime body: core.host_function.NativeF64F64Fn) LeafF64F64ToF64`。
- **作用**：qjs `f_f_f`。
- **实现**：两 double。
- **所有权 / 错误 / 调用**：comptime 工厂，产静态 thunk 指针。唯一调用方 `entryFromInternal` 的 `.f_f_f` 臂（`native_legacy.zig:228`）。

### `leafF64F64.thunk` (`src/exec/native_legacy.zig:147`)
...
### `entryFromInternal` (`src/exec/native_legacy.zig:156`)
...
### `primLeafOrManaged` (`src/exec/native_legacy.zig:242`)
...
- **实现**：无 prim_leaf 原样返回。要求 managed kind。`kind=.method_leaf`；effect：不抛、不重入 JS、读堆；`STRING_I32_TO_STRING` 才 `may_alloc`。
- **所有权 / 错误 / 调用**：`charCodeAt` 一类方法叶。

### `genericEntry` (`src/exec/native_legacy.zig:264`)
