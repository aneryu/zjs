...
- **作用**：服务 `op.push_i32`。
- **实现**：读 i32，`pc += 4`，`pushImmediateInt32MaybeFuse`。
...
- **所有权 / 错误 / 调用**：推的是立即数，不分配、不建根、不需写屏障。签名的 `!void` 是形式上的：`pushImmediateInt32MaybeFuse` 只调 `pushOwnedAssumeCapacity`（`src/exec/stack.zig:232`，Debug 下断言容量、Release 下无检查），容量由进帧时按 `stack_size` 一次性预留，所以这条臂永不返回错误。唯一调用方是冷表 `op.push_i16` 臂（`src/exec/tailcall_dispatch_colds.zig:191`）。
...
### `pushSmallIntMaybeFuse` (`src/exec/vm_value.zig:53`)

- **签名**：`pub fn pushSmallIntMaybeFuse(stack: *stack_mod.Stack, function: *const bytecode.FunctionBytecode, frame: *frame_mod.Frame, value: i32) !void`。
- **作用**：服务 `op.push_minus1` / `push_0`–`push_7`。
- **实现**：转 `pushImmediateInt32MaybeFuse`。
- **所有权 / 错误 / 调用**：热路径已内联 push，此为非 threaded 回退。

### `pushImmediateInt32MaybeFuse` (`src/exec/vm_value.zig:57`)

- **签名**：`fn pushImmediateInt32MaybeFuse( stack: *stack_mod.Stack, function: *const bytecode.FunctionBytecode, frame: *frame_mod.Frame, value: i32, ) !void`。
- **作用**：纯 int32 push。qjs 无运行时 push+binop 融合；名字里的 MaybeFuse 是历史，体是 `pushOwnedAssumeCapacity(int32)`。
- **实现**：`function`/`frame` 未用。
- **所有权 / 错误 / 调用**：assumeCapacity（handler 已保证）。

### `pushUndefined` (`src/exec/vm_value.zig:73`)
...
### `pushNull` (`src/exec/vm_value.zig:77`)
...
### `pushBoolean` (`src/exec/vm_value.zig:81`)
...
### `pushConst` (`src/exec/vm_value.zig:85`)
...
### `pushConst8` (`src/exec/vm_value.zig:93`)
...
### `pushAtomValue` (`src/exec/vm_value.zig:101`)
...
### `pushPrivateSymbol` (`src/exec/vm_value.zig:108`)
...
### `pushEmptyString` (`src/exec/vm_value.zig:120`)
...
### `pushThis` (`src/exec/vm_value.zig:125`)
...
### `pushThisVm` (`src/exec/vm_value.zig:131`)
...
### `toObject` (`src/exec/vm_value.zig:155`)
...
### `toObjectVm` (`src/exec/vm_value.zig:164`)
...
### `typeOf` (`src/exec/vm_value.zig:182`)
...
### `typeOfIsUndefined` (`src/exec/vm_value.zig:210`)
...
### `typeOfIsFunction` (`src/exec/vm_value.zig:215`)
...
### `logicalNot` (`src/exec/vm_value.zig:228`)
...
### `drop` (`src/exec/vm_value.zig:233`)
...
### `nipCatch` (`src/exec/vm_value.zig:248`)
...
### `swap` (`src/exec/vm_value.zig:276`)
...
### `dup2` (`src/exec/vm_value.zig:292`)
...
### `dup1` (`src/exec/vm_value.zig:303`)
...
### `dup3` (`src/exec/vm_value.zig:313`)
...
### `insert2` (`src/exec/vm_value.zig:327`)
...
### `insert3` (`src/exec/vm_value.zig:337`)
...
### `insert4` (`src/exec/vm_value.zig:349`)
...
### `rot3l` (`src/exec/vm_value.zig:363`)
...
### `rot3r` (`src/exec/vm_value.zig:374`)
...
### `rot4l` (`src/exec/vm_value.zig:385`)
...
### `rot5l` (`src/exec/vm_value.zig:398`)
...
### `perm3` (`src/exec/vm_value.zig:413`)
...
### `perm4` (`src/exec/vm_value.zig:424`)
...
### `perm5` (`src/exec/vm_value.zig:437`)
...
### `swap2` (`src/exec/vm_value.zig:452`)
...
### `isUndefinedOrNull` (`src/exec/vm_value.zig:465`)
...
### `isUndefined` (`src/exec/vm_value.zig:470`)
...
### `isNull` (`src/exec/vm_value.zig:475`)
...
### `adapterValueBorrow` (`src/exec/vm_value.zig:480`)
...
### `requireStackLen` (`src/exec/vm_value.zig:489`)
...
### `expectStackInt32s` (`src/exec/vm_value.zig:493`)
...
### `varRefCellFromValue` (`src/exec/vm_value.zig:500`)
...
### `functionObjectFromValue` (`src/exec/vm_value.zig:504`)
...
### `callableObjectFromValue` (`src/exec/vm_value.zig:514`)
...
### `proxyTargetIsCallable` (`src/exec/vm_value.zig:526`)
...
### `readInt` (`src/exec/vm_value.zig:532`)
...
### `countLivePrivateAtomsNamed` (`src/exec/vm_value.zig:536`)
...
### `function object lookup recognizes every bytecode function class` (`src/exec/vm_value.zig:547`)
...
### `push private symbol creates a fresh runtime atom per execution` (`src/exec/vm_value.zig:566`)
...
### `stack rearrange opcodes validate depth before mutating stack` (`src/exec/vm_value.zig:624`)
...
### `push private symbol stack failure does not retain transient private atom` (`src/exec/vm_value.zig:645`)
...
### `push private symbol releases fresh atom on allocation failure` (`src/exec/vm_value.zig:689`)
