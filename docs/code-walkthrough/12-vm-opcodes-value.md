...
### `pushInt32Operand` (`src/exec/vm_opcodes.zig:29`)
...
### `pushBigIntI32Operand` (`src/exec/vm_opcodes.zig:35`)
...
### `pushI16Operand` (`src/exec/vm_opcodes.zig:41`)
...
### `pushI8Operand` (`src/exec/vm_opcodes.zig:47`)
...
### `pushSmallInt` (`src/exec/vm_opcodes.zig:60`)
...
### `pushUndefined` (`src/exec/vm_opcodes.zig:64`)
...
### `pushNull` (`src/exec/vm_opcodes.zig:68`)
...
### `pushBoolean` (`src/exec/vm_opcodes.zig:72`)
...
### `pushConst` (`src/exec/vm_opcodes.zig:76`)
...
### `pushConst8` (`src/exec/vm_opcodes.zig:84`)
...
### `pushAtomValue` (`src/exec/vm_opcodes.zig:92`)
...
### `pushPrivateSymbol` (`src/exec/vm_opcodes.zig:99`)
...
### `pushEmptyString` (`src/exec/vm_opcodes.zig:114`)
...
### `pushThis` (`src/exec/vm_opcodes.zig:119`)
...
### `pushThisVm` (`src/exec/vm_opcodes.zig:125`)
...
### `toObject` (`src/exec/vm_opcodes.zig:149`)
...
### `toObjectVm` (`src/exec/vm_opcodes.zig:158`)
...
### `typeOf` (`src/exec/vm_opcodes.zig:176`)
...
### `typeOfIsUndefined` (`src/exec/vm_opcodes.zig:204`)
...
### `typeOfIsFunction` (`src/exec/vm_opcodes.zig:209`)
...
### `logicalNot` (`src/exec/vm_opcodes.zig:222`)
...
### `drop` (`src/exec/vm_opcodes.zig:227`)
...
### `nipCatch` (`src/exec/vm_opcodes.zig:242`)
...
### `dup` (`src/exec/vm_opcodes.zig:263`)
...
### `swap` (`src/exec/vm_opcodes.zig:270`)
...
### `nip` (`src/exec/vm_opcodes.zig:279`)
...
### `dup2` (`src/exec/vm_opcodes.zig:286`)
...
### `dup1` (`src/exec/vm_opcodes.zig:297`)
...
### `dup3` (`src/exec/vm_opcodes.zig:307`)
...
### `insert2` (`src/exec/vm_opcodes.zig:321`)
...
### `insert3` (`src/exec/vm_opcodes.zig:331`)

- **签名**：`pub fn insert3(ctx: *core.JSContext, stack: *stack_mod.Stack) !void`。
- **作用**：把顶插入到深度 3。
- **实现**：`[a,b,c] → [c,a,b,c]`。
- **所有权 / 错误 / 调用**：只搬运栈槽，不分配、不 retain：`pushOwnedAssumeCapacity` 与 `pushAssumeCapacity` 在去 rc 之后是同一段代码（`src/exec/stack.zig:226` / `:232`），`Owned` 后缀现在只是文档性的。error set 只有 `requireStackLen`（`src/exec/vm_opcodes.zig:489`）的 `error.StackUnderflow`，且**先检查后动栈**，这正是 `src/exec/vm_opcodes.zig:624` 那条测试断言的不变量。pop 与 push 之间值只活在 Zig 局部里，但中间不分配，且保守栈扫描覆盖这些局部。`ctx` 被 `_` 丢弃。唯一调用方冷表 `op.insert3` 臂（`src/exec/tailcall_dispatch_colds.zig:612`）。

### `insert4` (`src/exec/vm_opcodes.zig:343`)
...
### `rot3l` (`src/exec/vm_opcodes.zig:357`)
...
### `rot3r` (`src/exec/vm_opcodes.zig:368`)
...
### `rot4l` (`src/exec/vm_opcodes.zig:379`)

- **签名**：`pub fn rot4l(ctx: *core.JSContext, stack: *stack_mod.Stack) !void`。
- **作用**：四槽左旋。
- **实现**：`[a,b,c,d] → [b,c,d,a]`。
- **所有权 / 错误 / 调用**：同族：纯搬运、不分配、不 retain，唯一错误是先检查的 `error.StackUnderflow`。`rot4l` 已无一字节 id（旧 id 31 归 `get_var_ref0_get_loc8`），唯一可达分发是 `op.ext0` + `ext0_sub.rot4l`，在 `using_ops.execVm` 的二级 switch 里（`src/exec/vm_opcodes.zig:163`）；`src/exec/tailcall_dispatch_colds.zig:632` 那处写的是 `keep[9]`，是为保持冷表代码布局而保活的实例，不是可达的 opcode 臂。

### `rot5l` (`src/exec/vm_opcodes.zig:392`)

- **签名**：`pub fn rot5l(ctx: *core.JSContext, stack: *stack_mod.Stack) !void`。
- **作用**：五槽左旋。
- **实现**：`[a,b,c,d,e] → [b,c,d,e,a]`。
- **所有权 / 错误 / 调用**：同族：纯搬运、不分配、先检查深度。同样只以 `op.ext0` + `ext0_sub.rot5l` 分发（`src/exec/vm_opcodes.zig:143`）；`src/exec/tailcall_dispatch_colds.zig:637` 是 `keep[4]` 的布局保活实例，不可达。

### `perm3` (`src/exec/vm_opcodes.zig:407`)
...
### `perm4` (`src/exec/vm_opcodes.zig:418`)
...
### `perm5` (`src/exec/vm_opcodes.zig:431`)

- **签名**：`pub fn perm5(ctx: *core.JSContext, stack: *stack_mod.Stack) !void`。
- **作用**：五槽置换。
- **实现**：`[a,b,c,d,e] → [d,a,b,c,e]`。
- **所有权 / 错误 / 调用**：同族：纯搬运、不分配、先检查深度。只以 `op.ext0` + `ext0_sub.perm5` 分发（`src/exec/vm_opcodes.zig:147`）；`src/exec/tailcall_dispatch_colds.zig:652` 是 `keep[5]` 的布局保活实例，不可达。

### `swap2` (`src/exec/vm_opcodes.zig:446`)
...
### `isUndefinedOrNull` (`src/exec/vm_opcodes.zig:459`)
...
### `isUndefined` (`src/exec/vm_opcodes.zig:464`)
...
### `isNull` (`src/exec/vm_opcodes.zig:469`)
...
### `adapterValueBorrow` (`src/exec/vm_opcodes.zig:474`)
...
### `requireStackLen` (`src/exec/vm_opcodes.zig:483`)
...
### `expectStackInt32s` (`src/exec/vm_opcodes.zig:487`)
...
- **所有权 / 错误 / 调用**：测试辅助，只读：不改栈、不分配，error set 来自 `std.testing`。它按 `stack.values[index]` 从缓冲**底部**索引（不是相对栈顶），所以只对从空栈起搭起来的夹具成立。唯一调用方是 `src/exec/vm_opcodes.zig:624` 那条深度校验测试（3 次）。

### `varRefCellFromValue` (`src/exec/vm_opcodes.zig:494`)
...
### `functionObjectFromValue` (`src/exec/vm_opcodes.zig:498`)
...
### `callableObjectFromValue` (`src/exec/vm_opcodes.zig:508`)
...
### `proxyTargetIsCallable` (`src/exec/vm_opcodes.zig:520`)
...
### `readInt` (`src/exec/vm_opcodes.zig:526`)

- **签名**：`fn readInt(comptime T: type, bytes: []const u8) T`。
- **作用**：读立即数/atom/常量索引。
- **实现**：小端。
- **所有权 / 错误 / 调用**：纯读借用的字节码切片，不分配、无 error set，定长由调用点 `[0..N]` 切片保证。文件私有，调用方：`src/exec/vm_opcodes.zig:30`/`:36`/`:42`（立即数）、`:87`（常量索引）、`:102`（atom）、`:109`（private symbol 模板 atom）。

### `countLivePrivateAtomsNamed` (`src/exec/vm_opcodes.zig:530`)
...
### `function object lookup recognizes every bytecode function class` (`src/exec/vm_opcodes.zig:538`)
...
### `push private symbol creates a fresh runtime atom per execution` (`src/exec/vm_opcodes.zig:557`)
...
### `stack rearrange opcodes validate depth before mutating stack` (`src/exec/vm_opcodes.zig:615`)
...
### `push private symbol stack failure does not retain transient private atom` (`src/exec/vm_opcodes.zig:636`)

- **签名**：`test "..."`
- **作用**：capacity 0 时 `StackOverflow`，allocated_bytes 不变，活 private atom 仍为 1（仅模板）。
- **实现**：先校准回收槽，再 push。
- **所有权 / 错误 / 调用**：验证失败路径不泄漏：容量 0 的栈让 `pushPrivateSymbol` 在 `stack.reserveAdditional(1)` 处就以 `error.StackOverflow` 退出（`src/exec/vm_opcodes.zig:112`，在 `newSymbol` 之前），所以既没多分配字节也没多出 private atom。断言用 `rt.memory.allocated_bytes` 前后相等 + 活 private atom 恒为 1（只剩模板）；校准用的那个额外 symbol 必须先 `runObjectCycleRemoval` 回收，因为去 rc 后 `free` 不再即时退休 atom 表项。

### `push private symbol releases fresh atom on allocation failure` (`src/exec/vm_opcodes.zig:680`)
