...
### `JobQueueRootProvider.trace` (`src/tests/exec.zig:6049`)
...
### `JobQueueRootProvider.provider` (`src/tests/exec.zig:6054`)
...
### `ImportProbe.run` (`src/tests/exec.zig:11737`)
...
### `ImportProbe.run` (`src/tests/exec.zig:11789`)
...
### `LoaderProbe.load` (`src/tests/exec.zig:11835`)
...
### `ActiveProbe.run` (`src/tests/exec.zig:18104`)
...
### `Probe.run` (`src/tests/exec.zig:18204`)
...
### `retainedModuleExportCell` (`src/tests/exec.zig:19670`)
...
### `PromiseResult.get` (`src/tests/exec.zig:19906`)
...
### `ArmableOneShotAllocator.allocator` (`src/tests/exec.zig:20011`)
...
### `ArmableOneShotAllocator.arm` (`src/tests/exec.zig:20018`)
...
### `ArmableOneShotAllocator.disarm` (`src/tests/exec.zig:20023`)
...
### `ArmableOneShotAllocator.alloc` (`src/tests/exec.zig:20027`)
...
### `ArmableOneShotAllocator.resize` (`src/tests/exec.zig:20036`)
...
### `ArmableOneShotAllocator.remap` (`src/tests/exec.zig:20041`)
...
### `ArmableOneShotAllocator.free` (`src/tests/exec.zig:20046`)
...
### `HostFixture.findBySpecifierOrPath` (`src/tests/exec.zig:20252`)
...
### `HostFixture.findByPath` (`src/tests/exec.zig:20259`)
...
### `expectRejectedPromiseNamedError` (`src/tests/exec.zig:20267`)
...
### `hostHooks` (`src/tests/exec.zig:20286`)
...
### `resolveFixtureModule` (`src/tests/exec.zig:20294`)
...
### `loadFixtureModule` (`src/tests/exec.zig:20311`)
...
### `ReflectActiveRootSymbolProbe.trigger` (`src/tests/exec.zig:20370`)
...
### `reflectTestSetArrayIndex` (`src/tests/exec.zig:20390`)
...
### `ActiveInvocationRootProbe.call` (`src/tests/exec.zig:21975`)
...
### `ActiveInvocationRootProbe.Recorder.visitValue` (`src/tests/exec.zig:21987`)
...
### `ActiveInvocationRootProbe.Recorder.visitObject` (`src/tests/exec.zig:21996`)
...
### `assertLiveWindowsVisited` (`src/tests/exec.zig:22038`)
...
### `expectFrameVisited` (`src/tests/exec.zig:22060`)
...
### `expectValueVisitedSlice` (`src/tests/exec.zig:22094`)
...
### `expectValueVisited` (`src/tests/exec.zig:22098`)
...
### `S3MajorAtEveryAllocationProbe.trigger` (`src/tests/exec.zig:22329`)
...
### `Probe.trigger` (`src/tests/exec.zig:22473`)
...
### `PublishRootProbe.trigger` (`src/tests/exec.zig:22537`)
...
### `runUnderPublishProbe` (`src/tests/exec.zig:22556`)
...
### `Probe.call` (`src/tests/exec.zig:22942`)
...
### `Probe.call` (`src/tests/exec.zig:23238`)
...
### `Probe.tail` (`src/tests/exec.zig:23245`)
...
### `Probe.call` (`src/tests/exec.zig:23344`)
...
### `test "strict generator resident frame supports qjs argument counts beyond u16 storage"` (`src/tests/exec.zig:4111`)
...
### `test "a dynamic function outlives its teardown when its object held the last bytecode reference"` (`src/tests/exec.zig:4130`)
...
### `test "string leftover ToIntegerOrInfinity matches value_ops including bigint TypeError"` (`src/tests/exec.zig:4152`)
...
### `test "vm executes push constants arithmetic comparisons and return"` (`src/tests/exec.zig:4168`)
...
### `test "Engine executes both paths of a threaded with atom-label destructuring probe"` (`src/tests/exec.zig:4186`)
...
### `test "signed bigint-i32 neg preserves inline and generic BigInt semantics"` (`src/tests/exec.zig:4203`)
...
### `test "heap bigint multiplication still compacts a short-representable product"` (`src/tests/exec.zig:4221`)
...
### `test "numeric discarded immediates preserve comma control and completion semantics"` (`src/tests/exec.zig:4250`)
...
### `test "vm executes stack constants source locations and return_undef"` (`src/tests/exec.zig:4284`)
...
### `test "frame setLocal handles self-assignment without dropping object"` (`src/tests/exec.zig:4300`)
...
### `test "lookupFrameVarRef tolerates synthetic var-ref name mirrors"` (`src/tests/exec.zig:4320`)
...
### `test "derived constructor without nested this references has no owner cell"` (`src/tests/exec.zig:4346`)
...
### `test "derived constructor arrow creates exactly one owner this cell"` (`src/tests/exec.zig:4365`)
...
### `test "derived constructor parameter default arrow captures this by binding identity"` (`src/tests/exec.zig:4379`)
...
### `test "direct eval captures derived this while indirect eval does not"` (`src/tests/exec.zig:4393`)
...
### `test "ordinary constructor Machine completion preserves bindings eval recursion and abrupt teardown"` (`src/tests/exec.zig:4509`)
...
### `test "derived constructor Machine completion preserves inherited new target and teardown"` (`src/tests/exec.zig:4572`)
...
### `test "Reflect.construct keeps a fresh prototype getter result alive through instance allocation"` (`src/tests/exec.zig:4654`)
...
### `test "Proxy wrapping a class named Array never enters the native Array construct record"` (`src/tests/exec.zig:4683`)
...
### `test "Proxy native constructor forwarding resolves new target prototype before coercion"` (`src/tests/exec.zig:4711`)
...
### `test "default derived constructor follows the live constructor prototype"` (`src/tests/exec.zig:4744`)
...
### `test "class constructor opcode errors preserve QuickJS messages and realms"` (`src/tests/exec.zig:4780`)
...
### `test "direct spread and arrow super follow the live derived constructor prototype"` (`src/tests/exec.zig:4817`)
...
### `test "super call paths reject null live parents and do not authorize ordinary class calls"` (`src/tests/exec.zig:4866`)
...
### `test "constant pool execution retains returned constants"` (`src/tests/exec.zig:5004`)
...
### `test "property ops use shared object semantics"` (`src/tests/exec.zig:5022`)
...
### `test "value ops own primitive VM semantics"` (`src/tests/exec.zig:5047`)
...
### `test "closure helper stores closure state outside the VM"` (`src/tests/exec.zig:5115`)
...
### `test "resident stack permutations preserve assignment values and ownership"` (`src/tests/exec.zig:5211`)
...
### `test "mapped arguments rest-style 0-formal length and index (sc_list)"` (`src/tests/exec.zig:5280`)
...
### `test "typed array integer get uses class-id arm and qjs tag shape"` (`src/tests/exec.zig:5312`)
...
### `test "typed array prototype chain get reads canonical numeric indices"` (`src/tests/exec.zig:5345`)
...
### `test "typed array integer put uses class-id arm"` (`src/tests/exec.zig:5368`)
...
### `test "typed array int32 store fast arm preserves conversion and assignment semantics"` (`src/tests/exec.zig:5400`)
...
### `test "checked local replacement preserves int fast moves and refcounted fallbacks"` (`src/tests/exec.zig:5448`)
...
### `test "an expression helper emits an explicit return after a bytecode call"` (`src/tests/exec.zig:5467`)
...
### `test "function expressions execute wide closure operands past 255 constants"` (`src/tests/exec.zig:5750`)
...
### `test "native builtin record dispatch is independent from dispatch-name strings"` (`src/tests/exec.zig:5863`)
...
### `test "bytecode calls execute directly from the shared function bytecode"` (`src/tests/exec.zig:5924`)
...
### `test "Math cproto dispatch preserves observable ToNumber semantics"` (`src/tests/exec.zig:5985`)
...
### `test "local add_loc retains string snapshots after accumulator tail removal"` (`src/tests/exec.zig:6005`)
...
### `test "add_loc string+object goes through slow add after toPrimitive (qjs OP_add_loc)"` (`src/tests/exec.zig:6059`)
...
### `test "checked lexical string accumulation keeps rope depth bounded"` (`src/tests/exec.zig:6159`)
...
### `test "computed reads with cached string atoms preserve exotic and prototype semantics"` (`src/tests/exec.zig:6197`)
...
### `test "native dispatch metadata is internal and ignores user properties"` (`src/tests/exec.zig:6245`)
...
### `test "scope resolver skips popped lexical shadow for destructured parameter"` (`src/tests/exec.zig:6271`)
...
### `test "__zjs-prefixed user properties are ordinary own properties"` (`src/tests/exec.zig:6289`)
...
### `test "array species fast path markers are internal"` (`src/tests/exec.zig:6311`)
...
### `test "auto-init builtin markers are internal and ignore user properties"` (`src/tests/exec.zig:6337`)
...
### `test "immutable prototype marker is internal"` (`src/tests/exec.zig:6451`)
...
### `test "builtin dispatch function markers are internal"` (`src/tests/exec.zig:6471`)
...
### `test "proxy revocation target is internal"` (`src/tests/exec.zig:6512`)
...
### `test "regexp accessor realm TypeError constructor is internal"` (`src/tests/exec.zig:6553`)
...
### `test "throw type error intrinsic marker is internal"` (`src/tests/exec.zig:6587`)
...
### `test "async generator prototype method marker is internal"` (`src/tests/exec.zig:6639`)
...
### `test "generator instances inherit shared prototype methods"` (`src/tests/exec.zig:6661`)
...
### `test "generator object uses the prototype selected after parameter initialization"` (`src/tests/exec.zig:6759`)
...
### `test "closure-env var_ref hitting rc zero during remove_cycles stays a batch no-op"` (`src/tests/exec.zig:6776`)
...
### `test "parked generator open cell death path reclaims cell and generator together"` (`src/tests/exec.zig:6814`)
...
### `test "cycle drain frees leftover-rc rings under repeated forceGC"` (`src/tests/exec.zig:6859`)
...
### `test "major tracing keeps a heap BigInt reachable through an object"` (`src/tests/exec.zig:6894`)
...
### `test "generator continuation keeps its FunctionBytecode alive after every source binding is dropped"` (`src/tests/exec.zig:6905`)
...
### `test "initial_yield keeps sync generators in suspended-start after parameter initialization"` (`src/tests/exec.zig:6935`)
...
### `test "initial_yield keeps async generators in suspended-start"` (`src/tests/exec.zig:6968`)
...
### `test "initial_yield executes exported generator bytecode in module mode"` (`src/tests/exec.zig:6998`)
...
### `test "generator completion resumes keep the original function home object"` (`src/tests/exec.zig:7021`)
...
### `test "resident generator resumes preserve nested catch and finally targets"` (`src/tests/exec.zig:7068`)
...
### `test "surviving var references keep resident local slots bare"` (`src/tests/exec.zig:7134`)
...
### `test "direct eval captures only bindings visible at its call scope"` (`src/tests/exec.zig:7179`)
...
### `test "suspended generators retain one resident execution owner across resumes"` (`src/tests/exec.zig:7210`)
...
### `test "completed generators eagerly release their resident execution state"` (`src/tests/exec.zig:7265`)
...
### `test "iterator helper method marker is internal"` (`src/tests/exec.zig:7317`)
...
### `test "Iterator.from follows QuickJS wrapper selection"` (`src/tests/exec.zig:7365`)
...
### `test "number native builtin records cover static and prototype dispatch"` (`src/tests/exec.zig:7431`)
...
### `test "string prototype native builtin records ignore dispatch names"` (`src/tests/exec.zig:7594`)
...
### `test "String case conversion records preserve coercion and Unicode semantics"` (`src/tests/exec.zig:7640`)
...
### `test "date static native builtin records ignore dispatch names"` (`src/tests/exec.zig:7676`)
...
### `test "date constructor native builtin records ignore dispatch names"` (`src/tests/exec.zig:7718`)
...
### `test "constructValue AggregateError releases copied errors array owner"` (`src/tests/exec.zig:7774`)
...
### `test "date prototype native builtin records ignore dispatch names"` (`src/tests/exec.zig:7813`)
...
### `test "array static native builtin records ignore dispatch names"` (`src/tests/exec.zig:7858`)
...
### `test "array prototype native builtin records ignore dispatch names"` (`src/tests/exec.zig:7919`)
...
### `test "collection native builtin records ignore dispatch names"` (`src/tests/exec.zig:7999`)
...
### `test "buffer native builtin records ignore dispatch names"` (`src/tests/exec.zig:8092`)
...
### `test "typed array accessor native builtin records ignore dispatch names"` (`src/tests/exec.zig:8217`)
...
### `test "regexp static native builtin records ignore dispatch names"` (`src/tests/exec.zig:8291`)
...
### `test "regexp prototype native builtin records ignore dispatch names"` (`src/tests/exec.zig:8336`)
...
### `test "regexp symbol native builtin records ignore dispatch names"` (`src/tests/exec.zig:8426`)
...
### `test "regexp accessor native builtin records ignore dispatch names"` (`src/tests/exec.zig:8538`)
...
### `test "vm host native builtin records dispatch by id before name fallback"` (`src/tests/exec.zig:8605`)
...
### `test "vm collection constructors use registered prototype methods"` (`src/tests/exec.zig:8644`)
...
### `test "qjs alignment X-09 VARREF to GETSET detaches the stale cell"` (`src/tests/exec.zig:8832`)
...
### `test "default Function hasInstance uses Ordinary; other native records still Call"` (`src/tests/exec.zig:9078`)
...
### `test "local reference-tail lowering preserves binding semantics"` (`src/tests/exec.zig:9117`)
...
### `test "qjs alignment const local writes throw from resolved bytecode"` (`src/tests/exec.zig:9161`)
...
### `test "qjs alignment named function self-binding ignores every sloppy write form"` (`src/tests/exec.zig:9205`)
...
### `test "Engine eval strips TypeScript source kind before execution"` (`src/tests/exec.zig:9360`)
...
### `test "Engine eval strips TypeScript method annotations"` (`src/tests/exec.zig:9374`)
...
### `test "Engine eval preserves as and satisfies runtime property names in TypeScript files"` (`src/tests/exec.zig:9387`)
...
### `test "Engine eval supports TypeScript parameter properties"` (`src/tests/exec.zig:9398`)
...
### `test "Engine eval strips TypeScript automatically for ts filenames"` (`src/tests/exec.zig:9412`)
...
### `test "CallSite metadata is internal"` (`src/tests/exec.zig:9423`)
...
### `test "pc2line stack locations match QuickJS return and throw matrix"` (`src/tests/exec.zig:9462`)
...
### `test "X-89 sloppy and method tails keep the caller like QuickJS; strict tail_call reuses"` (`src/tests/exec.zig:9482`)
...
### `test "strict plain tail_call recursion stays in constant stack"` (`src/tests/exec.zig:9507`)
...
### `test "X-89 frame disasm: return call and method emit tail opcodes"` (`src/tests/exec.zig:9523`)
...
### `test "pc2line malformed transition reports zero location instead of header fallback"` (`src/tests/exec.zig:9577`)
...
### `test "Error stack uses object method runtime names"` (`src/tests/exec.zig:9604`)
...
### `test "native builtin errors capture a native callsite"` (`src/tests/exec.zig:9621`)
...
### `test "external host errors capture the native host callsite"` (`src/tests/exec.zig:9767`)
...
### `test "native record calls preflight the native stack and recover"` (`src/tests/exec.zig:9810`)
...
### `test "external C function preflight uses caller realm and callback errors use callee realm"` (`src/tests/exec.zig:9848`)
...
### `test "Error stack preserves construction frames across delayed access"` (`src/tests/exec.zig:9933`)
...
### `test "eval SyntaxError carries construction stack"` (`src/tests/exec.zig:9954`)
...
### `test "TypeError thrown via message helper carries stack exactly once"` (`src/tests/exec.zig:9977`)
...
### `test "Error prepareStackTrace formats captured frames lazily"` (`src/tests/exec.zig:10001`)
...
### `test "Error stack setter rejects non-string stack values"` (`src/tests/exec.zig:10031`)
...
### `test "Error stack copied accessor setter writes without recursion"` (`src/tests/exec.zig:10049`)
...
### `test "Error stack copied accessor setter writes through proxy without recursion"` (`src/tests/exec.zig:10068`)
...
### `test "Error stack reentrant formatting is capped to captured frames"` (`src/tests/exec.zig:10084`)
...
### `test "Array fill respects proxy prototypes"` (`src/tests/exec.zig:10110`)
...
### `test "Error.prepareStackTrace exceptions produce null stack"` (`src/tests/exec.zig:10130`)
...
### `test "Engine runtime-strict file eval matches QuickJS CLI script surface"` (`src/tests/exec.zig:10144`)
...
### `test "runtime-strict eval overrides parse-time mapped arguments subtype"` (`src/tests/exec.zig:10174`)
...
### `test "Promise result cycle is released by runtime cycle removal"` (`src/tests/exec.zig:10304`)
...
### `test "Promise reaction cycle is released by runtime cycle removal"` (`src/tests/exec.zig:10316`)
...
### `test "proxy revoke FunctionRare cycle is released by runtime cycle removal"` (`src/tests/exec.zig:10328`)
...
### `test "Promise finally FunctionRare cycle is released by runtime cycle removal"` (`src/tests/exec.zig:10340`)
...
### `test "DisposableStack resource self-cycle is released by runtime cycle removal"` (`src/tests/exec.zig:10352`)
...
### `test "module import-meta and eval-exception cycles are released by runtime cycle removal"` (`src/tests/exec.zig:10364`)
...
### `test "buffer constructor extras leftover runtime tables preserve ArrayBuffer SharedArrayBuffer and DataView"` (`src/tests/exec.zig:10496`)
...
### `test "iterator step leftover post-next decode preserves for-of and helper results"` (`src/tests/exec.zig:10533`)
...
### `test "class field initializer leftover runtime static preserves instance static private and computed fields"` (`src/tests/exec.zig:10583`)
...
### `test "leftover instance-computed public field initializer through shared emit"` (`src/tests/exec.zig:10626`)
...
### `test "leftover do while parse through one runtime flag"` (`src/tests/exec.zig:10660`)
...
### `test "leftover error stack at-line format through one runtime kind"` (`src/tests/exec.zig:10695`)
...
### `test "leftover array-from array-like through one runtime destination"` (`src/tests/exec.zig:10750`)
...
### `test "leftover proxy set trap through one runtime kind"` (`src/tests/exec.zig:10775`)
...
### `test "leftover proxy extensible trap through one runtime kind"` (`src/tests/exec.zig:10804`)
...
### `test "leftover proxy has trap through one outlined walk"` (`src/tests/exec.zig:10854`)
...
### `test "leftover proxy getPrototypeOf through one outlined walk"` (`src/tests/exec.zig:10891`)
...
### `test "leftover Object.isExtensible builtin through outlined extensible op"` (`src/tests/exec.zig:10929`)
...
### `test "leftover Object.getOwnPropertyNames through outlined enumerable own properties"` (`src/tests/exec.zig:10962`)
...
### `test "leftover Array.shift index-move through outlined arrayMoveIndex"` (`src/tests/exec.zig:10995`)
...
### `test "leftover Array.slice present-index through outlined arrayCopyPresentIndex"` (`src/tests/exec.zig:11031`)
...
### `test "leftover integer binary through live bitwise and number arms"` (`src/tests/exec.zig:11062`)
...
### `test "leftover Array.map generic get through one runtime tail"` (`src/tests/exec.zig:11090`)
...
### `test "leftover Array.toReversed get-define through outlined arrayCopyIndex"` (`src/tests/exec.zig:11114`)
...
### `test "leftover Array.sort generic set through one runtime tail"` (`src/tests/exec.zig:11139`)
...
### `test "leftover Array.fill generic set through one runtime tail"` (`src/tests/exec.zig:11170`)
...
### `test "leftover Array.indexOf direction through one runtime walk"` (`src/tests/exec.zig:11196`)
...
### `test "leftover Array.reduce direction through one runtime walk"` (`src/tests/exec.zig:11224`)
...
### `test "leftover iterator wrap next return through one runtime kind"` (`src/tests/exec.zig:11251`)
...
### `test "stamped native data-method leftover runtime stamp preserves async generator and iterator helpers"` (`src/tests/exec.zig:11279`)
...
### `test "fast prototype method leftover runtime domain preserves regexp and collection lookups"` (`src/tests/exec.zig:11312`)
...
### `test "data view extras leftover optional species preserves accessors and omits species"` (`src/tests/exec.zig:11364`)
...
### `test "defineNativeDataMethod leftover optional native id preserves iterator methods"` (`src/tests/exec.zig:11392`)
...
### `test "createStringValue leftover noinline preserves empty flags and ascii strings"` (`src/tests/exec.zig:11414`)
...
### `test "Engine eval TypeError with evaluated arguments does not double free constants"` (`src/tests/exec.zig:11427`)
...
### `test "vm call handler accepts allocator-backed argument lists"` (`src/tests/exec.zig:11440`)
...
### `test "Engine API eval and job queue are wired"` (`src/tests/exec.zig:11492`)
...
### `test "job queue enqueue propagates allocator failure"` (`src/tests/exec.zig:11541`)
...
### `test "prepared Promise reactions reserve storage without claiming FIFO order"` (`src/tests/exec.zig:11555`)
...
### `test "waitAsync completions enter one typed cross-realm FIFO after facade release"` (`src/tests/exec.zig:11601`)
...
### `test "public property key coercion accepts a Symbol.toPrimitive key"` (`src/tests/exec.zig:11663`)
...
### `test "waitAsync completion OOM stays at FIFO head for same-runtime retry"` (`src/tests/exec.zig:11682`)
...
### `test "dynamic import job OOM retains its FIFO position for retry"` (`src/tests/exec.zig:11729`)
...
### `test "dynamic import job keeps its enqueue Realm after creator facade release"` (`src/tests/exec.zig:11776`)
...
### `test "dynamic import loader mutates only the enqueue Realm registry after public owner release"` (`src/tests/exec.zig:11818`)
...
### `test "thenable job reservation OOM leaves resolving function retryable"` (`src/tests/exec.zig:11884`)
...
### `test "published Promise resolution survives resolver collection through typed FIFO owner"` (`src/tests/exec.zig:11939`)
...
### `test "Promise reaction retains callable Proxy classification after revocation"` (`src/tests/exec.zig:11993`)
...
### `test "job queue keeps symbol arguments rooted until release"` (`src/tests/exec.zig:12018`)
...
### `test "job queue symbol roots preserve weak map values"` (`src/tests/exec.zig:12044`)
...
### `test "ordinary script entry points do not run full-heap cycle collection on exit"` (`src/tests/exec.zig:12087`)
...
### `test "IC-R1: delete then get_field is undefined after a prior hit"` (`src/tests/exec.zig:12149`)
...
### `test "IC-P1: OrdinarySet forwards to a Proxy proto [[Set]] trap"` (`src/tests/exec.zig:12162`)
...
### `test "RegExp Symbol.split preserves captures returned by custom exec"` (`src/tests/exec.zig:12188`)
...
### `test "RegExp Symbol.split propagates invalid species exec TypeError"` (`src/tests/exec.zig:12206`)
...
### `test "RegExp Symbol.split appends sticky flag without narrowing wide species flags"` (`src/tests/exec.zig:12215`)
...
### `test "flagless RegExp flags accessor reuses the runtime empty string"` (`src/tests/exec.zig:12228`)
...
### `test "RegExp exec result template preserves metadata groups and indices"` (`src/tests/exec.zig:12242`)
...
### `test "RegExp compiler stack overflow is a catchable SyntaxError"` (`src/tests/exec.zig:12252`)
...
### `test "RegExp accepts literal astral group names in non-unicode mode"` (`src/tests/exec.zig:12264`)
...
### `test "RegExp literals reuse parse-time bytecode and the intrinsic realm shape"` (`src/tests/exec.zig:12276`)
...
### `test "RegExp legacy statics preserve the realm snapshot across constructor replacement"` (`src/tests/exec.zig:12300`)
...
### `test "regexp split and global match arrays use the realm Array prototype"` (`src/tests/exec.zig:12323`)
...
### `test "RegExp Symbol.split uses the realm intrinsic default species"` (`src/tests/exec.zig:12331`)
...
### `test "Engine eval preserves global lexical write fast path semantics"` (`src/tests/exec.zig:12344`)
...
### `test "Engine nested functions retain ancestor with environments during finalization"` (`src/tests/exec.zig:12362`)
...
### `test "Engine eval preserves selected with references during updates"` (`src/tests/exec.zig:12379`)
...
### `test "with compound assignment rechecks proxy binding before get and set"` (`src/tests/exec.zig:12402`)
...
### `test "Engine destructuring snapshots with binding references before property reads"` (`src/tests/exec.zig:12418`)
...
### `test "Engine with destructuring assignment reaches const fallback at runtime"` (`src/tests/exec.zig:12450`)
...
### `test "Engine eval assignments capture the target before dynamic var insertion"` (`src/tests/exec.zig:12476`)
...
### `test "Engine arrow eval assignments capture the target before dynamic var insertion"` (`src/tests/exec.zig:12517`)
...
### `test "Engine direct eval captures the caller arguments binding"` (`src/tests/exec.zig:12551`)
...
### `test "Engine arguments writes prefer the current function binding over outer lexical bindings"` (`src/tests/exec.zig:12602`)
...
### `test "Engine direct eval shares top-level lexical cells across nested closures"` (`src/tests/exec.zig:12633`)
...
### `test "Engine constructor parameter defaults use the initialized this binding"` (`src/tests/exec.zig:12671`)
...
### `test "Engine heritage closures retain the initialized inner class-name binding"` (`src/tests/exec.zig:12690`)
...
### `test "Engine inferred class names precede static initialization across named-evaluation sites"` (`src/tests/exec.zig:12714`)
...
### `test "using early exit before await using keeps sync disposal synchronous"` (`src/tests/exec.zig:12840`)
...
### `test "Engine eval preserves local numeric add host output semantics"` (`src/tests/exec.zig:12876`)
...
### `test "get_array_el2 dense indexed call keeps the receiver"` (`src/tests/exec.zig:12891`)
...
### `test "get_array_el dense direct arm preserves hits and indexed fallback"` (`src/tests/exec.zig:12908`)
...
### `test "int32 add sub mul overflow stays a number on the generic binary"` (`src/tests/exec.zig:12924`)
...
### `test "Engine eval preserves collection read host output semantics"` (`src/tests/exec.zig:12941`)
...
### `test "runtime teardown preserves closure capture metadata until objects are destroyed"` (`src/tests/exec.zig:12969`)
...
### `test "cycle teardown preserves restored strong counts for weakly referenced keys"` (`src/tests/exec.zig:12991`)
...
### `test "Engine eval preserves regexp UTF-16 test host output semantics"` (`src/tests/exec.zig:13013`)
...
### `test "Engine eval prepared RegExp call observes same-site property changes"` (`src/tests/exec.zig:13041`)
...
### `test "Engine eval preserves dense array join host output semantics"` (`src/tests/exec.zig:13063`)
...
### `test "Engine eval preserves dense array pop host output semantics"` (`src/tests/exec.zig:13081`)
...
### `test "Engine eval preserves ordinary array pop fast path semantics"` (`src/tests/exec.zig:13101`)
...
### `test "empty native array pop fast arm preserves observable length writes"` (`src/tests/exec.zig:13126`)
...
### `test "array pop length write removes elements added by the last-element getter"` (`src/tests/exec.zig:13172`)
...
### `test "array pop reports read-only length after deleting a configurable last element"` (`src/tests/exec.zig:13195`)
...
### `test "Engine eval preserves simple closure call host output semantics"` (`src/tests/exec.zig:13213`)
...
### `test "Engine eval preserves one-shot array literal host output semantics"` (`src/tests/exec.zig:13227`)
...
### `test "Engine eval preserves one-shot array named property host output semantics"` (`src/tests/exec.zig:13250`)
...
### `test "Engine eval preserves typed array constructor length host output semantics"` (`src/tests/exec.zig:13274`)
...
### `test "Engine eval preserves Int32Array indexed read fast path semantics"` (`src/tests/exec.zig:13295`)
...
### `test "strict arguments preserve qjs intrinsic metadata and dense element semantics"` (`src/tests/exec.zig:13380`)
...
### `test "mapped arguments use var-ref indexed storage and detach on descriptor changes"` (`src/tests/exec.zig:13445`)
...
### `test "apply resolves arguments length and preserves observable fallback"` (`src/tests/exec.zig:13532`)
...
### `test "resident generators preserve mapped arguments parameter aliases"` (`src/tests/exec.zig:13577`)
...
### `test "implicit arguments resolution preserves mapped aliases"` (`src/tests/exec.zig:13601`)
...
### `test "body function named arguments does not create a synthetic lexical collision"` (`src/tests/exec.zig:13646`)
...
### `test "resident mapped arguments share one open bare arg slot"` (`src/tests/exec.zig:13653`)
...
### `test "generic arg opcodes preserve mapped aliases in a bare resident slot"` (`src/tests/exec.zig:13702`)
...
### `test "generator mapped arguments closures and direct eval share one alias across resumes"` (`src/tests/exec.zig:13744`)
...
### `test "async mapped arguments and closures retain one alias across await"` (`src/tests/exec.zig:13780`)
...
### `test "escaped generator arg aliases retain resident backing across cycle collection"` (`src/tests/exec.zig:13809`)
...
### `test "generator completion closes escaped arg aliases before releasing resident backing"` (`src/tests/exec.zig:13857`)
...
### `test "get_length preserves qjs own-property-before-exotic ordering and actions"` (`src/tests/exec.zig:13898`)
...
### `test "missing-argument plain calls preserve parameter and arguments ownership"` (`src/tests/exec.zig:14060`)
...
### `test "inline calls release lazily materialized arguments state"` (`src/tests/exec.zig:14129`)
...
### `test "inline empty leaf abrupt teardown releases pending operands"` (`src/tests/exec.zig:14156`)
...
### `test "exact-args leaf abrupt teardown releases borrowed args exactly once"` (`src/tests/exec.zig:14180`)
...
### `test "leaf returns with leftover operands route through general teardown"` (`src/tests/exec.zig:14218`)
...
### `test "missing-argument calls read undefined across every entry arm"` (`src/tests/exec.zig:14253`)
...
### `test "missing-argument calls on leaf-excluded shapes keep generic-path outcomes"` (`src/tests/exec.zig:14359`)
...
### `test "missing-argument leftover-carrying returns route through general teardown"` (`src/tests/exec.zig:14409`)
...
### `test "zero-arg leaf leftover bodies are refused publication and balance rc"` (`src/tests/exec.zig:14441`)
...
### `test "capture leaf abrupt teardown releases operands and keeps borrowed cells"` (`src/tests/exec.zig:14514`)
...
### `test "capture leaf returns with leftover operands route through general teardown"` (`src/tests/exec.zig:14555`)
...
### `test "capture leaf shares live cells with its closure across calls"` (`src/tests/exec.zig:14593`)
...
### `test "inline empty leaf warm constructor preserves miss fallback and ownership"` (`src/tests/exec.zig:14625`)
...
### `test "forwarded leaf call semantics keep exclusions on the authoritative path"` (`src/tests/exec.zig:14726`)
...
### `test "forwarded leaf abrupt completion balances and keeps the native frame"` (`src/tests/exec.zig:14756`)
...
### `test "forwarded leaf returns with leftover operands route through general teardown"` (`src/tests/exec.zig:14796`)
...
### `test "method call empty leaf binds receiver as this and balances refcounts"` (`src/tests/exec.zig:14828`)
...
### `test "method call empty leaf abrupt teardown releases receiver"` (`src/tests/exec.zig:14862`)
...
### `test "method empty leaf warm constructor moves receiver ownership"` (`src/tests/exec.zig:14884`)
...
### `test "strict empty leaf preserves undefined this across call forms"` (`src/tests/exec.zig:14985`)
...
### `test "strict method empty leaf passes primitive receiver uncoerced"` (`src/tests/exec.zig:15027`)
...
### `test "strict empty leaf frame preserves undefined this and borrowed ownership"` (`src/tests/exec.zig:15064`)
...
### `test "inline call teardown releases every escaped storage shape"` (`src/tests/exec.zig:15130`)
...
### `test "inline operand Stack keeps limit and ownership flags in one word"` (`src/tests/exec.zig:15212`)
...
### `test "ordinary root bytecode call carves one operand window"` (`src/tests/exec.zig:15222`)
...
### `test "method calls preserve receiver arguments eval captures and abrupt ownership"` (`src/tests/exec.zig:15258`)
...
### `test "primitive prototype lookup preserves raw receiver and exotic prototype semantics"` (`src/tests/exec.zig:15352`)
...
### `test "computed named reads preserve prototype accessors proxies and operand ownership"` (`src/tests/exec.zig:15462`)
...
### `test "computed integer write misses preserve generic set semantics"` (`src/tests/exec.zig:15530`)
...
### `test "dense write leaf consumes reserved appends only inside the qjs capacity window"` (`src/tests/exec.zig:15600`)
...
### `test "static named getter and proxy fast paths preserve receivers throws and invariants"` (`src/tests/exec.zig:15654`)
...
### `test "proxy bytecode get continuation does not require spare operand capacity"` (`src/tests/exec.zig:15757`)
...
### `test "for-of bytecode next continuation preserves result and abrupt semantics"` (`src/tests/exec.zig:15774`)
...
### `test "IteratorNext bound proxy and native throws do not close the iterator"` (`src/tests/exec.zig:15903`)
...
### `test "destructuring abrupt completion closes every live outer iterator"` (`src/tests/exec.zig:15928`)
...
### `test "array destructuring rest roots direct symbol values while creating its result"` (`src/tests/exec.zig:15956`)
...
### `test "computed object-rest keys perform observable ToPropertyKey once"` (`src/tests/exec.zig:15975`)
...
### `test "object destructuring does not turn its source into a with environment"` (`src/tests/exec.zig:15998`)
...
### `test "object destructuring ToObject uses the current realm primitive prototypes"` (`src/tests/exec.zig:16014`)
...
### `test "for-in-of generic lvalues use QuickJS bottom-stack evaluation order"` (`src/tests/exec.zig:16033`)
...
### `test "computed proxy bytecode trap continuations preserve nested calls throws and invariants"` (`src/tests/exec.zig:16082`)
...
### `test "native tail calls preserve iterator and proxy continuation success and throws"` (`src/tests/exec.zig:16308`)
...
### `test "return conditional followed by newline comma keeps the comma expression"` (`src/tests/exec.zig:16377`)
...
### `test "Phase 7: inlined arrow keeps lexical this and ignores any receiver"` (`src/tests/exec.zig:16392`)
...
### `test "arrow direct eval reads captured this and new.target"` (`src/tests/exec.zig:16412`)
...
### `test "direct eval inherits QuickJS entry capabilities and var environment"` (`src/tests/exec.zig:16432`)
...
### `test "class field direct eval keeps QuickJS field initializer capabilities"` (`src/tests/exec.zig:16466`)
...
### `test "public instance fields initialize once in constructor order on every path"` (`src/tests/exec.zig:16482`)
...
### `test "arrow super property call keeps the enclosing method receiver"` (`src/tests/exec.zig:16600`)
...
### `test "super property assignment respects strictness when inherited descriptors reject writes"` (`src/tests/exec.zig:16632`)
...
### `test "bytecode constructability follows canonical function shape"` (`src/tests/exec.zig:16676`)
...
### `test "forwarded call releases ignored arrow thisArg"` (`src/tests/exec.zig:16704`)
...
### `test "function inherited data lookup preserves own and exotic semantics"` (`src/tests/exec.zig:16739`)
...
### `test "function caller and arguments restrictions follow immutable function shape"` (`src/tests/exec.zig:16794`)
...
### `test "Engine eval preserves one-shot object missing field host output semantics"` (`src/tests/exec.zig:16895`)
...
### `test "Engine eval preserves local string substring host output semantics"` (`src/tests/exec.zig:16914`)
...
### `test "String prim_self leaf arms (lane K) agree with the legacy bodies on every miss shape"` (`src/tests/exec.zig:16929`)
...
### `test "String index-read native records preserve primitive fast paths and observable coercion"` (`src/tests/exec.zig:16970`)
...
### `test "mod cold handler preserves fmod and ToNumeric fallbacks"` (`src/tests/exec.zig:16992`)
...
### `test "resident is_null preserves qjs true and refcounted false legs"` (`src/tests/exec.zig:17039`)
...
### `test "Engine generator return keeps finally rethrow control marker"` (`src/tests/exec.zig:17056`)
...
### `test "generator return runs nested finally before closing its for-of iterator"` (`src/tests/exec.zig:17077`)
...
### `test "return cleanup restores outer catch targets before finally and IteratorClose throws"` (`src/tests/exec.zig:17111`)
...
### `test "generator return crosses catch markers before closing its for-of iterator"` (`src/tests/exec.zig:17170`)
...
### `test "generator return closes an inner for-of iterator before its enclosing finally"` (`src/tests/exec.zig:17210`)
...
### `test "destructuring rest parameter defaults use the parameter environment"` (`src/tests/exec.zig:17259`)
...
### `test "caught destructuring error preserves IteratorClose output"` (`src/tests/exec.zig:17280`)
...
### `test "generator parameter eval cells close before body resume"` (`src/tests/exec.zig:17292`)
...
### `test "generator return executes an add_loc-terminated shared finally before completing"` (`src/tests/exec.zig:17311`)
...
### `test "generator default argument stores release refcounted stack values"` (`src/tests/exec.zig:17332`)
...
### `test "spread super brands derived instances before class field initializers"` (`src/tests/exec.zig:17344`)
...
### `test "computed class keys close over runtime private field identity"` (`src/tests/exec.zig:17368`)
...
### `test "nested same-name private fields isolate repeated class evaluations"` (`src/tests/exec.zig:17386`)
...
### `test "private fields isolate class evaluations and preserve lexical call and eval semantics"` (`src/tests/exec.zig:17430`)
...
### `test "private method brands use lexical initializers on every constructor path"` (`src/tests/exec.zig:17556`)
...
### `test "private methods and accessors preserve brands captures and readonly semantics"` (`src/tests/exec.zig:17614`)
...
### `test "started generator resumes preserve unmapped arguments from parked locals"` (`src/tests/exec.zig:17833`)
...
### `test "array named proto field uses ordinary lookup; length and index stay exotic"` (`src/tests/exec.zig:17888`)
...
### `test "iterator results use ordinary transitions without a sixth realm shape"` (`src/tests/exec.zig:17909`)
...
### `test "bytecode closures reuse the final function-prototype shape"` (`src/tests/exec.zig:17933`)
...
### `test "escaped closure keeps its compile realm after facade destruction"` (`src/tests/exec.zig:17956`)
...
### `test "standard constructors publish realm class prototype slots"` (`src/tests/exec.zig:17998`)
...
### `test "FunctionRealm query separates owned carriers from caller-semantics classes"` (`src/tests/exec.zig:18033`)
...
### `test "async resume callbacks remain callable and nonconstructible to all consumers"` (`src/tests/exec.zig:18076`)
...
### `test "fulfilled await queues a direct resume and retains suspended values"` (`src/tests/exec.zig:18098`)
...
### `test "fulfilled await checks state after the constructor getter settles its input"` (`src/tests/exec.zig:18170`)
...
### `test "fulfilled await roots its continuation through constructor getter GC"` (`src/tests/exec.zig:18199`)
...
### `test "pending and rejected await retain both callbacks and execute rejection recovery"` (`src/tests/exec.zig:18261`)
...
### `test "async direct settlement preserves adoption self resolution and once guards"` (`src/tests/exec.zig:18301`)
...
### `test "async resume callbacks preserve thenable metadata microtasks and realms"` (`src/tests/exec.zig:18345`)
...
### `test "generator async and wrapper noncarriers derive cross-realm state across GC"` (`src/tests/exec.zig:18402`)
...
### `test "FinalizationRegistry cleanup job keeps registry realm before invoking callback realm"` (`src/tests/exec.zig:18493`)
...
### `test "event-loop caller reaches external C function with one callee realm view"` (`src/tests/exec.zig:18569`)
...
### `test "true C function without its RealmRef fails the final-arm invariant"` (`src/tests/exec.zig:18646`)
...
### `test "legacy output writer failure is a catchable named Error"` (`src/tests/exec.zig:18672`)
...
### `test "native host error sentinel always has a pending named JS exception"` (`src/tests/exec.zig:18698`)
...
### `test "generator creation avoids a second payload copy of rooted input slices"` (`src/tests/exec.zig:18719`)
...
### `test "Engine generator return propagates an explicit finally throw"` (`src/tests/exec.zig:18839`)
...
### `test "async generator return awaits for-await iterator close before completing"` (`src/tests/exec.zig:18871`)
...
### `test "async generator return closes an inner iterator before its enclosing finally"` (`src/tests/exec.zig:18899`)
...
### `test "async generator return awaits its value once before a yielding finalizer"` (`src/tests/exec.zig:18926`)
...
### `test "external host arbitrary errors retain their Zig error name"` (`src/tests/exec.zig:18987`)
...
### `test "dynamic import failures preserve unsupported not-found and host I/O mappings"` (`src/tests/exec.zig:19008`)
...
### `test "static module read failures are named JS exceptions"` (`src/tests/exec.zig:19077`)
...
### `test "stalled module host progress is a named InternalError"` (`src/tests/exec.zig:19112`)
...
### `test "host module graph syntax diagnostics do not write to program output"` (`src/tests/exec.zig:19139`)
...
### `test "host commonjs wrapper passes directory dirname"` (`src/tests/exec.zig:19169`)
...
### `test "module graph evaluates block var declarations as module bindings"` (`src/tests/exec.zig:19205`)
...
### `test "module evaluation does not skip a body-leading function expression"` (`src/tests/exec.zig:19231`)
...
### `test "module evaluation does not mistake a body-leading this branch for a hoist prologue"` (`src/tests/exec.zig:19252`)
...
### `test "module cycles initialize wide function declaration closures before evaluation"` (`src/tests/exec.zig:19274`)
...
### `test "module cycles do not hoist a body-leading named function expression"` (`src/tests/exec.zig:19331`)
...
### `test "W1e: module namespace exposes sorted immutable live export properties"` (`src/tests/exec.zig:19378`)
...
### `test "module namespace has and super set preserve uninitialized export semantics"` (`src/tests/exec.zig:19446`)
...
### `test "W1e: named aliases and namespace reexports share live canonical bindings"` (`src/tests/exec.zig:19502`)
...
### `test "W1e: missing indirect export precedes bad import wiring"` (`src/tests/exec.zig:19559`)
...
### `test "W1e: one host source load spans declaration body TLA resume and dynamic import"` (`src/tests/exec.zig:19611`)
...
### `test "same module specifier keeps record cells namespace import meta and error state per Realm"` (`src/tests/exec.zig:19682`)
...
### `test "context module eval does not rerun evaluated or errored records"` (`src/tests/exec.zig:19749`)
...
### `test "context module eval resumes TLA from its reaction FIFO position"` (`src/tests/exec.zig:19804`)
...
### `test "Runtime loader keeps same-path TLA continuations and waiters in parent and child Realms"` (`src/tests/exec.zig:19839`)
...
### `test "module top-level await resumes in Promise reaction FIFO order"` (`src/tests/exec.zig:19936`)
...
### `test "module await reaction keeps its position on the awaited Promise"` (`src/tests/exec.zig:19974`)
...
### `test "module TLA continuation OOM retains FIFO node for retry"` (`src/tests/exec.zig:20005`)
...
### `test "async module dependency does not preempt an independent sibling"` (`src/tests/exec.zig:20148`)
...
### `test "import bytes module creates immutable ArrayBuffer backing store"` (`src/tests/exec.zig:20204`)
...
### `test "host global bootstrap installs and tears down builtin plus host domains"` (`src/tests/exec.zig:20332`)
...
### `test "engine eval host globals and throw intrinsic tear down cleanly"` (`src/tests/exec.zig:20344`)
...
### `test "reflect construct roots argument list while resolving prototype"` (`src/tests/exec.zig:20395`)
...
### `test "short conditional branches preserve immediate and full ToBoolean semantics"` (`src/tests/exec.zig:20466`)
...
### `test "if-throw fall-off form returns undefined (if_false8 branch-to-end)"` (`src/tests/exec.zig:20494`)
...
### `test "if-return fall-off form returns undefined on the fall-through leg"` (`src/tests/exec.zig:20508`)
...
### `test "else-return goto-to-end form returns undefined on the taken if leg"` (`src/tests/exec.zig:20520`)
...
### `test "nested-block branch-to-end survives trailing scope cleanup lowering"` (`src/tests/exec.zig:20532`)
...
### `test "arrow block body branch-to-end returns undefined"` (`src/tests/exec.zig:20549`)
...
### `test "generator branch-to-end completes with undefined value"` (`src/tests/exec.zig:20563`)
...
### `test "eval and script completion end in an explicit value return"` (`src/tests/exec.zig:20582`)
...
### `test "eval preserves completion through nested shared finalizers"` (`src/tests/exec.zig:20603`)
...
### `test "module top-level branch-to-end gets a terminator (no fall-off)"` (`src/tests/exec.zig:20615`)
...
### `test "W1d: module import.meta identity survives methods and nested closures"` (`src/tests/exec.zig:20624`)
...
### `test "module function declaration cells do not leak onto the global object"` (`src/tests/exec.zig:20646`)
...
### `test "call consumers derive receiver and direct-eval provenance from the final opcode"` (`src/tests/exec.zig:20663`)
...
### `test "optional chains use one unbounded shared label and preserve closed-chain calls"` (`src/tests/exec.zig:20716`)
...
### `test "direct eval inside a module function forwards module live bindings"` (`src/tests/exec.zig:20740`)
...
### `test "dynamic global put keeps cell and global-object legs semantically separate"` (`src/tests/exec.zig:20753`)
...
### `test "get_var uninitialized-cell inline global-object leg preserves the cold waterfall semantics"` (`src/tests/exec.zig:20863`)
...
### `test "named function expression self-binding materializes lazily with pinned QuickJS semantics"` (`src/tests/exec.zig:20930`)
...
### `test "K2 warm leaf miss retreat keeps call accounting balanced across chunk and carve misses"` (`src/tests/exec.zig:21010`)
...
### `test "latin1 high bytes survive raw-string byte bridges (qjs JS_ToCStringLen2 mirror)"` (`src/tests/exec.zig:21068`)
...
### `test "ToNumber latin1 high bytes are code points not UTF-8 whitespace"` (`src/tests/exec.zig:21115`)
...
### `test "JSON.rawJSON latin1 payload survives the simple stringify byte buffer"` (`src/tests/exec.zig:21181`)
...
### `test "native function toString keeps non-ASCII identifier names (qjs js_function_toString)"` (`src/tests/exec.zig:21196`)
...
### `test "switch dispatch trampoline shapes keep their identity and semantics"` (`src/tests/exec.zig:21212`)
...
### `test "switch fallthrough after while-family tails reaches the next case"` (`src/tests/exec.zig:21378`)
...
### `test "small-function-inlining: setter throw stack is setter, ctor, caller"` (`src/tests/exec.zig:21440`)
...
### `test "small-function-inlining: redefinition takes the new function"` (`src/tests/exec.zig:21465`)
...
### `test "small-function-inlining: new C field write is visible"` (`src/tests/exec.zig:21481`)
...
### `test "small-function-inlining: polymorphic site is not specialized"` (`src/tests/exec.zig:21494`)
...
### `test "small-function-inlining: R-2 getter on callee is invoked once per new"` (`src/tests/exec.zig:21525`)
...
### `test "small-function-inlining: inner throw stack and caller catch"` (`src/tests/exec.zig:21545`)
...
### `test "small-function-inlining: primitive ctor return keeps instance"` (`src/tests/exec.zig:21564`)
...
### `test "small-function-inlining: Reflect.construct with foreign NewTarget is not expanded"` (`src/tests/exec.zig:21578`)
...
### `test "small-function-inlining: derived class constructor is not eligible"` (`src/tests/exec.zig:21594`)
...
### `test "small-function-inlining: next-entry specialize is installed on the caller"` (`src/tests/exec.zig:21608`)
...
### `test "small-function-inlining: spec copy keeps simple_inline bits after extra TAKE locals"` (`src/tests/exec.zig:21633`)
...
### `test "small-function-inlining: sibling constructor sites both specialize"` (`src/tests/exec.zig:21667`)
...
### `test "small-function-inlining: proto replacement after specialize is observed"` (`src/tests/exec.zig:21693`)
...
### `test "small-function-inlining: call_constructor callers keep published frame geometry"` (`src/tests/exec.zig:21713`)
...
### `test "small-function-inlining: leftover-operand bodies are not small-inline eligible"` (`src/tests/exec.zig:21731`)
...
### `test "small-function-inlining: leftover ctor is not specialized and does not overflow"` (`src/tests/exec.zig:21755`)
...
### `test "small-function-inlining: extra ctor args do not overwrite callee fields"` (`src/tests/exec.zig:21779`)
...
### `test "small-function-inlining: monomorphic method is expanded"` (`src/tests/exec.zig:21802`)
...
### `test "small-function-inlining L1: apply-arguments ctor specializes"` (`src/tests/exec.zig:21816`)
...
### `test "small-function-inlining L1: next-entry take does not leak initialize return"` (`src/tests/exec.zig:21842`)
...
### `test "small-function-inlining L1: forwarded argc is the site argc"` (`src/tests/exec.zig:21859`)
...
### `test "small-function-inlining L1: Error.stack is initialize, apply native, ctor"` (`src/tests/exec.zig:21874`)
...
### `test "small-function-inlining L1: own apply misses take"` (`src/tests/exec.zig:21897`)
...
### `test "small-function-inlining L1: replaced Function.prototype.apply misses take"` (`src/tests/exec.zig:21916`)
...
### `test "flat string strict-eq matches content across distinct objects"` (`src/tests/exec.zig:21941`)
...
### `test "active invocation Adapter traces live VM windows and not unused stack capacity"` (`src/tests/exec.zig:22103`)
...
### `test "C0: final artifacts carry the carrier encoding and no direct to_propkey"` (`src/tests/exec.zig:22136`)
...
### `test "C0 closed: the carrier encoding executes and the quarantined direct id is rejected"` (`src/tests/exec.zig:22228`)
...
### `test "C0/D7: an unknown carrier tag is rejected by the artifact proof and by dispatch"` (`src/tests/exec.zig:22252`)
...
### `test "C1-1: computed-name function naming rides the carrier encoding"` (`src/tests/exec.zig:22273`)
...
### `test "TGC S3: JSON.parse object keys stay reachable across majors taken mid-parse"` (`src/tests/exec.zig:22347`)
...
### `test "TGC S3-c: operand-stack strings stay rooted while a later push materializes its atom"` (`src/tests/exec.zig:22406`)
...
### `test "TGC S3-d: an inline call's argument region stays rooted while the callee frame is pushed"` (`src/tests/exec.zig:22586`)
...
### `test "TGC S3-d: op_put_array_el's cold arm publishes before the dense grow allocates"` (`src/tests/exec.zig:22662`)
...
### `test "TGC S3-d: the string-primitive get_field2 arm publishes before the auto-init resolver allocates"` (`src/tests/exec.zig:22716`)
...
### `test "an unresolved binding names its identifier in the ReferenceError message (qjs JS_ThrowReferenceErrorNotDefined)"` (`src/tests/exec.zig:22764`)
...
### `test "print / console.log dump objects like QuickJS JS_PrintValue (qjs-generated expectations, 36 shapes)"` (`src/tests/exec.zig:22787`)
...
### `test "no-suspend async uses a same-Machine completion boundary"` (`src/tests/exec.zig:22876`)
...
### `test "no-suspend async preserves parameter exceptions finally aliases and nested boundaries"` (`src/tests/exec.zig:22890`)
...
### `test "no-suspend async leaves suspension eval host-observed cadence and wrappers on fallback"` (`src/tests/exec.zig:22919`)
...
### `test "no-suspend async completion roots survive declared-only GC before and after frame pop"` (`src/tests/exec.zig:22938`)
...
### `test "no-suspend async final code policy includes hidden suspension and excludes nested bodies"` (`src/tests/exec.zig:22982`)
...
### `test "no-suspend async transfers post-body OOM to FIFO without replay"` (`src/tests/exec.zig:23008`)
...
### `test "P-Cap intrinsic then preserves independent results and pending handlers"` (`src/tests/exec.zig:23068`)
...
### `test "P-Cap species observation keeps custom capability handshake"` (`src/tests/exec.zig:23088`)
...
### `test "P-Cap intrinsic prototype survives species replacing global Promise"` (`src/tests/exec.zig:23112`)
...
### `test "P-Cap thenable resolution identity thrower and FIFO preserve single observation"` (`src/tests/exec.zig:23135`)
...
### `test "P-Cap foreign species falls back and delayed foreign settlement keeps self error realm"` (`src/tests/exec.zig:23161`)
...
### `test "P-Cap post-handler and post-getter OOM retain FIFO completion without replay"` (`src/tests/exec.zig:23185`)
...
### `test "P-Cap retries reserved reaction phase before then getter without replaying handler"` (`src/tests/exec.zig:23235`)
...
### `test "fulfilled await preserves FIFO and bypasses an overridden then"` (`src/tests/exec.zig:23292`)
...
### `test "fulfilled await preserves the registration and body realms"` (`src/tests/exec.zig:23312`)
...
### `test "fulfilled await does not replay a resumed body after allocation failure"` (`src/tests/exec.zig:23341`)
...
### `test "print writes top-level strings raw including latin1 high bytes"` (`src/tests/exec.zig:23380`)
