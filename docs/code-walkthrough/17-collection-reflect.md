...
- **实现**：`expectObject` 把 receiver 值转成对象，再 `methodCallResolved(rt, null, globalObjectFromGlobals(rt, host.globals), object, method, args, host)`——ctx 传 null，global 只能从 legacy `globals` 槽里的 `globalThis` 反推。
- **所有权 / 错误 / 调用**：返回值归调用方；`collectionCall` 的无 global 分支与 `core` 侧集合算法复用点用它。

### `methodCallWithContext` (`src/exec/collection_ops.zig:321`)

- **签名**：`pub fn methodCallWithContext( ctx: *core.JSContext, object_value: core.JSValue, method: u32, args: []const core.JSValue, globals: []globals_mod.Slot, ) !core.JSValue`。
- **作用**：带 ctx、但 global 仍靠 legacy 槽反推的方法入口。
- **实现**：转发 `methodCallWithContextAndHost`，`CallbackHost` 只填 `.globals = globals`（`call` 为 null，所以同样不能驱动用户回调）。
- **所有权 / 错误 / 调用**：不分配也不建根（根在 `methodCallResolved` 里统一建）；`args`/`globals` 都只借用调用帧的切片。返回值是被调方法的结果，对象形态归 GC 管、调用方负责继续持有。error 全部来自被转发的 `methodCallWithContextAndHost`。树内调用方只有 `src/tests/core.zig:11274`。

### `methodCallWithContextAndHost` (`src/exec/collection_ops.zig:331`)
...
- **作用**：`methodCallWithContext` 的显式 host 版本。
- **实现**：`expectObject` 后 `methodCallResolved(ctx.runtime, ctx, globalObjectFromGlobals(ctx.runtime, host.globals), object, method, args, host)`：ctx 已知，global 仍从 legacy 槽推。
- **所有权 / 错误 / 调用**：不分配；`expectObject` 失败返回 `error.TypeError`，其余 error 由 `methodCallResolved` 及其调用的具体方法产生。receiver 的根由 `methodCallResolved`（:400-403 的 `rootObjects`/`activate`）负责，本层不建根。树内唯一调用方是薄壳 `methodCallWithContext`（`src/exec/collection_ops.zig:328`）。

### `methodCallWithGlobal` (`src/exec/collection_ops.zig:342`)

- **签名**：`pub fn methodCallWithGlobal( ctx: *core.JSContext, global: *core.Object, object_value: core.JSValue, method: u32, args: []const core.JSValue, globals: []globals_mod.Slot, ) !core.JSValue`。
- **作用**：realm global 已知时的方法入口。
- **实现**：转发 `methodCallWithGlobalAndHost`，host 只带 `.globals`。
- **所有权 / 错误 / 调用**：不分配、不建根，只补一个 `CallbackHost{ .globals = globals }` 后转发；`global` 由调用方保证存活。error 全部透传。树内调用方只有 `src/tests/core.zig:11286`。

### `methodCallWithGlobalAndHost` (`src/exec/collection_ops.zig:353`)
...
- **作用**：`methodCallWithGlobal` 的显式 host 版本。
- **实现**：`expectObject` 后 `methodCallResolved(ctx.runtime, ctx, global, object, method, args, host)`——ctx 与 global 都是直接给的，不再反推。
- **所有权 / 错误 / 调用**：不分配；`expectObject` 失败返回 `error.TypeError`。receiver 的根同样由 `methodCallResolved` 建，本层只做解包。树内唯一调用方是 `methodCallWithGlobal`（`src/exec/collection_ops.zig:350`）。

### `methodCallObjectWithGlobal` (`src/exec/collection_ops.zig:365`)
...
### `methodCallObjectWithGlobalAndHost` (`src/exec/collection_ops.zig:376`)
...
### `methodCallResolved` (`src/exec/collection_ops.zig:387`)
...
### `methodCallDroppedResult` (`src/exec/collection_ops.zig:465`)
...
### `groupBy` (`src/exec/collection_ops.zig:487`)
...
### `groupByWithCallbackHost` (`src/exec/collection_ops.zig:496`)
...
### `mapSet` (`src/exec/collection_ops.zig:523`)
...
### `mapSetNoResult` (`src/exec/collection_ops.zig:528`)
...
### `mapGet` (`src/exec/collection_ops.zig:558`)
...
### `iteratorRealm` (`src/exec/collection_ops.zig:593`)
...
### `collectionIterator` (`src/exec/collection_ops.zig:607`)
...
### `iteratorPrototype` (`src/exec/collection_ops.zig:645`)
...
### `createIteratorPrototype` (`src/exec/collection_ops.zig:672`)
...
### `objectPrototypeFromGlobal` (`src/exec/collection_ops.zig:710`)
...
### `globalObjectFromGlobals` (`src/exec/collection_ops.zig:716`)

- **签名**：`fn globalObjectFromGlobals(_: *core.JSRuntime, globals: []const globals_mod.Slot) ?*core.Object`。
- **作用**：从 legacy global 槽里反推 realm global 对象。
- **实现**：`globals_mod.getByAtom(globals, core.atom.ids.globalThis)` 取值，`expectObject` 转对象，失败返回 null。第一个 `rt` 参数没有使用（签名里就写成 `_`）。
- **所有权 / 错误 / 调用**：借用指针；被 `methodCallWithCallbackHost` 与 `methodCallWithContextAndHost` 使用。

### `defineToStringTag` (`src/exec/collection_ops.zig:723`)
...
### `collectionIteratorNext` (`src/exec/collection_ops.zig:729`)
...
### `iteratorValue` (`src/exec/collection_ops.zig:748`)
...
### `iteratorResult` (`src/exec/collection_ops.zig:773`)
...
### `testCallbackHost` (`src/exec/collection_ops.zig:837`)
...
### `testCallbackCallWithThis` (`src/exec/collection_ops.zig:841`)
...
### `collectionSize` (`src/exec/collection_ops.zig:856`)
...
### `collectionForEach` (`src/exec/collection_ops.zig:861`)

- **签名**：`fn collectionForEach( _: *core.JSRuntime, object: *core.Object, args: []const core.JSValue, host: CallbackHost, ) !core.JSValue`（第一个 `rt` 参数未使用）。
- **作用**：裸 `CallbackHost` 版本的 `Map/Set.prototype.forEach`。
- **实现**：class 必须是 `map` 或 `set`；`args[0]` 必须过 `isCallableObject`，否则 `error.TypeError`；`this_arg` 取 `args[1]`，缺省 `undefined`。遍历前 `object.retainCollectionCursor()` / defer `releaseCollectionCursor()`：qjs `js_map_forEach`（quickjs.c:52318-52332）锁住当前 record 再前进，zjs 按下标走，所以锁的是 entry 数组——回调可以删条目，但槽位不能在游标下移动。循环按下标读条目（先自增），跳过 `!entry.active`，key 与 value 先取出本地副本（对应 quickjs.c:52322 的「must duplicate in case the record is deleted」），Set 的 value 就是 key；回调实参是 `(value, key, object.value())`，通过 `host.callWithThis(args[0], this_arg, ...)` 发出。结束返回 `undefined`。
- **所有权 / 错误 / 调用**：不分配；空 `CallbackHost` 会让 `callWithThis` 直接 `error.TypeError`。

### `mapGetOrInsert` (`src/exec/collection_ops.zig:891`)
...
### `mapGetOrInsertComputed` (`src/exec/collection_ops.zig:908`)
...
### `canonicalizeKey` (`src/exec/collection_ops.zig:945`)
...
### `collectionHas` (`src/exec/collection_ops.zig:952`)
...
### `collectionDelete` (`src/exec/collection_ops.zig:963`)
...
### `collectionDeleteNoResult` (`src/exec/collection_ops.zig:967`)
...
### `collectionDeleteBool` (`src/exec/collection_ops.zig:971`)
...
### `collectionClear` (`src/exec/collection_ops.zig:985`)
...
### `setAdd` (`src/exec/collection_ops.zig:997`)
...
### `setAddNoResult` (`src/exec/collection_ops.zig:1002`)
...
### `setComposition` (`src/exec/collection_ops.zig:1038`)
...
### `setComparison` (`src/exec/collection_ops.zig:1133`)
...
### `setLikeRecord` (`src/exec/collection_ops.zig:1179`)

- **签名**：`fn setLikeRecord(rt: *core.JSRuntime, object: *core.Object) !SetLikeRecord`。
...
### `setLikeSize` (`src/exec/collection_ops.zig:1185`)

- **签名**：`fn setLikeSize(_: *core.JSRuntime, object: *core.Object) !usize`（`rt` 参数未使用）。
...
### `validateSetLikeMethods` (`src/exec/collection_ops.zig:1193`)

- **签名**：`fn validateSetLikeMethods(_: *core.JSRuntime, object: *core.Object) !void`（`rt` 参数未使用）。
...
### `setLikeHas` (`src/exec/collection_ops.zig:1205`)
...
### `setLikeKeys` (`src/exec/collection_ops.zig:1219`)
...
### `appendValue` (`src/exec/collection_ops.zig:1249`)
...
### `freeValueList` (`src/exec/collection_ops.zig:1268`)
...
### `Trigger.trigger` (`src/exec/collection_ops.zig:1293`)
...
### `groupString` (`src/exec/collection_ops.zig:1323`)
...
### `addGroupedItem` (`src/exec/collection_ops.zig:1339`)
...
### `appendArrayValue` (`src/exec/collection_ops.zig:1380`)
...
### `stringElementAt` (`src/exec/collection_ops.zig:1385`)
...
- **实现**：先 `ensureFlat`；读 `index.*` 处的码元并把下标 +1；若它是高代理且后面还有码元、且下一个是低代理，就再 +1 并用两个码元 `String.createUtf16` 造双码元字符串；否则用单个码元造字符串。孤立代理原样保留。
- **所有权 / 错误 / 调用**：返回的新字符串归调用方（立即交给 `addGroupedItem`）。

### `isCallableClosure` (`src/exec/collection_ops.zig:1403`)
...
### `isCallableObject` (`src/exec/collection_ops.zig:1410`)
...
### `collectionClassId` (`src/exec/collection_ops.zig:1442`)
...
### `defineNativeMethodWithRecordId` (`src/exec/collection_ops.zig:1455`)
...
### `defineNativeMethods` (`src/exec/collection_ops.zig:1465`)
...
### `stringFromValue` (`src/exec/collection_ops.zig:1503`)
...
### `ValueListRoot.init` (`src/exec/collection_ops.zig:1545`)
...
### `ValueListRoot.deinit` (`src/exec/collection_ops.zig:1554`)
...
### `collectionNativeRecord` (`src/exec/collection_ops.zig:1561`)
...
### `collectionCallResultIsDropped` (`src/exec/collection_ops.zig:1657`)
...
### `collectionForEachRecord` (`src/exec/collection_ops.zig:1661`)
...
### `setMethodRecord` (`src/exec/collection_ops.zig:1700`)
...
### `setMethodModeFromRecord` (`src/exec/collection_ops.zig:1728`)
...
### `getSetRecord` (`src/exec/collection_ops.zig:1741`)
...
### `setStrongSize` (`src/exec/collection_ops.zig:1814`)
...
### `constructPlainSet` (`src/exec/collection_ops.zig:1822`)
...
### `setAddValue` (`src/exec/collection_ops.zig:1827`)
...
### `setDeleteValue` (`src/exec/collection_ops.zig:1831`)
...
### `setHasValue` (`src/exec/collection_ops.zig:1835`)
...
### `setLikeHasCall` (`src/exec/collection_ops.zig:1840`)
...
### `setLikeKeysIterator` (`src/exec/collection_ops.zig:1865`)
...
### `setCloneReceiver` (`src/exec/collection_ops.zig:1894`)
...
### `setSnapshotKeys` (`src/exec/collection_ops.zig:1905`)
...
### `setDifference` (`src/exec/collection_ops.zig:1936`)
...
### `setIntersection` (`src/exec/collection_ops.zig:1976`)
...
### `setUnion` (`src/exec/collection_ops.zig:2010`)
...
### `setSymmetricDifference` (`src/exec/collection_ops.zig:2031`)
...
### `setIsDisjointFrom` (`src/exec/collection_ops.zig:2056`)
...
### `setIsSubsetOf` (`src/exec/collection_ops.zig:2092`)
...
### `setIsSupersetOf` (`src/exec/collection_ops.zig:2113`)
...
### `mapGroupByCall` (`src/exec/collection_ops.zig:2138`)
...
### `mapGroupByRecord` (`src/exec/collection_ops.zig:2150`)
...
### `mapGetOrInsertComputedCall` (`src/exec/collection_ops.zig:2200`)
...
### `collectionMethodOwnerClass` (`src/exec/collection_ops.zig:2250`)
...
### `canonicalizeMapKey` (`src/exec/collection_ops.zig:2256`)
...
### `throwCollectionReceiverTypeError` (`src/exec/collection_ops.zig:2263`)
...
### `throwCollectionMethodTypeError` (`src/exec/collection_ops.zig:2267`)
...
### `collectionReceiverMessage` (`src/exec/collection_ops.zig:2289`)
...
### `mapAppendGroupByValue` (`src/exec/collection_ops.zig:2299`)
...
`reflectConstruct` 对带 construct 记录的内建（Date/RegExp/String、Array、集合）走 `callConstructRecord`，Number/WeakRef/FinalizationRegistry/TypedArray 各有专门分支，全不命中则造普通对象。`proxyRevocable` 造 `{proxy, revoke}`；revoke 闭包是一个 data function，靠写死的 `.reflect` `proxy_revoke` native id 回到 `revokeProxy`。`reflectHasProperty` / `proxyReflectHasProperty` / `typedArrayReflectHas` 这一组目前只在本文件内互相调用。
...
### `reflectHasProperty` (`src/exec/reflect_ops.zig:314`)

- **签名**：`fn reflectHasProperty( ctx: *core.JSContext, output: ?*std.Io.Writer, global: ?*core.Object, globals: []globals_mod.Slot, object: *core.Object, atom_id: core.Atom, ) HostError!bool`。
- **作用**：沿原型链做一次 proxy / TypedArray 感知的 `has` 查询。
- **实现**：对起始对象依次判三件事——是 proxy 就转 `proxyReflectHasProperty`；`typedArrayReflectHas` 给出非 null 结果就直接返回它；`object.hasOwnProperty(atom_id)` 命中返回 true。都不命中时沿 `getPrototype()` 逐级重复同样三步，走到链尾返回 false。
- **所有权 / 错误 / 调用**：不分配；错误按 `HostError` 上抛。与 `proxyReflectHasProperty` 互相递归（trap 缺省时回落到 target）；src 内没有其它调用方，`Reflect.has` 的实际入口是 `reflectHasCall`。

### `proxyReflectHasProperty` (`src/exec/reflect_ops.zig:335`)

- **签名**：`fn proxyReflectHasProperty( ctx: *core.JSContext, output: ?*std.Io.Writer, global: ?*core.Object, globals: []globals_mod.Slot, proxy: *core.Object, atom_id: core.Atom, ) !bool`。
- **作用**：`reflectHasProperty` 的 proxy 分支：调 `has` trap 并校验不变量。
- **实现**：`proxy.proxyTarget()` / `proxy.proxyHandler()` 任一缺失 → `error.TypeError`；target 还要过 `expectObjectArg`。用 `getValuePropertyViaGlobalSlots` 读 handler 的 `has` trap，trap 是 undefined 或 null 时回落到 target 的 `reflectHasProperty`。否则 `object_ops.proxyTrapKeyValue` 把 atom 变成 key 值，`callValueWithThisGlobalsAndGlobal` 以 handler 为 this 调 trap，结果取 `value_ops.isTruthy`。没有 realm global 时走裸运行时回退：trap 为真返回 true；为假则看 target 的自有描述符，若 `configurable == false` 或 target 不可扩展就 `error.TypeError`，否则 false。有 global 时交 `object_ops.validateProxyHasResult` 做规范不变量校验（镜像 `js_proxy_has` 的 exotic 读取）。
- **所有权 / 错误 / 调用**：不分配；只被 `reflectHasProperty` 调用。

### `typedArrayReflectHas` (`src/exec/reflect_ops.zig:364`)

- **签名**：`fn typedArrayReflectHas(rt: *core.JSRuntime, object: *core.Object, atom_id: core.Atom) !?bool`。
- **作用**：对 TypedArray 的规范数字索引键给出 `has` 的确定答案，其它键交回普通查找。
- **实现**：`core.object.typedArrayCanonicalNumericIndex(rt, atom_id)` 的三态：`.none` → 返回 null（不是规范数字索引，调用方继续常规查找）；`.invalid` → false；`.index` → 与 `core.object.typedArrayLength` 比较，`index < length` 为结果，取长度失败（detached 等）按 false。
- **所有权 / 错误 / 调用**：不分配；只被 `reflectHasProperty` 调用（对对象本身和每一层原型各调一次）。

### `isBuiltinConstructorName` (`src/exec/reflect_ops.zig:375`)
...
### `reflectCallForNativeRecord` (`src/exec/reflect_ops.zig:402`)
...
### `reflectSetCall` (`src/exec/reflect_ops.zig:430`)
...
### `reflectIsExtensibleCall` (`src/exec/reflect_ops.zig:501`)
...
### `reflectPreventExtensionsCall` (`src/exec/reflect_ops.zig:514`)
...
### `reflectConstructCall` (`src/exec/reflect_ops.zig:531`)
...
### `reflectHasCall` (`src/exec/reflect_ops.zig:559`)
...
### `reflectApplyCall` (`src/exec/reflect_ops.zig:577`)
...
### `reflectGetCall` (`src/exec/reflect_ops.zig:615`)
...
### `reflectOwnKeysCall` (`src/exec/reflect_ops.zig:630`)
...
`.reflect` domain：13 个 Reflect.* + Proxy.revocable + revoke。`reflectCall` 要求可观察调用的 `callableRealm`。`RevocableProxy.revoke` 只把 `revoked=true`。
...
### `ownKeys` (`src/exec/reflect_proxy_ops.zig:116`)

- **签名**：`pub fn ownKeys(rt: *core.JSRuntime, object: *core.Object) ![]core.Atom`。
- **作用**：`object.ownKeys` 的薄转发封装。
- **实现**：直接 `return object.ownKeys(rt)`，不做过滤也不加工。
- **所有权 / 错误 / 调用**：返回的 atom 切片由 `object.ownKeys` 的协议决定归属；src 内当前没有调用方，属于对外导出的便利入口。

### `RevocableProxy.revoke` (`src/exec/reflect_proxy_ops.zig:123`)

- **签名**：`pub fn revoke(self: *RevocableProxy) void`。
- **作用**：把 `RevocableProxy` 这个只有一个 `revoked: bool = false` 字段的辅助结构标记为已撤销。
- **实现**：一句 `self.revoked = true;`。
- **所有权 / 错误 / 调用**：无所有权协议；真正生效的撤销路径是 `reflect_ops.revokeProxy`，`RevocableProxy` 本身在 src 内没有使用方。

## 覆盖核对

- 清单函数数: 133（`src/exec/collection_adapter.zig` 3 + `src/exec/collection_ops.zig` 102 + `src/exec/reflect_ops.zig` 23 + `src/exec/reflect_proxy_ops.zig` 5）
- 本文标题覆盖: 133
- 未覆盖: 无
