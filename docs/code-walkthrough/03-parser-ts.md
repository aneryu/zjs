...
### `isParameterModifier` (`src/parser.zig:2779`)
...
### `parseEnumDeclaration` (`src/parser.zig:8703`)
...
### `parseNamespaceDeclaration` (`src/parser.zig:8819`)
...
### `parseNamespaceDeclarationWithIdent` (`src/parser.zig:8824`)
...
### `parseNamespaceStatement` (`src/parser.zig:8914`)
...
### `parseEnumStatement` (`src/parser.zig:9047`)
...
- 清单函数数（本文件分到）: 6（`src/parser.zig` 全文件 649）
- 本文标题覆盖: 6
- 未覆盖: 无
