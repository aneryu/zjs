...
### `ParseState.pushScopeIdentity` (`src/parser.zig:1427`)
...
### `ParseState.pushScope` (`src/parser.zig:1437`)
...
### `ParseState.beginFunctionBody` (`src/parser.zig:1445`)
...
### `ParseState.beginFunctionBodyIdentityOnly` (`src/parser.zig:1454`)

- **签名**：`pub fn beginFunctionBodyIdentityOnly(self: *State) Error!void`。
- **作用**：创建函数体作用域身份，不发字节码。
- **实现**：pushScopeIdentity 成功后将当前 scope_level 写入 curFunc().body_scope；设置 errdefer 供后续错误路径撤销身份。
- **所有权 / 错误 / 调用**：作用域表由 ParseState 持有；与 beginFunctionBody 的区别是没有 emitEnterScope。

### `ParseState.popScopeIdentity` (`src/parser.zig:1464`)
...
### `ParseState.popScope` (`src/parser.zig:1485`)
...
### `ParseState.addScopeVar` (`src/parser.zig:1497`)
...
### `ParseState.atFunctionBodyScope` (`src/parser.zig:1550`)
...
### `ParseState.atProgramBodyScope` (`src/parser.zig:1554`)
...
### `ParseState.isChildScope` (`src/parser.zig:1558`)
...
### `ParseState.firstGlobalVarIndex` (`src/parser.zig:1571`)
...
### `ParseState.findLexicalDeclarationLegacy` (`src/parser.zig:1583`)
...
### `ParseState.findIndexedLexicalDeclaration` (`src/parser.zig:1606`)
...
### `ParseState.findLexicalDeclaration` (`src/parser.zig:1640`)
...
### `ParseState.findFunctionVarInChildScopeLegacy` (`src/parser.zig:1669`)
...
### `ParseState.findFunctionVarInChildScope` (`src/parser.zig:1677`)
...
### `ParseState.appendFunctionVarAtOrigin` (`src/parser.zig:1705`)
...
### `ParseState.defineVar` (`src/parser.zig:1724`)
...
### `ParseState.scopeHasVar` (`src/parser.zig:1824`)
...
### `ParseState.visibleLexicalScopeVar` (`src/parser.zig:1836`)
...
### `ParseState.findLexicalGlobalVar` (`src/parser.zig:1853`)
...
### `ParseState.findGlobalVar` (`src/parser.zig:1862`)
...
### `ParseState.ensureFunctionScopeVar` (`src/parser.zig:1875`)

- **签名**：`fn ensureFunctionScopeVar(self: *State, name: Atom) Error!u16`。
- **作用**：取当前函数 scope-0（函数级 var/arg 层）里叫 `name` 的行，没有就补建一条并返回其下标。
- **实现**：先 `findFunctionScopeVar(name)`，命中直接返回。没有现成的 scope-0 行时用 `appendFunctionVarAtOrigin(name, 0)` 补一条：Annex B 的 `create_func_var` 路径直接走 `add_var`，这行本来就不挂进 scope 0 的词法链，origin 取 QuickJS 零初始化行留下的 0。（源码里本函数上方那段注释描述的是 `define_var` 的 `is_global_var` 重定义检查，不是这个 helper 的行为。）
...
### `ParseState.findFunctionScopeVar` (`src/parser.zig:1883`)
...
### `ParseState.addGlobalVar` (`src/parser.zig:1893`)
...
### `ParseState.addGlobalAnnexBFunctionVar` (`src/parser.zig:1905`)
...
### `ParseState.addDirectEvalVarObjectVar` (`src/parser.zig:1920`)
...
### `ParseState.emitGlobalScopePutVar` (`src/parser.zig:1933`)
...
### `ParseState.emitEvalVarObjectScopePutVar` (`src/parser.zig:1940`)
...
### `ParseState.enableEvalReturn` (`src/parser.zig:1969`)
...
### `ParseState.enableReturnCompletion` (`src/parser.zig:1979`)
...
### `ParseState.emitEvalRetGet` (`src/parser.zig:1993`)
...
### `ParseState.emitEvalRetPut` (`src/parser.zig:1998`)
...
### `ParseState.finalizeEvalReturn` (`src/parser.zig:2007`)
...
### `ParseState.setEvalReturnUndefined` (`src/parser.zig:2016`)
...
### `ParseState.emitReturnUndefined` (`src/parser.zig:2022`)
...
### `ParseState.hasActiveLabel` (`src/parser.zig:2270`)
...
### `ParseState.pushLabelFrame` (`src/parser.zig:2277`)
...
### `ParseState.patchLabelBreaks` (`src/parser.zig:2294`)
...
### `ParseState.patchLabelContinues` (`src/parser.zig:2302`)
...
### `ParseState.popLabelFrame` (`src/parser.zig:2310`)
...
### `ParseState.findLabelFrame` (`src/parser.zig:2316`)
...
### `ParseState.emitLabelledBreak` (`src/parser.zig:2325`)
...
### `ParseState.emitLabelledContinue` (`src/parser.zig:2329`)
...
### `ParseState.labelStartAtomOwned` (`src/parser.zig:2333`)
...
### `ParseState.isReservedLabelIdentifier` (`src/parser.zig:2342`)
...
### `ParseState.deinitCurrentControlFrames` (`src/parser.zig:2350`)
...
### `ParseState.enterControlBoundary` (`src/parser.zig:2371`)
...
### `ParseState.leaveControlBoundary` (`src/parser.zig:2411`)
...
### `ParseState.truncateClassPrivateElements` (`src/parser.zig:2432`)

- **签名**：`fn truncateClassPrivateElements(self: *State, len: usize) void`。
- **作用**：将类私有元素记录恢复到指定长度。
- **实现**：尾部遍历体为空，实际操作是 shrinkRetainingCapacity(len)，保留容量。
- **所有权 / 错误 / 调用**：不逐项释放 atom 或对象；调用方保证 len 不超过当前长度。

### `ParseState.truncateClassPrivateBoundNames` (`src/parser.zig:2438`)

- **签名**：`fn truncateClassPrivateBoundNames(self: *State, len: usize) void`。
- **作用**：将类私有已绑定名字列表恢复到指定长度。
- **实现**：尾部遍历体为空，再 shrinkRetainingCapacity(len)。
- **所有权 / 错误 / 调用**：不逐项 unpin；保留底层容量，len 必须是有效的恢复水位。

### `ensureImplicitArgumentsLocal` (`src/parser.zig:3078`)
...
### `isDynamicEnvironmentCaptureAtom` (`src/parser.zig:3092`)
...
### `ensureClosureVar` (`src/parser.zig:3098`)
...
### `ensureArrowSpecialCapture` (`src/parser.zig:3292`)
...
### `findVisibleParentVarCapturingWith` (`src/parser.zig:3323`)
...
### `ensureClosureChain` (`src/parser.zig:3371`)
...
### `findClosureVarIndex` (`src/parser.zig:3415`)
...
### `findGlobalClosureVarIndex` (`src/parser.zig:3422`)
...
### `ensureGlobalClosureVarIndex` (`src/parser.zig:3433`)
...
### `emitGlobalVarOp` (`src/parser.zig:3448`)
...
### `emitGlobalVarOpNoSource` (`src/parser.zig:3455`)
...
### `scopeChainContains` (`src/parser.zig:3460`)
...
### `hasVisibleCurrentBinding` (`src/parser.zig:3469`)
...
### `pushBreakFrame` (`src/parser.zig:7786`)
...
### `pushBreakOnlyFrame` (`src/parser.zig:7800`)
...
### `pushControlBlock` (`src/parser.zig:7812`)
...
### `popControlBlock` (`src/parser.zig:7838`)
...
### `setCurrentBreakCleanupDrops` (`src/parser.zig:7843`)
...
### `setCurrentBreakCrossCleanupDrops` (`src/parser.zig:7849`)
...
### `emitUnlabelledBreakCleanup` (`src/parser.zig:7854`)
...
### `emitCrossFrameCleanup` (`src/parser.zig:7859`)
...
### `emitCatchMarkerDropsFromDepth` (`src/parser.zig:7870`)
...
### `emitUsingDisposesForCatchMarkerDepth` (`src/parser.zig:7880`)
...
### `emitUnlabelledBreak` (`src/parser.zig:7892`)
...
### `emitUnlabelledContinue` (`src/parser.zig:7897`)
...
### `enterSwitchContinueCleanup` (`src/parser.zig:7902`)
...
### `leaveSwitchContinueCleanup` (`src/parser.zig:7908`)
...
### `pushReturnFinallyFrame` (`src/parser.zig:10057`)
...
### `popReturnFinallyFrame` (`src/parser.zig:10075`)
...
### `enterReturnFinallyFunctionBoundary` (`src/parser.zig:10080`)
...
### `leaveReturnFinallyFunctionBoundary` (`src/parser.zig:10090`)
...
### `controlTargetCrossesFinallyFrame` (`src/parser.zig:10097`)
...
### `resolveFinallyControlTarget` (`src/parser.zig:10330`)
...
### `controlBlockMatchesTarget` (`src/parser.zig:10380`)
...
### `emitResolvedControlJump` (`src/parser.zig:10393`)
...
### `emitCrossedControlBlockCleanup` (`src/parser.zig:10417`)
...
### `emitControlBlocksUntil` (`src/parser.zig:10432`)
...
### `emitControlThroughFinally` (`src/parser.zig:10468`)
...
- **实现**：先 `resolveFinallyControlTarget` 定出目标帧深度、catch marker 深度与清理数，再用 `block_cursor`（从 `s.top_break` 起）、`scope_cursor`、`catch_marker_depth` 三个游标向外走。外层循环由内向外遍历 `return_finally_frames`：`controlTargetCrossesFinallyFrame` 判定不跨出的帧直接跳过；要跨出时先 `emitControlBlocksUntil` 走到该帧的 `block_boundary`（途中若已命中目标环境并发出跳转就直接返回），随后 `closeScopes` 退到帧的 `scope_level`、`emitCatchMarkerDropsFromDepth` 退到帧的 catch marker 深度，接着按 qjs `emit_break` / `emit_return`（quickjs.c:28373-28377、28447-28449）的形状发 `undefined` 占位保持栈深、按 `finally_label` 是 `.temp` 还是 `.builder` 发对应的 `gosub` 进 finalizer、回来再发 `drop` 丢掉被跨过的 finalizer 完成值。所有 finally 处理完后再 `emitControlBlocksUntil(..., boundary = null)` 走完剩下的环境；仍然没命中目标说明记账与环境链失配，报 `Error.ParserInvariant`。
...
### `needVarReference` (`src/parser.zig:10522`)
...
### `classAccessorKind` (`src/parser.zig:13968`)
...
### `registerClassPrivateElement` (`src/parser.zig:13984`)
...
- **所有权 / 错误 / 调用**：向 `State.class_private_elements` 追加一条（`function.memory.allocator`，OOM 上抛）；`const retained = atom_id;` 只是改个名字，**没有 retain**——存的是借用 id。列表归 `State`（`src/parser.zig:1062` 释放），类体结束时按保存长度 `truncateClassPrivateElements` 回退。冲突时返回 `failUnexpectedToken()` 的 `error.UnexpectedToken`。调用方 4 处：访问器 `src/parser.zig:13754`、方法 13822、字段 13847/13856。

### `addPrivateClassBinding` (`src/parser.zig:14002`)
...
### `addPrivateClassFieldBinding` (`src/parser.zig:14009`)
...
### `preparePrivateAccessorBinding` (`src/parser.zig:14017`)
...
### `privateSetterAtom` (`src/parser.zig:14029`)
...
### `markPrivateBrandNeeded` (`src/parser.zig:14039`)
...
### `isForbiddenPublicFieldName` (`src/parser.zig:14060`)
...
### `classNameAtomOwned` (`src/parser.zig:14066`)

- **签名**：`fn classNameAtomOwned(s: *State) ?Atom`。
- **作用**：取当前 class 声明名字的 atom，不能当类名时返回 null。
- **实现**：`TOK_IDENT` 时取 `token.payload.ident.atom`，但若 `escapedIdentifierIsReservedClassName` 判定这个带转义的写法其实是保留字则返回 null；`TOK_AWAIT` 且 `canUseAwaitAsIdentifier` 成立时返回 `tok.keywordAtom(kind)`（sloppy 非 async 上下文里 `await` 可以当类名）；其余 kind 返回 null。源码注释写明本函数交出的是「当前类名的一份 owned retain，由调用方释放」。
- **所有权 / 错误 / 调用**：返回的是一个**借用**的 atom id，不是 retain，调用方不释放——`parseClass` 两处调用点都只是把它存进 `class_name` 就继续解析，parser 里没有任何配对的释放。函数上方那句 `Return one owned retain for the current class name. The caller frees.` 是 rc 时代的遗留：`TOK_IDENT` 分支直接交出 `s.token.payload.ident.atom`，`TOK_AWAIT` 分支交出 `tok.keywordAtom(kind)` 这个预定义 id，两条路径都不调用任何 retain。TGC S3-c 之后 `lexer.releaseTokenPayload` 只释放 token 的字节缓冲、不再释放 atom（见 src/lexer.zig:127-140 的注释「the token's id is an ordinary borrow now」），id 的存活改由整场编译的 `CompileAtomScope` 根提供者保证，sweep 在编译结束后才回收。所以这个 id 在 `advance()` 之后仍然有效，跨整个 `parseClass` 都可用；`advance()` 释放的是 token payload，不是 atom。
### `escapedIdentifierIsReservedClassName` (`src/parser.zig:14079`)
...
### `enterFieldInitFunction` (`src/parser.zig:14112`)
...
### `leaveFieldInitFunction` (`src/parser.zig:14140`)
...
### `enterStaticBlockFunction` (`src/parser.zig:14154`)
...
### `leaveStaticBlockFunction` (`src/parser.zig:14169`)
...
### `emitFieldInitializer` (`src/parser.zig:14187`)
...
### `emitStaticFieldInitializer` (`src/parser.zig:14237`)
...
### `emitPublicFieldNoInitializer` (`src/parser.zig:14247`)
...
### `emitInstanceFieldInitializer` (`src/parser.zig:14255`)
...
### `ensureClassFieldsInitFunction` (`src/parser.zig:14264`)
...
### `ensureClassStaticInitFunction` (`src/parser.zig:14271`)
...
### `createClassFieldsInitFunction` (`src/parser.zig:14278`)
...
### `finishClassFieldsInitFunction` (`src/parser.zig:14325`)
...
### `finishClassStaticInitFunction` (`src/parser.zig:14330`)
...
### `finishClassInitFunction` (`src/parser.zig:14335`)
...
### `registerClassPrivateBoundName` (`src/parser.zig:14354`)
...
- **实现**：先线性扫 `class_private_bound_names`，已有同一个 atom 就直接返回（同名私有元素只登记一次，getter/setter 对偶共用一条）。否则经 `appendRetainedAtom` 追加；名字里的 retain 是 rc 时代遗留，该函数今天只做一次 `list.append`，存进去的仍是借用 id——这份名字要活到整个类体解析完，靠的是 `CompileAtomScope` 在编译期把 id 钉成 GC 根。
- **所有权 / 错误 / 调用**：去重后经 `appendRetainedAtom` 追加——名字里的 retain 是 rc 时代遗留，实现只做 `list.append`，存的是借用 id；列表内存来自 `function.memory.allocator`，`error.OutOfMemory` 是唯一 error。`class_private_bound_names` 归 `State`（`src/parser.zig:1064` 释放），类体结束按保存长度截断。唯一调用方：类体私有名收集 `src/parser.zig:14626`。

### `classPrivateNameIsBound` (`src/parser.zig:14361`)
...
### `classPrivateElementsConflict` (`src/parser.zig:14368`)
...
### `privateNameAtom` (`src/parser.zig:14379`)
...
### `privateNameDeclarationAtom` (`src/parser.zig:14387`)
...
### `findClassPrivateBoundName` (`src/parser.zig:14395`)
...
### `privateAtomMatchesName` (`src/parser.zig:14405`)
...
### `newClassPrivateAtom` (`src/parser.zig:14415`)
...
### `classComputedFieldTempAtom` (`src/parser.zig:14427`)
...
### `parseImport` (`src/parser.zig:15184`)
...
- **所有权 / 错误 / 调用**：具名分支在 fd 的 allocator 上建一条 `imports: ArrayList(ModuleImportSpec)`，由 `defer freeModuleImportSpecs` 释放（该函数今天只 `deinit` 数组本体，元素里的 atom 是借用 id，循环体是空的，`import_name_live` / `local_name_live` 这对标志同样已无实际作用）；真正留存的是 `curFunc().closure_var` 里的 import 槽与 `ensureModule()` 拿到的 `Record` 行（归 `FunctionBytecode`），存的都是借用 atom。错误：局部名重复、字符串 import 名缺 `as`、缺 import 子句等一律 `failExpectedDescription` 的 `error.UnexpectedToken`；closure 数超 u16 是 `error.BytecodeOverflow`；`internString` 与 `record.add*` 收成 `error.OutOfMemory`。唯一调用方 `parseImportStatement`（`src/parser.zig:9212`）。

### `validateModuleImportBindingName` (`src/parser.zig:15300`)
...
### `moduleHasExportName` (`src/parser.zig:15306`)
...
### `addModuleExportName` (`src/parser.zig:15319`)
...
### `validateModuleLocalExports` (`src/parser.zig:15325`)
...
### `addModuleImportAttribute` (`src/parser.zig:15332`)
...
### `addModuleImportBinding` (`src/parser.zig:15341`)
...
### `ensureModuleDefaultExportBinding` (`src/parser.zig:15372`)
...
### `addModuleIndirectExport` (`src/parser.zig:15379`)
...
### `addModuleStarExport` (`src/parser.zig:15391`)
...
### `addModuleRequestFromCurrentString` (`src/parser.zig:15398`)
...
### `moduleStringAtom` (`src/parser.zig:15404`)
...
### `isModuleNameToken` (`src/parser.zig:15409`)
...
### `moduleImportNameAtomOwned` (`src/parser.zig:15416`)
...
- **所有权 / 错误 / 调用**：`export { ... }` 分支在 fd 的 allocator 上建 `export_specs: ArrayList(ModuleExportSpec)`，由 `defer freeModuleExportSpecs` 释放（同样只 `deinit` 数组本体）；导出名是借用 atom——标识符取 token 的 id，字符串名经 `moduleStringAtom` → `internString` 新 intern，两者都不 retain，最终存进 `ensureModule()` 的 `Record`（归 `FunctionBytecode`）。错误：重名导出、非良构（含孤立代理）字符串名、缺 `from` 等是 `error.UnexpectedToken`；`parseClass` 有名却返回 null 是 `Error.ParserInvariant`；`record.add*` 与 intern 是 `error.OutOfMemory`；发射走 builder 三元组。唯一调用方 `parseExportStatement`（`src/parser.zig:9219`）。

### `exportDefaultFunctionNameOwned` (`src/parser.zig:15653`)

- **签名**：`fn exportDefaultFunctionNameOwned(s: *State) ?Atom`。
- **作用**：前瞻 `function` 之后是否有声明名字（含 `function*`），匿名时返回 null。
- **实现**：`takeLexerCursorSnapshot` 之后立刻 `defer restoreLexerCursorSnapshot`——恢复必须在可失败的扫描之前就武装好：`nextInto()` 会在失败之前把 `pos` 推过已经读到的 token（标识符 atom 是最后才 intern 的），若失败时恢复尚未生效，调用方会从 token 中间继续解析，`export function f()` 会从 `(` 接着读并报出假的 SyntaxError，而不是让分配失败原样上抛。扫描本身：取第一个 token，是 `'*'`（`export default function*`）就再取一个，两种情形下只要落点是 `TOK_IDENT` 就返回它的 atom，否则（匿名声明）返回 null；两个扫描 token 都由本函数 `defer freeToken` 释放。正因为扫描 token 在返回前就被释放，名字必须在这里取成一份 owned retain 再交给调用方，借用 id 能用只是碰巧命中了 atom 表 LIFO 空闲链的复用。
...
### `hasExportDefaultClassName` (`src/parser.zig:15679`)
...
### `parseFromClause` (`src/parser.zig:15692`)
...
### `parseWithClause` (`src/parser.zig:15715`)
...
### `ensureParameterArgumentsLocals` (`src/parser.zig:15747`)
...
- 清单函数数（本文件分到）: 146（`src/parser.zig` 全文件 649）
- 本文标题覆盖: 146
- 未覆盖: 无
