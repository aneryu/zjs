...
### `stringPrototypeMethod` (`src/exec/string_ops.zig:1987`)
...
### `appendUtf32FromStringValue` (`src/exec/string_ops.zig:2060`)
...
### `appendUtf16CodePoint` (`src/exec/string_ops.zig:2077`)
...
### `stringSearchPositionMethod` (`src/exec/string_ops.zig:2080`)
...
### `isRegExpForStringSearch` (`src/exec/string_ops.zig:2119`)
...
### `stringReplaceAll` (`src/exec/string_ops.zig:2130`)
...
### `stringSearch` (`src/exec/string_ops.zig:2142`)
...
### `stringIteratorCall` (`src/exec/string_ops.zig:2159`)
...
### `stringIteratorPrototypeFromContext` (`src/exec/string_ops.zig:2177`)
...
### `stringMatch` (`src/exec/string_ops.zig:2201`)
...
### `stringRegExpCreateAndInvoke` (`src/exec/string_ops.zig:2220`)
...
### `callStringWellKnownMethod` (`src/exec/string_ops.zig:2258`)
...
### `stringSplit` (`src/exec/string_ops.zig:2278`)
...
### `stringSplitBuiltinArray` (`src/exec/string_ops.zig:2340`)
...
### `codePointFromSurrogatePair` (`src/exec/string_ops.zig:2357`)

- **签名**：`pub fn codePointFromSurrogatePair(high: u16, low: u16) u21`。
- **作用**：把高低代理对合成一个码点（转调 `unicode_lib` 的同名实现）。
- **实现**：薄封装，主体转发到 `unicode_lib.codePointFromSurrogatePair`。
- **所有权 / 错误 / 调用**：无：`unicode_lib` 同名函数的 `pub` 再导出，纯标量运算、不分配无 error。树内没有经本模块的调用方（消费者要么直接用 `unicode_lib`，要么用本文件的 `combinedSurrogateCodePoint`）。

### `surrogatePairFromCodePoint` (`src/exec/string_ops.zig:2361`)

- **签名**：`pub fn surrogatePairFromCodePoint(code_point: u21) unicode_lib.SurrogatePair`。
- **作用**：把一个码点拆成高低代理对（转调 `unicode_lib` 的同名实现）。
- **实现**：薄封装，主体转发到 `unicode_lib.surrogatePairFromCodePoint`。
- **所有权 / 错误 / 调用**：无：`unicode_lib.surrogatePairFromCodePoint` 的 `pub` 再导出，返回值是按值的 `SurrogatePair`。树内没有经本模块的调用方。

### `RegExpMatch.captureAt` (`src/exec/string_ops.zig:2378`)
...
### `RegExpMatch.captureNameAt` (`src/exec/string_ops.zig:2389`)
...
### `encodeRegExpLegacyCaptureSlice` (`src/exec/string_ops.zig:2407`)
...
### `decodeRegExpLegacyCaptureSlice` (`src/exec/string_ops.zig:2413`)
...
### `defineSplitSliceElement` (`src/exec/string_ops.zig:2423`)
...
### `defineSplitValueElement` (`src/exec/string_ops.zig:2428`)
...
- **所有权 / 错误 / 调用**：把调用方给的值定义成下标属性，dense 快路径不适用才落 `defineOwnProperty`；值随后由数组持有。注意它与下面的 `Owned` 变体在当前实现里落到同一个 `Object.appendDenseArrayDefineIndexMode`，`take_ownership` 只剩契约标注（rc 时代才有 retain/release 差别）。调用方：`regExpSymbolSplitGeneric:1140`、`regexp_fastpath.zig:852`/`:853`、`array_ops.zig:405` 等 6 处。

### `defineSplitValueElementOwned` (`src/exec/string_ops.zig:2434`)
...
- **实现**：先试 `appendDenseArrayDefineIndexOwned`（消费引用），失败则 `defineOwnProperty` 以可写/可枚举/可配置的数据描述符定义。
...
### `initRegExpResultPropertyTemplate` (`src/exec/string_ops.zig:2445`)
...
### `regExpResultPropertyTemplate` (`src/exec/string_ops.zig:2456`)
...
### `createRegExpMatchArrayFromValue` (`src/exec/string_ops.zig:2463`)
...
### `initRegExpMatchArrayDenseElementsFromValue` (`src/exec/string_ops.zig:2492`)
...
### `updateRegExpLegacyStaticsForMatchValues` (`src/exec/string_ops.zig:2554`)
...
### `updateRegExpLegacyStaticsForMatch` (`src/exec/string_ops.zig:2607`)
...
- **实现**：先调 `updateRegExpLegacyStaticsLazyForMatch`，成功即返回；否则用 `found.captureAt` 逐个切出捕获字符串，再交 `updateRegExpLegacyStaticsForMatchValues`。
...
### `updateRegExpLegacyStaticsLazyForMatch` (`src/exec/string_ops.zig:2627`)
...
### `appendUtf8CodePointForRegExpName` (`src/exec/string_ops.zig:2686`)
...
### `isHighSurrogateCodePoint` (`src/exec/string_ops.zig:2690`)
...
### `isLowSurrogateCodePoint` (`src/exec/string_ops.zig:2694`)
...
### `combinedSurrogateCodePoint` (`src/exec/string_ops.zig:2698`)
...
### `stringSliceValue` (`src/exec/string_ops.zig:2702`)
...
- **实现**：非字符串体原样返回入参。区间先钳进 `[0, len]`；覆盖整串时返回原值，长度为 0 返回运行时空串；否则 `ensureFlat` 后：单码元且 `< 0x100` 取 runtime 共享单字节串，其余走 `String.createSlice`——它是 qjs `js_sub_string` 那样的**急切拷贝**（`string.zig:887` 注释写明「没有零拷贝视图，结果不持有父串」）。
- **所有权 / 错误 / 调用**：三种返回形态：整段覆盖时**原样交回** `value`（借用）、空片返回 runtime 共享空串、单个 latin1 码元返回 `rt.singleByteString` 的共享表项，其余才 `String.createSlice` 拷成新串（不共享父串 payload）。`ensureFlat` 会就地展平入参。调用方：本文件的 split / 匹配数组 / legacy statics 共 7 处，以及 `regexp_fastpath.zig:619` 等。

### `getStringPrototypeMethodId` (`src/exec/string_ops.zig:2718`)
...
### `bigIntPrototypeToString` (`src/exec/string_ops.zig:2725`)
...
### `standardStringMethodId` (`src/exec/string_ops.zig:2784`)
...
### `isStringMethodReceiver` (`src/exec/string_ops.zig:2788`)
...
### `annexBStringMethodId` (`src/exec/string_ops.zig:2817`)
...
### `errorToStringCall` (`src/exec/string_ops.zig:2821`)
...
### `toStringBytesForSymbol` (`src/exec/string_ops.zig:2862`)
...
### `consumePendingExceptionIfMatchesConstructor` (`src/exec/string_ops.zig:2882`)
...
### `thrownValueMatchesConstructor` (`src/exec/string_ops.zig:2889`)
...
### `arraySearchCall` (`src/exec/string_ops.zig:2910`)
...
### `arrayConcatCall` (`src/exec/string_ops.zig:3047`)
...
### `concatAppendValue` (`src/exec/string_ops.zig:3076`)
...
### `concatSpreadLengthValue` (`src/exec/string_ops.zig:3121`)
...
### `isConcatSpreadable` (`src/exec/string_ops.zig:3143`)
...
### `uint8ArrayStringBytes` (`src/exec/string_ops.zig:3158`)
...
### `appendSourceStringUtf8` (`src/exec/string_ops.zig:3166`)
...
### `iteratorConcatCall` (`src/exec/string_ops.zig:3177`)
...
### `regExpStringIteratorNext` (`src/exec/string_ops.zig:3186`)
...
### `getFastStringPrimitiveDataProperty` (`src/exec/string_ops.zig:3228`)
...
### `defineStringWrapperIndexProperty` (`src/exec/string_ops.zig:3268`)
...
### `getStringIndexValue` (`src/exec/string_ops.zig:3278`)
...
### `arrayToStringCall` (`src/exec/string_ops.zig:3296`)
...
### `arrayToLocaleStringCall` (`src/exec/string_ops.zig:3318`)
...
### `objectToLocaleStringCall` (`src/exec/string_ops.zig:3367`)
...
### `objectToStringCall` (`src/exec/string_ops.zig:3380`)
...
### `objectToStringIntrinsic` (`src/exec/string_ops.zig:3394`)
...
### `objectTagString` (`src/exec/string_ops.zig:3415`)
...
### `defaultObjectToStringTag` (`src/exec/string_ops.zig:3424`)
...
### `objectIsArrayForToString` (`src/exec/string_ops.zig:3498`)
...
### `stringObjectHasIndexProperty` (`src/exec/string_ops.zig:3507`)
...
### `appendUtf16UnitsAsUtf8` (`src/exec/string_ops.zig:3517`)
...
### `appendAsciiUnits` (`src/exec/string_ops.zig:3520`)
...
### `isAsciiDigitUnit` (`src/exec/string_ops.zig:3524`)
...
### `isAsciiDigitByte` (`src/exec/string_ops.zig:3528`)

- **签名**：`pub fn isAsciiDigitByte(byte: u8) bool`。
- **作用**：字节是否是 ASCII 数字。
- **实现**：薄封装，主体转发到 `unicode_lib.isAsciiDigitByte`。
- **所有权 / 错误 / 调用**：无：`unicode_lib` 谓词的再导出；树内没有经本模块的调用方（lexer / unicode 侧直接用原函数）。

### `isAsciiWordUnit` (`src/exec/string_ops.zig:3532`)

- **签名**：`pub fn isAsciiWordUnit(unit: u16) bool`。
- **作用**：码元是否是 RegExp `\w` 意义上的 word 字符。
- **实现**：薄封装，主体转发到 `unicode_lib.isAsciiWordUnit`。
- **所有权 / 错误 / 调用**：无：`unicode_lib` 谓词的再导出；树内没有经本模块的调用方。

### `isHighSurrogateUnit` (`src/exec/string_ops.zig:3536`)
...
### `isLowSurrogateUnit` (`src/exec/string_ops.zig:3540`)
...
### `StringBuffer.deinit` (`src/exec/string_ops.zig:3577`)
...
### `StringBuffer.putc8` (`src/exec/string_ops.zig:3583`)
...
### `StringBuffer.appendStringValue` (`src/exec/string_ops.zig:3591`)

- **签名**：`fn appendStringValue(self: *StringBuffer, rt: *core.JSRuntime, value: core.JSValue) !void`。
- **作用**：把一个字符串值的全部码元追加进缓冲（qjs `string_buffer_concat_value`）。
- **实现**：取 `asStringBody`（拿不到 `error.TypeError`）后 `ensureFlat`，再用 `resolveData()` 的整段交 `appendUnits`。
- **所有权 / 错误 / 调用**：`ensureFlat` 会就地展平入参串，随后只借用它的 `resolveData()` 视图往缓冲复制，不保留任何引用。取不到字符串体 → 裸 `error.TypeError`。调用方：`stringReplaceCore:502`、`appendSubstitutionStringSearch:583`。

### `StringBuffer.widen` (`src/exec/string_ops.zig:3598`)
...
### `StringBuffer.ensureCapacity` (`src/exec/string_ops.zig:3605`)
...
### `StringBuffer.appendUnits` (`src/exec/string_ops.zig:3614`)
...
### `StringBuffer.finish` (`src/exec/string_ops.zig:3648`)
...
### `stringPad` (`src/exec/string_ops.zig:3657`)
...
- **实现**：nullish 接收者抛「null or undefined are forbidden」，接收者 ToString 后 `ensureFlat` 并只 `resolveData()` 一次（qjs 直接读 JSString 的 len，quickjs.c:46313-46314）。目标长度经 `toLengthIndex`，不大于源长度时原样返回；填充串缺省为一个空格，为空串时也原样返回；目标长度超过 `js_string_len_max` 抛 RangeError「invalid string length」（quickjs.c:46331-46334）。随后按 `target_length` 预留 `StringBuffer`，padEnd 先写源串、padStart 后写源串，中间按 `fill_len` 分块循环写填充（quickjs.c:46338-46356）。
- **所有权 / 错误 / 调用**：目标长度不超过源长、或填充串为空时**原样交回** `string_value`（借用，不新建）；否则 `StringBuffer`（defer deinit）攒完由 `finish` 新建串。`source` / `fill` 都先 `ensureFlat` 再借用 `resolveData()` 视图。错误：nullish 接收者挂 "null or undefined are forbidden"，超过 `js_string_len_max` 挂 "invalid string length"，取不到字符串体是裸 `error.TypeError`。唯一调用方 `stringPrototypeMethod:2033`（id 34 / 35）。

### `stringNormalize` (`src/exec/string_ops.zig:3718`)
...
### `stringLocaleCompare` (`src/exec/string_ops.zig:3756`)
...
### `NormalizedUtf32.deinit` (`src/exec/string_ops.zig:3787`)
...
### `normalizedUtf32` (`src/exec/string_ops.zig:3792`)
...
### `stringNumericArgsMethod` (`src/exec/string_ops.zig:3802`)
...
### `fastLatin1Substring` (`src/exec/string_ops.zig:3846`)
...
- **实现**：接收者必须是字符串、参数不超过两个；`ensureFlat` 后只接受 latin1 表示（utf16 返回 `null`）。起止参数必须是 int32 或 undefined（undefined 的 end 取串长），否则返回 `null` 交回通用路径。两端钳进 `[0, len]` 后按大小排序，空区间返回运行时空串，其余 `String.createLatin1` 复制出子串。
- **所有权 / 错误 / 调用**：`ensureFlat` 就地展平入参；`bytes` 是借用视图，空区间返回 runtime 共享空串，否则 `String.createLatin1` 拷成新串归调用方。形状不符（宽串、参数不是 int32、参数超过两个）返回 `null` 让调用方回落。唯一调用方 `stringNumericArgsMethod:3830`（id 1）。

### `int32OrUndefinedStringIndex` (`src/exec/string_ops.zig:3868`)
...
### `stringSubstr` (`src/exec/string_ops.zig:3873`)
...
- **实现**：先把接收者展成 u16 码元。start 参数缺省或 undefined 取 0；NaN / 0 取 0，负数按 `size - |trunc|` 回绕（`-∞` 取 0），`+∞` 取 size，其余截断后钳到 size。length 参数缺省取到末尾，NaN 或 `<= 0` 取 0，`+∞` 取剩余长度，其余截断后钳到剩余长度；最后按区间 `String.createUtf16`。`output` / `global` 参数未使用。
- **所有权 / 错误 / 调用**：`units` 临时 ArrayList defer deinit，返回新建 UTF-16 串；`output` / `global` 参数未使用（只为与同族方法保持 ABI 一致）。参数此时已被调用方规整成数值，这里不再跑可观察转换，因此除分配外无 error。唯一调用方 `stringNumericArgsMethod:3837`（id 25）。

### `stringHtmlMethod` (`src/exec/string_ops.zig:3921`)
...
### `stringCreateHtml` (`src/exec/string_ops.zig:3956`)
...
- 清单函数数（本文件分到）: 90（`src/exec/string_ops.zig` 全文件 159）
- 本文标题覆盖: 90
- 未覆盖: 无
