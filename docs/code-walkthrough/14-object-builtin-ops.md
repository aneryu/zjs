...
[`src/exec/object_builtin_ops.zig`](../../src/exec/object_builtin_ops.zig) 是 `.object` 域的 native-record：构造入口 + `Object.*` 静态 + `Object.prototype.*`。接收者与参数 borrowed，返回 owned `JSValue`。对照 `js_object_constructor` 与表 `quickjs.c:40098`、`41006-41054`。
...
### `RootedValueCopies.init` (`src/exec/object_builtin_ops.zig:69`)
...
### `RootedValueCopies.deinit` (`src/exec/object_builtin_ops.zig:83`)
...
### `staticMethodId` (`src/exec/object_builtin_ops.zig:105`)
...
### `prototypeMethodId` (`src/exec/object_builtin_ops.zig:132`)
...
### `prototypeMethodOrdinal` (`src/exec/object_builtin_ops.zig:146`)
...
### `staticEntry` (`src/exec/object_builtin_ops.zig:162`)
...
### `prototypeEntry` (`src/exec/object_builtin_ops.zig:166`)
...
- **所有权 / 错误 / 调用**：comptime 构造，不分配：返回按值的 `InternalEntry`，`name` 指向编译期字符串字面量（静态存储），`native_function` 是 `objectCall` 的 `generic_magic` 包装（`src/exec/object_builtin_ops.zig:181`），全部是静态生命周期；无 error set。调用方是同文件的函数表字面量 `src/exec/object_builtin_ops.zig:232`-`:241` 共 9 项，以及 `prototypeExecDirectEntry`（`src/exec/object_builtin_ops.zig:176`）——后者先拿它建基础项再挂 `managed` 直通函数，`hasOwnProperty` 走的是那条。

### `prototypeExecDirectEntry` (`src/exec/object_builtin_ops.zig:170`)
...
### `objectEntry` (`src/exec/object_builtin_ops.zig:181`)
...
### `constructorEntry` (`src/exec/object_builtin_ops.zig:192`)
...
### `objectConstructorCall` (`src/exec/object_builtin_ops.zig:247`)
...
### `objectCall` (`src/exec/object_builtin_ops.zig:265`)
...
### `objectCallForNativeRecord` (`src/exec/object_builtin_ops.zig:307`)
...
### `literal` (`src/exec/object_builtin_ops.zig:361`)
