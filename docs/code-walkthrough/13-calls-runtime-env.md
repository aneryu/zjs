...
### `assertThrows` (`src/exec/call_runtime.zig:2672`)
...
### `callAssertThrowsCallback` (`src/exec/call_runtime.zig:2700`)
...
### `collectIteratorValues` (`src/exec/call_runtime.zig:2711`)
...
### `getIteratorMethod` (`src/exec/call_runtime.zig:2754`)
...
### `cacheIteratorNextMethod` (`src/exec/call_runtime.zig:2764`)
...
### `appendIteratorValues` (`src/exec/call_runtime.zig:2778`)
...
### `appendSpreadValuesEnumerate` (`src/exec/call_runtime.zig:2826`)
...
### `isCallableValue` (`src/exec/call_runtime.zig:2926`)
...
### `isIteratorIdentityFunction` (`src/exec/call_runtime.zig:2933`)
...
### `globalLexicalEnv` (`src/exec/call_runtime.zig:2938`)
...
### `existingGlobalLexicalEnv` (`src/exec/call_runtime.zig:2951`)
...
### `existingGlobalLexicalEnvForGlobal` (`src/exec/call_runtime.zig:2957`)
...
### `globalLexicalHasForGlobal` (`src/exec/call_runtime.zig:2966`)
...
### `globalLexicalCell` (`src/exec/call_runtime.zig:2975`)
...
### `selectOrdinaryGlobalClosureCell` (`src/exec/call_runtime.zig:2992`)
...
### `globalUninitializedVarsEnv` (`src/exec/call_runtime.zig:3024`)
...
### `globalObjectGetUninitializedVar` (`src/exec/call_runtime.zig:3036`)
...
### `globalObjectFindUninitializedVar` (`src/exec/call_runtime.zig:3055`)
...
### `ensureGlobalObjectVarRefCell` (`src/exec/call_runtime.zig:3073`)
...
### `defineGlobalDeclVarCell` (`src/exec/call_runtime.zig:3137`)

- **签名**：`pub fn defineGlobalDeclVarCell( ctx: *core.JSContext, global: *core.Object, function: *const bytecode.FunctionBytecode, frame: *frame_mod.Frame, ref_idx: u16, atom_id: core.Atom, configurable: bool, is_function: bool, ) !bool`。
- **作用**：非词法 GLOBAL_DECL 槽绑到全局 VARREF，并镜像 eval 提升的 local。
- **实现**：`ref_idx` 越界、槽不是 `.global_decl`、槽是词法、名字不符——四道门任一不过返回 false。过了则 `ensureGlobalObjectVarRefCell` 取/建全局 VARREF cell（它自己返回 null 也算 false），按需 `ensureVarRefsCapacity` 扩 `frame.var_refs` 后 `storeVarRefSlot` 绑定，最后把同名且 `varDefIsEvalHoistedVar` 的 local 槽写成 cell 值（兼容直接 eval lookup；权威身份仍在 `frame.var_refs`/全局属性 cell 上）。
- **返回值**：`true` = 这个槽已绑好，`false` = 四道门之一没过。（原先用一个初值即 `true`、循环里唯一的写也是 `rebound = true` 的局部变量表达「是否真的重绑到某个 local」，恒真、语义已失效，已改成直接 `return true`；同时删掉了一条只做越界读的 `_ = slot_ops.varRefSlot(frame, ref_idx);` rc 残骸。）
- **所有权 / 错误 / 调用**：`vm_property_globals` 的 global_decl 安装循环非词法臂（词法臂走 `defineGlobalDeclLexicalCell`）。

### `ensureGlobalLexicalCell` (`src/exec/call_runtime.zig:3177`)
...
### `globalLexicalValueForGlobal` (`src/exec/call_runtime.zig:3246`)
...
### `defineGlobalLexicalValue` (`src/exec/call_runtime.zig:3254`)
...
### `defineGlobalDeclLexicalCell` (`src/exec/call_runtime.zig:3267`)
...
### `setGlobalLexicalValueForGlobal` (`src/exec/call_runtime.zig:3288`)
...
### `setGlobalLexicalValueForFastPathOwned` (`src/exec/call_runtime.zig:3301`)
...
### `initializeGlobalLexicalValue` (`src/exec/call_runtime.zig:3307`)
...
### `varDefIsEvalHoistedVar` (`src/exec/call_runtime.zig:3334`)
...
### `indirectEval` (`src/exec/call_runtime.zig:3341`)
...
### `isSimpleIdentifierName` (`src/exec/call_runtime.zig:3424`)
...
### `ActiveRootValueProbe.trigger` (`src/exec/call_runtime.zig:3439`)
...
### `freeArgs` (`src/exec/call_runtime.zig:3454`)
...
### `Probe.trigger` (`src/exec/call_runtime.zig:3486`)
...
### `callFunctionBytecodeConstruct` (`src/exec/call_runtime.zig:3529`)
...
### `callFunctionBytecodeModeState` (`src/exec/call_runtime.zig:3559`)
...
### `callFunctionBytecodeModeStateAfterInterruptPoll` (`src/exec/call_runtime.zig:3620`)
...
### `runGeneratorParameterInit` (`src/exec/call_runtime.zig:3740`)
...
### `generatorNext` (`src/exec/call_runtime.zig:3808`)
...
### `generatorFunctionBytecodeFromExecution` (`src/exec/call_runtime.zig:3868`)
...
### `generatorHasYieldStarResult` (`src/exec/call_runtime.zig:3876`)
...
### `syncGeneratorStep` (`src/exec/call_runtime.zig:3891`)
...
### `generatorYieldStarSuspended` (`src/exec/call_runtime.zig:3943`)
...
### `setGeneratorYieldStarSuspended` (`src/exec/call_runtime.zig:3948`)
...
### `resumeGeneratorYieldStarCompletion` (`src/exec/call_runtime.zig:3963`)
...
### `generatorReturn` (`src/exec/call_runtime.zig:4001`)
...
### `resumeGeneratorCatchForRuntimeError` (`src/exec/call_runtime.zig:4072`)
...
### `generatorYieldStarReturnStep` (`src/exec/call_runtime.zig:4123`)
...
### `generatorYieldStarThrowStep` (`src/exec/call_runtime.zig:4158`)
...
### `generatorYieldStarCloseForMissingThrow` (`src/exec/call_runtime.zig:4194`)
...
### `generatorThrow` (`src/exec/call_runtime.zig:4208`)
...
### `generatorCatchResumeResultValue` (`src/exec/call_runtime.zig:4302`)
...
### `generatorPcAfterYieldStar` (`src/exec/call_runtime.zig:4306`)
...
### `wrapIteratorFromIterator` (`src/exec/call_runtime.zig:4315`)
...
### `pollGCSafePoint` (`src/exec/call_runtime.zig:4381`)
...
### `runNextOsTimer` (`src/exec/call_runtime.zig:4388`)
...
### `runNextOsRwHandler` (`src/exec/call_runtime.zig:4395`)
...
### `enqueuePendingMicrotask` (`src/exec/call_runtime.zig:4402`)
...
### `throwTypeErrorIntrinsic` (`src/exec/call_runtime.zig:4466`)
...
### `currentFrameFunctionIsStrict` (`src/exec/call_runtime.zig:4472`)
...
### `functionBytecodeFromValue` (`src/exec/call_runtime.zig:4484`)
...
### `isFunctionLikeClass` (`src/exec/call_runtime.zig:4489`)
...
### `throwPrivateBrandTypeError` (`src/exec/call_runtime.zig:4631`)
...
### `throwSetFailureTypeError` (`src/exec/call_runtime.zig:4659`)
...
### `setFailureShouldThrow` (`src/exec/call_runtime.zig:4675`)
...
### `functionRuntimeStrict` (`src/exec/call_runtime.zig:4680`)
...
### `ordinarySetWithReceiver` (`src/exec/call_runtime.zig:4684`)
...
### `definePropertiesCall` (`src/exec/call_runtime.zig:4715`)
...
### `PendingDescriptorRoots.traceRoots` (`src/exec/call_runtime.zig:4743`)
...
### `PendingDescriptorRoots.provider` (`src/exec/call_runtime.zig:4753`)
...
### `PendingDescriptorRoots.activate` (`src/exec/call_runtime.zig:4757`)
...
### `PendingDescriptorRoots.deactivate` (`src/exec/call_runtime.zig:4763`)
...
### `definePropertiesOnTarget` (`src/exec/call_runtime.zig:4771`)
...
### `callAccessorSetter` (`src/exec/call_runtime.zig:4843`)
...
### `inOp` (`src/exec/call_runtime.zig:4868`)
...
### `instanceofOp` (`src/exec/call_runtime.zig:4890`)
...
### `instanceofValue` (`src/exec/call_runtime.zig:4907`)
...
### `instanceofMethod` (`src/exec/call_runtime.zig:4927`)
...
### `instanceofMethodSlow` (`src/exec/call_runtime.zig:4942`)
...
### `instanceofValueWithMethod` (`src/exec/call_runtime.zig:4954`)
...
### `constructorNameEqlLocal` (`src/exec/call_runtime.zig:4991`)
...
### `nativeFunctionNameValueLocal` (`src/exec/call_runtime.zig:4999`)
...
### `isBlockedByUnscopables` (`src/exec/call_runtime.zig:5012`)
...
### `lookupFrameVarRef` (`src/exec/call_runtime.zig:5028`)
...
### `closureVarIsNonLexicalGlobalSentinel` (`src/exec/call_runtime.zig:5054`)
...
### `atomIdOrNameEql` (`src/exec/call_runtime.zig:5064`)
...
### `functionNameValueFromAtom` (`src/exec/call_runtime.zig:5071`)
...
### `mappedArgumentsValue` (`src/exec/call_runtime.zig:5106`)
...
### `setMappedArgumentsValue` (`src/exec/call_runtime.zig:5116`)
...
### `readInt` (`src/exec/call_runtime.zig:5130`)
