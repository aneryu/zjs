...
- **实现**：errdefer destroy runtime / deinit function。
- **所有权 / 错误 / 调用**：S2-G1/G2/G3/G4 形态测试。`defer h.deinit()`。

### `V2Parse.builder` (`src/compiler/tests.zig:50`)
...
### `V2Parse.childBuilder` (`src/compiler/tests.zig:54`)
...
### `V2Parse.grandchildBuilder` (`src/compiler/tests.zig:58`)
...
### `V2Parse.deinit` (`src/compiler/tests.zig:62`)

- **签名**：`fn deinit(h: *V2Parse) void`。
- **作用**：state → function → runtime。lexer 不在此释放（`state.deinit` 只 `lex.freeToken(&token)`；JS 源下 `Lexer.deinit` 只释放 TS 的 `skipped_intervals`，此夹具无此缓冲）。
- **实现**：`rt.destroy()`。
- **所有权 / 错误 / 调用**：测试夹具释放序：`state.deinit(rt)` 连带放掉 `function_def` 树上的各级 `v2_builder`（`builder`/`childBuilder`/`grandchildBuilder` 返回的都是这棵树上的借用指针），再 `function.deinit(rt)`，最后 `rt.destroy()` 关掉这些缓冲所挂的 memory 账户——runtime 必须最后走。`void`、不返回错误，由各测试的 `defer` 调用。

### `V2Exec.init` (`src/compiler/tests.zig:79`)
...
### `V2Exec.deinit` (`src/compiler/tests.zig:100`)
...
### `expectV2Stream` (`src/compiler/tests.zig:116`)
...
### `expectResolvedStream` (`src/compiler/tests.zig:130`)
...
### `expectLabel` (`src/compiler/tests.zig:153`)
...
### `expectResolvedLabel` (`src/compiler/tests.zig:161`)
...
### `installedFunctionHasShortOpcode` (`src/compiler/tests.zig:175`)
...
### `compileAndRun` (`src/compiler/tests.zig:198`)
...
### `compileAndRunWithHook` (`src/compiler/tests.zig:205`)
...
### `expectFunctionDefInertAfterEscape` (`src/compiler/tests.zig:255`)
...
### `expectPublishedAtomResolves` (`src/compiler/tests.zig:279`)
...
### `expectPublishedFunctionBytecodeOwnersResolve` (`src/compiler/tests.zig:284`)
...
### `expectRelocIntegrity` (`src/compiler/tests.zig:321`)
...
### `expectSourceOrder` (`src/compiler/tests.zig:370`)
...
### `expectSourceOffsets` (`src/compiler/tests.zig:379`)
...
### `oomScript` (`src/compiler/tests.zig:499`)
...
### `expectV2ExecutionCompletion` (`src/compiler/tests.zig:2990`)
...
### `countInstalledOpcode` (`src/compiler/tests.zig:2998`)
...
### `compileRunAndCount` (`src/compiler/tests.zig:3018`)
...
### `s3bDrainGc` (`src/compiler/tests.zig:3472`)
...
### `s3bExpectDefTreeMarked` (`src/compiler/tests.zig:3485`)
...
### `s3bMajorBeforeFinalize` (`src/compiler/tests.zig:3505`)
...
### `s3bExpectTemplateArraysMarked` (`src/compiler/tests.zig:3557`)
...
### `expectCoverageSkipSetMatches` (`src/compiler/tests.zig:3793`)
