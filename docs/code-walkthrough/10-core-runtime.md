...
清单 0 个函数。文件职责：把 core 身份与嵌入选项类型 re-export 成 exec/runtime 与公共 facade 的合法 import 面。**不增加所有权**；寿命契约留在定义模块。core 实现不得再依赖 parser/exec/runtime/builtins/CLI。QuickJS 没有对应翻译单元，这是 zjs 的层边界。
