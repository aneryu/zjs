...
### `parseArrowAssignment` (`src/parser.zig:3962`)
...
### `parseExpr2` (`src/parser.zig:4006`)
...
### `parseAssignExpr` (`src/parser.zig:4028`)
...
### `parseAssignExpr2` (`src/parser.zig:4035`)
...
### `parseDestructuringAssignment` (`src/parser.zig:4121`)
...
### `parseCondExpr` (`src/parser.zig:4663`)
...
### `parseCoalesceExpr` (`src/parser.zig:4686`)
...
### `parseLogicalAndOr` (`src/parser.zig:4707`)
...
### `parseExprBinary` (`src/parser.zig:4753`)
...
### `parseUnary` (`src/parser.zig:4799`)
...
### `parsePostfixExpr` (`src/parser.zig:5579`)
...
### `parseLhsExpr` (`src/parser.zig:5615`)
...
### `emitCapturedSuperConstructorCall` (`src/parser.zig:5684`)
...
### `emitClassFieldInitCall` (`src/parser.zig:5725`)
...
### `parseNewExpr` (`src/parser.zig:5740`)
...
### `parseNewCalleeMemberAccess` (`src/parser.zig:5808`)

- **签名**：`fn parseNewCalleeMemberAccess(s: *State, flags: ParseFlags) Error!void`。
...
- **所有权 / 错误 / 调用**：不分配；属性名 atom 全是借用的 token 内容，**必须在 `advance()` 之前写进操作数**（源码注释说明这与 qjs 的单次 retain 路径一致）。私有名经 `privateNameAtom` 得到的是类身份限定后的 atom，同样只是 id。`flags` 参数实际未被使用（末尾一条 `_ = flags;`），签名保留只为与 `parseMemberChain` 对齐。错误：`.` 后面不是可接受的属性名、类外用私有名、私有名未绑定，都是 `failUnexpectedToken`，另有 `expectPunct(']')`。三个调用点全在 `parseNewExpr`（`src/parser.zig:5767` / `:5774` / `:5777`）。

### `parseMemberChain` (`src/parser.zig:5864`)
...
### `parseTaggedTemplateInvocation` (`src/parser.zig:6023`)
...
### `parseCallArgs` (`src/parser.zig:6138`)
...
### `parseRegExpLiteral` (`src/parser.zig:6191`)
...
### `parsePrimary` (`src/parser.zig:6234`)
...
- `function`：函数表达式（含误把 async 当 TOK_FUNCTION 的路径）。
...
### `parseDynamicImportCall` (`src/parser.zig:6414`)
...
### `parseTemplate` (`src/parser.zig:6452`)
...
### `parseObjectProperty` (`src/parser.zig:6727`)
...
### `parseObjectAccessorProperty` (`src/parser.zig:6878`)
...
### `atomNameEquals` (`src/parser.zig:7189`)
...
### `atomsNameEqual` (`src/parser.zig:7193`)
...
### `evalAnnexBBlockedFunctionName` (`src/parser.zig:7200`)
...
### `atomNameIsPrivate` (`src/parser.zig:7207`)
...
- 清单函数数（本文件分到）: 32（`src/parser.zig` 全文件 649）
- 本文标题覆盖: 32
- 未覆盖: 无
