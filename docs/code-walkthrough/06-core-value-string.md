...
### `String.createLatin1ConcatWithSeed` (`src/core/string.zig:617`)

- **签名**：`pub fn createLatin1ConcatWithSeed(rt: *JSRuntime, a: []const u8, b: []const u8, seed: u32) !*String`。
- **作用**：保留带 seed 参数的拼接入口。
- **实现**：丢弃 seed，直接 createLatin1Concat。
- **所有权 / 错误 / 调用**：seed 不设置 hash、不影响输出或避免后续哈希计算；分配及长度边界与转发目标相同。

### `String.createUtf16Concat` (`src/core/string.zig:624`)
...
### `String.createAsciiSuffix` (`src/core/string.zig:639`)
...
### `String.createLatin1` (`src/core/string.zig:655`)
...
### `String.createAsciiCaseMapped` (`src/core/string.zig:666`)
...
### `String.createRope` (`src/core/string.zig:715`)
...
- **所有权 / 错误 / 调用**：要求输入为有效 string/rope 或支持的 symbol body，不执行 ToString。存储值副本形成 GC 边，不增加 RC；分配期间输入仍须可达。

### `String.createRopeOwned` (`src/core/string.zig:728`)

- **签名**：`pub fn createRopeOwned(rt: *JSRuntime, left: JSValue, right: JSValue) !*StringRope`。
- **作用**：保留 Owned 命名的 rope 创建入口。
- **实现**：与 createRope 相同地分类并调用 createRopeNode，catch 仅原样返回错误。
- **所有权 / 错误 / 调用**：当前无释放、消费或清空输入操作；旧注释“失败释放双方”不适用 tracing 实现，不能要求调用已删除的 free。

### `String.createBalancedRope` (`src/core/string.zig:739`)
...
### `String.value` (`src/core/string.zig:780`)
...
### `String.len` (`src/core/string.zig:784`)
...
### `String.isWide` (`src/core/string.zig:788`)
...
### `String.hash` (`src/core/string.zig:795`)
...
### `String.inlineBytesPtr` (`src/core/string.zig:801`)
...
### `String.inlineBytesPtrMut` (`src/core/string.zig:805`)
...
### `String.latin1` (`src/core/string.zig:810`)
...
### `String.utf16` (`src/core/string.zig:814`)
...
### `String.latin1Mut` (`src/core/string.zig:819`)
...
### `String.utf16Mut` (`src/core/string.zig:822`)
...
### `String.eqlBytes` (`src/core/string.zig:827`)
...
### `String.eqlString` (`src/core/string.zig:834`)
...
### `String.compare` (`src/core/string.zig:838`)
...
### `ResolvedData.len` (`src/core/string.zig:849`)
...
### `String.ensureFlat` (`src/core/string.zig:860`)

- **签名**：`pub fn ensureFlat(self: *String, rt: *JSRuntime) !void`。
- **作用**：为已有 flat String 保留可失败接口形状的空操作。
- **实现**：忽略 self 和 rt，直接成功。
- **所有权 / 错误 / 调用**：不会验证指针、展开 rope、分配或执行 GC；rope 与 String 为独立表示，转换发生在其他入口。

### `String.resolveData` (`src/core/string.zig:865`)
...
### `String.borrowLatin1` (`src/core/string.zig:870`)
...
### `String.codeUnitAt` (`src/core/string.zig:875`)
...
### `String.createSlice` (`src/core/string.zig:887`)
...
### `String.createUninitialized` (`src/core/string.zig:897`)
...
### `String.destroyFlat` (`src/core/string.zig:941`)
...
### `compareResolved` (`src/core/string.zig:977`)
...
### `compareSameWidth` (`src/core/string.zig:990`)
...
### `compareLatin1Utf16` (`src/core/string.zig:995`)
...
### `compareUtf16Latin1` (`src/core/string.zig:1007`)
...
### `compareLength` (`src/core/string.zig:1019`)
...
### `orderToI32` (`src/core/string.zig:1025`)
...
### `stringValueLen` (`src/core/string.zig:1034`)
...
### `stringValueLenUnchecked` (`src/core/string.zig:1040`)
...
### `StringValueIterator.init` (`src/core/string.zig:1056`)
...
### `StringValueIterator.next` (`src/core/string.zig:1061`)
...
### `stringValueCodeUnitAt` (`src/core/string.zig:1104`)
...
### `stringValueCodeUnitAtUnchecked` (`src/core/string.zig:1112`)
...
### `flatStringsEq` (`src/core/string.zig:1150`)
...
### `flatStringsEqNear` (`src/core/string.zig:1158`)
...
### `flatStringsEqMixedWidth` (`src/core/string.zig:1182`)
...
### `compareStringValues` (`src/core/string.zig:1191`)
...
### `resolvedSlice` (`src/core/string.zig:1277`)
...
### `stringValueInfo` (`src/core/string.zig:1291`)
...
### `createRopeNode` (`src/core/string.zig:1303`)
...
### `collectRopeRebalanceLeaves` (`src/core/string.zig:1396`)
...
### `rebalanceRope` (`src/core/string.zig:1416`)
...
### `writeTailUnits` (`src/core/string.zig:1452`)
...
### `tailBufferSeedCapacityFor` (`src/core/string.zig:1481`)
...
### `tailBufferCapacityFor` (`src/core/string.zig:1487`)
...
### `createTailBufferRope` (`src/core/string.zig:1496`)
...
### `appendTailBufferRope` (`src/core/string.zig:1528`)
...
### `allocRopeNode` (`src/core/string.zig:1577`)
...
### `copyRopeContent` (`src/core/string.zig:1593`)
...
### `copyRopeNodeContent` (`src/core/string.zig:1600`)
...
### `copyRopeValueContent` (`src/core/string.zig:1613`)
...
### `copyResolvedUnits` (`src/core/string.zig:1623`)
...
### `metaIsRope` (`src/core/string.zig:1651`)
...
### `accountedAllocationSizeFromHeader` (`src/core/string.zig:1658`)
...
### `destroyCellFromHeader` (`src/core/string.zig:1683`)
...
### `destroyAllStringCarriersForDeinit` (`src/core/string.zig:1733`)
...
### `sweepExtents` (`src/core/string.zig:1770`)
...
### `sweepYoungExtents` (`src/core/string.zig:1781`)
...
### `destroyDeadStringExtent` (`src/core/string.zig:1789`)
...
### `traceRopeEdges` (`src/core/string.zig:1842`)
...
### `callVisitStorageCell` (`src/core/string.zig:1861`)
...
### `callVisitValue` (`src/core/string.zig:1875`)
...
### `inlineAllocationLayout` (`src/core/string.zig:1886`)
...
### `finalLatin1AllocationLen` (`src/core/string.zig:1911`)
...
### `writeLatin1Terminator` (`src/core/string.zig:1915`)
...
### `foldHash30` (`src/core/string.zig:1927`)
...
### `hashLatin1` (`src/core/string.zig:1932`)
...
### `hashUtf16` (`src/core/string.zig:1938`)
...
### `eqlUtf16Latin1` (`src/core/string.zig:1944`)
...
### `scanUtf8` (`src/core/string.zig:1957`)
...
### `decodeUtf8` (`src/core/string.zig:1977`)
...
### `decodeOne` (`src/core/string.zig:2008`)
...
### `JSString.units` (`src/core/string_view.zig:106`)

- **签名**：`pub fn units(self: Self) ?Units`。
- **作用**：借用 flat 字符串的原始码元切片。
- **实现**：resolveData 为 Latin1/UTF-16 时分别包装对应 Units 分支，当前实现没有返回 null 的分支。
- **所有权 / 错误 / 调用**：UTF-16 长度是码元数，不是 Unicode 标量数；Latin1 高字节不等于 UTF-8。无转码、无复制，源必须保持存活。

### `JSString.toUtf8` (`src/core/string_view.zig:113`)
...
### `JSString.toOwnedUtf8` (`src/core/string_view.zig:117`)
...
### `JSString.toOwnedUtf8Cesu8` (`src/core/string_view.zig:121`)
...
### `utf8LenLatin1` (`src/core/string_view.zig:153`)
...
### `utf8LenUtf16` (`src/core/string_view.zig:159`)
...
### `utf8LenCodeUnit` (`src/core/string_view.zig:175`)
...
### `writeUtf8CodeUnit` (`src/core/string_view.zig:181`)
...
### `writeUtf8CodePoint` (`src/core/string_view.zig:185`)
...
- **实现**：createExternal 把 bytes/回调放入 SharedBufferStore，随后创建 null prototype SAB；失败时 errdefer store.release，成功安装不额外 retain，最后 self.disarm。
- **所有权 / 错误 / 调用**：若 createExternal 自身失败，原 Store 仍保有回调；若它成功而 Object.create 失败，release 会调用外部 deinit，但 self 未 disarm，原 Store 仍保留旧 bytes/回调。这是当前实现的失败所有权缺口，不能保证失败后再次 release 或重试安全。

### `JSBytes.Store.release` (`src/core/bytes_view.zig:141`)
...
### `JSBytes.Store.disarm` (`src/core/bytes_view.zig:146`)
...
### `JSBytes.fromArrayBufferObject` (`src/core/bytes_view.zig:151`)
...
### `JSBytes.fromTypedArrayObject` (`src/core/bytes_view.zig:163`)
...
### `JSBytes.fromDataViewObject` (`src/core/bytes_view.zig:196`)
...
### `JSBytes.objectFromValue` (`src/core/bytes_view.zig:219`)
...
### `JSBytes.typedArrayBufferObject` (`src/core/bytes_view.zig:226`)
...
### `JSBytes.isArrayBufferObject` (`src/core/bytes_view.zig:233`)
...
### `JSBytes.isTypedArrayObject` (`src/core/bytes_view.zig:238`)
...
### `JSBytes.isDataViewObject` (`src/core/bytes_view.zig:242`)
...
### `State.deinit` (`src/core/bytes_view.zig:268`)
...
### `Hooks.deinit` (`src/core/bytes_view.zig:290`)
...
### `State.deinit` (`src/core/bytes_view.zig:308`)
...
### `State.deinit` (`src/core/bytes_view.zig:350`)
...
### `State.deinit` (`src/core/bytes_view.zig:383`)
...
### `testObjectFromValue` (`src/core/bytes_view.zig:581`)
...
- 清单函数数: 175（`src/core/bytes_view.zig` 29 + `src/core/string.zig` 127 + `src/core/string_view.zig` 19）
- 本文标题覆盖: 175
- 未覆盖: 无
