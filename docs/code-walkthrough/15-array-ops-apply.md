...
### `uint8ArrayCodecCall` (`src/exec/array_ops.zig:5684`)
...
### `uint8ArrayCheckOptionsObject` (`src/exec/array_ops.zig:5770`)
...
### `expectUint8ArrayObject` (`src/exec/array_ops.zig:5775`)
...
### `uint8ArrayBase64Alphabet` (`src/exec/array_ops.zig:5792`)
...
### `uint8ArrayBase64LastChunkHandling` (`src/exec/array_ops.zig:5814`)
...
### `uint8ArrayBase64NamedOption` (`src/exec/array_ops.zig:5839`)
...
### `uint8ArrayOmitPadding` (`src/exec/array_ops.zig:5858`)
...
### `createUint8ArrayFromBytes` (`src/exec/array_ops.zig:5872`)
...
### `uint8ArrayViewBytes` (`src/exec/array_ops.zig:5882`)
...
### `uint8ArrayCodecResult` (`src/exec/array_ops.zig:5890`)
...
### `isTypedArrayPrototypeMethod` (`src/exec/array_ops.zig:5898`)
...
### `typedArrayStaticMethodId` (`src/exec/array_ops.zig:5903`)
...
### `putDenseArrayElementFast` (`src/exec/array_ops.zig:5923`)
...
### `putDenseArrayElementOverwriteOwnedFast` (`src/exec/array_ops.zig:5956`)
...
### `putDenseArrayElementAppendOwnedFast` (`src/exec/array_ops.zig:5984`)
...
### `argsFromArray` (`src/exec/array_ops.zig:6000`)
...
### `ValueSliceRoot.init` (`src/exec/array_ops.zig:6034`)
...
### `ValueSliceRoot.deinit` (`src/exec/array_ops.zig:6043`)
...
### `CellSliceRoot.init` (`src/exec/array_ops.zig:6057`)

- **签名**：`pub fn init(self: *CellSliceRoot, rt: *core.JSRuntime, cells: *[]*core.VarRef) void`。
- **作用**：`ValueSliceRoot` 的 var-ref 版：把正在构造的 `[]*VarRef` 以 `.cells` 切片登记成精确根。
- **实现**：记下 rt、填 `slices[0] = .{ .cells = cells }`、建帧并 `activate`。
- **所有权 / 错误 / 调用**：与 `ValueSliceRoot` 同形，只是登记的是 `[]*core.VarRef` 切片（`.cells` 变体），给 mapped arguments 的 var-ref 数组在构造期间挂根用。不分配、无 error set。**全树无调用方**：`rg CellSliceRoot src` 只命中本处定义，它是 VARREFS-SLOT-TYPING 阶段 D 预留的 pub 工具（与已被七个文件使用的 `ValueSliceRoot` 对照）。

### `CellSliceRoot.deinit` (`src/exec/array_ops.zig:6066`)

- **签名**：`pub fn deinit(self: *CellSliceRoot) void`。
- **作用**：摘下根帧；不释放 cells 数组本身，未 init 时是 no-op。
- **实现**：`frame.deactivate(rt)` 后把 `rt` 置回 null。
- **所有权 / 错误 / 调用**：只 `frame.deactivate(rt)` 并清空 `rt`，不释放 cells 数组；无 error set。与 `init` 成对使用。

### `OwnedArrayLikeArgs.deinit` (`src/exec/array_ops.zig:6085`)
...
### `OwnedArrayLikeArgs.takeHeap` (`src/exec/array_ops.zig:6098`)
...
### `argsFromArrayLike` (`src/exec/array_ops.zig:6106`)
...
### `FastApplyArgs.len` (`src/exec/array_ops.zig:6146`)
...
### `FastApplyArgs.copyTo` (`src/exec/array_ops.zig:6154`)
...
### `fastApplyArgs` (`src/exec/array_ops.zig:6164`)
...
### `argumentsLengthSlotMatches` (`src/exec/array_ops.zig:6187`)
...
### `ownedArgsFromArrayLike` (`src/exec/array_ops.zig:6195`)
...
### `materializeArgsFromArrayLike` (`src/exec/array_ops.zig:6214`)
...
### `arrayIteratorMethodRecord` (`src/exec/array_ops.zig:6337`)
...
### `iteratorZipFlattenableRecord` (`src/exec/array_ops.zig:6363`)
...
- **作用**：Iterator.zip / zipKeyed：收集输入迭代器、padding、步进或关闭。
- **实现**：关键调用：`iterator_ops.iteratorZipFlattenableRecord`。
- **所有权 / 错误 / 调用**：一行转发，本身不分配、不建根；返回的 `IteratorZipRecord` 及其错误语义全由 `iterator_ops.iteratorZipFlattenableRecord` 决定。**树内没有调用方**：`iterator_ops.zig:1734`、`:1823` 调的是它自己那个同名函数，这层壳是给 array 域留的转发口。

### `arrayIteratorNextFast` (`src/exec/array_ops.zig:6381`)
...
### `arrayPrototypeValuesFromGlobal` (`src/exec/array_ops.zig:6386`)
...
### `createArrayFromArgs` (`src/exec/array_ops.zig:6393`)
...
### `arrayLengthAssignmentValue` (`src/exec/array_ops.zig:6453`)
...
### `typedArrayReflectSetReceiverOwn` (`src/exec/array_ops.zig:6472`)
...
### `typedArrayPrototypeSet` (`src/exec/array_ops.zig:6522`)
...
### `arrayJoinCall` (`src/exec/array_ops.zig:6572`)
...
### `fastDensePrimitiveArrayJoin` (`src/exec/array_ops.zig:6632`)
...
### `canFastJoinPrimitive` (`src/exec/array_ops.zig:6666`)
...
### `objectEntryArrayValue` (`src/exec/array_ops.zig:6675`)
...
### `typedArrayValidateConstructArgsPreAllocate` (`src/exec/array_ops.zig:6780`)
...
### `arrayLengthDefineValue` (`src/exec/array_ops.zig:6802`)
...
### `typedArrayCanonicalGet` (`src/exec/array_ops.zig:6814`)
...
### `typedArrayCanonicalOwnDescriptor` (`src/exec/array_ops.zig:6822`)
...
### `typedArrayCanonicalIndexExists` (`src/exec/array_ops.zig:6844`)
...
### `coerceTypedArrayElementInput` (`src/exec/array_ops.zig:6857`)
...
### `coerceTypedArrayElementForSet` (`src/exec/array_ops.zig:6869`)
...
### `typedArrayDefineOwnPropertyVm` (`src/exec/array_ops.zig:6881`)
...
### `typedArrayCanonicalSet` (`src/exec/array_ops.zig:6917`)
...
### `typedArrayCanonicalHas` (`src/exec/array_ops.zig:6942`)
...
### `typedArrayCanonicalDelete` (`src/exec/array_ops.zig:6954`)
...
### `decodeHexBytes` (`src/exec/array_ops.zig:6968`)
...
### `decodeHexInto` (`src/exec/array_ops.zig:6981`)
...
### `encodeHexBytes` (`src/exec/array_ops.zig:6995`)
...
### `hexNibble` (`src/exec/array_ops.zig:7005`)
...
### `decodeBase64Bytes` (`src/exec/array_ops.zig:7014`)
...
### `decodeBase64Into` (`src/exec/array_ops.zig:7026`)
...
### `decodeBase64Internal` (`src/exec/array_ops.zig:7037`)
...
### `decodeBase64Chunk` (`src/exec/array_ops.zig:7108`)
...
### `encodeBase64Bytes` (`src/exec/array_ops.zig:7165`)
...
### `base64Value` (`src/exec/array_ops.zig:7191`)
...
### `lengthIndexValue` (`src/exec/array_ops.zig:7209`)
...
### `LengthIndexAtom.deinit` (`src/exec/array_ops.zig:7228`)
...
- 清单函数数（本文件分到）: 64（`src/exec/array_ops.zig` 全文件 234）
- 本文标题覆盖: 64
- 未覆盖: 无
