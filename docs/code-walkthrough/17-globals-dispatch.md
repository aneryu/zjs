...
### `comptimeInternalRecordExists` (`src/exec/standard_globals.zig:159`)
...
### `noRecordReason` (`src/exec/standard_globals.zig:263`)
...
### `preparedMethods` (`src/exec/standard_globals.zig:273`)
...
### `preparedMethod` (`src/exec/standard_globals.zig:507`)
...
### `standardStringAutoInitDescriptor` (`src/exec/standard_globals.zig:592`)
...
### `constructorClassPrototypeId` (`src/exec/standard_globals.zig:660`)
...
### `temporaryStringAtom` (`src/exec/standard_globals.zig:743`)
...
### `freeTemporaryStringAtom` (`src/exec/standard_globals.zig:750`)
...
### `createBuiltinAsciiStringValue` (`src/exec/standard_globals.zig:755`)
...
### `defineData` (`src/exec/standard_globals.zig:764`)
...
- **所有权 / 错误 / 调用**：唯一的所有权动作是 `temporaryStringAtom` 对非预定义名字打的 host pin，`defer freeTemporaryStringAtom` 在本函数内成对归还；`value` 只是借用地写进描述符，属性表建立 GC 边后所有权归对象。错误集由 `internAtom` / `defineOwnProperty` 推断（`error.OutOfMemory` 为主）。树内唯一调用方是 `defineNativeMethod`（`src/exec/standard_globals.zig:958`）——⚠️ `src/core/promise.zig:152` 与 `src/exec/construct.zig:300` 调的是那两个文件各自的同名局部函数（`promise.zig:243` / `construct.zig:555`），不是这一个；core 也不可能依赖 exec。作为 `pub` 接口留给嵌入方。

### `defineDataAssumingNew` (`src/exec/standard_globals.zig:782`)
...
### `defineDataAtom` (`src/exec/standard_globals.zig:794`)
...
### `defineDataAtomAssumingNew` (`src/exec/standard_globals.zig:804`)
...
### `defineStringConstantAtomAssumingNewWithRealm` (`src/exec/standard_globals.zig:814`)
...
### `defineAccessorAtom` (`src/exec/standard_globals.zig:827`)
...
### `applyNativeFunctionMetadata` (`src/exec/standard_globals.zig:852`)
...
### `defineLazyNativeGetterAtom` (`src/exec/standard_globals.zig:872`)
...
### `defineLazyNativeGetterAtomWithRealm` (`src/exec/standard_globals.zig:883`)
...
### `defineLazyNativeGetterAtomWithRealmAndMetadata` (`src/exec/standard_globals.zig:903`)
...
### `defineLazyNativeAccessorPairAtom` (`src/exec/standard_globals.zig:918`)
...
### `bootstrapPropertyRealm` (`src/exec/standard_globals.zig:942`)
...
### `defineNativeMethod` (`src/exec/standard_globals.zig:949`)

- **签名**：`pub fn defineNativeMethod(rt: *core.JSRuntime, target: *core.Object, method: Method) !void`。
- **作用**：立刻（非 AUTOINIT）在对象上装一个 native 方法：建函数对象、盖 native 记录 id、按 `method_flags`（可写、不可枚举、可配置）写成数据属性。
- **实现**：`bootstrapPropertyRealm(rt, target, null)` 取 realm → `core.function.nativeFunction(realm, method.name, method.length)` 建函数；`method.native_builtin_id != 0` 时 `setNativeBuiltinIdAndRecord`，源码注释点明漏掉这步会让 `nativeMethodFastDispatch` 拒绝该函数对象、每次调用退回 `callNativeCallableByName` 的名字级联；最后 `defineData(rt, target, method.name, value, method_flags)`，即走查重版写入。
- **所有权 / 错误 / 调用**：建一个函数对象后立即由 `defineData` 写进属性表（`defineData` 内部再 pin/unpin 一次名字 atom），无长期持有、无 GC 根。错误：`bootstrapPropertyRealm` 的 `error.InvalidBuiltinRegistry` + 分配失败。整条 bootstrap install 家族的错误协议一致：错误集由 Zig 推断（实际只有 `error.OutOfMemory` 与本文件自己 `return` 的 `error.InvalidBuiltinRegistry`），一路 `try` 上抛到 `installStandardGlobals`，再由 `JSRuntime.installStandardGlobals`（`src/core/runtime.zig:4303`）`@errorCast` 成 `errors.RuntimeError`；失败时 `rollbackIntrinsicBootstrap` 回滚 realm、context 创建失败——**不经 `materializeRuntimeError`，也不会变成 JS 异常**。 树内唯一调用方是下面的 `defineNativeMethods`（`src/exec/standard_globals.zig:962`）；注意 `src/core/promise.zig:27` 调的是同名但不同签名的 `core.function.defineNativeMethod`，不是这一个。

### `defineNativeMethods` (`src/exec/standard_globals.zig:961`)

- **签名**：`pub fn defineNativeMethods(rt: *core.JSRuntime, target: *core.Object, methods: []const Method) !void`。
- **作用**：对一张 `Method` 表逐条调 `defineNativeMethod` 的 eager 安装循环，与 AUTOINIT 版的 `defineNativeMethodsAssumingNew` 相对。
- **实现**：一个 `for (methods) |method| try defineNativeMethod(...)`；不预留属性容量、不假设对象是新的，任一条失败立即中断并把 error 上抛。今天树内没有调用点——bootstrap 全走 AUTOINIT 版的 `defineNativeMethodsAssumingNew`，它作为 `pub` 接口留给嵌入方。
- **所有权 / 错误 / 调用**：自身不分配；每轮的函数对象由 `defineNativeMethod` 当场交出，中途失败已装上的方法留在对象上（bootstrap 失败会整体回滚 realm，所以不做补偿）。整条 bootstrap install 家族的错误协议一致：错误集由 Zig 推断（实际只有 `error.OutOfMemory` 与本文件自己 `return` 的 `error.InvalidBuiltinRegistry`），一路 `try` 上抛到 `installStandardGlobals`，再由 `JSRuntime.installStandardGlobals`（`src/core/runtime.zig:4303`）`@errorCast` 成 `errors.RuntimeError`；失败时 `rollbackIntrinsicBootstrap` 回滚 realm、context 创建失败——**不经 `materializeRuntimeError`，也不会变成 JS 异常**。 树内确实没有调用方：`src/exec/collection_ops.zig:285` 的 `defineNativeMethods(realm, object, class_id)` 是该文件自己的三参同名函数，不是这一个。作为 `pub` 接口留给嵌入方。

### `defineNativeMethodsAssumingNew` (`src/exec/standard_globals.zig:981`)

- **签名**：`pub fn defineNativeMethodsAssumingNew(rt: *core.JSRuntime, target: *core.Object, methods: []const Method) !void`。
- **作用**：在全新普通对象上批量安装 AUTOINIT 方法占位（installStandardGlobals 的主要加速点）。
...
### `defineNativeMethodsAssumingNewWithRealm` (`src/exec/standard_globals.zig:985`)
...
### `defineGlobalLazyMethods` (`src/exec/standard_globals.zig:1000`)
...
### `publishMethodAlias` (`src/exec/standard_globals.zig:1011`)
...
### `publishMethodAliasValue` (`src/exec/standard_globals.zig:1023`)
...
### `publishTypedArrayToStringAlias` (`src/exec/standard_globals.zig:1042`)
...
### `createNamespaceObject` (`src/exec/standard_globals.zig:1059`)
...
### `defineLazyNamespace` (`src/exec/standard_globals.zig:1072`)
...
### `materializeBuiltinNamespaceAutoInit` (`src/exec/standard_globals.zig:1085`)
...
### `createJsonNamespaceObject` (`src/exec/standard_globals.zig:1115`)
...
### `objectPrototypeFromGlobal` (`src/exec/standard_globals.zig:1132`)
...
- **实现**：`getOwnDataObjectBorrowed("Object")` 再 `getOwnDataObjectBorrowed(prototype)`，任一缺失返回 null。`rt` 参数未用。
- **所有权 / 错误 / 调用**：返回借用指针，不加引用。

### `standardGlobalOwnPropertyCapacity` (`src/exec/standard_globals.zig:1147`)
...
### `constructorOwnPropertyCapacity` (`src/exec/standard_globals.zig:1154`)
...
### `prototypeOwnPropertyCapacity` (`src/exec/standard_globals.zig:1159`)
...
### `constructorExtraPropertyCount` (`src/exec/standard_globals.zig:1168`)
...
### `prototypeExtraPropertyCount` (`src/exec/standard_globals.zig:1200`)
...
### `constructorStaticMethodsBeforePrototype` (`src/exec/standard_globals.zig:1256`)
...
### `prototypeMethodsAreInstalledByExtras` (`src/exec/standard_globals.zig:1276`)
...
### `defineConstructor` (`src/exec/standard_globals.zig:1288`)
...
### `isErrorConstructorKind` (`src/exec/standard_globals.zig:1397`)
...
### `nativeErrorKind` (`src/exec/standard_globals.zig:1414`)
...
### `expectObjectAssumeBootstrap` (`src/exec/standard_globals.zig:1434`)
...
- **所有权 / 错误 / 调用**：无检查的解包：`objectFromValue(value).?`，非对象值会命中 `.?` 的 unreachable。返回借用指针，不加引用、不分配、无 error set。调用契约就是源码注释写的那句——只许传安装器自己刚造出来的对象，JS 来路的值要走 `core.value_semantics.expectObject`。本文件共 42 处调用方，非测试路径 12 处：`applyNativeFunctionMetadata`（858）、`defineLazyNativeAccessorPairAtom`（931/938）、`defineNativeMethod`（956）、`publishTypedArrayToStringAlias`（1052）、`defineConstructor`（1314/1332）、`installStandardConstructorWithPrototype`（1528）、`installStandardConstructors`（1655/1668）、`installTypedArrayIntrinsicExtras`（1824）、`bindMaterializedNativeRecordByAtom`（1956），其余 30 处在文件末尾的测试里。

### `installedConstructor` (`src/exec/standard_globals.zig:1438`)
...
### `constructorPrototypeObject` (`src/exec/standard_globals.zig:1442`)

- **签名**：`fn constructorPrototypeObject(rt: *core.JSRuntime, ctor: *core.Object) ?*core.Object`。
- **作用**：借出构造器自有的 `prototype` 数据属性对象。
- **实现**：`ctor.getOwnDataObjectBorrowed(core.atom.ids.prototype)`，缺失返回 null。`rt` 参数未用。
- **所有权 / 错误 / 调用**：返回借用指针，不加引用。

### `materializeBuiltinNamespace` (`src/exec/standard_globals.zig:1448`)
...
### `registerStandardGlobalsDefault` (`src/exec/standard_globals.zig:1455`)
...
### `configureRuntime` (`src/exec/standard_globals.zig:1462`)
...
### `installStandardConstructor` (`src/exec/standard_globals.zig:1471`)
...
### `installStandardConstructorWithPrototype` (`src/exec/standard_globals.zig:1484`)
...
### `installStandardConstructors` (`src/exec/standard_globals.zig:1634`)
...
### `installStandardGlobals` (`src/exec/standard_globals.zig:1725`)
...
### `installNumberParseAliases` (`src/exec/standard_globals.zig:1769`)
...
### `installNumberConstants` (`src/exec/standard_globals.zig:1777`)
...
### `numberConstantValue` (`src/exec/standard_globals.zig:1797`)
...
### `finalizeStandardConstructorGraph` (`src/exec/standard_globals.zig:1809`)
...
### `installTypedArrayIntrinsicExtras` (`src/exec/standard_globals.zig:1816`)
...
### `isNativeErrorSubclassKind` (`src/exec/standard_globals.zig:1833`)
...
### `isConcreteTypedArrayKind` (`src/exec/standard_globals.zig:1849`)
...
### `installMathConstants` (`src/exec/standard_globals.zig:1868`)
...
### `bindMathNativeRecords` (`src/exec/standard_globals.zig:1880`)
...
### `bindAtomicsNativeRecords` (`src/exec/standard_globals.zig:1886`)
...
### `bindReflectNativeRecords` (`src/exec/standard_globals.zig:1892`)
...
### `defineCollectionPrototypeMethodsAssumingNew` (`src/exec/standard_globals.zig:1899`)
...
### `defineCollectionSizeAccessorAssumingNew` (`src/exec/standard_globals.zig:1913`)
...
### `setDateConstructorNativeRecord` (`src/exec/standard_globals.zig:1930`)
...
### `bindNativeRecordByName` (`src/exec/standard_globals.zig:1934`)
...
### `bindMaterializedNativeRecordByAtom` (`src/exec/standard_globals.zig:1948`)
...
### `bindAutoInitNativeRecordByAtom` (`src/exec/standard_globals.zig:1960`)
...
### `installTypedArrayElementSize` (`src/exec/standard_globals.zig:1975`)
...
### `installTypedArrayConstructorElementSize` (`src/exec/standard_globals.zig:1987`)
...
### `installUint8ArrayConstructorCodecExtras` (`src/exec/standard_globals.zig:1993`)
...
### `installUint8ArrayCodecExtras` (`src/exec/standard_globals.zig:1997`)
...
### `installTypedArrayPrototypeAccessors` (`src/exec/standard_globals.zig:2007`)
...
### `keep` (`src/exec/standard_globals.zig:2065`)
...
### `methodsFromInternalEntries` (`src/exec/standard_globals.zig:2574`)
...
### `methodsFromInternalEntriesWhere` (`src/exec/standard_globals.zig:2592`)
...
### `countInternalEntriesWhere` (`src/exec/standard_globals.zig:2611`)
...
### `installSymbolExtras` (`src/exec/standard_globals.zig:2662`)
...
### `installWellKnownSymbolProperties` (`src/exec/standard_globals.zig:2675`)
...
### `defineWellKnownSymbol` (`src/exec/standard_globals.zig:2694`)
...
### `installArrayPrototypeSymbols` (`src/exec/standard_globals.zig:2700`)
...
### `bufferAccessorNativeId` (`src/exec/standard_globals.zig:2740`)
...
### `installArrayBufferExtras` (`src/exec/standard_globals.zig:2764`)
...
### `installSharedArrayBufferExtras` (`src/exec/standard_globals.zig:2777`)
...
### `installDataViewExtras` (`src/exec/standard_globals.zig:2790`)
...
### `installBufferConstructorExtras` (`src/exec/standard_globals.zig:2810`)
...
### `defineDatePrototypeMethodsAssumingNew` (`src/exec/standard_globals.zig:2843`)
...
### `installOnePrototypeAutoInit` (`src/exec/standard_globals.zig:2860`)
...
### `installDatePrototypeAliases` (`src/exec/standard_globals.zig:2873`)
...
### `installFunctionPrototypeExtras` (`src/exec/standard_globals.zig:2884`)
...
### `installErrorPrototypeExtras` (`src/exec/standard_globals.zig:2895`)
...
### `installPromiseExtras` (`src/exec/standard_globals.zig:2913`)
...
### `installIteratorExtras` (`src/exec/standard_globals.zig:2924`)
...
### `installStringPrototypeAliases` (`src/exec/standard_globals.zig:2960`)
...
### `defineObjectPrototypeMethodsAssumingNew` (`src/exec/standard_globals.zig:2970`)
...
### `installPerformance` (`src/exec/standard_globals.zig:2979`)
...
### `installNavigator` (`src/exec/standard_globals.zig:2985`)
...
### `installRegExpExtras` (`src/exec/standard_globals.zig:2991`)
...
### `installRegExpLegacyAccessors` (`src/exec/standard_globals.zig:3042`)
...
### `defineRegExpLegacyAccessor` (`src/exec/standard_globals.zig:3076`)
...
- **实现**：`ctor.nativeFunctionRealmGlobalPtr()` 取构造器所属 realm 的 global（拿不到即 `error.InvalidBuiltinRegistry`），作为后续两个 helper 的显式 realm。键走 `rt.internAtom(name)` 而不是 `predefinedId`——`$&`、`` $` ``、`$'` 这些名字不在预定义 atom 表里。getter id 由 `core.function.nativeBuiltinId(.regexp, @intFromEnum(getter_method))` 编码；有 setter 就走 `defineLazyNativeAccessorPairAtom`（setter length 1，setter id 同样这样编码），否则走 `defineLazyNativeGetterAtomWithRealm` 装只读 getter。
- **所有权 / 错误 / 调用**：⚠️ 这里的 `rt.internAtom(name)` **没有配对的 pin/unpin，也没有 `defer`**——它不像本文件其余路径那样走 `temporaryStringAtom`/`freeTemporaryStringAtom`，intern 出来的 atom 引用直接留给随后的 define 调用，由属性表接管。`realm_global` 从 `ctor.nativeFunctionRealmGlobalPtr()` 借出，取不到即 `error.InvalidBuiltinRegistry`。有 setter 的走 `defineLazyNativeAccessorPairAtom`（建两个函数对象），否则 `defineLazyNativeGetterAtomWithRealm`（建一个）。错误集由 Zig 推断（`error.OutOfMemory` 与本文件 `return` 的 `error.InvalidBuiltinRegistry`），`try` 上抛到 `installStandardGlobals` 再由 `JSRuntime.installStandardGlobals`（`src/core/runtime.zig:4303`）`@errorCast` 成 `errors.RuntimeError`，失败即回滚 realm 让 context 创建失败——**不经 `materializeRuntimeError`，不变成 JS 异常**。 唯一调用方 `installRegExpLegacyAccessors`（`src/exec/standard_globals.zig:3072`）。

### `installNativeMethodAlias` (`src/exec/standard_globals.zig:3105`)
...
### `installCollectionExtras` (`src/exec/standard_globals.zig:3113`)
...
### `installCollectionSpecies` (`src/exec/standard_globals.zig:3122`)
...
### `installTypedArraySpecies` (`src/exec/standard_globals.zig:3128`)
...
### `installCollectionPrototypeSymbols` (`src/exec/standard_globals.zig:3134`)
...
### `installNamespaceToStringTag` (`src/exec/standard_globals.zig:3154`)
...
### `installPrototypeToStringTag` (`src/exec/standard_globals.zig:3158`)
...
### `installDisposableStackExtras` (`src/exec/standard_globals.zig:3163`)
...
### `installAsyncDisposableStackExtras` (`src/exec/standard_globals.zig:3175`)
...
### `installDisposableStackCtorExtras` (`src/exec/standard_globals.zig:3193`)
...
### `installDOMExceptionExtras` (`src/exec/standard_globals.zig:3217`)
...
### `collectionTag` (`src/exec/standard_globals.zig:3229`)
...
### `collectionNameForKind` (`src/exec/standard_globals.zig:3237`)
...
### `collectionPrototypeMethods` (`src/exec/standard_globals.zig:3247`)
...
### `Intrinsics.init` (`src/exec/standard_globals.zig:3259`)
...
### `Intrinsics.deinit` (`src/exec/standard_globals.zig:3275`)

- **签名**：`pub fn deinit(self: *Intrinsics, rt: *core.JSRuntime) void`。
- **作用**：初始化或释放该结构持有的根 / 缓冲。
- **实现**：只 `self.context.destroy()`；`rt` 参数未用，global 随 runtime 回收。
- **所有权 / 错误 / 调用**：只 `self.context.destroy()`，`rt` 参数用 `_ = rt` 丢弃——**global 对象不在这里释放**，它由 context 销毁时连同 realm 一起走 GC。无 error set。唯一调用方是本文件的测试：每个 `Intrinsics.init` 后面跟一句 `defer intrinsics.deinit(rt)`（`src/exec/standard_globals.zig:3498`、3539、3584 等共 5 处）。

### `methodDescriptor` (`src/exec/standard_globals.zig:3281`)
...
### `getNamedPropertyForTest` (`src/exec/standard_globals.zig:3442`)
...
### `expectNativeAliasForTest` (`src/exec/standard_globals.zig:3448`)
...
### `getConstructorPrototypeForTest` (`src/exec/standard_globals.zig:3466`)
...
### `expectNativeFunctionForTest` (`src/exec/standard_globals.zig:3475`)
...
### `expectAutoInitOwnPropertyForTest` (`src/exec/standard_globals.zig:3488`)
...
- **实现**：`nativeReceiverSelf` miss 返回 null；否则按 `entry.sig` 走 `self_to_f64` / `self_to_i32`，其他 sig 是 `unreachable`（安装时已保证）。
- **所有权 / 错误 / 调用**：不分配、不建根、无 error set，返回 null 表示 receiver 类不符。⚠️ `switch` 的 `else` 臂是 `unreachable`：**只有两种 sig（`self_to_f64` / `self_to_i32`）合法**，注册了别的 sig 的 typed getter 会在这里 UB/panic 而不是优雅 miss。3 个调用方：`src/binding/property_site.zig:177`、`src/exec/tailcall_dispatch.zig:4125`（W1 属性 IC 的内联臂）、本文件 `invokeTypedGetter`（997）。

### `invokeTypedSetterFast` (`src/exec/builtin_dispatch.zig:978`)
...
- **实现**：先解 `self`，失败即 `.receiver_miss`；再按 sig 用 `marshalF64` / `marshalI32` 取值（规范策略不做 ToNumber），取不到即 `.value_miss`；其他 sig `unreachable`。
- **所有权 / 错误 / 调用**：不分配、无 error set；用三态枚举 `TypedSetterOutcome` 区分 `.stored` / `.receiver_miss` / `.value_miss`，让调用方分别抛两种不同的 TypeError。marshal 走 `marshalF64` / `marshalI32`，不做强制转换。`else` 臂同样是 `unreachable`（只允许 `self_f64_to_void` / `self_i32_to_void` 两种 sig）。虽然是 `pub`，树内唯一调用方是下面的 `invokeTypedSetter`（`src/exec/builtin_dispatch.zig:1002`）。

### `invokeTypedGetter` (`src/exec/builtin_dispatch.zig:996`)
...
### `invokeTypedSetter` (`src/exec/builtin_dispatch.zig:1001`)
...
### `throwTypedSetterValueTypeError` (`src/exec/builtin_dispatch.zig:1009`)
...
### `tryNativeAccessorCall` (`src/exec/builtin_dispatch.zig:1021`)
...
### `nativeAccessorTarget` (`src/exec/builtin_dispatch.zig:1045`)
...
### `callNativeAccessorTarget` (`src/exec/builtin_dispatch.zig:1056`)
...
### `invokeLeafFastEntry` (`src/exec/builtin_dispatch.zig:1084`)
...
### `invokeMethodLeafFastEntry` (`src/exec/builtin_dispatch.zig:1092`)
...
### `invokeMethodLeafFast` (`src/exec/builtin_dispatch.zig:1102`)
...
### `leafStringReceiver` (`src/exec/builtin_dispatch.zig:1130`)
...
### `leafCodeUnitString` (`src/exec/builtin_dispatch.zig:1142`)

- **签名**：`inline fn leafCodeUnitString(rt: *core.JSRuntime, unit: u16) ?core.JSValue`。
- **作用**：造 charAt / at 的单码元结果字符串。
- **实现**：`< 0x100` 走运行时单字节字符串表 `rt.singleByteString`（首次之后不再分配），否则 `String.createUtf16`；分配失败返回 null 当作 miss，由 fallback 重做并报错。
- **所有权 / 错误 / 调用**：`unit < 0x100` 时从 runtime 的单码元串表取（`singleByteString`，首次之后**零分配**），否则 `String.createUtf16` 真建一个一码元字符串。返回的值随后立刻被装箱返回给 VM，不建根。无 error set：**分配失败被 `catch return null` 变成一次 miss**，由 fallback 重做分配并走正常错误通道——这是本文件里唯一把 OOM 降级成 miss 的地方。唯一调用方 `invokeMethodLeafFast` 的 `sig_string_i32_to_string` 臂（`src/exec/builtin_dispatch.zig:1118`）。

### `invokeLeafFast` (`src/exec/builtin_dispatch.zig:1157`)
...
### `leafI32Arg` (`src/exec/builtin_dispatch.zig:1216`)
...
### `leafF64Arg` (`src/exec/builtin_dispatch.zig:1231`)
...
### `invokeLeafFallback` (`src/exec/builtin_dispatch.zig:1238`)
...
### `primitiveF64Arg` (`src/exec/builtin_dispatch.zig:1252`)

- **签名**：`inline fn primitiveF64Arg(args: []const core.JSValue, index: usize) ?f64`。
- **作用**：数值 cproto 的参数取值，对标 qjs 的 `JS_ToFloat64` 原始类型快路径。
...
### `callConstructRecord` (`src/exec/builtin_dispatch.zig:1277`)
...
### `callConstructRecordInNativeScope` (`src/exec/builtin_dispatch.zig:1295`)
...
### `callConstructRecordImpl` (`src/exec/builtin_dispatch.zig:1310`)
...
### `isConstructRecordRef` (`src/exec/builtin_dispatch.zig:1364`)
...
### `callerBytecode` (`src/exec/builtin_dispatch.zig:1370`)
...
### `callerFrame` (`src/exec/builtin_dispatch.zig:1375`)
...
### `callerResultIsDropped` (`src/exec/builtin_dispatch.zig:1384`)
...
### `toBigIntFromPrimitive` (`src/exec/builtin_glue.zig:123`)
...
### `globalIsNaNOrFinite` (`src/exec/builtin_glue.zig:134`)
...
### `toNumberLikeArgument` (`src/exec/builtin_glue.zig:158`)
...
### `globalParseInt` (`src/exec/builtin_glue.zig:170`)
...
### `globalParseFloat` (`src/exec/builtin_glue.zig:194`)
...
### `arrayNativeRecord` (`src/exec/builtin_glue.zig:221`)
...
### `bufferNativeRecord` (`src/exec/builtin_glue.zig:241`)
...
### `dataViewConstructorArgs` (`src/exec/builtin_glue.zig:302`)
...
### `dataViewAccessor` (`src/exec/builtin_glue.zig:326`)
...
### `dataViewGetCall` (`src/exec/builtin_glue.zig:341`)
...
### `dataViewSetCall` (`src/exec/builtin_glue.zig:357`)
...
### `dataViewSetCoerceValue` (`src/exec/builtin_glue.zig:375`)
...
### `weakRefEntry` (`src/exec/builtin_glue.zig:412`)
...
### `weakRefCall` (`src/exec/builtin_glue.zig:427`)
...
### `weakRefDerefCall` (`src/exec/builtin_glue.zig:445`)
...
### `finalizationRegistryRegister` (`src/exec/builtin_glue.zig:451`)
...
### `finalizationRegistryUnregister` (`src/exec/builtin_glue.zig:467`)
...
### `finalizationRegistryAppendCell` (`src/exec/builtin_glue.zig:475`)
...
### `symbolFor` (`src/exec/builtin_glue.zig:485`)
...
### `symbolKeyFor` (`src/exec/builtin_glue.zig:502`)
...
### `createDataFunction` (`src/exec/builtin_glue.zig:511`)
...
### `constructCollectionFromVm` (`src/exec/builtin_glue.zig:516`)
...
### `addCollectionEntriesFromIterator` (`src/exec/builtin_glue.zig:529`)
...
### `callCollectionAdderFromVm` (`src/exec/builtin_glue.zig:612`)
...
### `functionConstructorFromGlobal` (`src/exec/builtin_glue.zig:627`)

- **签名**：`pub fn functionConstructorFromGlobal(rt: *core.JSRuntime, global: *core.Object) ?*core.Object`。
- **作用**：从 global 借出 `Function` 构造器对象。
- **实现**：`global.getOwnDataObjectBorrowed(core.atom.ids.Function)`，没有返回 null；`rt` 参数未用。
- **所有权 / 错误 / 调用**：无：`rt` 参数用 `_ = rt` 丢弃，只 `getOwnDataObjectBorrowed(core.atom.ids.Function)` 读一次 global 的自有属性，返回**借用**指针或 null；不分配、无 error set。注意它读的是可变的 `globalThis.Function` 绑定（不是 realm 缓存槽），所以用户改写 `Function` 会被它看见。3 个调用点跨两个文件：`src/exec/object_ops.zig:258`、`src/exec/promise_ops.zig:144`/209（两个文件各在顶部 75 / 72 行起了 `const` 别名），用途都是给新造的构造器对象设 `[[Prototype]]`。

### `storeRealmValue` (`src/exec/builtin_glue.zig:633`)
...
### `defineNativeDataMethod` (`src/exec/builtin_glue.zig:643`)
...
### `defineNativeDataMethodWithNativeId` (`src/exec/builtin_glue.zig:650`)
...
### `defineNativeDataMethodMaybeId` (`src/exec/builtin_glue.zig:654`)
...
### `defineStampedNativeDataMethod` (`src/exec/builtin_glue.zig:678`)
...
### `defineNativeDataMethodNamedWithNativeId` (`src/exec/builtin_glue.zig:703`)
...
- **实现**：`rt.internAtom(name)` 取键，`nativeFunctionForGlobal` 造函数（非对象则 TypeError），`setNativeBuiltinIdAndRecord` 盖记录，最后 `defineOwnProperty` 成可写、不可枚举、可配置的数据属性。
- **所有权 / 错误 / 调用**：⚠️ `rt.internAtom(name)` **没有配对的 unpin / defer**——intern 出来的 atom 引用直接交给随后的 `defineOwnProperty`，由属性表接管（与 `standard_globals.defineRegExpLegacyAccessor` 同一个模式）。新建的函数对象也当场写进属性表，不建根。它与 `defineNativeDataMethodWithNativeId` 的唯一区别是收字节串而不是预定义 atom——注释点明这是为了那个方法名来自表而不是常量的调用方。错误：`expectObject` 失败的 `error.TypeError` + intern / 建函数 / define 的分配失败。错误一路 `try` 上抛到 VM 的 native 终端（`builtin_dispatch.nativeFromHostError` → `materializeRuntimeError`）才建 Error 对象、装 pending exception 并换成哨兵，本函数自己不碰 pending。 唯一调用方 `src/exec/object_ops.zig:1020`（CallSite 原型的方法表）。
...
帧局部/参数的开 `VarRef` 表。表持有每个活 cell 的一条引用；调用方 `acquire` 再拿一条。`close` 从帧存储摘下 cell（身份不变）并释放表侧持有。对照 qjs `get_var_ref`（quickjs.c:16997-17044）。

### 类型

- `Flags`：`is_const` / `is_lexical` / `is_function_name`。
- `Table.cells`：`[]?*VarRef`，按下标与帧槽对应。


### `Table.acquire` (`src/exec/open_bindings.zig:26`)

- **签名**：`pub inline fn acquire( self: *Table, rt: anytype, binding_index: u16, value_slot: *core.JSValue, flags: Flags, ) !*core.VarRef`。
- **作用**：取得 `binding_index` 上唯一的开 cell：已有则校验仍指向同一槽，否则 `VarRef.createOpen`。
- **实现**：下标越界 → `InvalidBytecode`；已有 cell 则要求它仍 `is_open` 且 `pvalue` 正好是同一个槽，否则 `InvalidBytecode`。新建走 `VarRef.createOpen` 并写入三个 flag。表留下初始引用；返回的指针是调用方另一条引用。对照 qjs `get_var_ref`（quickjs.c:16997-17044）。
- **所有权 / 错误 / 调用**：表留下初始引用，返回的 `*core.VarRef` 是调用方另一条引用。⚠️ 树内**没有调用方**：`Frame.captureLocal`（`src/exec/frame.zig:570`）与 `captureArg`（598）把同一段「已有就复用、否则 `VarRef.createOpen`」逻辑直接写在自己身上、操作 `self.open_var_refs`，并没有走这个 `acquire`（源码注释里的 “Force this small table body into Frame.captureLocal/captureArg” 说的正是这件事）。`Table` 的另外三个方法在 `frame.zig` 561/625/632/757 有真调用点。

### `Table.close` (`src/exec/open_bindings.zig:48`)
...
### `Table.closeAll` (`src/exec/open_bindings.zig:56`)
...
### `Table.hasOpen` (`src/exec/open_bindings.zig:64`)
...
- 清单函数数: 247（`src/exec/builtin_dispatch.zig` 77 + `src/exec/builtin_glue.zig` 35 + `src/exec/internal_builtins.zig` 3 + `src/exec/open_bindings.zig` 4 + `src/exec/standard_globals.zig` 128）
- 本文标题覆盖: 247
- 未覆盖: 无
