...
### `object` (`src/exec/vm_opcodes.zig:26`)
...
### `objectReserved2` (`src/exec/vm_opcodes.zig:36`)
...
### `newPlainObjectValue` (`src/exec/vm_opcodes.zig:55`)
...
### `newPlainObjectReserved2Value` (`src/exec/vm_opcodes.zig:60`)
...
### `defineFieldFast` (`src/exec/vm_opcodes.zig:95`)
...
### `arrayFrom` (`src/exec/vm_opcodes.zig:112`)
...
### `defineField` (`src/exec/vm_opcodes.zig:136`)
...
### `setProto` (`src/exec/vm_opcodes.zig:211`)
...
### `defineArrayEl` (`src/exec/vm_opcodes.zig:225`)
...
### `appendSpreadValues` (`src/exec/vm_opcodes.zig:255`)
...
### `appendSpreadValuesVm` (`src/exec/vm_opcodes.zig:276`)
...
### `copyDataProperties` (`src/exec/vm_opcodes.zig:292`)
...
### `handleLiteralRuntimeError` (`src/exec/vm_opcodes.zig:421`)
...
### `specialObject` (`src/exec/vm_opcodes.zig:434`)
...
### `getLength` (`src/exec/vm_opcodes.zig:470`)
...
### `rest` (`src/exec/vm_opcodes.zig:488`)
...
### `stackValueFromTop` (`src/exec/vm_opcodes.zig:512`)
...
### `readInt` (`src/exec/vm_opcodes.zig:518`)

- **签名**：`fn readInt(comptime T: type, bytes: []const u8) T`。
- **作用**：读 argc / atom / rest 索引。
- **实现**：小端。
- **所有权 / 错误 / 调用**：纯读借用的字节码切片，不分配、无 error set，定长由调用点切片保证。文件私有，调用方：`src/exec/vm_opcodes.zig:119`（argc）、`:145`（atom）、`:494`（rest 起始参数索引）。
