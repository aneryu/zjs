...
### `FunctionEntryContext.save` (`src/parser.zig:7140`)
...
### `FunctionEntryContext.restore` (`src/parser.zig:7154`)
...
### `parseFunctionDecl` (`src/parser.zig:11080`)
...
### `parseFunctionExpr` (`src/parser.zig:11151`)
...
### `parseAnonymousDefaultFunctionDecl` (`src/parser.zig:11213`)

- **签名**：`fn parseAnonymousDefaultFunctionDecl( s: *State, func_kind: ParseFunctionKind, source_start: FunctionSourceStart, ) Error!void`。
- **作用**：解析匿名的 `export default function`：它仍是声明，外部载体名 `_default_`，推断出的函数名是 `default`。
...
### `appendOwnedParserAtom` (`src/parser.zig:11247`)
...
### `deinitOwnedParserAtoms` (`src/parser.zig:11252`)
...
### `FunctionParameters.deinit` (`src/parser.zig:11262`)
...
### `parseFunctionParameters` (`src/parser.zig:11290`)
...
### `parseFunctionParamsAndBody` (`src/parser.zig:11519`)
...
### `parseArrowFunction` (`src/parser.zig:12143`)
...
### `PatternTarget.depth` (`src/parser.zig:12564`)
...
### `PatternTarget.defaultName` (`src/parser.zig:12571`)
...
### `PatternTarget.deinit` (`src/parser.zig:12581`)
...
### `scanPatternTopology` (`src/parser.zig:12598`)
...
### `tokenStartsNestedPattern` (`src/parser.zig:12622`)
...
### `checkPatternParameterDuplicate` (`src/parser.zig:12634`)
...
### `definePatternBindingAtom` (`src/parser.zig:12643`)
...
### `parsePatternBindingTarget` (`src/parser.zig:12691`)
...
### `parsePatternTarget` (`src/parser.zig:12702`)
...
### `shorthandPatternTarget` (`src/parser.zig:12712`)
...
### `shorthandPatternCanUseGetField2` (`src/parser.zig:12731`)
...
### `parseArrayPatternBody` (`src/parser.zig:12943`)
...
### `parseObjectPatternBody` (`src/parser.zig:13008`)
...
### `parseDestructuringElement` (`src/parser.zig:13121`)
...
### `appendArrowParamBindingName` (`src/parser.zig:13171`)
...
### `enterParameterExpressionScope` (`src/parser.zig:13433`)
...
### `appendParameterExpressionBinding` (`src/parser.zig:13446`)
...
### `initializeParameterScopeBinding` (`src/parser.zig:13450`)
...
### `parseParameterDestructuring` (`src/parser.zig:13455`)
...
### `leaveParameterExpressionScope` (`src/parser.zig:13488`)
...
### `scanParameterList` (`src/parser.zig:13520`)
...
### `ensureDestructuringArgSlot` (`src/parser.zig:13525`)
...
### `findCurrentScopeVar` (`src/parser.zig:13543`)
...
### `appendAnonymousTempLocal` (`src/parser.zig:13553`)
...
### `parseNamedBindingDefaultInitializer` (`src/parser.zig:13564`)
...
### `trailingClassNamePatch` (`src/parser.zig:13569`)
...
### `parseClassHeritage` (`src/parser.zig:13683`)
...
### `parseClassElement` (`src/parser.zig:13699`)
...
### `parseClass` (`src/parser.zig:14746`)
...
- 清单函数数（本文件分到）: 40（`src/parser.zig` 全文件 649）
- 本文标题覆盖: 40
- 未覆盖: 无
