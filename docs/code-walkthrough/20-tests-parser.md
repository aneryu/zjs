...
### `freeAndDrain` (`src/tests/parser.zig:68`)

- **签名**：`fn freeAndDrain(lx: *QjsLexer, tok: *t.Token) void`。
- **作用**：测试夹具/探针 `freeAndDrain`，给周围 `test` 块提供可注入行为或断言助手。
- **实现**：函数体只有 `lx.freeToken(tok)` 一行，把 token 持有的 atom/字符串所有权交还词法器；名字里的 "drain" 没有对应动作。关键调用：`lx.freeToken`。
- **所有权 / 错误 / 调用**：无独立 error set 时失败以断言或 panic 终止测试。

### `ParserTestEnv.init` (`src/tests/parser.zig:776`)
...
### `ParserTestEnv.deinit` (`src/tests/parser.zig:784`)
...
### `ParserTestEnv.compileContext` (`src/tests/parser.zig:789`)
...
### `compileForTest` (`src/tests/parser.zig:797`)
...
### `restoreFinalizedFragmentView` (`src/tests/parser.zig:808`)
...
### `parseExpr` (`src/tests/parser.zig:822`)
...
### `parseExprWithTopLevelChildren` (`src/tests/parser.zig:836`)
...
### `parseExprStrict` (`src/tests/parser.zig:852`)
...
### `parseStatement` (`src/tests/parser.zig:872`)
...
### `parseTSStatement` (`src/tests/parser.zig:887`)
...
### `parseTSProgram` (`src/tests/parser.zig:904`)
...
### `parseStatementWithTopLevelChildren` (`src/tests/parser.zig:924`)
...
### `parseModuleStatement` (`src/tests/parser.zig:940`)
...
### `parseModuleRefStatement` (`src/tests/parser.zig:957`)
...
### `moduleBodyStart` (`src/tests/parser.zig:975`)
...
### `moduleRecord` (`src/tests/parser.zig:989`)
...
### `rootCode` (`src/tests/parser.zig:994`)
...
### `rootConstants` (`src/tests/parser.zig:1001`)
...
### `rootClosureVars` (`src/tests/parser.zig:1008`)
...
### `rootVarDefs` (`src/tests/parser.zig:1014`)
...
### `expectAtomName` (`src/tests/parser.zig:1023`)
...
### `functionBytecodeFromValue` (`src/tests/parser.zig:1028`)
...
### `expectNoGlobalArgumentsVarOpcode` (`src/tests/parser.zig:1034`)
...
### `expectFunctionConstant` (`src/tests/parser.zig:1059`)
...
### `findFunctionConstantNamed` (`src/tests/parser.zig:1065`)
...
### `countFunctionConstantsNamed` (`src/tests/parser.zig:1077`)
...
### `globalDeclarationClosureNamed` (`src/tests/parser.zig:1090`)
...
### `declarationClosureNamed` (`src/tests/parser.zig:1103`)
...
### `globalDeclarationClosureCount` (`src/tests/parser.zig:1116`)
...
### `varDefNamed` (`src/tests/parser.zig:1124`)
...
### `countPutVarRefStores` (`src/tests/parser.zig:1138`)
...
### `countCheckedPutVarRefStores` (`src/tests/parser.zig:1146`)
...
### `countSetVarRefStores` (`src/tests/parser.zig:1151`)
...
### `countDynEnvProbe` (`src/tests/parser.zig:1159`)
...
### `countDynEnvProbeInFunctionBytecode` (`src/tests/parser.zig:1175`)
...
### `countDynEnvProbeRecursive` (`src/tests/parser.zig:1188`)
...
### `countOpcodeInFunctionBytecode` (`src/tests/parser.zig:1198`)
...
### `countOpcodeRecursive` (`src/tests/parser.zig:1208`)
...
### `countDefineClassNamedInCode` (`src/tests/parser.zig:1218`)
...
### `countDefineClassNamedInFunctionBytecode` (`src/tests/parser.zig:1238`)
...
### `countDefineClassNamedRecursive` (`src/tests/parser.zig:1252`)
...
### `countSemanticOpcodeInFunctionBytecode` (`src/tests/parser.zig:1266`)
...
### `countSemanticOpcodeRecursive` (`src/tests/parser.zig:1278`)
...
### `countVarRefStoresRecursive` (`src/tests/parser.zig:1287`)
...
### `expectOpcode` (`src/tests/parser.zig:1308`)
...
### `expectOpcodeSequence` (`src/tests/parser.zig:1312`)
...
### `semanticOpcodeForTest` (`src/tests/parser.zig:1324`)
...
### `expectSemanticOpcodeAt` (`src/tests/parser.zig:1357`)
...
### `expectSemanticOpcodeSequence` (`src/tests/parser.zig:1362`)
...
### `firstSemanticOpcodeOffset` (`src/tests/parser.zig:1373`)
...
### `countSemanticOpcode` (`src/tests/parser.zig:1384`)
...
### `integerPushValueAtOpcode` (`src/tests/parser.zig:1396`)
...
### `countIntegerPushValue` (`src/tests/parser.zig:1409`)
...
### `slotIndexAtOpcode` (`src/tests/parser.zig:1422`)
...
### `labelTargetAtOpcode` (`src/tests/parser.zig:1445`)
...
### `readU16AtOpcode` (`src/tests/parser.zig:1463`)
...
### `readConstIndexAtOpcode` (`src/tests/parser.zig:1467`)
...
### `expectOpcodeRecursive` (`src/tests/parser.zig:1475`)
...
### `expectModuleRecordCounts` (`src/tests/parser.zig:1479`)
...
### `expectModuleRequest` (`src/tests/parser.zig:1495`)
...
### `expectModuleImport` (`src/tests/parser.zig:1499`)
...
### `expectModuleExport` (`src/tests/parser.zig:1513`)
...
### `expectModuleIndirectExport` (`src/tests/parser.zig:1525`)
...
### `expectModuleStarExport` (`src/tests/parser.zig:1539`)
...
### `parseFunctionBodyStatement` (`src/tests/parser.zig:1551`)
...
### `parseStrictFunctionBodyStatement` (`src/tests/parser.zig:1566`)
...
### `expectParseStatementError` (`src/tests/parser.zig:1580`)
...
### `readU32` (`src/tests/parser.zig:1598`)
...
### `readI32` (`src/tests/parser.zig:1602`)
...
### `readRelTarget32` (`src/tests/parser.zig:1606`)
...
### `countSubOpcode` (`src/tests/parser.zig:1619`)
...
### `subOpcodeOffset` (`src/tests/parser.zig:1636`)
...
### `countOpcode` (`src/tests/parser.zig:1648`)
...
### `firstOpcodeOffset` (`src/tests/parser.zig:1661`)
...
### `countSpecialObjectSubtype` (`src/tests/parser.zig:1673`)
...
### `countVarOpcodeForAtom` (`src/tests/parser.zig:1686`)
...
### `hasAnyOpcode` (`src/tests/parser.zig:1704`)
...
### `Audit.scanCode` (`src/tests/parser.zig:5848`)
...
### `Audit.scanFunction` (`src/tests/parser.zig:5904`)
...
### `tracePhase1ScopeEvents` (`src/tests/parser.zig:6720`)
...
### `expectPhase1ScopeEvents` (`src/tests/parser.zig:6741`)
...
### `findPhase1Opcode` (`src/tests/parser.zig:6751`)
...
### `countPhase1Opcode` (`src/tests/parser.zig:6763`)
...
### `fdPhase1Code` (`src/tests/parser.zig:6778`)
...
### `fdLabelOffset` (`src/tests/parser.zig:6787`)
...
### `publishPhase1Stream` (`src/tests/parser.zig:6794`)
...
- **实现**：Publish the parser's compact phase-1 stream onto `function` so the byte-sequence tests below can inspect it after the ParseState — which owns the Builder the parser emitted into — is torn down.。含循环。热路径用 `try` 传播分配/引擎错误。关键调用：`function.setCode`、`function.retainAtomOperand`、`function.appendSourceLoc`。
- **所有权 / 错误 / 调用**：返回 `!void`，由测试 `try`/`expectError` 消费。

### `parseRawStatement` (`src/tests/parser.zig:6803`)
...
### `parseRawExprWithRuntime` (`src/tests/parser.zig:6817`)
...
### `parseRawTSProgram` (`src/tests/parser.zig:6829`)
...
### `expectContinueTargetFollowsBodyLeave` (`src/tests/parser.zig:7225`)
...
### `compilePaddedDeclarationIndexCase` (`src/tests/parser.zig:7493`)
...
### `runParserDeclarationIndexOomRetry` (`src/tests/parser.zig:7624`)
...
### `Chain.end` (`src/tests/parser.zig:8926`)
...
### `Chain.containsName` (`src/tests/parser.zig:8937`)
...
### `countCalls` (`src/tests/parser.zig:9725`)
...
### `countFunctionClosures` (`src/tests/parser.zig:9733`)
...
### `functionBytecodeHasKind` (`src/tests/parser.zig:9840`)
...
### `functionBytecodeHasClosure` (`src/tests/parser.zig:9850`)
...
### `findFunctionBytecodeCapturingAtom` (`src/tests/parser.zig:9867`)
...
### `findFunctionCapturingAtom` (`src/tests/parser.zig:9884`)
...
### `functionHasClosure` (`src/tests/parser.zig:9898`)
...
### `expectFunctionClosureRecursive` (`src/tests/parser.zig:9912`)
...
### `functionHasKind` (`src/tests/parser.zig:9921`)
...
### `expectFunctionKindRecursive` (`src/tests/parser.zig:9930`)
...
### `expectAtomOperandName` (`src/tests/parser.zig:10019`)
...
### `atomLiveEntryTotal` (`src/tests/parser.zig:13092`)
...
### `phase_ownership.Baseline.capture` (`src/tests/parser.zig:13463`)
...
### `phase_ownership.pendingFixupCount` (`src/tests/parser.zig:13486`)
...
### `phase_ownership.formatHasLabelOperand` (`src/tests/parser.zig:13494`)
...
### `phase_ownership.censusCode` (`src/tests/parser.zig:13501`)
...
### `phase_ownership.attachedSourceCount` (`src/tests/parser.zig:13533`)
...
### `phase_ownership.builderRelocCount` (`src/tests/parser.zig:13543`)
...
### `phase_ownership.builderSourceCount` (`src/tests/parser.zig:13550`)
...
### `phase_ownership.Window.init` (`src/tests/parser.zig:13569`)
...
### `phase_ownership.Window.deinit` (`src/tests/parser.zig:13620`)
...
### `phase_ownership.Window.discardTemporaries` (`src/tests/parser.zig:13625`)
...
### `phase_ownership.Window.releaseArtifact` (`src/tests/parser.zig:13636`)
...
### `phase_ownership.Window.sampleB1` (`src/tests/parser.zig:13647`)
...
### `phase_ownership.Window.sampleNext` (`src/tests/parser.zig:13653`)

- **签名**：`pub fn sampleNext(self: *Window, phase: CodePhase, previous: *const Snapshot) !Snapshot`。
- **作用**：测试夹具/探针 `phase_ownership.Window.sampleNext`，给周围 `test` 块提供可注入行为或断言助手。
- **实现**：关键调用：`self.sample`。
- **所有权 / 错误 / 调用**：返回 `!Snapshot`，由测试 `try`/`expectError` 消费。

### `phase_ownership.Window.sample` (`src/tests/parser.zig:13657`)

- **签名**：`fn sample(self: *Window, phase: CodePhase, previous: ?*const Snapshot) !Snapshot`。
- **作用**：测试夹具/探针 `phase_ownership.Window.sample`，给周围 `test` 块提供可注入行为或断言助手。
- **实现**：按 `phase` 取码流普查：`.phase1` 时 relocation 还在 Builder 里，`label_markers` 用 `builderRelocCount` 代替；`.final` 时对 `self.function.code` 走 `censusCode`。再用 `pendingFixupCount` 取未回填数，算出 reloc 三元（`bound = created - fixups`、`discarded = b1_reloc_created - label_markers`、`outstanding = label_markers + fixups`）；builder 三元由 `alloc_calls + create_calls`、`free_calls + destroy_calls`、`allocation_count` 减基线得到（先断言三者都不低于基线）；source 三元按相位取 `builderSourceCount` 或 `function.source_loc_slots.len`，`attached` 在 final 相位走 `attachedSourceCount`，`committed` 取 `function.pc2line_buf.len`。`previous` 参数当前被 `_ =` 丢弃。主体是 `switch` 分发。热路径用 `try` 传播分配/引擎错误。关键调用：`builderRelocCount`、`censusCode`、`std.testing.expect`、`pendingFixupCount`、`builderSourceCount`、`attachedSourceCount`。
- **所有权 / 错误 / 调用**：返回 `!Snapshot`，由测试 `try`/`expectError` 消费。

### `phase_ownership.warmRuntime` (`src/tests/parser.zig:13729`)
...
### `phase_ownership.setBuilderCommitted` (`src/tests/parser.zig:13738`)
...
### `phase_ownership.expectCommon` (`src/tests/parser.zig:13744`)
...
### `phase_ownership.expectB1` (`src/tests/parser.zig:13755`)
...
### `phase_ownership.expectB3` (`src/tests/parser.zig:13764`)
...
### `phase_ownership.expectB4` (`src/tests/parser.zig:13773`)
...
### `phase_ownership.expectSourceFromB1` (`src/tests/parser.zig:13783`)
...
### `phase_ownership.expectTerminal` (`src/tests/parser.zig:13791`)
...
### `phase_ownership.publishedFunctionBytecodeCount` (`src/tests/parser.zig:13802`)
...
### `phase_ownership.dump` (`src/tests/parser.zig:13810`)
...
### `test "F1.5: every keyword token maps to its predefined atom"` (`src/tests/parser.zig:74`)
...
### `test "F1: of remains an identifier in ordinary lexing"` (`src/tests/parser.zig:139`)
...
### `test "F1: freeToken releases its identifier atom owner"` (`src/tests/parser.zig:152`)
...
### `test "F1: replacing a token releases its owner and stays safe on lexer error"` (`src/tests/parser.zig:164`)
...
### `test "F1: punctuators use raw ASCII for single-character tokens"` (`src/tests/parser.zig:182`)
...
### `test "F1: multi-character operator sequences land on TOK_* values"` (`src/tests/parser.zig:197`)
...
### `test "F1.2: numeric literals (decimal, hex, octal, binary, exponent, separators)"` (`src/tests/parser.zig:237`)
...
### `test "direct eval this is a scope_get_var against the caller seed"` (`src/tests/parser.zig:264`)
...
### `test "F1.2: numeric literals have no 128-byte length cap"` (`src/tests/parser.zig:289`)
...
### `test "F1.2: bigint suffix records is_bigint and source text"` (`src/tests/parser.zig:323`)
...
### `test "F1.2: string escapes (basic, hex, unicode short and braced, surrogate pair)"` (`src/tests/parser.zig:335`)
...
### `test "M3.1 F4: string lexer preserves lone surrogate escapes as code units"` (`src/tests/parser.zig:349`)
...
### `test "F1.2: line continuation in string and \\0 NUL escape"` (`src/tests/parser.zig:360`)
...
### `test "F1.2: legacy octal in strict mode is rejected"` (`src/tests/parser.zig:372`)
...
### `test "G1/P0: template legacy octal escapes mark cooked value invalid"` (`src/tests/parser.zig:382`)
...
### `test "F1.2: template head/middle/tail produce TemplatePart classification"` (`src/tests/parser.zig:402`)
...
### `test "F1.2: no-substitution template"` (`src/tests/parser.zig:434`)
...
### `test "G1/P0: template token keeps raw escape bytes"` (`src/tests/parser.zig:445`)
...
### `test "G1/P0: template token normalizes raw CR line terminators"` (`src/tests/parser.zig:457`)
...
### `test "F1.2: regex literal exposes pattern and flags"` (`src/tests/parser.zig:469`)
...
### `test "F1.2: regex literal may begin with equals after slash rescan"` (`src/tests/parser.zig:483`)
...
### `test "F1.3: private name keeps the # prefix in the atom"` (`src/tests/parser.zig:501`)
...
### `test "F1.3: unicode escape inside identifier is decoded into the atom"` (`src/tests/parser.zig:512`)
...
### `test "F1.3: escaped keyword spelling is treated as identifier (per spec)"` (`src/tests/parser.zig:524`)
...
### `test "F1.3: raw Unicode identifier start accepts ID_Start and rejects emoji"` (`src/tests/parser.zig:535`)
...
### `test "F1.4: got_lf is true after a LineTerminator and false otherwise"` (`src/tests/parser.zig:551`)
...
### `test "F1.4: line_num and col_num are 1-based"` (`src/tests/parser.zig:569`)
...
### `test "F1: end-to-end lex of a small program"` (`src/tests/parser.zig:587`)
...
### `test "F1: HTML comments are stripped in script mode but rejected in module mode"` (`src/tests/parser.zig:613`)
...
### `test "F1: hashbang at start of file is skipped, but not later"` (`src/tests/parser.zig:639`)
...
### `test "F1.5: keyword block atom layout matches quickjs-atom.h ordering"` (`src/tests/parser.zig:649`)
...
### `test "F1: Lexer enableTypeScript strips variable and function TypeScript annotations dynamically"` (`src/tests/parser.zig:709`)
...
### `test "parser accepts computed public class fields"` (`src/tests/parser.zig:1590`)
...
### `test "F4: number literal lowers to short integer form"` (`src/tests/parser.zig:1713`)
...
### `test "F4: number literal with non-integer value lowers to push_const"` (`src/tests/parser.zig:1724`)
...
### `test "F4: large bigint literal lowers to constant pool value"` (`src/tests/parser.zig:1737`)
...
### `test "W5: signed bigint-i32 neg matches QuickJS final bytecode boundaries"` (`src/tests/parser.zig:1750`)
...
### `test "W5: folded parenthesized bigint keeps the unary minus source position"` (`src/tests/parser.zig:1799`)
...
### `test "F4: regexp literal stores pattern then parse-time bytecode in the constant pool"` (`src/tests/parser.zig:1818`)
...
### `test "F4: regexp pattern constant decodes UTF-8 before the following constant"` (`src/tests/parser.zig:1867`)
...
### `test "F4: invalid regexp releases its published pattern constant"` (`src/tests/parser.zig:1923`)
...
### `test "F4: boolean and null literals"` (`src/tests/parser.zig:1937`)
...
### `test "F4: identifier reads global via get_var"` (`src/tests/parser.zig:1954`)
...
### `test "F4: parseExprBinary level 1 (mul/div/mod)"` (`src/tests/parser.zig:1967`)
...
### `test "F4: parseExprBinary level 2 (add/sub) is left-associative"` (`src/tests/parser.zig:1976`)
...
### `test "F4: precedence — multiplication before addition"` (`src/tests/parser.zig:1985`)
...
### `test "F4: parentheses override precedence"` (`src/tests/parser.zig:1994`)
...
### `test "F4: comparison operators map to op.lt/op.lte/op.eq/op.strict_eq"` (`src/tests/parser.zig:2003`)
...
### `test "F4: null comparison lowering keeps strict folds and loose equality distinct"` (`src/tests/parser.zig:2024`)
...
### `test "F4: bitwise levels 6/7/8 (and/xor/or) and shifts"` (`src/tests/parser.zig:2051`)
...
### `test "F4: unary +/-/~/! lower to plus/neg/not/lnot"` (`src/tests/parser.zig:2076`)
...
### `test "F4: typeof identifier uses get_var_undef + typeof"` (`src/tests/parser.zig:2127`)
...
### `test "F4: typeof optional chain parses full chain"` (`src/tests/parser.zig:2139`)
...
### `test "F4: typeof comparisons select final short tests at the condition boundary"` (`src/tests/parser.zig:2148`)
...
### `test "F4: numeric discard in void keeps only undefined"` (`src/tests/parser.zig:2171`)
...
### `test "M3.1 F4: strict eval and arguments update targets are rejected"` (`src/tests/parser.zig:2180`)
...
### `test "F4: power operator is right-associative"` (`src/tests/parser.zig:2188`)
...
### `test "F4: logical && uses dup + if_false short-circuit"` (`src/tests/parser.zig:2196`)
...
### `test "F4: logical || uses dup + if_true short-circuit"` (`src/tests/parser.zig:2207`)
...
### `test "F4: if consumes three-term logical chains with final branch snapshots"` (`src/tests/parser.zig:2218`)
...
### `test "F4: logical producer uses one source-less shared merge label"` (`src/tests/parser.zig:2259`)
...
### `test "F4: collapsed multiline logical branches retain source progression"` (`src/tests/parser.zig:2331`)
...
### `test "F4: nullish coalescing ?? uses is_undefined_or_null gate"` (`src/tests/parser.zig:2374`)
...
### `test "M3.1 F4: nullish coalescing chains and rejects direct logical mixing"` (`src/tests/parser.zig:2385`)
...
### `test "F4: discarded short-circuit with assignment RHS keeps function stack balanced"` (`src/tests/parser.zig:2398`)
...
### `test "F4: discarded conditional assignment arms keep function stack balanced"` (`src/tests/parser.zig:2424`)
...
### `test "F4: ternary cond ? a : b folds the fragment exit goto to its terminator"` (`src/tests/parser.zig:2441`)
...
### `test "F4: simple assignment x = 1 emits push ; dup ; put_var (KEEP_TOP)"` (`src/tests/parser.zig:2452`)
...
### `test "F4: function-local assignment result reaches dup put to set fold"` (`src/tests/parser.zig:2461`)
...
### `test "F4: compound assignment x += 1 emits get_var ; rhs ; add ; dup ; put_var"` (`src/tests/parser.zig:2474`)
...
### `test "F4: numeric discard in comma removes pure left and keeps right"` (`src/tests/parser.zig:2483`)
...
### `test "F4: member access a.b emits get_var + get_field"` (`src/tests/parser.zig:2492`)

- **签名**：无参数测试块，返回 `!void`。
- **作用**：钉住场景「F4: member access a.b emits get_var + get_field」。
- **实现**：`parseExpr(&env, "a.b")` 后 `expectOpcodeSequence` 比 `get_var_field ; get_field`（测试名里写的 `get_var` 现已是合并形式 `get_var_field`）。
- **所有权 / 错误 / 调用**：无跨测试状态；失败即测试失败，不把 JS 异常漏到下一例。

### `test "F4: index access a[i] emits get_var ; get_var ; get_array_el"` (`src/tests/parser.zig:2501`)
...
### `test "F4: nested assignment 1 + (a = b) preserves the leading push"` (`src/tests/parser.zig:2512`)
...
### `test "F4: string literal lowers to push_atom_value"` (`src/tests/parser.zig:2521`)
...
### `test "F4: empty string literal lowers to push_empty_string"` (`src/tests/parser.zig:2532`)
...
### `test "F4: array literal lowers to push elements ; array_from N"` (`src/tests/parser.zig:2542`)
...
### `test "F4: empty array literal emits array_from 0"` (`src/tests/parser.zig:2553`)
...
### `test "F4: trailing comma in array literal is allowed"` (`src/tests/parser.zig:2565`)
...
### `test "F4: two-slot object literal carries its allocation capacity hint"` (`src/tests/parser.zig:2576`)
...
### `test "F4: object literal capacity hint counts unique static slots only"` (`src/tests/parser.zig:2585`)
...
### `test "F4: empty object literal emits object"` (`src/tests/parser.zig:2615`)
...
### `test "F4: shorthand object property { x } emits get_var x ; define_field x"` (`src/tests/parser.zig:2628`)
...
### `test "M3.1 F4: computed object property emits define_array_el"` (`src/tests/parser.zig:2637`)
...
### `test "M3.1 F4: object spread emits copy_data_properties"` (`src/tests/parser.zig:2656`)
...
### `test "M3.1 F4: keyword object property names parse as literal keys"` (`src/tests/parser.zig:2666`)
...
### `test "M3.1 F4: object literal __proto__ emits set_proto"` (`src/tests/parser.zig:2676`)
...
### `test "M3.1 F4: object method shorthand emits define_method"` (`src/tests/parser.zig:2686`)
...
### `test "M3.1 F4: for-await close keeps body statement source location"` (`src/tests/parser.zig:2700`)
...
### `test "M3.1 F4: the program root's scope marker and source authority are stream events"` (`src/tests/parser.zig:2759`)
...
### `test "M3.1 F4: computed object method emits define_method_computed"` (`src/tests/parser.zig:2788`)
...
### `test "M3.1 F4: object string getter emits define_method getter flag"` (`src/tests/parser.zig:2798`)
...
### `test "M3.1 F4: object numeric setter emits define_method setter flag"` (`src/tests/parser.zig:2809`)
...
### `test "M3.1 F4: computed object getter emits define_method_computed getter flag"` (`src/tests/parser.zig:2820`)
...
### `test "M3.1 F4: computed object keys emit to_propkey before definition"` (`src/tests/parser.zig:2830`)
...
### `test "M3.1 F4: duplicate non-computed __proto__ data fields reject"` (`src/tests/parser.zig:2841`)
...
### `test "M3.1 F4: computed __proto__ duplicate is permitted"` (`src/tests/parser.zig:2848`)
...
### `test "M3.1 F4: computed object key accepts logical and assignment"` (`src/tests/parser.zig:2858`)
...
### `test "M3.1 F4: computed object key accepts logical or assignment"` (`src/tests/parser.zig:2868`)
...
### `test "M3.1 F4: computed object key accepts indexed logical assignment"` (`src/tests/parser.zig:2878`)
...
### `test "M3.1 F4: computed object key accepts nullish assignment"` (`src/tests/parser.zig:2889`)
...
### `test "F4: simple call f(a, b) emits get_var ; args ; call argc"` (`src/tests/parser.zig:2899`)
...
### `test "F4: zero-arg call f() emits call 0"` (`src/tests/parser.zig:2908`)
...
### `test "F4: method call obj.m(x) uses get_field2 + call_method"` (`src/tests/parser.zig:2917`)
...
### `test "F4: get_length final fold consumes the ordinary length atom operand"` (`src/tests/parser.zig:2928`)
...
### `test "F4: length call consumer preserves get_field2 and its atom operand"` (`src/tests/parser.zig:2938`)
...
### `test "F4: indexed call obj[k](x) uses get_array_el2 + call_method"` (`src/tests/parser.zig:2950`)
...
### `test "F4: new X(a) emits get_var X ; dup ; get_var a ; call_constructor 1"` (`src/tests/parser.zig:2960`)
...
### `test "F4: bare new X (no args) emits call_constructor 0"` (`src/tests/parser.zig:2970`)
...
### `test "F4: postfix x++ emits get_var ; post_inc ; put_var"` (`src/tests/parser.zig:2980`)
...
### `test "F4: postfix x-- emits get_var ; post_dec ; put_var"` (`src/tests/parser.zig:2989`)
...
### `test "F4: prefix ++x emits get_var ; inc ; dup ; put_var"` (`src/tests/parser.zig:2998`)
...
### `test "F4: prefix --x emits get_var ; dec ; dup ; put_var"` (`src/tests/parser.zig:3007`)
...
### `test "F4: delete unresolvable identifier emits delete_var"` (`src/tests/parser.zig:3016`)
...
### `test "F4: delete a.b emits get_var a ; push_atom_value b ; delete"` (`src/tests/parser.zig:3026`)
...
### `test "F4: delete of a private field is rejected after ordinary-field transport"` (`src/tests/parser.zig:3035`)
...
### `test "F4: delete a.b.length rewrites optimized length load"` (`src/tests/parser.zig:3045`)
...
### `test "F4: delete a[i] emits get_var a ; get_var i ; delete"` (`src/tests/parser.zig:3054`)
...
### `test "F4: delete on a non-reference yields drop ; push_true"` (`src/tests/parser.zig:3063`)
...
### `test "F4: chained call f(a)(b) emits two call ops"` (`src/tests/parser.zig:3074`)
...
### `test "F4: dotted assignment a.b = v emits get_var ; rhs ; insert2 ; put_field"` (`src/tests/parser.zig:3085`)
...
### `test "F4: indexed assignment a[i] = v emits get_var ; key ; rhs ; insert3 ; put_array_el"` (`src/tests/parser.zig:3094`)
...
### `test "F4: compound dotted assignment a.b += v rewrites get_field to get_field2"` (`src/tests/parser.zig:3103`)
...
### `test "F4: compound indexed assignment a[i] += v keeps QuickJS indexed lvalue shape"` (`src/tests/parser.zig:3112`)
...
### `test "F4: postfix dotted a.b++ emits get_field2 ; post_inc ; perm3 ; put_field"` (`src/tests/parser.zig:3121`)
...
### `test "F4: postfix indexed a[i]-- emits QuickJS indexed lvalue read ; post_dec ; perm4 ; put_array_el"` (`src/tests/parser.zig:3130`)
...
### `test "F4: prefix ++a.b emits get_field2 ; inc ; insert2 ; put_field"` (`src/tests/parser.zig:3139`)
...
### `test "F4: prefix --a[i] emits QuickJS indexed lvalue read ; dec ; insert3 ; put_array_el"` (`src/tests/parser.zig:3148`)
...
### `test "F4: final bytecode applies QuickJS discarded lvalue and loop update peepholes"` (`src/tests/parser.zig:3157`)
...
### `test "F5: local x++ and --x reach the canonical final inc_loc snapshots"` (`src/tests/parser.zig:3181`)
...
### `test "F4: dotted assign value remains on stack via insert2 (chained)"` (`src/tests/parser.zig:3199`)
...
### `test "F4: array hole [1, , 3] emits sparse define_field for present elements"` (`src/tests/parser.zig:3212`)
...
### `test "F4: leading hole [, 1] emits sparse define_field at index 1"` (`src/tests/parser.zig:3223`)
...
### `test "F4: consecutive holes [, , 1] emits sparse define_field at index 2"` (`src/tests/parser.zig:3234`)
...
### `test "F4: multi-level delete a.b.c rewrites only the last get_field"` (`src/tests/parser.zig:3245`)
...
### `test "F4: multi-level delete a.b[i] truncates the trailing get_array_el"` (`src/tests/parser.zig:3254`)
...
### `test "F4: delete on a postfix update result evaluates and returns true"` (`src/tests/parser.zig:3263`)
...
### `test "F4: optional chain a?.b emits inline chain_test + normal get_field"` (`src/tests/parser.zig:3281`)
...
### `test "F4: optional length value reaches the get_length final fold"` (`src/tests/parser.zig:3292`)
...
### `test "F4: optional length call consumer preserves get_field2 and its atom operand"` (`src/tests/parser.zig:3313`)
...
### `test "F4: optional chain a?.[i] emits inline chain_test + get_array_el"` (`src/tests/parser.zig:3337`)
...
### `test "F4: optional chain a?.b.c — chain test only at the ?. site"` (`src/tests/parser.zig:3346`)
...
### `test "F4: a?.b?.c emits two chain_tests sharing a common chain exit"` (`src/tests/parser.zig:3355`)
...
### `test "F4: optional call a?.() emits chain_test + plain call"` (`src/tests/parser.zig:3368`)
...
### `test "F4: method-on-opt-chain obj?.b(x) uses get_field2 + call_method"` (`src/tests/parser.zig:3377`)
...
### `test "F4: parenthesized optional member call preserves receiver"` (`src/tests/parser.zig:3387`)
...
### `test "F4: optional call after parenthesized optional member keeps balanced exits"` (`src/tests/parser.zig:3411`)
...
### `test "F4: optional chain accepts keyword property names"` (`src/tests/parser.zig:3440`)
...
### `test "F4: indexed-call-on-opt-chain obj?.[k](x) uses get_array_el2 + call_method"` (`src/tests/parser.zig:3456`)
...
### `test "call-site cache: every final call instruction carries a cache_idx and the function owns one slot per site"` (`src/tests/parser.zig:3500`)
...
### `test "call-site cache: sites past the 255-slot budget carry the no-cache index"` (`src/tests/parser.zig:3545`)
...
### `test "prop-site cache: every final field instruction carries a cache_idx and the function owns one slot per site"` (`src/tests/parser.zig:3580`)
...
### `test "prop-site cache: sites past the 255-slot budget carry the no-cache index"` (`src/tests/parser.zig:3627`)
...
### `test "F4: optional call without chain receiver a?.()(b) — chain only on first call"` (`src/tests/parser.zig:3672`)
...
### `test "F4: simple template with one substitution uses get_field2 concat + call_method"` (`src/tests/parser.zig:3704`)
...
### `test "F4: returned interpolated template carries QuickJS tail-call source provenance"` (`src/tests/parser.zig:3714`)
...
### `test "F4: template with two substitutions accumulates argc correctly"` (`src/tests/parser.zig:3758`)
...
### `test "F4: spread call f(...x) emits array_from + apply 0"` (`src/tests/parser.zig:3778`)
...
### `test "F4: mixed spread call f(a, ...b) starts array_from with leading count"` (`src/tests/parser.zig:3788`)
...
### `test "F4: trailing element after spread uses define_array_el + inc"` (`src/tests/parser.zig:3799`)
...
### `test "F4: method call with spread obj.m(...x) uses perm3 + apply 0"` (`src/tests/parser.zig:3821`)
...
### `test "F4: new with spread new X(...args) uses apply with is_new=1"` (`src/tests/parser.zig:3840`)
...
### `test "F4: array literal spread [...a] starts with array_from 0 + push_i32 0"` (`src/tests/parser.zig:3851`)
...
### `test "F4: array literal mixed spread [a, ...b, c] uses define_array_el+inc"` (`src/tests/parser.zig:3860`)
...
### `test "F4: template with empty middle still emits call_method with correct argc"` (`src/tests/parser.zig:3869`)
...
### `test "F5: empty statement"` (`src/tests/parser.zig:3888`)
...
### `test "F5: block statement folds only the final expression discard"` (`src/tests/parser.zig:3897`)
...
### `test "F5: return statement without value"` (`src/tests/parser.zig:3906`)
...
### `test "F5: return statement with value"` (`src/tests/parser.zig:3916`)
...
### `test "F5: return comma and conditional expressions follow terminal goto folding"` (`src/tests/parser.zig:3925`)
...
### `test "F5: return conditional expression folds the then goto to a plain return"` (`src/tests/parser.zig:3947`)
...
### `test "F5: strict return call folds to tail_call plus return; sloppy stays call"` (`src/tests/parser.zig:3960`)
...
### `test "F5: return method call folds to tail_call_method plus return"` (`src/tests/parser.zig:3980`)
...
### `test "F5: throw statement"` (`src/tests/parser.zig:4001`)
...
### `test "F5: if statement without else"` (`src/tests/parser.zig:4010`)
...
### `test "F5: constant test conditions reach the final QuickJS fold"` (`src/tests/parser.zig:4021`)
...
### `test "W5: constant tests prune only the QuickJS-selected control-flow arm"` (`src/tests/parser.zig:4043`)
...
### `test "F5: if statement with else"` (`src/tests/parser.zig:4087`)
...
### `test "W5: final branches normalize at QuickJS instruction boundaries"` (`src/tests/parser.zig:4098`)
...
### `test "W5: production with atom-label target threads past destructuring fallback goto"` (`src/tests/parser.zig:4159`)
...
### `test "F5: while statement"` (`src/tests/parser.zig:4211`)
...
### `test "F5: do-while statement"` (`src/tests/parser.zig:4227`)
...
### `test "F5: for update moves optional chain code with atom operands"` (`src/tests/parser.zig:4237`)
...
### `test "F5: final expression statement discard folds into the terminator"` (`src/tests/parser.zig:4246`)
...
### `test "W5: numeric discarded immediates respect statement and completion boundaries"` (`src/tests/parser.zig:4255`)
...
### `test "W5: tagged-int numeric strings use cpool without changing other string producers"` (`src/tests/parser.zig:4350`)
...
### `test "W5: string discard follows QuickJS atom and completion boundaries"` (`src/tests/parser.zig:4436`)
...
### `test "F5: labelled break crossing switch drops discriminant"` (`src/tests/parser.zig:4565`)
...
### `test "F5: switch CaseBlock does not treat a function expression name as a declaration"` (`src/tests/parser.zig:4579`)
...
### `test "F5: labelled break to loop inside switch keeps discriminant stack balanced"` (`src/tests/parser.zig:4597`)
...
### `test "F5: labelled continue inside switch case keeps discriminant stack balanced"` (`src/tests/parser.zig:4628`)
...
### `test "F5: labelled continue from nested loop inside switch drops discriminant once"` (`src/tests/parser.zig:4645`)
...
### `test "F5: var declaration without initializer"` (`src/tests/parser.zig:4666`)
...
### `test "F5: var declaration with initializer"` (`src/tests/parser.zig:4677`)
...
### `test "F5: sloppy var initializer captures dynamic reference before RHS"` (`src/tests/parser.zig:4692`)
...
### `test "F5: destructuring dynamic reference publishes an exact long-tail label"` (`src/tests/parser.zig:4744`)
...
### `test "F5: module-ref var initializer consumes value unless next statement reuses binding"` (`src/tests/parser.zig:4809`)
...
### `test "F5: module-ref var initializer preserves value for immediate same-name expression"` (`src/tests/parser.zig:4820`)
...
### `test "F5: let declaration"` (`src/tests/parser.zig:4831`)
...
### `test "F5: let declaration with initializer"` (`src/tests/parser.zig:4847`)
...
### `test "F5: const declaration without initializer should fail"` (`src/tests/parser.zig:4860`)
...
### `test "F5: const declaration with initializer"` (`src/tests/parser.zig:4866`)
...
### `test "F5: multiple var declarations"` (`src/tests/parser.zig:4881`)
...
### `test "F5: directive prologue with 'use strict'"` (`src/tests/parser.zig:4897`)
...
### `test "M3.1 F4: strict object setter rejects eval and arguments parameters"` (`src/tests/parser.zig:4908`)
...
### `test "F5: directive prologue with multiple directives"` (`src/tests/parser.zig:4916`)
...
### `test "F5: directive prologue with ASI"` (`src/tests/parser.zig:4927`)
...
### `test "F6: simple function declaration"` (`src/tests/parser.zig:4940`)
...
### `test "F6: line_num before explicit return does not add implicit return"` (`src/tests/parser.zig:4954`)
...
### `test "F6: function declaration with parameters"` (`src/tests/parser.zig:4970`)
...
### `test "F6: var redeclaration of parameter keeps closure bound to arg"` (`src/tests/parser.zig:4985`)
...
### `test "F6: function declaration with rest parameter"` (`src/tests/parser.zig:4995`)
...
### `test "F6: arrow function with block body"` (`src/tests/parser.zig:5010`)
...
### `test "F6: arrow function with expression body"` (`src/tests/parser.zig:5023`)
...
### `test "F6: arrow function with single parameter"` (`src/tests/parser.zig:5037`)
...
### `test "F6: identifier arrow lookahead preserves trivia and line terminators"` (`src/tests/parser.zig:5052`)
...
### `test "F6: parenthesized arrow lookahead skips only context-free source"` (`src/tests/parser.zig:5100`)
...
### `test "F6: assignment-entry arrow dispatch preserves primary-expression boundaries"` (`src/tests/parser.zig:5162`)
...
### `test "F6: arrow function with multiple parameters"` (`src/tests/parser.zig:5196`)
...
### `test "F6: arrow function with rest parameter"` (`src/tests/parser.zig:5214`)
...
### `test "F6: function with object destructuring parameter"` (`src/tests/parser.zig:5229`)
...
### `test "F6: function with array destructuring parameter"` (`src/tests/parser.zig:5243`)
...
### `test "F6: arrow function with object destructuring parameter"` (`src/tests/parser.zig:5257`)
...
### `test "F6: arrow function with array destructuring parameter"` (`src/tests/parser.zig:5272`)
...
### `test "F6: direct shorthand destructuring bindings use get_field2"` (`src/tests/parser.zig:5287`)
...
### `test "F7: class with constructor"` (`src/tests/parser.zig:5300`)
...
### `test "F7: default class constructors use canonical entry gates"` (`src/tests/parser.zig:5317`)
...
### `test "F7: static field initializer is a synthetic child called with the class receiver"` (`src/tests/parser.zig:5336`)
...
### `test "F7: static field direct eval uses the synthetic child lexical scope"` (`src/tests/parser.zig:5391`)
...
### `test "F7: computed static fields and blocks share one ordered synthetic initializer"` (`src/tests/parser.zig:5421`)
...
### `test "F7: class with getter"` (`src/tests/parser.zig:5496`)
...
### `test "F7: class with setter"` (`src/tests/parser.zig:5511`)
...
### `test "F7: class field ASI before generator method"` (`src/tests/parser.zig:5526`)
...
### `test "F7: super keyword in class method"` (`src/tests/parser.zig:5561`)
...
### `test "F7: super property access"` (`src/tests/parser.zig:5573`)
...
### `test "F7: super() constructor call"` (`src/tests/parser.zig:5585`)
...
### `test "F7: explicit derived this uses local check and return fallback uses caller check"` (`src/tests/parser.zig:5602`)
...
### `test "F7: super() rejected in base constructor"` (`src/tests/parser.zig:5617`)
...
### `test "F9: yield expression"` (`src/tests/parser.zig:5624`)
...
### `test "W5: generator parameter boundary emits initial_yield in scripts and modules"` (`src/tests/parser.zig:5637`)
...
### `test "F9: yield* expression"` (`src/tests/parser.zig:5674`)
...
### `test "F8: export default statement"` (`src/tests/parser.zig:5689`)
...
### `test "F8: export named statement"` (`src/tests/parser.zig:5700`)
...
### `test "F7: private field in class"` (`src/tests/parser.zig:5712`)
...
### `test "assignment inferred class name is embedded before static initialization"` (`src/tests/parser.zig:5723`)
...
### `test "computed property class name is embedded before static initialization"` (`src/tests/parser.zig:5739`)
...
### `test "class field inferred names are embedded before static initialization"` (`src/tests/parser.zig:5755`)
...
### `test "comma expression does not infer an anonymous class name"` (`src/tests/parser.zig:5772`)
...
### `test "inferred function names do not become named-expression self bindings"` (`src/tests/parser.zig:5792`)
...
### `test "W1d: finalized private operations have no raw private atom operands"` (`src/tests/parser.zig:5810`)
...
### `test "F7: private method in class"` (`src/tests/parser.zig:5928`)
...
### `test "F7: private getter in class"` (`src/tests/parser.zig:5939`)
...
### `test "F7: private setter in class"` (`src/tests/parser.zig:5950`)
...
### `test "F7: paired private accessor shares one instance brand prologue"` (`src/tests/parser.zig:5961`)

- **签名**：无参数测试块，返回 `!void`。
- **作用**：钉住场景「F7: paired private accessor shares one instance brand prologue」。
- **实现**：用 `ParserTestEnv` 编同名私有 getter+setter 成对的 `class C { get #x() {...} set #x(value) {...} }`；断言仍只是存在性三查（`op.define_class` 顶层、`op.set_home_object`/`op.add_brand` 递归），不数 `add_brand` 出现次数。
- **所有权 / 错误 / 调用**：无跨测试状态；失败即测试失败，不把 JS 异常漏到下一例。

### `test "F7: private name in uses scope temp before resolver"` (`src/tests/parser.zig:5972`)
...
### `test "unresolved descendant lookup threads direct eval var objects inside-out"` (`src/tests/parser.zig:6000`)
...
### `test "direct eval pseudo var objects follow eval and parameter-expression gates"` (`src/tests/parser.zig:6066`)
...
### `test "parameter pre-scan balances regexp and template delimiters like QuickJS"` (`src/tests/parser.zig:6119`)
...
### `test "parameter initializer direct eval emits active global-declaration carriers"` (`src/tests/parser.zig:6141`)
...
### `test "nested direct eval does not capture a parent global declaration carrier"` (`src/tests/parser.zig:6182`)
...
### `test "F7: class with extends (derived constructor)"` (`src/tests/parser.zig:6205`)
...
### `test "F7: class without extends (base constructor)"` (`src/tests/parser.zig:6215`)
...
### `test "F8: basic import statement"` (`src/tests/parser.zig:6225`)
...
### `test "F8: side-effect import"` (`src/tests/parser.zig:6237`)
...
### `test "F8: named imports"` (`src/tests/parser.zig:6248`)
...
### `test "F8: renamed imports"` (`src/tests/parser.zig:6261`)
...
### `test "F8: namespace import"` (`src/tests/parser.zig:6274`)
...
### `test "F8: mixed import"` (`src/tests/parser.zig:6286`)
...
### `test "F8: export named"` (`src/tests/parser.zig:6299`)
...
### `test "F8: export renamed"` (`src/tests/parser.zig:6311`)
...
### `test "F8: export default expression"` (`src/tests/parser.zig:6323`)
...
### `test "F8: export default function"` (`src/tests/parser.zig:6337`)
...
### `test "anonymous default function uses a star-default global function carrier"` (`src/tests/parser.zig:6348`)
...
### `test "F8: export default class"` (`src/tests/parser.zig:6386`)
...
### `test "anonymous default class uses the lexical star-default carrier"` (`src/tests/parser.zig:6400`)
...
### `test "F8: export star"` (`src/tests/parser.zig:6414`)
...
### `test "F8: export star as namespace"` (`src/tests/parser.zig:6426`)
...
### `test "F8: export from"` (`src/tests/parser.zig:6439`)
...
### `test "F8: export var"` (`src/tests/parser.zig:6452`)

- **签名**：无参数测试块，返回 `!void`。
- **作用**：钉住场景「F8: export var」。
- **实现**：`parseModuleStatement` 编的源文其实是 `export const x = 1`（测试名里的 var 与源文不一致），计数 `(0, 0, 1, 0, 0)`，唯一导出 `x`←`x`。
- **所有权 / 错误 / 调用**：无跨测试状态；失败即测试失败，不把 JS 异常漏到下一例。

### `test "F8: export function"` (`src/tests/parser.zig:6463`)
...
### `test "F8: export class"` (`src/tests/parser.zig:6474`)
...
### `test "F9: async function expression"` (`src/tests/parser.zig:6487`)
...
### `test "F9: async arrow function"` (`src/tests/parser.zig:6499`)
...
### `test "F9: async function declaration"` (`src/tests/parser.zig:6511`)
...
### `test "F9: async function declaration with parameters"` (`src/tests/parser.zig:6523`)
...
### `test "F9: async function declaration with body"` (`src/tests/parser.zig:6536`)
...
### `test "F9: yield outside generator error"` (`src/tests/parser.zig:6548`)
...
### `test "F9: await outside async function error"` (`src/tests/parser.zig:6555`)
...
### `test "F9: await inside async function no error"` (`src/tests/parser.zig:6562`)
...
### `test "Object literal: computed property name"` (`src/tests/parser.zig:6576`)
...
### `test "Object literal: method shorthand"` (`src/tests/parser.zig:6587`)
...
### `test "Object literal: spread"` (`src/tests/parser.zig:6597`)
...
### `test "Object literal: get/set shorthand is accepted"` (`src/tests/parser.zig:6607`)
...
### `test "async arrow accepts sloppy context-keyword binding identifiers"` (`src/tests/parser.zig:6627`)
...
### `test "for-in/of var binding rejects yield in generators and await in async"` (`src/tests/parser.zig:6658`)
...
### `test "switch case after while-family still emits a fallthrough skip-goto"` (`src/tests/parser.zig:6676`)
...
### `test "escapedIdentifier reserved-word CurrentContext shares the Binding walk"` (`src/tests/parser.zig:6839`)
...
### `test "emitterOpU16 and NoSource share the u16 opcode walk"` (`src/tests/parser.zig:6855`)
...
### `test "emitterPushConst and Owned share the cpool patch walk"` (`src/tests/parser.zig:6871`)
...
### `test "emitterOp NoSource and At share the plain opcode walk"` (`src/tests/parser.zig:6888`)
...
### `test "emitScope var wrappers keep phase-1 opcode pairs"` (`src/tests/parser.zig:6904`)
...
### `test "M-SCOPE event producers: ordinary scopes match QuickJS phase-1 events"` (`src/tests/parser.zig:6930`)
...
### `test "M-SCOPE event producers: switch with and class layers are eventful"` (`src/tests/parser.zig:6996`)
...
### `test "M-SCOPE event producers: catch binding wrapper and body leave in LIFO order"` (`src/tests/parser.zig:7051`)
...
### `test "M-SCOPE event producers: structural body and namespace scopes stay identity-only"` (`src/tests/parser.zig:7096`)
...
### `test "M-SCOPE abrupt control: labelled break and continue close nested scopes at the source"` (`src/tests/parser.zig:7165`)
...
### `test "M-SCOPE abrupt control: classic and for-of continue targets follow the body leave"` (`src/tests/parser.zig:7202`)
...
### `test "M-SCOPE abrupt control: crossed finally sees scope leaves before gosub"` (`src/tests/parser.zig:7257`)
...
### `test "M-SCOPE negative contract: return cleanup and throw synthesize no scope leave"` (`src/tests/parser.zig:7282`)
...
### `test "F10.1a FunctionDef: program root has var scope 0 and body scope 1"` (`src/tests/parser.zig:7328`)
...
### `test "F10.1a FunctionDef: QuickJS root declaration rows keep body and block origins"` (`src/tests/parser.zig:7350`)
...
### `test "F10.1a FunctionDef: function vars retain parser origins without entering lexical chains"` (`src/tests/parser.zig:7384`)
...
### `test "F10.1a FunctionDef: every parsed function body has identity except class fields aggregator"` (`src/tests/parser.zig:7420`)
...
### `test "defineVar core matches pinned QuickJS declaration collision matrix"` (`src/tests/parser.zig:7453`)
...
### `test "parser declaration index preserves unique and duplicate same-scope lexical declarations"` (`src/tests/parser.zig:7521`)
...
### `test "parser declaration index preserves shadowing and catch plus two scopes"` (`src/tests/parser.zig:7537`)
...
### `test "parser declaration index preserves function-var ancestor and sibling origins"` (`src/tests/parser.zig:7558`)
...
### `test "parser declaration index rebuilds after bypassed linked and function-var writers"` (`src/tests/parser.zig:7574`)
...
### `test "parser declaration index activation is retryable across allocation failures"` (`src/tests/parser.zig:7684`)
...
### `test "F10.1a FunctionDef: empty ordinary block does not create a scope"` (`src/tests/parser.zig:7695`)
...
### `test "F10.1a FunctionDef: non-empty ordinary block pushes and pops one scope"` (`src/tests/parser.zig:7715`)
...
### `test "F10.1a FunctionDef: nested blocks build parent chain"` (`src/tests/parser.zig:7732`)
...
### `test "F10.1a FunctionDef: nested scope inherits the visible lexical head"` (`src/tests/parser.zig:7755`)
...
### `test "F10.1a FunctionDef: let registers as lexical, non-const"` (`src/tests/parser.zig:7779`)
...
### `test "F10.1a FunctionDef: const registers as lexical + const"` (`src/tests/parser.zig:7805`)
...
### `test "F10.1a FunctionDef: top-level block var registers as global var"` (`src/tests/parser.zig:7823`)
...
### `test "F10.1a FunctionDef: let in nested block attaches to inner scope"` (`src/tests/parser.zig:7844`)
...
### `test "F10.1a FunctionDef: simple catch binding keeps catch provenance"` (`src/tests/parser.zig:7864`)
...
### `test "F10.1a FunctionDef: catch has binding wrapper and body scopes"` (`src/tests/parser.zig:7891`)
...
### `test "F10.1a FunctionDef: for-of lexical head owns one binding"` (`src/tests/parser.zig:7921`)
...
### `test "F10.1a FunctionDef: assignment for-of still owns a head scope"` (`src/tests/parser.zig:7964`)
...
### `test "F10.1a FunctionDef: if statement owns one wrapper scope"` (`src/tests/parser.zig:7984`)
...
### `test "F10.1a FunctionDef: classic for always owns a head scope"` (`src/tests/parser.zig:8001`)
...
### `test "F10.1a FunctionDef: with scope emits its enter event"` (`src/tests/parser.zig:8018`)
...
### `test "F10.1a FunctionDef: class has name and private scopes"` (`src/tests/parser.zig:8048`)
...
### `test "F10.1a FunctionDef: findVar locates by name"` (`src/tests/parser.zig:8076`)
...
### `test "F10.1b Nested function: cur_func stack management"` (`src/tests/parser.zig:8099`)
...
### `test "nested function declarations fit the QuickJS native parser stack budget"` (`src/tests/parser.zig:8123`)
...
### `test "function expressions preserve closure operands across constant index 255"` (`src/tests/parser.zig:8145`)
...
### `test "QuickJS hoist metadata keeps only the final body local function initializer"` (`src/tests/parser.zig:8183`)
...
### `test "QuickJS hoist metadata keeps only the final parameter function initializer"` (`src/tests/parser.zig:8210`)
...
### `test "QuickJS block function metadata does not also use the body prologue fallback"` (`src/tests/parser.zig:8237`)
...
### `test "QuickJS final linkage rebuild includes implicit arguments by scope level"` (`src/tests/parser.zig:8258`)
...
### `test "QuickJS add_eval_variables stages pseudo locals in VarDef append order"` (`src/tests/parser.zig:8283`)
...
### `test "QuickJS direct eval arguments pseudo is distinct from a simple formal"` (`src/tests/parser.zig:8349`)
...
### `test "QuickJS entry contract carries grammar while bindings live in vardefs"` (`src/tests/parser.zig:8384`)
...
### `test "QuickJS parameter expression scope initializes lexical TDZ on entry"` (`src/tests/parser.zig:8449`)
...
### `test "QuickJS global declaration carriers precede child finalization"` (`src/tests/parser.zig:8472`)
...
### `test "dynamic global writes keep put_var distinct from plain var-ref stores"` (`src/tests/parser.zig:8506`)
...
### `test "QuickJS open binding indices follow child capture demand order"` (`src/tests/parser.zig:8587`)
...
### `test "dead scope refs do not capture across a live merge"` (`src/tests/parser.zig:8636`)
...
### `test "QuickJS postorder capture topology records exact forwarding rows"` (`src/tests/parser.zig:8677`)
...
### `test "QuickJS postorder capture topology follows lexical scope order"` (`src/tests/parser.zig:8715`)
...
### `test "QuickJS direct eval capture prefix preserves shadowed binding identities"` (`src/tests/parser.zig:8749`)
...
### `test "QuickJS direct eval capture prefix follows lexical scope order"` (`src/tests/parser.zig:8790`)
...
### `test "QuickJS eval prefix is stable before descendant capture demand"` (`src/tests/parser.zig:8830`)
...
### `test "QuickJS eval root appends child and own ordinary globals after declarations"` (`src/tests/parser.zig:8864`)
...
### `test "final eval operands address compact vardef chains"` (`src/tests/parser.zig:8895`)
...
### `test "final eval marker is combined and belongs only to the eval unit"` (`src/tests/parser.zig:8967`)
...
### `test "direct eval capture hints preserve the former parameter flag bit as scope data"` (`src/tests/parser.zig:8994`)
...
### `test "QuickJS direct eval captures only loop bindings live at the call site"` (`src/tests/parser.zig:9040`)
...
### `test "QuickJS class private direct eval has complete capture events"` (`src/tests/parser.zig:9074`)
...
### `test "QuickJS module closure order keeps all imports before global declarations"` (`src/tests/parser.zig:9091`)
...
### `test "QuickJS module declarations append without parser closure remapping"` (`src/tests/parser.zig:9116`)
...
### `test "QuickJS parent module declarations exist before child direct-eval seeding"` (`src/tests/parser.zig:9190`)
...
### `test "QuickJS module instantiation guard separates function hoists from the body"` (`src/tests/parser.zig:9224`)
...
### `test "QuickJS module instantiation guard excludes frame lexical preparation"` (`src/tests/parser.zig:9262`)
...
### `test "QuickJS module callback captures keep parent declaration indices"` (`src/tests/parser.zig:9283`)
...
### `test "QuickJS global eval capture stays distinct from appended declaration carrier"` (`src/tests/parser.zig:9320`)
...
### `test "QuickJS script global functions publish from bytecode through the first declaration carrier"` (`src/tests/parser.zig:9345`)
...
### `test "QuickJS direct eval hoist target walk distinguishes closure var-object and lexical conflict"` (`src/tests/parser.zig:9383`)
...
### `test "F10.1c Nested function: bytecode dual-buffering"` (`src/tests/parser.zig:9435`)
...
### `test "TS: Class Constructor Parameter Properties"` (`src/tests/parser.zig:9465`)
...
### `test "TS: Enum Declarations"` (`src/tests/parser.zig:9478`)
...
### `test "TS: Const Enum Declarations Lower As Runtime Enums"` (`src/tests/parser.zig:9494`)
...
### `test "TS: Nested Generic Greater Tokens"` (`src/tests/parser.zig:9508`)
...
### `test "TS: Const Type Parameters"` (`src/tests/parser.zig:9529`)
...
### `test "TS: Function Overload Signatures Are Skipped"` (`src/tests/parser.zig:9543`)
...
### `test "TS: Class Method Overload Signatures Are Skipped"` (`src/tests/parser.zig:9562`)
...
### `test "TS: Generic Arrow Type Parameters Are Skipped"` (`src/tests/parser.zig:9577`)
...
### `test "TS: Inline Object Type Parameter Constraints Are Skipped"` (`src/tests/parser.zig:9591`)
...
### `test "TS: Unsupported Syntax Scan Reports Feature And Position"` (`src/tests/parser.zig:9606`)
...
### `test "TS: Namespaces"` (`src/tests/parser.zig:9627`)
...
### `test "TS: Nested Constructor Block Parameter Re-emission"` (`src/tests/parser.zig:9643`)
...
### `test "TS: Derived Constructor Parameter Properties post-super"` (`src/tests/parser.zig:9660`)
...
### `test "TS: Namespace Scope Isolation"` (`src/tests/parser.zig:9674`)
...
### `test "TS: Dotted Namespaces"` (`src/tests/parser.zig:9687`)
...
### `test "TS: Strict Enum Constant Expression Rejection"` (`src/tests/parser.zig:9699`)
...
### `test "try finally parses one shared finalizer body for every abrupt exit"` (`src/tests/parser.zig:9737`)
...
### `test "try catch fixed topology removes calls to its empty finalizer"` (`src/tests/parser.zig:9791`)
...
### `test "empty finally producer reaches the phase2 and phase3 cascade"` (`src/tests/parser.zig:9809`)
...
### `test "arrow lexical this and new.target are ordinary closure captures"` (`src/tests/parser.zig:9934`)
...
### `test "arrow super property captures lexical this through an ordinary cell"` (`src/tests/parser.zig:9961`)
...
### `test "arrow super call captures active constructor state through ordinary cells"` (`src/tests/parser.zig:9988`)
...
### `test "syntax error deinit balances empty message allocation"` (`src/tests/parser.zig:10040`)
...
### `test "source positions and syntax errors carry filename line and column"` (`src/tests/parser.zig:10051`)
...
### `test "compile syntax errors report the failing token position"` (`src/tests/parser.zig:10065`)
...
### `test "expectToken syntax errors name the expected and actual token kinds"` (`src/tests/parser.zig:10089`)
...
### `test "parser error long tail names the unexpected source token"` (`src/tests/parser.zig:10101`)
...
### `test "parser error long tail names binding pattern and module tokens"` (`src/tests/parser.zig:10127`)
...
### `test "parser error long tail closes semantic and lookahead diagnostics"` (`src/tests/parser.zig:10153`)
...
### `test "parser source-reachable invariant masks carry specific diagnostics"` (`src/tests/parser.zig:10183`)
...
### `test "lexer syntax errors retain the failing token position"` (`src/tests/parser.zig:10230`)
...
### `test "direct eval propagates script or module identity without changing display filename"` (`src/tests/parser.zig:10242`)
...
### `test "script parse mode emits bytecode metadata without AST execution"` (`src/tests/parser.zig:10283`)
...
### `test "production root modes end in visible return opcodes"` (`src/tests/parser.zig:10299`)
...
### `test "ordinary script compile publishes one canonical function bytecode root"` (`src/tests/parser.zig:10341`)
...
### `test "canonical root ownership moves out of parser Result exactly once"` (`src/tests/parser.zig:10359`)
...
### `test "canonical module artifact ownership moves out of parser Result exactly once"` (`src/tests/parser.zig:10384`)
...
### `test "implicit arguments always resolves before final global var opcodes"` (`src/tests/parser.zig:10414`)
...
### `test "canonical root and child independently keep their compile realm alive"` (`src/tests/parser.zig:10471`)
...
### `test "runtime strict compile policy is published by root and child finalizers"` (`src/tests/parser.zig:10506`)
...
### `test "module compile publishes one canonical function bytecode plus metadata artifact"` (`src/tests/parser.zig:10532`)
...
### `test "module nested function independently keeps its compile realm alive"` (`src/tests/parser.zig:10548`)
...
### `test "canonical root and child survive parser arena release allocation churn and GC"` (`src/tests/parser.zig:10582`)
...
### `test "root strictness comes from directives or host options, never source comments"` (`src/tests/parser.zig:10650`)
...
### `test "eval type owns var and lexical declaration carriers like pinned QuickJS"` (`src/tests/parser.zig:10682`)
...
### `test "ordinary block string literal is not a function-body directive"` (`src/tests/parser.zig:10744`)
...
### `test "function body declarations preserve QuickJS source VarDef order"` (`src/tests/parser.zig:10760`)
...
### `test "body var discovery does not cross arrow or class method boundaries"` (`src/tests/parser.zig:10778`)
...
### `test "generic for-of accepts complete parenthesized and indexed member targets"` (`src/tests/parser.zig:10798`)
...
### `test "for-of contextual async lookahead follows QuickJS grammar"` (`src/tests/parser.zig:10817`)
...
### `test "generic for-of parses computed target exactly once in source order"` (`src/tests/parser.zig:10847`)
...
### `test "for statement dispatch only scans top-level semicolons"` (`src/tests/parser.zig:10864`)
...
### `test "for-in-of keeps Annex B call targets and rejects other invalid assignment targets"` (`src/tests/parser.zig:10886`)
...
### `test "script top-level lexical captured before declaration uses QuickJS global op"` (`src/tests/parser.zig:10909`)
...
### `test "captured reads encode lexical TDZ in the final var-ref opcode"` (`src/tests/parser.zig:10930`)
...
### `test "named function self-binding writes do not reach var-ref stores"` (`src/tests/parser.zig:10960`)
...
### `test "final bytecode authorizes plain var-ref stores before execution"` (`src/tests/parser.zig:11017`)
...
### `test "assignment target scan ignores atom operand bytes"` (`src/tests/parser.zig:11244`)
...
### `test "print calls emit global lookup generic call and receiver-preserving property call bytecode"` (`src/tests/parser.zig:11269`)
...
### `test "simple variable assignments emit var bytecode"` (`src/tests/parser.zig:11305`)
...
### `test "quick parser emits compound assignment and update statements"` (`src/tests/parser.zig:11323`)
...
### `test "add_loc finalization accepts only QuickJS RHS producers"` (`src/tests/parser.zig:11338`)
...
### `test "add_loc finalization attributes a multiline local RHS to the operator"` (`src/tests/parser.zig:11431`)
...
### `test "quick parser emits arithmetic compound assignment operators"` (`src/tests/parser.zig:11472`)
...
### `test "quick parser does not claim update expression values"` (`src/tests/parser.zig:11486`)
...
### `test "quick parser emits basic array and object literals"` (`src/tests/parser.zig:11496`)
...
### `test "quick parser emits object property assignment"` (`src/tests/parser.zig:11514`)
...
### `test "quick parser emits optional property access for object and nullish bases"` (`src/tests/parser.zig:11529`)
...
### `test "quick parser preserves parenthesized postfix bases"` (`src/tests/parser.zig:11542`)
...
### `test "quick parser keeps conditional member callee branches at one stack slot"` (`src/tests/parser.zig:11559`)
...
### `test "quick parser retrofits forward var captures into nested closures"` (`src/tests/parser.zig:11585`)
...
### `test "quick parser still promotes unconditional parenthesized member calls"` (`src/tests/parser.zig:11613`)
...
### `test "call consumers use final-op provenance for eval with super and comma tags"` (`src/tests/parser.zig:11625`)
...
### `test "quick parser lowers JSON stringify and parse to transitional JSON bytecode"` (`src/tests/parser.zig:11683`)
...
### `test "quick parser lowers Math calls to transitional Math bytecode"` (`src/tests/parser.zig:11694`)
...
### `test "quick parser lowers URI calls to transitional URI bytecode"` (`src/tests/parser.zig:11705`)
...
### `test "quick parser lowers Number parse helpers to transitional number bytecode"` (`src/tests/parser.zig:11716`)
...
### `test "quick parser lowers supported Date helpers to receiver-preserving property calls"` (`src/tests/parser.zig:11731`)
...
### `test "quick parser lowers supported RegExp helpers to receiver-preserving property calls"` (`src/tests/parser.zig:11748`)
...
### `test "RegExp property calls keep QuickJS call_method bytecode"` (`src/tests/parser.zig:11764`)
...
### `test "function predeclare scan skips slash-equals regexp literals"` (`src/tests/parser.zig:11791`)
...
### `test "quick parser lowers supported Promise helpers to receiver-preserving property calls"` (`src/tests/parser.zig:11813`)
...
### `test "quick parser lowers supported collection helpers to receiver-preserving property calls"` (`src/tests/parser.zig:11837`)
...
### `test "template interpolation emits string concatenation"` (`src/tests/parser.zig:11875`)
...
### `test "simple arrays emit receiver-preserving property calls"` (`src/tests/parser.zig:11889`)
...
### `test "simple functions and arrows emit inline helper bytecode"` (`src/tests/parser.zig:11907`)
...
### `test "unsupported spread call reports syntax guard"` (`src/tests/parser.zig:11930`)
...
### `test "test262 frontmatter does not affect quick parser behavior"` (`src/tests/parser.zig:11940`)
...
### `test "test262 prelude frontmatter parses nested private methods after line_num temp"` (`src/tests/parser.zig:11963`)
...
### `test "arrow early errors reject non-simple strict and invalid rest parameters"` (`src/tests/parser.zig:12008`)
...
### `test "strict parameter binding names follow directive and method grammar"` (`src/tests/parser.zig:12029`)
...
### `test "retroactive strict arrow parameter errors retain the binding position"` (`src/tests/parser.zig:12074`)
...
### `test "arrow early error checks do not reject valid nested rest destructuring"` (`src/tests/parser.zig:12101`)
...
### `test "destructuring rest parameter defaults enforce await and yield early errors"` (`src/tests/parser.zig:12117`)
...
### `test "assignment destructuring early errors reject invalid rest forms"` (`src/tests/parser.zig:12136`)
...
### `test "assignment destructuring early errors allow reserved property names"` (`src/tests/parser.zig:12183`)
...
### `test "assignment early errors reject invalid assignment target types"` (`src/tests/parser.zig:12206`)
...
### `test "async arrow early errors reject await-context parse negatives"` (`src/tests/parser.zig:12224`)
...
### `test "object computed property names parse async arrow and module await expressions"` (`src/tests/parser.zig:12241`)
...
### `test "class early errors reject class parse negatives"` (`src/tests/parser.zig:12270`)
...
### `test "module parse mode records import export metadata and strict flag"` (`src/tests/parser.zig:12291`)
...
### `test "module parser preserves regex literals across zod-like lookahead scans"` (`src/tests/parser.zig:12340`)
...
### `test "parser rescans divide-assign token as regex literal beginning with equals"` (`src/tests/parser.zig:12389`)
...
### `test "module import local names are compiled as module var refs"` (`src/tests/parser.zig:12420`)
...
### `test "module parser rejects duplicate exported names across export forms"` (`src/tests/parser.zig:12452`)
...
### `test "module parser validates local export bindings after full body parse"` (`src/tests/parser.zig:12469`)
...
### `test "module parser rejects duplicate import attribute keys per with clause"` (`src/tests/parser.zig:12495`)
...
### `test "module parser accepts empty side-effect import attributes"` (`src/tests/parser.zig:12519`)
...
### `test "module parser validates string module export names"` (`src/tests/parser.zig:12531`)
...
### `test "module namespace metadata is syntax-driven when the imported name is star"` (`src/tests/parser.zig:12552`)
...
### `test "module parser rejects comma expression as default export expression"` (`src/tests/parser.zig:12577`)
...
### `test "module parser accepts keyword module export and import names"` (`src/tests/parser.zig:12590`)
...
### `test "module parser allows duplicate top-level var declarations"` (`src/tests/parser.zig:12607`)
...
### `test "module parser hoists block var declarations to module var refs"` (`src/tests/parser.zig:12617`)
...
### `test "direct eval closure seed lowers unresolved read to var ref"` (`src/tests/parser.zig:12637`)
...
### `test "direct eval rebuilds private grammar bindings from ordered closure rows"` (`src/tests/parser.zig:12673`)
...
### `test "only direct eval enables private grammar from closure seeds"` (`src/tests/parser.zig:12735`)
...
### `test "direct eval ref closure seed preserves table identity only"` (`src/tests/parser.zig:12758`)
...
### `test "parameter direct eval keeps arg var object ahead of declaration globals"` (`src/tests/parser.zig:12785`)
...
### `test "QuickJS direct eval destructuring declares through the variable object"` (`src/tests/parser.zig:12817`)
...
### `test "parser accepts dynamic import call expressions"` (`src/tests/parser.zig:12873`)
...
### `test "dynamic import arguments do not leak anonymous function named evaluation"` (`src/tests/parser.zig:12902`)
...
### `test "parser rejects invalid dynamic import call syntax"` (`src/tests/parser.zig:12918`)
...
### `test "module parser accepts default as explicit namespace export name"` (`src/tests/parser.zig:12931`)
...
### `test "eval function class private destructuring spread async generator features are recorded"` (`src/tests/parser.zig:12945`)
...
### `test "bytecode constants retain values through Phase 4 structures"` (`src/tests/parser.zig:12981`)
...
### `test "flow-tail summary: label/patch/move/truncate corpus compiles under the Debug oracle"` (`src/tests/parser.zig:13005`)

- **签名**：无参数测试块，返回 `!void`。
- **作用**：钉住场景「flow-tail summary: label/patch/move/truncate corpus compiles under the Debug oracle」。
- **实现**：直接 `JSRuntime.create` 建裸 Runtime（不经 TestEngine）。断言 1 处 `std.testing.expect*`。约 1 个 Zig expect、0 个 JS `assert.*`。
- **所有权 / 错误 / 调用**：测试持有 Runtime/Context 所有权，`defer destroy/deinit`；GC 对象靠 root frame 或精确扫描。

### `test "QCP-1 S2P: v2 veneer emits through the FunctionDef builder and deinit releases it"` (`src/tests/parser.zig:13050`)
...
### `test "parser releases identifier and private-name token atoms"` (`src/tests/parser.zig:13098`)
...
### `test "parser releases module and import-attribute token atoms"` (`src/tests/parser.zig:13161`)
...
### `test "parser returns the atom table to balance across every token-bearing construct"` (`src/tests/parser.zig:13198`)
...
### `test "four-ledger phase-boundary ownership accounting parse-only"` (`src/tests/parser.zig:13847`)
