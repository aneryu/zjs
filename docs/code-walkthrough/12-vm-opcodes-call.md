...
### `CallDepthGuard.deinit` (`src/exec/vm_opcodes.zig:46`)
...
### `enterCallDepth` (`src/exec/vm_opcodes.zig:55`)
...
### `bytecodeFrameAllocaSize` (`src/exec/vm_opcodes.zig:79`)
...
### `bytecodeLeafFrameAllocaSize` (`src/exec/vm_opcodes.zig:101`)
...
### `bytecodeStackBudgetWouldOverflow` (`src/exec/vm_opcodes.zig:109`)
...
### `admissionCeilingsReject` (`src/exec/vm_opcodes.zig:122`)
...
### `canEnterInlineCallDepthBytes` (`src/exec/vm_opcodes.zig:140`)
...
### `commitInlineCallDepthBytes` (`src/exec/vm_opcodes.zig:149`)
...
### `tryCommitInlineCallDepthBytesRt` (`src/exec/vm_opcodes.zig:169`)
...
### `retreatInlineCallDepthBytesMiss` (`src/exec/vm_opcodes.zig:191`)
...
### `enterInlineCallDepthBytes` (`src/exec/vm_opcodes.zig:200`)
...
### `leaveInlineCallDepthBytes` (`src/exec/vm_opcodes.zig:211`)
...
### `leaveInlineCallDepthBytesRt` (`src/exec/vm_opcodes.zig:221`)
...
### `checkTailCallChainStackBudget` (`src/exec/vm_opcodes.zig:236`)
...
### `inlineCallDepthOverflow` (`src/exec/vm_opcodes.zig:254`)
...
### `initFrameLocals` (`src/exec/vm_opcodes.zig:259`)
...
### `initFrameVarRefs` (`src/exec/vm_opcodes.zig:286`)
...
### `allocFrameVarRefWindow` (`src/exec/vm_opcodes.zig:320`)
...
### `closure` (`src/exec/vm_opcodes.zig:327`)
...
### `call` (`src/exec/vm_opcodes.zig:352`)
...
### `resolvedNativeCallTargetAssumeCFunction` (`src/exec/vm_opcodes.zig:385`)
...
### `resolvedNativeMethodRecord` (`src/exec/vm_opcodes.zig:399`)
...
### `resolvedNativeMethodRecordAssumeCFunction` (`src/exec/vm_opcodes.zig:408`)
...
### `callResolvedNativeMethod` (`src/exec/vm_opcodes.zig:424`)
...
### `callMethod` (`src/exec/vm_opcodes.zig:449`)
...
### `dropUnusedCallResult` (`src/exec/vm_opcodes.zig:531`)
...
### `fastNativeMethodCall` (`src/exec/vm_opcodes.zig:542`)
...
### `apply` (`src/exec/vm_opcodes.zig:594`)
...
### `constructor` (`src/exec/vm_opcodes.zig:735`)
...
### `throwCtorTypeError` (`src/exec/vm_opcodes.zig:774`)
...
### `checkCtor` (`src/exec/vm_opcodes.zig:779`)
...
### `checkCtorVm` (`src/exec/vm_opcodes.zig:785`)
...
### `checkCtorReturn` (`src/exec/vm_opcodes.zig:800`)
...
### `checkCtorReturnVm` (`src/exec/vm_opcodes.zig:815`)
...
### `initCtor` (`src/exec/vm_opcodes.zig:830`)
...
### `initCtorVm` (`src/exec/vm_opcodes.zig:859`)
...
### `maxNativeJsCallDepth` (`src/exec/vm_opcodes.zig:875`)
...
### `maxLogicalJsCallDepth` (`src/exec/vm_opcodes.zig:879`)
...
### `readInt` (`src/exec/vm_opcodes.zig:883`)

- **签名**：`fn readInt(comptime T: type, bytes: []const u8) T`。
- **作用**：读 argc / closure 索引。
- **实现**：小端。
- **所有权 / 错误 / 调用**：纯读：`bytes` 是 `function.byteCode()` 的借用切片，函数不分配、不持有、无 error set；`bytes[0..@sizeOf(T)]` 的长度由**调用点**的定长切片保证（越界是 panic，不是错误返回）。文件私有，调用方全在本文件的操作数解码点：`src/exec/vm_opcodes.zig:428`、`:548`、`:692` 等 5 处。同名副本另见 `vm_control` / `vm_literal` / `vm_property` / `vm_value` / `vm_eval_module`，各文件一份。
