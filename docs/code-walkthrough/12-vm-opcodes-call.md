...
- **签名**：`pub inline fn initFrameVarRefs( ctx: *core.JSContext, global: *core.Object, function: *const bytecode.FunctionBytecode, frame: *frame_mod.Frame, var_refs: []const *core.VarRef, use_inline_storage: bool, windows: frame_mod.FrameStorageWindows, ) !void`。
- **作用**：安装闭包 cell 窗口。
- **实现**：传入 `var_refs` 非空：指针拷贝（qjs JS_CLOSURE_REF，17322）。空但有 `closureVar`：仅 legacy adapter 可无 closure2 捕获，否则 `InvalidBytecode`；逐个 `legacyInitialClosureVarRef`。`varRefNamesLen` 路径（顶层脚本）：legacy only。global_decl：PASS1 前造 TDZ placeholder（不能先建 lexicals cell，否则自见重声明）；否则 alias `globalLexicalCell` 或 `createClosed` 普通全局。
- **所有权 / 错误 / 调用**：窗口来自 carve 或 `allocFrameVarRefWindow`。define_var 稍后把 placeholder 换成真正 VARREF。

### `allocFrameVarRefWindow` (`src/exec/vm_call.zig:386`)
...
### `legacyInitialClosureVarRef` (`src/exec/vm_call.zig:393`)

- **签名**：`fn legacyInitialClosureVarRef(ctx: *core.JSContext, global: *core.Object, cv: bytecode.function_bytecode.BytecodeClosureVar) !*core.VarRef`。
- **作用**：W1e legacy 模块/夹具：按 closure 类型造初始 cell。
- **实现**：global/global_ref/global_decl → `selectOrdinaryGlobalClosureCell`。其它：module_* 未初始化；local/arg/ref 取 lexical 或 global 属性。`createClosed`，填 const/lexical/function_name 标志。
- **所有权 / 错误 / 调用**：`InvalidBytecode` 若 cell 句柄不是 VarRef。

### `closure` (`src/exec/vm_call.zig:415`)
...
### `resolvedNativeCallTargetAssumeCFunction` (`src/exec/vm_call.zig:473`)
...
### `resolvedNativeMethodRecord` (`src/exec/vm_call.zig:487`)
...
### `resolvedNativeMethodRecordAssumeCFunction` (`src/exec/vm_call.zig:496`)
...
### `callResolvedNativeMethod` (`src/exec/vm_call.zig:512`)
...
### `callMethod` (`src/exec/vm_call.zig:537`)
...
### `dropUnusedCallResult` (`src/exec/vm_call.zig:619`)
...
### `fastNativeMethodCall` (`src/exec/vm_call.zig:630`)
...
### `apply` (`src/exec/vm_call.zig:682`)
...
### `constructor` (`src/exec/vm_call.zig:823`)
...
### `throwCtorTypeError` (`src/exec/vm_call.zig:862`)
...
### `checkCtor` (`src/exec/vm_call.zig:867`)
...
### `checkCtorVm` (`src/exec/vm_call.zig:873`)
...
### `checkCtorReturn` (`src/exec/vm_call.zig:888`)
...
### `checkCtorReturnVm` (`src/exec/vm_call.zig:903`)
...
### `initCtor` (`src/exec/vm_call.zig:918`)
...
### `initCtorVm` (`src/exec/vm_call.zig:947`)
...
### `maxNativeJsCallDepth` (`src/exec/vm_call.zig:963`)
...
### `maxLogicalJsCallDepth` (`src/exec/vm_call.zig:967`)
...
### `readInt` (`src/exec/vm_call.zig:971`)
