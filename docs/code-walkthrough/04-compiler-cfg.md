...
### `canonicalIdentityAtOffset` (`src/compiler/cfg.zig:694`)
...
- **所有权 / 错误 / 调用**：每条引用 O(log binds)。它是本文件唯一的 canonical 身份入口——过去还有一个按 label_index 线性找行的 `canonicalBoundaryIdentity`，生产零调用方，已删，那条「同边界的别名折叠、不同边界必须区分」的 RULING 注释也搬到了本函数头上。

### `tempFromHeader` (`src/compiler/cfg.zig:708`)
...
### `tempAtomInstructionSize` (`src/compiler/cfg.zig:717`)
...
### `formatHasAtom` (`src/compiler/cfg.zig:738`)
...
### `formRowIndex` (`src/compiler/cfg.zig:842`)
...
### `tempInstruction` (`src/compiler/cfg.zig:908`)
...
### `phase1Instruction` (`src/compiler/cfg.zig:924`)
...
### `isUnconditionalTerminal` (`src/compiler/cfg.zig:935`)
...
### `isContinuationOp` (`src/compiler/cfg.zig:952`)
...
### `labelOperandOffset` (`src/compiler/cfg.zig:970`)
...
### `validateAndAdvanceAtom` (`src/compiler/cfg.zig:977`)
...
### `readLabelIndex` (`src/compiler/cfg.zig:992`)
...
### `labelTargetOffset` (`src/compiler/cfg.zig:1010`)
...
### `isEmptyGosub` (`src/compiler/cfg.zig:1021`)
...
### `Graph.deinit` (`src/compiler/cfg.zig:1045`)
...
### `Graph.edgesForBlock` (`src/compiler/cfg.zig:1058`)
...
### `Graph.isReachable` (`src/compiler/cfg.zig:1066`)
...
### `bitWordCount` (`src/compiler/cfg.zig:1074`)
...
### `bitIsSet` (`src/compiler/cfg.zig:1080`)
...
### `setBit` (`src/compiler/cfg.zig:1085`)
...
### `findBlockStart` (`src/compiler/cfg.zig:1090`)
...
### `appendEdge` (`src/compiler/cfg.zig:1104`)
...
### `validateBindIndex` (`src/compiler/cfg.zig:1110`)
...
- **实现**：计数 + 单调扫描。
- **所有权 / 错误 / 调用**：`build` 入口；同时证明 `canonicalIdentityAtOffset` 会把同偏移的别名折叠成同一身份，而不同边界保持区分。

### `indexedControlKind` (`src/compiler/cfg.zig:1136`)
...
### `validateControlIndex` (`src/compiler/cfg.zig:1145`)
...
### `relocOpcodeOffset` (`src/compiler/cfg.zig:1174`)
...
### `indexedRelocOpcodeValid` (`src/compiler/cfg.zig:1190`)
...
### `buildFromControlIndex` (`src/compiler/cfg.zig:1205`)
...
### `build` (`src/compiler/cfg.zig:1359`)
...
### `enqueueAuditOffset` (`src/compiler/cfg.zig:1519`)
...
### `auditInstructionOwnership` (`src/compiler/cfg.zig:1542`)
...
### `validateBoundaryAuditInput` (`src/compiler/cfg.zig:1648`)
...
### `resolveDivergenceOrigin` (`src/compiler/cfg.zig:1691`)
...
### `formatDivergenceOrigin` (`src/compiler/cfg.zig:1739`)
...
### `writeReportIdentity` (`src/compiler/cfg.zig:1783`)
...
### `panicBoundaryUniquenessViolation` (`src/compiler/cfg.zig:1792`)
...
### `boundaryViolation` (`src/compiler/cfg.zig:1818`)
...
### `aliasGroupSplit` (`src/compiler/cfg.zig:1853`)
...
### `duplicateReplacementOwner` (`src/compiler/cfg.zig:1885`)
...
### `optimizationBoundaryLessThan` (`src/compiler/cfg.zig:1898`)
...
### `bindInsideFold` (`src/compiler/cfg.zig:1915`)
...
### `referenceToRetiredAlias` (`src/compiler/cfg.zig:1935`)
...
### `survivingSibling` (`src/compiler/cfg.zig:1962`)
...
### `isInstructionStart` (`src/compiler/cfg.zig:1978`)
...
### `blockContainingOffset` (`src/compiler/cfg.zig:1982`)
...
### `referencingInstructionIsLive` (`src/compiler/cfg.zig:1998`)
...
### `roleForLabelOperand` (`src/compiler/cfg.zig:2007`)
...
### `referenceChainDepth` (`src/compiler/cfg.zig:2038`)
...
### `censusAdd` (`src/compiler/cfg.zig:2062`)
...
### `reportAdd` (`src/compiler/cfg.zig:2066`)
...
### `recordDiffBucket` (`src/compiler/cfg.zig:2071`)
...
### `oracleReportSnapshot` (`src/compiler/cfg.zig:2076`)
...
### `resetOracleReport` (`src/compiler/cfg.zig:2122`)
...
### `BoundaryReportAccounting.claim` (`src/compiler/cfg.zig:2178`)
...
### `censusMax` (`src/compiler/cfg.zig:2209`)
...
### `recordFanout` (`src/compiler/cfg.zig:2213`)
...
### `recordChainDepth` (`src/compiler/cfg.zig:2224`)
...
### `recordFinalSourceCensus` (`src/compiler/cfg.zig:2235`)
...
### `recordFinalBoundaryHops` (`src/compiler/cfg.zig:2247`)
...
### `fixedPointHundred` (`src/compiler/cfg.zig:2252`)
...
### `histogramP95Index` (`src/compiler/cfg.zig:2258`)
...
### `writeHistogramP95` (`src/compiler/cfg.zig:2271`)
...
### `formatIdentityHealth` (`src/compiler/cfg.zig:2298`)
...
### `formatOracleReport` (`src/compiler/cfg.zig:2336`)
...
### `boundaryProductOffset` (`src/compiler/cfg.zig:2397`)
...
### `boundaryFirstLabel` (`src/compiler/cfg.zig:2412`)
...
### `nearestSourceSite` (`src/compiler/cfg.zig:2427`)
...
### `opAt` (`src/compiler/cfg.zig:2439`)
...
### `classifyAnchorSplits` (`src/compiler/cfg.zig:2456`)
...
### `hasSourceEventAt` (`src/compiler/cfg.zig:2633`)
...
### `boundaryFanoutAt` (`src/compiler/cfg.zig:2643`)
...
### `boundaryRoleMask` (`src/compiler/cfg.zig:2652`)
...
### `auditBoundaryUniqueness` (`src/compiler/cfg.zig:2668`)
...
### `AnchorSplitFixture.init` (`src/compiler/cfg.zig:3399`)
...
### `AnchorSplitFixture.deinit` (`src/compiler/cfg.zig:3417`)
...
### `AnchorSplitFixture.boundary` (`src/compiler/cfg.zig:3423`)
...
### `SourcePoint.eql` (`src/compiler/cfg.zig:3789`)
