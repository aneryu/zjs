...
### `isTypeScriptPath` (`src/lexer.zig:1776`)
...
### `shouldStrip` (`src/lexer.zig:1783`)
...
### `TSToken.text` (`src/lexer.zig:1805`)
...
### `markTypeRanges` (`src/lexer.zig:1810`)
...
### `findUnsupportedTypeScriptSyntax` (`src/lexer.zig:1839`)
...
### `unsupportedSyntaxAt` (`src/lexer.zig:1887`)
...
### `tsSkipLineComment` (`src/lexer.zig:1958`)
...
### `tsSkipBlockComment` (`src/lexer.zig:1964`)
...
### `tsSkipQuoted` (`src/lexer.zig:1970`)
...
### `tsSkipTemplate` (`src/lexer.zig:1989`)
...
### `tsSkipRegExp` (`src/lexer.zig:2007`)
...
### `tsSkipNumber` (`src/lexer.zig:2039`)
...
### `tsPunctuatorLen` (`src/lexer.zig:2052`)
...
### `tsCanStartRegExp` (`src/lexer.zig:2065`)
...
### `markTypeOnlyStatements` (`src/lexer.zig:2084`)
...
### `addDeclareRange` (`src/lexer.zig:2126`)
...
### `addInterfaceRange` (`src/lexer.zig:2155`)
...
### `looksLikeTypeAlias` (`src/lexer.zig:2179`)
...
### `markMixedTypeSpecifiers` (`src/lexer.zig:2198`)
...
### `markClassAndTypeModifiers` (`src/lexer.zig:2243`)
...
### `modifierCanAppearsHere` (`src/lexer.zig:2295`)
...
### `isTsModifier` (`src/lexer.zig:2305`)
...
### `findImplementsClassBrace` (`src/lexer.zig:2310`)
...
### `markImplementsClauses` (`src/lexer.zig:2332`)
...
### `markFunctionOverloadSignatures` (`src/lexer.zig:2347`)
...
### `markClassMethodOverloadSignatures` (`src/lexer.zig:2405`)
...
### `findClassBodyOpen` (`src/lexer.zig:2438`)
...
### `skipClassMemberSeparators` (`src/lexer.zig:2474`)
...
### `parseClassMethodSignature` (`src/lexer.zig:2480`)
...
### `isClassMethodModifierAt` (`src/lexer.zig:2529`)
...
### `hasFollowingClassMethodImplementation` (`src/lexer.zig:2538`)
...
### `sameTokenText` (`src/lexer.zig:2552`)
...
### `nextClassMemberStart` (`src/lexer.zig:2556`)
...
### `markTypeParameters` (`src/lexer.zig:2590`)
...
### `looksLikeTypeParameterStart` (`src/lexer.zig:2607`)
...
### `markTypeAnnotations` (`src/lexer.zig:2618`)
...
### `isTypeAnnotationColon` (`src/lexer.zig:2635`)
...
### `isParameterList` (`src/lexer.zig:2663`)
...
### `parameterListOwnerIndex` (`src/lexer.zig:2680`)
...
### `findTypeAngleStartBackward` (`src/lexer.zig:2691`)
...
### `leadingGreaterCount` (`src/lexer.zig:2708`)
...
### `returnTypeAfterParameterListLeadsToBody` (`src/lexer.zig:2714`)
...
### `isControlKeyword` (`src/lexer.zig:2720`)
...
### `isVariableDeclarationKeyword` (`src/lexer.zig:2725`)
...
### `isVariableDeclarationType` (`src/lexer.zig:2729`)
...
### `classFieldSegmentAllowsType` (`src/lexer.zig:2822`)
...
### `markTypeAssertions` (`src/lexer.zig:2839`)
...
### `markNonNullAssertions` (`src/lexer.zig:2856`)
...
### `isTypeAssertionOperator` (`src/lexer.zig:2875`)
...
### `previousTokenCanEndExpression` (`src/lexer.zig:2887`)
...
### `identifierCanEndExpression` (`src/lexer.zig:2898`)
...
### `findTypeEnd` (`src/lexer.zig:2912`)
...
### `findTypeAssertionEnd` (`src/lexer.zig:2949`)
...
### `isExpressionDelimiter` (`src/lexer.zig:2979`)
...
### `isValidTypeParameterList` (`src/lexer.zig:2988`)
...
### `typeParameterListCanBeFollowedBy` (`src/lexer.zig:3028`)
...
### `findTypeAngleEnd` (`src/lexer.zig:3036`)
...
### `startsWithGreater` (`src/lexer.zig:3076`)
...
### `consumeTypeAngleClosers` (`src/lexer.zig:3080`)
...
### `findStatementEnd` (`src/lexer.zig:3091`)
...
### `continuesAcrossLine` (`src/lexer.zig:3112`)
...
### `findMatchingForward` (`src/lexer.zig:3118`)
...
### `findEnclosingOpen` (`src/lexer.zig:3133`)
...
### `braceBelongsToClass` (`src/lexer.zig:3155`)
...
### `hasUnmatchedTernaryQuestionBefore` (`src/lexer.zig:3168`)
...
### `insideImportOrExportStatement` (`src/lexer.zig:3195`)
...
### `isStatementStart` (`src/lexer.zig:3230`)
...
### `findTokenBeforeOffset` (`src/lexer.zig:3236`)
...
### `hasLineBreakBetween` (`src/lexer.zig:3244`)
...
### `rangeLessThan` (`src/lexer.zig:3257`)
...
### `tokenTextEql` (`src/lexer.zig:3261`)
...
### `tsIsIdentStart` (`src/lexer.zig:3269`)
...
### `tsIsIdentContinue` (`src/lexer.zig:3273`)
