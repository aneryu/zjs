...
- **所有权 / 错误 / 调用**：不分配。`pos` 是调用方局部变量，不是 `LexerImpl.pos`。`simpleNextIsArrowNoLineTerminator`（`src/lexer.zig:316`）用 `true` 只认 `.arrow` / `.unsupported`。`import_keyword` / `export_keyword` 仍由本扫描器产出，目前没有 CLI 消费者。无 error set。
