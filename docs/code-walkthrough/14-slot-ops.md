...
[`src/exec/slot_ops.zig`](../../src/exec/slot_ops.zig) 给 VM 与 call runtime 共用：`get/put/set_loc`、`get/put/set_arg`、`get/put/set_var_ref`，以及 `VarRef` 单元格的边界视图。槽类型已翻成「每个 `frame.var_refs[i]` 都是活 cell」（对照 qjs `JSVarRef **var_refs`）。
...
### `execGetLoc` (`src/exec/slot_ops.zig:26`)
...
### `execPutLoc` (`src/exec/slot_ops.zig:45`)
...
### `execSetLoc` (`src/exec/slot_ops.zig:59`)
...
### `execGetArg` (`src/exec/slot_ops.zig:75`)
...
### `execPutArg` (`src/exec/slot_ops.zig:93`)
...
### `execSetArg` (`src/exec/slot_ops.zig:107`)
...
### `execGetVarRefMaybeTdz` (`src/exec/slot_ops.zig:122`)
...
### `execPutVarRef` (`src/exec/slot_ops.zig:192`)
...
### `isVarRefInitOpcode` (`src/exec/slot_ops.zig:242`)
...
### `constVarRefWriteAllowed` (`src/exec/slot_ops.zig:251`)
...
### `execSetVarRef` (`src/exec/slot_ops.zig:256`)
...
### `adapterValueBorrow` (`src/exec/slot_ops.zig:271`)
...
### `adapterValueIsUninitialized` (`src/exec/slot_ops.zig:284`)
...
### `replaceAdapterOwned` (`src/exec/slot_ops.zig:301`)
...
### `varRefCellFromValue` (`src/exec/slot_ops.zig:321`)
...
### `varRefSlotCell` (`src/exec/slot_ops.zig:336`)
...
### `varRefSlot` (`src/exec/slot_ops.zig:342`)
...
### `storeVarRefSlot` (`src/exec/slot_ops.zig:352`)
...
### `replaceVarRefValueOwned` (`src/exec/slot_ops.zig:360`)
