...
### `globalVarAtom` (`src/exec/vm_property.zig:41`)
...
### `varRefReadableBorrowed` (`src/exec/vm_property.zig:47`)
...
### `fastInstalledGlobalDataValueForAtomAtPc` (`src/exec/vm_property.zig:59`)
...
### `hasObjectBinding` (`src/exec/vm_property.zig:75`)
...
### `canUseFastGlobalVarLookup` (`src/exec/vm_property.zig:88`)
...
### `canUseInstalledGlobalDataIc` (`src/exec/vm_property.zig:98`)

- **签名**：`pub fn canUseInstalledGlobalDataIc( ctx: *core.JSContext, function: *const bytecode.FunctionBytecode, atom_id: core.Atom, frame: *const frame_mod.Frame, ) bool`。
- **作用**：比 lookup 门更严：还要排除 `ctx.lexicals` 自有绑定。
- **实现**：同样排除 undefined/arguments 与 var-ref 名；`ctx.lexicals.hasOwnProperty(atom_id)` 则 false。（原先还带一个只被 `_ =` 丢弃的 `global: *const core.Object` 形参，已删。）
- **所有权 / 错误 / 调用**：无。IC 安装/命中前。调用：同文件 `fastInstalledGlobalDataValueForAtomAtPc`（`:88`）。

### `functionFrameBindingShadowsGlobal` (`src/exec/vm_property.zig:112`)
...
### `functionHasDynamicScopeBindings` (`src/exec/vm_property.zig:119`)
...
### `canFuseGlobalDataWrite` (`src/exec/vm_property.zig:138`)
...
### `frameHasVarRefBinding` (`src/exec/vm_property.zig:148`)
