...
### `IteratorZipCompletion.initNormal` (`src/exec/iterator_ops.zig:1528`)
...
### `IteratorZipCompletion.initThrow` (`src/exec/iterator_ops.zig:1532`)
...
### `IteratorZipCompletion.capture` (`src/exec/iterator_ops.zig:1538`)
...
### `IteratorZipCompletion.restore` (`src/exec/iterator_ops.zig:1546`)
...
### `IteratorZipCompletion.deinit` (`src/exec/iterator_ops.zig:1551`)
...
### `iteratorZipCall` (`src/exec/iterator_ops.zig:1566`)
...
### `iteratorZipModeFromOptions` (`src/exec/iterator_ops.zig:1678`)
...
### `iteratorZipCollectIndexed` (`src/exec/iterator_ops.zig:1697`)
...
### `iteratorZipCollectKeyed` (`src/exec/iterator_ops.zig:1787`)
...
### `iteratorZipFlattenableRecord` (`src/exec/iterator_ops.zig:1850`)
...
- **所有权 / 错误 / 调用**：返回的 `IteratorZipRecord` 两个字段都是值（iterator 可能就是入参自身），所有权交给调用方，由它 `iteratorZipStoreIndex` 存进 iters/nexts；本函数不建根——注意它会调用户 `@@iterator` 与 Get `next`，中间产生的 iterator 只靠调用方的 root frame 与实参保活。error set：非对象、`@@iterator` 不可调用、调用结果非对象给 `error.TypeError`，其余是用户代码透传。调用方 `iterator_ops.zig:1734`（indexed 收集）与 `:1823`（keyed 收集）；`array_ops` 里那个零调用方的同名转发壳（alias wall 残留）已删。

### `iteratorZipNextMethod` (`src/exec/iterator_ops.zig:1872`)
...
### `iteratorZipCreateHelper` (`src/exec/iterator_ops.zig:1887`)
...
### `iteratorZipStoreIndex` (`src/exec/iterator_ops.zig:1940`)
...
### `iteratorZipGetIndex` (`src/exec/iterator_ops.zig:2010`)
...
### `iteratorZipSetIndex` (`src/exec/iterator_ops.zig:2014`)
...
### `iteratorZipCloseWithCompletion` (`src/exec/iterator_ops.zig:2023`)
...
### `iteratorZipCloseAllWithCompletion` (`src/exec/iterator_ops.zig:2042`)
...
### `iteratorZipClose` (`src/exec/iterator_ops.zig:2062`)
...
### `iteratorZipCloseAllAndPropagate` (`src/exec/iterator_ops.zig:2086`)
...
### `iteratorCloseWithCompletionAndPropagate` (`src/exec/iterator_ops.zig:2133`)
...
### `iteratorHelperCloseWithCompletionAndPropagate` (`src/exec/iterator_ops.zig:2149`)
...
### `iteratorPrototypeMethodCall` (`src/exec/iterator_ops.zig:2169`)
...
### `iteratorDisposeCall` (`src/exec/iterator_ops.zig:2196`)
...
### `iteratorToArrayCall` (`src/exec/iterator_ops.zig:2212`)
...
### `iteratorPredicateCall` (`src/exec/iterator_ops.zig:2247`)
...
### `iteratorReduceCall` (`src/exec/iterator_ops.zig:2316`)
...
### `iteratorStepFromNextResult` (`src/exec/iterator_ops.zig:2376`)
...
### `iteratorStepWithNext` (`src/exec/iterator_ops.zig:2411`)
...
### `iteratorStepWithSyncCall` (`src/exec/iterator_ops.zig:2424`)
...
### `iteratorStepWithSyncValues` (`src/exec/iterator_ops.zig:2436`)
...
### `iteratorCreateCallbackHelper` (`src/exec/iterator_ops.zig:2457`)
...
### `iteratorCreateLimitHelper` (`src/exec/iterator_ops.zig:2474`)
...
### `iteratorLimitArgument` (`src/exec/iterator_ops.zig:2491`)

- **签名**：`fn iteratorLimitArgument( ctx: *core.JSContext, output: ?*std.Io.Writer, global: *core.Object, args: []const core.JSValue, ) !usize`。
- **作用**：把 `take`/`drop` 的参数转成非负整数计数（Infinity 记作 usize 上限）。
- **实现**：缺参按 undefined 处理；对象先 `toPrimitiveForNumber`，再 `toNumberValue` 取数（float 优先，否则由 int32 转 f64）。NaN → `error.RangeError`；非有限 → `maxInt(usize)`；`trunc` 后为负 → `error.RangeError`；否则 `@intFromFloat`。
- **所有权 / 错误 / 调用**：只做数值转换，不分配、不持有。error set：NaN 与负数给 `error.RangeError`（裸 sentinel，`createSentinelError` 会造一个空 message 的 RangeError），另有 `toPrimitiveForNumber`/`toNumberValue` 里用户 `valueOf`/`@@toPrimitive` 抛的异常；`Infinity` **不报错**而是折成 `maxInt(usize)`（等价于无限配额）。唯一调用方 `iterator_ops.zig:2492`（`iteratorCreateLimitHelper`），它负责失败时的 IteratorClose。

### `iteratorCreateHelper` (`src/exec/iterator_ops.zig:2511`)
...
### `iteratorZipPutResult` (`src/exec/iterator_ops.zig:2606`)
...
### `iteratorZipCompleteAbrupt` (`src/exec/iterator_ops.zig:2622`)
...
### `iteratorZipHelperNext` (`src/exec/iterator_ops.zig:2650`)
...
### `iteratorZipHelperReturn` (`src/exec/iterator_ops.zig:2768`)
...
### `iteratorHelperNext` (`src/exec/iterator_ops.zig:2803`)
...
### `iteratorHelperSetInner` (`src/exec/iterator_ops.zig:2920`)
...
### `iteratorHelperSetInnerFromIterator` (`src/exec/iterator_ops.zig:2943`)
...
### `iteratorHelperReturn` (`src/exec/iterator_ops.zig:2970`)
...
### `iteratorHelperClose` (`src/exec/iterator_ops.zig:2994`)
...
### `iteratorHelperClear` (`src/exec/iterator_ops.zig:3015`)
...
### `iteratorHelperCloseInner` (`src/exec/iterator_ops.zig:3025`)
...
### `iteratorHelperClearInner` (`src/exec/iterator_ops.zig:3041`)
...
### `testIteratorGetValuePropertyOptional` (`src/exec/iterator_ops.zig:3046`)
...
### `iteratorCloseValue` (`src/exec/iterator_ops.zig:3064`)
...
### `iteratorForValue` (`src/exec/iterator_ops.zig:3079`)
...
### `iteratorStepValue` (`src/exec/iterator_ops.zig:3117`)
...
### `iteratorStepResult` (`src/exec/iterator_ops.zig:3142`)
...
- **实现**：与 `iteratorStepValue` 同形（缓存 next / 可调用性 / 结果必须是对象），但把 `next_arg` 作为唯一实参传下去，并把 `next()` 的结果对象一并返回。注意取 value 的条件与 `iteratorStepValue` 相反：只有 `done` 为真时才 Get `value`，`done` 为假时 value 留 `undefined`（调用方自己从 `result` 上读）。这是**有意**的而非写反——唯一调用方是 `yield*`，done 步的 value 就是委托的完成值，not-done 步要把整个结果对象再 yield 出去，在那里额外 Get 一次 `value` 反而多一次可观察读；函数里已加注释写明。
...
### `iteratorCallForNativeRecord` (`src/exec/iterator_ops.zig:3171`)
...
### `iteratorStaticCall` (`src/exec/iterator_ops.zig:3201`)
...
### `iteratorFromSourceForIteratorFrom` (`src/exec/iterator_ops.zig:3219`)
...
### `iteratorWrapMethodCall` (`src/exec/iterator_ops.zig:3273`)
...
### `iteratorWrapNext` (`src/exec/iterator_ops.zig:3313`)
...
### `iteratorWrapReturn` (`src/exec/iterator_ops.zig:3325`)
...
### `createIteratorResult` (`src/exec/iterator_ops.zig:3346`)
...
### `closeIteratorForFromEntriesAbrupt` (`src/exec/iterator_ops.zig:3373`)
