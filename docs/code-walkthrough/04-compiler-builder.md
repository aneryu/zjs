...
### `reserve` (`src/compiler/builder.zig:90`)
...
### `reserveSlowBytes` (`src/compiler/builder.zig:126`)
...
### `Builder.init` (`src/compiler/builder.zig:204`)
...
### `Builder.enableControlIndex` (`src/compiler/builder.zig:211`)
...
### `Builder.hasControlIndex` (`src/compiler/builder.zig:217`)
...
### `Builder.controlCount` (`src/compiler/builder.zig:221`)
...
### `Builder.controlAt` (`src/compiler/builder.zig:225`)
...
### `Builder.ensureControlCapacity` (`src/compiler/builder.zig:232`)
...
### `Builder.appendControlAssumeCapacity` (`src/compiler/builder.zig:255`)
...
### `Builder.recordControl` (`src/compiler/builder.zig:270`)
...
### `Builder.deinit` (`src/compiler/builder.zig:290`)

- **签名**：`pub fn deinit(self: *Builder) void`。
- **作用**：按满 capacity 释放所有 backing，并把字段收成空 builder。幂等。
- **实现**：code / atom / label / reloc / control_spill / source 各 `memory.free`（capacity 0 跳过）。atom 前缀不在这里逐项 unretain——与文件头「item-wise release of the initialized prefix」的契约由调用方在放弃产物时通过 MemoryAccount/AtomTable 生命周期覆盖；本函数把 slice 置空、len 清零、`last_opcode_pos = -1`。
- **所有权 / 错误 / 调用**：`releaseConsumedBuilder`、测试 `defer`、失败路径。可连调两次。

### `Builder.newLabel` (`src/compiler/builder.zig:321`)
...
### `Builder.emitJump` (`src/compiler/builder.zig:342`)
...
### `Builder.bindLabel` (`src/compiler/builder.zig:382`)
...
### `Builder.bindLabelMatchBarrier` (`src/compiler/builder.zig:394`)
...
### `Builder.retargetLabelRefs` (`src/compiler/builder.zig:416`)
...
### `Builder.firstUnboundLabel` (`src/compiler/builder.zig:495`)
...
### `Builder.emitOp` (`src/compiler/builder.zig:503`)

- **签名**：`pub fn emitOp(self: *Builder, op_id: u8) Error!void`。
- **作用**：发一字节纯 opcode。
- **实现**：`reserveCode(1)`，写字节，更新 `last_opcode_pos` 与 `code_len`。
- **所有权 / 错误 / 调用**：不涉及 atom 账本；只往 builder 自己的 `code` 缓冲写一字节并更新 `last_opcode_pos`，缓冲归 `self.memory`、由 `Builder.deinit` 释放。唯一的错误来自 `reserveCode`（`OutOfMemory` / `BytecodeOverflow`），失败时一个字节也没写。parser 侧经 `State.emitOp`（`src/parser.zig:3638`）与 emitter 门面进来，`resolve_labels` / `resolve_variables` 的测试夹具也直接用它拼字节流。

### `Builder.emitOpU8` (`src/compiler/builder.zig:513`)
...
### `Builder.emitOpU16` (`src/compiler/builder.zig:525`)

- **签名**：`pub fn emitOpU16(self: *Builder, op_id: u8, val: u16) Error!void`。
- **作用**：opcode + LE u16（槽索引、scope 等）。
- **实现**：3 字节。
- **所有权 / 错误 / 调用**：不涉及 atom；三字节写进 `code` 缓冲（归 `self.memory`），失败只来自 `reserveCode`，且失败时未写任何字节。S3 仍可能原样拷这些宽形式，S4 short 再压窄。parser 侧入口是 `State.emitOpU16`（`src/parser.zig:3648`）与 `emitterOpU16`（`:7500`），`enter_scope` 也走它（`:3736`）。

### `Builder.emitCallOp` (`src/compiler/builder.zig:539`)
...
### `Builder.emitOpU32` (`src/compiler/builder.zig:552`)
...
### `Builder.emitOpI32` (`src/compiler/builder.zig:564`)

- **签名**：`pub fn emitOpI32(self: *Builder, op_id: u8, val: i32) Error!void`。
- **作用**：opcode + LE i32。
- **实现**：5 字节。
- **所有权 / 错误 / 调用**：与 `emitOpU32` 同形，只是按 i32 写（`push_i32` 等带符号立即数）；不涉及 atom，错误只来自 `reserveCode`。唯一生产调用点 `State.emitOpI32`（`src/parser.zig:3658`）。

### `Builder.emitAtomOpOwned` (`src/compiler/builder.zig:577`)
...
### `Builder.emitAtomOpU8Owned` (`src/compiler/builder.zig:616`)
...
### `Builder.emitAtomOpU16Owned` (`src/compiler/builder.zig:646`)
...
### `Builder.emitScopeRefOpOwned` (`src/compiler/builder.zig:680`)
...
### `Builder.takeLastAtomOwned` (`src/compiler/builder.zig:745`)
...
### `Builder.takeTrailingAtomOpcodeOwned` (`src/compiler/builder.zig:754`)
...
### `Builder.replaceAtomOperand` (`src/compiler/builder.zig:803`)
...
### `Builder.rewriteTrailingAtomOpAsPlain` (`src/compiler/builder.zig:835`)
...
### `Builder.addSourceMarker` (`src/compiler/builder.zig:861`)
...
### `Builder.snapshot` (`src/compiler/builder.zig:885`)
...
### `Builder.rollback` (`src/compiler/builder.zig:901`)
...
### `Builder.truncateTail` (`src/compiler/builder.zig:958`)

- **签名**：`pub fn truncateTail(self: *Builder, new_code_len: u32) void`。
- **作用**：qjs `fd->byte_code.size = last_opcode_pos` rewind。只允许无 reloc、无 bind 的尾巴。
- **实现**：最新 reloc 操作数是高水位（发射有序）→ O(1) 无 reloc 检查。Debug 扫 bind。source/control 用 `>= new_code_len` 丢掉（正好在切点的 marker 也走——与 rollback 按 ledger 长度恢复不同）。`last_opcode_pos = -1`。
- **所有权 / 错误 / 调用**：返回 `void`、**没有 error set**：前置条件全部是断言（新长度不超过当前、最后一条 reloc 的操作数不跨边界、Debug 下没有已绑定标签落在边界之后），断言失败即契约破坏。只截断 `code_len` / source 槽 / 控制事件并把 `last_opcode_pos` 置 −1，不碰 atom 账本（所以调用方若删掉的是带 atom 的指令，必须另行把账本对齐）。⚠️ **当前树内没有生产调用方**：唯一调用点是本文件的单测（`src/compiler/builder.zig:1680`）——投机 LHS 去 getter 的生产路径实际走的是 `takeTrailingAtomOpcodeOwned` 与 `truncateLastOpcodePreserveSources`。

### `Builder.truncateLastMarkedOpcode` (`src/compiler/builder.zig:988`)

- **签名**：`pub fn truncateLastMarkedOpcode(self: *Builder, new_code_len: u32) Error!void`。
- **作用**：撤掉带 marker 的最后一条 parser opcode，但留下同一 compact 偏移上更早的零宽 source。
- **实现**：要求 `last_opcode_pos == new_code_len`、无越界 reloc/bind、**最新** source 正好在该偏移。只 `source_len -= 1`。
- **所有权 / 错误 / 调用**：与 `truncateTail` 不同，这里所有前置条件都是 `error.InvalidBytecode`（新长度必须正等于 `last_opcode_pos`、最后一条 reloc 不跨边界、无已绑定标签在边界之后、且**边界上必须正好有一条源槽**），检查全过才动状态。它只丢掉那一条源槽：legacy 的 `OP_line_num` 字节让更早的零宽事件物理上排在 getter 之前，compact 账本把它们压在同一偏移上，所以只有最新那条属于被删的 opcode。不碰 atom 账本。⚠️ **当前树内没有生产调用方**，唯一调用点是本文件单测（`src/compiler/builder.zig:1734`）；生产路径用的是下面那个 `truncateLastOpcodePreserveSources`。

### `Builder.truncateLastOpcodePreserveSources` (`src/compiler/builder.zig:1026`)
...
- **所有权 / 错误 / 调用**：前三项校验与 `truncateLastMarkedOpcode` 相同（都返回 `error.InvalidBytecode`），但**不要求边界上有源槽**，且只丢 `temp_offset > new_code_len` 的槽——正好等于边界的那些留下来描述随后写在同一位置的替换指令。不碰 atom 账本。生产调用方三处：`getLValue` 撤掉 `get_array_el` / `get_super_value` 一字节 getter（`src/parser.zig:4442` / `:4450`）、可选链 delete 重写（`:5131`），以及本文件内的 `rewriteTrailingAtomOpAsPlain`（`src/compiler/builder.zig:853`）。

### `Builder.detachTail` (`src/compiler/builder.zig:1069`)
...
### `Builder.spliceSegment` (`src/compiler/builder.zig:1210`)
...
### `Builder.discardSegment` (`src/compiler/builder.zig:1345`)
...
### `Builder.invalidateLastOpcode` (`src/compiler/builder.zig:1351`)
...
### `Builder.freeSegmentBackings` (`src/compiler/builder.zig:1355`)
...
### `Builder.reserveCode` (`src/compiler/builder.zig:1365`)
...
### `expectRelocChain` (`src/compiler/builder.zig:1387`)
...
### `s2g4OomScript` (`src/compiler/builder.zig:1892`)
