...
### `Input.len` (`src/libs/regexp.zig:61`)
...
### `CaptureSlotBuffer.initDefault` (`src/libs/regexp.zig:91`)
...
### `CaptureSlotBuffer.init` (`src/libs/regexp.zig:98`)
...
### `CaptureSlotBuffer.deinit` (`src/libs/regexp.zig:107`)
...
### `REExecContext.deinit` (`src/libs/regexp.zig:229`)
...
### `REExecContext.pollTimeout` (`src/libs/regexp.zig:240`)
...
### `REExecContext.btFrameRealloc` (`src/libs/regexp.zig:249`)
...
### `REExecContext.undoRealloc` (`src/libs/regexp.zig:261`)
...
### `buildLRECtypeBits` (`src/libs/regexp.zig:286`)
...
### `buildLRECanonicalizeLatin1` (`src/libs/regexp.zig:299`)
...
### `lreCanonicalize` (`src/libs/regexp.zig:308`)
...
### `normalizeStartIndex` (`src/libs/regexp.zig:329`)
...
### `slotOptional` (`src/libs/regexp.zig:343`)
...
### `captureSlotValue` (`src/libs/regexp.zig:347`)
...
### `captureCountFromBytecode` (`src/libs/regexp.zig:351`)
...
### `captureCount` (`src/libs/regexp.zig:356`)

- **签名**：`pub fn captureCount(bytecode: []const u8) usize`。
- **作用**：公开包装。
- **实现**：转私有。
- **所有权 / 错误 / 调用**：与 `Compiled.captureCount` 同名不同接收者。

### `registerCountFromBytecode` (`src/libs/regexp.zig:360`)

- **签名**：`fn registerCountFromBytecode(bytecode: []const u8) usize`。
- **作用**：读量化器用的寄存器数。
- **实现**：`bytecode[3]`。
- **所有权 / 错误 / 调用**：`allocCount`。

### `allocCountFromBytecode` (`src/libs/regexp.zig:365`)
...
### `allocCount` (`src/libs/regexp.zig:369`)

- **签名**：`pub fn allocCount(bytecode: []const u8) usize`。
- **作用**：公开包装。
- **实现**：转私有。
- **所有权 / 错误 / 调用**：调用方预分配 slot 用；`exec/string_ops`、`exec/regexp_fastpath` 走的是同名方法 `Compiled.allocCount`。

### `getFlags` (`src/libs/regexp.zig:373`)
...
### `groupNameFromBytecode` (`src/libs/regexp.zig:378`)
...
### `groupName` (`src/libs/regexp.zig:396`)
...
### `Compiled.deinit` (`src/libs/regexp.zig:405`)
...
### `Compiled.captureCount` (`src/libs/regexp.zig:410`)

- **签名**：`pub fn captureCount(self: Compiled) usize`。
- **作用**：方法版。
- **实现**：`captureCountFromBytecode(self.bytecode)`。
- **所有权 / 错误 / 调用**：按值接收 `Compiled`，只复制切片头、不取 `bytecode` 的所有权（仍由调用方持有的 `Compiled.deinit` 释放）；只读字节码头部，不分配、无 error。调用方 `src/exec/regexp_fastpath.zig:797`、`src/exec/string_ops.zig:1491`；`tools/perf/direct/zjs_direct_bench.zig:240` 走的是裸切片版 `captureCount`（:356）。

### `Compiled.allocCount` (`src/libs/regexp.zig:414`)

- **签名**：`pub fn allocCount(self: Compiled) usize`。
- **作用**：方法版 slot 数。
- **实现**：转私有。
- **所有权 / 错误 / 调用**：同 `captureCount`：借用字节码、不分配、无 error。调用方 `src/exec/regexp_fastpath.zig:769`、`src/exec/string_ops.zig:1490`；裸切片版在 :369。

### `Compiled.groupName` (`src/libs/regexp.zig:418`)
...
### `Compiled.flagBits` (`src/libs/regexp.zig:422`)
...
### `compilePatternAndFlags` (`src/libs/regexp.zig:427`)
...
### `compilePatternAndFlagsWithOptions` (`src/libs/regexp.zig:431`)
...
### `compilePatternWithFlagBitsAndOptions` (`src/libs/regexp.zig:440`)
...
### `isSupportedUnicodePropertyExpression` (`src/libs/regexp.zig:449`)
...
- **所有权 / 错误 / 调用**：本文件之外目前没有调用方；引擎侧的同名校验走 `core/regexp.zig` → `unicode/regexp_properties.zig`。

### `exec` (`src/libs/regexp.zig:453`)
...
- **实现**：`execWithOptions(..., .{})`。
- **所有权 / 错误 / 调用**：树内无调用方，只作对外 API；生产路径走 trusted 入口 + `regexp_adapter`。

### `execWithOptions` (`src/libs/regexp.zig:457`)
...
### `execIntoMatchWithOptions` (`src/libs/regexp.zig:467`)
...
### `execIntoMatchTrustedWithOptions` (`src/libs/regexp.zig:490`)
...
### `execCaptureSlotsSliceTrustedWithOptions` (`src/libs/regexp.zig:512`)
...
### `execCaptureSlotsParsed` (`src/libs/regexp.zig:524`)
...
### `testMatchTrustedWithOptions` (`src/libs/regexp.zig:593`)
...
### `ExecState.init` (`src/libs/regexp.zig:620`)
...
### `ExecState.cbufUtf16` (`src/libs/regexp.zig:653`)
...
### `ExecState.checkFrameSpace` (`src/libs/regexp.zig:657`)
...
### `ExecState.checkUndoSpace` (`src/libs/regexp.zig:670`)
...
### `ExecState.ensurePc` (`src/libs/regexp.zig:683`)
...
### `ExecState.pcWithOffset` (`src/libs/regexp.zig:691`)
...
### `ExecState.getU8` (`src/libs/regexp.zig:712`)
...
### `ExecState.readU8At` (`src/libs/regexp.zig:719`)
...
### `ExecState.getU16` (`src/libs/regexp.zig:724`)
...
### `ExecState.readU16UncheckedAt` (`src/libs/regexp.zig:731`)
...
### `ExecState.getU32` (`src/libs/regexp.zig:735`)
...
### `ExecState.readU32At` (`src/libs/regexp.zig:742`)
...
### `ExecState.readU32UncheckedAt` (`src/libs/regexp.zig:747`)
...
### `ExecState.getI32` (`src/libs/regexp.zig:751`)
...
### `ExecState.compactIndex` (`src/libs/regexp.zig:755`)
...
### `ExecState.compactCaptureValue` (`src/libs/regexp.zig:762`)
...
### `ExecState.expandCaptureValue` (`src/libs/regexp.zig:770`)
...
### `ExecState.pcOffset` (`src/libs/regexp.zig:774`)
...
### `ExecState.pcFromOffset` (`src/libs/regexp.zig:781`)
...
### `ExecState.frameType` (`src/libs/regexp.zig:790`)
...
### `ExecState.pushExecState` (`src/libs/regexp.zig:797`)
...
### `ExecState.saveCapture` (`src/libs/regexp.zig:809`)
...
### `ExecState.pushUndo` (`src/libs/regexp.zig:816`)
...
### `ExecState.pushUndoAssumeSpace` (`src/libs/regexp.zig:821`)
...
### `ExecState.saveCaptureCheck` (`src/libs/regexp.zig:833`)
...
### `ExecState.restoreOneUndo` (`src/libs/regexp.zig:852`)
...
### `ExecState.restoreUndoTo` (`src/libs/regexp.zig:865`)
...
### `ExecState.currentUndoBase` (`src/libs/regexp.zig:874`)
...
### `ExecState.popFrameRestore` (`src/libs/regexp.zig:878`)
...
### `ExecState.popFrameKeepUndo` (`src/libs/regexp.zig:890`)
...
### `ExecState.registerSlot` (`src/libs/regexp.zig:901`)
...
### `ExecState.readRegisterValue` (`src/libs/regexp.zig:905`)
...
### `ExecState.getCharAtBounded` (`src/libs/regexp.zig:917`)
...
### `ExecState.getPrevCharAtBounded` (`src/libs/regexp.zig:945`)
...
### `ExecState.getCharUnchecked` (`src/libs/regexp.zig:972`)
...
### `ExecState.peekChar` (`src/libs/regexp.zig:990`)
...
### `ExecState.peekPrevChar` (`src/libs/regexp.zig:1007`)
...
### `ExecState.prevChar` (`src/libs/regexp.zig:1025`)
...
### `ExecState.scanUntilChar8` (`src/libs/regexp.zig:1044`)
...
### `ExecState.scanGreedyClass8` (`src/libs/regexp.zig:1068`)
...
### `ExecState.matchRawForward` (`src/libs/regexp.zig:1106`)
...
### `ExecState.matchRawBackward` (`src/libs/regexp.zig:1124`)
...
### `lreExecBacktrack` (`src/libs/regexp.zig:1141`)
...
### `writeMatch` (`src/libs/regexp.zig:1562`)
...
### `parseHeader` (`src/libs/regexp.zig:1596`)
...
### `parseHeaderTrusted` (`src/libs/regexp.zig:1608`)
...
### `checkedAllocCount` (`src/libs/regexp.zig:1620`)
...
### `decodeOp` (`src/libs/regexp.zig:1627`)
...
### `isLineTerminator` (`src/libs/regexp.zig:1632`)
...
### `lreIsSpaceByte` (`src/libs/regexp.zig:1636`)
...
### `lreIsSpace` (`src/libs/regexp.zig:1640`)
...
### `isHiSurrogate` (`src/libs/regexp.zig:1645`)
...
### `isLoSurrogate` (`src/libs/regexp.zig:1649`)
...
### `fromSurrogate` (`src/libs/regexp.zig:1653`)
...
### `decodeWtf8Surrogate` (`src/libs/regexp.zig:1662`)
...
### `ModifierGroup.applyFlag` (`src/libs/regexp.zig:1707`)
...
### `parseModifierGroup` (`src/libs/regexp.zig:1715`)
...
### `startsWithAt` (`src/libs/regexp.zig:1748`)
...
### `isRegExpModifierFlag` (`src/libs/regexp.zig:1752`)
...
### `modifierFlagSlot` (`src/libs/regexp.zig:1756`)
...
### `REStringList.init` (`src/libs/regexp.zig:1779`)
...
### `REStringList.deinit` (`src/libs/regexp.zig:1783`)
...
### `REStringList.containsString` (`src/libs/regexp.zig:1789`)
...
### `REStringList.addOwnedString` (`src/libs/regexp.zig:1797`)
...
### `REStringList.unionWith` (`src/libs/regexp.zig:1805`)
...
### `REStringList.intersectWith` (`src/libs/regexp.zig:1815`)
...
### `REStringList.subtract` (`src/libs/regexp.zig:1829`)
...
### `compile` (`src/libs/regexp.zig:1851`)
...
### `compileWithOptions` (`src/libs/regexp.zig:1855`)
...
### `compileWithFlagBitsAndOptions` (`src/libs/regexp.zig:1864`)
...
### `parseFlagBits` (`src/libs/regexp.zig:1904`)
...
### `parseGroupNameAt` (`src/libs/regexp.zig:1942`)
...
### `groupNamesEqual` (`src/libs/regexp.zig:1960`)
...
### `readGroupNameCodePoint` (`src/libs/regexp.zig:1971`)
...
### `readGroupNameLiteralCodePoint` (`src/libs/regexp.zig:2000`)
...
### `readUnicodeEscapeCodePoint` (`src/libs/regexp.zig:2015`)
...
### `isRegExpGroupNameStart` (`src/libs/regexp.zig:2043`)

- **签名**：`pub fn isRegExpGroupNameStart(cp: u21) bool`。
- **作用**：组名首字符。
- **实现**：`$` `_` ASCII 字母；非法表 false；`>0x7F` 真。
- **所有权 / 错误 / 调用**：虽是 `pub`，树内调用方只有本文件的 `parseGroupNameAt`（:1949）与 `isRegExpGroupNameContinue`（:2053）。

### `isRegExpGroupNameContinue` (`src/libs/regexp.zig:2050`)

- **签名**：`pub fn isRegExpGroupNameContinue(cp: u21) bool`。
- **作用**：组名后续。
- **实现**：非法表 false；U+104A4 特准；Start 或 ASCII 数字或 U+1D7DA。
- **所有权 / 错误 / 调用**：不分配、无 error；只读 comptime 表。虽然是 `pub`，树内唯一调用方是本文件的组名扫描 `src/libs/regexp.zig:1950`。

### `isInvalidRegExpGroupNameStart` (`src/libs/regexp.zig:2059`)
...
### `isInvalidRegExpGroupNameContinue` (`src/libs/regexp.zig:2067`)

- **签名**：`pub fn isInvalidRegExpGroupNameContinue(cp: u21) bool`。
- **作用**：Continue 排除表（比 Start 少 U+104A4/1D7DA）。
- **实现**：switch。
- **所有权 / 错误 / 调用**：纯谓词、无 error；虽是 `pub`，树内唯一调用方是 `isRegExpGroupNameContinue`（:2051）。

## `REParseState` 解析

### `REParseState.lreCheckStackOverflow` (`src/libs/regexp.zig:2097`)
...
### `REParseState.atomResult` (`src/libs/regexp.zig:2102`)
...
### `REParseState.putGroupName` (`src/libs/regexp.zig:2113`)
...
### `REParseState.isDuplicateGroupName` (`src/libs/regexp.zig:2125`)
...
### `REParseState.findGroupName` (`src/libs/regexp.zig:2136`)
...
### `REParseState.reParseCaptures` (`src/libs/regexp.zig:2152`)
...
### `REParseState.reCountCaptures` (`src/libs/regexp.zig:2202`)
...
### `REParseState.reHasNamedCaptures` (`src/libs/regexp.zig:2211`)
...
### `REParseState.emitHeader` (`src/libs/regexp.zig:2218`)
...
### `REParseState.patchHeader` (`src/libs/regexp.zig:2222`)
...
### `REParseState.patchSearchLiteralPrefix` (`src/libs/regexp.zig:2234`)
...
- **所有权 / 错误 / 调用**：不分配、无 error：只在 `self.byte_code.items` 上原地重写前缀的 6 个字节（opcode、needle、`-11` 偏移；偏移按原值写回，真正变的是前两字节），形态不匹配就静默返回。唯一调用方是编译收尾 `src/libs/regexp.zig:1898`。

### `REParseState.reParseDisjunction` (`src/libs/regexp.zig:2258`)
...
### `REParseState.reParseAlternative` (`src/libs/regexp.zig:2281`)
...
### `REParseState.reParseTerm` (`src/libs/regexp.zig:2297`)
...
### `REParseState.parseGroup` (`src/libs/regexp.zig:2346`)
...
### `REParseState.parseCaptureGroup` (`src/libs/regexp.zig:2402`)
...
### `REParseState.parseEscape` (`src/libs/regexp.zig:2418`)
...
### `REParseState.reParseCharClass` (`src/libs/regexp.zig:2613`)
...
### `REParseState.atMatch` (`src/libs/regexp.zig:2645`)
...
### `REParseState.reParseNestedClass` (`src/libs/regexp.zig:2657`)
...
### `REParseState.reParseClassSetOperand` (`src/libs/regexp.zig:2730`)
...
### `REParseState.parseClassStringDisjunction` (`src/libs/regexp.zig:2786`)
...
### `REParseState.reEmitStringList` (`src/libs/regexp.zig:2829`)
...
### `REParseState.longerFirst` (`src/libs/regexp.zig:2844`)
...
### `REParseState.reParseClassAtomOrRange` (`src/libs/regexp.zig:2894`)
...
### `REParseState.getClassAtom` (`src/libs/regexp.zig:2945`)
...
### `REParseState.parseQuantifier` (`src/libs/regexp.zig:3069`)
...
### `REParseState.tryFoldGreedyClass8Loop` (`src/libs/regexp.zig:3135`)
...
### `REParseState.wrapGenericQuantifier` (`src/libs/regexp.zig:3146`)
...
### `REParseState.parseDecimalEscape` (`src/libs/regexp.zig:3225`)
...
### `REParseState.parseLegacyDecimalEscape` (`src/libs/regexp.zig:3231`)
...
### `REParseState.parseLegacyOctalAfterZero` (`src/libs/regexp.zig:3254`)
...
### `REParseState.parseLegacyClassDecimalEscape` (`src/libs/regexp.zig:3266`)
...
### `REParseState.parseGroupName` (`src/libs/regexp.zig:3291`)
...
### `REParseState.parseStringPropertyEscape` (`src/libs/regexp.zig:3300`)
...
### `REParseState.buildStringPropertyStringList` (`src/libs/regexp.zig:3315`)
...
### `REParseState.addSequenceToStringList` (`src/libs/regexp.zig:3334`)
...
### `REParseState.parseUnicodePropertyEscapeWithOrdering` (`src/libs/regexp.zig:3353`)
...
### `REParseState.parseUnicodePropertyEscape` (`src/libs/regexp.zig:3366`)
...
### `REParseState.parseDigits` (`src/libs/regexp.zig:3387`)
...
### `REParseState.parseFixedHexEscape` (`src/libs/regexp.zig:3402`)
...
### `REParseState.parseUnicodeEscape` (`src/libs/regexp.zig:3414`)
...
### `REParseState.isBracedUnicodeEscape` (`src/libs/regexp.zig:3434`)
...
### `REParseState.combineEscapedSurrogatePair` (`src/libs/regexp.zig:3438`)
...
### `REParseState.readPatternCodePoint` (`src/libs/regexp.zig:3451`)
...
### `REParseState.readClassCodePoint` (`src/libs/regexp.zig:3458`)
...
### `REParseState.readUtf8CodePoint` (`src/libs/regexp.zig:3475`)
...
### `REParseState.looksLikeQuantifier` (`src/libs/regexp.zig:3494`)
...
### `REParseState.reEmitChar` (`src/libs/regexp.zig:3507`)
...
### `REParseState.emitCharacterAtom` (`src/libs/regexp.zig:3515`)
...
### `REParseState.emitCanonicalChar` (`src/libs/regexp.zig:3521`)
...
### `REParseState.emitDirectional` (`src/libs/regexp.zig:3525`)
...
### `REParseState.emitDirectionalRange` (`src/libs/regexp.zig:3531`)
...
### `REParseState.emitNonUnicodeSurrogatePairAtom` (`src/libs/regexp.zig:3537`)
...
### `REParseState.emitNonUnicodeCodePointAtom` (`src/libs/regexp.zig:3550`)
...
### `REParseState.emitInvalidControlEscape` (`src/libs/regexp.zig:3559`)
...
### `REParseState.emitNonUnicodeSurrogatePairTerms` (`src/libs/regexp.zig:3570`)
...
### `REParseState.reEmitRange` (`src/libs/regexp.zig:3586`)
...
### `REParseState.reEmitClass8` (`src/libs/regexp.zig:3629`)
...
### `REParseState.backReferenceOp` (`src/libs/regexp.zig:3634`)
...
### `REParseState.emitBackReference` (`src/libs/regexp.zig:3639`)
...
### `REParseState.reEmitOp` (`src/libs/regexp.zig:3645`)
...
### `REParseState.reEmitOpU8` (`src/libs/regexp.zig:3649`)
...
### `REParseState.reEmitOpU16` (`src/libs/regexp.zig:3654`)
...
### `REParseState.reEmitOpU32` (`src/libs/regexp.zig:3659`)
...
### `REParseState.reEmitOpU32At` (`src/libs/regexp.zig:3664`)
...
### `REParseState.reEmitOpI32` (`src/libs/regexp.zig:3671`)
...
### `REParseState.reEmitGoto` (`src/libs/regexp.zig:3675`)
...
### `REParseState.reEmitGotoU8` (`src/libs/regexp.zig:3684`)
...
### `REParseState.reEmitGotoU8U32` (`src/libs/regexp.zig:3694`)
...
### `REParseState.reInsertSaveReset` (`src/libs/regexp.zig:3705`)
...
### `REParseState.appendU16` (`src/libs/regexp.zig:3712`)
...
### `REParseState.appendU32` (`src/libs/regexp.zig:3718`)
...
### `REParseState.insertBytes` (`src/libs/regexp.zig:3724`)
...
### `REParseState.moveTermToStart` (`src/libs/regexp.zig:3734`)
...
### `class8Mask` (`src/libs/regexp.zig:3750`)
...
### `class8BitmapContains` (`src/libs/regexp.zig:3754`)
...
### `class8CodePointMatches` (`src/libs/regexp.zig:3758`)
...
### `setClass8BitmapBit` (`src/libs/regexp.zig:3763`)
...
### `buildClass8IncludedBitmap` (`src/libs/regexp.zig:3767`)
...
### `buildClass8ExcludedBitmap` (`src/libs/regexp.zig:3781`)
...
### `rangesContainTailFrom` (`src/libs/regexp.zig:3793`)
...
### `rangesContainCodePoint` (`src/libs/regexp.zig:3803`)
...
### `addAtomToCharRange` (`src/libs/regexp.zig:3813`)
...
### `addInclusiveRange` (`src/libs/regexp.zig:3829`)
...
### `addNonUnicodeSurrogatePair` (`src/libs/regexp.zig:3838`)
...
### `addClassEscape` (`src/libs/regexp.zig:3844`)
...
### `addUnicodeProperty` (`src/libs/regexp.zig:3867`)
...
### `reNeedCheckAdvAndCaptureInit` (`src/libs/regexp.zig:3883`)
...
### `reComputeRegisterCount` (`src/libs/regexp.zig:3928`)
...
### `opFixedSize` (`src/libs/regexp.zig:3977`)
...
### `loopSplitOp` (`src/libs/regexp.zig:3995`)
...
### `canonicalizeLiteral` (`src/libs/regexp.zig:4002`)
...
### `opByte` (`src/libs/regexp.zig:4009`)
...
### `isRegexSyntax` (`src/libs/regexp.zig:4013`)
...
### `isSyntaxEscape` (`src/libs/regexp.zig:4020`)
...
### `isUnicodeSetsReservedClassByte` (`src/libs/regexp.zig:4027`)
...
### `isUnicodeSetsReservedDoublePunctuator` (`src/libs/regexp.zig:4035`)

- **签名**：`pub fn isUnicodeSetsReservedDoublePunctuator(first: u8, second: u8) bool`。
- **作用**：`&&` `!!` `##` … 在 `/v` class 里非法（`--` 由解析器当算子）。
- **实现**：须相等，再认一批标点（不含 `-`）。
- **所有权 / 错误 / 调用**：纯谓词、无 error；虽是 `pub`，树内调用方只有本文件的 `reParseClassSetOperand`（:2744）与 `reParseClassAtomOrRange`（:2900）。

### `fromHex` (`src/libs/regexp.zig:4045`)
...
### `isDigit` (`src/libs/regexp.zig:4052`)
...
### `lreIsWordByte` (`src/libs/regexp.zig:4056`)
