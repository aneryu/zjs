...
### `forInIsArraySlot` (`src/exec/forof_ops.zig:46`)
...
### `forInInProtoChainSlot` (`src/exec/forof_ops.zig:51`)
...
### `createForInIterator` (`src/exec/forof_ops.zig:59`)
...
### `forInFastArrayCount` (`src/exec/forof_ops.zig:113`)
...
### `forInSnapshotOwnStringKeys` (`src/exec/forof_ops.zig:144`)
...
### `forInOwnKeyIsEnumerable` (`src/exec/forof_ops.zig:182`)
...
### `forInDefineVisited` (`src/exec/forof_ops.zig:206`)
...
### `forInHasEnumerableStringKey` (`src/exec/forof_ops.zig:219`)
...
### `iteratorCatchMarker` (`src/exec/forof_ops.zig:263`)
...
### `iteratorCatchMarkerPreviousTarget` (`src/exec/forof_ops.zig:273`)
...
### `asyncIteratorCatchMarker` (`src/exec/forof_ops.zig:280`)
...
### `isAsyncIteratorCatchMarker` (`src/exec/forof_ops.zig:284`)
...
### `isIteratorCatchMarker` (`src/exec/forof_ops.zig:288`)
...
### `closeStackTopForOfIteratorForPendingError` (`src/exec/forof_ops.zig:308`)
...
- **实现**：以 `frame = null` 转发 `closeStackTopForOfIteratorForPendingErrorInternal`。
- **所有权 / 错误 / 调用**：薄转发，自身不分配、不消费栈槽（清理由 internal 做）。虽然签名是 `!void`，但 internal 把所有 close 失败都 `catch {}` 吞掉，推断出的 error set 为空，调用方的 `try` 只是形式。调用方是异常上抛前的 7 个 unwind 点：`call_runtime.zig:117`、`tailcall_dispatch.zig:679`/`:7237`、`vm_control.zig:111`、`vm_gen_async.zig:710`、`inline_calls.zig:5202`/`:5232`。

### `closeStackTopForOfIteratorForPendingErrorWithFrame` (`src/exec/forof_ops.zig:317`)

- **签名**：`pub fn closeStackTopForOfIteratorForPendingErrorWithFrame( ctx: *core.JSContext, output: ?*std.Io.Writer, global: *core.Object, stack: *stack_mod.Stack, frame: *const frame_mod.Frame, ) !void`。
- **作用**：同上，但多带一个当前帧参数的入口（供带帧上下文的 unwind 点调用）。
- **实现**：以调用方给的 `frame` 转发 `closeStackTopForOfIteratorForPendingErrorInternal`（该参数目前在 internal 里被 `_ = frame` 丢弃，只保留签名）。
- **所有权 / 错误 / 调用**：同上，只是多传一个当前帧（internal 里 `_ = frame` 未用）。调用方 4 处：`call_runtime.zig:173`（`tryCatchInFrame` 在查本帧 catch target 之前先关掉本帧活着的 for-of 迭代器）与 `vm_property_field.zig:199`/`:231`/`:267`。

### `closeStackTopForOfIteratorForPendingErrorInternal` (`src/exec/forof_ops.zig:327`)

- **签名**：`pub fn closeStackTopForOfIteratorForPendingErrorInternal( ctx: *core.JSContext, output: ?*std.Io.Writer, global: *core.Object, stack: *stack_mod.Stack, frame: ?*const frame_mod.Frame, ) !void`。
- **作用**：两个公开壳的共同实现：把栈上所有「其上没有 catch 边界」的 for-of 记录逐个 IteratorClose，close 期间的新异常一律丢弃，最后把原 pending 异常原样放回。
- **实现**：不可捕获中断直接返回，不 take/rethrow，也就不会清掉 uncatchable 标志。`frame` 参数当前未使用（`_ = frame`）。随后记下 `exceptionIsOutOfMemory`、`takeException` 取出 pending 异常，从栈顶往下用 `findTopClosableForOfRecordIndexBefore` 找无 catch 边界的 for-of 记录：先把 iterator 槽写成 `undefined`（IteratorClose 一次性语义，防止后续 unwind 再 return()），再 `closeIteratorFromVm`，close 的错误与异常被吞掉（`catch {}` + `clearException`），然后继续往栈底找。最后把原异常 `throwValue` 回去，OOM 标志用 `markExceptionOutOfMemory` 单独恢复。
- **所有权 / 错误 / 调用**：所有权：每关一个记录前先把栈槽里的 iterator 换成 `undefined`，即把该迭代器的所有权移出栈再跑用户 `return()`，兑现 IteratorClose 的一次性语义，后续 unwind/deinit 不会二次 `return()`。错误：pending 异常先 `takeException` 取走（OOM 标志另记、事后 `markExceptionOutOfMemory` 补回），close 期间的新异常与 Zig error 一律 `catch {}` + `clearException` 丢弃，最后把原异常 `throwValue` 回去——因此本函数**推断出的 error set 是空的**，不会给调用方新错误；uncatchable 中断直接返回，避免 take/rethrow 抹掉 uncatchable 位。调用方只有本文件两个公开壳 `forof_ops.zig:314`/`:324`。

### `closeIteratorForAbruptCompletion` (`src/exec/forof_ops.zig:369`)
...
### `findTopClosableForOfRecordIndexBefore` (`src/exec/forof_ops.zig:386`)
...
- **所有权 / 错误 / 调用**：纯扫描：只读 `stack.values`，不分配、不改栈、无 error。唯一调用方 `forof_ops.zig:350`（`closeStackTopForOfIteratorForPendingErrorInternal` 的关闭循环，每关一个就把 `before` 收缩到该记录）。

### `isForOfRecordAt` (`src/exec/forof_ops.zig:400`)
...
- **所有权 / 错误 / 调用**：纯判定，不分配、无 error、不动栈。调用方都在本文件：`forof_ops.zig:391`（扫描器）、`:410`（`abandonForOfIteratorAtIndex` 的断言）、`:418`（`abandonForOfIteratorAtDepth` 的合法性检查）。

### `abandonForOfIteratorAtIndex` (`src/exec/forof_ops.zig:409`)

- **签名**：`pub fn abandonForOfIteratorAtIndex(_: *core.JSRuntime, stack: *stack_mod.Stack, index: usize) void`。
- **作用**：放弃指定槽位上的 for-of 迭代器：把 iterator 槽写成 `undefined`，后续不再对它 `return()`。
- **实现**：断言该处确实是 for-of 记录，再把 iterator 槽写成 `undefined`（runtime 参数未使用）。对齐 qjs `js_for_of_next` 在 IteratorNext 异常完成时替换当前迭代器的做法，使后续 IteratorClose 不会对刚失败的迭代器再调 `return()`。
- **所有权 / 错误 / 调用**：所有权：把栈槽写成 `undefined` 就是放弃这一份迭代器引用——TGC 下没有 release 动作，值失去最后一个根后由 GC 回收，这也是形参 `_: *core.JSRuntime` 被忽略的原因（RC 时代的残留签名）。无 error（越界由 `debug.assert` 在 Debug 下兜）。调用方：`iterator_ops.zig:519`（`forOfNext` 的 `errdefer`）、`:562`（`finishForOfNextResult` 的 `errdefer`）与本文件 `:419`。

### `abandonForOfIteratorAtDepth` (`src/exec/forof_ops.zig:414`)

- **签名**：`pub fn abandonForOfIteratorAtDepth(rt: *core.JSRuntime, stack: *stack_mod.Stack, depth: u8) !void`。
...
- **所有权 / 错误 / 调用**：只按 depth 定位后转发，不分配、不交出所有权。error set 只有 `error.InvalidBytecode`，而它**不在 `exception_ops.runtimeErrorInfo` 的映射表里**：`tryCatchInFrame` 认不出它就返回 false，所以它不会变成可被 JS `catch` 的异常，而是作为引擎错误逃出 dispatch 循环交给宿主。调用方：`inline_calls.zig:5200`/`:5230` 与 `tailcall_dispatch.zig:732`（for-of 体内 return/throw 前放弃当前迭代器）。

### `hasCatchMarkerAboveForOfRecord` (`src/exec/forof_ops.zig:422`)
...
### `closeIteratorFromVm` (`src/exec/forof_ops.zig:433`)
...
### `closeIteratorFromVmImpl` (`src/exec/forof_ops.zig:442`)
