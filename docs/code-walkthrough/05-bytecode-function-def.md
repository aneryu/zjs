...
- `FlowTailSummary` 由 parser 统一发射维护。
...
### `function_def.freeOwnedValue` (`src/bytecode.zig:4966`)
...
### `function_def.growSliceBy` (`src/bytecode.zig:5055`)
...
### `function_def.growSliceBySlowBytes` (`src/bytecode.zig:5088`)
...
### `function_def.freeGrowableSlice` (`src/bytecode.zig:5113`)
...
### `function_def.freeGrowableAtomSlice` (`src/bytecode.zig:5126`)

- **签名**：`fn freeGrowableAtomSlice( _: *atom.AtomTable, mem: *memory.MemoryAccount, slice: *[]atom.Atom, capacity: *usize, ) void`。
- **作用**：原子切片：capacity 优先。
- **实现**：不减 atom ref（tracing）。
- **所有权 / 错误 / 调用**：deinit。


### `function_def.freeGrowableNamedSlice` (`src/bytecode.zig:5143`)

- **签名**：`fn freeGrowableNamedSlice( comptime T: type, _: *atom.AtomTable, mem: *memory.MemoryAccount, slice: *[]T, capacity: *usize, ) void`。
- **作用**：带 atom 字段的结构切片。
- **实现**：按 T free。
- **所有权 / 错误 / 调用**：vars/args/global/closure。


### `FunctionDef.init` (`src/bytecode.zig:5328`)
...
### `FunctionDef.replaceSourceText` (`src/bytecode.zig:5338`)
...
### `FunctionDef.deinitInitFailure` (`src/bytecode.zig:5349`)
...
### `FunctionDef.appendScope` (`src/bytecode.zig:5370`)
...
### `FunctionDef.rebuildFinalScopeLinks` (`src/bytecode.zig:5383`)
...
### `FunctionDef.validateFinalScopeLinks` (`src/bytecode.zig:5448`)
...
### `FunctionDef.invalidateScopeLinkCache` (`src/bytecode.zig:5521`)
...
### `FunctionDef.proveAncestorScopeLinks` (`src/bytecode.zig:5525`)
...
### `FunctionDef.consumeGlobalVars` (`src/bytecode.zig:5536`)
...
### `FunctionDef.ensureFuncExprSelfBinding` (`src/bytecode.zig:5553`)
...
### `FunctionDef.ensureThisBinding` (`src/bytecode.zig:5575`)
...
### `FunctionDef.ensureNewTargetBinding` (`src/bytecode.zig:5593`)
...
### `FunctionDef.ensureThisActiveFunctionBinding` (`src/bytecode.zig:5605`)
...
### `FunctionDef.ensureHomeObjectBinding` (`src/bytecode.zig:5617`)
...
### `FunctionDef.ensureArgumentsBinding` (`src/bytecode.zig:5635`)
...
### `FunctionDef.ensureArgumentsArgumentBinding` (`src/bytecode.zig:5651`)
...
### `FunctionDef.hasExplicitArgumentsVar` (`src/bytecode.zig:5692`)
...
### `FunctionDef.appendVar` (`src/bytecode.zig:5704`)
...
- **所有权 / 错误 / 调用**：分配只发生在 `growSliceBy`（`src/bytecode.zig:5055`）的几何增长里，backing 归 `FunctionDef`，由 `deinit` 的 `freeGrowableSlice` 释放；error set 推导下来只有 `error.OutOfMemory`，它不在此处变成 JS 异常，而是沿 parser 的 `Error` 上抛到 `src/exec/eval_entry.zig:93` 的 `try parser.compile`。 `var_name` 只是值拷贝——源码注释里的「The atom is duplicated」是 rc 时代遗留，tracing GC 下 atom 没有每槽引用计数（`freeOwnedAtomSlice` 的 `AtomTable` 参数已是 `_`，`src/bytecode.zig:11423`）。调用方：本结构体的六个 `ensure*Binding`（`:5558`/`:5577`/`:5595`/`:5607`/`:5619`/`:5637`）、`ensureArgumentsArgumentBinding`（`:5672`）、`addScopeVar`（`:5758`），finalize 的 `addEvalVariables` 建 var_object/arg_var_object 两行（`src/bytecode.zig:10484`/`:10492`，`catch return error.OutOfMemory`），以及 parser 的 `src/parser.zig:1708`/`:13554`；`src/compiler/` 下的调用全在 test 块内。


### `FunctionDef.appendGlobalVar` (`src/bytecode.zig:5714`)
...
### `FunctionDef.appendArg` (`src/bytecode.zig:5724`)
...
### `FunctionDef.addChild` (`src/bytecode.zig:5737`)
...
### `FunctionDef.addScopeVar` (`src/bytecode.zig:5746`)
...
### `FunctionDef.addClosureVar` (`src/bytecode.zig:5775`)
...
### `FunctionDef.captureBinding` (`src/bytecode.zig:5794`)
...
### `FunctionDef.captureLocal` (`src/bytecode.zig:5809`)
...
### `FunctionDef.captureArg` (`src/bytecode.zig:5815`)
...
### `FunctionDef.findVar` (`src/bytecode.zig:5823`)
...
### `FunctionDef.findArg` (`src/bytecode.zig:5832`)
...
### `FunctionDef.appendByteCode` (`src/bytecode.zig:5843`)

- **签名**：`pub fn appendByteCode(self: *FunctionDefImpl, bytes: []const u8) !void`。
- **作用**：往 FunctionDef 临时码追加字节。
- **实现**：几何增长。 内部 `try` 传播错误。 经几何增长缓冲追加。
- **所有权 / 错误 / 调用**：分配只发生在 `growSliceBy`（`src/bytecode.zig:5055`）的几何增长里，backing 归 `FunctionDef`，由 `deinit` 的 `freeGrowableSlice` 释放；error set 推导下来只有 `error.OutOfMemory`，它不在此处变成 JS 异常，而是沿 parser 的 `Error` 上抛到 `src/exec/eval_entry.zig:93` 的 `try parser.compile`。 空输入直接返回，不触碰缓冲。生产调用方两处，都在 parser：`src/parser.zig:3572`（`appendBytesNoSource`，phase-1 的无源码位置发射）与 `:15082`（合成类字段初始化调用）。


### `FunctionDef.appendSourceLoc` (`src/bytecode.zig:5849`)

- **签名**：`pub fn appendSourceLoc(self: *FunctionDefImpl, pc: u32, line_num: i32, col_num: i32) !void`。
- **作用**：追加 (pc,line,col) 槽。
- **实现**：`line_num <= 0 或 col_num <= 0` 直接丢弃；断言 pc 单调不减（phase-2 流式重映射 P2-R1T 的要求）；与码、atom 同一事务。 内部 `try` 传播错误。 经几何增长缓冲追加。
- **所有权 / 错误 / 调用**：分配同 `appendVar`（几何增长 + `error{OutOfMemory}`），`line_num`/`col_num` 非正时直接丢弃不记；断言要求 pc 单调不减（P2-R1T 流式重映射的前提）。**整棵树里没有调用方**——parser 的源码位置走 `v2_builder` 的 slot，`Bytecode` 侧的同名函数也只被测试助手用（`src/tests/parser.zig:6799`）。


### `FunctionDef.truncateSourceLocs` (`src/bytecode.zig:5863`)
...
### `FunctionDef.appendAtomOperand` (`src/bytecode.zig:5869`)
...
### `FunctionDef.reserveAtomOperands` (`src/bytecode.zig:5876`)

- **签名**：`pub fn reserveAtomOperands(self: *FunctionDefImpl, additional: usize) !void`。
- **作用**：只扩容不发布 atom。
- **实现**：与 AssumeCapacity 配对。 内部 `try` 传播错误。 经几何增长缓冲追加。
- **所有权 / 错误 / 调用**：只预留容量再把可见 `len` 收回原值，不写入任何 atom；分配与 `error{OutOfMemory}` 同 `appendVar`。**整棵树里没有调用方**：与之配对的 `appendAtomOperandAssumeCapacity` 的容量由别处保证，`Bytecode` 侧的同名函数同样无人调用。


### `FunctionDef.appendAtomOperandAssumeCapacity` (`src/bytecode.zig:5883`)

- **签名**：`pub fn appendAtomOperandAssumeCapacity(self: *FunctionDefImpl, atom_id: atom.Atom) void`。
- **作用**：已 reserve 后无检查追加。
- **实现**：调用方保证容量。
- **所有权 / 错误 / 调用**：不分配、无 error set；容量不足只有 `std.debug.assert` 挡（ReleaseFast 下会越界写）。唯一调用方是 parser 的 `State.appendAtomOperandAssumeCapacity`（`src/parser.zig:2886`），那是 `emit_to_function_def` 为真的那一臂；为假时改走 `Bytecode.retainAtomOperandAssumeCapacity`（`:2888`），两臂互斥。


### `FunctionDef.takeLastAtomOperand` (`src/bytecode.zig:5892`)

- **签名**：`pub fn takeLastAtomOperand(self: *FunctionDefImpl) atom.Atom`。
- **作用**：弹出最后一条 atom 操作数，调用方接管。
- **实现**：不释放 atom。
- **所有权 / 错误 / 调用**：不分配、无 error set；只收缩可见 `len` 并把最后一个 atom 交还调用方（doc 注释说「调用方负责转移或释放」，tracing GC 下实际没有可释放的引用）。**整棵树里没有调用方**，`Bytecode` 侧的同名函数亦然。


### `FunctionDef.appendCpool` (`src/bytecode.zig:5899`)
...
### `FunctionDef.appendCpoolOwned` (`src/bytecode.zig:5906`)

- **签名**：`pub fn appendCpoolOwned(self: *FunctionDefImpl, value: JSValue) !u32`。
- **作用**：接管所有权的常量追加。
- **实现**：同 appendCpool。 内部 `try` 传播错误。 经几何增长缓冲追加。
- **所有权 / 错误 / 调用**：函数体与 `appendCpool` 逐字相同——「Owned」只是 rc 时代的命名残留，两者都不改引用计数；分配、error set 与 GC 根同上。生产树里没有调用方，只有测试用（`src/bytecode.zig:11328` 的 test 块、`src/tests/bytecode.zig:1056`/`:1109`）。


### `FunctionDef.traceCompileRoots` (`src/bytecode.zig:5937`)
...
### `FunctionDef.truncateByteCode` (`src/bytecode.zig:5947`)
...
### `FunctionDef.truncateAtomOperands` (`src/bytecode.zig:5954`)

- **签名**：`pub fn truncateAtomOperands(self: *FunctionDefImpl, target_len: usize) void`。
- **作用**：截 atom_operands，保留 backing。
- **实现**：tracing 下不减 refcount。 循环扫描切片或字节码。
- **所有权 / 错误 / 调用**：不分配、不释放 backing、无 error set。注意 doc 注释写的「releasing the per-element atom refcounts」已经失真：循环体是空的（`src/bytecode.zig:5957`），tracing GC 下本来就没有每槽引用可放。调用方 `src/parser.zig:2840`（`rollbackEmission`）与 `:3811`（`truncateAtomOperands`）。


### `FunctionDef.deinit` (`src/bytecode.zig:5961`)
...
### `function_mod.growSliceBy` (`src/bytecode.zig:11385`)
...
- **所有权 / 错误 / 调用**：与 `function_def.growSliceBy`（`src/bytecode.zig:5055`）同形的私有副本；新块记在 `MemoryAccount`，backing 归调用者的 `Bytecode` 字段并由 `deinit` 的 `freeGrowableSlice` 释放，失败时旧 backing 不动。error set 只有 `error.OutOfMemory`。四个调用方都在 `BytecodeImpl`：`appendCode`（`:11828`）、`appendSourceLoc`（`:11838`）、`retainAtomOperand`（`:11929`）、`reserveAtomOperands`（`:11936`）。


### `function_mod.freeGrowableSlice` (`src/bytecode.zig:11410`)
...
- **所有权 / 错误 / 调用**：释放走 `MemoryAccount`（计入该编译单元的内存账），无 error set。调用方全在 `BytecodeImpl` 上：`deinit` 释放 `code` 与 `source_loc_slots`（`src/bytecode.zig:11643`-`11644`），`setCode`、`installCode`、`installCodeWithCapacity`、`installAtomOperands`、`installAtomOperandsWithCapacity` 在换缓冲前先释放旧块（`:11814`、`:11859`、`:11874`、`:11892`、`:11905`）。FunctionDef 侧另有一个同名同形的私有副本（`src/bytecode.zig:5113`），两者互不调用。


### `function_mod.freeOwnedAtomSlice` (`src/bytecode.zig:11423`)

- **签名**：`fn freeOwnedAtomSlice(_: *atom.AtomTable, mem: *memory.MemoryAccount, slot: *[]atom.Atom) void`。
- **作用**：释放 `BytecodeImpl` 自有的精确长度 atom 数组（`var_ref_names`），不涉及容量计数。
- **实现**：先把 `slot.*` 取出并清成空切片，再对非空的 `items` 调 `mem.free`；顺序保证释放期间槽已失效。首个 `*atom.AtomTable` 形参写成 `_` 被忽略——行内 atom 的引用不在这里归还，签名保留它只是为了与其余 `freeOwned*` 家族同形。
- **所有权 / 错误 / 调用**：内存经 `MemoryAccount` 归还，无 error set。唯一调用方是 `BytecodeImpl.deinit`（`src/bytecode.zig:11641`）。


### `function_mod.freeOwnedVarDefSlice` (`src/bytecode.zig:11429`)

- **签名**：`fn freeOwnedVarDefSlice(_: *atom.AtomTable, mem: *memory.MemoryAccount, slot: *[]function_bytecode_mod.BytecodeVarDef) void`。
- **作用**：释放 `BytecodeImpl` 自有的 `BytecodeVarDef` 行数组（形参表 `argdefs` 与局部表 `vardefs`）。
- **实现**：与 `freeOwnedAtomSlice` 同形：取出 `slot.*`、清空槽、非空才 `mem.free`；`*atom.AtomTable` 形参同样是被忽略的 `_`，行里的 `var_name` atom 不在此释放。
...
### `function_mod.freeOwnedClosureVarSlice` (`src/bytecode.zig:11435`)

- **签名**：`fn freeOwnedClosureVarSlice(_: *atom.AtomTable, mem: *memory.MemoryAccount, slot: *[]function_bytecode_mod.BytecodeClosureVar) void`。
- **作用**：释放 `BytecodeImpl` 自有的 `BytecodeClosureVar` 行数组（`closure_var`，即闭包/var-ref 拓扑表）。
- **实现**：同族三兄弟的第三个：取出、清空槽、非空才 `mem.free`；`*atom.AtomTable` 形参为 `_`，行里的 `var_name` 引用不在此归还。
...
### `function_mod.freeGrowableAtomSlice` (`src/bytecode.zig:11441`)

- **签名**：`fn freeGrowableAtomSlice( _: *atom.AtomTable, mem: *memory.MemoryAccount, slice: *[]atom.Atom, capacity: *usize, ) void`。
- **作用**：释放 `atom_operands` 这块几何增长的 atom 缓冲——它既可能由 `growSliceBy` 撑出容量，也可能被 pipeline 换成精确长度切片，所以两种形态都要能收。
- **实现**：先取出 `items` 与 `old_capacity` 并把槽清零，再二选一释放：`old_capacity != 0` 释放 `items.ptr[0..old_capacity]` 整块，否则退化成按 `items` 自身长度释放。这正是它与 `freeGrowableSlice` 的唯一区别（后者在 `capacity == 0` 时视作不拥有、什么都不放）。首个 `*atom.AtomTable` 形参为 `_`：操作数 atom 的引用由调用方显式处理。
- **所有权 / 错误 / 调用**：无 error set。调用方：`BytecodeImpl.deinit`（`src/bytecode.zig:11638`）与 FunctionDef 侧同名副本的调用点（`:5984`）互不相干。


### `Bytecode.init` (`src/bytecode.zig:11623`)
...
### `Bytecode.deinit` (`src/bytecode.zig:11634`)

- **签名**：`pub fn deinit(self: *BytecodeImpl, rt: anytype) void`。
- **作用**：释放 code/atom_operands/argdefs/vardefs/var_ref_names/closure_var/source_loc_slots/constants/module_record/debug_table/pc2line。
- **实现**：先把 name/filename/script_or_module 置 null_atom；几何缓冲按 capacity free，owned 切片按 len free；pc2line 只有 `owns_pc2line_buf` 为真才释放。
- **所有权 / 错误 / 调用**：rt 传给 `constants.deinit`（reserved BigInt）。


### `Bytecode.byteCode` (`src/bytecode.zig:11659`)
...
### `Bytecode.funcName` (`src/bytecode.zig:11662`)
...
### `Bytecode.argVarDefs` (`src/bytecode.zig:11665`)
...
### `Bytecode.varDefs` (`src/bytecode.zig:11668`)
...
### `Bytecode.closureVar` (`src/bytecode.zig:11671`)
...
### `Bytecode.cpoolSlice` (`src/bytecode.zig:11674`)
...
### `Bytecode.constantAt` (`src/bytecode.zig:11677`)
...
### `Bytecode.pc2lineBuf` (`src/bytecode.zig:11680`)
...
### `Bytecode.lineNum` (`src/bytecode.zig:11683`)
...
### `Bytecode.colNum` (`src/bytecode.zig:11686`)
...
### `Bytecode.scriptOrModule` (`src/bytecode.zig:11689`)
...
### `Bytecode.realmContext` (`src/bytecode.zig:11692`)
...
### `Bytecode.isGlobalVar` (`src/bytecode.zig:11695`)
...
### `Bytecode.isModule` (`src/bytecode.zig:11698`)
...
### `Bytecode.functionKind` (`src/bytecode.zig:11701`)
...
### `Bytecode.isDerivedClassConstructor` (`src/bytecode.zig:11711`)
...
### `Bytecode.hasPrototype` (`src/bytecode.zig:11714`)
...
### `Bytecode.hasSimpleParameterList` (`src/bytecode.zig:11717`)
...
### `Bytecode.needHomeObject` (`src/bytecode.zig:11720`)
...
### `Bytecode.newTargetAllowed` (`src/bytecode.zig:11723`)
...
### `Bytecode.superCallAllowed` (`src/bytecode.zig:11726`)
...
### `Bytecode.superAllowed` (`src/bytecode.zig:11729`)
...
### `Bytecode.argumentsAllowed` (`src/bytecode.zig:11732`)
...
### `Bytecode.isDirectOrIndirectEval` (`src/bytecode.zig:11735`)
...
### `Bytecode.isAsync` (`src/bytecode.zig:11738`)
...
### `Bytecode.isGenerator` (`src/bytecode.zig:11741`)
...
### `Bytecode.entryContract` (`src/bytecode.zig:11744`)
...
### `Bytecode.isStrictMode` (`src/bytecode.zig:11747`)
...
### `Bytecode.runtimeStrictMode` (`src/bytecode.zig:11750`)
...
### `Bytecode.hasMappedArguments` (`src/bytecode.zig:11753`)
...
### `Bytecode.simpleInlineEligible` (`src/bytecode.zig:11756`)
...
### `Bytecode.strictSimpleInlineEligible` (`src/bytecode.zig:11759`)
...
### `Bytecode.strictSimpleSnapshotInlineEligible` (`src/bytecode.zig:11762`)
...
### `Bytecode.simpleInlineEmptyLeaf` (`src/bytecode.zig:11765`)
...
### `Bytecode.rawThisInlineEmptyLeaf` (`src/bytecode.zig:11768`)
...
### `Bytecode.simpleInlineExactArgsLeaf` (`src/bytecode.zig:11771`)
...
### `Bytecode.rawThisInlineExactArgsLeaf` (`src/bytecode.zig:11774`)
...
### `Bytecode.exactArgsLeafKind` (`src/bytecode.zig:11777`)
...
### `Bytecode.captureLeafKind` (`src/bytecode.zig:11780`)

- **签名**：`pub inline fn captureLeafKind(self: *const BytecodeImpl) function_bytecode_mod.ExactArgsLeafKind`。
- **作用**：零参捕获叶的分类（none / sloppy / raw_this）。
- **实现**：返回 `self.capture_leaf_kind`。
- **所有权 / 错误 / 调用**：纯字段读、不分配、无 error set。整棵树里没有调用方：该分类在 `publishExecutionFlags`（`src/bytecode.zig:12233`）写进 `CallFacts` 之后，消费者读的是 `FunctionBytecode.captureLeafKind()` / `call_facts.execution.capture_leaf_kind`。


### `Bytecode.varRefIsLexicalAt` (`src/bytecode.zig:11788`)
...
### `Bytecode.varRefIsConstAt` (`src/bytecode.zig:11792`)
...
### `Bytecode.varRefIsGlobalDeclAt` (`src/bytecode.zig:11796`)
...
### `Bytecode.varRefNamesLen` (`src/bytecode.zig:11805`)
...
### `Bytecode.varRefName` (`src/bytecode.zig:11808`)
...
### `Bytecode.setCode` (`src/bytecode.zig:11813`)
...
### `Bytecode.appendCode` (`src/bytecode.zig:11826`)

- **签名**：`pub fn appendCode(self: *BytecodeImpl, bytes: []const u8) !void`。
- **作用**：几何增长追加最终/中间码。
- **实现**：len 是已用。 内部 `try` 传播错误。 经几何增长缓冲追加。
- **所有权 / 错误 / 调用**：经 `growSliceBy` 几何增长 `MemoryAccount` 上的 `code` 缓冲，backing 归 `Bytecode`（`deinit` 的 `freeGrowableSlice`）；error set 只有 `error.OutOfMemory`，空输入直接返回。唯一生产调用方是 parser 的 `appendBytesNoSource`（`src/parser.zig:3574`），它同时也写 `FunctionDef.appendByteCode`。


### `Bytecode.appendSourceLoc` (`src/bytecode.zig:11832`)
...
### `Bytecode.truncateSourceLocs` (`src/bytecode.zig:11843`)
...
### `Bytecode.truncateCode` (`src/bytecode.zig:11850`)
...
### `Bytecode.installCode` (`src/bytecode.zig:11858`)

- **签名**：`pub fn installCode(self: *BytecodeImpl, owned: []u8) void`。
- **作用**：接管精确拟合的新 code 切片，释放旧。
- **实现**：resolve_variables 整缓冲重写。
- **所有权 / 错误 / 调用**：所有权转移：接管 `owned`（`code_capacity` 记成 `owned.len`），并先 `freeGrowableSlice` 释放旧 backing；无 error set。生产树内无调用方——pipeline 走的是带容量的变体；本名只被 `src/tests/bytecode.zig:3033` 用来验证旧缓冲确实被释放。


### `Bytecode.installCodeWithCapacity` (`src/bytecode.zig:11871`)

- **签名**：`pub fn installCodeWithCapacity(self: *BytecodeImpl, owned_used: []u8, owned_capacity: usize) void`。
- **作用**：接管 used+capacity 分离的 code。
- **实现**：所有权转移。
- **所有权 / 错误 / 调用**：所有权转移的主用变体：接管 `owned_used` 所在的整块 backing（`owned_capacity` 个元素），先释放旧 backing，此后 `deinit`/`setCode`/`installCode` 都按 `code.ptr[0..code_capacity]` 释放——所以 `owned_used.len == 0` 时调用方必须传 `backing.ptr[0..0]` 让指针仍指向 backing 头。无 error set，两条前置由断言守。唯一生产调用方是 resolve_labels 的 `commit`（`src/compiler/resolve_labels.zig:3686`），那是「全部操作都不分配、不可观测半装」的移交点。


### `Bytecode.installPc2Line` (`src/bytecode.zig:11879`)
...
### `Bytecode.installAtomOperands` (`src/bytecode.zig:11891`)

- **签名**：`pub fn installAtomOperands(self: *BytecodeImpl, owned: []atom.Atom) void`。
- **作用**：接管精确 atom ledger。
- **实现**：不在这里减旧 atom ref。
- **所有权 / 错误 / 调用**：接管 `owned` 并释放旧 backing；旧条目持有的 atom 引用**不**在这里释放（tracing GC 下也没有可释放的引用）。无 error set。生产树内无调用方，只有 `src/tests/bytecode.zig:3034`；pipeline 用的是带容量的变体。


### `Bytecode.installAtomOperandsWithCapacity` (`src/bytecode.zig:11902`)
...
### `Bytecode.addConstant` (`src/bytecode.zig:11910`)
...
### `Bytecode.traceCompileRoots` (`src/bytecode.zig:11921`)
...
### `Bytecode.retainAtomOperand` (`src/bytecode.zig:11928`)

- **签名**：`pub fn retainAtomOperand(self: *BytecodeImpl, atom_id: atom.Atom) !void`。
- **作用**：追加 atom 操作数（可变根 Bytecode）。
- **实现**：几何增长；名字保留 rc 时代语义，tracing 下并不增引用。 内部 `try` 传播错误。 经几何增长缓冲追加。
- **所有权 / 错误 / 调用**：几何增长 + `error{OutOfMemory}`；名字保留 rc 时代语义，实际只写入 atom 值、不提引用。生产树内无调用方，只有测试助手 `publishPhase1Stream`（`src/tests/parser.zig:6797`）；生产 phase-1 走的是下面的 assume-capacity 变体。


### `Bytecode.reserveAtomOperands` (`src/bytecode.zig:11933`)

- **签名**：`pub fn reserveAtomOperands(self: *BytecodeImpl, additional: usize) !void`。
- **作用**：只扩容不发布 atom。
- **实现**：与 AssumeCapacity 配对。 内部 `try` 传播错误。 经几何增长缓冲追加。
- **所有权 / 错误 / 调用**：只预留容量再把可见 `len` 收回，不写任何 atom；分配与 `error{OutOfMemory}` 同 `retainAtomOperand`。**整棵树里没有调用方**（`FunctionDef` 侧的同名函数亦然），assume-capacity 追加的容量由调用方在别处保证。


### `Bytecode.retainAtomOperandAssumeCapacity` (`src/bytecode.zig:11940`)

- **签名**：`pub fn retainAtomOperandAssumeCapacity(self: *BytecodeImpl, atom_id: atom.Atom) void`。
- **作用**：已 reserve 后追加。
- **实现**：无检查。
- **所有权 / 错误 / 调用**：不分配、无 error set；容量不足只有断言挡。唯一调用方是 parser 的 `State.appendAtomOperandAssumeCapacity`（`src/parser.zig:2888`），与 `FunctionDef.appendAtomOperandAssumeCapacity`（`:2886`）在同一处成对写入两份 ledger。


### `Bytecode.takeLastAtomOperand` (`src/bytecode.zig:11948`)

- **签名**：`pub fn takeLastAtomOperand(self: *BytecodeImpl) atom.Atom`。
- **作用**：弹出最后一条 atom 操作数，调用方接管。
- **实现**：不释放 atom。
- **所有权 / 错误 / 调用**：不分配、无 error set，只收缩可见 `len` 并返回最后一个 atom。**整棵树里没有调用方**；`FunctionDef` 侧的同名函数同样无人调用。


### `Bytecode.truncateAtomOperands` (`src/bytecode.zig:11957`)

- **签名**：`pub fn truncateAtomOperands(self: *BytecodeImpl, target_len: usize) void`。
- **作用**：截 atom_operands，保留 backing。
- **实现**：tracing 下不减 refcount。 循环扫描切片或字节码。
- **所有权 / 错误 / 调用**：不分配、不释放 backing、无 error set。和 `FunctionDef` 侧一样，doc 注释说的「releasing the per-element atom refcounts」已失真——循环体是空的（`src/bytecode.zig:11960`）。调用方是 parser 的两个回滚点：`src/parser.zig:2844`（`rollbackEmission`）与 `:3814`。


### `Bytecode.localOpenBindingIndex` (`src/bytecode.zig:11964`)
...
### `Bytecode.argOpenBindingIndex` (`src/bytecode.zig:11970`)
...
### `Bytecode.ensureModule` (`src/bytecode.zig:11976`)
...
### `Bytecode.ensureDebug` (`src/bytecode.zig:11981`)

- **签名**：`pub fn ensureDebug(self: *BytecodeImpl, filename: atom.Atom) *debug.Table`。
- **作用**：懒创建 debug.Table。
- **实现**：filename atom。 返回 `&self.debug_table.?`。
- **所有权 / 错误 / 调用**：懒建 `debug.Table`（带 filename atom）存进 `self.debug_table`，由 `Bytecode` 拥有、`deinit`（`src/bytecode.zig:11655`）销毁；返回借用指针，`Table.init` 不分配故无 error set。生产树内无调用方——生产 debug 信息走 `DebugInfo` 尾与 pc2line 缓冲；本名只被 `src/tests/bytecode.zig:134` 用。


### `LegacyExecutionAdapter.init` (`src/bytecode.zig:12005`)
...
### `function_mod.codeMaterializesArgumentsObject` (`src/bytecode.zig:12068`)
...
### `function_mod.scanSmallInlineEligible` (`src/bytecode.zig:12093`)
...
### `function_mod.classifyAsyncExecution` (`src/bytecode.zig:12140`)
...
### `function_mod.publishExecutionFlags` (`src/bytecode.zig:12161`)
...
- 清单函数数（本文件分组）: 125（`src/bytecode.zig` 全文件 538）
- 本文标题覆盖: 125
- 未覆盖: 无
