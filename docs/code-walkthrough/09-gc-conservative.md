...
### `Table.arenasIncomplete` (`src/core/gc_address_registry.zig:139`)
...
### `Table.setArenasIncomplete` (`src/core/gc_address_registry.zig:143`)
...
### `Table.occupantsIncomplete` (`src/core/gc_address_registry.zig:151`)
...
### `Table.setOccupantsIncomplete` (`src/core/gc_address_registry.zig:155`)
...
### `Table.noteArenaCreated` (`src/core/gc_address_registry.zig:172`)
...
### `Table.resyncArenas` (`src/core/gc_address_registry.zig:193`)
...
### `Table.Sync.visit` (`src/core/gc_address_registry.zig:198`)
...
### `Table.noteArenaReleased` (`src/core/gc_address_registry.zig:218`)
...
### `Table.forEachGcObjectInArena` (`src/core/gc_address_registry.zig:234`)
...
### `Table.auditArenas` (`src/core/gc_address_registry.zig:273`)
...
### `Table.Audit.visit` (`src/core/gc_address_registry.zig:278`)
...
### `Table.verifyIndex` (`src/core/gc_address_registry.zig:315`)
...
### `Table.rebuildScanFilter` (`src/core/gc_address_registry.zig:389`)
...
### `Table.forEachGcObjectInBlocks` (`src/core/gc_address_registry.zig:436`)
...
### `Table.filterRulesOut` (`src/core/gc_address_registry.zig:468`)
...
### `Table.filterRulesOutBits` (`src/core/gc_address_registry.zig:472`)
...
### `Table.deinit` (`src/core/gc_address_registry.zig:476`)
...
### `Table.occupantFor` (`src/core/gc_address_registry.zig:487`)
...
### `Table.insert` (`src/core/gc_address_registry.zig:494`)
...
### `Table.insertOccupant` (`src/core/gc_address_registry.zig:498`)
...
### `Table.remove` (`src/core/gc_address_registry.zig:526`)
...
### `Table.removePtr` (`src/core/gc_address_registry.zig:530`)
...
### `Table.compactIfTombstoned` (`src/core/gc_address_registry.zig:558`)
...
### `Table.forEachTraceCandidateAt` (`src/core/gc_address_registry.zig:585`)
...
### `Table.containsHeader` (`src/core/gc_address_registry.zig:654`)
...
### `Table.rollbackPages` (`src/core/gc_address_registry.zig:687`)
...
### `Table.noteFailedInsert` (`src/core/gc_address_registry.zig:713`)
...
### `occupantsEqual` (`src/core/gc_address_registry.zig:718`)
...
`SpillImage`：AArch64 为 768 字节，31 个 GPR 后有 8 字节 padding，SIMD 从偏移 256 开始；x86_64 为 384 字节，15 个已写 GPR 槽（偏移 0..112）后保留偏移 120 的空槽，XMM 从偏移 128 开始。两者对齐 16；缓冲初始 undefined，汇编不会写 padding。Metrics.candidates 仅累计扫描机器字数。栈高及失败结果按线程缓存；OS 查询不可用时才回退 runtime 的 native_stack_top。
...
### `linux_stack.stackHigh` (`src/core/gc_conservative.zig:87`)
...
### `stackHigh` (`src/core/gc_conservative.zig:98`)
...
### `darwin_stack.stackHigh` (`src/core/gc_conservative.zig:106`)
...
### `stackHigh` (`src/core/gc_conservative.zig:112`)
...
### `windows_limits.stackHigh` (`src/core/gc_conservative.zig:123`)
...
### `stackHigh` (`src/core/gc_conservative.zig:130`)
...
### `threadStackHigh` (`src/core/gc_conservative.zig:149`)
...
### `scanHigh` (`src/core/gc_conservative.zig:166`)
...
### `dumpRegisters` (`src/core/gc_conservative.zig:175`)
...
### `scanWords` (`src/core/gc_conservative.zig:256`)
...
### `spillRegistersAndScan` (`src/core/gc_conservative.zig:287`)
...
### `diagCurrentWord` (`src/core/gc_conservative.zig:337`)
...
### `diagCaptureFrames` (`src/core/gc_conservative.zig:381`)
...
### `diagOwnerPcs` (`src/core/gc_conservative.zig:411`)
...
### `diagRegisterName` (`src/core/gc_conservative.zig:437`)

- **签名**：`fn diagRegisterName(index: usize, buf: []u8) []const u8`。
- **作用**：为转储 image 的机器字槽生成诊断名称。
- **实现**：AArch64 槽0..30为x0..x30，31为pad，其余按两字一个q寄存器输出lo/hi。x86_64 使用固定16项名称表（槽0..14 为 rax/rbx/rcx/rdx/rsi/rdi/rbp/r8..r15，槽15 为未写的 pad），随后按两字一个xmm输出lo/hi。格式化缓冲不足返回 "?"。
- **所有权 / 错误 / 调用**：动态名称借用 buf，固定名称为静态字符串；不校验 index 上界。x86 名称表已与 dumpRegisters 的实际写入顺序对齐（r8 在槽7/偏移56，未写的 padding 是槽15/偏移120）。

### `StabilityEntry.kindCount` (`src/core/gc_conservative.zig:507`)
...
### `diagThreadId` (`src/core/gc_conservative.zig:516`)
...
### `RootsDiagCensus.offsetBucket` (`src/core/gc_conservative.zig:571`)
...
### `RootsDiagCensus.offsetBucketName` (`src/core/gc_conservative.zig:579`)
...
### `RootsDiagCensus.hashKey` (`src/core/gc_conservative.zig:710`)
...
### `RootsDiagCensus.currentFunction` (`src/core/gc_conservative.zig:716`)
...
### `RootsDiagCensus.internName` (`src/core/gc_conservative.zig:729`)
...
### `RootsDiagCensus.internPcSilent` (`src/core/gc_conservative.zig:746`)
...
### `RootsDiagCensus.internPc` (`src/core/gc_conservative.zig:762`)
...
### `RootsDiagCensus.classify` (`src/core/gc_conservative.zig:768`)
...
### `RootsDiagCensus.noteSite` (`src/core/gc_conservative.zig:843`)
...
### `RootsDiagCensus.entryVerdict` (`src/core/gc_conservative.zig:900`)
...
### `RootsDiagCensus.apply` (`src/core/gc_conservative.zig:935`)
...
### `RootsDiagCensus.nameAt` (`src/core/gc_conservative.zig:972`)
...
### `RootsDiagCensus.percent` (`src/core/gc_conservative.zig:977`)
...
### `RootsDiagCensus.slotName` (`src/core/gc_conservative.zig:982`)
...
### `RootsDiagCensus.mergeSite` (`src/core/gc_conservative.zig:989`)
...
### `RootsDiagCensus.siteCountFor` (`src/core/gc_conservative.zig:1005`)
...
### `RootsDiagCensus.writeTopSites` (`src/core/gc_conservative.zig:1023`)
...
### `RootsDiagCensus.reportVerdicts` (`src/core/gc_conservative.zig:1059`)
...
### `RootsDiagCensus.Sorter.frameTotal` (`src/core/gc_conservative.zig:1092`)
...
### `RootsDiagCensus.Sorter.more` (`src/core/gc_conservative.zig:1095`)
...
### `RootsDiagCensus.writeOwnerPc` (`src/core/gc_conservative.zig:1130`)
...
### `RootsDiagCensus.report` (`src/core/gc_conservative.zig:1145`)
...
### `RootsDiagCensus.moreHits` (`src/core/gc_conservative.zig:1184`)
...
### `RootsDiagCensus.moreHits` (`src/core/gc_conservative.zig:1216`)
...
### `GlobalLock.lock` (`src/core/gc_conservative.zig:1264`)
...
### `GlobalLock.unlock` (`src/core/gc_conservative.zig:1270`)
...
### `diagThreadDirect` (`src/core/gc_conservative.zig:1281`)
...
### `noteDirect` (`src/core/gc_conservative.zig:1287`)
...
### `noteProbe` (`src/core/gc_conservative.zig:1299`)
...
### `reportGlobal` (`src/core/gc_conservative.zig:1308`)
...
### `diagVerdictForHeader` (`src/core/gc_conservative.zig:1321`)
...
### `diagCensusSnapshot` (`src/core/gc_conservative.zig:1338`)
...
### `shade.f` (`src/core/gc_conservative.zig:1349`)
...
### `Probe.shade` (`src/core/gc_conservative.zig:1383`)
...
### `diagRescanFixedSlot` (`src/core/gc_conservative.zig:1436`)
...
### `Probe.shade` (`src/core/gc_conservative.zig:1441`)
