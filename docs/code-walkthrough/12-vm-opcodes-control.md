...
### `returnTop` (`src/exec/vm_opcodes.zig:25`)
...
### `returnUndefined` (`src/exec/vm_opcodes.zig:41`)
...
### `finishFunctionReturn` (`src/exec/vm_opcodes.zig:48`)
...
### `jump32` (`src/exec/vm_opcodes.zig:56`)
...
### `jump16` (`src/exec/vm_opcodes.zig:62`)
...
### `jump8` (`src/exec/vm_opcodes.zig:68`)
...
### `branch32` (`src/exec/vm_opcodes.zig:74`)
...
### `branch8` (`src/exec/vm_opcodes.zig:85`)
...
### `throwTop` (`src/exec/vm_opcodes.zig:96`)
...
### `createAtomError` (`src/exec/vm_opcodes.zig:123`)
...
### `createThrowErrorValue` (`src/exec/vm_opcodes.zig:142`)
...
### `deliverPendingThrow` (`src/exec/vm_opcodes.zig:158`)
...
### `throwErrorVm` (`src/exec/vm_opcodes.zig:171`)
...
### `catchTarget` (`src/exec/vm_opcodes.zig:196`)
...
### `gosub` (`src/exec/vm_opcodes.zig:205`)
...
### `ret` (`src/exec/vm_opcodes.zig:214`)
...
### `relativePc` (`src/exec/vm_opcodes.zig:223`)
...
### `adapterValueBorrow` (`src/exec/vm_opcodes.zig:227`)
...
### `varRefCellFromValue` (`src/exec/vm_opcodes.zig:236`)

- **签名**：`fn varRefCellFromValue(value: core.JSValue) ?*core.VarRef`。
- **作用**：薄包装 `VarRef.fromValue`。
- **实现**：一行转发。
- **所有权 / 错误 / 调用**：只做 tag 判读，返回的是**借用** `*core.VarRef`（指向 GC 堆里的 cell），不 retain、不建根，也不分配；无 error set，非 VarRef 值返回 null。文件私有，两处调用：`src/exec/vm_opcodes.zig:244` 解一层 cell，`:247` 是 Debug 断言「解出来的值不会再是一层 cell」——即不存在 VarRef 套 VarRef。

### `readInt` (`src/exec/vm_opcodes.zig:240`)

- **签名**：`fn readInt(comptime T: type, bytes: []const u8) T`。
- **作用**：小端读跳转立即数。
- **实现**：`std.mem.readInt`。
- **所有权 / 错误 / 调用**：纯读借用的字节码切片，不分配、无 error set；定长由调用点的 `[0..N]` 切片保证。文件私有，调用方是本文件的跳转/分支立即数解码：`src/exec/vm_opcodes.zig:64`、`:70`、`:82`、`:214`、`:223`。注意 `:196` 读 atom 时直接用了 `std.mem.readInt` 而没走这个包装。
