...
- **所有权 / 错误 / 调用**：弹出的 iterable 由本函数消费。同步路径把 iterator、next、marker 依次压成三槽记录——注意 TGC 下 `stack.pushOwned` 与 `stack.push` 是同一份实现（`stack.zig:214`/`:220` 都只是 reserve + 写槽），「owned」只剩文档语义，没有运行时动作；失败时靠链式 `errdefer` 把已压的槽弹回，不留半截记录。局部 `owns_iterator_value` 标志只标注意图，树内没有任何 `errdefer` 读它（RC 时代残留）。async 路径里 sync iterator 的引用移交给 `createAsyncFromSyncIterator` 造的 wrapper 槽。错误：不可迭代时先 `exception_ops.throwTypeErrorMessage(ctx, global, "value is not iterable")` 挂上消息再返回 `error.TypeError`；其余是 Get `@@asyncIterator`/`@@iterator`、调用迭代器方法、`expectObject` 的透传（用户异常、`error.TypeError`、OOM）。唯一调用方 `iterator_ops.zig:104`（`forOfStartVm`）。

### `forOfStartVm` (`src/exec/iterator_ops.zig:94`)
...
### `catchTargetMarkerValue` (`src/exec/iterator_ops.zig:111`)
...
### `iteratorNextMethod` (`src/exec/iterator_ops.zig:115`)
...
### `pushForAwaitRecord` (`src/exec/iterator_ops.zig:129`)
...
### `createAsyncFromSyncIterator` (`src/exec/iterator_ops.zig:147`)
...
### `testAsyncFromSyncIsCallable` (`src/exec/iterator_ops.zig:206`)
...
### `asyncFromSyncMethod` (`src/exec/iterator_ops.zig:262`)
...
### `defineValueProperty` (`src/exec/iterator_ops.zig:271`)
...
### `forInStart` (`src/exec/iterator_ops.zig:275`)
...
### `forInStartVm` (`src/exec/iterator_ops.zig:287`)
...
### `iteratorNext` (`src/exec/iterator_ops.zig:302`)
...
### `iteratorNextVm` (`src/exec/iterator_ops.zig:325`)
...
### `iteratorCheckObject` (`src/exec/iterator_ops.zig:341`)
...
### `iteratorCheckObjectVm` (`src/exec/iterator_ops.zig:347`)
...
### `forAwaitOfNext` (`src/exec/iterator_ops.zig:362`)
...
### `forAwaitOfNextVm` (`src/exec/iterator_ops.zig:382`)
...
### `iteratorGetValueDone` (`src/exec/iterator_ops.zig:398`)
...
### `iteratorGetValueDoneVm` (`src/exec/iterator_ops.zig:425`)
...
### `iteratorCall` (`src/exec/iterator_ops.zig:441`)
...
### `iteratorCallVm` (`src/exec/iterator_ops.zig:476`)
...
### `forOfIteratorIndex` (`src/exec/iterator_ops.zig:496`)
...
### `forOfNext` (`src/exec/iterator_ops.zig:507`)
...
### `finishForOfNextResult` (`src/exec/iterator_ops.zig:551`)
...
### `iteratorResultProperty` (`src/exec/iterator_ops.zig:610`)
...
### `fastArrayForOfNext` (`src/exec/iterator_ops.zig:637`)
...
### `fastMapSetForOfNext` (`src/exec/iterator_ops.zig:701`)
...
### `fastGeneratorForOfNext` (`src/exec/iterator_ops.zig:756`)
...
### `buildCollectionEntryPair` (`src/exec/iterator_ops.zig:789`)
...
### `finishMapSetForOfDone` (`src/exec/iterator_ops.zig:808`)
...
### `forOfNextVm` (`src/exec/iterator_ops.zig:821`)
...
### `forInNext` (`src/exec/iterator_ops.zig:844`)
...
### `pushForInDone` (`src/exec/iterator_ops.zig:924`)
...
### `forInPrepareProtoChainEnum` (`src/exec/iterator_ops.zig:934`)
...
### `forInNextVm` (`src/exec/iterator_ops.zig:986`)
...
### `iteratorClose` (`src/exec/iterator_ops.zig:1001`)
...
### `iteratorCloseVm` (`src/exec/iterator_ops.zig:1026`)
...
### `arrayIteratorPrototypeFromContext` (`src/exec/iterator_ops.zig:1041`)
...
### `arrayIteratorMethod` (`src/exec/iterator_ops.zig:1083`)
...
### `arrayIteratorNext` (`src/exec/iterator_ops.zig:1114`)
...
### `arrayIteratorValue` (`src/exec/iterator_ops.zig:1143`)
...
### `testArrayIteratorGetValueProperty` (`src/exec/iterator_ops.zig:1180`)
...
### `iteratorPrototypeFromGlobal` (`src/exec/iterator_ops.zig:1241`)
...
### `defineToStringTag` (`src/exec/iterator_ops.zig:1254`)
...
### `iteratorPrototype` (`src/exec/iterator_ops.zig:1260`)
...
### `iteratorPrototypeAccessor` (`src/exec/iterator_ops.zig:1277`)
...
### `iteratorPrototypeAccessorSet` (`src/exec/iterator_ops.zig:1308`)
...
### `iteratorFromCall` (`src/exec/iterator_ops.zig:1338`)
...
### `installIteratorHelperMethod` (`src/exec/iterator_ops.zig:1364`)
...
### `iteratorMethodsPrototype` (`src/exec/iterator_ops.zig:1374`)
...
### `iteratorHelperPrototype` (`src/exec/iterator_ops.zig:1393`)
...
### `iteratorConcatCall` (`src/exec/iterator_ops.zig:1400`)
...
### `testIteratorConcatArrayPrototypeFromGlobal` (`src/exec/iterator_ops.zig:1445`)
...
### `testIteratorConcatGetIteratorMethod` (`src/exec/iterator_ops.zig:1451`)
