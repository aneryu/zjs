...
### `toPrimitiveForAdditionFree` (`src/exec/coercion_ops.zig:27`)
...
- **作用**：ToPrimitive（default hint），对照 `JS_ToPrimitiveFree`：**消费**入参。
- **实现**：非对象（加法热路径的 int/float）直接原样返回，不 dup 不 free；只有对象落到 outlined 的 `toPrimitiveForAdditionObject`。
- **所有权 / 错误 / 调用**：tracing GC 下 dup/free 都是空操作，实际等于借用入参、返回值归 GC。本身不产生错误：所有错误来自对象臂的 `toPrimitiveForAdditionObject`，其 TypeError 已由 `throwTypeErrorMessage` 挂成 pending exception，上层只见哨兵。7 处调用全在 `src/exec/vm_arith.zig`（`:539`、`:542`、`:590`、`:656` 等加法/比较慢路径）。


### `toPrimitiveForAddition` (`src/exec/coercion_ops.zig:39`)

- **签名**：`pub inline fn toPrimitiveForAddition( ctx: *core.JSContext, output: ?*std.Io.Writer, global: *core.Object, value: core.JSValue, ) !core.JSValue`。
- **作用**：给持借用值的调用方用的包装。
- **实现**：函数体就是转调 `toPrimitiveForAdditionFree`（注释说的「先 dup」在当前 tracing-GC 下已无实际操作）。
- **所有权 / 错误 / 调用**：借用入参、返回值归 GC，自身不分配也不产生错误，全部透传被包装者。6 处调用全在 `src/exec/vm_arith.zig`（`:61`、`:62`、`:822`、`:826` 等）。


### `toPrimitiveForAdditionObject` (`src/exec/coercion_ops.zig:48`)
...
### `toPrimitiveForNumber` (`src/exec/coercion_ops.zig:72`)
...
### `toOrdinaryPrimitive` (`src/exec/coercion_ops.zig:97`)
...
### `toOrdinaryPrimitiveNumber` (`src/exec/coercion_ops.zig:109`)
...
### `valueTruthy` (`src/exec/coercion_ops.zig:121`)
...
### `toUint16CodeUnit` (`src/exec/coercion_ops.zig:125`)
...
### `toLengthIndex` (`src/exec/coercion_ops.zig:132`)
...
### `toLengthIndexInto` (`src/exec/coercion_ops.zig:141`)
...
### `toLengthIndexSlow` (`src/exec/coercion_ops.zig:155`)
...
### `toLengthNumber` (`src/exec/coercion_ops.zig:161`)
...
### `fastToLengthIndex` (`src/exec/coercion_ops.zig:176`)
...
### `toUint32Number` (`src/exec/coercion_ops.zig:191`)
...
### `uint32NumberValue` (`src/exec/coercion_ops.zig:198`)
...
### `coerceOptionalNumberMethodArgument` (`src/exec/coercion_ops.zig:203`)
...
### `primitiveWrapperStoredValue` (`src/exec/coercion_ops.zig:221`)

- **签名**：`pub fn primitiveWrapperStoredValue(rt: *core.JSRuntime, value: core.JSValue) ?core.JSValue`。
- **作用**：取包装对象里存着的那个原始值。
- **实现**：非对象或 `expectObject` 失败返回 null；只认 `number` / `boolean` / `big_int` / `symbol` 四个 class（String 包装不在内），命中才取 `object.objectData()`，取不到也返回 null。`rt` 参数未使用。
- **所有权 / 错误 / 调用**：返回的是包装对象内部槽里的**借用**值（`object.objectData()`），不 dup、不建根，调用方不得释放；`rt` 参数未使用。无 error set：`property_ops.expectObject` 的失败被 `catch return null` 吞掉，非包装类一律 `null`。调用方 `src/exec/json_ops.zig:2432`（把 `null` 转成 `error.TypeError`）；`src/exec/construct.zig:749` 另有一份同名的文件私有副本。

### `toNumberForDateMethod` (`src/exec/coercion_ops.zig:235`)
...
- **实现**：对象分支：`toPrimitiveForNumber` 后是 BigInt → TypeError `"cannot convert bigint to number"`（quickjs.c:12959），否则 `value_ops.toNumberValue`。非对象分支：BigInt 同样 TypeError，其余直接 `toNumberValue`。`caller_function` / `caller_frame` 未使用。
...
- **签名**：`pub fn length(_: *core.JSRuntime, value: core.JSValue) !core.JSValue`。
...
- **所有权 / 错误 / 调用**：`rt` 参数未使用。字符串臂读 rope 节点长度，不物化不分配；数组臂读 `arrayLength()`；一般对象臂走 `core.Object.getProperty`，返回的是属性槽里的**借用**值，且该读路径不触发 accessor（accessor 返回的是 getter 函数本身），因此不会重入 JS。`null`/`undefined` 返回裸 `error.TypeError` 哨兵，`getProperty` 的 `error.ReferenceError`（未初始化 var_ref）也可能上浮。调用方 `src/exec/object_ops.zig:2517`、`:2609`。
...
### `typeOf` (`src/exec/value_ops.zig:305`)

- **签名**：`pub fn typeOf(rt: *core.JSRuntime, value: core.JSValue) !core.JSValue`。
- **作用**：`typeof` 的字符串结果。
- **实现**：按顺序判：bigint / number / boolean / string / undefined / `isHTMLDDA`（也报 `"undefined"`）/ symbol / null（报 `"object"`）/ 直接的 function bytecode / `isFunctionObject` 或 `proxyTargetIsFunction`（报 `"function"`）/ 兜底 `"object"`，最后 `createStringValue`。
- **所有权 / 错误 / 调用**：每次调用都用 `createStringValue` 新建一个 GC 字符串（错误只有 OOM）。**树内无调用方**：VM 的 `typeof` 走 `src/exec/vm_value.zig:182` 的 atom 版本（解析成预定义 atom 后取其缓存字符串，零分配），本函数只剩 pub API 形态。

### `logical` (`src/exec/value_ops.zig:331`)

- **签名**：`pub fn logical(op: u8, a: core.JSValue, b: core.JSValue) core.JSValue`。
- **作用**：`&&` / `||` / `??` 的取值（返回操作数本身，不装箱布尔）。
- **实现**：`and`：a 真取 b 否则 a；`or`：a 真取 a 否则 b；`is_undefined_or_null`：a 是 null/undefined 取 b 否则 a；其他 op `unreachable`。
- **所有权 / 错误 / 调用**：无：按真值选中并返回入参之一（借用，不 dup），不分配、无 error set。**树内无调用方**：`&&`/`||`/`??` 在 VM 里是跳转，不经过这个值级 helper。

### `toStringValue` (`src/exec/value_ops.zig:341`)
...
### `primitiveToStringValueFast` (`src/exec/value_ops.zig:349`)
...
### `fastStringToInt32` (`src/exec/value_ops.zig:379`)
...
### `numberToValue` (`src/exec/value_ops.zig:451`)
...
### `createStringValue` (`src/exec/value_ops.zig:464`)
...
### `createAsciiStringValue` (`src/exec/value_ops.zig:476`)
...
### `createBigIntI128` (`src/exec/value_ops.zig:504`)
...
### `createBigIntOwned` (`src/exec/value_ops.zig:512`)

- **签名**：`pub fn createBigIntOwned(rt: *core.JSRuntime, value: bignum.BigInt) !core.JSValue`。
...
- **所有权 / 错误 / 调用**：**消费**入参 `bignum.BigInt`：能压成 short 时立刻 `owned.deinit()` 并返回立即数，否则 `createFromOwned` 接管其 limb 缓冲；失败路径由 `errdefer owned.deinit()` 兜底。调用方交出后不得再 `deinit`——这正是它与 `createBigIntValue` 的分工。标 `pub` 但树内只有本文件两个调用方：`binaryBigInt`（`:760`）与 `addPositiveShortToBigInt`（`:772`）。


### `createBigIntValue` (`src/exec/value_ops.zig:525`)
...
### `bigIntToNumber` (`src/exec/value_ops.zig:537`)
...
### `toIntegerOrInfinity` (`src/exec/value_ops.zig:545`)
...
### `toIndexUsize` (`src/exec/value_ops.zig:560`)
...
### `toBigIntValue` (`src/exec/value_ops.zig:570`)
...
### `heapBigInt` (`src/exec/value_ops.zig:593`)
...
### `bigIntFromValueBorrowed` (`src/exec/value_ops.zig:599`)
...
### `isTruthy` (`src/exec/value_ops.zig:609`)
...
### `isFunctionObject` (`src/exec/value_ops.zig:613`)
...
- **所有权 / 错误 / 调用**：无：只读 class_id 与 proxy target 的谓词，不分配、无 error set。调用方 `typeOf`（`:324`）、`proxyTargetIsFunction`（`:631`）与 `src/exec/reflect_ops.zig:217`。

### `proxyTargetIsFunction` (`src/exec/value_ops.zig:626`)
...
- **所有权 / 错误 / 调用**：无：读 proxy target 后回调 `isFunctionObject`（两者互相递归，沿代理链下降），不分配、无 error set；revoked/非代理走 `orelse false`。文件私有，调用方 `typeOf`（`:324`）与 `isFunctionObject`（`:617`）。

### `atomNameEql` (`src/exec/value_ops.zig:653`)
...
### `appendRawString` (`src/exec/value_ops.zig:667`)
...
### `formatFiniteNumber` (`src/exec/value_ops.zig:671`)

- **签名**：`pub fn formatFiniteNumber(buffer: []u8, value: f64) ![]const u8`。
- **作用**：有限 double 的十进制文本格式化（缓冲不够会报错）。
- **实现**：转调 `core.value_format.formatFiniteNumber`。
- **所有权 / 错误 / 调用**：无所有权：写进调用方给的栈缓冲，不分配；错误是 `core.value_format.formatFiniteNumber` 的缓冲不足。**生产代码无调用方**（只有 `src/tests/exec.zig:8683` 起的单测用到）——现存格式化点全用下面的 `formatFiniteNumberAssumeCapacity`，它只作为 pub API 保留。


### `formatFiniteNumberAssumeCapacity` (`src/exec/value_ops.zig:675`)
...
### `binaryBigInt` (`src/exec/value_ops.zig:679`)
...
### `addPositiveShortToBigInt` (`src/exec/value_ops.zig:763`)
...
### `shortBigIntBinary` (`src/exec/value_ops.zig:775`)
...
### `shortBigIntUnary` (`src/exec/value_ops.zig:787`)
...
### `shortBigIntAdd` (`src/exec/value_ops.zig:797`)
...
### `shortBigIntSub` (`src/exec/value_ops.zig:804`)
...
### `shortBigIntMul` (`src/exec/value_ops.zig:811`)
...
### `binaryNumber` (`src/exec/value_ops.zig:818`)
...
### `toInt32` (`src/exec/value_ops.zig:838`)
...
### `stringAdd` (`src/exec/value_ops.zig:846`)
...
### `stringAddStringInt` (`src/exec/value_ops.zig:867`)
...
### `addStringsOwned` (`src/exec/value_ops.zig:912`)
...
### `appendAsciiSuffixOwned` (`src/exec/value_ops.zig:919`)
...
### `stringAddStringsOwned` (`src/exec/value_ops.zig:931`)
...
### `concatFlatStringBodiesOwned` (`src/exec/value_ops.zig:1027`)
...
### `percentHexConcat` (`src/exec/value_ops.zig:1065`)
...
### `upperHexValue` (`src/exec/value_ops.zig:1079`)
...
### `stringObject` (`src/exec/value_ops.zig:1083`)
...
### `appendStringUtf16Units` (`src/exec/value_ops.zig:1087`)
...
### `shiftBigInt` (`src/exec/value_ops.zig:1096`)
...
### `parseJsNumber` (`src/exec/value_ops.zig:1117`)
...
### `valuesEqual` (`src/exec/value_ops.zig:1121`)
...
### `compareBigIntValues` (`src/exec/value_ops.zig:1145`)
...
### `bigIntParts` (`src/exec/value_ops.zig:1158`)
...
### `isHTMLDDA` (`src/exec/value_ops.zig:1181`)
...
### `compareStringValues` (`src/exec/value_ops.zig:1185`)
...
### `jsMathPow` (`src/exec/value_ops.zig:1189`)
...
### `valuesStrictEqual` (`src/exec/value_ops.zig:1196`)
...
### `cloneBigIntValue` (`src/exec/value_ops.zig:1223`)
...
### `appendValueString` (`src/exec/value_ops.zig:1228`)
...
### `buildErrorStackValue` (`src/exec/error_stack_ops.zig:44`)
...
### `formatCapturedErrorStackValue` (`src/exec/error_stack_ops.zig:64`)
...
### `throwParseSyntaxError` (`src/exec/error_stack_ops.zig:99`)
...
### `defineParseErrorSurface` (`src/exec/error_stack_ops.zig:121`)
...
### `errorPrepareStackTrace` (`src/exec/error_stack_ops.zig:145`)

- **签名**：`pub fn errorPrepareStackTrace(_: *core.JSRuntime, global: *core.Object) !?core.JSValue`。
- **作用**：取 `Error.prepareStackTrace` hook，不可调用（或 `Error` 不是对象）时返回 null。
- **实现**：`global.getProperty(Error)` → `expectObject`（失败 null）→ `getProperty(prepareStackTrace)` → `isCallableValue` 过滤。`rt` 参数未使用。
- **所有权 / 错误 / 调用**：第一个参数（runtime）未使用。两次 `getProperty` 都是不触发 accessor 的普通读，返回的 `prepare` 是**借用**值，调用方在 `formatting_error_stack` 窗口内立即用掉。`Error` 不是对象或钩子不可调用时返回 `null` 而不是错误；错误只有 `getProperty` 自身的透传（如未初始化 var_ref 的 `error.ReferenceError`）。文件私有，调用方 `buildErrorStackValue`（`:47`）与 `formatCapturedErrorStackValue`（`:74`）。

### `backtraceFunctionNameEql` (`src/exec/error_stack_ops.zig:157`)
...
### `callSiteFunctionName` (`src/exec/error_stack_ops.zig:169`)
...
### `callSiteFunctionNameValue` (`src/exec/error_stack_ops.zig:177`)
...
### `errorStackTraceLimit` (`src/exec/error_stack_ops.zig:185`)
...
### `appendBacktraceFunctionName` (`src/exec/error_stack_ops.zig:198`)
...
### `appendCallSiteFunctionName` (`src/exec/error_stack_ops.zig:216`)
...
### `appendCallSiteFileName` (`src/exec/error_stack_ops.zig:228`)
...
### `errorStackGetter` (`src/exec/error_stack_ops.zig:240`)
...
### `errorStackSetter` (`src/exec/error_stack_ops.zig:257`)
...
### `isErrorStackSetterValue` (`src/exec/error_stack_ops.zig:323`)

- **签名**：`pub fn isErrorStackSetterValue(value: core.JSValue) bool`。
...
- **所有权 / 错误 / 调用**：无：解码函数对象的 native builtin id 后比对 domain 与 id，不分配、无 error set。标 `pub` 但树内唯一调用方是 `errorStackSetter` 的「setter 就是内建 stack setter」判定（`src/exec/error_stack_ops.zig:295`）。


### `errorCaptureStackTrace` (`src/exec/error_stack_ops.zig:329`)
...
### `sourceFunction` (`src/exec/function_ops.zig:368`)

- **签名**：`pub fn sourceFunction(realm: *core.RealmContext, name: []const u8, source: []const u8) !core.JSValue`。
- **作用**：造一个带源码文本的 native 函数对象，供 `Function.prototype.toString` 之类回显。
- **实现**：可返回 `error.InvalidBuiltinRegistry`。 关键调用：`core.function.nativeFunctionWithPrototypeAndCapacity`、`core.Object.expect`、`core.string.String.createUtf8`、`function_object.setOptionalValueSlot`、`function_object.functionSourceSlot`。 用 `ValueRootFrame` / `rootValues` 钉住跨分配窗口的值。
- **所有权 / 错误 / 调用**：这是本册里少见的**显式建根**点：`core.runtime.rootValues(.{ &function_value, &source_value })` 建 `ValueRootFrame` 并 `activate`/`defer deactivate`，保证 `String.createUtf8` 触发 GC 时新函数对象与源码字符串都不会被回收；`source_value` 在挂进 slot 后立即清成 `undefined`，缩短根窗口。返回值归 GC。realm 没有 `cached_function_proto` 时返回裸 `error.InvalidBuiltinRegistry`。**树内无生产调用方**：只有同文件的单测 `src/exec/function_ops.zig:402` 调它。

### `objectFromFunctionValue` (`src/exec/function_ops.zig:409`)

- **签名**：`fn objectFromFunctionValue(value: core.JSValue) ?*core.Object`。
- **作用**：把函数值还原成 `*core.Object`。
- **实现**：非对象或取不到 ref header 返回 null，否则 `core.Object.fromHeader`。
- **所有权 / 错误 / 调用**：无：只做 tag 检查 + `Object.fromHeader`，返回**借用**指针，不分配、无 error set。**树内无生产调用方**：只有同文件的 `sourceFunction` 单测（`src/exec/function_ops.zig:403`）用到。

### `constructFunctionFromSource` (`src/exec/function_ops.zig:415`)
...
### `constructGeneratorFunctionFromSource` (`src/exec/function_ops.zig:427`)
...
### `constructDynamicFunctionFromSource` (`src/exec/function_ops.zig:446`)
...
对照 `JS_PrintValue`（quickjs.c:13678-14432）字节级输出：深度 2、字符串 1000、每容器 100 项。顶层字符串由调用方原样写出；其余走 `printValueRec`。除 BigInt 十进制外不分配。


### `State.puts` (`src/exec/print_inspector.zig:41`)
...
### `State.putc` (`src/exec/print_inspector.zig:45`)
...
### `State.printf` (`src/exec/print_inspector.zig:49`)
...
### `State.putUnicodeEscape` (`src/exec/print_inspector.zig:53`)
...
### `makeState` (`src/exec/print_inspector.zig:60`)
...
### `printValue` (`src/exec/print_inspector.zig:66`)

- **签名**：`pub fn printValue(ctx: *core.JSContext, global: *core.Object, output: ?*std.Io.Writer, writer: *std.Io.Writer, value: core.JSValue) Error!void`。
- **作用**：一个**非字符串** `print` 实参的入口（`JS_PrintValue` + 默认选项，quickjs.c:14440）。
- **实现**：`makeState` 后直接 `printValueRec`。
- **所有权 / 错误 / 调用**：`value` 借用，不产生 JS 值；`State` 在栈上，`print_stack` 的循环检测指针也只在本次调用内有效。本文件 `Error` = `std.Io.Writer.Error || error{OutOfMemory}`，不是 `RuntimeError`：这里既不挂 pending exception 也不经 `materializeRuntimeError`，错误一路退到唯一外部出口 `src/exec/call.zig:2157`，那里 `error.WriteFailed` 交给 `exception_ops.throwHostError` 变成 JS 异常，`error.OutOfMemory` 原样上浮。 **树内无调用方**：CLI 的 `print`/`console.log` 走下面的 `printHostArgument`，本函数只剩 pub API 形态。

### `printHostArgument` (`src/exec/print_inspector.zig:73`)
...
### `printFloat64` (`src/exec/print_inspector.zig:81`)
...
### `Units.len` (`src/exec/print_inspector.zig:96`)

- **签名**：`fn len(self: Units) usize`。
- **作用**：code-unit 视图的长度。
- **实现**：latin1 取 `bytes.len`，utf16 取 `units.len`。
- **所有权 / 错误 / 调用**：无：读 union 里**借用**切片的长度，不分配、无 error set；切片的生命周期由持有它的 `*core.string.String` 决定（调用方必须先 `ensureFlat`）。调用方 `printUnits` 的循环上界与各 `printString`/`printRegExp` 站点。

### `Units.at` (`src/exec/print_inspector.zig:103`)
...
### `printUnits` (`src/exec/print_inspector.zig:113`)
...
### `unitsOfString` (`src/exec/print_inspector.zig:166`)
...
### `printString` (`src/exec/print_inspector.zig:175`)

- **签名**：`fn printString(s: *State, value: core.JSValue) Error!void`。
- **作用**：带引号、带转义、带截断的字符串输出（`js_print_string`，quickjs.c:13812-13829）。
- **实现**：取不到 String 体写 `<invalid string tag>`；`ensureFlat` 失败折成 `error.OutOfMemory`。只打印前 1000 个 code unit（`default_max_string_length`），超出时补 `... N more character(s)`（N>1 才加 s）。
- **所有权 / 错误 / 调用**：`body.ensureFlat(s.rt)` 可能就地物化 rope（分配归 GC 堆，失败被改写成 `error.OutOfMemory`）——这是本模块「除 BigInt 文本外不分配」那句注释的一个例外；其余只写 writer。非字符串 tag 打印 `<invalid string tag>` 而不报错。文件私有，唯一调用方 `printValueRec`（`:664`）。

### `printRawString` (`src/exec/print_inspector.zig:191`)

- **签名**：`fn printRawString(s: *State, value: core.JSValue) Error!void`。
- **作用**：字符串原文输出，不加引号不转义（`js_print_raw_string`，quickjs.c:13831）。
- **实现**：取不到 String 体直接返回；`ensureFlat` 失败折成 OOM。latin1：<0x80 原样，否则手写两字节 UTF-8；utf16：用 `Utf16LeIterator` 逐码点编码成 UTF-8（编码失败的码点跳过）。
- **所有权 / 错误 / 调用**：同样会 `ensureFlat`（rope 物化，失败折成 `error.OutOfMemory`）；latin1 的 0x80-0xFF 按码点重新编成两字节 UTF-8 而不是裸字节，UTF-16 用 `Utf16LeIterator` 并对非法序列 `catch continue` 静默跳过。非字符串值直接 `return`。5 处调用：`printHostArgument`（`:75`）、`printError` 的 name/message（`:408`、`:416`）、`[Function name]`（`:537`）、Date 的 ISO 文本（`:629`）。

### `isAsciiIdent` (`src/exec/print_inspector.zig:216`)
...
### `printAtom` (`src/exec/print_inspector.zig:229`)
...
### `printNameBytes` (`src/exec/print_inspector.zig:236`)
...
### `printClassName` (`src/exec/print_inspector.zig:255`)
...
### `printComma` (`src/exec/print_inspector.zig:288`)
...
### `printMoreItems` (`src/exec/print_inspector.zig:298`)
...
### `ownOrProtoDataString` (`src/exec/print_inspector.zig:305`)
...
### `printRegExp` (`src/exec/print_inspector.zig:332`)
...
- **所有权 / 错误 / 调用**：`object` 与 `regexpSource()` 都是借用；`ensureFlat` 可能物化 rope（失败折成 `error.OutOfMemory`）。未编译/无源码的 RegExp 打印 `[uninitialized_regexp]` 而不报错。flag 位取自 `regexp_adapter.flagBitsFromBytecode`，不分配。文件私有，唯一调用方 `printObject` 的 regexp 臂（`:562`）。

### `putUnitRaw` (`src/exec/print_inspector.zig:397`)
...
### `printError` (`src/exec/print_inspector.zig:406`)
...
### `isTypedArrayClass` (`src/exec/print_inspector.zig:450`)
...
### `isCallableClass` (`src/exec/print_inspector.zig:456`)
...
### `printObject` (`src/exec/print_inspector.zig:476`)
...
- **所有权 / 错误 / 调用**：`object` 是**借用**的 `*const core.Object`，整个 dump 期间不建根——安全性靠调用方持有该值、且各臂只读现成数据（`arrayElements`、`typedArrayPayloadFast`、`collectionPayloadBorrowed` 都是借用视图，不复制）。会分配的只有几处 `ensureFlat` 与 Error 臂里的 `errorStackGetter`（它可能新建 stack 字符串、甚至经 `Error.prepareStackTrace` 重入 JS；`printError` 用 `catch break :blk null` 把那条路径上的一切错误吞掉）。error set 同族。文件私有，唯一调用方 `printValueRec`（`:679`）。

### `dateIsoText` (`src/exec/print_inspector.zig:626`)
...
### `printStackIndex` (`src/exec/print_inspector.zig:633`)
...
### `printValueRec` (`src/exec/print_inspector.zig:641`)
...
- **实现**：先 `rootValues` 钉住原型值跨越后面的分配。`expectObject` 失败 → `error.InvalidBuiltinRegistry`；以该原型 `core.Object.create(class error_)`，定义 own `message`（writable+configurable、非 enumerable），再 `attachStackToErrorValue` 抓栈。**`name` 参数未使用**（`_ = name;`）：名字由原型链上的 `name` 决定，可变的全局构造器绑定不参与 realm 选择。
...
### `createNamedErrorWithoutStack` (`src/exec/exception_ops.zig:113`)
...
### `createPreallocatedOutOfMemoryError` (`src/exec/exception_ops.zig:123`)
...
### `buildNamedErrorObject` (`src/exec/exception_ops.zig:131`)
...
### `throwTdzReferenceError` (`src/exec/exception_ops.zig:212`)
...
### `normalizeEvalRuntimeError` (`src/exec/exception_ops.zig:243`)
...
### `runtimeErrorValueForGeneratorCatch` (`src/exec/exception_ops.zig:250`)
...
### `promiseAggregateError` (`src/exec/exception_ops.zig:273`)
...
### `promiseErrorValue` (`src/exec/exception_ops.zig:295`)
...
### `rejectedPromiseForRuntimeError` (`src/exec/exception_ops.zig:323`)
...
### `throwTypeErrorMessage` (`src/exec/exception_ops.zig:344`)
...
### `throwRangeErrorMessage` (`src/exec/exception_ops.zig:350`)
...
### `throwInternalErrorMessage` (`src/exec/exception_ops.zig:357`)
...
- **实现**：造 `InternalError(message)`、`ctx.throwValue`，返回的哨兵是 **`error.StackOverflow`**（对照 qjs `JS_ThrowInternalError` 的主要使用场景）。
...
### `throwInterrupted` (`src/exec/exception_ops.zig:365`)
...
### `pollInterrupt` (`src/exec/exception_ops.zig:390`)
...
### `pollInterruptSlowLeg` (`src/exec/exception_ops.zig:398`)
...
### `throwReferenceErrorMessage` (`src/exec/exception_ops.zig:403`)
...
### `throwReferenceErrorNotDefined` (`src/exec/exception_ops.zig:415`)
...
### `throwSyntaxErrorMessage` (`src/exec/exception_ops.zig:427`)
...
### `isCallSiteObject` (`src/exec/exception_ops.zig:433`)
...
### `callSiteMethodById` (`src/exec/exception_ops.zig:439`)
...
### `backtraceFunctionNameAtom` (`src/exec/exception_ops.zig:452`)

- **签名**：`pub fn backtraceFunctionNameAtom(ctx: *core.JSContext, fallback: core.Atom, current_function_value: core.JSValue) !core.Atom`。
...
- **所有权 / 错误 / 调用**：`std.ArrayList(u8)` 局部缓冲 `defer deinit`；返回的是 `internString` 登记后的 **Atom**（atom 表拥有，调用方不释放）。非对象返回传入的 `fallback`，没有 `name` 或非字符串 data 属性返回 `ids.empty_string`。错误是 `getOwnProperty` 与分配的透传。标 `pub` 但树内唯一调用方是本文件的 `resolveBacktraceFunctionName`（`src/exec/exception_ops.zig:467`）。


### `resolveBacktraceFunctionName` (`src/exec/exception_ops.zig:463`)
...
### `resolveBacktraceLocation` (`src/exec/exception_ops.zig:472`)
...
### `frameBacktraceSnapshot` (`src/exec/exception_ops.zig:488`)
...
### `isErrorConstructorName` (`src/exec/exception_ops.zig:508`)
...
### `functionNameBytes` (`src/exec/exception_ops.zig:512`)
...
### `pendingExceptionMatchesError` (`src/exec/exception_ops.zig:522`)
...
### `objectDataStringPropertyMatches` (`src/exec/exception_ops.zig:534`)
...
### `stringBodyEqualsAscii` (`src/exec/exception_ops.zig:554`)
...
### `runtimeErrorInfo` (`src/exec/exception_ops.zig:573`)
...
### `promiseErrorInfo` (`src/exec/exception_ops.zig:610`)
...
### `hostIoErrorInfo` (`src/exec/exception_ops.zig:638`)
...
### `hostErrorValue` (`src/exec/exception_ops.zig:679`)
...
### `throwHostError` (`src/exec/exception_ops.zig:692`)
...
### `throwModuleHostStall` (`src/exec/exception_ops.zig:709`)
...
### `errorNameForRuntimeError` (`src/exec/exception_ops.zig:724`)
...
### `sourceLocationFromPc2Line` (`src/exec/exception_ops.zig:744`)
...
### `defineNonEnumValueProperty` (`src/exec/exception_ops.zig:757`)
...
### `throwReferenceErrorSentinel` (`src/exec/exception_ops.zig:761`)

- **签名**：`fn throwReferenceErrorSentinel(ctx: *core.JSContext) void`。
- **作用**：造不出 Error 对象时的兜底：把一个**裸 int32** 塞进异常槽。
- **实现**：`ctx.throwValue(core.JSValue.int32(209))`——209 是 ReferenceError 的 atom id。零分配，用于 TDZ 路径的分配失败兜底；调用方仍然返回 `error.ReferenceError` 哨兵，上层可在异常边界重新物化。
- **所有权 / 错误 / 调用**：把值写进 ctx 的 pending exception 槽；不分配、不返回 error。

## 覆盖核对

- 清单函数数: 206（`src/exec/coercion_ops.zig` 18 + `src/exec/error_ops.zig` 2 + `src/exec/error_stack_ops.zig` 18 + `src/exec/exception_ops.zig` 42 + `src/exec/function_ops.zig` 18 + `src/exec/performance_ops.zig` 2 + `src/exec/primitive_ops.zig` 6 + `src/exec/print_inspector.zig` 30 + `src/exec/value_ops.zig` 70）
- 本文标题覆盖: 206
- 未覆盖: 无
