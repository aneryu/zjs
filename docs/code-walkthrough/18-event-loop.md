...
- **作用**：读出已请求的退出码。
- **实现**：返回 `exit_code`。
- **所有权 / 错误 / 调用**：无：`*const` 只读访问器，返回一个 `?u8` 标量，不分配、不建根、无 error。CLI 的 `exitIfRequested`（`src/cli/zjs.zig:440`）直接调这个方法；此外还经 `vtable.exitCode`（表项在 :585，`*anyopaque` 适配器在 :610）暴露给 `core.context.HostEventLoop`。
