...
### `getSuper` (`src/exec/object_ops.zig:3930`)
...
### `getSuperValue` (`src/exec/object_ops.zig:3964`)
...
### `putSuperValue` (`src/exec/object_ops.zig:3998`)
...
### `setHomeObject` (`src/exec/object_ops.zig:4031`)
...
### `checkBrand` (`src/exec/object_ops.zig:4043`)
...
### `checkBrandVm` (`src/exec/object_ops.zig:4050`)
...
### `addBrand` (`src/exec/object_ops.zig:4080`)
...
### `addBrandVm` (`src/exec/object_ops.zig:4105`)
...
### `privateIn` (`src/exec/object_ops.zig:4120`)
...
### `privateInVm` (`src/exec/object_ops.zig:4144`)
...
### `defineClass` (`src/exec/object_ops.zig:4160`)
...
### `defineMethodComputed` (`src/exec/object_ops.zig:4284`)
...
### `defineObjectMethod` (`src/exec/object_ops.zig:4308`)
...
### `defineObjectMethodValue` (`src/exec/object_ops.zig:4323`)
...
### `stackValueFromTop` (`src/exec/object_ops.zig:4386`)
...
### `ensureHomeObjectBrand` (`src/exec/object_ops.zig:4392`)
...
### `hasPrivateBrand` (`src/exec/object_ops.zig:4405`)
...
### `readInt` (`src/exec/object_ops.zig:4414`)
...
### `proxySetWithTrap` (`src/exec/object_ops.zig:4460`)
...
### `proxySetTrapForErrorStackSetter` (`src/exec/object_ops.zig:4498`)
...
### `proxyCreateDataPropertyOrThrow` (`src/exec/object_ops.zig:4512`)

- **签名**：`fn proxyCreateDataPropertyOrThrow( ctx: *core.JSContext, output: ?*std.Io.Writer, global: *core.Object, proxy: *core.Object, atom_id: core.Atom, value: core.JSValue, caller_function: ?*const bytecode.FunctionBytecode, caller_frame: ?*frame_mod.Frame, ) !void`（文件私有；原先恒被丢弃的 `receiver_value` 形参已删）。
- **作用**：CreateDataPropertyOrThrow 的 Proxy 臂：`defineProperty` trap 或直接在 target 上 define W/E/C=true。
- **实现**：缺 target/handler TypeError。缺 trap → target.defineOwnProperty data。trap 须可调用；造描述符对象（`value` 加 writable/enumerable/configurable 三个 true）；调用；假值 TypeError。
- **所有权 / 错误 / 调用**：fromEntries 等。此处不跑完整 define invariant（throw 语义要求 trap 真）。

### `validateProxyOwnKeysResult` (`src/exec/object_ops.zig:4547`)
...
### `proxyAwareOwnPropertyDescriptor` (`src/exec/object_ops.zig:4593`)
...
### `proxyAwareExistsOwnProperty` (`src/exec/object_ops.zig:4650`)
...
### `proxyAwareExtensibleOp` (`src/exec/object_ops.zig:4680`)
...
### `proxyAwareIsExtensible` (`src/exec/object_ops.zig:4731`)
...
### `proxyAwarePreventExtensions` (`src/exec/object_ops.zig:4742`)
...
### `proxyAwareSetPrototypeOf` (`src/exec/object_ops.zig:4753`)
...
### `completeProxyDescriptor` (`src/exec/object_ops.zig:4785`)

- **签名**：`fn completeProxyDescriptor(desc: core.Descriptor) !core.Descriptor`（文件私有；原先恒被丢弃的 `rt` 形参已删）。
- **作用**：把部分描述符补成完整 data/accessor（缺省 false / undefined）。
- **实现**：generic/data → data；accessor → accessor。
- **所有权 / 错误 / 调用**：gopd trap 结果在 invariant 前。

### `isCompatibleProxyDescriptor` (`src/exec/object_ops.zig:4802`)
...
### `proxyTargetIsCallable` (`src/exec/object_ops.zig:4824`)
...
### `proxyTargetIsConstructor` (`src/exec/object_ops.zig:4830`)
...
### `callProxyApply` (`src/exec/object_ops.zig:4836`)
...
- **所有权 / 错误 / 调用**：`proxy_value` 形参不被读取（需要的信息全在 `proxy` 上），保留是因为两个调用点 `call.zig` / `call_runtime.zig` 手上已有装箱的 callee；函数头注释已写明。

### `constructProxy` (`src/exec/object_ops.zig:4866`)
...
### `getProxyProperty` (`src/exec/object_ops.zig:4899`)
...
### `validateProxyGetResult` (`src/exec/object_ops.zig:4928`)
...
### `validatePlainProxyGetResultFast` (`src/exec/object_ops.zig:4964`)
...
### `firstProxyInPrototypeSetPath` (`src/exec/object_ops.zig:4984`)
...
### `proxySetValueProperty` (`src/exec/object_ops.zig:4993`)
...
### `validateProxySetResult` (`src/exec/object_ops.zig:5007`)
...
### `proxyDefineValueForReflectSet` (`src/exec/object_ops.zig:5030`)
...
- **所有权 / 错误 / 调用**：与完整 `proxyDefineOwnProperty` 不同：这是 Set 落到 receiver 上的 define，不是用户 `defineProperty`。`receiver_value` 形参不被读取（trap 收到的是 target），保留是为了和 `array_ops` 调用点的普通 define 路径对称；函数尾部注释已写明。

### `proxyTargetIsCallableObject` (`src/exec/object_ops.zig:5082`)
...
### `proxyDefineOwnProperty` (`src/exec/object_ops.zig:5089`)
...
### `validateProxyHasResult` (`src/exec/object_ops.zig:5138`)
...
### `proxyTrapKeyValue` (`src/exec/object_ops.zig:5161`)
