...
### `h_varref.b` (`src/exec/tailcall_dispatch_colds.zig:41`)
...
### `h_checkedloc.b` (`src/exec/tailcall_dispatch_colds.zig:46`)
...
### `h_loc.b` (`src/exec/tailcall_dispatch_colds.zig:51`)
...
### `h_arg.b` (`src/exec/tailcall_dispatch_colds.zig:56`)
...
### `h_get_arg_short.b` (`src/exec/tailcall_dispatch_colds.zig:61`)
...
### `h_binary.b` (`src/exec/tailcall_dispatch_colds.zig:66`)
...
### `h_unary.b` (`src/exec/tailcall_dispatch_colds.zig:71`)
...
### `h_field.b` (`src/exec/tailcall_dispatch_colds.zig:76`)
...
### `h_get_array_element.b` (`src/exec/tailcall_dispatch_colds.zig:81`)
...
### `h_put_array_element.b` (`src/exec/tailcall_dispatch_colds.zig:86`)
...
### `h_get_var.b` (`src/exec/tailcall_dispatch_colds.zig:92`)
...
### `h_put_var.b` (`src/exec/tailcall_dispatch_colds.zig:101`)
...
### `h_dyn_env_probe.b` (`src/exec/tailcall_dispatch_colds.zig:107`)
...
### `h_make_slot_ref.b` (`src/exec/tailcall_dispatch_colds.zig:113`)
...
### `h_define_class.b` (`src/exec/tailcall_dispatch_colds.zig:118`)
...
### `h_for_of_start.b` (`src/exec/tailcall_dispatch_colds.zig:123`)
...
### `h_for_of_start.h` (`src/exec/tailcall_dispatch_colds.zig:129`)
...
### `h_for_of_start.b` (`src/exec/tailcall_dispatch_colds.zig:131`)
...
### `buildTable` (`src/exec/tailcall_dispatch_colds.zig:170`)

- **签名**：`pub fn buildTable(s: SpecialHandlers, comptime fast: bool) BuiltTable`。
- **作用**：组装 256 槽冷表，并在 `fast=true` 时用热 handler 覆盖。
- **实现**：先把 256 槽和 `keep` 的 12 槽全填成 `s.op_invalid`（未分配/被隔离的字节一律落到 invalid handler）；再按组填冷 handler：一次性 body 用 `h(...)`/`coldStd(...)` 就地生成（`vm_value.pushInt32Operand`、`vm_value.pushConst`、`vm_regexp.pushLiteral` 等），locals/args/var_refs/checked 这些成组的 opcode 用 `inline for` 指向共享的 `h_loc`/`h_arg`/`h_get_arg_short`/`h_varref`/`h_checkedloc`，带 opcode 立即数的用 `handlerCompare*`/`handlerPost`/`handlerAppend`/`handlerPutVarInit` 工厂；调用/返回/throw/yield/await 这些由主文件传进来的 `SpecialHandlers` 直接装；fusion 收回的旧槽装进 `keep[0..11]`。融合 opcode 的 `dispatch.op_*_cold` 也在门之前装，两张表都有。`if (!fast) return` 之后才用 `dispatch.op_*_fast` 覆盖热 opcode，最后返回 `.{ .table = t, .keep = keep }`。
...
### `op.push_i32.b` (`src/exec/tailcall_dispatch_colds.zig:180`)
...
### `op.push_bigint_i32.b` (`src/exec/tailcall_dispatch_colds.zig:185`)
...
### `op.push_i16.b` (`src/exec/tailcall_dispatch_colds.zig:190`)
...
### `op.push_i8.b` (`src/exec/tailcall_dispatch_colds.zig:195`)
...
### `op.push_const.b` (`src/exec/tailcall_dispatch_colds.zig:200`)
...
### `op.push_const8.b` (`src/exec/tailcall_dispatch_colds.zig:205`)
...
### `op.private_symbol.b` (`src/exec/tailcall_dispatch_colds.zig:210`)
...
### `op.regexp.b` (`src/exec/tailcall_dispatch_colds.zig:215`)
...
### `op.fclosure.b` (`src/exec/tailcall_dispatch_colds.zig:220`)
...
### `op.undefined.b` (`src/exec/tailcall_dispatch_colds.zig:226`)
...
### `op.null.b` (`src/exec/tailcall_dispatch_colds.zig:231`)
...
### `op.push_false.b` (`src/exec/tailcall_dispatch_colds.zig:236`)
...
### `op.push_true.b` (`src/exec/tailcall_dispatch_colds.zig:241`)
...
### `family.push_7.b` (`src/exec/tailcall_dispatch_colds.zig:247`)

- **签名**：`fn b(vm: *Vm) HostError!void`。
- **作用**：`inline for` 出来的共享 body，一次给 `OP_push_minus1` 与 `OP_push_0`..`OP_push_7` 九个槽各生成一份（常量 `e.v` 是 comptime 烘焙进去的 -1..7）。
- **实现**：关键调用：`vm_value.pushSmallIntMaybeFuse`。 栈效应：+1 压该小整数——`pushSmallIntMaybeFuse` 只转调 `pushImmediateInt32MaybeFuse`，后者显式忽略 `function`/`frame` 做一次纯 push，名字里的 fuse 在此路径上不成立（vm_value.zig:53-72 注释：qjs 无 push+binop 运行期融合）。 下一跳：本函数是 `coldStd`/`h()` 的 body，不自己尾分发；外壳成功则 `coldNext`（`maybeStop` 后 `active_dispatch_tbl[npc[0]]`），失败 `vm.fail` → `.threw`。
...
### `op.push_atom_value.b` (`src/exec/tailcall_dispatch_colds.zig:253`)
...
### `op.push_empty_string.b` (`src/exec/tailcall_dispatch_colds.zig:258`)
...
### `op.set_name.b` (`src/exec/tailcall_dispatch_colds.zig:276`)
...
### `op.nip_catch.b` (`src/exec/tailcall_dispatch_colds.zig:283`)
...
### `op.in.b` (`src/exec/tailcall_dispatch_colds.zig:306`)

- **签名**：`fn b(vm: *Vm) HostError!void`。
- **作用**：OP_in 的尾分发 handler：在寄存器 `pc/sp/var_buf` 上执行该 opcode，并以尾调用进入下一条——但这条实例在同一个 `buildTable` 里立刻被覆盖，实际不进任何表（见「调用」），真正执行 OP_in 的是后面那份带 comptime opcode 实参的 handler。
...
- **所有权 / 错误 / 调用**：所有权：不适用——这个 body 从不执行。 错误：不适用——外壳的 `vm.fail` → `.threw` 路径在这里不会兑现。 调用：**无**——这条赋值在同一个 `buildTable` 里立刻被 :310 的 `handlerComparePlaceholder(op.in)` 覆盖（而且它把 opcode 实参传成 `undefined`，真跑会是 UB），两张表都不含它，没有任何分发能进入。

### `op.private_in.b` (`src/exec/tailcall_dispatch_colds.zig:313`)
...
### `op.not.b` (`src/exec/tailcall_dispatch_colds.zig:318`)
...
### `op.lnot.b` (`src/exec/tailcall_dispatch_colds.zig:323`)
...
### `op.post_inc.b` (`src/exec/tailcall_dispatch_colds.zig:328`)

- **签名**：`fn b(vm: *Vm) HostError!void`。
- **作用**：OP_post_inc 的尾分发 handler：在寄存器 `pc/sp/var_buf` 上执行该 opcode，并以尾调用进入下一条——但这条实例在同一个 `buildTable` 里立刻被覆盖，实际不进任何表（见「调用」），真正执行 OP_post_inc 的是后面那份带 comptime opcode 实参的 handler。
- **实现**：关键调用：`vm_arith.postUpdateVm`。 栈效应：+1——弹出旧值，压回「旧值、旧值±1」两槽（int32 / short BigInt 走快臂，其余先 `toPrimitive` 再 `unary`）。 下一跳：本函数是 `coldStd`/`h()` 的 body，不自己尾分发；外壳成功则 `coldNext`（`maybeStop` 后 `active_dispatch_tbl[npc[0]]`），失败 `vm.fail` → `.threw`。
- **所有权 / 错误 / 调用**：所有权：不适用——这个 body 从不执行。 错误：不适用——外壳的 `vm.fail` → `.threw` 路径在这里不会兑现。 调用：**无**——这条赋值在同一个 `buildTable` 里立刻被 :332 的 `handlerPost(op.post_inc)` 覆盖（而且它把 opcode 实参传成 `undefined`，真跑会是 UB），两张表都不含它，没有任何分发能进入。

### `op.goto.b` (`src/exec/tailcall_dispatch_colds.zig:348`)
...
### `op.goto16.b` (`src/exec/tailcall_dispatch_colds.zig:354`)
...
### `op.goto8.b` (`src/exec/tailcall_dispatch_colds.zig:360`)
...
### `op.if_false.b` (`src/exec/tailcall_dispatch_colds.zig:366`)
...
### `op.if_true.b` (`src/exec/tailcall_dispatch_colds.zig:372`)
...
### `op.if_false8.b` (`src/exec/tailcall_dispatch_colds.zig:378`)
...
### `op.if_true8.b` (`src/exec/tailcall_dispatch_colds.zig:384`)
...
### `op.gosub.b` (`src/exec/tailcall_dispatch_colds.zig:390`)
...
### `op.ret.b` (`src/exec/tailcall_dispatch_colds.zig:395`)
...
### `op.make_var_ref.b` (`src/exec/tailcall_dispatch_colds.zig:406`)
...
### `op.get_ref_value.b` (`src/exec/tailcall_dispatch_colds.zig:411`)
...
### `op.put_ref_value.b` (`src/exec/tailcall_dispatch_colds.zig:416`)
...
### `op.get_private_field.b` (`src/exec/tailcall_dispatch_colds.zig:425`)
...
### `op.put_private_field.b` (`src/exec/tailcall_dispatch_colds.zig:430`)
...
### `op.define_private_field.b` (`src/exec/tailcall_dispatch_colds.zig:435`)
...
### `op.get_super.b` (`src/exec/tailcall_dispatch_colds.zig:442`)
...
### `op.get_super_value.b` (`src/exec/tailcall_dispatch_colds.zig:447`)
...
### `op.get_length.b` (`src/exec/tailcall_dispatch_colds.zig:452`)
...
### `op.object.b` (`src/exec/tailcall_dispatch_colds.zig:459`)
...
### `op.object_slots2.b` (`src/exec/tailcall_dispatch_colds.zig:464`)
...
### `op.array_from.b` (`src/exec/tailcall_dispatch_colds.zig:469`)
...
### `op.define_field.b` (`src/exec/tailcall_dispatch_colds.zig:474`)
...
### `op.set_home_object.b` (`src/exec/tailcall_dispatch_colds.zig:479`)
...
### `op.define_array_el.b` (`src/exec/tailcall_dispatch_colds.zig:486`)
...
### `op.define_method.b` (`src/exec/tailcall_dispatch_colds.zig:491`)
...
### `op.define_method_computed.b` (`src/exec/tailcall_dispatch_colds.zig:496`)
...
### `op.append.b` (`src/exec/tailcall_dispatch_colds.zig:501`)

- **签名**：`fn b(vm: *Vm) HostError!void`。
- **作用**：OP_append 的尾分发 handler：在寄存器 `pc/sp/var_buf` 上执行该 opcode，并以尾调用进入下一条——但这条实例在同一个 `buildTable` 里立刻被覆盖，实际不进任何表（见「调用」），真正执行 OP_append 的是后面那份带 comptime opcode 实参的 handler。
...
- **所有权 / 错误 / 调用**：所有权：不适用——这个 body 从不执行。 错误：不适用——外壳的 `vm.fail` → `.threw` 路径在这里不会兑现。 调用：**无**——这条赋值在同一个 `buildTable` 里立刻被 :505 的 `handlerAppend(op.append)` 覆盖（而且它把 opcode 实参传成 `undefined`，真跑会是 UB），两张表都不含它，没有任何分发能进入。

### `op.copy_data_properties.b` (`src/exec/tailcall_dispatch_colds.zig:507`)
...
### `op.put_var_init.b` (`src/exec/tailcall_dispatch_colds.zig:514`)

- **签名**：`fn b(vm: *Vm) HostError!void`。
- **作用**：OP_put_var_init 的尾分发 handler：在寄存器 `pc/sp/var_buf` 上执行该 opcode，并以尾调用进入下一条——但这条实例在同一个 `buildTable` 里立刻被覆盖，实际不进任何表（见「调用」），真正执行 OP_put_var_init 的是后面那份带 comptime opcode 实参的 handler。
...
- **所有权 / 错误 / 调用**：所有权：不适用——这个 body 从不执行。 错误：不适用——外壳的 `vm.fail` → `.threw` 路径在这里不会兑现。 调用：**无**——这条赋值在同一个 `buildTable` 里立刻被 :518 的 `handlerPutVarInit(op.put_var_init)` 覆盖（而且它把 opcode 实参传成 `undefined`，真跑会是 UB），两张表都不含它，没有任何分发能进入。

### `op.special_object.b` (`src/exec/tailcall_dispatch_colds.zig:520`)
...
### `op.ext0.b` (`src/exec/tailcall_dispatch_colds.zig:525`)
...
### `op.rest.b` (`src/exec/tailcall_dispatch_colds.zig:530`)
...
### `op.typeof.b` (`src/exec/tailcall_dispatch_colds.zig:537`)
...
### `keep[0].b` (`src/exec/tailcall_dispatch_colds.zig:546`)
...
### `keep[1].b` (`src/exec/tailcall_dispatch_colds.zig:551`)
...
### `op.is_undefined_or_null.b` (`src/exec/tailcall_dispatch_colds.zig:558`)
...
### `keep[2].b` (`src/exec/tailcall_dispatch_colds.zig:563`)
...
### `op.is_null.b` (`src/exec/tailcall_dispatch_colds.zig:569`)
...
### `op.dup.b` (`src/exec/tailcall_dispatch_colds.zig:576`)
...
### `op.swap.b` (`src/exec/tailcall_dispatch_colds.zig:581`)
...
### `op.nip.b` (`src/exec/tailcall_dispatch_colds.zig:586`)
...
### `keep[11].b` (`src/exec/tailcall_dispatch_colds.zig:591`)
...
### `keep[6].b` (`src/exec/tailcall_dispatch_colds.zig:596`)
...
### `keep[10].b` (`src/exec/tailcall_dispatch_colds.zig:601`)
...
### `op.insert2.b` (`src/exec/tailcall_dispatch_colds.zig:606`)
...
### `op.insert3.b` (`src/exec/tailcall_dispatch_colds.zig:611`)
...
### `keep[3].b` (`src/exec/tailcall_dispatch_colds.zig:616`)
...
### `op.rot3l.b` (`src/exec/tailcall_dispatch_colds.zig:621`)
...
### `keep[8].b` (`src/exec/tailcall_dispatch_colds.zig:626`)
...
### `keep[9].b` (`src/exec/tailcall_dispatch_colds.zig:631`)
...
### `keep[4].b` (`src/exec/tailcall_dispatch_colds.zig:636`)
...
### `op.perm3.b` (`src/exec/tailcall_dispatch_colds.zig:641`)
...
### `op.perm4.b` (`src/exec/tailcall_dispatch_colds.zig:646`)
...
### `keep[5].b` (`src/exec/tailcall_dispatch_colds.zig:651`)
...
### `keep[7].b` (`src/exec/tailcall_dispatch_colds.zig:656`)
...
### `op.catch.b` (`src/exec/tailcall_dispatch_colds.zig:663`)
...
### `op.check_ctor.b` (`src/exec/tailcall_dispatch_colds.zig:668`)
...
### `op.init_ctor.b` (`src/exec/tailcall_dispatch_colds.zig:673`)
...
### `op.check_brand.b` (`src/exec/tailcall_dispatch_colds.zig:678`)
...
### `op.add_brand.b` (`src/exec/tailcall_dispatch_colds.zig:683`)
...
### `op.close_loc.b` (`src/exec/tailcall_dispatch_colds.zig:688`)
...
### `op.nop.b` (`src/exec/tailcall_dispatch_colds.zig:693`)
...
### `op.push_this.b` (`src/exec/tailcall_dispatch_colds.zig:698`)
...
### `op.delete_var.b` (`src/exec/tailcall_dispatch_colds.zig:703`)
...
### `op.delete.b` (`src/exec/tailcall_dispatch_colds.zig:708`)
...
### `op.apply_eval.b` (`src/exec/tailcall_dispatch_colds.zig:715`)
...
### `op.import.b` (`src/exec/tailcall_dispatch_colds.zig:720`)
...
### `op.for_in_start.b` (`src/exec/tailcall_dispatch_colds.zig:729`)
...
### `op.iterator_next.b` (`src/exec/tailcall_dispatch_colds.zig:734`)
...
### `op.iterator_check_object.b` (`src/exec/tailcall_dispatch_colds.zig:739`)
...
### `op.iterator_get_value_done.b` (`src/exec/tailcall_dispatch_colds.zig:744`)
...
### `op.iterator_call.b` (`src/exec/tailcall_dispatch_colds.zig:749`)
...
### `op.for_await_of_next.b` (`src/exec/tailcall_dispatch_colds.zig:755`)
...
### `op.for_in_next.b` (`src/exec/tailcall_dispatch_colds.zig:760`)
...
### `op.iterator_close.b` (`src/exec/tailcall_dispatch_colds.zig:765`)
...
### `handlerPost.b` (`src/exec/tailcall_dispatch_colds.zig:1023`)
...
### `handlerAppend.b` (`src/exec/tailcall_dispatch_colds.zig:1031`)
...
### `handlerPutVarInit.b` (`src/exec/tailcall_dispatch_colds.zig:1039`)
