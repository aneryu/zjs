...
### `BigInt.toFloat64` (`src/libs/bigint.zig:322`)

- **签名**：`pub fn toFloat64(self: BigInt) f64`。
- **作用**：BigInt → double，就近偶舍入，过大给 ±Infinity；对齐 qjs `js_bigint_to_float64`（quickjs.c:12258）。
- **实现**：零与单 limb 直接 `@floatFromInt`。否则取最高两 limb，其余 limb 只留 sticky 位，规格化到 64 位后再压成 63 位 + sticky，`shr_rndn` 10 位得 53 位，进位溢出则指数加一；指数 > 1023 直接无穷。位拼装 `sign | (e+1023)<<52 | mant`。
- **所有权 / 错误 / 调用**：不分配、不抛。唯一调用方 `exec/value_ops.bigIntToNumber`；此前绕十进制串经解析内核，超过 38 位有效数字会被内核截断（`Number(2n**1024n - 2n**970n)` 曾得 MAX_VALUE 而非 Infinity）。

### `BigInt.bitLengthAbs` (`src/libs/bigint.zig:352`)
...
### `BigInt.isPowerOfTwoAbs` (`src/libs/bigint.zig:360`)
...
### `BigInt.modPowerOfTwo` (`src/libs/bigint.zig:370`)
...
### `BigInt.testBit` (`src/libs/bigint.zig:384`)
...
### `BigInt.lowBits` (`src/libs/bigint.zig:391`)
...
### `BigInt.addPositiveSmallInPlace` (`src/libs/bigint.zig:406`)
...
### `BigInt.absCloneWithAllocator` (`src/libs/bigint.zig:411`)
...
### `BigInt.divRemSmallInPlace` (`src/libs/bigint.zig:417`)
...
### `BigInt.shrAbs` (`src/libs/bigint.zig:432`)
...
### `BigInt.toTwosComplement` (`src/libs/bigint.zig:451`)
...
### `divRemAlloc` (`src/libs/bigint.zig:470`)
...
### `divRemAllocOutput` (`src/libs/bigint.zig:474`)
...
### `parseBase10` (`src/libs/bigint.zig:502`)
...
### `parseBase10Alloc` (`src/libs/bigint.zig:506`)
...
### `parseAutoAlloc` (`src/libs/bigint.zig:510`)
...
### `compareParts` (`src/libs/bigint.zig:537`)
...
### `divRemAbsByLimbAlloc` (`src/libs/bigint.zig:561`)
...
### `divRemAbsByLimb` (`src/libs/bigint.zig:587`)
...
### `normalizedReciprocalInit` (`src/libs/bigint.zig:633`)
...
### `divTwoByOneReciprocal` (`src/libs/bigint.zig:652`)
...
### `divRemAbsNormalizedLong` (`src/libs/bigint.zig:699`)
...
### `shiftLeftInto` (`src/libs/bigint.zig:836`)
...
### `unshiftedLimbAt` (`src/libs/bigint.zig:850`)
...
### `subMulAt` (`src/libs/bigint.zig:871`)
...
### `addBackAt` (`src/libs/bigint.zig:889`)
...
### `divRemAbsAlloc` (`src/libs/bigint.zig:901`)
...
### `addAlloc` (`src/libs/bigint.zig:922`)
...
### `subAlloc` (`src/libs/bigint.zig:943`)
...
### `mulAlloc` (`src/libs/bigint.zig:949`)
...
### `parseBaseAlloc` (`src/libs/bigint.zig:1006`)
...
### `addAbsAlloc` (`src/libs/bigint.zig:1035`)
...
### `subAbsAlloc` (`src/libs/bigint.zig:1060`)
...
### `mulSmallInPlace` (`src/libs/bigint.zig:1077`)
...
### `addSmallInPlace` (`src/libs/bigint.zig:1096`)
...
### `fromTwosComplement` (`src/libs/bigint.zig:1115`)
...
### `compareAbs` (`src/libs/bigint.zig:1139`)
...
### `compareAbsParts` (`src/libs/bigint.zig:1143`)
...
### `normalize` (`src/libs/bigint.zig:1158`)
...
### `invertOrder` (`src/libs/bigint.zig:1175`)
