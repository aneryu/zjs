...
| 名字 | 含义 |
| --- | --- |
| `JSContext` / `JSValue` / `NativeEntry` | 宿主门面、core 值、core entry 的别名；Spec.template 和 Call.entry 公开使用 NativeEntry 类型。Call.entry 是 const 指针，但底层 Runtime 的 retire 操作可以改变 entry 状态。 |
...
### `Call.runtime` (`src/native.zig:58`)
...
### `Call.throwError` (`src/native.zig:75`)
...
### `Call.throwTypeError` (`src/native.zig:81`)
...
### `Call.throwRangeError` (`src/native.zig:85`)
...
### `native.thunk` (`src/native.zig:124`)
...
### `leaf` (`src/native.zig:173`)

- **签名**：`pub fn leaf(comptime f: anytype) Spec`。
- **作用**：无 state 的 typed leaf（K1）。
- **实现**：`leafSpec(f, false)`。
- **所有权 / 错误 / 调用**：纯编译期构造：返回按值的 `Spec`，内含指向编译期生成的 `T.*` thunk 的静态代码指针，没有运行期分配、没有 error set；不支持的签名是 `@compileError`，不会退化成 generic 臂。`Spec` 由 `ctx.defineFunction` 消费（`src/tests/embedding_examples.zig:149`），本文件测试 `src/native.zig:262` 只读 `template`。

### `leafWithState` (`src/native.zig:179`)
...
### `native.stateOf` (`src/native.zig:204`)
...
### `native.void_to_void` (`src/native.zig:207`)
...
### `native.i32_to_i32` (`src/native.zig:210`)
...
### `native.i32_i32_to_i32` (`src/native.zig:213`)
...
### `native.f64_to_f64` (`src/native.zig:216`)
...
### `native.f64_f64_to_f64` (`src/native.zig:219`)
...
### `native.f64_to_void` (`src/native.zig:222`)
...
### `native.bool_to_bool` (`src/native.zig:225`)
...
### `native.state_f64_to_void` (`src/native.zig:228`)
...
### `native.state_i32_to_i32` (`src/native.zig:231`)
...
### `P.add` (`src/native.zig:261`)

- **签名**：`fn add(a: i32, b: i32) i32`。
- **作用**：验证 `leaf` 推断 `sig_i32_i32_to_i32`。
- **实现**：`a +% b`。
- **所有权 / 错误 / 调用**：测试夹具，非引擎代码：`+%` 回绕、不分配、无 error set。只被同文件测试 `src/native.zig:262`/`:264`/`:265` 当作 `leaf` 的推断样本，从不注册进 runtime，因此没有任何调用契约。

### `P.half` (`src/native.zig:264`)

- **签名**：`fn half(x: f64) f64`。
- **作用**：验证 `leaf` 推断 `sig_f64_to_f64`。
- **实现**：`x / 2`。
- **所有权 / 错误 / 调用**：同上：测试夹具，不分配、无 error set，只被 `src/native.zig:263` 用来断言推断出 `sig_f64_to_f64`。

### `Probe.f` (`src/native.zig:276`)
...
### `Class.classId` (`src/native.zig:343`)
...
### `Class.unwrap` (`src/native.zig:349`)
...
### `native.thunk` (`src/native.zig:355`)
...
### `Class.constructThunk` (`src/native.zig:377`)
...
### `Class.memberTable` (`src/native.zig:410`)
...
### `Handle.classId` (`src/native.zig:433`)
...
### `Handle.create` (`src/native.zig:440`)
...
### `Handle.unwrap` (`src/native.zig:446`)
...
### `Handle.dispose` (`src/native.zig:452`)
...
### `Handle.prototype` (`src/native.zig:459`)
...
### `selfParamCheck` (`src/native.zig:466`)
...
### `native.thunk` (`src/native.zig:488`)
...
### `native.self_to_f64` (`src/native.zig:512`)
...
### `native.self_f64_to_void` (`src/native.zig:515`)
...
### `native.self_f64_f64_to_void` (`src/native.zig:518`)
...
### `native.self_i32_to_i32` (`src/native.zig:521`)
...
### `native.self_to_i32` (`src/native.zig:524`)
...
### `native.self_i32_to_void` (`src/native.zig:527`)
...
### `native.self_to_void` (`src/native.zig:530`)
...
### `native.self_to_f64` (`src/native.zig:567`)
...
### `native.self_to_i32` (`src/native.zig:570`)
...
### `native.thunk` (`src/native.zig:583`)
...
### `native.self_f64_to_void` (`src/native.zig:628`)
...
### `native.self_i32_to_void` (`src/native.zig:631`)
...
### `native.thunk` (`src/native.zig:645`)
...
### `State.query` (`src/native.zig:693`)
...
### `State.setTime` (`src/native.zig:700`)
