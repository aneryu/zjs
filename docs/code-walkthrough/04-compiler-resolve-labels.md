...
### `bindLessThan` (`src/compiler/resolve_labels.zig:67`)
...
### `lowerBoundBindOffset` (`src/compiler/resolve_labels.zig:73`)

- **签名**：`fn lowerBoundBindOffset(binds: []const BindEntry, bound_offset: u32) usize`。
- **作用**：产物偏移二分下界。
- **实现**：标准二分。
- **所有权 / 错误 / 调用**：canonical 查询、源/跳转审计。

### `canonicalIdentityAtProductOffset` (`src/compiler/resolve_labels.zig:89`)

- **签名**：`fn canonicalIdentityAtProductOffset( binds: []const BindEntry, bound_offset: u32, ) ?u32`。
- **作用**：CFG `canonicalIdentityAtOffset` 的产物坐标版：该偏移 alias 组最低 label。
- **实现**：调用方已持 LabelSlot，用 `slot.bound_offset` 保持 O(log binds)。
- **所有权 / 错误 / 调用**：JumpSlot 记录与最终审计。

### `finalAliasSplit` (`src/compiler/resolve_labels.zig:105`)

- **签名**：`fn finalAliasSplit(group: []const BindEntry, addr: []const u32) ?FinalAliasSplit`。
- **作用**：同组活身份最终地址不同 → 分裂。
- **实现**：跳过 unbound；第二个不同地址即返回。
- **所有权 / 错误 / 调用**：`auditFinalBoundaryIdentity`。

### `aliasLivenessSplit` (`src/compiler/resolve_labels.zig:133`)

- **签名**：`fn aliasLivenessSplit(group: []const BindEntry, addr: []const u32) ?AliasLivenessSplit`。
- **作用**：同组一个活一个退休 → 分裂（S3 应整组保留或整组退休）。
- **实现**：同时看到 live 与 unbound 即返回。
- **所有权 / 错误 / 调用**：S4 skipDeadCode 对活组整组 retire 或整组留下。

### `retiredIdentityStillReferenced` (`src/compiler/resolve_labels.zig:163`)

- **签名**：`fn retiredIdentityStillReferenced(entry: BindEntry, slot: labels.LabelSlot) bool`。
- **作用**：整组退休必须是故意的且两手空空：dead_skipped、无 reloc、ref_count 0。
- **实现**：`match_barrier` **不是**取消资格。屏障表示「即使 ref 0 也不得跨过折叠」；`hasBindInRange` 读 `initially_referenced`/`match_barrier` 不看 `dead_skipped`。只有 `skipDeadCode` 能退休屏障（扫不可达码），对应 S3 `deadBoundaryAt` 不可达臂。
- **所有权 / 错误 / 调用**：最终审计 BINDING_MISMATCH。

### `panicFinalBoundaryUniquenessViolation` (`src/compiler/resolve_labels.zig:169`)

- **签名**：`fn panicFinalBoundaryUniquenessViolation( bucket: cfg.DiffBucket, category: []const u8, role: []const u8, canonical_identities: [2]u32, raw_labels: [2]u32, product_offsets: [2]u32, final_addresses: [2]u32, ) noreturn`。
- **作用**：S4 边界违规：先记桶再 panic。先比 canonical，地址只作佐证。
- **实现**：固定格式 `key=canonical_identities=…:raw_labels=…`。
- **所有权 / 错误 / 调用**：`auditFinalBoundaryIdentity`。

### `resolvedJumpAddress` (`src/compiler/resolve_labels.zig:199`)

- **签名**：`fn resolvedJumpAddress( output: []const u8, output_len: u32, jump: JumpSlot, ) Error!u32`。
- **作用**：从最终流相对操作数还原目标 PC，校验 opcode 仍在 `pos - opcode_delta`。
- **实现**：dyn_env_probe 用 probe_label 偏移，否则 jump_label。size 1/2/4 读 i8/i16/i32。`target = pos + operand`。
- **所有权 / 错误 / 调用**：审计：编码地址必须等于 `addr[label]`。

### `decodeInstruction` (`src/compiler/resolve_labels.zig:237`)
...
### `opId` (`src/compiler/resolve_labels.zig:251`)
...
### `formOf` (`src/compiler/resolve_labels.zig:258`)
...
### `readU16` (`src/compiler/resolve_labels.zig:288`)
...
### `readU32At` (`src/compiler/resolve_labels.zig:294`)
...
### `readI32` (`src/compiler/resolve_labels.zig:301`)
...
### `validateProductMetadata` (`src/compiler/resolve_labels.zig:310`)
...
### `validateProductCode` (`src/compiler/resolve_labels.zig:348`)

- **签名**：`fn validateProductCode(product: *const resolve_variables.ResolvedProduct) Error!void`。
- **作用**：独立调用方的完整 S3 码证明。packed 路径跳过：S3 已按 opcode 大小发射，S4 边消费边解码，packed finalizer 再融合校验 S4。
- **实现**：goto/if/catch/gosub 必须 5 字节且 LabelId 合法；dyn_env_probe 10 字节；其它带 label 的形式拒绝（S3 只发宽 LabelId）。atom 与 ledger 锁步。
- **所有权 / 错误 / 调用**：qjs 同样信任 resolve_variables→resolve_labels 的内部字节码（34796）。

### `updateLabel` (`src/compiler/resolve_labels.zig:389`)
...
### `Resolver.deinit` (`src/compiler/resolve_labels.zig:498`)
...
### `Resolver.releaseConsumedProduct` (`src/compiler/resolve_labels.zig:527`)
...
### `Resolver.initScratch` (`src/compiler/resolve_labels.zig:561`)
...
### `Resolver.growOutput` (`src/compiler/resolve_labels.zig:656`)
...
### `Resolver.ensureOutput` (`src/compiler/resolve_labels.zig:671`)
...
### `Resolver.growOutputSources` (`src/compiler/resolve_labels.zig:678`)
...
### `Resolver.ensureOutputSources` (`src/compiler/resolve_labels.zig:693`)
...
### `Resolver.appendByte` (`src/compiler/resolve_labels.zig:700`)
...
### `v4Mask` (`src/compiler/resolve_labels.zig:719`)
...
### `loadV4Mask` (`src/compiler/resolve_labels.zig:726`)
...
### `v4On` (`src/compiler/resolve_labels.zig:742`)
...
### `noteFusionA` (`src/compiler/resolve_labels.zig:746`)
...
### `maybeFusePrev` (`src/compiler/resolve_labels.zig:880`)
...
### `appendRaw` (`src/compiler/resolve_labels.zig:896`)
...
### `appendU16` (`src/compiler/resolve_labels.zig:904`)
...
### `appendI16` (`src/compiler/resolve_labels.zig:911`)
...
### `appendU32` (`src/compiler/resolve_labels.zig:918`)
...
### `appendI32` (`src/compiler/resolve_labels.zig:925`)
...
### `appendOutputAtom` (`src/compiler/resolve_labels.zig:932`)
...
### `consumeAtomsRange` (`src/compiler/resolve_labels.zig:941`)
...
### `consumeInstructionAtom` (`src/compiler/resolve_labels.zig:979`)
...
### `absorbSources` (`src/compiler/resolve_labels.zig:995`)
...
### `absorbSourcesAtEnd` (`src/compiler/resolve_labels.zig:1003`)
...
### `hasInputSourceAt` (`src/compiler/resolve_labels.zig:1011`)
...
### `attachSource` (`src/compiler/resolve_labels.zig:1030`)
...
### `Resolver.attachSourceSlow` (`src/compiler/resolve_labels.zig:1062`)
...
### `attachInputSourceRangeAt` (`src/compiler/resolve_labels.zig:1089`)
...
### `lowerBoundBind` (`src/compiler/resolve_labels.zig:1120`)
...
### `hasBindInRange` (`src/compiler/resolve_labels.zig:1133`)
...
### `readIndex` (`src/compiler/resolve_labels.zig:1154`)
...
### `matchReturnAfter` (`src/compiler/resolve_labels.zig:1166`)
...
### `matchTailReturnAfterStrict` (`src/compiler/resolve_labels.zig:1192`)
...
### `matchSeq` (`src/compiler/resolve_labels.zig:1224`)
...
### `addReloc` (`src/compiler/resolve_labels.zig:1269`)
...
### `writeRelative` (`src/compiler/resolve_labels.zig:1291`)
...
### `retireSpannedDeadBinds` (`src/compiler/resolve_labels.zig:1332`)
...
### `refreshBindFrontier` (`src/compiler/resolve_labels.zig:1352`)
...
### `processBindsAt` (`src/compiler/resolve_labels.zig:1366`)
...
### `processBindsAtCold` (`src/compiler/resolve_labels.zig:1371`)
...
### `appendJumpSlot` (`src/compiler/resolve_labels.zig:1403`)
...
### `labelsShareBindOffset` (`src/compiler/resolve_labels.zig:1443`)
...
### `isSwitchDispatchBridgeAt` (`src/compiler/resolve_labels.zig:1482`)
...
### `codeHasLabel` (`src/compiler/resolve_labels.zig:1505`)
...
### `deadSwitchTrampolineCanReachLabel` (`src/compiler/resolve_labels.zig:1531`)
...
### `findJumpTarget` (`src/compiler/resolve_labels.zig:1581`)
...
### `findFoldedBranchTarget` (`src/compiler/resolve_labels.zig:1646`)
...
### `skipDeadCode` (`src/compiler/resolve_labels.zig:1653`)
