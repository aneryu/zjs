...
### `ParseState.initRootEmitter` (`src/parser.zig:925`)
...
### `ParseState.init` (`src/parser.zig:966`)
...
### `ParseState.initWithRuntime` (`src/parser.zig:975`)
...
### `ParseState.initCanonicalRootWithRuntime` (`src/parser.zig:989`)
...
### `ParseState.deinit` (`src/parser.zig:1003`)
...
### `ParseState.traceCompileValueRoots` (`src/parser.zig:1095`)
...
### `ParseState.traceCompileValueRootsThunk` (`src/parser.zig:1105`)
...
### `ParseState.compileValueRootProvider` (`src/parser.zig:1110`)
...
### `ParseState.activateCompileRoots` (`src/parser.zig:1118`)
...
### `ParseState.deactivateCompileValueRoots` (`src/parser.zig:1128`)
...
### `ParseState.setCurrentNamespaceAtom` (`src/parser.zig:1134`)
...
- **所有权 / 错误 / 调用**：只覆盖 `current_namespace_atom` 这个 `?Atom` 字段，**不 retain 旧值也不 release 新值**：TGC S3-c 之后编译期 atom 是普通借用 id，由 `CompileAtomScope` 统一作根，所以函数体里那层 `if (atom_id) |atom| atom else null` 的拆包是旧引用计数协议留下的空壳。不分配、无 error set。调用方是 TypeScript `namespace` 擦除路径（`parseNamespaceDeclarationWithIdent`）的进出点。

### `ParseState.setLastDeclaredAtom` (`src/parser.zig:1139`)

- **签名**：`fn setLastDeclaredAtom(self: *State, atom_id: Atom) void`。
- **作用**：记下刚声明完的那个名字 atom；TS 命名空间嵌套用它把内层绑定挂到外层命名空间对象上。
- **实现**：
两行：直接把 `last_declared_atom` 覆盖成传入的 atom，不 retain 旧值也不 release 旧值（atom 由 `atom_scope` 统一记账）。
- **所有权 / 错误 / 调用**：同上：单字段赋值，只存 id，不 retain/release（`State.deinit` 里对应的清理也只是置 `null`）。不分配、无 error set。给「匿名函数取名自最近一次声明」这类回填用。

### `ParseState.curFunc` (`src/parser.zig:1147`)
...
### `ParseState.funcAtVirtualIndex` (`src/parser.zig:1154`)
...
### `ParseState.deinitDeclarationConflictIndices` (`src/parser.zig:1159`)
...
### `ParseState.discardDeclarationConflictIndex` (`src/parser.zig:1169`)
...
### `ParseState.declarationConflictIndex` (`src/parser.zig:1182`)
...
### `ParseState.readyDeclarationConflictIndexForWrite` (`src/parser.zig:1213`)
...
### `ParseState.prepareLinkedDeclarationIndexWrite` (`src/parser.zig:1228`)
...
### `ParseState.commitLinkedDeclarationIndexWrite` (`src/parser.zig:1255`)
...
### `ParseState.prepareFunctionVarOriginIndexWrite` (`src/parser.zig:1290`)
...
### `ParseState.commitFunctionVarOriginIndexWrite` (`src/parser.zig:1326`)
...
### `ParseState.pushFunction` (`src/parser.zig:1370`)
...
### `ParseState.popFunction` (`src/parser.zig:1402`)
...
### `ParseState.discardCurrentFunction` (`src/parser.zig:1408`)
...
### `ParseState.discardFunctionDef` (`src/parser.zig:1413`)
...
### `ModuleArtifact.deinit` (`src/parser.zig:15798`)
...
### `Result.deinit` (`src/parser.zig:15820`)
...
### `Result.functionBytecode` (`src/parser.zig:15832`)
...
### `Result.takeFunctionBytecodeValue` (`src/parser.zig:15846`)
...
### `Result.byteCode` (`src/parser.zig:15855`)
...
### `Result.constants` (`src/parser.zig:15860`)
...
### `Result.closureVars` (`src/parser.zig:15865`)
...
### `Result.varDefs` (`src/parser.zig:15870`)
...
### `Result.openVarRefCount` (`src/parser.zig:15875`)
...
### `Result.filenameAtom` (`src/parser.zig:15880`)
...
### `Result.scriptOrModuleAtom` (`src/parser.zig:15885`)
...
### `Result.entryContract` (`src/parser.zig:15890`)
...
### `Result.isStrict` (`src/parser.zig:15900`)
...
### `Result.isGlobalVar` (`src/parser.zig:15905`)
...
### `Result.isDirectOrIndirectEval` (`src/parser.zig:15915`)
...
### `Result.isModule` (`src/parser.zig:15920`)
...
### `Result.moduleArtifact` (`src/parser.zig:15924`)
...
### `Result.moduleRecord` (`src/parser.zig:15931`)
...
### `Result.takeModuleArtifact` (`src/parser.zig:15939`)
...
### `Result.hasFeature` (`src/parser.zig:15948`)
...
### `compile_entry.isPrivateEvalClosureKind` (`src/parser.zig:15981`)
...
### `compile_entry.isPrivateSetterCompanion` (`src/parser.zig:15993`)
...
### `compile_entry.restoreDirectEvalPrivateBoundNames` (`src/parser.zig:15999`)
...
### `compile_entry.elapsedNanosSince` (`src/parser.zig:16032`)
...
### `compile_entry.initCompileCarrier` (`src/parser.zig:16037`)
...
### `compile_entry.compile` (`src/parser.zig:16059`)
...
### `compile_entry.compileQjsProgram` (`src/parser.zig:16180`)

- **签名**：`fn compileQjsProgram( rt: *JSRuntime, filename_atom: atom.Atom, source: []const u8, options: OptionsImpl, compile_context: bytecode.CompileContext, function: *bytecode.Bytecode, features: *std.EnumSet(FeatureImpl), pending_diagnostic: *?parser_impl.PendingDiagnostic, ) !*bytecode.FunctionBytecode`。
...
- **所有权 / 错误 / 调用**：三层 `defer` 决定了拆解顺序：`lex.deinit()`、`state.deinit(rt)`、以及**finalize 期间那次 allocator 切换的还原**——注释点明必须在 `State.deinit` 之前还原成解析用的 allocator，否则 parser 的临时缓冲会用错误的 allocator 回收。`errdefer pending_diagnostic.* = state.pending_diagnostic;` 是唯一的错误出参：`State` 马上要被 `defer` 拆掉，所以诊断要在那之前按值抄给 `compile`。返回的 `*FunctionBytecode` 指向 `createFunctionBytecode` 在 `compile_context.artifactAllocator()` 上发布的切片首元素，所有权交给调用方（`compile` 装进 `Result.artifact`）；module 形态的 `record` 仍留在 `function.module_record` 上，由 `compile` 移走。参数 `filename_atom` **实际未被使用**（函数末尾一条 `_ = filename_atom;`），文件名是经 `initCompileCarrier` 提前写进 `function` 的。错误：`OutOfMemory` 与全部解析错误原样上抛给 `compile` 分类。唯一调用方 `compile`（`src/parser.zig:16119`）。

### `compile_entry.setPendingSyntaxError` (`src/parser.zig:16336`)
...
### `compile_entry.isInternalCompilerError` (`src/parser.zig:16352`)
...
### `compile_entry.setInternalCompilerError` (`src/parser.zig:16374`)
...
### `compile_entry.setFallbackSyntaxError` (`src/parser.zig:16396`)
...
### `compile_entry.nextFallbackSyntaxTokenInto` (`src/parser.zig:16434`)
...
### `compile_entry.fallbackSlashStartsRegexp` (`src/parser.zig:16447`)
...
- 清单函数数（本文件分到）: 76（`src/parser.zig` 全文件 649）
- 本文标题覆盖（本文件）: 76
