...
### `arrayAtCall` (`src/exec/array_ops.zig:1320`)
...
### `arrayIterationModeFromRecordId` (`src/exec/array_ops.zig:1391`)
...
### `arrayIterationModeIsFind` (`src/exec/array_ops.zig:1406`)
...
### `arrayIterationCall` (`src/exec/array_ops.zig:1413`)
...
### `arrayIterationModeCall` (`src/exec/array_ops.zig:1459`)
...
### `typedArrayMapFilter` (`src/exec/array_ops.zig:1599`)
...
### `typedArrayCreateWithLength` (`src/exec/array_ops.zig:1682`)
...
### `arrayReduceCall` (`src/exec/array_ops.zig:1699`)
...
### `arrayReduceRightSparseLarge` (`src/exec/array_ops.zig:1798`)
...
### `ArrayIterationMode.lessThan` (`src/exec/array_ops.zig:1817`)
...
### `arrayMethodTypedArrayLength` (`src/exec/array_ops.zig:1840`)
...
### `typedArraySearchScan` (`src/exec/array_ops.zig:1863`)
...
### `searchScanResult` (`src/exec/array_ops.zig:1997`)
...
### `scanU8` (`src/exec/array_ops.zig:2002`)
...
### `scanElem` (`src/exec/array_ops.zig:2016`)
...
### `readFloat` (`src/exec/array_ops.zig:2033`)
...
### `scanFloat16` (`src/exec/array_ops.zig:2039`)
...
### `TypedSearchMode.match` (`src/exec/array_ops.zig:2044`)
...
### `TypedSearchMode.match` (`src/exec/array_ops.zig:2052`)
...
### `scanFloat` (`src/exec/array_ops.zig:2076`)
...
### `TypedSearchMode.match` (`src/exec/array_ops.zig:2081`)
...
### `scanFloatPredicate` (`src/exec/array_ops.zig:2105`)
...
### `arrayLastIndexSparseLarge` (`src/exec/array_ops.zig:2121`)
...
### `TypedSearchMode.lessThan` (`src/exec/array_ops.zig:2142`)
...
### `arrayFirstIndexStart` (`src/exec/array_ops.zig:2153`)
...
### `arrayLastIndexStart` (`src/exec/array_ops.zig:2172`)
...
### `arraySliceCall` (`src/exec/array_ops.zig:2192`)
...
### `typedArraySliceSubarrayCall` (`src/exec/array_ops.zig:2299`)
...
### `typedArrayConstructorForObject` (`src/exec/array_ops.zig:2396`)
...
### `typedArraySpeciesConstructorForObject` (`src/exec/array_ops.zig:2406`)
...
### `fastDenseArraySplice` (`src/exec/array_ops.zig:2440`)
...
### `arraySpliceCall` (`src/exec/array_ops.zig:2573`)
...
### `arraySpliceCallImpl` (`src/exec/array_ops.zig:2588`)
...
### `arrayCopyWithinCall` (`src/exec/array_ops.zig:2711`)
...
### `arrayFillCall` (`src/exec/array_ops.zig:2819`)
...
### `arrayPrototypeChainHasNoIndexedProperties` (`src/exec/array_ops.zig:2919`)
...
### `arrayPushCall` (`src/exec/array_ops.zig:2929`)
...
### `tryFastArrayPush` (`src/exec/array_ops.zig:2951`)
...
### `arrayPushCallImpl` (`src/exec/array_ops.zig:2970`)
...
### `arrayPopCall` (`src/exec/array_ops.zig:3013`)
...
### `arrayPopCallImpl` (`src/exec/array_ops.zig:3030`)
...
### `fastDenseArrayPop` (`src/exec/array_ops.zig:3079`)
...
### `fastEmptyArrayPop` (`src/exec/array_ops.zig:3092`)
...
### `arrayShiftCall` (`src/exec/array_ops.zig:3101`)
...
### `fastDenseArrayShift` (`src/exec/array_ops.zig:3145`)
...
### `fastDenseArrayUnshift` (`src/exec/array_ops.zig:3173`)
...
### `arrayUnshiftCall` (`src/exec/array_ops.zig:3229`)
...
### `arrayReverseCall` (`src/exec/array_ops.zig:3285`)
...
### `arrayUnshiftSparseLarge` (`src/exec/array_ops.zig:3390`)
...
### `TypedSearchMode.lessThan` (`src/exec/array_ops.zig:3411`)
...
### `arrayMoveIndex` (`src/exec/array_ops.zig:3431`)
...
### `arrayCopyPresentIndex` (`src/exec/array_ops.zig:3464`)
...
### `ensureSettableForArrayBuiltin` (`src/exec/array_ops.zig:3488`)
...
### `ensureLengthWritableForArrayBuiltin` (`src/exec/array_ops.zig:3502`)
...
### `arrayRelativeIndex` (`src/exec/array_ops.zig:3509`)
...
### `arrayRelativeIndexFromNumber` (`src/exec/array_ops.zig:3517`)

- **签名**：`pub fn arrayRelativeIndexFromNumber(length: usize, n: f64) usize`。
- **作用**：把相对下标（负从尾）夹到 [0, length]；NaN/−Infinity 一律给 0（缺省值由 `arrayRelativeIndex` 在参数缺席时处理）。
- **实现**：NaN 与 −∞ → 0；+∞ → `length`；`@trunc` 后为负时按 `length + integer` 从尾部折回（≤0 给 0），非负时 `>= length` 给 `length`，其余原样转 usize。
- **所有权 / 错误 / 调用**：无：纯数值夹紧（NaN 与 -∞ → 0，+∞ 与超长 → length，负数从尾部折回），不分配、无 error set、不跑用户代码。缺省值不经本函数：那是 `arrayRelativeIndex` 的「参数缺席」早退分支。调用方 `arrayCopyWithinCall` 的 TypedArray 臂与 `arrayRelativeIndex`。

### `toIntegerOrInfinityForArrayMethod` (`src/exec/array_ops.zig:3532`)
...
- 清单函数数（本文件分到）: 57（`src/exec/array_ops.zig` 全文件 231）
- 本文标题覆盖: 57
- 未覆盖: 无
