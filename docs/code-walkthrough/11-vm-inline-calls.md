...
### `InlineTarget.captureSlice` (`src/exec/inline_calls.zig:108`)
...
### `ResolvedInlineFunction.bind` (`src/exec/inline_calls.zig:123`)
...
### `resolveInlineFunction` (`src/exec/inline_calls.zig:137`)
...
### `resolveInlineFunctionFromObject` (`src/exec/inline_calls.zig:151`)
...
### `resolveNoSuspendAsync` (`src/exec/inline_calls.zig:196`)
...
### `resolveInlineDirectConstructorFunction` (`src/exec/inline_calls.zig:218`)
...
### `resolveInlineSpreadConstructorFunction` (`src/exec/inline_calls.zig:246`)
...
### `resolveInlineTarget` (`src/exec/inline_calls.zig:278`)

- **签名**：`pub inline fn resolveInlineTarget(ctx: *core.JSContext, global: *core.Object, receiver: core.JSValue, func: core.JSValue) ?InlineTarget`。
...
### `resolveInlineTargetInto` (`src/exec/inline_calls.zig:286`)

- **签名**：`pub inline fn resolveInlineTargetInto( target: *InlineTarget, ctx: *core.JSContext, global: *core.Object, receiver: core.JSValue, func: core.JSValue, ) bool`。
- **作用**：解析加绑定一步到位：判定 callable 可否同机内联，可以就把结果连同 receiver 就地写进调用方给的 `InlineTarget` 槽，省掉一次按值返回的结构体搬运。
- **实现**：`_ = ctx`（参数只为保持签名对称）；`resolveInlineFunction(global, func)` 失败返回 false，成功则 `target.* = resolved.bind(receiver, func)` 并返回 true。
- **所有权 / 错误 / 调用**：错误：无，返回 false 表示不可内联。所有权：把结果就地写进调用方栈上的 `target`，不分配；`ctx` 形参当前未使用（`_ = ctx`）。调用：`src/exec/call_runtime.zig:491` 与本文件 `resolveInlineTarget`（`:280`）。

### `ReturnContinuation.deinit` (`src/exec/inline_calls.zig:329`)

- **签名**：`pub fn deinit(self: *ReturnContinuation, _: *core.JSRuntime) void`。
- **作用**：把 continuation 记录复位成 `.next`/0。
- **实现**：`.proxy_get` 且 payload 非 null_atom 的分支体目前是空的（不释放 atom）；随后无条件 `self.action = .next; self.payload = 0`。
- **所有权 / 错误 / 调用**：错误：无。所有权：`proxy_get` 的 atom payload 已不再需要释放（该分支体为空，只保留位置），函数只把 `action`/`payload` 复位；`*JSRuntime` 形参未用。调用：`src/exec/inline_calls.zig:1394`、`:5168`、`:5192`、`:5223`——`Machine` 重定向与丢弃续延的四处。

### `ReturnContinuation.takeAtom` (`src/exec/inline_calls.zig:336`)
...
### `ReturnContinuation.takeForOfDepth` (`src/exec/inline_calls.zig:344`)
...
### `Entry.takeContinuation` (`src/exec/inline_calls.zig:422`)
...
### `Entry.isEmptyLeaf` (`src/exec/inline_calls.zig:429`)
...
### `Entry.isExactArgsLeaf` (`src/exec/inline_calls.zig:433`)
...
### `Entry.isForwardedLeaf` (`src/exec/inline_calls.zig:437`)
...
### `Entry.hasSpecialReturn` (`src/exec/inline_calls.zig:441`)
...
### `Entry.isNativeBoundaryReturn` (`src/exec/inline_calls.zig:445`)
...
### `Entry.isLeanBoundaryReturn` (`src/exec/inline_calls.zig:454`)
...
### `Entry.completesConstructor` (`src/exec/inline_calls.zig:459`)
...
### `Entry.emptyLeafResumeWords` (`src/exec/inline_calls.zig:473`)
...
### `Entry.setEmptyLeafResume` (`src/exec/inline_calls.zig:478`)
...
### `Entry.emptyLeafResumePc` (`src/exec/inline_calls.zig:485`)
...
### `Entry.emptyLeafResumeSp` (`src/exec/inline_calls.zig:490`)
...
### `Entry.tailChainBudgetSlot` (`src/exec/inline_calls.zig:504`)
...
### `Entry.adoptContinuation` (`src/exec/inline_calls.zig:515`)
...
### `Entry.isOrdinaryReturn` (`src/exec/inline_calls.zig:588`)
...
### `Entry.canUseSimpleTeardown` (`src/exec/inline_calls.zig:595`)
...
### `Entry.deinit` (`src/exec/inline_calls.zig:605`)
...
### `Entry.deinitReturned` (`src/exec/inline_calls.zig:629`)
...
### `deinitEmptyLeaf` (`src/exec/inline_calls.zig:641`)
...
- **实现**：唯一语句是 `self.deinitEmptyLeafInline(ctx.runtime)`。热 handler 的返回臂直接调 `deinitEmptyLeafInline`，让 arena restore 不经过 bl/ret；本包装把冷消费者（驱动侧 `.returned` / falloff）从 opcode 体里拉开。
...
### `Entry.deinitEmptyLeafInline` (`src/exec/inline_calls.zig:660`)
...
### `Entry.deinitExactArgsLeafInline` (`src/exec/inline_calls.zig:682`)
...
### `Entry.deinitForwardedLeafInline` (`src/exec/inline_calls.zig:703`)
...
### `Entry.deinitSimple` (`src/exec/inline_calls.zig:724`)
...
### `Entry.deinitSimpleResources` (`src/exec/inline_calls.zig:733`)
...
### `Entry.deinitOrdinarySimpleResources` (`src/exec/inline_calls.zig:750`)
...
### `Entry.deinitOrdinaryReturned` (`src/exec/inline_calls.zig:766`)
...
### `Entry.deinitConstructorReturned` (`src/exec/inline_calls.zig:794`)
...
### `Entry.deinitGeneral` (`src/exec/inline_calls.zig:815`)
...
### `Entry.deinitGeneralResources` (`src/exec/inline_calls.zig:820`)
...
### `Entry.deinitForTailReplacement` (`src/exec/inline_calls.zig:830`)
...
### `ExecutionLevel.function` (`src/exec/inline_calls.zig:858`)
...
### `MachineBacktraceView.root` (`src/exec/inline_calls.zig:894`)
...
### `MachineBacktraceView.segment` (`src/exec/inline_calls.zig:901`)
...
### `MachineBacktraceView.freeze` (`src/exec/inline_calls.zig:913`)
...
### `MachineBacktraceView.thaw` (`src/exec/inline_calls.zig:919`)
...
### `resolveMachineBacktraceRange` (`src/exec/inline_calls.zig:926`)
...
### `consumeInlineThenPhysical` (`src/exec/inline_calls.zig:952`)
...
### `resolveMachineBacktraceView` (`src/exec/inline_calls.zig:965`)
...
### `nativeBacktraceSnapshot` (`src/exec/inline_calls.zig:977`)
...
### `copyValueSlotPinned` (`src/exec/inline_calls.zig:1012`)
...
### `activeInvocation` (`src/exec/inline_calls.zig:1026`)
...
### `NativeBoundaryScope.init` (`src/exec/inline_calls.zig:1064`)
...
### `NativeBoundaryScope.push` (`src/exec/inline_calls.zig:1090`)
...
### `NativeBoundaryScope.deinit` (`src/exec/inline_calls.zig:1101`)
...
### `NativeBoundaryScope.finish` (`src/exec/inline_calls.zig:1127`)
...
### `NativeBoundaryScope.fenceDepth` (`src/exec/inline_calls.zig:1149`)
...
### `NativeBoundaryScope.expectedTop` (`src/exec/inline_calls.zig:1153`)
...
### `NativeBoundaryScope.popBacktrace` (`src/exec/inline_calls.zig:1157`)
...
### `IdleBoundaryScope.init` (`src/exec/inline_calls.zig:1189`)
...
### `IdleBoundaryScope.fenceDepth` (`src/exec/inline_calls.zig:1196`)
...
### `IdleBoundaryScope.expectedTop` (`src/exec/inline_calls.zig:1200`)
...
### `IdleBoundaryScope.push` (`src/exec/inline_calls.zig:1204`)
...
### `IdleBoundaryScope.finish` (`src/exec/inline_calls.zig:1206`)
...
### `deinit` (`src/exec/inline_calls.zig:1212`)
...
### `LeanFrame.isIntact` (`src/exec/inline_calls.zig:1248`)
...
### `LeanFrame.initInPlace` (`src/exec/inline_calls.zig:1255`)
...
### `Machine.init` (`src/exec/inline_calls.zig:1341`)
...
### `Machine.alreadyTargets` (`src/exec/inline_calls.zig:1360`)
...
### `Machine.retarget` (`src/exec/inline_calls.zig:1364`)
...
### `Machine.deinitStorage` (`src/exec/inline_calls.zig:1375`)
...
### `Machine.deinit` (`src/exec/inline_calls.zig:1390`)
...
### `Machine.topEntry` (`src/exec/inline_calls.zig:1403`)
...
### `Machine.loadCurrentLevel` (`src/exec/inline_calls.zig:1411`)
...
### `Machine.currentLevel` (`src/exec/inline_calls.zig:1430`)
...
### `Machine.entryAt` (`src/exec/inline_calls.zig:1436`)
...
### `Machine.acquireSlot` (`src/exec/inline_calls.zig:1440`)
...
### `acquireSlotSlow` (`src/exec/inline_calls.zig:1451`)
...
### `Machine.ArgsSource.initStack` (`src/exec/inline_calls.zig:1493`)
...
### `Machine.ArgsSource.initMoved` (`src/exec/inline_calls.zig:1497`)
...
### `Machine.ArgsSource.init` (`src/exec/inline_calls.zig:1503`)
...
### `Machine.ArgsSource.valueCount` (`src/exec/inline_calls.zig:1514`)
...
### `Machine.ArgsSource.slice` (`src/exec/inline_calls.zig:1518`)
...
### `Machine.ArgsSource.argCount` (`src/exec/inline_calls.zig:1522`)
...
### `Machine.pushFrame` (`src/exec/inline_calls.zig:1544`)
...
### `Machine.isSimpleInlineFrame` (`src/exec/inline_calls.zig:1629`)
...
### `Machine.paddedSimpleInlineMode` (`src/exec/inline_calls.zig:1672`)
...
### `Machine.methodSimpleInlineMode` (`src/exec/inline_calls.zig:1690`)
...
### `Machine.isStrictSimpleInlineFrame` (`src/exec/inline_calls.zig:1720`)
...
### `setupFallbackInlineEntry` (`src/exec/inline_calls.zig:1740`)
...
### `Machine.simpleInlineSlabTotal` (`src/exec/inline_calls.zig:1762`)
...
### `setupSimpleInlineEntryWarm` (`src/exec/inline_calls.zig:1787`)
...
### `Machine.setupSimpleInlineEntryDispatch` (`src/exec/inline_calls.zig:1878`)
...
### `Machine.setupSimpleConstructorEntryDispatch` (`src/exec/inline_calls.zig:1900`)
...
### `setupSimpleInlineEntry` (`src/exec/inline_calls.zig:1942`)
...
### `setupSimpleConstructorEntry` (`src/exec/inline_calls.zig:1962`)
...
### `Machine.setupSimpleInlineEntryImpl` (`src/exec/inline_calls.zig:1966`)
...
### `pushExactSimpleFrame` (`src/exec/inline_calls.zig:2159`)
...
### `pushExactSimpleFrameSlow` (`src/exec/inline_calls.zig:2294`)
...
### `Machine.pushExactSimpleOrSlow` (`src/exec/inline_calls.zig:2306`)
...
### `Machine.pushExactSimpleFrameImpl` (`src/exec/inline_calls.zig:2328`)
...
### `pushEmptyLeafFrame` (`src/exec/inline_calls.zig:2390`)
...
### `pushExactArgsLeafFrame` (`src/exec/inline_calls.zig:2445`)
...
### `pushCaptureLeafFrame` (`src/exec/inline_calls.zig:2505`)
...
### `Machine.assertLeafEligible` (`src/exec/inline_calls.zig:2559`)
...
### `Machine.assertExactArgsLeafEligible` (`src/exec/inline_calls.zig:2573`)
...
### `Machine.assertCaptureLeafEligible` (`src/exec/inline_calls.zig:2588`)
...
### `Machine.finishEmptyLeafFrame` (`src/exec/inline_calls.zig:2614`)
...
### `Machine.finishExactArgsLeafFrame` (`src/exec/inline_calls.zig:2677`)
...
### `Machine.finishCaptureLeafFrame` (`src/exec/inline_calls.zig:2782`)
...
### `Machine.callerResumePc` (`src/exec/inline_calls.zig:2838`)
...
### `Machine.tryPushEmptyLeafCallFast` (`src/exec/inline_calls.zig:2851`)
...
### `Machine.tryPushExactArgsLeafCallFast` (`src/exec/inline_calls.zig:2907`)
...
### `Machine.tryPushForwardedExactArgsLeafFast` (`src/exec/inline_calls.zig:2965`)
...
### `Machine.tryPushCaptureLeafCallFast` (`src/exec/inline_calls.zig:3014`)
...
### `setupInlineEntry` (`src/exec/inline_calls.zig:3067`)
...
### `Machine.isBorrowedIteratorSimpleInlineFrame` (`src/exec/inline_calls.zig:3221`)
...
### `setupBorrowedIteratorEntry` (`src/exec/inline_calls.zig:3237`)
...
### `Machine.sourceCallableSlot` (`src/exec/inline_calls.zig:3300`)
...
### `Machine.sourceReceiverSlot` (`src/exec/inline_calls.zig:3304`)
...
### `Machine.takeSourceSlot` (`src/exec/inline_calls.zig:3308`)
...
### `Machine.storeOpenVarRefNulls` (`src/exec/inline_calls.zig:3318`)
...
### `Machine.sourceHasStackRegion` (`src/exec/inline_calls.zig:3331`)
...
### `Machine.sourceArgCount` (`src/exec/inline_calls.zig:3335`)
...
### `Machine.sourceArgs` (`src/exec/inline_calls.zig:3339`)
...
### `Machine.canBorrowSourceArgs` (`src/exec/inline_calls.zig:3344`)
...
### `Machine.cleanupSource` (`src/exec/inline_calls.zig:3356`)

- **签名**：`fn cleanupSource(rt: *core.JSRuntime, source: ArgsSource, mode: SourceCleanupMode) void`。
...
### `Machine.cleanupStackSource` (`src/exec/inline_calls.zig:3364`)

- **签名**：`fn cleanupStackSource(rt: *core.JSRuntime, source: ArgsSource) void`。
- **作用**：把调用方栈上这段调用区整段置空：建帧在把值交给帧之前失败时用它，保证没有槽既被放弃又仍被当成活值。
- **实现**：`source.metadata.moved` 为真直接返回（moved 区不归调用方栈）。否则从 `source.valueCount()` 倒着往 0 走，逐槽 `freeSourceSlot(rt, &source.values[index])`。倒序是为了与建帧时的正序转移对称。
...
### `Machine.cleanupStackSourcePreserveArgs` (`src/exec/inline_calls.zig:3373`)

- **签名**：`fn cleanupStackSourcePreserveArgs(rt: *core.JSRuntime, source: ArgsSource) void`。
...
### `Machine.freeSourceSlot` (`src/exec/inline_calls.zig:3379`)

- **签名**：`inline fn freeSourceSlot(_: *core.JSRuntime, slot: *core.JSValue) void`。
- **作用**：放弃调用区里的一个槽：VM 值不计引用计数，所以「释放」就是把它写成 undefined，让 GC 的精确 walk 不再把它当活引用。
- **实现**：单行 `slot.* = core.JSValue.undefinedValue()`。`*JSRuntime` 形参写成 `_:` 直接丢弃，保留在签名里只是为了与需要 runtime 的历史/未来实现保持同一调用形态。
- **所有权 / 错误 / 调用**：错误：无。所有权：VM 值不计引用计数，所以「释放」就是把槽写成 undefined，让收集器的精确 walk 不再把它当活值；`rt` 形参直接以 `_:` 丢弃。调用：本文件 10 处，如 `:2411`、`:2471`、`:3369`（`cleanupStackSource` 的循环体）。

### `Machine.pushPlainCall` (`src/exec/inline_calls.zig:3389`)
...
### `Machine.pushEmptyLeafCall` (`src/exec/inline_calls.zig:3420`)
...
### `Machine.pushExactArgsLeafCall` (`src/exec/inline_calls.zig:3438`)
...
### `Machine.pushCaptureLeafCall` (`src/exec/inline_calls.zig:3458`)
...
### `Machine.pushMethodCall` (`src/exec/inline_calls.zig:3493`)
...
### `pushConstructorCall` (`src/exec/inline_calls.zig:3526`)
...
### `pushDerivedConstructorCall` (`src/exec/inline_calls.zig:3619`)
...
### `Machine.pushCall` (`src/exec/inline_calls.zig:3668`)
...
### `Machine.nativeBoundarySimpleEligible` (`src/exec/inline_calls.zig:3683`)
...
### `Machine.pushNativeBoundaryCopiedArgs` (`src/exec/inline_calls.zig:3695`)
...
### `Machine.pushNativeBoundaryMovedArgs` (`src/exec/inline_calls.zig:3707`)
...
### `Machine.pushLeanEntry` (`src/exec/inline_calls.zig:3721`)
...
### `Machine.tryPushNativeBoundaryCopiedArgsFast` (`src/exec/inline_calls.zig:3784`)
...
### `Machine.tryPushNativeBoundaryMovedArgsFast` (`src/exec/inline_calls.zig:3795`)
...
### `Machine.tryPushNativeBoundaryArgsFast` (`src/exec/inline_calls.zig:3804`)
...
### `Machine.tryPushNativeBoundaryEmptyFast` (`src/exec/inline_calls.zig:3840`)
...
### `Machine.tryPushNativeBoundaryLeafArgsFast` (`src/exec/inline_calls.zig:3911`)
...
### `Machine.tryPushNativeBoundarySimpleGeneralFast` (`src/exec/inline_calls.zig:4014`)
...
### `Machine.pushNativeBoundarySimple` (`src/exec/inline_calls.zig:4119`)
...
### `Machine.pushNativeBoundaryLeafArgs` (`src/exec/inline_calls.zig:4180`)
...
### `Machine.pushNativeBoundaryEmpty` (`src/exec/inline_calls.zig:4273`)
...
### `setupNativeBoundarySimpleEntry` (`src/exec/inline_calls.zig:4346`)
...
### `Machine.pushMovedCall` (`src/exec/inline_calls.zig:4460`)
...
### `Machine.pushAsyncMovedCall` (`src/exec/inline_calls.zig:4491`)
...
### `Machine.completeAsync` (`src/exec/inline_calls.zig:4500`)
...
### `Machine.catchAsyncBoundary` (`src/exec/inline_calls.zig:4509`)
...
### `Machine.pushBorrowedIteratorNext` (`src/exec/inline_calls.zig:4523`)
...
### `Machine.assertBorrowedIteratorWarmEligible` (`src/exec/inline_calls.zig:4547`)
...
### `Machine.readValueAsIntPair` (`src/exec/inline_calls.zig:4563`)
...
### `Machine.tryPushBorrowedIteratorNextFast` (`src/exec/inline_calls.zig:4591`)
...
### `Machine.finishBorrowedIteratorFrame` (`src/exec/inline_calls.zig:4643`)
...
### `Machine.tailCallReuse` (`src/exec/inline_calls.zig:4722`)
...
### `Machine.popFrameMode` (`src/exec/inline_calls.zig:4819`)
...
### `popTailChainFrameMode` (`src/exec/inline_calls.zig:4858`)
...
### `Machine.popOrdinaryFrame` (`src/exec/inline_calls.zig:4889`)
...
### `Machine.popFrame` (`src/exec/inline_calls.zig:4914`)
...
### `Machine.popReturnedFrame` (`src/exec/inline_calls.zig:4920`)
...
### `Machine.popReturnedNativeBoundary` (`src/exec/inline_calls.zig:4931`)
...
### `Machine.popReturnedLean` (`src/exec/inline_calls.zig:4976`)
...
### `Machine.popReturnedEmptyLeaf` (`src/exec/inline_calls.zig:4993`)

- **签名**：`pub inline fn popReturnedEmptyLeaf(self: *Machine, rt: *core.JSRuntime) void`。
- **作用**：空叶帧正常返回的专用出帧臂——整条返回路径上最短的一条，代价只有一次 rc 递减和一次 arena 水位回滚。
- **实现**：一串断言把这条最短路径的前提钉死：`rt == self.ctx.runtime`（runtime 由返回 handler 从 `Vm` 的驻留字段直接传入，省掉 machine→ctx→runtime 那条链）、`dying.isEmptyLeaf()`、非 `tail_chain`、`return_action == .next`、payload 为 0，以及两条来自发布事实的静态结论——`!teardown.copy_argv` 且 `frame.function.arg_count == 0`（空叶几何按 `bytecode.zig` 的 `empty_leaf_geometry` 就是零参、从不按 copy_argv 定价，所以补参前缀静态为空）。再用 `bytecodeLeafFrameAllocaSize` 对账 `planned_stack_bytes`。真动作三条：`deinitEmptyLeafInline(rt)`（内联展开，注释说明这样能把空叶返回路径上唯一的 bl/ret 去掉，只留 rc==0 分支后面那个 outline 的 `destroyZeroRef`）、`leaveInlineCallDepthBytesRt` 退预算、`depth -= 1` 与 `top = dying.prev` 解链。
...
### `Machine.popReturnedExactArgsLeaf` (`src/exec/inline_calls.zig:5024`)
...
### `Machine.popReturnedForwardedLeaf` (`src/exec/inline_calls.zig:5053`)
...
### `Machine.popReturn` (`src/exec/inline_calls.zig:5077`)
...
### `popConstructorReturn` (`src/exec/inline_calls.zig:5128`)
...
### `Machine.discardToDepth` (`src/exec/inline_calls.zig:5163`)
...
### `Machine.unwindForErrorToDepth` (`src/exec/inline_calls.zig:5176`)
...
### `Machine.unwindForError` (`src/exec/inline_calls.zig:5214`)
...
### `writeProbeFile` (`src/exec/small_inline.zig:36`)

- **签名**：`pub fn writeProbeFile() void`。
- **作用**：进程收尾时按需打印小函数内联的两个探针计数（`probe_prep` = 准备过多少次特化，`probe_take` = 真正命中执行多少次），供调优时看 take 率。
...
- **所有权 / 错误 / 调用**：错误：无；`ZJS_INLINE_PROBE` 未设置或为空串直接返回。所有权：只读 `probe_prep`/`probe_take` 两个全局计数器并打印，不分配、不建根。调用：`src/internal_root.zig` 的收尾路径（进程退出时打印探针计数）。

### `decodeCallerState` (`src/exec/small_inline.zig:135`)
...
### `callerState` (`src/exec/small_inline.zig:141`)
...
### `callerStateMut` (`src/exec/small_inline.zig:147`)
...
### `setCallerState` (`src/exec/small_inline.zig:151`)
...
### `applyForwardEligible` (`src/exec/small_inline.zig:164`)
...
### `anyApplyForwardSite` (`src/exec/small_inline.zig:178`)
...
### `siteSlot` (`src/exec/small_inline.zig:186`)
...
### `applyForwardColdOf` (`src/exec/small_inline.zig:195`)
...
### `siteApplyForwarded` (`src/exec/small_inline.zig:199`)
...
### `markApplyForwardInlined` (`src/exec/small_inline.zig:203`)
...
### `setBorrowedRealm` (`src/exec/small_inline.zig:210`)
...
### `destroyCallerState` (`src/exec/small_inline.zig:216`)

- **签名**：`pub fn destroyCallerState(rt: *JSRuntime, fb: *FunctionBytecode) void`。
- **作用**：`FunctionBytecode` 销毁时释放它挂着的小函数内联状态，并把 pad 里的两个借用字清零。
- **实现**：`callerStateMut(fb)` 为 null 直接返回（从没装过）。否则先 `setCallerState(fb, null)` 断开指针，再走一个遍历 `inlined_len` 的空循环体——`traceCallerStateAtoms` 的文档注释把自己称作「`destroyCallerState` 的 atom 释放的镜像」，但当前这个循环体确实是空的，站点里的 atom 不在这里释放，`setBorrowedRealm(fb, null)` 清掉 realm 字，最后 `rt.memory.destroy(CallerState, state)` 归还内存。
...
### `traceCallerStateAtoms` (`src/exec/small_inline.zig:234`)
...
- **作用**：GC 标记阶段的边报告函数：在 `FunctionBytecode` 被 trace 时把 `CallerState` 里持有的 atom id 全部报给 tracer——这是 `destroyCallerState` 那侧 atom 释放的镜像（TGC S3 §2.2 边 H）。
...
### `fillDefaultCallerState` (`src/exec/small_inline.zig:255`)
...
### `ensureCallerState` (`src/exec/small_inline.zig:262`)
...
### `hasTrailingAfterReturn` (`src/exec/small_inline.zig:272`)
...
### `budgetRemaining` (`src/exec/small_inline.zig:285`)
...
### `findInlinedSite` (`src/exec/small_inline.zig:293`)
...
### `siteForPc` (`src/exec/small_inline.zig:302`)
...
### `mapCalleePc` (`src/exec/small_inline.zig:312`)
...
### `noteMonomorphic` (`src/exec/small_inline.zig:325`)
...
### `specializeCallSite` (`src/exec/small_inline.zig:367`)
...
### `locIndexOf` (`src/exec/small_inline.zig:423`)
...
### `isPutLoc` (`src/exec/small_inline.zig:435`)
...
### `isGetLoc` (`src/exec/small_inline.zig:442`)
...
### `isPutArg` (`src/exec/small_inline.zig:449`)
...
### `isForwardForbiddenOp` (`src/exec/small_inline.zig:456`)
...
### `analyzeApplyForward` (`src/exec/small_inline.zig:469`)
...
### `prevOpBefore` (`src/exec/small_inline.zig:608`)
...
### `firstThisLocal` (`src/exec/small_inline.zig:620`)
...
### `emitByte` (`src/exec/small_inline.zig:648`)
...
### `emitSlice` (`src/exec/small_inline.zig:655`)
...
### `emitLocOp` (`src/exec/small_inline.zig:662`)
...
### `emitCallMethodApplyFwd` (`src/exec/small_inline.zig:681`)
...
### `clearPropSiteIndices` (`src/exec/small_inline.zig:695`)
...
### `emitGetField2` (`src/exec/small_inline.zig:707`)
...
### `emitGoto` (`src/exec/small_inline.zig:718`)
...
### `recordMap` (`src/exec/small_inline.zig:725`)
...
### `rewriteBody` (`src/exec/small_inline.zig:733`)
...
### `relTarget` (`src/exec/small_inline.zig:959`)
...
### `patchJump` (`src/exec/small_inline.zig:966`)
...
### `collectSameCalleeConstructorPcs` (`src/exec/small_inline.zig:987`)
...
### `cloneAndExpand` (`src/exec/small_inline.zig:1038`)
...
### `consumedArgSlots` (`src/exec/small_inline.zig:1331`)
...
### `windowFits` (`src/exec/small_inline.zig:1337`)
...
### `installInlineWindow` (`src/exec/small_inline.zig:1345`)
...
### `logicalInlineFrames` (`src/exec/small_inline.zig:1371`)
...
### `inlinedSnapshot` (`src/exec/small_inline.zig:1395`)
...
### `resolveCalleeLocation` (`src/exec/small_inline.zig:1408`)
...
### `sampleCtorCache` (`src/exec/small_inline.zig:1423`)
...
### `calleeMatches` (`src/exec/small_inline.zig:1436`)
...
### `realmFunctionApply` (`src/exec/small_inline.zig:1441`)
...
### `isFunctionApplyBuiltin` (`src/exec/small_inline.zig:1446`)
...
### `lookupProtoChainDataFunction` (`src/exec/small_inline.zig:1452`)
...
### `applyForwardGuardHolds` (`src/exec/small_inline.zig:1466`)
...
### `applyForwardTakeOk` (`src/exec/small_inline.zig:1481`)
...
### `applyForwardSiteAfterCall` (`src/exec/small_inline.zig:1498`)
...
### `realmApplyBuiltin` (`src/exec/small_inline.zig:1509`)
...
### `tryFusedConstructor` (`src/exec/small_inline.zig:1517`)
