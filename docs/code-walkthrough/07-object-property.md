...
### `Object.isAccessorOrAccessorPlaceholderAt` (`src/core/object.zig:7879`)
...
### `Object.defineOwnDataValueAssumingNew` (`src/core/object.zig:8219`)
...
### `Object.reserveOwnPropertyCapacityAssumingPlain` (`src/core/object.zig:8470`)
...
### `Object.appendModuleAutoInitProperty` (`src/core/object.zig:8627`)
...
### `Object.canExtendFastArray` (`src/core/object.zig:8951`)
...
### `Object.appendDenseArrayIndex` (`src/core/object.zig:8957`)

- **签名**：`pub fn appendDenseArrayIndex(self: *Object, rt: *JSRuntime, index: u32, atom_id: atom.Atom, new_value: JSValue) !bool`。
- **作用**：尝试追加 dense 自有索引值。
- **实现**：委托 appendDenseArrayIndexMode，take_ownership=false。
- **所有权 / 错误 / 调用**：true 表示已安装，false 或 error 交调用方处理；当前两种 ownership 模式在底层都按值赋值，无 RC 操作。沿用相应 mode 的原型与索引前提。

### `Object.appendDenseArrayIndexOwned` (`src/core/object.zig:8964`)

- **签名**：`pub fn appendDenseArrayIndexOwned(self: *Object, rt: *JSRuntime, index: u32, atom_id: atom.Atom, new_value: JSValue) !bool`。
- **作用**：尝试追加 dense 自有索引值。
- **实现**：委托 appendDenseArrayIndexMode，take_ownership=true。
- **所有权 / 错误 / 调用**：true 表示已安装，false 或 error 交调用方处理；当前两种 ownership 模式在底层都按值赋值，无 RC 操作。沿用相应 mode 的原型与索引前提。

### `Object.appendDenseArrayIndexMode` (`src/core/object.zig:8968`)

- **签名**：`fn appendDenseArrayIndexMode(self: *Object, rt: *JSRuntime, index: u32, atom_id: atom.Atom, new_value: JSValue, comptime take_ownership: bool) !bool`。
...
- **所有权 / 错误 / 调用**：不要求 count==length，允许填入尾部 hole 范围的 count 位置。false 表示回退；分配错误传播。take_ownership 两支当前都是 new_value，没有 RC 差异；atom 必须对应 index。

### `Object.appendFastArrayPushValues` (`src/core/object.zig:8988`)
...
### `Object.initDenseArrayIndexZeroAssumingEmpty` (`src/core/object.zig:9005`)
...
### `Object.appendDenseArrayLiteralIndex` (`src/core/object.zig:9018`)
...
### `Object.appendDenseArrayDefineIndex` (`src/core/object.zig:9025`)

- **签名**：`pub fn appendDenseArrayDefineIndex(self: *Object, rt: *JSRuntime, index: u32, atom_id: atom.Atom, new_value: JSValue) !bool`。
- **作用**：尝试追加 dense 自有索引值。
- **实现**：委托 appendDenseArrayDefineIndexMode，take_ownership=false。
- **所有权 / 错误 / 调用**：true 表示已安装，false 或 error 交调用方处理；当前两种 ownership 模式在底层都按值赋值，无 RC 操作。沿用相应 mode 的原型与索引前提。

### `Object.appendDenseArrayDefineIndexOwned` (`src/core/object.zig:9033`)

- **签名**：`pub fn appendDenseArrayDefineIndexOwned(self: *Object, rt: *JSRuntime, index: u32, atom_id: atom.Atom, new_value: JSValue) !bool`。
- **作用**：尝试追加 dense 自有索引值。
- **实现**：委托 appendDenseArrayDefineIndexMode，take_ownership=true。
- **所有权 / 错误 / 调用**：true 表示已安装，false 或 error 交调用方处理；当前两种 ownership 模式在底层都按值赋值，无 RC 操作。沿用相应 mode 的原型与索引前提。

### `Object.appendDenseArrayDefineIndexMode` (`src/core/object.zig:9037`)

- **签名**：`fn appendDenseArrayDefineIndexMode(self: *Object, rt: *JSRuntime, index: u32, atom_id: atom.Atom, new_value: JSValue, comptime take_ownership: bool) !bool`。
...
- **所有权 / 错误 / 调用**：不检查 canExtendFastArray 或 inherited setter，也没有显式 hasExoticMethods 拒绝；适用条件由上层保证。take_ownership 两分支目前相同；index 与 atom 对应关系未验证。

### `Object.initDenseArrayLiteralValuesAssumingEmpty` (`src/core/object.zig:9049`)
...
### `Object.initDenseArrayLiteralValuesOwnedTrusted` (`src/core/object.zig:9078`)
...
### `Object.barrierInitializedDenseArrayValues` (`src/core/object.zig:9095`)
...
### `Object.barrierInitializedDenseArrayValuesSlow` (`src/core/object.zig:9103`)
...
### `Object.reserveDenseArrayElements` (`src/core/object.zig:9107`)
...
### `Object.defineDenseArrayDataProperty` (`src/core/object.zig:9112`)
...
- **所有权 / 错误 / 调用**：已有 index<count 的覆盖不要求 extensible/length writable；追加不接受 count<length 的尾洞，这是与 appendDenseArrayDefineIndexMode 不同的边界。false 需上层回退。

### `Object.markIndexedProperties` (`src/core/object.zig:9145`)
...
### `Object.invalidateStandardArrayPrototypeForTaggedIndexMutation` (`src/core/object.zig:9153`)
...
### `Object.publishStandardArrayPrototype` (`src/core/object.zig:9161`)
...
### `Object.isStandardArrayPrototype` (`src/core/object.zig:9167`)
...
### `Object.canDefineDenseArrayDataPropertiesUnchecked` (`src/core/object.zig:9171`)
...
### `Object.defineDenseArrayDataPropertyUnchecked` (`src/core/object.zig:9180`)
...
### `Object.truncateArrayElements` (`src/core/object.zig:10008`)

- **签名**：`pub fn truncateArrayElements(self: *Object, _: *JSRuntime, new_len: u32) void`。
- **作用**：截短 fast Array 的有效 dense count。
- **实现**：非 Array 或非 fast 返回；取 min(new_len,count)，递减 count 到该界限。
- **所有权 / 错误 / 调用**：不清尾槽、不改逻辑 length/capacity、不 free backing 或执行 RC 释放；rt 未使用。

### `Object.convertDenseArrayElementsToSparseProperties` (`src/core/object.zig:10016`)
...
### `Object.denseArrayElement` (`src/core/object.zig:10037`)
...
### `Object.hasDenseArrayElement` (`src/core/object.zig:10045`)
...
### `Object.setDenseArrayElement` (`src/core/object.zig:10049`)
...
### `Object.ensureArrayElementCapacity` (`src/core/object.zig:10056`)
...
### `Object.updateArrayStorageMode` (`src/core/object.zig:10060`)

- **签名**：`fn updateArrayStorageMode(self: *Object, index: u32) void`。
- **作用**：把 Array 标为 sparse 模式。
- **实现**：非 Array 返回；忽略 index，直接 fast_array=false。
- **所有权 / 错误 / 调用**：不释放/迁移 backing 或重新计算密度，调用方须先保持布局与追踪合同。

### `Object.recomputeArrayStorageMode` (`src/core/object.zig:10066`)

- **签名**：`fn recomputeArrayStorageMode(self: *Object, rt: *JSRuntime) void`。
- **作用**：按 arm 容量和 live shape 索引重新设置 Array fast 标志。
- **实现**：非 Array 返回；先置 fast_array=(capacity>=count)，再扫描未 deleted 的 shape 属性，任一可解析为 array index 则 updateArrayStorageMode 清 fast。
- **所有权 / 错误 / 调用**：不迁移元素或分配存储，不验证 hole/length；零 capacity/count 也满足初始 fast 条件。

### `Object.barrierPropertySlot` (`src/core/object.zig:10083`)
...
### `Object.addProperty` (`src/core/object.zig:10096`)
...
### `Object.definePlainDataPropertyKnownFast` (`src/core/object.zig:10139`)
...
### `Object.definePlainDataPropertyKnownFastMut` (`src/core/object.zig:10164`)
...
### `Object.appendPreparedPropertyEntry` (`src/core/object.zig:10194`)
...
### `Object.appendPreparedPropertyEntryImpl` (`src/core/object.zig:10211`)
...
### `Object.appendPreparedPropertyEntryRooted` (`src/core/object.zig:10230`)
...
### `Object.appendPreparedPropertyEntryWork` (`src/core/object.zig:10260`)
...
### `Object.shapeNeedsMutationCopy` (`src/core/object.zig:10382`)
...
### `Object.ensureUniqueShapeForMutation` (`src/core/object.zig:10386`)
...
### `Object.adoptShapeForNewProperty` (`src/core/object.zig:10395`)
...
### `Object.ensurePropertyCapacity` (`src/core/object.zig:10425`)
...
### `Object.propertyStorageCapacity` (`src/core/object.zig:10460`)
...
### `Object.emptyPropertyStorageBase` (`src/core/object.zig:10464`)
...
### `Object.trailingPropertyStorageBase` (`src/core/object.zig:10476`)
...
### `Object.createPropertyStorageCell` (`src/core/object.zig:10493`)
...
### `Object.propertyStorageCellHeader` (`src/core/object.zig:10518`)
...
### `Object.propertyStoragePointerIsExternal` (`src/core/object.zig:10522`)
...
### `Object.setPropertyStorageExternal` (`src/core/object.zig:10529`)
...
### `Object.setPropertyStorageInline` (`src/core/object.zig:10535`)
...
### `Object.setPropertyStorageEmptyForDestroy` (`src/core/object.zig:10540`)
...
### `Object.restorePropertyStorage` (`src/core/object.zig:10547`)
...
### `Object.propertyStorageBase` (`src/core/object.zig:10553`)
...
### `Object.propertyEntry` (`src/core/object.zig:10558`)
...
### `Object.propertyStorageEntries` (`src/core/object.zig:10562`)
...
### `Object.trailingPropertyStorageEntries` (`src/core/object.zig:10569`)
...
### `Object.propertyEntries` (`src/core/object.zig:10580`)
...
### `Object.replaceProperty` (`src/core/object.zig:10584`)
...
### `Object.propAtomAt` (`src/core/object.zig:10629`)
...
### `Object.propFlagsAt` (`src/core/object.zig:10634`)
...
### `Object.propKindAt` (`src/core/object.zig:10649`)
...
### `Object.asDataAt` (`src/core/object.zig:10655`)
...
### `Object.replaceOwnDataPropertyValueAtAssumingShapeOwned` (`src/core/object.zig:10665`)
...
### `Object.asAccessorAt` (`src/core/object.zig:10675`)
...
### `Object.asVarRefAt` (`src/core/object.zig:10682`)
...
### `Object.isAutoInitAt` (`src/core/object.zig:10688`)
...
### `Object.replaceOwnPropertyWithVarRefCell` (`src/core/object.zig:10697`)
...
### `Object.setEntryKindAndSlot` (`src/core/object.zig:10741`)
...
### `Object.shapeProps` (`src/core/object.zig:10770`)
...
### `Object.findPropertyProbeTrusted` (`src/core/object.zig:10774`)
...
- **所有权 / 错误 / 调用**：只有索引范围断言，没有运行时步数上限或环检测；调用方保证 hash 链有效且无环。返回不单独过滤 deleted，依赖删除时维护 hash。

### `Object.findOwnPropertySlotTrusted` (`src/core/object.zig:10790`)
...
### `Object.findOwnDataValueFast` (`src/core/object.zig:10804`)
...
### `Object.findOwnDataSlotFast` (`src/core/object.zig:10835`)
...
### `Object.findWritableOwnDataSlotFast` (`src/core/object.zig:10877`)
...
### `Object.findPropertyIndexTrusted` (`src/core/object.zig:10903`)
...
### `Object.findProperty` (`src/core/object.zig:10908`)
...
### `flagsFromDescriptor` (`src/core/object.zig:11050`)
...
### `slotFromDescriptor` (`src/core/object.zig:11058`)
...
### `arrayLengthValue` (`src/core/object.zig:11275`)
...
### `arrayLengthFromValue` (`src/core/object.zig:11282`)
...
### `arrayLengthNumber` (`src/core/object.zig:11291`)
...
### `arrayLengthStringNumber` (`src/core/object.zig:11313`)

- **签名**：`fn arrayLengthStringNumber(rt: *JSRuntime, value: JSValue) !f64`。
- **作用**：按内部 ASCII 路径解析长度字符串数值。
- **实现**：取 String body、ensureFlat；分配临时字节表，任一 code unit>127 返回 NaN；只 trim 空格/tab/CR/LF，空为 0，处理 ±Infinity，0x/0X 用 u64 十六进制解析，其余 parseFloat；解析失败 NaN。
- **所有权 / 错误 / 调用**：不等于完整 ECMAScript StringNumericLiteral，Unicode 空白被拒绝，十六进制限制 u64。临时数组 defer 释放，分配/flatten 错误传播。

### `varRefCellFromValue` (`src/core/object.zig:11336`)
...
### `appendAtom` (`src/core/object.zig:11340`)
...
### `hasPropertyIndexKeys` (`src/core/object.zig:11355`)
...
### `indexKeyLessThan` (`src/core/object.zig:11364`)
...
### `ownEntriesExpectObject` (`src/core/object.zig:11383`)
...
### `entriesAtomToStringValue` (`src/core/object.zig:11389`)
...
### `entryArrayValue` (`src/core/object.zig:11393`)

- **签名**：`fn entryArrayValue(rt: *JSRuntime, key: atom.Atom, value: JSValue, prototype: ?*Object) !JSValue`。
- **作用**：构造 [key字符串,value] 两项数组。
- **实现**：对输入 value 激活 rootValues 帧；createArray 后设置失败 destroy，atom 转字符串，再创建容量 2 的 array_storage cell，写两项并 adopt，标 indexed properties 后返回。
- **所有权 / 错误 / 调用**：原型直接取参数，不自行解析 realm。此函数显式根帧仅列 value，源码所谓 key_value 的 local defer/root 并未在本体出现；key、prototype、临时对象的存活须遵循外围协议，不能宣称全部显式保护。

### `ownEntriesArray` (`src/core/object.zig:11414`)
...
### `stringIteratorPrimitiveValue` (`src/core/object.zig:11466`)
...
### `defineStringIteratorToStringTag` (`src/core/object.zig:11475`)
...
### `stringIteratorPrototype` (`src/core/object.zig:11481`)
...
### `stringIterator` (`src/core/object.zig:11500`)
...
- 清单函数数（本文件分到）: 98（`src/core/object.zig` 全文件 879）
- 本文标题覆盖: 98
- 未覆盖: 无
