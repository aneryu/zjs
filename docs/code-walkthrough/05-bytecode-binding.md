...
### `binding_rules.decodeScopeOperand` (`src/bytecode.zig:6655`)
...
### `binding_rules.markEvalCapturedVariables` (`src/bytecode.zig:6675`)
...
### `binding_rules.encodeEvalScopeHead` (`src/bytecode.zig:6694`)
...
### `binding_rules.validateFunctionDefParentChain` (`src/bytecode.zig:6706`)
...
### `binding_rules.JSContext.init` (`src/bytecode.zig:6737`)

- **签名**：`pub fn init(function: *bytecode_function.Bytecode) JSContext`。
- **作用**：只绑 Bytecode，无 FunctionDef。
- **实现**：memory/atoms 来自 function。
- **所有权 / 错误 / 调用**：只借 `function`（`memory` / `atoms` 都取自它），不分配、无 error set；`function_def` 留空、`scope_link_proof` 为 `.none`，因此只能做不依赖局部槽的解析。全树无调用方——实际入口都走 `initWithFunctionDef`。


### `binding_rules.JSContext.initWithFunctionDef` (`src/bytecode.zig:6745`)
...
### `binding_rules.JSContext.proveScopeLinksForResolution` (`src/bytecode.zig:6762`)
...
### `binding_rules.JSContext.proveParentScopeLinksForResolution` (`src/bytecode.zig:6769`)
...
### `binding_rules.fclosureEncodingSize` (`src/bytecode.zig:6783`)
...
### `binding_rules.emitFClosure` (`src/bytecode.zig:6788`)
...
### `binding_rules.lowerScopeVarOpGlobal` (`src/bytecode.zig:6809`)
...
### `binding_rules.lowerScopeVarOpLocal` (`src/bytecode.zig:6825`)
...
### `binding_rules.selectShortLoc` (`src/bytecode.zig:6852`)
...
### `binding_rules.shortOpcodesEnabled` (`src/bytecode.zig:6878`)
...
### `binding_rules.selectLocForm` (`src/bytecode.zig:6883`)
...
### `binding_rules.closureVarIsRuntimeVarRef` (`src/bytecode.zig:6888`)
...
### `binding_rules.closureVarSourceIsDynamicGlobal` (`src/bytecode.zig:6899`)
...
### `binding_rules.lookupClosureVar` (`src/bytecode.zig:6919`)
...
### `binding_rules.lookupGlobalClosureVar` (`src/bytecode.zig:6935`)
...
### `binding_rules.addOrFindClosureSource` (`src/bytecode.zig:6947`)
...
### `binding_rules.threadClosureSource` (`src/bytecode.zig:6974`)
...
### `binding_rules.ensureGlobalClosureVar` (`src/bytecode.zig:7000`)
...
### `binding_rules.emitGlobalVarOp` (`src/bytecode.zig:7041`)
...
### `binding_rules.lookupTopLevelModuleLexicalClosureVar` (`src/bytecode.zig:7049`)
...
### `binding_rules.preferTopLevelModuleClassBinding` (`src/bytecode.zig:7058`)
...
### `binding_rules.closureVarKind` (`src/bytecode.zig:7069`)
...
### `binding_rules.closureVarWriteThrowsReadOnly` (`src/bytecode.zig:7089`)
...
### `binding_rules.closureVarConstWriteThrows` (`src/bytecode.zig:7095`)
...
### `binding_rules.writeThrowVarReadOnly` (`src/bytecode.zig:7122`)
...
### `binding_rules.writeThrowVarError` (`src/bytecode.zig:7126`)
...
### `binding_rules.writeThrowVarRedeclaration` (`src/bytecode.zig:7143`)
...
### `binding_rules.lowerScopeVarOpForClosure` (`src/bytecode.zig:7152`)
...
### `binding_rules.resolvePrivateField` (`src/bytecode.zig:7234`)
...
### `binding_rules.isPrivateVarKind` (`src/bytecode.zig:7260`)
...
### `binding_rules.isPrivateSetterCompanionName` (`src/bytecode.zig:7272`)
...
### `binding_rules.resolvePrivateSetter` (`src/bytecode.zig:7281`)
...
### `binding_rules.privateAccessorSize` (`src/bytecode.zig:7306`)
...
### `binding_rules.writePrivateAccessor` (`src/bytecode.zig:7310`)
...
### `binding_rules.loweredPrivateFieldSize` (`src/bytecode.zig:7334`)
...
### `binding_rules.loweredPrivateFieldAtomCount` (`src/bytecode.zig:7358`)
...
### `binding_rules.writePrivateCallMethodZero` (`src/bytecode.zig:7367`)
...
### `binding_rules.writePrivateCallMethod` (`src/bytecode.zig:7373`)
...
### `binding_rules.writeLoweredPrivateField` (`src/bytecode.zig:7380`)
...
### `binding_rules.varNeedsTdzRearm` (`src/bytecode.zig:7461`)
...
### `binding_rules.varNeedsScopeFunctionInit` (`src/bytecode.zig:7465`)
...
### `binding_rules.enterScopeRefreshSize` (`src/bytecode.zig:7474`)
...
### `binding_rules.writeEnterScopeRefresh` (`src/bytecode.zig:7496`)
...
### `binding_rules.leaveScopeCloseSize` (`src/bytecode.zig:7522`)
...
### `binding_rules.writeLeaveScopeClose` (`src/bytecode.zig:7536`)
...
### `binding_rules.lowerScopeVarOpClosure` (`src/bytecode.zig:7553`)
...
### `binding_rules.selectShortVarRef` (`src/bytecode.zig:7565`)
...
### `binding_rules.selectVarRefForm` (`src/bytecode.zig:7582`)
...
### `binding_rules.selectShortArg` (`src/bytecode.zig:7587`)
...
### `binding_rules.selectArgForm` (`src/bytecode.zig:7603`)
...
### `binding_rules.lookupArg` (`src/bytecode.zig:7608`)
...
### `binding_rules.lookupCurrentFunctionName` (`src/bytecode.zig:7620`)
...
### `binding_rules.lookupCurrentPseudoBinding` (`src/bytecode.zig:7630`)
...
### `binding_rules.lowerScopeVarOpArg` (`src/bytecode.zig:7649`)
...
### `binding_rules.resolveScopeVarLookupImpl` (`src/bytecode.zig:7670`)
...
### `binding_rules.resolveScopeVarImpl` (`src/bytecode.zig:7704`)
...
### `binding_rules.resolveScopeVar` (`src/bytecode.zig:7718`)
...
### `binding_rules.resolveLocalOrArgImpl` (`src/bytecode.zig:7738`)
...
### `binding_rules.resolveLocalOrArg` (`src/bytecode.zig:7767`)
...
### `binding_rules.isEvalVarObjectAtom` (`src/bytecode.zig:7778`)
...
### `binding_rules.isDynamicEnvObjectAtom` (`src/bytecode.zig:7782`)
...
### `binding_rules.closureVarRangeHasDynamicEnvObjects` (`src/bytecode.zig:7786`)
...
### `binding_rules.functionHasDynamicEnvObjects` (`src/bytecode.zig:7799`)
...
### `binding_rules.scopeUsesArgumentEnvironmentOnly` (`src/bytecode.zig:7808`)
...
### `binding_rules.ClosureDynamicEnvProbeIterator.init` (`src/bytecode.zig:7826`)
...
### `binding_rules.ClosureDynamicEnvProbeIterator.next` (`src/bytecode.zig:7853`)
...
### `binding_rules.LocalWithProbeIterator.init` (`src/bytecode.zig:7871`)
...
### `binding_rules.LocalWithProbeIterator.next` (`src/bytecode.zig:7883`)
...
### `binding_rules.staticBindingStopsDynamicEnvProbes` (`src/bytecode.zig:7902`)
...
- **所有权 / 错误 / 调用**：只读，不分配、无 error set。虽然进了 `surface`（`bytecode.zig:9546`），`resolve_variables.zig` 当前一次都没用；实际唯一调用方是 namespace 内的 `bytecode.zig:8065`（`evalVarObjectProbePlan`）。


### `binding_rules.scopeVarDynamicProbeEligible` (`src/bytecode.zig:7929`)
...
### `binding_rules.resolvedBindingStopsDynamicEnvProbes` (`src/bytecode.zig:7938`)
...
### `binding_rules.closureDynamicEnvProbeIteratorInitResolved` (`src/bytecode.zig:7973`)
...
### `binding_rules.scopeContainsBinding` (`src/bytecode.zig:7994`)
...
### `binding_rules.EvalVarObjectProbeKind.matches` (`src/bytecode.zig:8016`)
...
### `binding_rules.EvalVarObjectProbeKind.wireKind` (`src/bytecode.zig:8026`)
...
### `binding_rules.evalVarObjectProbePlan` (`src/bytecode.zig:8042`)
...
### `binding_rules.ScopeVarAction.size` (`src/bytecode.zig:8091`)
...
### `binding_rules.ScopeVarAction.atomCount` (`src/bytecode.zig:8095`)
...
### `binding_rules.ScopeVarAction.form` (`src/bytecode.zig:8099`)
...
### `binding_rules.ScopeVarAction.throwReadonly` (`src/bytecode.zig:8103`)
...
### `binding_rules.ScopeVarAction.dropAction` (`src/bytecode.zig:8111`)
...
### `binding_rules.ScopeVarProbePlan.from` (`src/bytecode.zig:8124`)

- **签名**：`fn from(plan: ?EvalVarObjectProbePlan) Error!ScopeVarProbePlan`。
- **作用**：把 `usize` 计数的 `EvalVarObjectProbePlan` 收窄成 `u32` 的存档形式，顺带把「没有探针」这一情形规约成全零计划。
- **实现**：`plan` 为 null 时返回默认值 `.{}`（count/prefix_size 均为 0）。否则先检查 `count` 与 `prefix_size` 是否超过 `maxInt(u32)`，超了返回 `error.BytecodeOverflow`，否则 `@intCast` 两个字段写进 `ScopeVarProbePlan`。
- **所有权 / 错误 / 调用**：错误：`error.BytecodeOverflow`，沿编译流水线上抛后由 parser/finalize 变成编译失败。唯一调用方是同文件的 `planScopeVarLowering`（`src/bytecode.zig:8294`），它把探针计划与 `ScopeVarAction` 一起装进降级方案供审计对账。


### `binding_rules.scopeVarProbeKind` (`src/bytecode.zig:8146`)
...
### `binding_rules.globalScopeVarAction` (`src/bytecode.zig:8156`)
...
### `binding_rules.closureScopeVarAction` (`src/bytecode.zig:8168`)
...
### `binding_rules.planScopeVarAction` (`src/bytecode.zig:8187`)

- **签名**：`noinline fn planScopeVarAction( ctx: *const JSContext, atom_id: atom.Atom, scope_level: i32, op_id: u8, ) Error!ScopeVarAction`。
- **作用**：完整绑定分类器：从 scope_level 选出 global / 模块词法闭包 / 局部或参数 / 已有闭包 / 再回退全局，并投影成 `ScopeVarAction`。
- **实现**：`scope_level < 0` → `globalScopeVarAction`。否则 `lookupTopLevelModuleLexicalClosureVar` 命中走 `closureScopeVarAction`。`resolveLocalOrArg`：arg 用 `selectArgForm`；local 若 sloppy-eval 非词法则改走全局，模块 class 绑定改走闭包；`scope_put_var` 只读抛 `throwReadonly`、函数名绑定 `dropAction`；词法 TDZ 用 `lowerScopeVarOpLexical`；否则 `selectLocForm`。都没有则 `lookupClosureVar`，再否则全局。outlined 是为了不把这份分类器塞进调用方热走（调用方还握着字节码 walk 与事务分配）。
- **所有权 / 错误 / 调用**：只读规划，不分配（`ctx` 是 `*const`，不会新建闭包行）；错误只有 `globalScopeVarAction` 传上来的 `error.ClosureVarNotFound`。私有，唯一调用方 `bytecode.zig:8290`（`planScopeVarLowering` 的静态动作半）。


### `binding_rules.planResolvedScopeVarAction` (`src/bytecode.zig:8239`)
...
### `binding_rules.planScopeVarLowering` (`src/bytecode.zig:8282`)

- **签名**：`fn planScopeVarLowering( ctx: *const JSContext, atom_id: atom.Atom, scope_operand: ScopeOperand, op_id: u8, has_dynamic_env_objects: bool, ) Error!ScopeVarLoweringPlan`。
- **作用**：把一条 scope_* 指令收成探针计划 + 静态动作。
- **实现**：动作半调 `planScopeVarAction`（按名字重新分类，**不是** `planResolvedScopeVarAction`）；`scope_operand` 由调用方解码后传入，本函数不再解码。只有 `has_dynamic_env_objects` 为真时才算探针半：`scopeVarProbeKind` 拿不到 kind 就原样返回，否则 `ScopeVarProbePlan.from(evalVarObjectProbePlan(...))`。
- **所有权 / 错误 / 调用**：不分配，返回按值的 `ScopeVarLoweringPlan`；错误 = `planScopeVarAction` 的 `ClosureVarNotFound` 与 `ScopeVarProbePlan.from` 的 `BytecodeOverflow`。经 `surface.planScopeVarLowering`（`bytecode.zig:9528`）由 `compiler/resolve_variables.zig:1651` 调用——那是 `audit_oracles` 下的预言机对账点。


### `binding_rules.writeScopeVarAction` (`src/bytecode.zig:8305`)
...
### `binding_rules.evalVarObjectProbeAccessorSize` (`src/bytecode.zig:8326`)
...
### `binding_rules.evalVarObjectProbeIsWith` (`src/bytecode.zig:8333`)
...
### `binding_rules.evalVarObjectClosureProbe` (`src/bytecode.zig:8340`)
...
### `binding_rules.loweredScopeDeleteVarSize` (`src/bytecode.zig:8347`)
...
### `binding_rules.loweredScopeGetRefSize` (`src/bytecode.zig:8357`)
...
### `binding_rules.loweredScopeMakeRefSize` (`src/bytecode.zig:8373`)
...
### `binding_rules.loweredScopeMakeRefAtomCount` (`src/bytecode.zig:8395`)
...
### `binding_rules.writeEvalVarObjectProbeAccessor` (`src/bytecode.zig:8415`)
...
### `binding_rules.writeLoweredScopeDeleteVar` (`src/bytecode.zig:8443`)
...
### `binding_rules.writeLoweredScopeGetRef` (`src/bytecode.zig:8479`)
...
### `binding_rules.writeFunctionNameDummyRef` (`src/bytecode.zig:8536`)
...
### `binding_rules.writeLoweredScopeMakeRef` (`src/bytecode.zig:8563`)
...
### `binding_rules.isLexicalLocal` (`src/bytecode.zig:8645`)
...
### `binding_rules.isEvalNonLexicalLocal` (`src/bytecode.zig:8651`)
...
### `binding_rules.bytecodeFunctionIsEval` (`src/bytecode.zig:8675`)
...
### `binding_rules.localIsFunctionName` (`src/bytecode.zig:8685`)
...
### `binding_rules.localWriteThrowsReadOnly` (`src/bytecode.zig:8694`)
...
### `binding_rules.lowerScopeVarOpLexical` (`src/bytecode.zig:8711`)
...
### `binding_rules.localLexicalAccessNeedsCheck` (`src/bytecode.zig:8726`)
...
### `binding_rules.writeSelectedLocForm` (`src/bytecode.zig:8731`)
...
### `binding_rules.markReferenceTakenBinding` (`src/bytecode.zig:8742`)
...
### `binding_rules.functionIsStrict` (`src/bytecode.zig:8759`)
...
### `binding_rules.functionDeclaresGlobalVar` (`src/bytecode.zig:8764`)
...
### `binding_rules.canOptimizeGlobalRefPutTail` (`src/bytecode.zig:8772`)
...
### `binding_rules.closureVarIsGlobalFamily` (`src/bytecode.zig:8776`)
...
### `binding_rules.resolveEvalGlobalVarTargets` (`src/bytecode.zig:8786`)
...
### `binding_rules.hasDirectEvalLexicalRedeclaration` (`src/bytecode.zig:8817`)
...
### `binding_rules.isPseudoBindingAtom` (`src/bytecode.zig:8838`)
...
### `binding_rules.threadParentLocalSource` (`src/bytecode.zig:8845`)
...
### `binding_rules.threadParentArgSource` (`src/bytecode.zig:8863`)
...
### `binding_rules.discoverParentScopedSource` (`src/bytecode.zig:8889`)
...
### `binding_rules.threadParentEvalObject` (`src/bytecode.zig:8918`)

- **签名**：`fn threadParentEvalObject( target: *function_def_mod.FunctionDef, parent: *function_def_mod.FunctionDef, local_idx_i32: i32, ) Error!u16`。
- **作用**：穿父 eval 变量对象。
- **实现**：local 下标。 错误臂：`error.InvalidBytecode`。
- **所有权 / 错误 / 调用**：范围检查后转交 `threadParentLocalSource`，因此同样会 capture 并新建闭包行；越界为 `error.InvalidBytecode`。私有，调用方 `bytecode.zig:9114`、`:9117`（`resolveBindingTopologyAfterCurrentMiss`，分别处理 `var_object_idx` 与 `arg_var_object_idx`）。


### `binding_rules.ensureParentArgumentsBinding` (`src/bytecode.zig:8927`)
...
### `binding_rules.ensureCurrentPseudoBinding` (`src/bytecode.zig:8935`)
...
### `binding_rules.findResolvedClosureBinding` (`src/bytecode.zig:8958`)
...
### `binding_rules.resolveBindingTopologyAfterCurrentMiss` (`src/bytecode.zig:8983`)
...
### `binding_rules.resolveBindingTopologyResultImpl` (`src/bytecode.zig:9156`)
...
- **所有权 / 错误 / 调用**：自身只做分派，但下游 `resolveBindingTopologyAfterCurrentMiss` 会 capture 并新建闭包行（见 `threadParentLocalSource`），所以这是有副作用的调用。错误集是完整的 `Error`（`OutOfMemory`/`InvalidBytecode`/`BytecodeOverflow`/`NoFunctionDef`/`NoParentScope`/`ClosureVarNotFound`）。私有，四个实例化站点 `bytecode.zig:9183`（`resolveBindingTopologyResult`，trust=false）、`:9195`（`resolveScopeVarBindingTopologyImpl`）、`:9321`（`resolveScopeVarPlanImpl`，`@call(.always_inline)`）、`:9371`（`resolveBindingTopologyV2`，trust=true）。


### `binding_rules.resolveBindingTopologyResult` (`src/bytecode.zig:9178`)
...
### `binding_rules.resolveScopeVarBindingTopologyImpl` (`src/bytecode.zig:9189`)
...
### `binding_rules.resolveScopeVarBindingTopology` (`src/bytecode.zig:9221`)

- **签名**：`fn resolveScopeVarBindingTopology( ctx: *JSContext, atom_id: atom.Atom, scope_level: i32, ) Error!ScopeVarBinding`。
- **作用**：trust=false。
- **实现**：Error。 返回 `resolveScopeVarBindingTopologyImpl(false, ctx, atom_id, scope_level)`。
- **所有权 / 错误 / 调用**：trust=false 的兼容包装，副作用/错误同 `resolveScopeVarBindingTopologyImpl`。**树内无调用方**：`surface.resolveScopeVarBindingTopology`（`bytecode.zig:9524`）指向的是 V2 变体，`resolve_variables.zig:1711`/`:1778` 拿到的是 V2 而不是这个函数。


### `binding_rules.ResolvedScopeVarPlan.init` (`src/bytecode.zig:9260`)
...
### `binding_rules.ResolvedScopeVarPlan.withBinding` (`src/bytecode.zig:9280`)
...
### `binding_rules.ResolvedScopeVarPlan.binding` (`src/bytecode.zig:9291`)
...
### `binding_rules.ResolvedScopeVarPlan.action` (`src/bytecode.zig:9300`)
...
### `binding_rules.resolveScopeVarPlanImpl` (`src/bytecode.zig:9312`)
...
### `binding_rules.resolveScopeVarPlan` (`src/bytecode.zig:9348`)

- **签名**：`noinline fn resolveScopeVarPlan( ctx: *JSContext, atom_id: atom.Atom, scope_level: i32, op_id: u8, ) Error!ResolvedScopeVarPlan`。
- **作用**：非 V2 路径：拓扑 + 动作一次返回（`trust_final_scope_links=false`）。
- **实现**：直接 `resolveScopeVarPlanImpl(false, …)`，即不信任 `scope_link_proof` 的那一份特化；与 `inline` 的 `resolveScopeVarPlanV2` 成对。
- **所有权 / 错误 / 调用**：副作用与错误集同 impl。**树内无调用方**：`surface.resolveScopeVarPlan`（`bytecode.zig:9525`）导出的是 V2 变体，`compiler/resolve_variables.zig:1643` 拿到的也是 V2；本函数只是保留的非信任入口。


### `binding_rules.resolveScopeVarPlanV2` (`src/bytecode.zig:9357`)
...
### `binding_rules.resolveBindingTopology` (`src/bytecode.zig:9366`)
...
### `binding_rules.resolveBindingTopologyV2` (`src/bytecode.zig:9370`)

- **签名**：`inline fn resolveBindingTopologyV2(ctx: *JSContext, atom_id: atom.Atom, scope_level: i32) Error!void`。
- **作用**：V2 拓扑。
- **实现**：inline。 内部 `try` 传播错误。
- **所有权 / 错误 / 调用**：trust=true 的 `inline` 包装，副作用/错误同 `resolveBindingTopologyResultImpl`。**树内无调用方**：它只被 `surface.resolveBindingTopology`（`bytecode.zig:9523`）命名，而 `compiler/resolve_variables.zig` 一次也没调用该导出。


### `binding_rules.resolveScopeVarBindingTopologyV2` (`src/bytecode.zig:9374`)
...
### `binding_rules.privateBindingOwner` (`src/bytecode.zig:9387`)
...
### `binding_rules.findPrivateSetterOwnerBinding` (`src/bytecode.zig:9415`)
...
### `binding_rules.resolvePrivateBindingTopology` (`src/bytecode.zig:9429`)
...
### `binding_rules.makeRefBindingIsGlobal` (`src/bytecode.zig:9457`)

- **签名**：`fn makeRefBindingIsGlobal(ctx: *const JSContext, atom_id: atom.Atom, scope_level: i32) bool`。
- **作用**：make-ref 名是否真全局（不创建闭包行）。仅用于 qjs 删掉 make-ref 本身的尾折叠。
- **实现**：无 fd 保守当全局（true）。当前函数只要命中顶层模块词法行、局部/参数、已有闭包行、具名函数表达式自名或隐式 `arguments`，就 false。随后逐层父函数重复同样的判定：可见 scope 链上同名槽 → false；链未止于 `arg_scope_end` 时还查 `findArg` 与 scope-0 槽；父的伪绑定/`arguments`/具名自名 → false；父闭包表里同名行按 closureType 判——global 族 true，其余 false。走到没有父才返回 true。
- **所有权 / 错误 / 调用**：只读判定，明确**不新建 closure 行**、不分配、无 error set。**树内无调用方**：虽然进了 `surface`（`bytecode.zig:9560`），`compiler/resolve_variables.zig` 一次也没调用它。


## 覆盖核对

- 清单函数数（本文件分组）: 147（`src/bytecode.zig` 全文件 538）
- 本文标题覆盖: 147
- 未覆盖: 无
