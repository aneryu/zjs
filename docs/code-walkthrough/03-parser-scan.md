...
### `ParseState.advance` (`src/parser.zig:2028`)
...
### `ParseState.setPendingDiagnostic` (`src/parser.zig:2059`)
...
### `ParseState.currentDiagnosticPosition` (`src/parser.zig:2070`)
...
### `ParseState.recordFailureHere` (`src/parser.zig:2078`)
...
### `ParseState.failHere` (`src/parser.zig:2085`)
...
### `ParseState.propagateFailureHere` (`src/parser.zig:2090`)
...
### `ParseState.tokenKindLabel` (`src/parser.zig:2098`)
...
### `ParseState.currentTokenKindLabel` (`src/parser.zig:2126`)
...
### `ParseState.failExpectedToken` (`src/parser.zig:2136`)
...
### `ParseState.formatExpectedGot` (`src/parser.zig:2142`)
...
### `ParseState.failExpectedDescription` (`src/parser.zig:2154`)
...
### `ParseState.failExpectedDescriptionAt` (`src/parser.zig:2163`)
...
### `ParseState.failUnexpectedToken` (`src/parser.zig:2176`)
...
### `ParseState.failWithMessage` (`src/parser.zig:2189`)
...
### `ParseState.failUndefinedLabel` (`src/parser.zig:2194`)
...
### `ParseState.peekKind` (`src/parser.zig:2205`)
...
### `ParseState.currentTokenStartOffset` (`src/parser.zig:2209`)
...
### `ParseState.currentFunctionSourceStart` (`src/parser.zig:2216`)
...
### `ParseState.currentTokenEndOffset` (`src/parser.zig:2224`)
...
### `ParseState.captureFunctionSource` (`src/parser.zig:2228`)
...
### `ParseState.setFunctionSourceRange` (`src/parser.zig:2232`)
...
### `ParseState.setChildFunctionSourceByCpoolIndex` (`src/parser.zig:2242`)
...
### `ParseState.isPunct` (`src/parser.zig:2255`)
...
### `ParseState.gotLineTerminator` (`src/parser.zig:2260`)
...
### `ParseState.expectSemicolon` (`src/parser.zig:2446`)
...
### `expectToken` (`src/parser.zig:2460`)
...
### `peekNextKind` (`src/parser.zig:2467`)
...
### `peekNextIsOfToken` (`src/parser.zig:2476`)
...
### `peekNextKindNoLineTerminator` (`src/parser.zig:2488`)
...
### `peekNextKindWithLineTerminator` (`src/parser.zig:2498`)
...
### `forHeadHasNoTopLevelSemicolon` (`src/parser.zig:2513`)
...
### `advanceLocal.call` (`src/parser.zig:2525`)
...
### `rhsContainsDirectEval` (`src/parser.zig:2588`)
...
### `advanceLocal.call` (`src/parser.zig:2601`)
...
### `templateContainsDirectEval` (`src/parser.zig:2692`)
...
### `isAsyncIdentifier` (`src/parser.zig:2773`)
...
### `canTreatLetAsForInitializerExpression` (`src/parser.zig:2797`)
...
### `checkIdentArrowHead` (`src/parser.zig:3827`)
...
### `checkAsyncArrowHeadAfterAsync` (`src/parser.zig:3841`)
...
### `isAsyncArrowBindingIdentifierKind` (`src/parser.zig:3865`)
...
### `checkArrowHead` (`src/parser.zig:3880`)
...
### `lookaheadErrorAsNoMatch` (`src/parser.zig:3890`)
...
### `diagnosticTokenFromToken` (`src/parser.zig:3902`)
...
### `mapLookaheadLexerError` (`src/parser.zig:3918`)
...
### `peekNextDiagnosticToken` (`src/parser.zig:3929`)
...
### `nextRegexpAwareLookaheadKind` (`src/parser.zig:3943`)
...
### `rescanLookaheadTokenIfRegexp` (`src/parser.zig:3951`)
...
### `appendRetainedAtom` (`src/parser.zig:4628`)

- **签名**：`fn appendRetainedAtom(list: *std.ArrayList(Atom), allocator: std.mem.Allocator, _: *atom_module.AtomTable, atom_id: Atom) Error!void`。
- **作用**：把一个 atom 追加进解析期的 atom 列表。
- **实现**：名字里的 retain 是 rc 时代的遗留：函数体只有 `const retained = atom_id;` 加一次 `list.append(allocator, atom_id)`，`*atom_module.AtomTable` 形参被 `_` 忽略。tracing GC 之后 atom 不再计数，id 的存活由整场编译的 `CompileAtomScope` 根列表保证，这里既不需要也无从 retain。
- **所有权 / 错误 / 调用**：不额外分配；列表内存归调用方传入的 `allocator`。追加进去的 id 只是借用，编译期间由 `CompileAtomScope` 作为 GC 根钉住（每个 intern/dup 入口都会 `noteCompileScope`），无人需要释放。唯一的失败是 `error.OutOfMemory`。
### `tokenStartsPrimaryExpression` (`src/parser.zig:4633`)
...
### `tokenStartsYieldExpressionOperand` (`src/parser.zig:4654`)
...
### `tokenCanStartSlashRegexp` (`src/parser.zig:4658`)
...
### `escapedIdentifierIsReservedWordForShorthandBinding` (`src/parser.zig:7015`)
...
### `escapedIdentifierIsReservedWordForCurrentContext` (`src/parser.zig:7032`)
...
### `isInvalidStrictFunctionBindingName` (`src/parser.zig:7036`)
...
### `recordInvalidStrictParameterName` (`src/parser.zig:7041`)
...
### `rejectInvalidStrictParameterName` (`src/parser.zig:7047`)
...
### `canUseAwaitAsIdentifier` (`src/parser.zig:7052`)
...
### `isIdentifierLikeToken` (`src/parser.zig:7056`)
...
### `isSloppyFutureReservedBindingToken` (`src/parser.zig:7065`)
...
### `isSloppyFutureReservedToken` (`src/parser.zig:7069`)
...
### `tokenCanStartExpression` (`src/parser.zig:7082`)
...
### `identifierLikeAtom` (`src/parser.zig:7099`)
...
- **实现**：一个三元：`TOK_IDENT` 取 `token.payload.ident.atom`，否则按关键字 kind 查 `tok.keywordAtom(kind)`。返回的是借用的 id（源码处标了 `borrowed-atom`）：TGC S3-c 之后 token 不再持 atom 的引用计数，`advance()` 只换掉 token payload，这个 id 由整场编译的 `CompileAtomScope` 根列表钉住，跨 `advance()` 仍然有效；同一行注释里「valid only until advance(); retain via identifierLikeAtomOwned」是 rc 时代的遗留，`identifierLikeAtomOwned` 今天只是本函数的同义转发。
...
### `identifierLikeAtomOwned` (`src/parser.zig:7105`)
...
### `identifierLikeHasInvalidEscapeForBinding` (`src/parser.zig:7109`)
...
### `expectPunct` (`src/parser.zig:7312`)
...
### `numberIsExactI32` (`src/parser.zig:7317`)
...
### `skipFunctionInPredeclareScan` (`src/parser.zig:8149`)
...
### `skipTemplateInPredeclareScan` (`src/parser.zig:8180`)
...
### `skipRegexpInPredeclareScan` (`src/parser.zig:8235`)
...
### `predeclareSlashStartsRegexp` (`src/parser.zig:8245`)
...
### `usingDeclarationStart` (`src/parser.zig:8324`)
...
### `awaitUsingDeclarationStart` (`src/parser.zig:8333`)
...
### `directUsingDeclarationKind` (`src/parser.zig:8353`)
...
### `tokenKindCanStartUsingBinding` (`src/parser.zig:8359`)
...
### `advanceUsingDeclarationPrefixForLookahead` (`src/parser.zig:8369`)
...
### `usingDeclarationBindingIsOf` (`src/parser.zig:8385`)
...
### `takeParserSnapshot` (`src/parser.zig:13199`)
...
### `restoreParserLexerSnapshot` (`src/parser.zig:13227`)
...
### `takeLexerCursorSnapshot` (`src/parser.zig:13256`)
...
### `restoreLexerCursorSnapshot` (`src/parser.zig:13268`)
...
### `scanBalancedAfterOpening` (`src/parser.zig:13291`)
...
### `scanBalancedToken` (`src/parser.zig:13384`)
...
- 清单函数数（本文件分到）: 84（`src/parser.zig` 全文件 649）
- 本文标题覆盖: 84
- 未覆盖: 无
