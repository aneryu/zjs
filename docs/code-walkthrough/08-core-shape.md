...
Shape 生命周期由 tracer 管理；源码开头的 Refcounted 标题已不代表当前机制。Shape 固定头64字节、对齐8，GC metadata 在头前；属性元数据占满 prop_size 容量，其后才是桶，props() 与 hashBuckets() 都借出可变 slice。
...
### `propertyCapacityForNeeded` (`src/core/shape.zig:30`)
...
### `famRegionBytes` (`src/core/shape.zig:66`)
...
### `ShapeTraceListState.init` (`src/core/shape.zig:93`)
...
### `ShapeTraceListState.previous` (`src/core/shape.zig:97`)
...
### `ShapeTraceListState.setPrevious` (`src/core/shape.zig:103`)
...
### `ShapeTraceListState.isHashed` (`src/core/shape.zig:109`)
...
### `ShapeTraceListState.setHashed` (`src/core/shape.zig:113`)
...
### `initialOwnership` (`src/core/shape.zig:126`)

- **签名**：`inline fn initialOwnership(is_hashed: bool) ShapeOwnership`。
- **作用**：初始化未共享标志。
- **实现**：忽略is_hashed参数，返回shared=0。
- **所有权 / 错误 / 调用**：hash表成员与shared是不同事实，不把hashed直接当作共享。
...
### `Shape.sameTransition` (`src/core/shape.zig:299`)

- **签名**：`pub fn sameTransition(self: *const Shape, other: *const Shape) bool`。
- **作用**：比较两个Shape的逻辑属性布局与原型。
- **实现**：proto或prop_count不同即false；逐个有效Property比较atom_id和flags，全部相同返回true。
- **所有权 / 错误 / 调用**：包含deleted槽的原始atom/flags，不比较hash_next、容量、hash值、identity或shared；依赖有效props布局。

### `Shape.hasPropertyHash` (`src/core/shape.zig:307`)
...
### `Shape.firstPropertyIndex` (`src/core/shape.zig:311`)
...
### `Shape.firstPropertyIndexAssumeHash` (`src/core/shape.zig:316`)
...
### `Shape.traceChildEdgesFallible` (`src/core/shape.zig:322`)
...
### `Shape.traceChildEdgesFallible.callVisitObject` (`src/core/shape.zig:325`)
...
### `Shape.traceChildEdgesNoFail` (`src/core/shape.zig:349`)
...
### `Registry.adoptionBarrier` (`src/core/shape.zig:379`)
...
### `Registry.shadeAdoptedShape` (`src/core/shape.zig:388`)
...
### `Registry.Visitor.visitObject` (`src/core/shape.zig:399`)
...
### `Registry.Visitor.visitAtom` (`src/core/shape.zig:404`)
...
### `Registry.init` (`src/core/shape.zig:417`)
...
### `Registry.deinit` (`src/core/shape.zig:421`)
...
### `Registry.create` (`src/core/shape.zig:433`)
...
### `Registry.publish` (`src/core/shape.zig:439`)
...
### `Registry.createObjectRoot` (`src/core/shape.zig:444`)
...
### `Registry.createObjectRootReserved` (`src/core/shape.zig:450`)
...
### `Registry.createObjectRootMaybeReserved` (`src/core/shape.zig:456`)
...
### `Registry.createObjectRootWithPropertyCapacity` (`src/core/shape.zig:473`)
...
### `Registry.createObjectRootWithPropertyCapacityReserved` (`src/core/shape.zig:490`)
...
### `Registry.createInitialShape` (`src/core/shape.zig:509`)
...
### `Registry.createShape` (`src/core/shape.zig:519`)
...
### `Registry.createShapeReserved` (`src/core/shape.zig:545`)
...
### `Registry.createShapeWithPropertyCapacity` (`src/core/shape.zig:562`)
...
### `Registry.createShapeWithPropertyCapacityReserved` (`src/core/shape.zig:588`)
...
### `Registry.tryCachedTransition` (`src/core/shape.zig:621`)
...
### `Registry.transitionPropertyUncached` (`src/core/shape.zig:638`)
...
### `Registry.findHashedShapeProperty` (`src/core/shape.zig:682`)
...
### `Registry.cloneForMutation` (`src/core/shape.zig:708`)
...
### `Registry.prepareUpdate` (`src/core/shape.zig:713`)
...
### `Registry.replacePrototypeAssumePrepared` (`src/core/shape.zig:734`)
...
### `Registry.relocateShape` (`src/core/shape.zig:772`)

- **签名**：`fn relocateShape(self: *Registry, shape_ptr: **Shape, new_prop_size: u32, new_bucket_count: usize) !void`。
- **作用**：以新分配替换未共享Shape的FAM布局。
- **实现**：断言未shared，保存旧尺寸；唯一fallible步骤是createWithFam。新头保留proto/hash/identity/有效计数，初始化全部属性容量并复制有效项；桶数相同直接复制，否则重建跳过null atom的链。摘旧hash/GC登记，登记新Shape并恢复hashed链，释放旧raw分配，最后写shape_ptr。
- **所有权 / 错误 / 调用**：成功迁移保留逻辑identity，不逐atom/原型retain释放；调用方保证新容量容得下有效属性且只有允许修复的拥有指针。分配失败旧指针不变；没有复制Object值存储或重写任意外部裸借用。

### `Registry.addProperty` (`src/core/shape.zig:846`)
...
### `Registry.markPropertyDeleted` (`src/core/shape.zig:854`)
...
### `Registry.compactProperties` (`src/core/shape.zig:894`)
...
### `Registry.updatePropertyFlags` (`src/core/shape.zig:1007`)
...
### `Registry.restorePropertyLayout` (`src/core/shape.zig:1016`)
...
- **所有权 / 错误 / 调用**：只恢复Shape，不恢复Object值数组、prototype或旧identity；调用方负责提供匹配基线/值布局与可存活atom。分配失败旧指针不变；旧源码“dup/free atoms”不代表本体执行RC，旧atom清理循环为空。

### `Registry.reserveProperties` (`src/core/shape.zig:1088`)
...
### `Registry.reservePropertyAppend` (`src/core/shape.zig:1103`)
...
### `Registry.reservePropertyHash` (`src/core/shape.zig:1119`)
...
### `Registry.hasReservedOwnPropertyCapacity` (`src/core/shape.zig:1126`)
...
### `Registry.dropUnshared` (`src/core/shape.zig:1146`)
...
### `Registry.destroyFromHeader` (`src/core/shape.zig:1153`)
...
### `Registry.destroyShape` (`src/core/shape.zig:1158`)

- **签名**：`fn destroyShape(self: *Registry, shape: *Shape) void`。
- **作用**：摘除GC与hash登记并释放Shape分配。
- **实现**：优先取slab accounted payload，否则按当前FAM计算；unlinkObjectWithBytes后unlink hash，遍历有效Property的循环为空，最后destroyWithFam。
- **所有权 / 错误 / 调用**：没有旧注释所称逐atom或proto释放；不销毁Object的值数组。不检查shared或使用者，原始分配释放后指针失效。

### `Registry.cloneShape` (`src/core/shape.zig:1175`)
...
- **所有权 / 错误 / 调用**：不拷贝Object值数组，不自动按新proto重算hash；调用者须提供足够容纳source.prop_count的needed及正确hash后续处理。错误释放新FAM，freePropertyAtoms当前为空；noteHolderStore不等价RC dup。

### `Registry.appendProperty` (`src/core/shape.zig:1232`)
...
### `Registry.freePropertyAtoms` (`src/core/shape.zig:1260`)

- **签名**：`fn freePropertyAtoms(_: *Registry, props: []const Property) void`。
- **作用**：保留历史清理接口但不释放atom。
- **实现**：只对props做空循环，registry参数未用。
- **所有权 / 错误 / 调用**：不是逐atom引用计数release；不能按函数名推断减少保活计数。

### `Registry.rebuildPropertyHash` (`src/core/shape.zig:1264`)
...
### `Registry.linkPropertyHash` (`src/core/shape.zig:1271`)
...
### `Registry.freshIdentity` (`src/core/shape.zig:1284`)
...
### `Registry.link` (`src/core/shape.zig:1290`)
...
### `Registry.unlink` (`src/core/shape.zig:1320`)
...
### `Registry.ensureShapeHashCapacity` (`src/core/shape.zig:1329`)
...
### `Registry.firstShapeWithHash` (`src/core/shape.zig:1364`)
...
### `Registry.insertShapeHash` (`src/core/shape.zig:1369`)
...
### `Registry.removeShapeHash` (`src/core/shape.zig:1377`)
...
### `Registry.rehashShape` (`src/core/shape.zig:1385`)
...
### `Registry.delistCondemnedShape` (`src/core/shape.zig:1423`)
...
### `Registry.verifyHashIndex` (`src/core/shape.zig:1450`)
...
### `Registry.removeShapeHashEverywhere` (`src/core/shape.zig:1465`)
...
### `Registry.removeShapeHashFromBucket` (`src/core/shape.zig:1480`)
...
### `nextPowerOfTwo` (`src/core/shape.zig:1496`)
...
### `lockstepBucketCount` (`src/core/shape.zig:1509`)
...
### `initialHash` (`src/core/shape.zig:1515`)
...
### `transitionHash` (`src/core/shape.zig:1526`)
...
### `hashIndex` (`src/core/shape.zig:1530`)
...
### `propertyBucketIndex` (`src/core/shape.zig:1536`)
...
### `shapeHash` (`src/core/shape.zig:1543`)
