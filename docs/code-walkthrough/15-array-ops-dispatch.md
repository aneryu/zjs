...
### `setValuePropertyOrThrow` (`src/exec/array_ops.zig:96`)
...
### `popCatchMarker` (`src/exec/array_ops.zig:118`)
...
### `arrayPrototypeFromGlobal` (`src/exec/array_ops.zig:133`)
...
### `arrayIteratorPrototypeFromContext` (`src/exec/array_ops.zig:143`)
...
### `isArrayMethodReceiver` (`src/exec/array_ops.zig:147`)
...
### `pushFunctionClosure` (`src/exec/array_ops.zig:152`)
...
### `arrayMethodFastCall` (`src/exec/array_ops.zig:165`)
...
### `arrayPrototypeNativeRecord` (`src/exec/array_ops.zig:222`)
...
### `buildCallSiteArray` (`src/exec/array_ops.zig:276`)
...
### `aggregateErrorsIterableToArray` (`src/exec/array_ops.zig:302`)
...
### `regExpLegacyNoCaptureSliceValue` (`src/exec/array_ops.zig:371`)
...
### `throwRegExpAccessorTypeError` (`src/exec/array_ops.zig:388`)

- **签名**：`pub fn throwRegExpAccessorTypeError(ctx: *core.JSContext, global: *core.Object, getter_value: core.JSValue) !?core.JSValue`。
...
### `constructArrayBufferNativeRecord` (`src/exec/array_ops.zig:423`)
...
### `typedArrayConstructVm` (`src/exec/array_ops.zig:459`)
...
### `typedArrayConstructLengthVm` (`src/exec/array_ops.zig:510`)
...
### `typedArrayConstructBufferVm` (`src/exec/array_ops.zig:524`)
...
### `typedArrayConstructArrayLikeVm` (`src/exec/array_ops.zig:554`)
...
### `typedArrayConstructArrayLikeOwnDataFast` (`src/exec/array_ops.zig:600`)
...
### `typedArrayArrayLikeOwnDataFastPathUsable` (`src/exec/array_ops.zig:643`)
...
### `typedArrayConstructorPrototypeVm` (`src/exec/array_ops.zig:657`)
...
### `typedArrayConstructToIndex` (`src/exec/array_ops.zig:674`)
...
### `arrayBufferConstructWithPrototype` (`src/exec/array_ops.zig:693`)
...
### `arrayBufferMaxByteLengthOption` (`src/exec/array_ops.zig:710`)
...
### `typedArrayConstructFromIterable` (`src/exec/array_ops.zig:726`)

- **签名**：`pub fn typedArrayConstructFromIterable( ctx: *core.JSContext, output: ?*std.Io.Writer, global: *core.Object, constructor: core.JSValue, args: []const core.JSValue, caller_function: ?*const bytecode.FunctionBytecode, caller_frame: ?*frame_mod.Frame, ) !?core.JSValue`。
...
### `sharedArrayBufferAccessor` (`src/exec/array_ops.zig:847`)

- **签名**：`pub fn sharedArrayBufferAccessor(ctx: *core.JSContext, receiver: core.JSValue, accessor: []const u8) !core.JSValue`。
- **作用**：`SharedArrayBuffer.prototype` 上三个 getter —— `byteLength`、`maxByteLength`、`growable`。
- **实现**：receiver 必须是 `class_id == core.class.ids.shared_array_buffer`，否则 TypeError。`byteLength` 直接给 `byteStorage().len`：共享 buffer 没有 detach 概念，所以不像 `arrayBufferAccessor` 那样先判 detached。`maxByteLength` 在非 growable 时回落到当前长度；`growable` 即 `arrayBufferMaxByteLength() != null`。`ctx` 在函数里被 `_ = ctx;` 丢弃，只为与 `arrayBufferAccessor` 保持同一签名。
- **所有权 / 错误 / 调用**：同 `arrayBufferAccessor` 的立即数返回与借用约定，但 class 必须是 `shared_array_buffer`（ArrayBuffer 走到这里就是 `error.TypeError`），而且 `ctx` 整个没用上（末尾 `_ = ctx`）——因为 shared buffer 没有 immutable 概念，不需要 runtime。唯一调用方 `exec/builtin_glue.zig:252`。

### `arrayBufferIsView` (`src/exec/array_ops.zig:863`)
...
### `arrayBufferPrototypeNativeRecord` (`src/exec/array_ops.zig:869`)
...
### `arrayBufferSliceCall` (`src/exec/array_ops.zig:927`)
...
### `arrayBufferSpeciesConstructor` (`src/exec/array_ops.zig:962`)
...
### `arrayBufferSliceToImmutableCall` (`src/exec/array_ops.zig:988`)
...
### `arrayBufferResizeCall` (`src/exec/array_ops.zig:1004`)
...
### `sharedArrayBufferGrowCall` (`src/exec/array_ops.zig:1021`)
...
### `arrayBufferLengthNumber` (`src/exec/array_ops.zig:1038`)
...
### `arrayBufferTransferCall` (`src/exec/array_ops.zig:1052`)
...
### `arrayBufferTransferToImmutableCall` (`src/exec/array_ops.zig:1059`)
...
### `arrayBufferLengthArgument` (`src/exec/array_ops.zig:1066`)
...
### `relativeSliceIndex` (`src/exec/array_ops.zig:1072`)
...
### `typedArrayAccessor` (`src/exec/array_ops.zig:1105`)
...
### `typedArrayNameFromKind` (`src/exec/array_ops.zig:1130`)
...
### `typedArraySetCall` (`src/exec/array_ops.zig:1134`)
...
### `typedArraySetElementValue` (`src/exec/array_ops.zig:1286`)
...
### `addCollectionEntriesFromArray` (`src/exec/array_ops.zig:1301`)
...
- 清单函数数（本文件分到）: 44（`src/exec/array_ops.zig` 全文件 234）
- 本文标题覆盖: 44
- 未覆盖: 无
