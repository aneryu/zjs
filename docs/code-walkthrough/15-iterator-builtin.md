...
- `Result`：`{value_index, done}`，给简单下标迭代。
...
### `staticMethodId` (`src/exec/iterator_builtin_ops.zig:26`)
...
### `prototypeMethodId` (`src/exec/iterator_builtin_ops.zig:34`)
...
### `next` (`src/exec/iterator_builtin_ops.zig:49`)

- **签名**：`pub fn next(index: *usize, length: usize) Result`。
- **作用**：下标游标前进一步：返回本次要用的下标，或报告已到尾。
- **实现**：`index.* >= length` 时返回 `{ .value_index = index.*, .done = true }` 且不动游标；否则记下当前值、`index.* += 1`，返回 `{ .value_index = current, .done = false }`。
- **所有权 / 错误 / 调用**：无所有权协议、无分配、不会失败。当前树内没有调用方（`rg` 全仓无命中），是留给 Result 形式下标迭代的公开小工具。

### `iteratorEntry` (`src/exec/iterator_builtin_ops.zig:95`)
...
### `iteratorCall` (`src/exec/iterator_builtin_ops.zig:112`)
