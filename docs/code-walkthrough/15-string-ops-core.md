...
- **实现**：对照 `js_string_replace`（quickjs.c:46012，magic 0/1）。nullish this → 「cannot convert to object」。search 是对象时：replaceAll 先 `isRegExpObservable`，flags 为 nullish → 「cannot convert to object」，flags 字符串无 `g` → 「regexp must have the 'g' flag」；然后 `callStringReplaceMethod`（@@replace）命中即返回。随后 this/search/replace 走 `toStringForAnnexB`。函数替换用 `CallSite`；字面替换走 `appendSubstitutionStringSearch`（captures 为空，只认 `$$` `$&` `$`` `$'`，`$n`/`$<name>` 原样拷）。`stringIndexOfData`；空 search 第一次命中 0，之后每次前进 1。第一次搜索未命中直接返回源字符串。`noinline` 防止经薄包装内联进 prototype dispatcher 挤掉 NumericArgs 热路径（实测 charCodeAt +4.6%）。
...
### `resolvedCodeUnitAt` (`src/exec/string_ops.zig:512`)
...
### `stringIndexOfCharData` (`src/exec/string_ops.zig:521`)
...
### `stringIndexOfData` (`src/exec/string_ops.zig:532`)
...
### `appendSubstitutionStringSearch` (`src/exec/string_ops.zig:558`)

- **签名**：`fn appendSubstitutionStringSearch( b: *StringBuffer, matched_value: core.JSValue, sp_data: core.string.String.ResolvedData, position: usize, matched_len: usize, rep_data: core.string.String.ResolvedData, ) !void`。
...
### `callStringReplaceMethod` (`src/exec/string_ops.zig:595`)
...
### `errorStackStringValue` (`src/exec/string_ops.zig:647`)
...
### `buildErrorStackStringValue` (`src/exec/string_ops.zig:733`)
...
### `formatCapturedErrorStackStringValue` (`src/exec/string_ops.zig:737`)
...
### `stringFromCodePoint` (`src/exec/string_ops.zig:741`)
...
### `stringRaw` (`src/exec/string_ops.zig:764`)
...
### `toObjectForStringRaw` (`src/exec/string_ops.zig:801`)
...
### `stringFromCharCode` (`src/exec/string_ops.zig:809`)
...
### `regExpNativeBuiltinMatches` (`src/exec/string_ops.zig:846`)
...
### `regExpAutoInitBuiltinMatches` (`src/exec/string_ops.zig:852`)
...
### `regExpToString` (`src/exec/string_ops.zig:858`)
...
### `regexpInternalStringValue` (`src/exec/string_ops.zig:885`)
...
### `regExpSymbolSearch` (`src/exec/string_ops.zig:892`)
...
### `regExpSymbolMatch` (`src/exec/string_ops.zig:907`)
...
### `regExpSymbolMatchAll` (`src/exec/string_ops.zig:922`)
...
### `stringMatchAll` (`src/exec/string_ops.zig:960`)
...
### `regExpStringIteratorPrototype` (`src/exec/string_ops.zig:1001`)
...
### `regExpSymbolReplace` (`src/exec/string_ops.zig:1009`)
...
### `regExpSymbolSplit` (`src/exec/string_ops.zig:1025`)
...
### `regExpSplitFlags` (`src/exec/string_ops.zig:1060`)
...
### `stringValueContainsByte` (`src/exec/string_ops.zig:1077`)
...
### `regExpSymbolSplitGeneric` (`src/exec/string_ops.zig:1088`)
...
- **实现**：借用接收串的扁平体（不预先拷贝码元，避免把 latin1 拓宽）。空串输入时只调一次 exec，未命中就把整串作为唯一元素。主循环里每轮把 `lastIndex` 写成当前 `pos` 后调 `regExpExecGeneric`：结果为 null 或 `end == start` 时按 `advanceStringIndexBody` 推进；否则切出 `[start, pos)` 段，再把结果数组里 1..length 的捕获原样（不做 ToString）追加，任一时刻 `out_index` 达到 `limit` 立即返回。循环结束补上尾段。
...
### `advanceStringIndexBody` (`src/exec/string_ops.zig:1183`)
...
### `advanceStringIndexUnits` (`src/exec/string_ops.zig:1191`)
...
### `regExpSymbolSearchGeneric` (`src/exec/string_ops.zig:1199`)
...
### `regExpSymbolMatchGeneric` (`src/exec/string_ops.zig:1226`)
...
### `ReplaceMatchRoots.traceRoots` (`src/exec/string_ops.zig:1300`)
...
### `ReplaceMatchRoots.provider` (`src/exec/string_ops.zig:1310`)
...
### `ReplaceMatchRoots.activate` (`src/exec/string_ops.zig:1314`)
...
### `ReplaceMatchRoots.deactivate` (`src/exec/string_ops.zig:1320`)
...
### `regExpSymbolReplaceGeneric` (`src/exec/string_ops.zig:1327`)
...
### `regExpReplaceFast` (`src/exec/string_ops.zig:1447`)
...
- **实现**：前置条件任一不满足就返回 `null` 交回通用驱动：接收者不是 `class.ids.regexp` 对象、`lastIndex` 需要可观察强制转换、没有已编译字节码、或字节码带命名分组（`$<name>` 只有通用路径支持）。flags 直接从编译字节码的 flag bits 读（不碰可能被改写的 flags getter），global 时先把 `lastIndex` 清零。源串与模板串都直接借用扁平载荷；捕获槽优先用栈上 `small_exec_slots`，不够才堆分配。循环里 `last_index > source_len` 或匹配失败时（global/sticky 会清零 `lastIndex`）结束；命中则把命中前的片段与 `appendRegExpSubstitutionFromSlots` 展开的模板写进 `StringBuffer`；非 global 只走一轮（sticky 时把 `lastIndex` 写成命中末尾），global 下空命中按 `advanceStringIndexData` 推进、否则从命中末尾继续。最后补尾段并 `b.finish`。
...
### `advanceStringIndexData` (`src/exec/string_ops.zig:1546`)
...
### `appendStringValueUnits` (`src/exec/string_ops.zig:1553`)

- **签名**：`pub fn appendStringValueUnits(rt: *core.JSRuntime, out: *std.ArrayList(u16), value: core.JSValue) !void`。
- **作用**：将字符串的码元追加到 u16 缓冲。
- **实现**：字符串按 `resolveData()` 分宽度：Latin-1 字节逐个扩为 u16，UTF-16 直接 appendSlice；非字符串先 appendRawString 到临时字节缓冲，再逐字节扩为 u16。
- **所有权 / 错误 / 调用**：临时字节缓冲由 defer 释放，out 由调用方清理。无返回值对象，转换与分配错误向上传播。

### `stringValueContainsUnitByte` (`src/exec/string_ops.zig:1567`)
...
### `stringValueUnitsEqualBytes` (`src/exec/string_ops.zig:1580`)
...
### `getRegExpFlagsStringForReplace` (`src/exec/string_ops.zig:1594`)
...
### `getRegExpFlagsString` (`src/exec/string_ops.zig:1605`)
...
### `captureReplaceMatch` (`src/exec/string_ops.zig:1618`)
...
### `freeReplaceMatches` (`src/exec/string_ops.zig:1670`)
...
### `callReplaceFunction` (`src/exec/string_ops.zig:1676`)
...
### `getSubstitutionString` (`src/exec/string_ops.zig:1699`)
...
### `replacementCaptureUnits` (`src/exec/string_ops.zig:1772`)
...
### `parseSlotCaptureRefData` (`src/exec/string_ops.zig:1802`)
...
### `appendRegExpSubstitutionFromSlots` (`src/exec/string_ops.zig:1843`)
...
### `stringLengthIndex` (`src/exec/string_ops.zig:1893`)
...
### `isEmptyStringValue` (`src/exec/string_ops.zig:1899`)
...
### `advanceStringIndexNumber` (`src/exec/string_ops.zig:1911`)
...
### `replaceRegExpLegacySlot` (`src/exec/string_ops.zig:1933`)
...
### `stringAtomId` (`src/exec/string_ops.zig:1944`)
...
### `callStringBody` (`src/exec/string_ops.zig:1958`)
...
### `callStringCharAtBody` (`src/exec/string_ops.zig:1971`)
...
- 清单函数数（本文件分到）: 69（`src/exec/string_ops.zig` 全文件 155）
- 本文标题覆盖: 69
- 未覆盖: 无
