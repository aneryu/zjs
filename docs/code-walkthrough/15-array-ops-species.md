...
### `arraySpeciesCreate` (`src/exec/array_ops.zig:3543`)
...
### `arrayHasDefaultSpecies` (`src/exec/array_ops.zig:3604`)
...
### `defaultArraySpeciesCreate` (`src/exec/array_ops.zig:3635`)
...
### `arrayConstructorFromGlobal` (`src/exec/array_ops.zig:3645`)
...
### `arraySpeciesOriginalIsArray` (`src/exec/array_ops.zig:3650`)
...
### `arraySpeciesConstructorIsForeignIntrinsicArray` (`src/exec/array_ops.zig:3667`)
...
### `arraySpeciesConstructorIsRealmIntrinsicArray` (`src/exec/array_ops.zig:3675`)
...
### `arrayFromCall` (`src/exec/array_ops.zig:3681`)
...
### `fromAsyncStateSet` (`src/exec/array_ops.zig:3795`)
...
### `fromAsyncStateGet` (`src/exec/array_ops.zig:3800`)
...
- **所有权 / 错误 / 调用**：返回值语义与文档注释不一致，以代码为准：它只是 `getOwnDataPropertyValue` 的**借用**读出（TGC 下没有 retain，值的存活靠 state 对象这条 GC 边），槽不存在时断言确实没有该属性并返回 undefined。不分配、无 error set、`rt` 参数未用。调用方遍布 fromAsync 状态机。

### `fromAsyncStateNumber` (`src/exec/array_ops.zig:3806`)
...
### `fromAsyncGetMethod` (`src/exec/array_ops.zig:3812`)
...
### `arrayFromAsyncCall` (`src/exec/array_ops.zig:3837`)
...
### `fromAsyncStart` (`src/exec/array_ops.zig:3863`)
...
### `fromAsyncAwait` (`src/exec/array_ops.zig:3981`)
...
### `fromAsyncContinuation` (`src/exec/array_ops.zig:3998`)
...
### `arrayFromAsyncContinuationCall` (`src/exec/array_ops.zig:4011`)
...
### `fromAsyncResume` (`src/exec/array_ops.zig:4041`)
...
### `fromAsyncOnNextResult` (`src/exec/array_ops.zig:4108`)
...
### `fromAsyncAdvanceIterIndex` (`src/exec/array_ops.zig:4155`)
...
### `fromAsyncIterStep` (`src/exec/array_ops.zig:4177`)
...
### `fromAsyncArrayLikeStep` (`src/exec/array_ops.zig:4194`)
...
- **实现**：关键调用：`fromAsyncStateNumber`、`fromAsyncFinish`、`fromAsyncStateGet`、`propertyAtomFromLengthIndex`、`@intFromFloat`、`getValueProperty`、`fromAsyncAwait`。
- **所有权 / 错误 / 调用**：无返回值；k ≥ len 时转 `fromAsyncFinish` 收尾，否则从 `items` 槽读第 k 个元素再 Await 进 phase 3。`propertyAtomFromLengthIndex` 产出的 atom **没有 `deinit`**——大索引（>2^31）的 pin 在这里不会被释放，是本函数与其它索引读取点（都写了 `defer key.deinit`）的一处不一致。error set：属性 getter 与 Await 透传。调用方 `fromAsyncStart`（`:3975`）与 `fromAsyncResume` 的 phase 3/4 臂（`:4087`、`:4093`）。

### `fromAsyncDefineElement` (`src/exec/array_ops.zig:4216`)
...
- **实现**：从 state 取 `target`（不是对象则 `error.TypeError`）与 `k`，下标 atom 经 `propertyAtomFromLengthIndex`，再 `createDataPropertyOrThrow`。错误：error.TypeError。
- **所有权 / 错误 / 调用**：无返回值；`value` 借用，写入后由 target 对象持有。`target` 从 state 槽读出，不是对象 → `error.TypeError`。index atom 同样**未 `deinit`**（见 `fromAsyncArrayLikeStep` 的同一问题）。error set：`createDataPropertyOrThrow` 在目标拒绝定义时的 `error.TypeError`，以及 OOM。调用方 `fromAsyncResume`（phase 2/3/4 共三处：`:4068`、`:4085`、`:4091`）与 `fromAsyncOnNextResult`（`:4147`）。

### `fromAsyncFinish` (`src/exec/array_ops.zig:4234`)
...
### `fromAsyncReject` (`src/exec/array_ops.zig:4250`)
...
### `fromAsyncCloseWithError` (`src/exec/array_ops.zig:4265`)
...
### `fromAsyncCloseWithValue` (`src/exec/array_ops.zig:4282`)
...
### `typedArrayFromStaticCall` (`src/exec/array_ops.zig:4309`)
...
### `typedArrayFromIteratorValue` (`src/exec/array_ops.zig:4354`)
...
### `fromArrayLikeSource` (`src/exec/array_ops.zig:4390`)
...
### `typedArrayFromArrayLikeSource` (`src/exec/array_ops.zig:4469`)
...
### `arrayFromArrayLike` (`src/exec/array_ops.zig:4484`)
...
### `arrayFromIteratorLike` (`src/exec/array_ops.zig:4499`)
...
- **实现**：输出对象 = 构造器 constructor-like 时 `Construct(C)`（无参），否则新建默认 Array。`next` 必须可调用，否则 TypeError。循环里 `next()`、结果非对象、读 `done`/`value`、mapper、define 六处失败都先 `iteratorCloseValue` 再把**原**错误抛出去。`done` 用 `asBool() == true` 判定（非布尔的 truthy done 不会终止循环）。含循环：按 length 或迭代器步进处理元素。错误：error.TypeError。
...
### `arrayOfCall` (`src/exec/array_ops.zig:4559`)
...
### `isArrayStaticRecord` (`src/exec/array_ops.zig:4598`)
...
### `arrayPrototypeRecordId` (`src/exec/array_ops.zig:4603`)
...
### `isArrayPrototypeRecord` (`src/exec/array_ops.zig:4633`)
...
### `typedArrayOfStaticCall` (`src/exec/array_ops.zig:4637`)
...
### `createArrayFactoryDataPropertyOrThrow` (`src/exec/array_ops.zig:4657`)
...
### `createArrayDataOrTypedArrayElement` (`src/exec/array_ops.zig:4684`)
...
### `typedArrayConstructorObject` (`src/exec/array_ops.zig:4708`)
...
### `arrayMapCall` (`src/exec/array_ops.zig:4714`)
...
### `ArraySortEntry.freeEntry` (`src/exec/array_ops.zig:4751`)
...
### `SortScratch` (`src/exec/array_ops.zig:4764`)
...
### `ArraySortEntry.acquire` (`src/exec/array_ops.zig:4769`)
...
### `ArraySortEntry.release` (`src/exec/array_ops.zig:4774`)
...
### `SortEntryRootWindow.activate` (`src/exec/array_ops.zig:4789`)
...
### `SortEntryRootWindow.deactivate` (`src/exec/array_ops.zig:4799`)
...
### `arraySortCall` (`src/exec/array_ops.zig:4811`)
...
### `arraySortCompare` (`src/exec/array_ops.zig:4972`)
...
### `stableArraySortEntries` (`src/exec/array_ops.zig:5021`)
...
### `arrayByCopyCall` (`src/exec/array_ops.zig:5105`)
...
### `typedArrayByCopyCall` (`src/exec/array_ops.zig:5271`)
...
### `arrayFlatCall` (`src/exec/array_ops.zig:5346`)
...
### `flattenIntoArray` (`src/exec/array_ops.zig:5412`)
...
### `createArrayByCopyOutput` (`src/exec/array_ops.zig:5463`)
...
### `typedArrayCreateSameType` (`src/exec/array_ops.zig:5470`)
...
### `typedArrayByCopyCoerceValue` (`src/exec/array_ops.zig:5483`)
...
### `defineArrayByCopyElement` (`src/exec/array_ops.zig:5502`)
...
### `arrayCopyIndex` (`src/exec/array_ops.zig:5511`)
...
### `toIntegerOrInfinityForArrayByCopy` (`src/exec/array_ops.zig:5528`)
...
### `arrayByCopySortCompare` (`src/exec/array_ops.zig:5543`)
...
### `arraySortStringKey` (`src/exec/array_ops.zig:5576`)
...
### `typedArrayDefaultSortCompare` (`src/exec/array_ops.zig:5594`)
...
### `stableSortTieBreak` (`src/exec/array_ops.zig:5621`)
...
### `typedArrayOwnKeys` (`src/exec/array_ops.zig:5627`)
...
### `isTypedArrayInternalOwnKey` (`src/exec/array_ops.zig:5660`)

- **签名**：`pub fn isTypedArrayInternalOwnKey(rt: *core.JSRuntime, atom_id: core.Atom) bool`。
- **作用**：判断某个 atom 是不是 TypedArray 的内部访问器名（`buffer` / `length` / `byteLength` / `byteOffset`）——ownKeys 不把它们当自有属性列出来。
- **实现**：`rt` 未使用；直接与四个预定义 atom 比对。
- **所有权 / 错误 / 调用**：无：四个预定义 atom 的等值比较，不分配、不 retain atom、无 error；`rt` 形参被 `_ = rt` 丢掉，只为跟同族谓词保持签名一致。唯一调用方 `typedArrayOwnKeys`（`src/exec/array_ops.zig:5648`），在合并 ordinary own keys 时把这四个名字滤掉。

### `atomicsTypedArray` (`src/exec/array_ops.zig:5668`)
...
### `atomicsTypedArrayIsBigInt` (`src/exec/array_ops.zig:5680`)
...
- 清单函数数（本文件分到）: 69（`src/exec/array_ops.zig` 全文件 234）
- 本文标题覆盖: 69
- 未覆盖: 无
