...
### `appendEvalClosureSeed` (`src/exec/eval_entry.zig:37`)
...
### `directEvalVarIsInParameterScope` (`src/exec/eval_entry.zig:61`)
...
- **所有权 / 错误 / 调用**：纯 atom id 比较，不分配、不抛。唯一调用方 `createDirectEvalClosureSeed` 的 argument-scope 分支（`src/exec/eval_entry.zig:122`），用来把参数初始化器里的 eval 可见集裁到 QuickJS 的伪参数绑定。

### `createDirectEvalClosureSeed` (`src/exec/eval_entry.zig:75`)
...
### `directEvalOuterVarRefView` (`src/exec/eval_entry.zig:161`)
...
### `ownedCellFromValue` (`src/exec/eval_entry.zig:172`)
...
### `directEvalSeedFrameVarRef` (`src/exec/eval_entry.zig:178`)
...
### `resolveDirectEvalClosureCell` (`src/exec/eval_entry.zig:228`)
...
### `execDirectEval` (`src/exec/eval_entry.zig:260`)
...
### `isContextIntrinsicEval` (`src/exec/eval_entry.zig:334`)
...
### `execApplyEval` (`src/exec/eval_entry.zig:338`)
...
### `directEval` (`src/exec/eval_entry.zig:385`)
...
### `directEvalThisValue` (`src/exec/eval_entry.zig:495`)
...
### `capturedSpecialValue` (`src/exec/eval_entry.zig:515`)
...
### `directEvalNewTargetValue` (`src/exec/eval_entry.zig:529`)
...
- **所有权 / 错误 / 调用**：返回的是借用值——要么是 caller 帧 var_ref 里的 `new.target`（`capturedSpecialValue`），要么是 `frame.newTargetValue()`；不 retain、不建根，由调用方 `directEval` 在同一帧存活期内立刻交给 `runWithCallEnv`（`src/exec/eval_entry.zig:448`，且只在 `eval_allows_new_target` 时调用）。无 error set。

### `directEvalVisibleLocalNameCount` (`src/exec/eval_entry.zig:537`)
