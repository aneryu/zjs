...
### `destroyDetachedClassPayload` (`src/core/object.zig:173`)
...
### `Object.auditNeedsFinalizerBit` (`src/core/object.zig:2678`)
...
### `Object.destroyFromHeader` (`src/core/object.zig:2714`)
...
### `destroyFromHeaderSlow` (`src/core/object.zig:2734`)
...
### `Object.clearBorrowedReferencesForDestroyedObject` (`src/core/object.zig:2897`)
...
### `Object.drainBorrowedWeakCleanup` (`src/core/object.zig:2921`)
...
### `Object.clearBorrowedReferencesForDestroyedIdentity` (`src/core/object.zig:2929`)
...
### `Object.clearBorrowedReferencesForMatcher` (`src/core/object.zig:2933`)
...
- **实现**：先用 refreshBorrowedReferenceHolderIndexes 修复 cached indices，创建本次扫描共享的finalization_enqueue_blocked=false；逐项跳过无相关状态者，否则调用其清理。当前项仍在同索引则前进；列表变化后重新查current索引，已移除则留当前位置处理替入项，仍存在则调整游标到其后。
- **所有权 / 错误 / 调用**：不以固定长度for遍历；清理回调可能修改列表。局部blocked状态传给各holder处理，具体弱表/FR行为由下层实现；不在此直接释放Object。

### `Object.refreshBorrowedReferenceHolderIndexes` (`src/core/object.zig:2966`)

- **签名**：`fn refreshBorrowedReferenceHolderIndexes(rt: *JSRuntime) void`。
- **作用**：修复borrowed holder的缓存索引。
- **实现**：遍历当前列表并setBorrowedReferenceHolderIndex(index)。
- **所有权 / 错误 / 调用**：不删除或压缩任何entry，长度和顺序均不在此改变；旧名 compactBorrowedReferenceHolders 延续的是已退役的 weak husk 压缩机制，现已按实际行为改名。

### `Object.runtimeBorrowedReferenceHolderIndex` (`src/core/object.zig:2972`)
...
### `Object.pruneBorrowedReferenceHolderIfEmpty` (`src/core/object.zig:2986`)

- **签名**：`pub fn pruneBorrowedReferenceHolderIfEmpty(self: *Object, rt: *JSRuntime) void`。
- **作用**：从runtime列表移除已无borrowed状态的holder。
- **实现**：未标holder返回；mayContainBorrowedReferences为false才unregisterBorrowedReferenceHolder。
- **所有权 / 错误 / 调用**：不销毁对象或payload，是否为空依赖该谓词的语义，不等于JS对象没有普通属性。

### `Object.mayContainBorrowedReferences` (`src/core/object.zig:2995`)

- **签名**：`fn mayContainBorrowedReferences(self: *const Object) bool`。
- **作用**：查询对象是否仍有需要borrowed清理的弱状态；prune 与扫描筛选共用这一个谓词。
- **实现**：WeakRef identity非null、collection weak_entries非空或FR cells非空任一满足true，其余false。
- **所有权 / 错误 / 调用**：不检查每个cell是否active，也不遍历普通强属性，不是更宽泛的class判断，pending/queued等仍计入非空cells。逐字重复的 hasBorrowedReferences 双胞胎已并入本函数，死参 `_: *JSRuntime` 一并删除。

### `BorrowedIdentityMatcher.matches` (`src/core/object.zig:3012`)
...
### `Object.clearBorrowedReferencesToDestroyedIdentities` (`src/core/object.zig:3020`)
...
### `Object.clearWeakIdentities` (`src/core/object.zig:3030`)
...
### `Object.weakIdentityIsLive` (`src/core/object.zig:6707`)
...
### `Object.objectFromValue` (`src/core/object.zig:6718`)
...
### `Object.markClassPayload` (`src/core/object.zig:6724`)
...
### `Object.collectReachableObjects` (`src/core/object.zig:6747`)
...
### `Object.ClassPayloadTraceAdaptor` (`src/core/object.zig:6753`)
...
### `ClassPayloadTraceAdaptor.visitValue` (`src/core/object.zig:6757`)
...
### `ClassPayloadTraceAdaptor.visitObject` (`src/core/object.zig:6777`)
...
### `Object.traceClassPayloadRootEdges` (`src/core/object.zig:6803`)
...
### `traceClassPayloadRootEdges.Adaptor.visitValue` (`src/core/object.zig:6808`)
...
### `traceClassPayloadRootEdges.Adaptor.visitObject` (`src/core/object.zig:6816`)
...
### `Object.isDetachedGeneratorShellForGc` (`src/core/object.zig:6834`)
...
### `Object.traceDetachedGeneratorShellEdges` (`src/core/object.zig:6844`)
...
### `Object.recordTraceStorageFootprint` (`src/core/object.zig:6857`)
...
### `recordTraceStorageFootprint.Helper.backing` (`src/core/object.zig:6866`)
...
### `Object.traceUnusualPropertyFallible` (`src/core/object.zig:7052`)
...
### `Object.tracePropertyEntriesFallible` (`src/core/object.zig:7076`)
...
### `Object.traceDataPropertyEntriesFallible` (`src/core/object.zig:7097`)
...
### `Object.tracePropertyEdgesFallible` (`src/core/object.zig:7102`)
...
### `Object.traceChildEdgesFallible` (`src/core/object.zig:7138`)
...
### `traceChildEdgesFallible.Helper.callVisitObject` (`src/core/object.zig:7140`)
...
### `traceChildEdgesFallible.Helper.callVisitValue` (`src/core/object.zig:7144`)
...
### `traceChildEdgesFallible.Helper.callVisitShape` (`src/core/object.zig:7148`)
...
### `traceChildEdgesFallible.Helper.callVisitRealm` (`src/core/object.zig:7152`)
...
### `traceChildEdgesFallible.Helper.traceOptValue` (`src/core/object.zig:7156`)
...
### `Object.traceChildEdges` (`src/core/object.zig:7361`)
...
### `Object.traceChildEdgesNoFail` (`src/core/object.zig:7365`)
...
### `Object.collectDirectChildObjects` (`src/core/object.zig:7373`)
...
### `collectDirectChildObjects.CollectVisitor.visitObject` (`src/core/object.zig:7379`)
...
### `collectDirectChildObjects.CollectVisitor.visitValue` (`src/core/object.zig:7386`)
...
### `collectDirectChildObjects.CollectVisitor.visitWeakCollectionEntry` (`src/core/object.zig:7390`)
...
### `collectDirectChildObjects.CollectVisitor.visitFinalizationCell` (`src/core/object.zig:7394`)
...
### `Object.collectValueObject` (`src/core/object.zig:7404`)
...
### `Object.collectFunctionBytecodeChildObjects` (`src/core/object.zig:7413`)
...
### `Object.weakIdentityFromValue` (`src/core/object.zig:7423`)
...
### `Object.weakIdentityFromValuePeek` (`src/core/object.zig:7431`)
...
### `Object.objectFromWeakCandidate` (`src/core/object.zig:7437`)
...
### `Object.objectIsCycleGarbage` (`src/core/object.zig:7446`)
...
### `Object.headerIsCycleGarbage` (`src/core/object.zig:7450`)
...
### `Object.clearValueReferenceToVisited` (`src/core/object.zig:7460`)
...
### `Object.clearFunctionBytecodeReferencesToVisited` (`src/core/object.zig:7478`)
...
### `Object.valueReferencesVisited` (`src/core/object.zig:7488`)
...
- 清单函数数（本文件分到）: 61（`src/core/object.zig` 全文件 873）
- 本文标题覆盖: 61
- 未覆盖: 无
