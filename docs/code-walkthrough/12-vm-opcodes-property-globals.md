...
### `closureVarAt` (`src/exec/vm_property_globals.zig:32`)
...
### `throwGlobalTdzReferenceError` (`src/exec/vm_property_globals.zig:37`)
...
### `getVarFromGlobalObject` (`src/exec/vm_property_globals.zig:55`)
...
### `getVar` (`src/exec/vm_property_globals.zig:95`)
...
- **所有权 / 错误 / 调用**：`useFastGlobalDataValue` push。分发冷路径；热路径有自己的 own-data 内联。

### `useFastGlobalDataValue` (`src/exec/vm_property_globals.zig:202`)

- **签名**：`fn useFastGlobalDataValue( ctx: *core.JSContext, output: ?*std.Io.Writer, stack: *stack_mod.Stack, function: *const bytecode.FunctionBytecode, global: *core.Object, frame: *frame_mod.Frame, catch_target: *?usize, site_pc: usize, atom_id: core.Atom, value: core.JSValue, ) !Step`。
- **作用**：把 IC 命中的全局数据压栈；空的 int/bigint/string/URI 前瞻块保留给剖析挂钩。
- **实现**：若干 `_ =` 与空 if（`nextOpCanStartGlobalUriCall1`），然后 `stack.push(value)`。
- **所有权 / 错误 / 调用**：value 按 property_direct 约定。getVar。

### `nextOpCanStartGlobalUriCall1` (`src/exec/vm_property_globals.zig:229`)

- **签名**：`fn nextOpCanStartGlobalUriCall1(function: *const bytecode.FunctionBytecode, frame: *const frame_mod.Frame) bool`。
- **作用**：下一指令是否像 `encodeURI(x)` 那种 `push/get + call1`（剖析/融合提示）。
- **实现**：看 `call1` 是否跟在 `push_atom_value` / `get_var_ref*` / `get_var*` 后，并检查长度。
- **所有权 / 错误 / 调用**：`useFastGlobalDataValue`。

### `putVar` (`src/exec/vm_property_globals.zig:242`)
...
### `globalOwnRejectedNonStrictSet` (`src/exec/vm_property_globals.zig:357`)
...
### `canUseFastGlobalVarWrite` (`src/exec/vm_property_globals.zig:373`)
...
### `canUseFastGlobalUndefinedLookup` (`src/exec/vm_property_globals.zig:384`)
...
### `evalFunctionDeclaresGlobalVar` (`src/exec/vm_property_globals.zig:392`)
...
### `globalOwnAccessorWithoutSetter` (`src/exec/vm_property_globals.zig:400`)
...
### `globalDeclIsFunction` (`src/exec/vm_property_globals.zig:405`)
...
### `validateGlobalVarDeclaration` (`src/exec/vm_property_globals.zig:409`)
...
### `validateGlobalVarDeclarations` (`src/exec/vm_property_globals.zig:444`)
...
### `instantiateGlobalVarDeclarationCells` (`src/exec/vm_property_globals.zig:496`)
...
### `globalDefinition` (`src/exec/vm_property_globals.zig:516`)
...
### `QuickJS global declaration validation does not materialize auto-init properties` (`src/exec/vm_property_globals.zig:456`)
