...
`src/js_context.zig` 提供 core realm 的宿主门面。拥有型构造建立 core context 的宿主根，deinit/destroy 撤销这份根；realm 本身由 tracing GC 管理，仍被函数、job 等对象引用时可以继续存活。borrowCore 只借用已有 core 指针。公共构造在成功返回前完成 global/intrinsic bootstrap，但各个 auto-init 属性仍可延迟物化；不能把“公共 context 已就绪”理解为所有 builtin 属性均已实例化。
...
| 名字 | 含义 |
| --- | --- |
| `ContextCreateTiming` | 内部诊断：`raw_create_ns` / `bootstrap_ns`。完整公共边界仍是调用方在 `createMeasured` 外层计时。 |
...
### `PublicValueRootWindow` (`src/js_context.zig:28`)
...
### `PublicValueRootWindow.init` (`src/js_context.zig:38`)
...
### `PublicValueRootWindow.activate` (`src/js_context.zig:42`)
...
### `PublicValueRootWindow.deactivate` (`src/js_context.zig:50`)
...
### `ensureStandardGlobalsRegistered` (`src/js_context.zig:56`)
...
### `context.cb` (`src/js_context.zig:59`)
...
### `initWithOptionsImpl` (`src/js_context.zig:80`)
...
### `createImpl` (`src/js_context.zig:105`)

- **签名**：`fn createImpl( comptime measure: bool, rt: *JSRuntime, options: core.ContextOptions, timing: if (measure) *ContextCreateTiming else void, ) !*JSContext`。
- **作用**：从 runtime 分配器分配门面，再 `initWithOptionsImpl`。
- **实现**：`rt.memory.create(JSContext)`，`errdefer destroy`。
- **所有权 / 错误 / 调用**：返回的指针由调用方 `destroy` 一次。

### `createMeasured` (`src/js_context.zig:119`)

- **签名**：`pub fn createMeasured( rt: *JSRuntime, options: core.ContextOptions, timing: *ContextCreateTiming, ) !*JSContext`。
- **作用**：走同一构造实现，分别测量 core 创建和 bootstrap 两个区间；成功路径每个区间各读取起止时钟。
- **实现**：`createImpl(true, ...)`。
- **所有权 / 错误 / 调用**：时间累加到传入结构，不先清零；阶段失败时不会累加该阶段的耗时，此前完成阶段的计数保留。统计不包括门面分配、完整退出清理等外层成本，完整构造时间需调用方另测。对象所有权同 `create`。

### `JSContext.borrowCore` (`src/js_context.zig:134`)
...
### `JSContext.create` (`src/js_context.zig:138`)

- **签名**：`pub fn create(rt: *JSRuntime, options: core.ContextOptions) !*JSContext`。
- **作用**：带 `ContextOptions` 造一个 realm 门面；默认写 `create(rt, .{})`。
- **实现**：`createImpl(false, ..., {})`。
- **所有权 / 错误 / 调用**：调用方拥有宿主引用，必须 `destroy` 恰好一次。

### `JSContext.init` (`src/js_context.zig:142`)
...
### `JSContext.deinit` (`src/js_context.zig:146`)
...
### `JSContext.destroy` (`src/js_context.zig:151`)

- **签名**：`pub fn destroy(self: *JSContext) void`。
- **作用**：`deinit` 再加上释放门面堆块。
- **实现**：先记下 `rt`，cleanup waiters，`core.destroy()`，`rt.memory.destroy(JSContext, self)`。
- **所有权 / 错误 / 调用**：`create` 的配对释放。`createRealm` 的孩子不要走这条（引用在 realm-record JSValue 上）。
...
### `JSContext.runtimePtr` (`src/js_context.zig:159`)
...
### `JSContext.createValueHandle` (`src/js_context.zig:163`)
...
### `JSContext.takeValueHandle` (`src/js_context.zig:167`)
...
### `JSContext.hasException` (`src/js_context.zig:171`)
...
### `JSContext.takeException` (`src/js_context.zig:175`)
...
### `JSContext.clearException` (`src/js_context.zig:179`)
...
### `JSContext.throwValue` (`src/js_context.zig:183`)
...
### `JSContext.recordUnhandledRejection` (`src/js_context.zig:187`)
...
### `JSContext.recordUnhandledPromiseRejection` (`src/js_context.zig:191`)
...
### `JSContext.hasUnhandledRejection` (`src/js_context.zig:195`)
...
### `JSContext.takeUnhandledRejection` (`src/js_context.zig:199`)
...
### `JSContext.clearUnhandledRejection` (`src/js_context.zig:203`)
...
### `JSContext.classPrototypeSlotCount` (`src/js_context.zig:207`)
...
### `JSContext.takePendingException` (`src/js_context.zig:211`)
...
### `JSContext.pushBacktraceFrame` (`src/js_context.zig:215`)
...
### `JSContext.pushBacktraceFrameWithResolver` (`src/js_context.zig:225`)
...
### `JSContext.popBacktraceFrame` (`src/js_context.zig:237`)
...
### `JSContext.updateBacktracePc` (`src/js_context.zig:241`)
...
### `JSContext.borrowBacktracePc` (`src/js_context.zig:245`)
...
### `JSContext.updateBacktraceLocation` (`src/js_context.zig:249`)
...
### `JSContext.defineDataProperty` (`src/js_context.zig:253`)
...
### `JSContext.arrayBuffer` (`src/js_context.zig:270`)
...
### `JSContext.setStackLimit` (`src/js_context.zig:274`)
...
### `JSContext.stackLimit` (`src/js_context.zig:278`)
...
### `JSContext.setTrackUnhandledRejections` (`src/js_context.zig:282`)
...
### `JSContext.tracksUnhandledRejections` (`src/js_context.zig:286`)
...
### `JSContext.setPreserveUncaughtException` (`src/js_context.zig:290`)
...
### `JSContext.preservesUncaughtException` (`src/js_context.zig:294`)
...
### `JSContext.setHostEventLoop` (`src/js_context.zig:298`)
...
### `JSContext.clearHostEventLoop` (`src/js_context.zig:302`)
...
### `JSContext.hostEventLoop` (`src/js_context.zig:306`)
...
### `JSContext.globalObject` (`src/js_context.zig:311`)
...
### `JSContext.createObject` (`src/js_context.zig:316`)
...
### `JSContext.createString` (`src/js_context.zig:321`)
...
### `JSContext.getPropertyAtom` (`src/js_context.zig:333`)
...
### `JSContext.getProperty` (`src/js_context.zig:338`)
...
### `JSContext.getPropertyKey` (`src/js_context.zig:347`)
...
### `JSContext.deleteProperty` (`src/js_context.zig:356`)
...
### `JSContext.deletePropertyKey` (`src/js_context.zig:365`)
...
### `JSContext.hasOwnProperty` (`src/js_context.zig:374`)
...
### `JSContext.hasOwnPropertyKey` (`src/js_context.zig:383`)
...
### `JSContext.ownPropertyDescriptor` (`src/js_context.zig:392`)
...
### `JSContext.toString` (`src/js_context.zig:401`)
...
### `JSContext.toOwnedUtf8` (`src/js_context.zig:406`)
...
### `JSContext.toNumber` (`src/js_context.zig:412`)
...
### `JSContext.toIntegerOrInfinity` (`src/js_context.zig:420`)
...
### `JSContext.isCallable` (`src/js_context.zig:427`)
...
### `JSContext.isConstructor` (`src/js_context.zig:432`)
...
### `JSContext.callFunction` (`src/js_context.zig:446`)
...
### `JSContext.createError` (`src/js_context.zig:468`)
...
### `JSContext.throwError` (`src/js_context.zig:474`)
...
### `JSContext.pendingExceptionMatchesErrorName` (`src/js_context.zig:480`)
...
### `JSContext.consumePendingExceptionIfErrorName` (`src/js_context.zig:485`)
...
### `JSContext.runtimeErrorMatchesErrorName` (`src/js_context.zig:492`)
...
### `JSContext.createRealm` (`src/js_context.zig:501`)
...
### `JSContext.realmGlobal` (`src/js_context.zig:505`)
...
### `JSContext.realmGlobalObject` (`src/js_context.zig:509`)
...
### `JSContext.isArray` (`src/js_context.zig:514`)
...
### `JSContext.arrayLength` (`src/js_context.zig:520`)
...
### `JSContext.getIndex` (`src/js_context.zig:526`)
...
### `JSContext.hasOwnPropertyAtom` (`src/js_context.zig:530`)
...
### `JSContext.deletePropertyAtom` (`src/js_context.zig:534`)
...
### `JSContext.ownPropertyDescriptorAtom` (`src/js_context.zig:540`)
...
### `JSContext.retainSharedArrayBuffer` (`src/js_context.zig:553`)
...
### `JSContext.sharedArrayBufferFromRef` (`src/js_context.zig:565`)
...
### `JSContext.functionRealmGlobal` (`src/js_context.zig:579`)
...
### `JSContext.restoreUncaughtOutOfMemory` (`src/js_context.zig:597`)
...
### `JSContext.evalScriptSource` (`src/js_context.zig:604`)
...
### `JSContext.evalScriptValue` (`src/js_context.zig:614`)
...
### `JSContext.eval` (`src/js_context.zig:624`)
...
### `JSContext.runJobs` (`src/js_context.zig:630`)
...
### `JSContext.defineFunction` (`src/js_context.zig:639`)
...
### `JSContext.createFunction` (`src/js_context.zig:655`)
...
### `JSContext.defineClass` (`src/js_context.zig:694`)
...
### `JSContext.installClassInRealm` (`src/js_context.zig:706`)
...
### `JSContext.defineMemberProperty` (`src/js_context.zig:778`)
...
### `JSContext.formatException` (`src/js_context.zig:685`)
...
### `JSContext.formatExceptionStack` (`src/js_context.zig:718`)
...
### `getPropertyString` (`src/js_context.zig:731`)
...
### `arrayObjectFromValue` (`src/js_context.zig:741`)
...
### `CallSite.init` (`src/js_context.zig:142`)
...
### `CallSite.deinit` (`src/js_context.zig:146`)
...
### `CallSite.call` (`src/js_context.zig:904`)
...
### `CallSite.call0` (`src/js_context.zig:910`)

- **签名**：`pub inline fn call0(self: *CallSite) !JSValue`。
- **作用**：零参快路径。
- **实现**：`callFixed(0, &.{})`。
- **所有权 / 错误 / 调用**：`inline` 转发到 `callFixed(0, &.{})`（`src/js_context.zig:923`），后者调 `exec.call_site.callFixedInto` 并用 `pinnedLoad` 取回结果。错误是 `HostError`：JS 抛出时返回 `error.JSException` 并把异常留在 realm 的异常槽里；若该异常是 OOM 异常，`restoreUncaughtOutOfMemory`（`src/js_context.zig:601`）把它换回 `error.OutOfMemory`。返回的 `JSValue` 是借用值，不 retain、不入句柄；宿主栈上的副本靠保守栈扫描保活。调用方：`src/tests/embedding_examples.zig:239`、`:259`。

### `CallSite.call1` (`src/js_context.zig:914`)
...
### `CallSite.call2` (`src/js_context.zig:920`)
...
### `CallSite.callFixed` (`src/js_context.zig:927`)
...
### `CallSite.callWithThis` (`src/js_context.zig:935`)
