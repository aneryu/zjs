...
### `parseProgramStatements` (`src/parser.zig:8089`)

- **签名**：`pub fn parseProgramStatements(s: *State, decl_mask: DeclMask) Error!void`。
- **作用**：程序体的顶层语句循环。
- **实现**：
先压一层 using 帧（`OpenUsingBlock.push`，`errdefer using_block.unwind(s)` 在失败时把帧栈与 `active_catch_marker_depth` 还原），然后 `while (peekKind() != TOK_EOF)` 反复 `parseStatementOrDecl`，失败经 `propagateFailureHere` 钉住诊断位置；正常结束再 `finalizeCurrentUsingBlockFrame` 发顶层的 dispose 收尾。
- **所有权 / 错误 / 调用**：唯一持有的资源是 `s.using_block_frames` 上新压的那一帧（用 `s.function.memory.allocator` 扩容），失败由 `errdefer using_block.unwind(s)`（内部 `restoreUsingBlockFramesAfterError`）连同 `active_catch_marker_depth` 一起回滚；正常路径由 `finalizeCurrentUsingBlockFrame` 消费。语句产生的字节码与 `FunctionDef` 写入不可回滚。错误经 `propagateFailureHere` 钉住位置后上抛 `parser_core.Error`。`pub` 出口，除 `compile` 主路径（`src/parser.zig:16283`）外还被编译器测试直接调用（`src/compiler/test_entry.zig:87`、`src/compiler/tests.zig` 多处）。

### `parseBlockContentsAfterOpen` (`src/parser.zig:8100`)
...
- **所有权 / 错误 / 调用**：与 `parseProgramStatements` 同一套 using 帧协议：`OpenUsingBlock.push` → `errdefer using_block.unwind(s)` → 正常路径 `using_block.finalize(s)` 即 `finalizeCurrentUsingBlockFrame`。TypeScript 参数属性那段会把 `s.is_outer_constructor_block` 置回 `false`（一次性消费，不恢复），读的是 `s.current_parameter_properties` 里借来的 atom id，不 retain、也不负责释放（那份列表归 `parseFunctionDecl` / `parseClassElementFunction` 的 `defer`）。本函数**自己吃掉收尾的 `}`**，但不管 `{`（由调用方 `parseBlock` / `parseFunctionBodyBlock` 消费）。错误：缺 `}` 经 `expectToken` 折成 `error.UnexpectedToken`，其余由 `parseStatementOrDecl` 上抛。两个调用方：`parseBlock`（`src/parser.zig:8558`）与 `parseFunctionBodyBlock`（`:8570`）。

### `parseBlock` (`src/parser.zig:8129`)
...
### `parseFunctionBodyBlock` (`src/parser.zig:8145`)
...
### `parseDirectives` (`src/parser.zig:8155`)
...
### `stringLiteralStatementHasDirectiveTerminator` (`src/parser.zig:8196`)
...
### `lineTerminatorContinuesStringLiteralExpression` (`src/parser.zig:8229`)
...
### `startsKeywordAt` (`src/parser.zig:8249`)
...
### `isAsciiIdentifierContinue` (`src/parser.zig:8256`)
...
### `emitStringLiteralValue` (`src/parser.zig:8260`)
...
### `parseStatementOrDecl` (`src/parser.zig:8509`)
...
### `parseStatementOrDeclSlow` (`src/parser.zig:8536`)
...
- **所有权 / 错误 / 调用**：标签路径是唯一有资源的一段，用三层配对守住：`label_atom` 由 `labelStartAtom` 取出后由本地变量持有，跨过 `advance` 与整条被标签语句（`LabelFrame` 有意**不**持有 atom）；`pushLabelFrame` 配 `errdefer s.popLabelFrame(label_frame)`，正常路径在 `patchLabelBreaks` 之后手工 pop；`pushControlBlock` 配一条无条件 `defer popControlBlock`（块只是 `top_break` 链上的一环，退出时机不影响后面的 patch）。循环类语句（`while`/`do`/`for`/`switch`）不建这两层，而是把 atom 放进 `pending_label_atom` 并用 `defer` 还原，由语句自身建帧。错误：保留字作标签、重复标签、被标签的 class / generator / async function 都经 `failUnexpectedToken`。唯一调用方 `parseStatementOrDecl`（`src/parser.zig:8953`）。

### `parseBlockStatement` (`src/parser.zig:8608`)
...
### `parseStringStatement` (`src/parser.zig:8612`)
...
### `parseReturnStatement` (`src/parser.zig:8634`)
...
### `parseThrowStatement` (`src/parser.zig:8654`)
...
### `parseVariableStatement` (`src/parser.zig:8671`)
...
### `parseClassDeclarationStatement` (`src/parser.zig:8701`)
...
### `parseIdentifierStatement` (`src/parser.zig:8709`)
...
### `parseAwaitStatement` (`src/parser.zig:8748`)
...
### `parseImportStatement` (`src/parser.zig:8769`)
...
### `parseExportStatement` (`src/parser.zig:8792`)
...
### `parseIfStatement` (`src/parser.zig:8799`)
...
### `parseDoOrWhileStatement` (`src/parser.zig:8856`)
...
### `parseWithStatement` (`src/parser.zig:8908`)
...
### `parseDoStatement` (`src/parser.zig:8912`)
...
### `parseForStatement` (`src/parser.zig:8916`)
...
C 风格 `for (init; test; update)`：先 `pushScope`（即使 init 为空或非词法也开，对齐 QuickJS）。init 四选一——`using` / `await using`（开一层 using 帧后 `parseUsingDeclaration`）、`var`/`let`/`const`（`let` 还要过 `canTreatLetAsForInitializerExpression` 排除「`let` 只是标识符」）、普通表达式（求值后 `drop`）、或空；非空 init 之后 `closeScopes` 回到块层级。接着绑 `top_label`（用 `Emitter.bindParser`，保留 legacy 的物理 `OP_label` 语义以维持 Stage-4 顺序匹配屏障），发测试（省略测试时压 `push_true`），再建 `exit_label` + `if_false`。**update 的处理是本函数的核心手法**：它在括号里就地解析并发码，随后用 `Emitter.detachTail` 从 Builder 尾部整段摘下（并 `Emitter.discardDetachedSources` 丢掉带外源槽），等 body、`closeScopes`、`patchContinueFrame` / `patchLabelContinues` 都做完之后，再 `Emitter.spliceSegment` 接回去——于是 `continue` 落点自然位于 update 之前，而源码顺序与 LabelId 都不受影响（空 update 不摘任何东西，保持字节形状）。body 之前照例 `pushBreakFrame` + 可选 `pushLabelFrame` + `pushControlBlock`，之后 `goto top_label`、绑 `exit_label`、依次 pop 控制块 / break 帧 / label 帧，最后 `finalizeCurrentUsingBlockFrame` 与 `popScope`。
- **所有权 / 错误 / 调用**：这是本册里状态最多的一个函数。`errdefer` 只覆盖前半段（`for_scope: OpenScope` → `pop`、`for_using_block: ?OpenUsingBlock` → `unwind`，以及 Builder 的 `snapshot` / `rollback`），后半段的 break 帧、label 帧、控制块与 `parseDoOrWhileStatement` 一样只有控制块带 `defer`，其余错误路径靠上层丢弃整个 `FunctionDef`。`update_seg` 这段摘下来的字节码由 `defer s.activeBuilder().discardSegment(&update_seg)` 兜底，正常路径被 `Emitter.spliceSegment` 消费；这是 update 搬移的**唯一**机制——过去每个 C 风格 `for` 还会把 update 的原始字节与 atom 各 `alloc`+`@memcpy`+`free` 一份（`saved_update` / `saved_update_atoms`），那两块缓冲从来没有读者，已删。`emit_lexical_tdz_at_decl` 在 `let`/`const` 头期间被临时置起并 `defer` 还原。`pending_label_atom` 先取走再在两条 for-in/of 改道前写回，供被调方建帧。错误：`for await` 出现在非 async 里直接 `Error.AwaitOutsideAsyncFunction`（不走诊断），其余来自 `expectToken` / 各子解析器 / emit。唯一调用方是 `parseStatementOrDeclSlow` 的 `TOK_FOR` 臂。

### `parseBreakOrContinueStatement` (`src/parser.zig:9081`)
...
### `parseTryStatement` (`src/parser.zig:9306`)
...
- **所有权 / 错误 / 调用**：本函数的配对最密：`active_catch_marker_depth` 与 `pushReturnFinallyFrame` 成对出现两次（try 体一组、catch 体一组），每组是一个 `emitter.ProtectedRegion`（`enter` 压帧并加深度，`leave` 弹帧并还原深度；幂等，所以 `errdefer region.leave(s)` 与正常路径的 `leave` 共存）；catch 的两层作用域各是一个 `OpenScope`（`errdefer scope.pop(s)`），正常路径按**内层先出**的顺序 `close`。`finally_ref` 只是包着 `label_finally` 的值类型，被两个 return-finally 帧共享，`return` / `break` / `continue` 经 `emitControlThroughFinally`（`src/parser.zig:10468`）穿过它。错误：既无 `catch` 又无 `finally` 时 `failUnexpectedToken`；严格模式下 catch 参数叫 `eval`/`arguments` 同样报错。唯一调用方是 `parseStatementOrDeclSlow` 的 `TOK_TRY` 臂（`src/parser.zig:9021`）。

### `parseDebuggerStatement` (`src/parser.zig:9452`)
...
### `parseEmptyStatement` (`src/parser.zig:9457`)
...
### `parseExpressionStatement` (`src/parser.zig:9462`)
...
### `parseUsingDeclaration` (`src/parser.zig:9486`)
...
### `canParseModuleDeclarationHere` (`src/parser.zig:9534`)
...
### `canTreatLetAsExpressionStatement` (`src/parser.zig:9547`)
...
### `parseLetKeywordExpressionStatement` (`src/parser.zig:9586`)
...
### `parseSharedFinallyBlock` (`src/parser.zig:9671`)
...
### `parseWith` (`src/parser.zig:10233`)
...
### `declareForInOfVarBinding` (`src/parser.zig:10262`)
...
### `parseForInOf` (`src/parser.zig:10274`)

- **结构（2026-09-19 拆分后）**：三种迭代目标各一函数，结果写进 `ForInOfTarget`：`parseForInOfUsingTarget`（`using` / `await using`）、`parseForInOfDeclarationTarget`（`var`/`let`/`const`）、`parseForInOfExpressionTarget`（赋值目标或模式）；循环 lowering 只读 `target.*`。
