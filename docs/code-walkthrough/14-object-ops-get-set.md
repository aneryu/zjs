...
### `toPropertyKeyValue` (`src/exec/object_ops.zig:2410`)
...
### `toPropertyKeyAtom` (`src/exec/object_ops.zig:2422`)
...
### `callObjectToPrimitiveMethod` (`src/exec/object_ops.zig:2434`)
...
### `getMethodPropertyForOrdinaryToPrimitive` (`src/exec/object_ops.zig:2454`)
...
### `getValueProperty` (`src/exec/object_ops.zig:2478`)
...
### `probeNamedDataProperty` (`src/exec/object_ops.zig:2570`)
...
### `probePublicNamedDataPropertyFromObject` (`src/exec/object_ops.zig:2584`)
...
### `getValuePropertyNonObject` (`src/exec/object_ops.zig:2597`)
...
### `functionCallerArgumentsProperty` (`src/exec/object_ops.zig:2622`)
...
### `getPrivateValueProperty` (`src/exec/object_ops.zig:2666`)
...
### `setPrivateValueProperty` (`src/exec/object_ops.zig:2689`)
...
### `getPrimitiveProperty` (`src/exec/object_ops.zig:2719`)
...
### `ownDataOrAutoInitPropertyValue` (`src/exec/object_ops.zig:2739`)
...
### `getValuePropertyWithReceiver` (`src/exec/object_ops.zig:2751`)
...
- **实现**：从 target 沿原型：遇 Proxy → `getProxyProperty(receiver)`；own 描述符 data/accessor（getter 用 receiver 调）。链尽则退回 `getValueProperty(target_value)`（覆盖 mapped args 等入口特判）。
- **所有权 / 错误 / 调用**：receiver 可以不是 target。

### `primitiveObjectForAccess` (`src/exec/object_ops.zig:2779`)
...
- **实现**：root 住 primitive。按类型选原型。String：create string class，`ensureFlat`，逐 code unit `defineStringWrapperIndexProperty`，不可写 `length`。其它 Number/Boolean/BigInt/Symbol 只 `setOptionalValueSlot`。
- **所有权 / 错误 / 调用**：`Object.assign` 装箱、宽松 this。属性 **读** 不得走这里。

### `setValueProperty` (`src/exec/object_ops.zig:2858`)
...
### `setValuePropertyWithThrow` (`src/exec/object_ops.zig:2876`)
...
### `setWithOwnDescriptor` (`src/exec/object_ops.zig:2993`)
...
### `bytecodeFunctionObjectTag` (`src/exec/object_ops.zig:3042`)
...
### `definePropertyWithKind` (`src/exec/object_ops.zig:3050`)
...
### `PendingPropertyDescriptor.destroy` (`src/exec/object_ops.zig:3102`)
...
### `objectEnumerableOwnPropertiesCall` (`src/exec/object_ops.zig:3118`)
...
### `objectProtoGetterCall` (`src/exec/object_ops.zig:3183`)
...
### `objectProtoSetterCall` (`src/exec/object_ops.zig:3197`)
...
### `objectIsExtensibleCall` (`src/exec/object_ops.zig:3226`)
...
### `objectSetPrototypeOfCall` (`src/exec/object_ops.zig:3239`)
...
### `reflectSetPrototypeOfCall` (`src/exec/object_ops.zig:3271`)
...
### `reflectConstructPrototypeVm` (`src/exec/object_ops.zig:3296`)
...
### `objectHasImmutablePrototype` (`src/exec/object_ops.zig:3321`)

- **签名**：`pub fn objectHasImmutablePrototype(rt: *core.JSRuntime, object: *core.Object) bool`。
- **作用**：不可变原型（模块命名空间、部分 intrinsic）。
- **实现**：忽略 rt；`object.hasImmutablePrototype()`。
- **所有权 / 错误 / 调用**：setPrototypeOf 在非 Proxy 上先挡。

### `reflectDeletePropertyCall` (`src/exec/object_ops.zig:3326`)
...
### `reflectGetOwnPropertyDescriptorCall` (`src/exec/object_ops.zig:3340`)
...
### `reflectGetPrototypeOfCall` (`src/exec/object_ops.zig:3356`)
...
### `descriptorObjectFromDescriptor` (`src/exec/object_ops.zig:3369`)
...
### `descriptorFromObject` (`src/exec/object_ops.zig:3429`)
...
### `optionalBoolDescriptorProperty` (`src/exec/object_ops.zig:3515`)
...
### `getPropertyValueFromObjectChain` (`src/exec/object_ops.zig:3530`)
...
### `getSlowPropertyValueFromObject` (`src/exec/object_ops.zig:3573`)
...
### `getSuperPropertyValue` (`src/exec/object_ops.zig:3609`)
...
### `setSuperPropertyValue` (`src/exec/object_ops.zig:3642`)
...
### `findPropertyDescriptor` (`src/exec/object_ops.zig:3684`)
...
### `sameObjectIdentity` (`src/exec/object_ops.zig:3690`)
...
### `hasPropertyForWith` (`src/exec/object_ops.zig:3708`)
...
### `hasValueProperty` (`src/exec/object_ops.zig:3721`)
...
- **实现**：忽略 receiver。无 proxy → `ordinaryHasValueProperty(..., false)`。有 trap：Get `has`；缺则递归 target；调用后 `validateProxyHasResult`。
- **所有权 / 错误 / 调用**：`in`、描述符 Has、with。

### `ordinaryHasValueProperty` (`src/exec/object_ops.zig:3745`)
...
### `indexedExoticHasProperty` (`src/exec/object_ops.zig:3786`)
...
### `deleteValuePropertyOrThrow` (`src/exec/object_ops.zig:3792`)
...
### `deleteValueProperty` (`src/exec/object_ops.zig:3803`)
...
- **作用**：`[[Delete]]`。
- **实现**：忽略 receiver。非 Proxy → `object.deleteProperty`。Proxy：Get `deleteProperty`；缺 trap 递归。trap 假 → false。真则读 target gopd（嵌套 Proxy 会再 trap）：不可配置 TypeError；再 `proxyAwareIsExtensible`（**会** 打 isExtensible trap，与 `js_proxy_has` 不同，`quickjs.c:51157`）。
- **所有权 / 错误 / 调用**：返回是否删除。

### `defineValueProperty` (`src/exec/object_ops.zig:3839`)
...
### `defineFunctionNameProperty` (`src/exec/object_ops.zig:3843`)
...
### `objectHasNonEmptyName` (`src/exec/object_ops.zig:3849`)
...
### `throwNullishPropertyTypeError` (`src/exec/object_ops.zig:3858`)
...
### `throwNullishComputedPropertyTypeError` (`src/exec/object_ops.zig:3871`)
...
### `atomPropertyName` (`src/exec/object_ops.zig:3885`)
