...
### `takeEmissionSnapshot` (`src/parser.zig:2818`)
...
- **所有权 / 错误 / 调用**：不分配、不改状态：把 `curFunc()`（或 root 侧 `self.function`）里已有的四个长度和两项 provenance 抄进一个按值返回的 `EmissionSnapshot`，返回值不是借用、无需释放。无 error set。三个调用方都紧跟一条 `errdefer rollbackEmission`：`emitOpcodeBytesNoSource`（`src/parser.zig:2877`）、`parseReturnStatement`（`:9063`）、`parseThrowStatement`（`:9082`）。

### `rollbackEmission` (`src/parser.zig:2837`)
...
- **实现**：按 `emit_to_function_def` 选 `curFunc()` 或 `self.function` 作为目标，依次 `truncateAtomOperands` / `truncateSourceLocs` / 截断字节码（FunctionDef 侧是 `truncateByteCode`，root 侧是 `truncateCode`）；再 `setParserLabelCount(snapshot.label_count)`、写回 `curFunc().last_opcode_pos` 与 `self.last_opcode_source_offset`。flow-tail 摘要不恢复而是直接 `self.flowTail().valid = false`——注释记着回滚只发生在 OOM 中止路径（编译语料实测零命中），而在 `takeEmissionSnapshot` 里顺带复制一份摘要要占全部编译指令约 0.8%，所以宁可作废、让下一次查询重建。与 QuickJS 的差别也写在 doc 注释里：qjs 在 DynBuf 失败后毒化整个编译且不再恢复，zjs 返回 OOM 并保持 runtime 可用，因此绝不能让任何消费者看到这份半发布的 code/atom/source/provenance 状态。
- **所有权 / 错误 / 调用**：只退长度，不释放也不重分配任何缓冲：`truncateAtomOperands` / `truncateSourceLocs` / `truncateByteCode`（root 侧 `truncateCode`）都只改 slice 的 len、保留容量。被丢掉的 atom 操作数只是 id——`bytecode.zig:5954` 与 `:11957` 两个 `truncateAtomOperands` 里那条本该 release 的 `while` 循环体在 TGC S3-c 之后已经是空的，编译期 atom 的存活统一由 `CompileAtomScope` 这个 GC RootProvider 负责，所以这里没有任何引用计数义务。不碰 `v2_builder`（Builder 有自己的 `snapshot`/`rollback`），也不碰已发布的 `FunctionBytecode`。无 error set，三处调用全在 errdefer 上：`emitOpcodeBytesNoSource`（`:2878`）、`parseReturnStatement`（`:9064`）、`parseThrowStatement`（`:9083`）。

### `commitLastOpcode` (`src/parser.zig:2858`)

- **签名**：`fn commitLastOpcode(self: *State, opcode_pos: usize) void`。
- **作用**：记下刚写完那条指令的起始偏移，供窥孔改写（如 `getLValue` 回收最后一条 getter）定位。
- **实现**：单行：把 `opcode_pos` `@intCast` 成 `i32` 存进 `curFunc().last_opcode_pos`。只在 `emitOpcodeBytesNoSource` 追加字节成功之后调用，所以这个位置总指向一条完整指令。
- **所有权 / 错误 / 调用**：不分配、无 error set：就地写 `curFunc().last_opcode_pos` 一个 `i32` 字段。唯一调用方 `emitOpcodeBytesNoSource`（`src/parser.zig:2881`），且必在 `appendBytesNoSource` 返回成功之后，所以该位置总指向一条完整指令。

### `currentParserLabelCount` (`src/parser.zig:2862`)
...
### `setParserLabelCount` (`src/parser.zig:2868`)
...
### `emitOpcodeBytesNoSource` (`src/parser.zig:2876`)

- **签名**：`fn emitOpcodeBytesNoSource(self: *State, bytes: []const u8) Error!void`。
- **作用**：parser 阶段所有整条指令写入的公共出口：整条追加、失败整条回滚、成功记下本条起点。
- **实现**：所有 parser 阶段整条指令写入的共同入口：先 `takeEmissionSnapshot()` 并挂 `errdefer self.rollbackEmission(snapshot)`，取追加前的 `currentCodeLen()` 当本条指令起点，`appendBytesNoSource(bytes)` 把 opcode 连同立即数一次写完，成功后 `commitLastOpcode(opcode_pos)`。名字里的 NoSource 指它不发 source marker——源码位置由拥有该处的语法产生式显式发出。
- **所有权 / 错误 / 调用**：写的是 **phase-1 原始字节流** `FunctionDef.byte_code`（`appendBytesNoSource` → `FunctionDef.appendByteCode` → `growSliceBy(fd.memory)`），不是 `v2_builder`；本函数唯一的失败源就是那次 `growSliceBy` 的 `OutOfMemory`，`errdefer rollbackEmission` 把已追加的 code / atom / source_loc / label 计数退回快照点。错误一路冒泡到 `compile`（`src/parser.zig:16059`），`OutOfMemory` 原样返回给宿主而不变成 JS 异常。调用方：同文件的五个 raw 薄包装 `emitOp`/`emitOpU8`/`emitOpU16`/`emitOpI32`/`emitOpU32`（`:2901`–`:2926`，这五个本身树内零调用方）与 `emitParserLabelJumpNoSource`（`:7698`）；后者只挂在 `FinallyLabel.temp` 臂上，而该臂在树内从未被构造，所以整条 raw 字节路径目前是不可达的遗留后端。

### `appendAtomOperandAssumeCapacity` (`src/parser.zig:2884`)

- **签名**：`fn appendAtomOperandAssumeCapacity(self: *State, atom_id: Atom) void`。
- **作用**：把一条指令的 atom 立即数登记进当前发射目标的 atom 操作数表（容量已由调用方预留，故不会失败）。
- **实现**：两路转发：`emit_to_function_def` 时调 `curFunc().appendAtomOperandAssumeCapacity(atom_id)`；root 发射时调 `self.function.retainAtomOperandAssumeCapacity(atom_id)`。两个被调方（`bytecode.zig:5883` / `:11940`）如今函数体逐字相同，都只是往预留容量里放一个 atom id，名字里的 retain 是 TGC S3-c 之前的遗迹，现在并不补引用计数。
- **所有权 / 错误 / 调用**：**树内零调用方**（`State` 这一层的转发；`:2886` 那处命中是它自己调 `FunctionDef` 的同名方法）。即便被调用也不分配、不 retain：两条臂 `FunctionDef.appendAtomOperandAssumeCapacity`（`bytecode.zig:5883`）与 `Bytecode.retainAtomOperandAssumeCapacity`（`:11940`）如今函数体逐字相同，都只是在预留容量里放一个 atom id——名字里的 retain 是 TGC S3-c 之前的遗迹。编译期 atom 的存活由 `CompileAtomScope` 这个 GC RootProvider 整场编译负责，不是引用计数。无 error set（容量不足是 `assert`）。

### `markDirectEvalCall` (`src/parser.zig:2892`)
...
### `emitOp` (`src/parser.zig:2897`)

- **签名**：`fn emitOp(self: *State, op_id: u8) Error!void`。
- **作用**：追加一条无操作数指令（qjs `emit_op` 的对应物）。
- **实现**：把单字节 `op_id` 包成一元数组字面量交给 `emitOpcodeBytesNoSource`。对应 qjs `emit_op()`：只追加指令本身，source 位置由拥有它的语法产生式另行发出（quickjs.c:23852-23870）。
- **所有权 / 错误 / 调用**：不分配；唯一出口是 `emitOpcodeBytesNoSource`，写的是 **phase-1 原始字节流** `FunctionDef.byte_code`（不是 `v2_builder`），失败只可能是那次增长的 `OutOfMemory`，由 `errdefer rollbackEmission` 把整条指令退回。**树内零调用方**（`self.`/`s.` 前缀全文无命中；同名的 `Builder.emitOp` 是另一个类型的方法），属于 raw 字节后端的遗留薄包装。

### `emitOpU8` (`src/parser.zig:2904`)

- **签名**：`fn emitOpU8(self: *State, op_id: u8, val: u8) Error!void`。
- **作用**：追加一条「opcode + 单字节立即数」指令。
- **实现**：拼 `{ op_id, val }` 两字节数组，整条交给 `emitOpcodeBytesNoSource`——立即数与 opcode 同一次追加，回滚粒度才是「整条指令」。
- **所有权 / 错误 / 调用**：不分配；唯一出口是 `emitOpcodeBytesNoSource`，写的是 **phase-1 原始字节流** `FunctionDef.byte_code`（不是 `v2_builder`），失败只可能是那次增长的 `OutOfMemory`，由 `errdefer rollbackEmission` 把整条指令退回。**树内零调用方**（`self.`/`s.` 前缀全文无命中；同名的 `Builder.emitOpU8` 是另一个类型的方法），属于 raw 字节后端的遗留薄包装。

### `emitOpU16` (`src/parser.zig:2908`)

- **签名**：`fn emitOpU16(self: *State, op_id: u8, val: u16) Error!void`。
- **作用**：追加一条「opcode + u16 小端立即数」指令。
- **实现**：在 3 字节栈缓冲里写 opcode，再用 `std.mem.writeInt(u16, bytes[1..3], val, .little)` 填小端立即数，整条交给 `emitOpcodeBytesNoSource`。
- **所有权 / 错误 / 调用**：不分配；唯一出口是 `emitOpcodeBytesNoSource`，写的是 **phase-1 原始字节流** `FunctionDef.byte_code`（不是 `v2_builder`），失败只可能是那次增长的 `OutOfMemory`，由 `errdefer rollbackEmission` 把整条指令退回。**树内零调用方**（`self.`/`s.` 前缀全文无命中；同名的 `Builder.emitOpU16` 是另一个类型的方法），属于 raw 字节后端的遗留薄包装。

### `emitOpI32` (`src/parser.zig:2915`)

- **签名**：`fn emitOpI32(self: *State, op_id: u8, val: i32) Error!void`。
- **作用**：追加一条「opcode + i32 小端立即数」指令。
- **实现**：5 字节栈缓冲：`bytes[0]` 是 opcode，`std.mem.writeInt(i32, bytes[1..5], val, .little)` 写有符号小端立即数，再整条交给 `emitOpcodeBytesNoSource`。
- **所有权 / 错误 / 调用**：不分配；唯一出口是 `emitOpcodeBytesNoSource`，写的是 **phase-1 原始字节流** `FunctionDef.byte_code`（不是 `v2_builder`），失败只可能是那次增长的 `OutOfMemory`，由 `errdefer rollbackEmission` 把整条指令退回。**树内零调用方**（`self.`/`s.` 前缀全文无命中；同名的 `Builder.emitOpI32` 是另一个类型的方法），属于 raw 字节后端的遗留薄包装。

### `emitOpU32` (`src/parser.zig:2922`)

- **签名**：`fn emitOpU32(self: *State, op_id: u8, val: u32) Error!void`。
- **作用**：追加一条「opcode + u32 小端立即数」指令（常量池下标、scope 编号一类）。
- **实现**：与 `emitOpI32` 同形，只是立即数按 `u32` 小端写入 `bytes[1..5]`；用于常量池下标、scope 编号一类无符号操作数。
- **所有权 / 错误 / 调用**：不分配；唯一出口是 `emitOpcodeBytesNoSource`，写的是 **phase-1 原始字节流** `FunctionDef.byte_code`（不是 `v2_builder`），失败只可能是那次增长的 `OutOfMemory`，由 `errdefer rollbackEmission` 把整条指令退回。**树内零调用方**（`self.`/`s.` 前缀全文无命中；同名的 `Builder.emitOpU32` 是另一个类型的方法），属于 raw 字节后端的遗留薄包装。

### `emitFClosure8` (`src/parser.zig:2929`)
...
### `emitFClosure` (`src/parser.zig:2944`)
...
### `emitCloseLoc` (`src/parser.zig:2954`)
...
### `emitEnterScope` (`src/parser.zig:2967`)
...
### `emitLeaveScope` (`src/parser.zig:2975`)
...
### `closeScopes` (`src/parser.zig:2985`)
...
### `emitScopeVar` (`src/parser.zig:3000`)
...
### `emitScopeGetVar` (`src/parser.zig:3024`)
...
### `emitScopeGetVarCheckThis` (`src/parser.zig:3028`)
...
### `emitScopePutVar` (`src/parser.zig:3032`)
...
### `emitScopePutVarNoSource` (`src/parser.zig:3036`)
...
### `emitScopeGetVarUndef` (`src/parser.zig:3040`)
...
### `emitScopePutVarInit` (`src/parser.zig:3048`)
...
### `emitScopePutVarInitNoSource` (`src/parser.zig:3052`)
...
### `emitThisValue` (`src/parser.zig:3056`)
...
### `emitBigIntLiteral` (`src/parser.zig:3484`)
...
### `invalidateLastOpcode` (`src/parser.zig:3519`)
...
- **所有权 / 错误 / 调用**：不分配、无 error set：把 `activeBuilder()`（`curFunc().v2_builder.?`）的 `last_opcode_pos` 置成无效，之后的窥孔改写就不会跨过这个点。`curFunc()` 为空 builder 时 `.?` 会 panic，所以只能在 `ensureBuilderForFd` 之后调用。同名方法有两层：本条是 `State` 的转发，直接调 `Builder.invalidateLastOpcode` 的还有 `builderBindLabel`（`:3694`）/`builderBindParserLabel`（`:3703`）等 7 处（`:5638`、`:13636`、`:13668`、`:14315`、`:15062`）。`State` 这一层的调用方是 `truncateCode`（`:3770`）、`parseExpr2`（`:4023`）、`parseTaggedTemplateInvocation`（`:6044`）等 4 处。

### `flowTail` (`src/parser.zig:3523`)

- **签名**：`fn flowTail(self: *State) *bytecode.FlowTailSummary`。
- **作用**：取当前发射目标那份「尾部控制流摘要」`bytecode.FlowTailSummary` 的可写指针。
- **实现**：`emit_to_function_def` 时返回 `&self.curFunc().flow_tail`，否则返回 root 侧的 `&self.root_flow_tail`；`noteEmittedOp` / `truncateCode` / `rollbackEmission` 都经它拿同一份摘要。
- **所有权 / 错误 / 调用**：返回的是**借用**指针，指向 `curFunc().flow_tail` 或 `self.root_flow_tail` 这两块内联字段之一，调用方不得保存跨越 `pushFunction`/`popFunction`。不分配、无 error set。调用方 4 处：`rollbackEmission`（`src/parser.zig:2855`）、`noteEmittedOp`（`:3536`）、`noteEmittedLabelOperand`（`:3558`）、`truncateCode`（`:3787`）。

### `noteEmittedOp` (`src/parser.zig:3535`)

- **签名**：`fn noteEmittedOp(self: *State, op_id: u8, pos: usize, len: usize) void`。
- **作用**：每追加一条指令后增量更新尾部摘要：最后一条非 `line_num` 指令、最后一条非清理指令、尾段起点，外加可回退一步的影子历史。
- **实现**：取 `flowTail()`；`!ft.valid` 直接返回；`pos + len` 超出 `u32` 上限就把摘要判废。先把当前 `last_non_line_op` / `last_non_cleanup_op` / `tail_start` 抄进 `prev_*` 影子字段，记 `prev_op_pos = pos`、`has_prev = true`、`last_note_had_label = false`——`truncateCode` 的精确回退就靠这一层深度。再按 opcode 分级：`op.line_num` 到此返回（不算真指令）；否则更新 `last_non_line_op`；`op.leave_scope` / `op.close_loc` 属清理码，也就此返回；其余才更新 `last_non_cleanup_op` 并把 `tail_start` 推到 `pos + len`。doc 注释写明这套增量记法的前提：parser 阶段所有代码追加都经 `appendBytesNoSource` / `appendBytesNoSourceAssumeCapacity` 且每次恰好携带一条指令；唯一的多指令调用方 `appendMovedCodeWithAtoms` 会先作废摘要，而对已作废摘要 note 是 no-op；Debug 下 `flowSummary` 里的 oracle 在每次查询时交叉校验这条契约（注意该注释已陈旧：`appendBytesNoSourceAssumeCapacity` 与 `appendMovedCodeWithAtoms` 两个名字如今在树内都只出现在注释里，没有实现）。
- **所有权 / 错误 / 调用**：不分配、无 error set：只就地更新 `flowTail()` 指向的摘要；`pos + len` 超过 `u32` 时把 `valid` 置 false 让摘要作废（下次查询重建），这是本函数唯一的「失败」表达。唯一调用方 `appendBytesNoSource`（`src/parser.zig:3576`），即那条目前不可达的 raw 字节路径。

### `noteEmittedLabelOperand` (`src/parser.zig:3557`)

- **签名**：`fn noteEmittedLabelOperand(self: *State, value: u32) void`。
- **作用**：登记刚写出的跳转目标操作数（带 tag 的 parser label、前向占位 0、或已知绝对目标），让尾部摘要知道还有多少未解析目标、最大绝对目标到哪。
- **实现**：`flowTail()` 无效直接返回。`value & opcode.op.parser_label_tag` 非零即 tagged parser label：`tagged_target_count += 1` 并置 `last_note_had_label = true`；否则若 `value > ft.max_absolute_target` 就抬高 `max_absolute_target` 并同样置旗标。该旗标会让 `truncateCode` 放弃精确回退、直接判废摘要。前向占位 0 两个条件都不满足，因此对摘要无影响。
- **所有权 / 错误 / 调用**：不分配、无 error set：只累加 `flowTail()` 摘要里的 `tagged_target_count` / `max_absolute_target`，`valid` 为假时整条 no-op。唯一调用方 `emitParserLabelJumpNoSource`（`src/parser.zig:7699`），挂在从未被构造的 `FinallyLabel.temp` 臂上。

### `appendBytesNoSource` (`src/parser.zig:3569`)

- **签名**：`fn appendBytesNoSource(self: *State, bytes: []const u8) Error!void`。
- **作用**：把一条指令的原始字节追加进当前发射目标的字节码流，并顺手喂给尾部摘要。
- **实现**：先取 `pos = currentCodeLen()` 作为该指令的起点；按 `emit_to_function_def` 调 `curFunc().appendByteCode(bytes)` 或 `self.function.appendCode(bytes)`（两者都可能 OOM，错误直接外抛给 `emitOpcodeBytesNoSource` 的 `errdefer` 回滚）；成功且 `bytes.len != 0` 时以 `bytes[0]` 为 opcode 调 `noteEmittedOp(bytes[0], pos, bytes.len)`。
- **所有权 / 错误 / 调用**：唯一一处向 phase-1 原始字节流追加的地方：`FunctionDef.appendByteCode`（`bytecode.zig:5843`）或 root 侧 `Bytecode.appendCode`（`:11826`），两者都是 `growSliceBy` 在 `memory`（MemoryAccount）上扩容，所以本函数的唯一失败是 `OutOfMemory`；缓冲归 `FunctionDef` / `Bytecode` 所有，本函数不持有。成功后顺手喂一次 `noteEmittedOp`。唯一调用方 `emitOpcodeBytesNoSource`（`src/parser.zig:2880`），由它负责 `errdefer rollbackEmission`。

### `activeBuilder` (`src/parser.zig:3587`)
...
### `builderAddSourceMarker` (`src/parser.zig:3594`)
...
### `builderRecordPlainControl` (`src/parser.zig:3606`)
...
### `builderRecordU16Control` (`src/parser.zig:3617`)
...
### `builderRecordU32Control` (`src/parser.zig:3625`)
...
### `builderRecordAtomU8Control` (`src/parser.zig:3630`)
...
### `builderEmitOp` (`src/parser.zig:3637`)
...
### `builderEmitOpU8` (`src/parser.zig:3643`)
...
### `builderEmitOpU16` (`src/parser.zig:3647`)
...
### `builderEmitOpU32` (`src/parser.zig:3652`)
...
### `builderEmitOpI32` (`src/parser.zig:3657`)
...
### `builderEmitAtomOpOwned` (`src/parser.zig:3663`)
...
### `builderEmitAtomOpU8Owned` (`src/parser.zig:3667`)
...
### `builderEmitAtomOpU16Owned` (`src/parser.zig:3672`)
...
### `builderEmitJump` (`src/parser.zig:3678`)
...
### `builderNewLabel` (`src/parser.zig:3682`)
...
### `builderBindLabel` (`src/parser.zig:3691`)
...
### `builderBindParserLabel` (`src/parser.zig:3700`)
...
### `ensureBuilderForFd` (`src/parser.zig:3709`)
...
### `beginBuilderEmissionForTest` (`src/parser.zig:3721`)
...
### `beginProgramEmission` (`src/parser.zig:3728`)
...
### `currentCodeLen` (`src/parser.zig:3740`)
...
- **所有权 / 错误 / 调用**：无：纯读长度，不分配、无 error set。读的是 phase-1 原始字节流（`curFunc().byte_code` / `self.function.code`），**不是** Builder 的 `code_len`——这两个流是分开的。调用方 6 处：`takeEmissionSnapshot`（`src/parser.zig:2820`）、`emitOpcodeBytesNoSource`（`:2879`）、`appendBytesNoSource`（`:3570`），其余在 `parseForStatement`（`:9444`）、`parseFunctionParamsAndBody`（`:11608`）、`takeParserSnapshot`（`:13216`）。

### `currentCode` (`src/parser.zig:3745`)

- **签名**：`fn currentCode(self: *State) []u8`。
- **作用**：取当前发射目标的字节码可写切片，供窥孔改写、原地打补丁、尾段搬移读写。
- **实现**：两路二选一：`emit_to_function_def` 时 `curFunc().byte_code`，否则 `self.function.code`。返回的是活切片，任何截断/追加之后都要重新取。
- **所有权 / 错误 / 调用**：返回的是对 `curFunc().byte_code` / `self.function.code` 的**借用可写 slice**，调用方既不拥有也不能让它跨越任何一次追加（扩容会 realloc 使其失效）。不分配、无 error set。唯一调用方 `parseForStatement`（`src/parser.zig:9457`），当场取切片当场用完。

### `currentAtomOperands` (`src/parser.zig:3750`)

- **签名**：`fn currentAtomOperands(self: *State) []Atom`。
- **作用**：取当前发射目标的 atom 操作数表切片（指令里的 atom 立即数按发射顺序排在这里）。
- **实现**：两路二选一：`curFunc().atom_operands` 或 `self.function.atom_operands`。
- **所有权 / 错误 / 调用**：返回**借用** slice（`curFunc().atom_operands` / `self.function.atom_operands`），元素是裸 atom id：既不 retain 也不需要 release，整场编译由 `CompileAtomScope` 这个 GC RootProvider 兜底。同样不能跨越追加使用。不分配、无 error set。唯一调用方 `parseForStatement`（`src/parser.zig:9458`）。

### `truncateCode` (`src/parser.zig:3764`)

- **签名**：`fn truncateCode(self: *State, target_len: usize) Error!void`。
- **作用**：把字节码流截回 `target_len`，并同步修掉所有描述被删区间的元数据。
- **实现**：四步。①若 `curFunc().last_opcode_pos >= 0` 且已落在 `target_len` 之后，先 `invalidateLastOpcode()`（QCP-1 L3 gate：v2 解析期间回卷 legacy 流与向它发射属于同一个迁移窟窿）。②source_loc 槽：从尾部起把 `pc >= target_len` 的槽逐个退掉（`keep` 递减后 `truncateSourceLocs(keep)`），FunctionDef 与 root 两条路径各做一遍——注释说明这些 marker 在字节消失后就是垃圾，过去会被悄悄重映射到后来占据同一 pc 的代码上，而 phase-2 的流式 source 重映射也依赖槽 pc 非递减。③flow tail：只有「回退的正好是最近那一次单指令发射」（`has_prev` 且 `!last_note_had_label` 且 `prev_op_pos == target_len`）才用 `prev_*` 影子字段精确恢复并清 `has_prev`，其余情况一律 `ft.valid = false`。④真正截字节：`curFunc().truncateByteCode(target_len)` 或 `self.function.truncateCode(target_len)`。doc 注释还给出调用场景与契约：`parseAssignExpr2` / `parsePostfixExpr` 在认出赋值/自增运算符后回滚投机发出的 LHS；atom 计数要调用方另行用 `truncateAtomOperands` 配对回滚，retain/free 才平衡；底层是可增长切片，截断后缓冲仍在，重发不必重新分配。
- **所有权 / 错误 / 调用**：只缩短、不释放也不重分配：`truncateSourceLocs` / `truncateByteCode`（root 侧 `truncateCode`）都只改 slice 的 `len`，缓冲留着给重发用；被丢掉的 atom 操作数不在这里处理（调用方须另行配对 `truncateAtomOperands`）。签名上的 `Error!void` 是名义的——函数体里没有任何 `try`/`return error`。⚠️ **树内零调用方**：`self.truncateCode(` 全文无命中，doc 注释里说的 `parseAssignExpr2` / `parsePostfixExpr` 回滚路径如今走的是 Builder 自己的 `snapshot`/`rollback`；`:2846` 与 `:3803` 那两处 `truncateCode` 调的是 `Bytecode` 的同名方法，不是这一层。

### `truncateAtomOperands` (`src/parser.zig:3809`)

- **签名**：`fn truncateAtomOperands(self: *State, target_len: usize) Error!void`。
- **作用**：把 atom 操作数表截回 `target_len`，释放越界表项持有的 atom 引用计数。
- **实现**：按 `emit_to_function_def` 二选一转调 `curFunc().truncateAtomOperands(target_len)` 或 `self.function.truncateAtomOperands(target_len)`；对应的 retain 发生在 `emitOpAtom` / `retainAtomOperand` 一侧，两边必须成对使用。
- **所有权 / 错误 / 调用**：两条臂都转给 `FunctionDef.truncateAtomOperands`（`bytecode.zig:5954`）或 `Bytecode.truncateAtomOperands`（`:11957`），二者如今只改 slice `len`——里面那条本该 release 的 `while` 循环体在 TGC S3-c 之后已是空的，编译期 atom 的存活统一由 `CompileAtomScope` 这个 GC RootProvider 负责，所以并没有「释放引用计数」的义务（函数头注释已陈旧）。`Error!void` 同样是名义签名，无失败路径。⚠️ **树内零调用方**：`self.truncateAtomOperands(` 全文无命中，`rollbackEmission` 调的是 `FunctionDef`/`Bytecode` 的同名方法。

### `currentAtomOperandLen` (`src/parser.zig:3817`)

- **签名**：`fn currentAtomOperandLen(self: *State) usize`。
- **作用**：读当前发射目标已记账的 atom 操作数个数。
- **实现**：一个三元式：`emit_to_function_def` 时取 `curFunc().atom_operands.len`，否则取 `self.function.atom_operands.len`。`takeEmissionSnapshot` 用它记 atom 回滚点，`emitOpcodeBytesNoSource` 的失败路径据此截回。
...
### `emitLogicalAssignLValue` (`src/parser.zig:4145`)
...
### `LValue.deinit` (`src/parser.zig:4217`)
...
### `isRuntimeInvalidCallOpcode` (`src/parser.zig:4224`)
...
### `emitInvalidAssignmentTarget` (`src/parser.zig:4241`)
...
### `hasWithScopeFrom` (`src/parser.zig:4259`)
...
### `reemitLValueGetter` (`src/parser.zig:4284`)
...
### `hasKnownBinding` (`src/parser.zig:4561`)
...
### `strictUnresolvedAssignmentNeedsReference` (`src/parser.zig:4610`)
...
### `argumentsIdentifierIsForbidden` (`src/parser.zig:4620`)
...
### `emitYieldStarDelegation` (`src/parser.zig:5015`)
...
### `emitSuperThis` (`src/parser.zig:5101`)
...
### `emitSuperThisAndHomeObject` (`src/parser.zig:5116`)
...
### `discardTrailingGetSuper` (`src/parser.zig:5125`)
...
### `parseDelete` (`src/parser.zig:5152`)
...
### `compactAppendedTailReplacement` (`src/parser.zig:5163`)
...
### `optionalChainExitAtEnd` (`src/parser.zig:5254`)
...
### `emitDeleteNonReference` (`src/parser.zig:5287`)
...
### `finishDelete` (`src/parser.zig:5299`)
...
### `rewriteOptionalChainDeleteBuilder` (`src/parser.zig:5368`)
...
### `prepareCallReference` (`src/parser.zig:5449`)
...
### `emitPreparedCall` (`src/parser.zig:5536`)
...
### `emitOptionalChainTest` (`src/parser.zig:6107`)
...
### `emitTaggedTemplateSingletonObject` (`src/parser.zig:6496`)
...
### `realmArrayPrototype` (`src/parser.zig:6509`)
...
### `TaggedTemplateObjectBuilder.init` (`src/parser.zig:6535`)
...
### `TaggedTemplateObjectBuilder.addPart` (`src/parser.zig:6558`)
...
### `TaggedTemplateObjectBuilder.finish` (`src/parser.zig:6589`)
...
### `parseArrayLiteral` (`src/parser.zig:6602`)
...
### `parseObjectLiteral` (`src/parser.zig:6679`)
...
### `ObjectLiteralCapacityHint.invalidate` (`src/parser.zig:6709`)
...
### `ObjectLiteralCapacityHint.noteStaticProperty` (`src/parser.zig:6713`)
...
### `emitObjectMethodFunction` (`src/parser.zig:7167`)
...
### `formatBigIntPropertyName` (`src/parser.zig:7211`)
...
### `compoundAssignOpcode` (`src/parser.zig:7233`)
...
### `logicalAssignKind` (`src/parser.zig:7251`)
...
### `matchBinaryOp` (`src/parser.zig:7261`)
...
### `parseBigIntI32` (`src/parser.zig:7324`)
...
### `Emitter.newLabel` (`src/parser.zig:7369`)
...
### `Emitter.jump` (`src/parser.zig:7372`)
...
### `Emitter.jumpNoSource` (`src/parser.zig:7375`)
...
### `Emitter.bind` (`src/parser.zig:7378`)
...
### `Emitter.bindTarget` (`src/parser.zig:7381`)

- **签名**：`fn bindTarget(s: *State, label: *Label) Error!void`。
- **作用**：`bind` 的同义动词，用在「此处是某条跳转的目标」读起来更顺的调用点。
- **实现**：函数体与 `bind` 逐字相同，都是 `return emitterBindLabel(s, label.id);`；两个名字只为调用点语义清晰而并存。
- **所有权 / 错误 / 调用**：`Emitter` 是**函数命名空间**而非值类型（`struct { s: *State }` 接收者在 Debug 下每个调用点多一个临时量，足够撑爆 64 KiB 原生栈）。本函数只是一层转发：不分配、不持有资源，失败全部来自底层 Builder（`OutOfMemory` / `BytecodeOverflow` / `InvalidBytecode`），经 `mapBuilderError`（`:7448`）折成 `Error.OutOfMemory` / `Error.BytecodeOverflow` / `Error.ParserInvariant`。`label` 是调用方**栈上**的容器，`id` 只是 Builder 内的索引，没有释放义务；绑定之前指向它的跳转都是 resolver 负责回填的重定位。与 `bind` **函数体逐字相同**（都转 `emitterBindLabel(s, label.id)`），只是给「跳转目标」这个语义留的别名；树内仅 1 处调用。

### `Emitter.newPhysLabel` (`src/parser.zig:7384`)
...
### `Emitter.jumpPhysNoSource` (`src/parser.zig:7387`)
...
### `Emitter.bindPhys` (`src/parser.zig:7390`)
...
### `Emitter.op` (`src/parser.zig:7394`)
...
### `Emitter.opNoSource` (`src/parser.zig:7397`)
...
### `Emitter.opAt` (`src/parser.zig:7400`)
...
### `Emitter.opU8` (`src/parser.zig:7403`)
...
### `Emitter.opU16` (`src/parser.zig:7406`)
...
### `Emitter.opU16NoSource` (`src/parser.zig:7409`)
...
### `Emitter.callOp` (`src/parser.zig:7413`)
...
### `Emitter.opU16At` (`src/parser.zig:7416`)
...
### `Emitter.opU32` (`src/parser.zig:7419`)
...
### `Emitter.opI32` (`src/parser.zig:7422`)
...
### `Emitter.opAtom` (`src/parser.zig:7425`)
...
### `Emitter.opAtomU8` (`src/parser.zig:7428`)
...
### `Emitter.opAtomU16` (`src/parser.zig:7431`)
...
### `Emitter.opAtomU16NoSource` (`src/parser.zig:7434`)
...
### `Emitter.pushConst` (`src/parser.zig:7437`)
...
### `Emitter.pushConstOwned` (`src/parser.zig:7440`)
...
### `mapBuilderError` (`src/parser.zig:7448`)
...
### `emitterNewLabel` (`src/parser.zig:7458`)
...
### `emitterJump` (`src/parser.zig:7463`)
...
### `emitterJumpNoSource` (`src/parser.zig:7468`)
...
- **实现**：`s.activeBuilder().emitJump(op_id, label)` + `mapBuilderError`。与 `emitterJump` 的差别只在少走一层 `State.builderEmitJump`，而后者也就是同一次调用——两个名字保留下来是为了和 legacy 的 marker'd / NoSource 成对映射，方便逐站点对账。
- **所有权 / 错误 / 调用**：不分配（字节码 / reloc / atom 台账的增长都在 Builder 的 `fd.memory` 账上）；这一层的全部作用是把 `compiler.builder.Error` 经 `mapBuilderError`（`:7448`）换成 parser 的 `Error`（`OutOfMemory` / `BytecodeOverflow` 原样过，fail-closed 的 `InvalidBytecode` → `Error.ParserInvariant` 走 ICE 出口）。它不受 `rollbackEmission` 管——那条回滚的是 phase-1 raw 字节流；这里的事务性由调用方自己的 `v2b.snapshot()` / `rollback` 提供。`label` 只是 Builder 内索引，无释放义务。与 `emitterJump` 的唯一差别是少走一层 `State.builderEmitJump`（而后者也就是同一次 `activeBuilder().emitJump`），今天两者行为完全一致。树内 15 处调用。

### `emitterOp` (`src/parser.zig:7476`)
...
### `emitterOpNoSource` (`src/parser.zig:7482`)
...
### `emitterOpAt` (`src/parser.zig:7488`)
...
### `emitterOpU16At` (`src/parser.zig:7495`)
...
### `emitterAddSourceMarker` (`src/parser.zig:7506`)
...
### `emitGrammarSource` (`src/parser.zig:7512`)
...
### `emitterAtomOpOwned` (`src/parser.zig:7518`)
...
### `emitterAtomOpOwnedNoSource` (`src/parser.zig:7523`)
...
### `emitterAtomOpU16OwnedNoSource` (`src/parser.zig:7528`)
...
### `emitterScopeRefOp` (`src/parser.zig:7534`)
...
### `emitterAtomOpU8Owned` (`src/parser.zig:7540`)
...
### `emitterOpU8` (`src/parser.zig:7545`)
...
### `emitterOpU8NoSource` (`src/parser.zig:7551`)
...
### `emitterOpU16` (`src/parser.zig:7560`)
...
### `emitterCallOp` (`src/parser.zig:7567`)
...
### `emitterOpU32` (`src/parser.zig:7573`)
...
### `emitterOpI32` (`src/parser.zig:7579`)
...
### `emitterPushConst` (`src/parser.zig:7590`)
...
### `emitterPushConstOwned` (`src/parser.zig:7606`)
...
### `emitterOpU16NoSource` (`src/parser.zig:7612`)
...
### `emitterOpU32NoSource` (`src/parser.zig:7617`)
...
### `emitterDetachTail` (`src/parser.zig:7623`)
...
### `emitterDiscardDetachedSources` (`src/parser.zig:7631`)
...
- **实现**：把 `seg.sources` 取出、字段置成空切片，原切片非空时用 `s.function.memory.free(compiler.builder.SourceSlot, sources)` 释放。注释给出理由：走 legacy `appendMovedCodeWithAtoms` 契约搬运的代码保留 code 与 atom 的所有权，但 `truncateCode` 会丢掉它的 parser source 槽、splice 也不会重放这些槽（类运行时块与 for 更新块都用这个契约），所以搬家前要主动把 source 槽扔掉，免得留下对不上 pc 的 marker。
...
### `emitterSpliceSegment` (`src/parser.zig:7637`)
...
### `emitterBindLabel` (`src/parser.zig:7643`)
...
### `emitterBindLabelRaw` (`src/parser.zig:7650`)
...
### `emitterBindParserLabel` (`src/parser.zig:7656`)
...
### `emitterBindParserLabelRaw` (`src/parser.zig:7662`)
...
### `emitterRetargetLabel` (`src/parser.zig:7670`)
...
### `emitParserLabelJumpNoSource` (`src/parser.zig:7693`)

- **签名**：`fn emitParserLabelJumpNoSource(s: *State, op_id: u8, label: ParserLabelRef) Error!void`。
- **作用**：发一条跳向 legacy parser 标签的跳转，目标带 tag 等后续解析。
- **实现**：先校验 opcode：`opcode.formatOf(op_id)` 必须是 `.label` 且不能是 `opcode.op.label` 本身（那是标签定义不是跳转），否则 `Error.ParserInvariant`。然后在 5 字节缓冲里放 opcode，把 `opcode.op.parser_label_tag | label.id` 以小端 `u32` 写进操作数——tag 位标明这是待解析的 parser 标签而非绝对目标；`s.emitOpcodeBytesNoSource(&bytes)` 整条写出；最后用同一个带 tag 的值调 `s.noteEmittedLabelOperand`，让尾部摘要的 `tagged_target_count` 计上这条未解析目标。
- **所有权 / 错误 / 调用**：⚠️ 这是整份文档里唯一还走 **phase-1 raw 字节流**的发射函数：它自己拼 5 字节（opcode + `parser_label_tag | label.id`）交给 `s.emitOpcodeBytesNoSource`，因此也是唯一真正受 `errdefer rollbackEmission` 保护的路径，随后 `s.noteEmittedLabelOperand` 把这个 tagged 目标登记进 flow-tail 摘要。不分配。前置校验：`opcode.formatOf(op_id) != .label` 或 `op_id == opcode.op.label` → `Error.ParserInvariant`；其余失败是底层 `growSliceBy` 的 `OutOfMemory`。`label` 是 `ParserLabelRef`（裸 u32 id），无释放义务。**树内实际不可达**：唯一引用它的是 `FinallyLabel.temp` 臂，而该臂在树内从未被构造。

### `parserPhaseAtomTempInstruction` (`src/parser.zig:7710`)

- **签名**：`fn parserPhaseAtomTempInstruction(code: []const u8, atoms: []const Atom, pc: usize, atom_index: usize) ?ParserPhaseInstruction`。
- **作用**：解码器：若 `pc` 处是带 atom 操作数的 phase-1 临时 opcode（`scope_*` 家族 7 字节、`scope_make_ref` 11 字节、`get_field_opt_chain` 6 字节）且其 atom 与 `atoms[atom_index]` 对得上，返回长度与 `is_temp=true`，否则 null。
- **实现**：先按 opcode 给 phase-1 带 atom 的临时指令定长：`scope_get_var_undef` / `scope_get_var` / `scope_put_var` / `scope_delete_var` / `scope_get_ref` / `scope_put_var_init` / `scope_get_var_checkthis` / `scope_get_private_field` / `scope_get_private_field2` / `scope_put_private_field` / `scope_in_private_field` 一律 7 字节（opcode + atom u32 + scope u16），`scope_make_ref` 11 字节（多一个 aux32 label），`get_field_opt_chain` 在 W1 下是 6 字节的 `atom_cache_u8`；其余直接 `null`。随后两道核对：指令不得越过 `code` 末尾、`atom_index` 不得越过 `atoms` 末尾，并且指令里那 4 字节 atom 必须等于 `atoms[atom_index]`——对不上说明这个 pc 根本不是指令边界（扫描走偏），返回 `null`。全部通过才返回 `.{ .size = size, .is_temp = true }`。
- **所有权 / 错误 / 调用**：无：纯函数，参数是借来的 `code`/`atoms` 两个 slice，不分配、不改写、无 error set，形状不符就返回 `null`。它读的 `atoms[atom_index]` 只是 id 比对，没有任何引用计数动作——编译期 atom 由 `CompileAtomScope` 统一作根。唯一调用方 `parserPhaseInstruction`（`src/parser.zig:7736`）。

### `parserPhaseInstruction` (`src/parser.zig:7735`)

- **签名**：`fn parserPhaseInstruction(code: []const u8, atoms: []const Atom, pc: usize, atom_index: usize) ParserPhaseInstruction`。
- **作用**：解码器：给出 `pc` 处 phase-1 指令的长度与是否临时码——先试 `parserPhaseAtomTempInstruction`，再认 `enter_scope`/`leave_scope`/`label`/`get_array_el_opt_chain`/`set_class_name`/`line_num` 这批无 atom 临时码，其余按 `opcode.sizeOf` 算。
- **实现**：三级判定。①先试 `parserPhaseAtomTempInstruction`，命中就返回它（带 atom 的 phase-1 临时码）。②再看一组不带 atom 的临时码——`enter_scope` / `leave_scope` / `label` / `get_array_el_opt_chain` / `set_class_name` / `line_num`——用 `opcode.sizeOfPhase1(op_id)` 定长并标 `is_temp = true`。③都不是就当普通指令，长度取 `opcode.sizeOf(code[pc])`，`is_temp` 保持默认 false。
- **所有权 / 错误 / 调用**：无：纯函数，参数 `code`/`atoms` 是借来的只读 slice，不分配、不改写、无 error set（形状异常时靠返回值保守退化而不是报错）；读 `atoms[...]` 只是 id 比对，编译期 atom 由 `CompileAtomScope` 统一作根，没有引用计数动作。调用方 3 处，全是 phase-1 指令行走器：`lastParserPhaseOpcode`（`:8024`）、`trailingCleanupStart`（`:8049`）、`hasJumpToCurrentEnd`（`:8080`）。

### `parserPhaseInstructionHasAtom` (`src/parser.zig:7751`)

- **签名**：`fn parserPhaseInstructionHasAtom(op_id: u8, is_temp: bool) bool`。
- **作用**：谓词：该指令是否消耗一个 atom 操作数——临时码查固定名单，非临时码看 `opcode.formatOf` 是否属于 atom 系格式。
- **实现**：分两路。`is_temp` 为真时走白名单 `switch`：十二个 `scope_*` 临时码（含 `scope_make_ref`）与 `get_field_opt_chain` 返回 true，其余 false。非临时码则查 `opcode.formatOf(op_id)`，格式属于 `.atom` / `.atom_u8` / `.atom_cache_u8` / `.atom_u16` / `.atom_label_u8` / `.atom_label_u16` 之一的返回 true。扫描器据此决定是否把 atom 操作数游标往前推一格。
- **所有权 / 错误 / 调用**：无：只看 `op_id` 与 `is_temp` 两个标量的查表 switch，不碰 atom 表也不分配，无 error set；作用只是让调用方的 atom 游标加不加一。三个调用方，都在 phase-1 指令行走器里：`lastParserPhaseOpcode`（`src/parser.zig:8032`）、`trailingCleanupStart`（`:8058`）、`hasJumpToCurrentEnd`（`:8087`）。

### `parserPhaseLabelOperandOffset` (`src/parser.zig:7776`)

- **签名**：`fn parserPhaseLabelOperandOffset(op_id: u8, pc: usize, is_temp: bool) ?usize`。
- **作用**：给出该指令 label 操作数的字节偏移：临时码 `scope_make_ref` 在 `pc+5`，`.label` 格式在 `pc+1`（`op.label` 自身返回 null），`.atom_label_u8/u16` 在 `pc+5`，`.label_u16` 在 `pc+1`，其余返回 null。
- **实现**：返回该指令里 label 操作数的绝对偏移，没有则 `null`。特例先行：临时码 `scope_make_ref` 的 aux label 在 `pc + 5`（opcode + atom u32 之后）。其余按 `opcode.formatOf(op_id)` 分：`.label` 在 `pc + 1`，但 `opcode.op.label` 自己是标签定义而非跳转，返回 `null`；`.atom_label_u8` / `.atom_label_u16` 在 `pc + 5`；`.label_u16` 在 `pc + 1`；其余 `null`。
- **所有权 / 错误 / 调用**：无：纯查表，返回 label 立即数在 `code` 里的偏移或 `null`，不分配、无 error set。唯一调用方 `hasJumpToCurrentEnd`（`src/parser.zig:8083`）。

### `expressionStatementKeepsCompletion` (`src/parser.zig:7914`)
...
### `caseTailCanFallthrough` (`src/parser.zig:7943`)
...
### `isLiveCode` (`src/parser.zig:7984`)
...
### `emitPlainTailForTest` (`src/parser.zig:8014`)
...
### `lastParserPhaseOpcode` (`src/parser.zig:8018`)

- **签名**：`fn lastParserPhaseOpcode(code: []const u8, atoms: []const Atom, skip_cleanup: bool) ?u8`。
- **作用**：线性扫一段 parser 阶段字节码，取出最后一条「有意义的」指令的 opcode。
- **实现**：从 `pc = 0` 起用 `parserPhaseInstruction(code, atoms, pc, atom_index)` 逐条定长前进（长度为 0 或越界返回 `null`，让调用方保守处理）。每条指令：`line_num` 一律不算；`skip_cleanup` 为真时 `leave_scope` / `close_loc` 也不算；其余记进 `last`。若 `parserPhaseInstructionHasAtom` 说这条带 atom，就把 `atom_index` 推进一格（atom 校验靠它对齐）。扫完返回 `last`，空代码返回 `null`。
- **所有权 / 错误 / 调用**：无：纯函数，参数 `code`/`atoms` 是借来的只读 slice，不分配、不改写、无 error set（形状异常时靠返回值保守退化而不是报错）；读 `atoms[...]` 只是 id 比对，编译期 atom 由 `CompileAtomScope` 统一作根，没有引用计数动作。长度为 0 或越界时返回 `null`，让调用方保守处理。⚠️ 树内**唯一**调用方是 `functionNeedsImplicitReturn`（`:8103`），而后者自己零调用方——这条扫描路径整体已被摘要驱动的 `isLiveCode` 取代。

### `trailingCleanupStart` (`src/parser.zig:8043`)

- **签名**：`fn trailingCleanupStart(code: []const u8, atoms: []const Atom) usize`。
- **作用**：算出尾部「纯清理指令串」的起点偏移——即 `lastParserPhaseOpcode(skip_cleanup=true)` 会跳过的那段最大后缀（`line_num` / `leave_scope` / `close_loc`）的第一个字节。
- **实现**：与 `lastParserPhaseOpcode` 同一条扫描骨架，只是记录的是 `tail_start`：每遇到一条既不是 `line_num`、也不是 `leave_scope` / `close_loc` 的真指令，就把 `tail_start` 设成它的结束位置 `pc + size`。扫完返回 `tail_start`——末条是真指令时它就等于 `code.len`。指令长度为 0 或越界时返回 0，使调用方的「是否有跳到末尾」答案保守地退化为「当作跳到末尾」。
- **所有权 / 错误 / 调用**：无：纯函数，参数 `code`/`atoms` 是借来的只读 slice，不分配、不改写、无 error set（形状异常时靠返回值保守退化而不是报错）；读 `atoms[...]` 只是 id 比对，编译期 atom 由 `CompileAtomScope` 统一作根，没有引用计数动作。畸形代码（指令长度 0 或越界）返回 0，使调用方的「是否有跳到末尾」答案保守退化为「当作跳到末尾」。树内唯一调用方 `hasJumpToCurrentEnd`（`:8075`）。

### `hasJumpToCurrentEnd` (`src/parser.zig:8074`)

- **签名**：`pub fn hasJumpToCurrentEnd(code: []const u8, atoms: []const Atom) bool`。
- **作用**：谓词：扫一遍已发射的指令流，看有没有跳转目标落在当前末尾（含尾部清理串）。
- **实现**：先用 `trailingCleanupStart` 求出 `tail_start`，再从头线性扫：每条指令用 `parserPhaseInstruction` 定长（长度 0 或越界时直接 `return true` 保守处理），若 `parserPhaseLabelOperandOffset` 给出 label 操作数偏移，就小端读出 4 字节目标，`target >= tail_start` 即判定有跳到当前末尾的边、`return true`；带 atom 的指令推进 `atom_index`。全扫完没命中返回 false。doc 注释说明为何把尾部清理串也算进「末尾」：这些尾随指令要么在 lowering 时消失（`line_num`、`leave_scope`、未被捕获的 `close_loc`，于是解析后的目标就变成 `code_end`），要么执行完就掉出末尾；而寄存器常驻的分派与 qjs 一样没有逐条的越界检查，所以这类跳转必须落在尾声补出的真终结指令上（qjs 的形状是 `js_is_live_code` 之后的 `emit_return`）。
- **所有权 / 错误 / 调用**：无：纯函数，参数 `code`/`atoms` 是借来的只读 slice，不分配、不改写、无 error set（形状异常时靠返回值保守退化而不是报错）；读 `atoms[...]` 只是 id 比对，编译期 atom 由 `CompileAtomScope` 统一作根，没有引用计数动作。越界/畸形一律 `return true`（保守）。⚠️ 虽是 `pub`，**全仓零调用方**（`src/` 与测试树里都只有定义与注释提及）：现役的活代码判定走摘要驱动的 `isLiveCode`（`:7984`），它读 Builder 的 `label_slots` 入边而不是重扫字节流。

### `functionNeedsImplicitReturn` (`src/parser.zig:8102`)

- **签名**：`pub fn functionNeedsImplicitReturn(code: []const u8, atoms: []const Atom) bool`。
- **作用**：谓词：函数体尾部是否还要补一条隐式 return——取 `lastParserPhaseOpcode(code, atoms, true)`（跳过 `leave_scope`/`close_loc` 清理码），末条是 `return`/`return_undef`/`return_async`/`tail_call`/`tail_call_method`/`throw` 就不用补。
- **实现**：`lastParserPhaseOpcode(code, atoms, true)` 取最后一条非清理指令（空代码返回 `null` → 需要隐式 return）。若它是 `return` / `return_undef` / `return_async` / `tail_call` / `tail_call_method` / `throw` 之一，说明直线路径已经终结，返回 false；其余返回 true。这只是直线可达性；「有跳转落到末尾」那一路由调用方另外用 `hasJumpToCurrentEnd` 或上判定——末条虽是终结码，构造收尾的跳转（if/else 分支、break）仍可能落到这里，那条路径同样需要一个着陆的终结指令。
- **所有权 / 错误 / 调用**：**树内零调用方**（唯一命中是 `:8101` 的注释提到它）：现役路径是摘要驱动的 `isLiveCode`。函数本身也无所有权：参数是借来的 `code`/`atoms` 两个 slice，不分配、无 error set，只把 `lastParserPhaseOpcode` 的结果对六个终结 opcode 做查表。

### `patchContinueFrame` (`src/parser.zig:8116`)
...
### `popBreakFrameAndPatch` (`src/parser.zig:8121`)
...
### `popBreakOnlyFrameAndPatch` (`src/parser.zig:8138`)
...
### `emitCreateUsingDisposableStack` (`src/parser.zig:8392`)
...
### `emitUsingAwait` (`src/parser.zig:8401`)
...
### `emitUsingAddResource` (`src/parser.zig:8409`)
...
### `emitUsingAwaitIfNeeded` (`src/parser.zig:8417`)
...
### `emitUsingDisposeStack` (`src/parser.zig:8430`)
...
### `emitUsingDisposeStackForThrow` (`src/parser.zig:8439`)
...
### `armCurrentUsingBlockFrame` (`src/parser.zig:8449`)
...
### `noteUsingResourceHint` (`src/parser.zig:8469`)
...
### `finalizeCurrentUsingBlockFrame` (`src/parser.zig:8476`)
...
### `restoreUsingBlockFramesAfterError` (`src/parser.zig:8502`)
...
### `emitReturnValue` (`src/parser.zig:10167`)
...
- **实现**：值已在栈顶时发出完整的 return 清理链。①`await_before_unwind` 为真先发 `op.await`——异步生成器的显式返回值要在展开之前 await（qjs `emit_return`，quickjs.c:28401-28405）。②用两个可变游标 `block_cursor = s.top_break`、`catch_marker_depth = s.active_catch_marker_depth` 从内到外遍历 `return_finally_frames`（倒序）：每一帧先 `emitBlockEnvReturnCleanupUntil` 把迭代器记录展开到该帧的 `block_boundary`，再 `emitStackTopCatchMarkerDropsToDepth` 把 catch 标记降到该帧深度，最后按 `frame.finally_label` 是 legacy `.temp` 还是 `.builder` 分别用 `emitParserLabelJumpNoSource` / `emitterJumpNoSource` 发一条 `gosub`，逐个执行跨过的 finally（quickjs.c:28447-28449）。③帧走完后再做一次到底的清理：`emitBlockEnvReturnCleanupUntil(..., null, ...)` 与 `emitStackTopCatchMarkerDropsToDepth(..., 0)`。④`emitFunctionReturn(s, true)` 收尾。可变游标的作用是保证每个迭代器 / catch 记录只展开一次，而所有活跃的 finalizer 共用同一个 gosub 目标。
- **所有权 / 错误 / 调用**：不分配：只读 `s.return_finally_frames` / `s.top_break` 两个栈并逐层发清理码，不弹栈也不改帧（`block_cursor` 与 `catch_marker_depth` 是栈上的游标副本，按指针传给两个 helper 更新）。`finally_label` 的 `.temp` 臂走 `emitParserLabelJumpNoSource`（phase-1 raw 流），但该臂在树内从未被构造，实际只走 `.builder`。错误来自被调的清理 helper 与 Builder 经 `mapBuilderError` 的折叠。调用方 3 处：`parseUnary`（`:4958`，`yield` 的 return 完成）、`emitYieldStarDelegation`（`:5069`）、`emitParsedReturn`（`:10288`）。

### `restoreSourceLoc` (`src/parser.zig:10206`)
...
- **实现**：按 `UpdatedSourceLoc` 的两个变体分派：`.temp` 时按 `emit_to_function_def` 选 `curFunc().source_loc_slots` 或 `self.function.source_loc_slots`，断言下标合法后写回 `previous`；`.builder` 时取 `activeBuilder()`，断言 `index < source_len` 后写回 `builder.source_slots[index]`。只改这一个槽，不动长度。
- **所有权 / 错误 / 调用**：不分配、无 error set：把保存下来的 `SourceSlot` 原样写回 `Builder.source_slots[index]`（或 `.temp` 臂的 `source_loc_slots`）。`.temp` 臂**在树内从未被构造**——唯一的生产者 `reattributeReturnTailCallSource` 只返回 `.builder`，所以那条臂是遗留后端的死分支。唯一调用方 `parseReturnStatement`（`src/parser.zig:9066`）的 `errdefer`。

### `reattributeReturnTailCallSource` (`src/parser.zig:10224`)
...
### `emitParsedReturn` (`src/parser.zig:10272`)
...
### `emitFunctionReturn` (`src/parser.zig:10291`)
...
### `emitDirectPatternPut` (`src/parser.zig:12738`)
...
### `putPatternTarget` (`src/parser.zig:12750`)
...
### `parsePatternDefault` (`src/parser.zig:12757`)
...
### `rotateNamedSourcePastTarget` (`src/parser.zig:12773`)
...
### `rotateComputedSourcePastTarget` (`src/parser.zig:12783`)
...
### `addNamedObjectRestExclusion` (`src/parser.zig:12796`)
...
### `addComputedObjectRestExclusion` (`src/parser.zig:12803`)
...
### `objectRestCopyMask` (`src/parser.zig:12811`)
...
### `emitArrayPatternRest` (`src/parser.zig:12820`)
...
### `pushPatternIteratorBlock` (`src/parser.zig:12838`)
...
### `popPatternIteratorBlock` (`src/parser.zig:12854`)
...
### `emitStackTopCatchMarkerDropsToDepth` (`src/parser.zig:12862`)
...
### `emitBlockEnvReturnCleanupUntil` (`src/parser.zig:12875`)
...
### `setObjectName` (`src/parser.zig:13606`)
...
### `setObjectNameComputed` (`src/parser.zig:13647`)
...
### `emitAnonymousDefaultName` (`src/parser.zig:13675`)
...
### `parseClassElementFunction` (`src/parser.zig:14434`)
...
### `parseClassComputedName` (`src/parser.zig:14473`)
...
### `emitStaticClassComputedElement` (`src/parser.zig:14482`)
...
### `emitInstanceComputedPublicFieldInitializer` (`src/parser.zig:14517`)
...
### `emitInstanceClassComputedElement` (`src/parser.zig:14521`)
...
### `emitClassComputedMethod` (`src/parser.zig:14544`)
...
### `emitClassStaticBlock` (`src/parser.zig:14563`)
...
### `parseClassBodyAfterOpen` (`src/parser.zig:14589`)
...
### `collectClassPrivateBoundNames` (`src/parser.zig:14601`)
...
- **所有权 / 错误 / 调用**：**预扫描**：用 `takeLexerCursorSnapshot`/`restoreLexerCursorSnapshot`（`src/parser.zig:13256`/`:13268`）把 lexer 光标借出去再还回来，`defer` 保证任何错误路径都复位；扫描里每个 `scan_token` 都由 `defer s.lex.freeToken` 释放 payload（释放的是 token 的字符串缓冲，不是 atom）。真正的分配在 `registerClassPrivateBoundName`（`:14354`）→ `appendRetainedAtom`（`:4628`）往 `s.class_private_bound_names` 追加（缓冲归 `State`，`State.deinit` 释放）——那个函数名里的 retain 同样是 TGC S3-c 之前的遗迹，现在只 append id；`privateNameDeclarationAtom`（`:14387`）可能经 `newClassPrivateAtom` 新建一个 `.private` symbol atom，同样由 `CompileAtomScope` 作根。错误：`OutOfMemory`（append / newSymbol）、lexer 的 `SyntaxError` 族、`Error.InvalidIdentifier`（atom 无名）。唯一调用方 `parseClass`（`:14826`），调用前后靠 `bound_start` 游标划分本类的私有名区间。

### `emitClassLocalInitFromClassStack` (`src/parser.zig:14660`)
...
### `emitClassFieldsInitValue` (`src/parser.zig:14669`)
...
### `emitClassFieldsInitLocalInitFromClassStack` (`src/parser.zig:14686`)
...
### `emitClassStaticInitCall` (`src/parser.zig:14693`)
...
### `emitClassPrivateBrands` (`src/parser.zig:14716`)
...
### `emitClassDefineOperands` (`src/parser.zig:14736`)
...
### `appendClassFieldInitCallToFunctionDef` (`src/parser.zig:15030`)
...
- **实现**：先按 `fd.is_derived_class_constructor` 选读 `this` 的 opcode：派生构造器用 `get_loc_check`（它的 `this` 是词法绑定、要 TDZ 检查），基类默认构造器用普通 `get_loc`。然后按有没有 v2 Builder 分两条等价路径。①`fd.v2_builder != null`：依次 `emitAtomOpU16Owned(scope_get_var, atom_class_fields_init, fd.scope_level)`、`emitOp(dup)`、`newLabel()` 得到 `skip`、`emitJump(if_false, skip)`、`emitOpU16(this_read_op, this_idx)`、`emitOp(swap)`、`emitCallOp(call_method, 0)`、`bindLabel(skip)` + `invalidateLastOpcode()`、`emitOp(drop)`；所有 builder 错误经 `mapBuilderError`。②没有 Builder 时手工拼一段 22 字节定长代码：`scope_get_var`(atom+scope) / `dup` / `if_false` 目标写死成 `base + 21`（那条 `drop` 的绝对偏移）/ `this_read_op`+`this_idx` / `swap` / `call_method` argc=0 + cache_idx 占位 0 / `drop`，然后 `appendAtomOperand(atom_class_fields_init)`、把 `fd.flow_tail.valid` 置假（整段是多指令追加，摘要无法增量维护）、`appendByteCode(&code)`。注释解释了为何这里全用长形式：qjs `emit_class_field_init`（quickjs.c:25184-25207）用 phase-1 的 `scope_get_var` 读 `this`，而 `resolve_scope_var` 只有在绑定是词法的时候才把它降成 `get_loc_check`——`add_var_this`（quickjs.c:32834-32845）只对派生构造器这么做；基类默认构造器的 `this` 是普通 var，qjs 用 `get_loc` 读、再由 `resolve_labels` 缩成 `get_loc0`。这里短槽号落在 phase-1 临时码的重叠区间里不能用，所以连 `if_false` 也发长形式：`resolve_labels` 会重映射它的绝对目标并在 `get_loc` 缩短之后重新缩短这条跳转，而裸的 `if_false8` 相对操作数永远不会被重映射，因此绝不能让它跨越尺寸会变的指令。
- **所有权 / 错误 / 调用**：⚠️ 签名里没有 `*State`：它直接往传入的 `fd` 上发射，**两条后端臂并存**——`fd.v2_builder != null` 走 Builder（`emitAtomOpU16Owned` + `newLabel`/`bindLabel` + `invalidateLastOpcode`，错误经 `mapBuilderError`），否则手写 22 字节 code 数组写进 phase-1 raw 流（`fd.appendAtomOperand` + `fd.appendByteCode`，并把 `fd.flow_tail.valid = false`；那条 `if_false` 的绝对目标 `base + 21` 是手算的）。raw 臂在生产中不可达——`appendDefaultClassConstructor` 在调用它之前必定 `ensureBuilderForFd`。`atom_class_fields_init` 是预定义 atom（借用），`this_idx` 是借用槽号。失败即 `OutOfMemory` / Builder 折叠出的错误，没有局部回滚。调用方 2 处，都在 `appendDefaultClassConstructor`（`:15138` 派生、`:15150` 基类）。

### `appendDefaultClassConstructor` (`src/parser.zig:15086`)
...
- 清单函数数（本文件分到）: 221（`src/parser.zig` 全文件 649）
- 本文标题覆盖: 221
- 未覆盖: 无
