...
| 文件 | 职责 |
| --- | --- |
| `unicode.zig` | 表加载、属性名、点查、分类、大小写、NFC/NFD、`CharRange` |
...
## `src/libs/unicode.zig`：表怎么来、谁消费

表来自 QuickJS `libunicode-table.h`，载体是 `unicode_tables.bin`（`@embedFile`）。**不要**手改表体。查找仍走同一套 QuickJS RLE 解码（区间构造与点查各一份）。

`unicode_tables.bin`：`ZJSU` + u32le version=1 + u32le 表个数，随后每表 16 字节 `{name_off, data_off, n_elem, elem_size}`，再是 NUL 名和 4 字节对齐的 payload。`u8` 表是 blob 上的切片；`u16`/`u32` 在 comptime 按小端解成原生整数。编码仍是 QuickJS 的 RLE/索引，查找必须走同一套解码（`isInTable`、`unicodeProp1`、`unicodeGeneralCategory1`、`unicodeScript`）。
...
### `blobU32` (`src/libs/unicode.zig:11`)
...
### `tableBytes` (`src/libs/unicode.zig:15`)
...
### `tableInts` (`src/libs/unicode.zig:39`)
...
### `GC.count` (`src/libs/unicode.zig:171`)

- **签名**：`pub fn count() usize`。
- **作用**：GC 标签个数，给 comptime 名字表校验。
- **实现**：`@typeInfo(@This()).@"enum".fields.len`。
- **所有权 / 错误 / 调用**：同文件 `countNameGroupsComptime` 比较。

### `Script.count` (`src/libs/unicode.zig:354`)
...
### `SequenceProp.count` (`src/libs/unicode.zig:451`)

- **签名**：`pub fn count() usize`。
- **作用**：序列属性个数。
- **实现**：枚举字段数。
- **所有权 / 错误 / 调用**：同文件 comptime 块拿它比 `unicode_sequence_prop_name_table` 的组数，并传给 `validateNameTable` 作越界上界。

### `propTable` (`src/libs/unicode.zig:470`)
...
- **所有权 / 错误 / 调用**：`unicodeProp1`、`isSupported`。borrowed 静态数据。

---

名字表格式：每组 `Alias,Alias2\0`，表尾多一个 `0`。comptime 解析成 `(alias, group_index)`。`propIndex` 把表内下标加上 `prop_public_first`。文件尾 comptime 断言组数、无空别名、无冲突、首个别名等于枚举字段名。随后是派生 `Op` 程序和零分配点查。

### `parseNameTableComptime` (`src/libs/unicode.zig:490`)
...
### `countNameGroupsComptime` (`src/libs/unicode.zig:519`)
...
### `validateNameTable` (`src/libs/unicode.zig:534`)
...
### `firstAliasMatchesEnumField` (`src/libs/unicode.zig:552`)
...
### `findName` (`src/libs/unicode.zig:573`)
...
### `isScriptPropertyName` (`src/libs/unicode.zig:591`)
...
### `isScriptExtensionsPropertyName` (`src/libs/unicode.zig:595`)
...
### `isGeneralCategoryPropertyName` (`src/libs/unicode.zig:599`)
...
### `scriptIndex` (`src/libs/unicode.zig:603`)

- **签名**：`pub fn scriptIndex(script_name: []const u8) ?Script`。
- **作用**：`Greek`/`Grek`/`Zzzz` → 枚举。
- **实现**：`findName(unicode_script_name_table)` 再 `@enumFromInt`。
- **所有权 / 错误 / 调用**：未知 null。

### `gcIndex` (`src/libs/unicode.zig:608`)

- **签名**：`pub fn gcIndex(gc_name: []const u8) ?GC`。
- **作用**：`Lu`/`Uppercase_Letter` → GC。
- **实现**：`unicode_gc_name_table`。
- **所有权 / 错误 / 调用**：组名 `L` 也在表里。

### `propIndex` (`src/libs/unicode.zig:613`)

- **签名**：`pub fn propIndex(prop_name: []const u8) ?Prop`。
- **作用**：公共属性名 → `Prop`。
- **实现**：表下标 **加上** `prop_public_first`（跳过内部 Hyphen…）。
- **所有权 / 错误 / 调用**：`ID_Compat_Math_Start` 能解析但未必 `isSupported`。

### `sequencePropIndex` (`src/libs/unicode.zig:618`)

- **签名**：`pub fn sequencePropIndex(prop_name: []const u8) ?SequenceProp`。
- **作用**：`Basic_Emoji` / `RGI_Emoji` 等。
- **实现**：`unicode_sequence_prop_name_table`。
- **所有权 / 错误 / 调用**：`addSequenceProperty`、LRE `\p` of strings。

### `gcBit` (`src/libs/unicode.zig:623`)

- **签名**：`pub fn gcBit(comptime name: []const u8) u32`。
- **作用**：`1 << GC.name`。
- **实现**：comptime `@field(GC, name)`。
- **所有权 / 错误 / 调用**：派生 `Op` 与 `gc_mask_table`。

### `gcMaskByIndex` (`src/libs/unicode.zig:638`)

- **签名**：`pub fn gcMaskByIndex(gc: GC) u32`。
- **作用**：单类一位；组类（`LC` 起）用 `gc_mask_table` 或起来。
- **实现**：`gc_idx < LC` 则单 bit，否则表 `[gc_idx - LC]`。
- **所有权 / 错误 / 调用**：`\p{L}` 要匹配所有 Letter。

### `parsePropertyExpression` (`src/libs/unicode.zig:655`)
...
### `derived` (`src/libs/unicode.zig:873`)

- **签名**：`pub fn derived(prop: Prop) ?Derived`。
- **作用**：若属性由其它集合算出来，返回表达式。
- **实现**：switch：ASCII/Any 特殊；其余返回静态 `ops` 切片；else null（走 `propTable`）。
- **所有权 / 错误 / 调用**：切片是 `const`，无分配。同文件 `unicodeProp` 区间构造。

### `isSupported` (`src/libs/unicode.zig:898`)

- **签名**：`pub fn isSupported(prop: Prop) bool`。
- **作用**：LRE 能否实现该 `\p` 名。
...
与后面的区间构造**同一套解码**，但目标是 `bool` 而不是 `CharRange`。栈上最多 4 个 bool。`isSupportedUnicodePropertyExpression` 给 `\p{…}` 校验器用；per-code-point 的 `isUnicodePropertyMatches` 目前只有本文件测试夹具调用；编译 `\p` 仍走 `propertyRangePoints`。

### `matchGeneralCategory` (`src/libs/unicode.zig:905`)

- **签名**：`fn matchGeneralCategory(code_point: u21, gc_mask: u32) bool`。
- **作用**：码点是否落在 GC 掩码里。
- **实现**：扫 `unicode_gc_table`：高 3 位长度（7 则变长扩展），低 5 位类别。命中区间：`v==31` 表示 Lu/Ll 交错，按掩码奇偶；否则测 bit。
- **所有权 / 错误 / 调用**：无分配。未命中 false。

### `matchPropTable` (`src/libs/unicode.zig:958`)

- **签名**：`fn matchPropTable(code_point: u21, prop: Prop) bool`。
- **作用**：binary property 成员。
- **实现**：`propTable` null → false。RLE：`b<64` 两段短 run 并翻转 bit；`>=0x80` 长 skip；`0x40/0x60` 多字节长度。
- **所有权 / 错误 / 调用**：与后文 `unicodeProp1` 区间构造对偶。

### `matchCaseMask` (`src/libs/unicode.zig:988`)

- **签名**：`fn matchCaseMask(code_point: u21, case_mask: u32) bool`。
...
### `matchPropOps` (`src/libs/unicode.zig:1033`)

- **签名**：`fn matchPropOps(code_point: u21, ops: []const Op) bool`。
- **作用**：在 bool 栈上执行派生 `Op`。
- **实现**：`[4]bool`；union/inter/xor/invert 弹栈。末尾 `assert(stack_len==1)`。
- **所有权 / 错误 / 调用**：ops 来自 `derived`。

### `matchProp` (`src/libs/unicode.zig:1071`)

- **签名**：`fn matchProp(code_point: u21, prop: Prop) bool`。
- **作用**：派生或表驱动的属性测试。
- **实现**：`.ascii` → `<0x80`；`.any` → `<0x110000`；`.ops` → `matchPropOps`；否则 `matchPropTable`。
- **所有权 / 错误 / 调用**：`isUnicodePropertyMatches`。

### `isSupportedProperty` (`src/libs/unicode.zig:1083`)

- **签名**：`fn isSupportedProperty(prop: Prop) bool`。
- **作用**：转同文件 `isSupported`。
- **实现**：一行。
- **所有权 / 错误 / 调用**：公共入口过滤。

### `matchScript` (`src/libs/unicode.zig:1087`)

- **签名**：`fn matchScript(code_point: u21, script_idx: Script, is_ext: bool) bool`。
...
### `isSupportedUnicodePropertyExpression` (`src/libs/unicode.zig:1192`)
...
### `isUnicodePropertyMatches` (`src/libs/unicode.zig:1199`)

- **签名**：`pub fn isUnicodePropertyMatches(code_point: u21, name: []const u8) bool`。
- **作用**：单码点是否匹配属性表达式。
- **实现**：script → `matchScript`；gc → `matchGeneralCategory`；prop → 支持且 `matchProp`。
- **所有权 / 错误 / 调用**：零分配。当前只有 `unicode.zig` 的测试夹具调用，对照 `propertyRangePoints` 建出的区间。

---

分类、规范化与区间构造：`Category`、`CaseMapping`（最多 3 个码点）、`NormalizationForm`、`CodePointRange`、`SurrogatePair`、`CharRange`（点对 `lo,hi,lo,hi,…` 半开）、`CharRangePointRange`。`char_range_sentinel = u32::MAX` 当宇宙上界。`UnicodeError = Allocator.Error || InvalidProperty`。

ASCII 谓词给 lexer/parser/regexp；非 ASCII 标识符走 `ID_Start`/`ID_Continue1` 表。规范化走 QuickJS 分解表 + 韩文算法。

### `asciiCategory` (`src/libs/unicode.zig:1270`)
...
### `isIdentifierStart` (`src/libs/unicode.zig:1324`)
...
### `isIdentifierContinue` (`src/libs/unicode.zig:1332`)
...
### `caseConvert` (`src/libs/unicode.zig:1341`)
...
### `regexpCanonicalize` (`src/libs/unicode.zig:1348`)
...
### `isCased` (`src/libs/unicode.zig:1364`)
...
### `isEcmaLineTerminatorCodePoint` (`src/libs/unicode.zig:1373`)
...
### `isEcmaLineTerminatorUnit` (`src/libs/unicode.zig:1377`)
...
### `isEcmaWhitespaceOrLineTerminatorCodePoint` (`src/libs/unicode.zig:1381`)
...
### `isEcmaWhitespaceOrLineTerminatorUnit` (`src/libs/unicode.zig:1388`)
...
### `isAsciiDigitUnit` (`src/libs/unicode.zig:1396`)
...
### `isAsciiWordUnit` (`src/libs/unicode.zig:1400`)
...
### `isAsciiIdentifierStartByte` (`src/libs/unicode.zig:1404`)
...
### `isAsciiIdentifierPartByte` (`src/libs/unicode.zig:1408`)
...
### `asciiRadixDigitValueByte` (`src/libs/unicode.zig:1412`)
...
### `isAsciiBinaryDigitByte` (`src/libs/unicode.zig:1421`)
...
### `isAsciiOctalDigitByte` (`src/libs/unicode.zig:1425`)
...
### `isAsciiDigitByte` (`src/libs/unicode.zig:1429`)
...
### `isAsciiUpperByte` (`src/libs/unicode.zig:1433`)

- **签名**：`pub fn isAsciiUpperByte(byte: u8) bool`。
- **作用**：A–Z。
- **实现**：转码点谓词。
- **所有权 / 错误 / 调用**：不分配、无 error。树内唯一调用方是同文件 `isAsciiAlphaByte`（`src/libs/unicode.zig:190`），其余为单测。

### `isAsciiLowerByte` (`src/libs/unicode.zig:1437`)
...
### `isAsciiAlphaByte` (`src/libs/unicode.zig:1441`)
...
### `isAsciiAlphanumericByte` (`src/libs/unicode.zig:1445`)
...
### `isAsciiWordByte` (`src/libs/unicode.zig:1449`)
...
### `asciiHexDigitValueByte` (`src/libs/unicode.zig:1453`)
...
### `asciiUpperHexDigitValueByte` (`src/libs/unicode.zig:1459`)
...
### `asciiLowerHexDigitChar` (`src/libs/unicode.zig:1465`)
...
### `asciiUpperHexDigitChar` (`src/libs/unicode.zig:1470`)
...
### `isAsciiHexDigitByte` (`src/libs/unicode.zig:1475`)
...
### `asciiHexDigitValueUnit` (`src/libs/unicode.zig:1479`)
...
### `isAsciiHexDigitUnit` (`src/libs/unicode.zig:1484`)
...
### `isAsciiDigitCodePoint` (`src/libs/unicode.zig:1488`)
...
### `isAsciiUpperCodePoint` (`src/libs/unicode.zig:1492`)
...
### `isAsciiLowerCodePoint` (`src/libs/unicode.zig:1496`)
...
### `isAsciiAlphaCodePoint` (`src/libs/unicode.zig:1500`)
...
### `isAsciiAlphanumericCodePoint` (`src/libs/unicode.zig:1504`)
...
### `isAsciiWordCodePoint` (`src/libs/unicode.zig:1508`)
...
### `isHighSurrogateUnit` (`src/libs/unicode.zig:1512`)
...
### `isLowSurrogateUnit` (`src/libs/unicode.zig:1516`)
...
### `isHighSurrogateCodePoint` (`src/libs/unicode.zig:1520`)
...
### `codePointFromSurrogatePair` (`src/libs/unicode.zig:1532`)
...
### `surrogatePairFromCodePoint` (`src/libs/unicode.zig:1536`)
...
### `appendUtf16CodePoint` (`src/libs/unicode.zig:1544`)
...
### `appendUtf8CodePoint` (`src/libs/unicode.zig:1554`)
...
### `appendUtf16UnitsAsUtf8` (`src/libs/unicode.zig:1572`)
...
### `normalizeAlloc` (`src/libs/unicode.zig:1589`)
...
### `propertyRangePoints` (`src/libs/unicode.zig:1647`)
...
### `propertyRangeSet` (`src/libs/unicode.zig:1660`)
...
### `toUpperAscii` (`src/libs/unicode.zig:1680`)
...
### `toLowerAscii` (`src/libs/unicode.zig:1685`)
...
### `equalsIgnoreAsciiCase` (`src/libs/unicode.zig:1690`)
...
### `caseConv1` (`src/libs/unicode.zig:1723`)
...
### `caseConv` (`src/libs/unicode.zig:1727`)
...
### `caseConvEntry` (`src/libs/unicode.zig:1741`)
...
### `caseFoldingEntry` (`src/libs/unicode.zig:1819`)
...
### `findCaseEntry` (`src/libs/unicode.zig:1841`)
...
### `getLe24` (`src/libs/unicode.zig:1860`)
...
### `getIndexPosition` (`src/libs/unicode.zig:1869`)
...
### `isInTable` (`src/libs/unicode.zig:1898`)
...
### `shortCode` (`src/libs/unicode.zig:1964`)
...
### `lowerSimple` (`src/libs/unicode.zig:1976`)
...
### `get16` (`src/libs/unicode.zig:1986`)
...
### `decompEntry` (`src/libs/unicode.zig:1990`)
...
### `decompTypeI` (`src/libs/unicode.zig:2076`)
...
### `decompTypeB` (`src/libs/unicode.zig:2085`)
...
### `decompChar` (`src/libs/unicode.zig:2100`)
...
### `unicodeComposePair` (`src/libs/unicode.zig:2122`)
...
### `combiningClass` (`src/libs/unicode.zig:2150`)
...
### `sortCanonicalCombiningClass` (`src/libs/unicode.zig:2183`)
...
### `toNfdRec` (`src/libs/unicode.zig:2208`)
...
### `composePair` (`src/libs/unicode.zig:2229`)
...
### `CharRange.init` (`src/libs/unicode.zig:2248`)
...
### `CharRange.deinit` (`src/libs/unicode.zig:2252`)
...
### `CharRange.items` (`src/libs/unicode.zig:2256`)
...
### `CharRange.addInterval` (`src/libs/unicode.zig:2260`)
...
### `CharRange.appendPoints` (`src/libs/unicode.zig:2267`)
...
### `CharRange.addSet` (`src/libs/unicode.zig:2271`)
...
### `CharRange.normalize` (`src/libs/unicode.zig:2275`)
...
### `CharRange.compress` (`src/libs/unicode.zig:2279`)
...
### `CharRange.compressAdjacent` (`src/libs/unicode.zig:2283`)
...
### `CharRange.sortAndRemoveOverlap` (`src/libs/unicode.zig:2304`)
...
### `CharRange.compressOverlapping` (`src/libs/unicode.zig:2309`)
...
### `CharRange.unionWith` (`src/libs/unicode.zig:2332`)
...
### `CharRange.intersectWith` (`src/libs/unicode.zig:2336`)
...
### `CharRange.xorWith` (`src/libs/unicode.zig:2340`)
...
### `CharRange.subWith` (`src/libs/unicode.zig:2344`)
...
### `CharRange.opWith` (`src/libs/unicode.zig:2355`)
...
### `CharRange.makeOp` (`src/libs/unicode.zig:2362`)
...
### `CharRange.invert` (`src/libs/unicode.zig:2414`)
...
### `CharRange.regexpCanonicalize` (`src/libs/unicode.zig:2423`)
...
### `CharRange.rangeCount` (`src/libs/unicode.zig:2481`)
...
### `CharRange.rangeAt` (`src/libs/unicode.zig:2485`)
...
### `CharRange.isEmpty` (`src/libs/unicode.zig:2493`)
...
### `CharRange.lastHi` (`src/libs/unicode.zig:2497`)
...
### `insertionSortPointPairs` (`src/libs/unicode.zig:2504`)
...
### `generalCategory` (`src/libs/unicode.zig:2519`)
...
### `scriptRanges` (`src/libs/unicode.zig:2725`)
...
### `addSequenceProperty` (`src/libs/unicode.zig:2804`)
...
### `isSequencePropertyName` (`src/libs/unicode.zig:2816`)
...
### `sequenceProp1` (`src/libs/unicode.zig:2822`)

- **签名**：`fn sequenceProp1( allocator: std.mem.Allocator, comptime Context: type, ctx: *Context, prop: SequenceProp, comptime callback: fn (*Context, []const u21) std.mem.Allocator.Error!void, ) UnicodeError!void`。
...
### `emitPropertySequences` (`src/libs/unicode.zig:2905`)

- **签名**：`fn emitPropertySequences( allocator: std.mem.Allocator, comptime Context: type, ctx: *Context, prop: Prop, comptime suffix: []const u21, comptime callback: fn (*Context, []const u21) std.mem.Allocator.Error!void, ) UnicodeError!void`。
- **作用**：属性区间每个码点加上 suffix 回调。
- **实现**：`unicodeProp1` + normalize。
- **所有权 / 错误 / 调用**：Basic_Emoji / Keycap。

### `emitZwjSequences` (`src/libs/unicode.zig:2930`)
...
### `rangesContain` (`src/libs/unicode.zig:3378`)
...
### `pointsContain` (`src/libs/unicode.zig:3385`)
...
### `expectRegexpShortcutMatchesRangeBuilder` (`src/libs/unicode.zig:3393`)
...
### `expectRegexpShortcutPointMatchesRanges` (`src/libs/unicode.zig:3413`)
...
- 清单函数数: 147（`src/libs/unicode.zig`）
- 本文标题覆盖: 147
- 未覆盖: 无
