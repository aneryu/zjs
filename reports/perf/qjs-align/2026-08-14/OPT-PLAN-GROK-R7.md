...
## 5. 结果表（2026-08-14 执行完毕；driver CPU19 亲验 case-pure 1.298 / s8 1.007）

| lane | case 比值/宏观比值 | 定位构造 | 机制裁决 |
|---|---|---|---|
| R7-R raytrace | **1.296 / 1.291** | `initialize.apply` + 热路径 `new Color/new Vector`；三刀消融后 **1.002**（三 pad 1.002/1.011/1.009）；成对税 1.00G = 全部超出（z 1.41G / q 0.42G = 3.39x） | **R7-R1 登记（上限 +1.7pp，R8 主线）** + R7-R2 apply 残余 |
| R7-T TS 内层 | 1.201 / 1.215 | 无——去 emit 保持、只 parse 1.228 仍保持，**删到空仍保持比** | 与 R3 JS 级干净一致：弥散每-op 地板 |
| R7-H richards | 1.109 / 1.104 | 无——内联 TCB/addTo/schedule 全保持 | 弥散 OO |
| R7-Z zlib | 1.07-1.09 / 1.073 | 无（emscripten 产物不可再删） | 前端：小工作集可翻优势但 IPC 不单调消失 ⇒ I-cache/布局与间接跳两因并存；喂 R6-F 候选 a |

**头条：四条赤字基准里唯一被提纯成可修构造的是 raytrace——但它就是最差的那个，
且上限 +1.7pp 是登记册最大条目。提纯法的另一产出同样值钱：TS/richards/zlib 的
「删到空仍保持比」把弥散结论从推断升级为构造性证明。**
