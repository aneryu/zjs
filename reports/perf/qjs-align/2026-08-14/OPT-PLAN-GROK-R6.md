...
## 2. Lane R6-F「前端微架构诊断」（**SUPERSEDED → 并入 R7-Z**）

> 2026-08-14：用户指示转向「zoo 提纯 case + 微基准定位」。R6-F 的判决实验
> 由 R7-Z 的提纯 case 天然承担（小工作集 case 的 IPC 差距存亡直接裁决 I-cache 假设）。
> R6-K 独立照跑（改用 CPU 15/16）。以下原文保留作背景。
