...
**v1.5 真门判定（@0d4169ba）：FAIL 0.8895，且结论升级为「2/3 门结构性不可达」——升用户裁决**：
v1.5 冒烟全达标（覆盖 96%、18.2 cyc/take、R-v15 三修订落实、test-exec 452/452），
raw-script v15/del cyc 0.9882；但 EB zoo 真门 **0.8895**（v1 0.9060/裸删 0.9092，三者差在
单 pad 噪声内；对 0.9697 门的失败稳健）。
**结构账（24.3G 窗口修正）**：bypass 价值 ≈**128 cyc/构造**（与 N2 阶梯 +98 吻合；
driver 早前 24 cyc 估算因窗口取 4.5G 而错）；v1+v1.5 合法收回 ~20（16%）；
剩余 ~108 = 构造体走真实 opcode 机器 vs bypass 熔合 writer 之差——
...
N2f/N3f 冒烟 0.906/0.916（接通，+8-9%）。**EB 真门 0.9060 ≈ 裸删 0.9092 = 零回收**；
64MB 预算探针证伪预算假设；driver 探针实测 **PREP 16.8M / SPEC 38 / TAKE 6.3M（37% 覆盖）**
——机制大规模生效但每 take 净省仅 ~2 cyc。
**结构性根因（driver 失察，批简报时就该算出）**：R8 锚点空调用 Δ=+18 而 N0 空构造=+92，
构造大头在**管线**（prepare/建实例/校验/返回判定）不在 callee 调用；v1 只消 +18 段，
回收天花板 ~1/4，2/3 门结构性不可达。
⇒ **v1.5「构造管线熔合」设计简报已下达**（特化点缓存 proto/shape 直建实例+失效协议、
消已证不变量重校验逐条列被消项+deopt、体照常执行——JSC create-this 缓存同族，
通用性原则内）+ 37% 覆盖缺口诊断。包继续不交付。
