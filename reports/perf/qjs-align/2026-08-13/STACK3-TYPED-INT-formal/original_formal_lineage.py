#!/usr/bin/env python3
"""Formal 7-pad Zoo lineage for stack3 + typed-int, with 2x2 cold builds.

This is intentionally stricter than tools/perf/layout_lineage/run_lineage.py:
each side gets two independent local+global cold caches at every pad, all four
binary combinations are measured, and each pad still has eight paired Zoo
samples in total (four combinations x ABBA2).
"""
...
BASE = Path("/home/aneryu/zjs")
CAND = Path("/home/aneryu/worktree-stack-permute-typed-int")
ZOO = Path("/home/aneryu/javascript-zoo")
RUNNER = BASE / "tools/perf/zoo/run_zoo_compare.py"
...
MDE_LOG_PP = 0.278
EXPECTED_HEAD = "e3e8190a2ee50968602dc2f1d5c758379e5c37db"
EXPECTED_CAND_FILES = [
...
def check_sources() -> dict:
    if git_head(BASE) != EXPECTED_HEAD or git_head(CAND) != EXPECTED_HEAD:
        raise RuntimeError("base/candidate HEAD drift")
    if git_status(BASE):
        raise RuntimeError(f"baseline worktree is dirty: {git_status(BASE)}")
    cand_status = git_status(CAND)
    changed = sorted(line[3:] for line in cand_status if line[:2].strip())
    if changed != EXPECTED_CAND_FILES:
        raise RuntimeError(f"candidate scope drift: {changed}")
    zoo_status = git_status(ZOO)
    if zoo_status:
        raise RuntimeError(f"Zoo worktree is dirty: {zoo_status}")
    return {
        ...
        "candidate": {
            ...
            "head": git_head(CAND),
            "status": cand_status,
            "diffSha256": git_diff_sha(CAND),
        },
        "zoo": {"path": str(ZOO), "head": git_head(ZOO), "status": []},
    }


def new_state(source: dict) -> dict:
    return {
        "tool": "zjs-formal-zoo-lineage-2x2",
        "schemaVersion": 1,
        "createdUnix": time.time(),
        "preRegisteredDesign": {
            "pads": PADS,
            "coldBuildsPerSidePerPad": 2,
            "buildCombinationsPerPad": 4,
            "samplesPerSidePerCombination": SAMPLES_PER_COMBO,
            "samplesPerSidePerPad": 4 * SAMPLES_PER_COMBO,
            "cpu": CPU,
            "executionMode": "serial-alternating",
            "effect": "100 * ln(candidate/base causal Zoo throughput geomean)",
            "mdeLogPercentagePoints": MDE_LOG_PP,
            "passRule": "median effect > MDE and worst pad >= 0",
        },
...
def combo_order(pad_index: int) -> list[tuple[str, str]]:
    # The four 2x2 cells form a cycle, balancing reuse of each frozen build.
    ...
    for pad_index, pad in enumerate(PADS):
        for base_inst, cand_inst in combo_order(pad_index):
            ...
            if artifact_valid(out, b["sha256"], c["sha256"]):
                ...
                started = time.time()
                with log.open("w") as f:
                    proc = subprocess.run(cmd, cwd=BASE, stdout=f, stderr=subprocess.STDOUT, text=True)
                if proc.returncode != 0:
