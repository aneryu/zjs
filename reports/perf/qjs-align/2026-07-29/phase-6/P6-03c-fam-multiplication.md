...
### 7.1 peak RSS 上升 128–196 KB

| workload | p0 maxRSS | p1 maxRSS |
|---|---:|---:|
| `local_arith_loop`（非 BigInt 对照） | 4612 KB | **4612 KB** |
| `2×2` | 4676 | 4804 |
| `8×8` | 4672 / 4672 | 4804 / 4868 |
| `16×16` | 4676 / 4672 | 4868 / 4804 |
| `28×28` | 4740 | 4868 |
| `28×29` | 4676 | 4804 |

非 BigInt 对照**零差异**，所以这不是二进制体积的常数偏移，而是 BigInt 负载特有的。
方向对 P1 不利，幅度 +2.7% ~ +4.2%。

与 PRD 预见的一致：融合分配把 GC 触发点移到了整块之前
（`JSRuntime.triggerGCOnAllocation` 出现在 P1 的 profile 里，占 2.10%），
堆在被回收前可以多涨一点。**但这条因果未经确认** —— 也可能是新的块尺寸类
（`2×2` 用 96 B 类而非 64 B + 40 B 两类）引入了额外 arena。
本刀不做进一步归因；工作负载的活跃 BigInt 只有一个乘积，
所以 per-op 的分配节省在 RSS 这个尺度上看不出来。
