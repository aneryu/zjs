...
pub fn addTest262Host(ctx: Ctx, engine: *std.Build.Module, host: *std.Build.Module) *std.Build.Module {
    return ctx.b.createModule(.{
        ...
        .link_libc = true,
        .imports = &.{
            .{ .name = "zjs", .module = engine },
            .{ .name = "zjs_host", .module = host },
        },
    });
}

/// Internal host capabilities sharing the consumer's engine type identity.
pub fn addHost(ctx: Ctx, engine: *std.Build.Module) *std.Build.Module {
    return ctx.b.createModule(.{
        .root_source_file = ctx.b.path("src/host/root.zig"),
        .target = ctx.target,
        .optimize = ctx.optimize,
        .link_libc = true,
